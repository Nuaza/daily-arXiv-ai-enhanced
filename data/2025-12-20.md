...
Motivation: 现有的视觉-语言-动作模型在机器人操作中表现出良好的泛化能力，但主要是反应式和2D中心的，在需要精确3D推理的任务中可靠性不足。

Method: GeoPredict引入两个预测模块：轨迹级模块编码运动历史并预测机器人手臂的多步3D关键点轨迹；预测性3D高斯几何模块沿未来关键点轨迹预测工作空间几何。这些模块仅作为训练时的监督，通过基于深度的渲染实现，推理时只需轻量级查询标记。
...
TL;DR: 本研究评估了YOLOv8不同变体在车牌识别和字符识别任务上的性能，提出了使用YOLOv8 Nano进行车牌检测、YOLOv8 Small进行字符识别的优化流水线，在保持计算效率的同时实现了高精度。


<details>
  <summary>Details</summary>
Motivation: 在交通管理和车辆监控领域，高效的车牌检测和识别至关重要。尽管已有多种方法，但在多样化环境中保持实时准确性仍然是一个挑战。本研究旨在通过评估YOLOv8变体性能，为智能交通系统提供更优解决方案。
...
TL;DR: 本文提出了一种紧凑的图像到文本架构，仅使用单张胸部X光正面图像生成报告发现部分，通过层级的解剖注意力机制提升临床相关区域的关注，在资源有限的情况下实现了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的放射学报告生成系统（如MAIRA-2和MedPaLM-M）依赖于大规模多模态训练、临床元数据和多视角成像，资源密集且对大多数场景不可及。需要开发更紧凑、仅依赖单张图像的方法来降低资源需求。

Method: 结合冻结的DINOv3 ViT编码器和GPT-2解码器，通过层级的解剖注意力机制整合肺部和心脏分割掩码。该机制通过分层高斯平滑将注意力偏向临床相关区域，不增加可训练参数。
...
Conclusion: 尽管模型规模小且仅依赖图像条件，但解码器级的解剖指导改善了空间定位能力，增强了临床相关区域的一致性，证明了紧凑架构在放射学报告生成中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 人类手是我们与物理世界的主要接口，但自我中心感知很少知道何时、何地以及以多大力度接触物体。现有的可穿戴触觉传感器稀少，且没有野外数据集能将第一人称视频与全手触摸对齐。

Method: 创建OpenTouch数据集，包含5.1小时同步的视频-触觉-姿态数据，2,900个精选片段和详细的文本标注。基于此数据集引入检索和分类基准测试，探索触觉如何支撑感知和行动。

Result: 触觉信号为抓握理解提供了紧凑而强大的线索，增强了跨模态对齐，并能从野外视频查询中可靠地检索。展示了触觉在连接视觉感知与物理交互中的重要作用。

Conclusion: 通过发布这个标注的视觉-触觉-姿态数据集和基准测试，旨在推进多模态自我中心感知、具身学习和接触丰富的机器人操作研究。
...
<details>
  <summary>Details</summary>
Motivation: 自动化文本到图像模型评估面临挑战：需要评分模型评估正确性，测试提示需要挑战当前模型但不能挑战评分模型。这些约束可能导致基准随时间漂移，静态基准评分模型无法跟上新模型能力的发展

Method: 1. 分析GenEval基准的漂移问题；2. 通过大规模人类研究验证基准饱和；3. 提出新基准GenEval 2，改进基础视觉概念覆盖和组合性；4. 提出Soft-TIFA评估方法，结合基础视觉概念的判断

Result: GenEval基准已与人类判断严重偏离，绝对误差高达17.7%；GenEval 2基准对当前模型更具挑战性；Soft-TIFA评估方法比VQAScore等整体评分方法更符合人类判断，且更不容易随时间漂移

Conclusion: 基准漂移是文本到图像模型评估的重要问题，需要持续审计和改进。GenEval 2和Soft-TIFA提供了更好的评估方案，但避免基准漂移仍需持续努力
...
<details>
  <summary>Details</summary>
Motivation: 现有基于指令的图像编辑模型在处理指令-视觉复杂性（IV-Complexity）时表现不佳，即当复杂指令遇到杂乱或模糊场景时难以精确执行编辑

Method: 提出RePlan框架，包含视觉语言规划器和扩散编辑器。规划器通过逐步推理分解指令并显式定位到目标区域；编辑器使用免训练的注意力区域注入机制进行并行多区域编辑。采用GRPO强化学习增强规划能力

Result: 在IV-Complex设置下，RePlan显著优于使用更大数据集训练的基线模型，提高了区域精度和整体保真度。还提出了IV-Edit基准测试

Conclusion: RePlan通过规划-执行框架有效解决了复杂指令图像编辑问题，在区域对齐和知识密集型编辑方面表现出色
...
TL;DR: ReMeDI-SAM3是一个无需训练的增强型手术器械分割框架，通过改进记忆机制和重识别模块，在零样本设置下显著提升内窥镜视频中手术器械的分割性能。


<details>
  <summary>Details</summary>
Motivation: 内窥镜视频中的手术器械分割面临遮挡、快速运动、镜面伪影和器械重新进入等挑战。虽然SAM3提供了强大的时空分割框架，但其在手术场景中受到记忆更新机制不区分、固定记忆容量和遮挡后身份恢复能力弱等限制。

Method: 提出了三个核心组件：1) 相关性感知记忆过滤，包含专门的遮挡感知记忆存储遮挡前帧；2) 分段插值方案扩展有效记忆容量；3) 基于特征的重识别模块，通过时间投票实现遮挡后可靠的身份消歧。

Result: 在EndoVis17和EndoVis18数据集上的零样本评估显示，相比原始SAM3分别获得约7%和16%的绝对mcIoU提升，甚至超过了先前基于训练的方法。

Conclusion: ReMeDI-SAM3通过改进记忆机制和重识别能力，有效缓解了错误累积问题，实现了遮挡后的可靠恢复，为手术器械分割提供了强大的零样本解决方案。
...
TL;DR: M-PhyGs方法从视频中估计复杂自然物体（如花朵）的材料组成和物理参数，通过联合分割和参数恢复解决多材料物体物理建模问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法假设物体是单一材料、预先学习动力学或简单拓扑结构，而真实物体（如花朵）通常具有复杂的材料组成和几何结构，需要新的方法来估计多材料物理参数。

Method: 提出Multi-material Physical Gaussians (M-PhyGs)方法，从自然环境中拍摄的短视频中联合分割物体为相似材料并恢复其连续介质力学参数，考虑重力影响。采用级联3D和2D损失函数以及时间小批量处理来提高效率。

Result: 在Phlowers数据集（人们与花朵交互的视频数据集）上的实验结果表明，M-PhyGs及其组件在多材料物理参数估计任务中具有准确性和有效性。

Conclusion: M-PhyGs能够从视频中准确估计复杂自然物体的多材料物理参数，为真实世界物体的物理建模提供了新方法，特别是在材料组成复杂的对象如花朵上表现出色。
...
Method: LinkedOut从原始视频帧中提取语义基础、知识感知的token，使用可提示查询和可选辅助模态引导VLLM。引入跨层知识融合MoE，从丰富的VLLM特征中选择适当的抽象层次，实现个性化、可解释和低延迟推荐。
...
TL;DR: FlashPortrait是一种端到端的视频扩散变换器，能够合成保持身份一致性的无限长度肖像动画视频，同时实现高达6倍的推理加速。
...
Method: 1) 使用现成提取器计算身份无关的面部表情特征；2) 引入标准化面部表情块，通过均值和方差归一化对齐面部特征与扩散潜在空间；3) 采用动态滑动窗口方案和重叠区域加权混合确保平滑过渡；4) 基于特定时间步的潜在变化率和扩散层间的导数幅度比，使用当前时间步的高阶潜在导数直接预测未来时间步的潜在表示，跳过多个去噪步骤。

Result: 在基准测试中，FlashPortrait在定性和定量上都显示出有效性，能够合成身份保持的无限长度视频，同时实现高达6倍的推理加速。

Conclusion: FlashPortrait通过创新的面部特征对齐、动态滑动窗口和高阶潜在导数预测机制，成功解决了长肖像动画中身份一致性和推理效率的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的视频编辑方法通常在简单编辑操作的配对数据上训练，这限制了它们处理多样化和复杂真实世界指令的泛化能力。

Method: 1) 引入VLM-based instructor编码文本指令、视频首帧和可选参考图像为视觉基础指令表示；2) 提出Edit-GRPO后训练阶段，将组相对策略优化应用于视频编辑领域；3) 设计数据构建管道生成多样化的合成配对视频-指令数据。

Result: 实验表明VIVA在指令遵循、泛化能力和编辑质量方面优于现有最先进方法。

Conclusion: VIVA通过VLM引导编码和奖励优化框架，有效提升了基于指令的视频编辑在处理复杂真实世界指令时的泛化能力和编辑质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D手部轨迹预测研究存在两个主要限制：1) 数据集将运动与语义监督解耦；2) 模型对推理和动作的链接较弱。需要同时解决数据和方法层面的问题

Method: 1) 创建EgoMAN数据集：大规模第一人称数据集，包含219K个6DoF轨迹和3M结构化QA对，支持语义、空间和运动推理；2) 提出EgoMAN模型：推理到运动的框架，通过轨迹-标记接口连接视觉语言推理和运动生成，采用渐进式训练对齐推理与运动动态

Result: 该方法能够生成准确且阶段感知的轨迹，并在真实世界场景中展现出良好的泛化能力

Conclusion: 通过结合大规模语义增强数据集和推理到运动的框架，实现了更好的3D手部轨迹预测，解决了现有方法中运动与语义分离、推理与动作弱连接的问题
...
TL;DR: 该论文提出了SceneDiff Benchmark，这是首个带物体实例标注的多视角变化检测基准，以及SceneDiff方法，一种无需训练的多视角物体变化检测方法，在多个基准测试中大幅优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 检测场景中物体在不同时间点的添加、移除或移动对于机器人整理、施工进度监控等应用很重要，但现有方法面临视角变化导致误判的挑战。

Method: 提出SceneDiff方法：1）在3D中对齐捕获场景；2）提取物体区域；3）比较空间和语义区域特征来检测变化。该方法利用预训练的3D、分割和图像编码模型，无需额外训练。

Result: 在SceneDiff Benchmark（包含350个多样化视频对）和其他基准测试上，SceneDiff方法相比现有方法取得了显著提升（相对AP分别提高94%和37.4%）。

Conclusion: 论文提出了首个多视角物体变化检测基准和有效的训练免费方法，为场景变化检测领域提供了重要工具和评估标准。
...
TL;DR: MomaGraph：面向家庭移动机器人的统一场景图表示，包含数据集、评估基准和7B视觉语言模型，在任务规划场景理解上达到SOTA


<details>
  <summary>Details</summary>
Motivation: 家庭移动机器人需要同时导航和操作，现有场景表示方法存在以下问题：1）空间和功能关系分离；2）将场景视为静态快照，缺乏对象状态和时序更新；3）忽略当前任务最相关的信息

Method: 提出MomaGraph统一场景表示，整合空间-功能关系和部件级交互元素。创建MomaGraph-Scenes大规模数据集和MomaGraph-Bench评估套件。基于此开发MomaGraph-R1（7B视觉语言模型），采用强化学习训练，实现任务导向场景图预测和零样本任务规划

Result: 模型在基准测试中达到71.6%准确率（比最佳基线提升11.4%），在开源模型中达到SOTA。在公共基准上表现良好，并能有效迁移到真实机器人实验

Conclusion: MomaGraph为具身智能体提供了统一的场景表示框架，通过数据集、评估基准和模型的三位一体设计，显著提升了家庭环境中机器人的任务理解和规划能力
...
<details>
  <summary>Details</summary>
Motivation: 当前离散分词器在图像重建质量上仍落后于连续分词器，限制了其在多模态系统中的采用。需要开发既能保持离散分词器与自回归范式自然对齐的优势，又能提升重建质量的新方法。

Method: 提出SFTok离散分词器，采用多步迭代机制进行精确重建。核心创新包括：1）自强制引导视觉重建技术；2）去偏与拟合训练策略，解决了多步过程中的训练-推理不一致性问题。

Result: 在每张图像仅使用64个token的高压缩率下，SFTok在ImageNet上达到了最先进的图像重建质量（rFID = 1.21），在类别到图像生成任务中表现出色（gFID = 2.29）。
...
Method: 提出AdaTooler-V模型，采用AT-GRPO强化学习算法，基于每个样本的工具效益分数自适应调整奖励尺度，鼓励模型只在工具能真正带来改进时才调用。构建了两个训练数据集：AdaTooler-V-CoT-100k用于监督微调冷启动，AdaTooler-V-300k用于带有可验证奖励的强化学习训练。
...
<details>
  <summary>Details</summary>
Motivation: 图像编辑技术发展迅速，但视频编辑仍面临一致性、控制和泛化等挑战。研究旨在探索数据、架构和控制的设计空间，开发简单有效的视频编辑框架。

Method: 1) 数据方面：组合现有专家模型构建多样视频对，通过单帧监督将图像编辑对提升为视频，挖掘密集标注的视频片段，添加过渡监督；2) 模型方面：利用预训练文本到视频模型的编辑能力，采用简单的序列拼接条件和轻量LoRA微调；3) 控制方面：通过单一掩码机制统一时空控制，支持可选参考图像。

Result: EasyV2V在视频编辑任务上取得最先进的结果，超越了同期和商业系统，支持灵活输入（视频+文本、视频+掩码+文本、视频+掩码+参考图像+文本）。

Conclusion: EasyV2V通过系统性的数据、架构和控制设计，提供了一个简单而强大的视频编辑框架，解决了视频编辑中的关键挑战，并在多个方面超越了现有方法。
...
TL;DR: AuditDM是一个自动化框架，通过主动发现和纠正多模态大语言模型的失败模式来评估模型能力，使用强化学习训练审计模型生成挑战性问题和反事实图像，揭示模型弱点并作为无标注数据进行修复。
...
Method: 通过强化学习微调一个MLLM作为审计模型，生成能够最大化目标模型之间分歧的挑战性问题和反事实图像，从而发现模型的失败模式。

Result: 在Gemma-3和PaliGemma-2等最先进模型上发现了超过20种不同的失败类型，基于这些发现进行微调后，所有模型在16个基准测试中均有提升，甚至使3B模型超越了其28B对应模型。

Conclusion: 随着数据扩展收益递减，有针对性的模型审计为模型诊断和改进提供了有效途径，AuditDM框架能够主动发现和纠正MLLM的失败模式。
...
TL;DR: 提出NEPA方法，通过预测未来图像块嵌入来训练视觉模型，无需像素重建、离散标记或对比损失，在ImageNet和ADE20K上取得良好效果
...
Method: 提出Next-Embedding Predictive Autoregression (NEPA)，训练模型使用因果掩码和停止梯度来预测未来图像块嵌入，仅以此为目标在ImageNet-1k上预训练Transformer

Result: ViT-B和ViT-L在ImageNet-1K上分别达到83.8%和85.3%的top-1准确率，在ADE20K语义分割任务上也有良好迁移效果
...
TL;DR: 提出Generative Refocusing方法，通过DeblurNet恢复全焦图像和BokehNet生成可控散景，采用半监督训练结合合成配对数据和真实未配对散景图像，在去模糊、散景合成和重对焦任务中达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 景深控制在摄影中至关重要，但获得完美对焦通常需要多次尝试或特殊设备。单图像重对焦仍然困难，涉及恢复清晰内容和创建逼真散景。现有方法存在显著缺陷：需要全焦输入、依赖模拟器生成的合成数据、对光圈控制有限。

Method: 提出Generative Refocusing两步流程：1) DeblurNet从各种输入恢复全焦图像；2) BokehNet生成可控散景。主要创新是半监督训练方法，结合合成配对数据和未配对真实散景图像，利用EXIF元数据捕捉真实光学特性，超越模拟器能力。

Result: 实验表明在去模糊、散景合成和重对焦基准测试中达到最优性能。此外，Generative Refocusing支持文本引导调整和自定义光圈形状。

Conclusion: 提出的Generative Refocusing方法通过创新的半监督训练策略，有效解决了单图像重对焦的挑战，在保持高质量的同时提供了更好的控制灵活性。
...
TL;DR: WorldCanvas是一个多模态可提示世界事件框架，通过结合文本、轨迹和参考图像实现用户导向的丰富模拟，支持多智能体交互、对象进出、参考引导外观等复杂事件生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：纯文本方法表达能力有限，现有轨迹控制的图像到视频方法缺乏语义意图和视觉基础。需要一种能够生成连贯、可控世界事件的方法，支持复杂交互和用户导向的模拟。

Method: 采用多模态方法，结合三种输入：1) 轨迹编码运动、时序和可见性；2) 自然语言表达语义意图；3) 参考图像提供对象身份的视觉基础。这种组合使得能够生成包含多智能体交互、对象进出、参考引导外观和反直觉事件的连贯可控事件。

Result: 生成的视频不仅具有时间连贯性，还表现出涌现一致性，即使在对象暂时消失的情况下也能保持对象身份和场景一致性。WorldCanvas能够支持表达丰富的世界事件生成。

Conclusion: WorldCanvas将世界模型从被动预测器推进为交互式、用户可塑的模拟器，通过多模态输入支持丰富、用户导向的世界事件生成，为可控视频生成提供了新框架。
