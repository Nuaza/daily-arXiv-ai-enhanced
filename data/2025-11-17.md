...
TL;DR: 本文提出了一个分析框架来研究AI能力在有限时间内无限增长的条件，通过物理和信息理论限制定义服务边界，建立内生增长模型来区分超线性和亚临界状态，并提供可实施的测试和控制方法。


<details>
  <summary>Details</summary>
Motivation: 研究AI系统在什么可测量的条件下会出现能力无限增长（失控增长），以及在什么条件下可以排除这种可能性，用可测试的方法替代推测。

Method: 开发递归自我改进的分析框架，将能力增长与资源建设和部署策略联系起来，利用物理和信息理论限制（功率、带宽、内存）定义服务边界，建立内生增长模型耦合资本与计算、数据和能源。

Result: 推导出将可观测序列映射到失控与非奇异行为的决策规则，提供基于改进加速速度相对于当前水平的可证伪测试，以及功率限制、吞吐量节流和评估门等可直接实施的安全控制。

Conclusion: 该框架用可测试条件和可部署控制替代了关于AI奇点的推测，能够认证或排除失控增长的可能性，但依赖于所选能力指标和规律性诊断，未来工作将解决随机动态、多智能体竞争和架构突变等问题。
...
Conclusion: 这项工作代表了数据归因方法在真实世界模型（如Stable Diffusion）上大规模应用的重要进展。
...
TL;DR: 开发并验证了首个基于共识的视频评估工具ColoWorkflow，用于微创结直肠手术的工作流程分析，该工具具有中等程度的评分者间可靠性，可用于标准化培训和人工智能驱动的质量改进。
...
Result: 达成了10个程序无关阶段和34个程序特定步骤的共识，ColoWorkflow应用于54个结直肠手术视频，显示出广泛适用性，平均Cohen's K为0.71（阶段）和0.66（步骤）。

Conclusion: ColoWorkflow是首个基于共识的验证视频评估工具，为视频性能评估建立了可重复框架，支持跨机构基准测试和人工智能工作流程识别发展，有望标准化培训并加速能力获取。
...
Method: 使用Layer-wise Relevance Propagation(LRP)技术进行模型剪枝，在少量数据条件下实现高效剪枝

Result: 相比现有方法，在保持精度方面表现更好，实现了更高的剪枝率

Conclusion: 该方法在数据有限场景下实现了比现有方法更好的精度保持和更高剪枝率，具有更好的实用性
...
TL;DR: 提出了一种短窗口滑动学习框架，用于CCTV视频中的实时暴力检测。该方法将视频分割为1-2秒的短片段，利用LLM自动标注构建细粒度数据集，在RWF-2000上达到95.25%准确率，在UCF-Crime长视频上达到83.25%准确率。
...
Method: 采用短窗口滑动学习框架，将视频分割为1-2秒片段，使用LLM自动标注构建数据集，充分利用所有帧保持时间连续性。

Result: 在RWF-2000数据集上达到95.25%准确率，在UCF-Crime长视频数据集上达到83.25%准确率，表现出强泛化能力和实时适用性。

Conclusion: 该方法在智能监控系统中具有优异的实时暴力检测性能，能够精确识别快速暴力事件，验证了其在实际应用中的有效性。
...
TL;DR: 本文提出了一种竞争性强且计算效率高的概率性降雨临近预报方法，使用预训练卫星视觉编码器结合视频投影器和轻量级概率头，在Weather4Cast 2025基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 开发更高效准确的概率性降雨临近预报方法，提升短期降雨预测的可靠性和计算效率。

Method: 使用预训练的卫星视觉编码器(DINOv3-SAT493M)，结合视频投影器(V-JEPA Vision Transformer)和轻量级概率头，将编码器标记映射到离散经验累积分布函数(eCDF)上，通过连续排序概率得分(CRPS)进行端到端优化。

Result: 在Weather4Cast 2025基准测试中，该方法取得了CRPS为3.5102的成绩，相比最佳3D-UNET基线模型提升了约26%的效果。

Conclusion: 所提出的方法在概率性降雨临近预报任务中表现出色，计算效率高且性能优越，为短期降雨预测提供了有效的解决方案。
...
TL;DR: PhaseWin是一种新颖的相位窗口搜索算法，用于对象级基础模型的可信区域归因，具有近线性复杂度，显著提高了计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有的基于子模子集选择的方法虽然具有高可信度，但计算效率低下，限制了在实际场景中的部署应用。

Method: 采用分阶段粗到细搜索策略，结合自适应剪枝、窗口化细粒度选择和动态监督机制，替代传统的二次成本贪心选择。

Result: 理论上在温和单调子模假设下保持近贪心近似保证；实证中仅使用20%计算预算即可达到贪心归因95%以上的可信度，在对象检测和视觉定位任务中持续优于其他基线方法。
...
TL;DR: 本研究比较了轻量级模型YOLOv11n和YOLOv12n在面部表情识别中的表现，发现YOLOv12n在干净数据集上表现更好，而YOLOv11n在噪声环境下具有更高的精度。
...
Conclusion: 轻量级YOLO模型能有效平衡性能和效率，在敏感性和精度之间存在权衡，适合实时、资源受限的情感AI应用。
...
TL;DR: 本文提出了一种名为异构互补蒸馏（HCD）的新框架，用于解决异构架构（如ViT到ResNet18）知识蒸馏中的特征表示差异问题。该方法通过整合教师和学生的互补特征，在共享logits中对齐表示，并通过子logit解耦蒸馏和正交性损失来促进多样化的知识转移。


<details>
  <summary>Details</summary>
Motivation: 异构架构知识蒸馏面临空间特征表示差异的挑战，传统方法主要针对同构架构设计，而现有的异构方法往往计算成本高、设计复杂或过度依赖logit对齐，无法充分利用互补特征。

Method: 提出HCD框架，通过卷积投影器和自适应池化处理学生中间特征，与教师倒数第二层特征拼接，通过互补特征映射器（CFM）生成共享logits。进一步引入子logit解耦蒸馏（SDD）将共享logits划分为n个子logits，并与教师logits融合来修正分类。使用正交性损失（OL）确保子logit多样性并减少冗余知识转移。
...
TL;DR: 本文提出FedBCS方法，通过领域不变上下文原型对齐来解决联邦学习中的特征异构性问题，特别关注医学图像分割任务中的多级上下文表示学习和层级风格偏差累积问题。


<details>
  <summary>Details</summary>
Motivation: 联邦学习中不同医疗机构的扫描设备或协议差异导致特征异构性，现有方法主要关注最终层特征而忽略多级线索，且未能处理中间层的领域特定偏差累积问题。
...
Conclusion: FedBCS通过领域不变上下文原型对齐有效解决了联邦学习中的特征异构性问题，在医学图像分割任务中表现出优越性能。
...
TL;DR: DEFT-LLM通过多专家解耦实现运动语义对齐，解决微表情识别中静态外观与动态运动线索纠缠以及文本标签与面部肌肉运动语义鸿沟的问题，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 微表情识别对于推断真实情感至关重要，但现有方法面临两个核心挑战：静态外观与动态运动线索的纠缠阻碍模型关注细微运动；现有数据集的文本标签与面部肌肉运动不完全对应，存在语义鸿沟。

Method: 提出DEFT-LLM框架，首先构建Uni-MER运动驱动指令数据集，利用光流和动作单元标签的双重约束确保时空一致性和运动对应关系；然后设计三专家架构将面部动态解耦为独立可解释的表征（结构、动态纹理和运动语义）。

Result: 在多个具有挑战性的微表情识别基准测试中展示了最先进的性能，在局部面部运动的可解释建模方面具有特别优势。
...
TL;DR: 提出了一种基于语言引导图表示学习的视频摘要方法LGRLN，通过构建视频图结构、图内关系推理和跨模态嵌入，解决了现有方法在全局依赖建模和多模态定制方面的挑战。
...
Method: 使用视频图生成器将视频帧转换为结构化图，构建前向、后向和无向图；设计图内关系推理模块和语言引导的跨模态嵌入模块；将摘要生成建模为伯努利分布的混合，并用EM算法求解。
...
TL;DR: ERMoE是一种稀疏MoE架构，通过将专家重新参数化为学习正交特征基，并用特征基得分（输入特征与专家基的余弦相似度）替代学习门控逻辑，解决了路由不稳定和专家利用率低的问题。


<details>
  <summary>Details</summary>
Motivation: 解决MoE架构中路由逻辑与专家内部结构不对齐导致的路由不稳定、专家利用率低，以及负载不平衡造成的瓶颈问题。传统负载均衡方法会削弱专家专业化并损害下游性能。
...
Conclusion: ERMoE为稀疏专家模型引入了新的架构原则，直接解决路由不稳定性，在保持稀疏性的同时实现可扩展、可解释的专业化，并提高性能。
...
TL;DR: DMC是一个用于CLIP基础类增量学习的简单有效框架，通过解耦视觉编码器适配和文本软提示优化来解决分类器偏差问题。DMC-OT是其增强版本，引入最优传输校准策略来对齐内存统计量。


<details>
  <summary>Details</summary>
Motivation: 现有CLIP基础类增量学习方法存在分类器偏差问题，学习新类别的任务特定软提示会导致文本原型过拟合到最近类别，且现有方法忽略了视觉编码器更新时的分布漂移问题。

Method: 提出两阶段框架DMC，分别冻结视觉编码器和文本软提示进行交替训练。DMC-OT进一步引入最优传输指导的校准策略来对齐内存统计量，并采用任务特定提示设计增强任务间可分性。

Result: 在CIFAR-100、Imagenet-R、CUB-200和UCF-101上的广泛实验表明，DMC和DMC-OT都达到了最先进的性能，DMC-OT进一步将准确率平均提高了1.80%。

Conclusion: DMC框架通过解耦视觉和文本模态训练有效缓解了类增量学习中的分类器偏差问题，DMC-OT的最优传输校准策略进一步提升了性能，证明了该方法在CLIP基础类增量学习中的有效性。
...
TL;DR: 本文提出Phase Aggregated Smoothing (PAS)方法，通过应用小相位偏移并聚合多头输出来解决视频LLMs中的时间不一致性问题，无需训练即可提升时间编码的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 视频LLMs存在时间不一致性问题：帧时序的微小变化会导致注意力翻转并抑制相关帧。这种不稳定性源于将Rotary Position Embeddings扩展到视频时产生的逆傅里叶时间核，该核在帧尺度上产生波纹，扰乱了本应由原始查询键内积控制的注意力。

Method: 提出Phase Aggregated Smoothing (PAS)，一种无需训练的简单机制：在多个注意力头之间应用小的相反相位偏移，然后聚合它们的输出。该方法保持每个头的频谱幅度，同时通过聚合有效平滑时间核并降低相位敏感性，而不改变位置编码结构。

Result: 在多个视频理解基准测试中，在匹配的token预算下，PAS方法显示出持续的改进，计算开销可忽略不计。分析表明，RoPE旋转对数可以近似为内容点积按时间核缩放；平滑该核可使注意力对小时间偏移具有Lipschitz稳定性；多相位平均衰减高频波纹同时保持每个头的频谱。

Conclusion: PAS为视频LLMs中的鲁棒时间编码提供了一个即插即用的升级方案，能够有效解决时间不一致性问题，提升模型性能。
...
TL;DR: 提出了一种无需训练的二值验证工作流程，用于零样本视觉任务，通过量化和二值化将开放性问题转换为多项选择题和真/假验证问题。


<details>
  <summary>Details</summary>
Motivation: 旨在通过推理时设计而非任务特定训练来提升现成视觉语言模型在零样本视觉任务中的性能。

Method: 包含两个步骤：(1) 量化：将开放性问题转换为具有明确候选答案的多项选择题；(2) 二值化：为每个候选答案提出真/假问题，并确定性解决。

Result: 在所有评估任务中，该工作流程均带来显著改进，量化到多项选择题带来大幅增益，真/假二值化提供一致的额外提升。

Conclusion: 该工作流程提供了一个简单统一的解决方案，强调推理时设计而非任务特定训练，为当前视觉语言模型提供了实用的零样本视觉增强路径。
...
TL;DR: CLUE是一个基于Stable Diffusion的生成模型框架，通过固定格式提示实现多样且稳定的图像生成，无需额外数据。在医学领域（中耳炎数据集）上显著提升生成质量和下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 解决文本到图像合成模型在专业领域（如医学）中面临的数据类型有限、数量不足的问题，实现在有限数据集上的多样且稳定图像生成。

Method: 基于Stable Diffusion架构，引入Style Encoder处理图像和提示生成风格嵌入，通过新的第二注意力层和Kullback-Leibler散度实现潜在空间的连续高斯表示。

Result: 在中耳炎数据集上，FID从46.81降至9.30，召回率从49.60%提升至70.29%。仅使用合成数据训练的分类器F1分数达83.21%，合成与真实数据结合达94.76%。
...
TL;DR: PROMISE是一个新颖的多模态框架，通过提示学习和分层对比学习来处理模态缺失问题，提高跨模态表示的一致性。


<details>
  <summary>Details</summary>
Motivation: 现有多模态模型在现实场景中当某些模态缺失时性能显著下降，主要原因是完整多模态数据与不完整模态场景之间的表示学习不一致。现有方法通过简单的生成方法处理缺失模态，但无法充分保持跨模态一致性。

Method: PROMISE将多模态提示学习整合到分层对比学习框架中，配备专门设计的提示注意力机制，动态为缺失模态场景生成鲁棒且一致的表示。

Result: 在基准数据集上的广泛实验和消融研究显示，PROMISE相比当前最先进的多模态方法具有优越性能。

Conclusion: PROMISE通过提示注意力分层对比学习框架，有效弥合了完整数据与不完整数据之间的表示差距，在处理模态缺失问题上表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在复杂视觉任务中存在"视觉处理瓶颈"，容易失去视觉证据的锚定，并在长时间生成过程中缺乏情境化视觉体验。受人类认知记忆理论启发，需要为模型提供动态视觉记忆能力。

Method: 提出VisMem框架，包含短期模块用于细粒度感知保持和长期模块用于抽象语义整合的动态潜在视觉记忆。这些记忆在推理过程中无缝调用，使模型在思考和生成过程中保持感知保真度和语义一致性。

Result: 在多样化的视觉理解、推理和生成基准测试中，VisMem相比原始模型平均性能提升11.8%，并优于所有对比方法，建立了潜在空间记忆增强的新范式。

Conclusion: VisMem通过认知对齐的记忆框架有效解决了视觉语言模型的视觉处理瓶颈问题，为模型提供了维持感知保真度和语义一致性的能力，在多个视觉任务中实现了显著性能提升。
...
Method: SP-Guard通过估计提示词危害性并应用选择性引导掩码，仅在图像的不安全区域进行引导，而不是对整个图像施加统一的引导。
...
<details>
  <summary>Details</summary>
Motivation: 解决U-Net变体在逆问题中存在的信息丢失问题，特别是难以恢复精细高频细节的局限性。
...
Conclusion: SUPER解码器块具有即插即用通用性，在统一的重构感知框架内实现高频保真度和全局一致性，同时保持可比较的计算成本。
...
TL;DR: AirCopBench是首个针对多智能体空中协作感知的综合性基准测试，包含14.6k+问题，涵盖场景理解、物体理解、感知评估和协作决策四个维度，在退化感知条件下评估MLLMs的性能。
...
Method: 使用模拟器和真实世界数据构建基准，通过模型、规则和人工方法在严格质量控制下生成大规模问题，涵盖14种任务类型。
...
Method: 提出两阶段扩散框架：首先训练冻结帧级编码器提取多焦点特征；然后引入多焦点特征融合策略构建3D感知形态表示；设计混合语义边界条件块注入扩散去噪过程。

Result: 在两个基准数据集上实现最先进结果，仅用单次去噪步骤就达到82.8%和81.3%的准确率。

Conclusion: EmbryoDiff通过扩散模型和多焦点融合有效解决了胚胎发育阶段识别中的特征模糊问题，显著提升了分类性能。
...
Method: 引入CrossMed基准，使用MAT框架评估组合泛化能力，将四个医学数据集(CheXpert、SIIM-ACR、BraTS 2020、MosMedData)统一为视觉问答格式，评估LLaVA-Vicuna-7B和Qwen2-VL-7B模型在相关、不相关和零重叠设置下的表现。

Result: 在相关分割上模型达到83.2%分类准确率和0.75分割cIoU，但在不相关和零重叠条件下性能显著下降。多模态LLM在组合泛化方面表现优异，传统模型增益有限。
...
<details>
  <summary>Details</summary>
Motivation: 传统4D流MRI需要高时空分辨率但扫描时间长，现有PINNs方法训练缓慢且需为每个患者单独训练，限制了实际应用。

Method: 提出PINGS-X框架，采用轴向对齐时空高斯表示，包括归一化高斯溅射、轴向对齐高斯和合并过程，扩展3D高斯溅射概念。

Result: 在计算流体动力学和真实4D流MRI数据集上的实验表明，PINGS-X显著减少训练时间并实现优越的超分辨率精度。

Conclusion: PINGS-X通过创新的高斯表示方法，有效解决了4D流MRI超分辨率重建中训练效率与精度之间的权衡问题。
...
Method: 扩展现有生成式合成模型为多参考版本，允许使用任意数量的前景参考图像，并提出校准参考图像的全局和局部特征以使其与背景信息兼容。
...
TL;DR: LiteAttention通过利用扩散注意力中稀疏模式的时间连贯性，在去噪序列中实现进化计算跳过，消除冗余注意力计算，显著加速视频扩散模型而不损失质量。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器在视频生成中质量出色但存在二次注意力复杂度问题，导致延迟过高。现有加速方法面临动态估计稀疏模式计算开销大、静态模式固定且次优的基本权衡。

Method: 识别扩散注意力稀疏模式在去噪步骤间具有强时间连贯性，提出LiteAttention方法：早期标记非必要tile并向前传播跳过决策，在FlashAttention基础上实现高度优化的内核。

Result: 在生产视频扩散模型上实现显著加速，无质量下降。

Conclusion: LiteAttention结合了动态方法的自适应性和静态方法的效率，为扩散变换器提供有效的注意力加速解决方案。
...
TL;DR: 这篇论文系统综述了2016-2025年间糖尿病视网膜病变(DR)检测领域的研究进展，整合了50多项研究和20多个数据集，重点关注深度学习方法在解决类别不平衡、标签稀缺、领域偏移和可解释性等方面的创新。


<details>
  <summary>Details</summary>
Motivation: 糖尿病视网膜病变是全球可预防性失明的主要原因，早期检测对减少视力丧失至关重要。过去十年深度学习改变了DR筛查方式，但缺乏系统性综述来总结技术进展和临床转化障碍。

Method: 采用系统性综述方法，分析自监督学习、半监督学习、领域泛化、联邦训练和混合神经符号模型等技术，同时评估协议、报告标准和可重复性挑战。

Result: 建立了跨数据集的基准性能表，显示深度学习在DR检测方面取得了显著进展，但仍存在多中心验证和临床信任方面的差距。

Conclusion: 这项工作为可重复、保护隐私且可临床部署的DR人工智能制定了实用议程，同时所调查的许多创新可广泛扩展到大规模医学影像领域。
...
TL;DR: 提出S2D-Align方法，通过浅到深的策略建立解剖学基础的跨模态对齐，提升放射学报告生成质量


<details>
  <summary>Details</summary>
Motivation: 现有方法仅通过图像-文本对的实例级对齐，无法建立解剖学基础的对齐，导致生成质量不佳
...
TL;DR: 本文比较了去噪扩散概率模型和自回归因果变换器在3D形状生成和补全任务中的表现，发现扩散模型在连续潜在空间中表现最佳，而自回归模型在相同离散潜在空间中也能达到或超越扩散模型性能。


<details>
  <summary>Details</summary>
Motivation: 虽然生成模型在3D数据等多种模态中得到了广泛应用，但关于哪种模型最适合特定任务尚未达成共识。此外，像文本和图像这样的条件信息经常被用于引导生成过程，而其他条件信息（如部分3D数据）尚未得到充分评估。

Method: 本研究比较了两种最有前景的生成模型——去噪扩散概率模型和自回归因果变换器，并将它们适配用于生成形状建模和补全任务。进行了全面的定量评估和比较，包括基线判别模型和广泛的消融研究。

Result: 结果显示：（1）具有连续潜在空间的扩散模型在从单个噪声深度图像进行多模态形状补全的任务中，优于判别模型和自回归方法，并在现实条件下实现了最先进的性能；（2）在相同的离散潜在空间中比较时，自回归模型在这些任务上能够匹配或超越扩散模型的性能。

Conclusion: 扩散模型在连续潜在空间中表现最佳，而自回归模型在离散潜在空间中具有竞争力，两种方法在不同设置下各有优势，为3D生成任务提供了有价值的参考。
...
<details>
  <summary>Details</summary>
Motivation: 自主机器人执行精确液体操作任务时，容器运动会导致液体表面变形，使液体状态评估变得复杂。现有数据集缺乏涵盖多样化动态场景下真实液体行为的物理信息模拟数据。

Method: 引入Phys-Liquid数据集，包含多个实验室场景、光照条件、液体颜色和容器旋转下的液体动力学数据。提出四阶段重建和估计流程：液体分割、多视角掩码生成、3D网格重建和真实世界缩放。

Result: 实验结果表明，该方法在重建液体几何和体积方面提高了准确性和一致性，优于现有基准方法。
...
TL;DR: SaTformer是一个基于全时空注意力的视频变换器，能够从卫星辐射数据中熟练预测极端降水。该模型将降水回归问题重新表述为分类问题，并采用类别加权损失来处理标签不平衡，在NeurIPS Weather4Cast 2025累积降雨挑战中获得第一名。


<details>
  <summary>Details</summary>
Motivation: 传统数值天气预报模型在临近预报时间尺度（0-4小时）性能下降，且计算成本高。虽然AI天气预测方法取得了成功，但基于视频理解架构的天气预报方法仍未被充分探索。

Method: 提出SaTformer：基于全时空注意力的视频变换器架构，将降水回归重新表述为分类问题，使用类别加权损失处理长尾分布数据集的不平衡问题。

Result: 该模型在NeurIPS Weather4Cast 2025累积降雨挑战中获得第一名，证明了其在极端降水预测方面的有效性。

Conclusion: SaTformer展示了视频变换器架构在天气预报中的潜力，特别是在处理极端降水事件方面，为实时洪水预警提供了更有效的解决方案。
...
TL;DR: VideoP2R是一个面向视频语言模型的过程感知强化微调框架，将感知和推理建模为两个独立过程，在SFT阶段生成高质量CoT数据集，在RL阶段使用过程感知组相对策略优化算法，在多个视频推理基准上取得SotA性能。
...
Method: 提出VideoP2R框架：SFT阶段使用三步流水线生成VideoP2R-CoT-162K数据集；RL阶段引入过程感知组相对策略优化(PA-GRPO)算法，为感知和推理提供独立奖励。

Result: 在7个视频推理和理解基准中的6个上达到最先进性能，消融研究证实过程感知建模和PA-GRPO的有效性，且模型感知输出对下游推理信息充分。

Conclusion: VideoP2R通过过程感知建模成功将强化微调扩展到视频语言模型，显著提升了视频推理能力。
...
TL;DR: 本文回顾了24篇关于AI生成媒体检测的最新研究，分析了现有方法的局限性和关键挑战，并建议以多模态深度学习模型为未来研究方向，以提供更鲁棒和通用的检测能力。


<details>
  <summary>Details</summary>
Motivation: 随着GAN和扩散模型等AI技术的快速发展，生成逼真图像的能力大幅提升，导致真实与合成内容难以区分。深度伪造等技术的滥用可能传播虚假信息、侵犯隐私和进行欺诈，因此需要开发有效的检测方法。

Method: 通过系统回顾24篇最新研究，逐一分析每项研究的贡献和弱点，总结现有方法的共同局限性和关键挑战。

Result: 现有检测模型主要使用CNN和ViT等深度学习方法，寻找视觉、空间或时间异常，但往往无法泛化到未见数据，且在处理不同模型生成的内容、多模态数据和高度修改内容时效果不佳。

Conclusion: 建议未来研究聚焦于多模态深度学习模型，这些模型有潜力提供更鲁棒和通用的检测能力，为构建更强防御体系提供明确起点。
...
TL;DR: 本文提出HinD框架和KEPO方法，通过后见蒸馏推理来激发MLLMs的内部知识推理能力，在KBVQA任务中实现无需外部知识或商业API的优越性能。


<details>
  <summary>Details</summary>
Motivation: 解决现有KBVQA方法推理过程不明确的问题，现有方法要么使用MLLMs的隐式知识，要么使用检索增强生成，但缺乏明确的推理轨迹。

Method: 1. 使用冻结的7B规模MLLM构建Hindsight-Zero训练数据；2. 自蒸馏为CoT生成器和知识生成器；3. 使用KEPO优化知识生成器，偏好低置信度但有用的知识。

Result: 在OK-VQA和A-OKVQA数据集上的实验验证了HinD的有效性，仅使用7B规模MLLM就能达到优越性能。
...
TL;DR: OT-ALD是一种基于最优传输理论的图像到图像翻译框架，解决了DDIB方法中的翻译效率低和潜在分布不匹配问题，通过OT映射改进反向扩散过程，在保持图像质量的同时显著提升翻译效率。


<details>
  <summary>Details</summary>
Motivation: DDIB方法虽然保持循环一致性且灵活性强，但面临翻译效率低和潜在分布不匹配导致的翻译轨迹偏差问题。
...
Conclusion: OT-ALD有效平衡了快速图像翻译与图像质量，通过最优传输理论解决了DDIB方法的局限性，实现了更高效的图像到图像翻译。
...
<details>
  <summary>Details</summary>
Motivation: 现有轨迹预测方法难以显式学习和预测智能体对轨迹变化事件的延迟响应特性，这种延迟偏好的缺失会影响预测系统的因果连续性和轨迹合理性。

Method: 受声学中混响曲线的启发，提出混响变换和Rev轨迹预测模型，使用两个显式可学习的混响核来模拟每个智能体的不同延迟偏好及其随机性。

Result: 在多个数据集上的实验表明，Rev在保持竞争力的预测精度的同时，能够揭示跨智能体和场景的可解释延迟动态。

Conclusion: 混响变换作为一种通用的延迟建模方法具有潜力，能够显式建模智能体对轨迹变化事件的延迟响应特性，提高轨迹预测的合理性和可控性。
...
<details>
  <summary>Details</summary>
Motivation: 现有可解释异常检测方法通常只能识别异常但无法区分异常类型，且需要为每个对象类别训练和维护单独模型，这成本高昂。识别异常类型对于准确诊断和成本节约决策至关重要。

Method: MultiTypeFCDD使用简单的卷积框架，仅需图像级标签来学习并生成多通道热图，每个通道对应特定的异常类型。该模型作为统一框架，能够跨多个对象类别区分异常类型。

Result: 在Real-IAD数据集上的评估表明，该方法在显著减少参数负载和推理时间的情况下，与最先进复杂模型的结果相竞争。

Conclusion: MultiTypeFCDD是一个高度实用和可行的解决方案，特别适用于计算资源受限的实际应用场景。
...
TL;DR: CATS-V2V是首个针对复杂恶劣交通场景的V2V协同感知真实世界数据集，包含10种天气和光照条件下的100个片段，提供60K帧LiDAR点云和1.26M多视角相机图像，支持4D BEV表示。
...
Method: 使用两辆硬件时间同步的车辆采集数据，覆盖10个不同地点的10种天气和光照条件，提供LiDAR点云、相机图像、GNSS/IMU记录，并提出基于目标的时间对齐方法确保多模态数据精确对齐。

Result: 构建了包含60K帧LiDAR点云、1.26M相机图像、750K GNSS/IMU记录的大规模数据集，提供时间一致的3D边界框标注和静态场景信息，支持4D BEV表示构建。

Conclusion: CATS-V2V是目前同类数据集中规模最大、支持最全面、质量最高的数据集，有望推动自动驾驶社区在复杂恶劣交通场景下的协同感知研究。
...
TL;DR: 本文提出了AlignVQA框架，通过基于辩论的多智能体方法改进视觉问答系统的校准性能，使AI系统的置信度更准确地反映其预测的正确性。


<details>
  <summary>Details</summary>
Motivation: 现代视觉问答系统在医疗诊断和自主导航等高风险领域应用日益广泛，但这些系统经常产生过度自信的响应，其置信度估计的可靠性尚未得到充分研究。

Method: 引入AlignVQA辩论式多智能体框架，让采用不同提示策略的专门化视觉语言模型生成候选答案，然后通过两阶段交互：通用智能体对这些提议进行批判、精炼和聚合。

Result: 实验结果表明，更校准的专门化智能体产生更好对齐的置信度，在多个基准VQA数据集上显著减少了校准差异。

Conclusion: 辩论过程产生更准确反映模型真实预测性能的置信度估计，提出的可微分校准感知损失函数有效提高了各智能体置信度估计的保真度。
...
TL;DR: 提出一种用于从非同步多视角视频进行高质量4D高斯溅射重建的时序对齐策略，通过粗到精的对齐模块估计和补偿每个相机的时间偏移。


<details>
  <summary>Details</summary>
Motivation: 现实场景中多视角视频常因相机触发延迟或独立录制设置导致时序不同步，而现有4DGS方法假设输入视频流时序同步，这限制了其在真实场景中的应用。

Method: 采用粗到精对齐模块，先确定粗粒度的帧级偏移，然后细化到亚帧精度，该策略可作为即插即用模块集成到现有4DGS框架中。

Result: 实验表明该方法能有效处理时序不对齐的视频，并显著提升基线方法的性能。

Conclusion: 所提出的时序对齐策略增强了4DGS在处理异步数据时的鲁棒性，为真实场景下的多视角视频重建提供了有效解决方案。
...
TL;DR: Viper-F1是一个混合状态空间视觉语言模型，用高效的液体状态空间动态替换注意力机制，通过Token-Grid相关模块增强视觉定位，在保持线性时间推理的同时显著提高效率。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型计算成本高，限制了在资源受限场景中的部署，且小型视觉语言模型难以精确捕捉细粒度视觉区域，影响了在细粒度推理任务上的性能。

Method: 提出Viper-F1模型，用液体状态空间动态替代基于Transformer的交叉注意力，引入Token-Grid相关模块计算文本标记与图像块之间的轻量级相关性，并通过FiLM条件调节状态空间动态。

Result: 在多个基准测试中，Viper-F1实现了准确、细粒度的理解，同时显著提高了效率。

Conclusion: Viper-F1通过状态空间模型和视觉定位增强，有效解决了多模态大语言模型的计算效率和细粒度理解问题，为资源受限场景提供了可行的解决方案。
...
TL;DR: 本文提出了一种用于印度次大陆及周边地区PM1、PM2.5和PM10污染物6小时临近预报的高效框架，使用CAMS全球大气成分预报数据，在特定空间区域进行训练和预测，相比基础模型取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 开发专门针对有限空间域的紧凑深度学习模型，以提高污染物短期预报的准确性、减少系统偏差并实现快速推理。

Method: 利用CAMS全球大气成分预报的分析场数据，在256x256输入区域训练三个轻量级参数特定架构模型，预测中心128x128区域的污染物浓度。

Result: 使用RMSE、MAE、Bias和SSIM指标评估，相比Aurora基础模型取得了显著性能提升，证明了紧凑专业化深度学习模型在有限空间域短期预报中的有效性。

Conclusion: 专门设计的轻量级深度学习模型在污染物临近预报任务中表现出色，能够提供准确、快速且偏差较小的预测结果，特别适合特定区域的应用需求。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个有效的短期降雨预测框架，能够从有限的观测数据中准确预测未来降雨，满足气象预测的实际需求。

Method: 使用ConvGRU模型，采用两阶段训练策略：第一阶段预测SEVIRI亮度温度，第二阶段通过非线性变换将预测结果转换为OPERA兼容的降雨率。对于事件预测任务，使用3D事件检测和时空特征提取。

Result: 在累积降雨任务中获得第二名，在事件预测任务中表现与基线模型相当。

Conclusion: 提出的ConvGRU迁移学习框架在短期降雨预测中表现优异，证明了从亮度温度预测到降雨率转换的有效性，且模型具有良好的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的VQA模型虽然能够扩展遥感数据的解释能力，但缺乏处理复杂地理空间查询所需的结构化推理能力，特别是在灾害监测、基础设施风险评估等高风险气候领域。

Method: 集成思维链推理与直接偏好优化，通过生成中间推理过程来更好地处理检测、分类、空间关系和比较分析等任务。

Result: 实验表明，思维链监督比直接基线准确率提高34.9%，而直接偏好优化在准确性和推理质量方面带来额外增益。
...
TL;DR: 本文首次对视觉语言模型在微小语义保留扰动下的鲁棒性进行了大规模系统研究，发现现代VLM对像素级偏移、几何变换、重缩放、改写等无害扰动高度敏感，稳定性与正确性密切相关。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型取得了显著进展，但其在微小语义保留输入变化下的可靠性尚未得到充分理解，需要系统评估模型对良性扰动的鲁棒性。

Method: 通过像素级偏移、轻微几何变换、填充重缩放、改写和多语言重写等五种语义保留扰动，在多个模型和数据集上进行大规模系统性测试。

Result: 现代VLM对微小扰动高度敏感：大量样本在至少一种视觉或文本修改下改变预测答案；稳定性与正确性高度相关，稳定样本更可能被正确回答；小模型稳定性模式可预测大模型正确性。
...
TL;DR: 提出了首个3D视觉中的一对多后门攻击框架，使用可配置的球形触发器，能够将单个触发器设计编码多个目标类别。


<details>
  <summary>Details</summary>
Motivation: 现有的3D点云后门攻击局限于刚性的一对一范式，而安全敏感的3D领域如自动驾驶和机器人需要更复杂的攻击模型。

Method: 基于新颖的可配置球形触发器，利用球体的空间特性作为参数空间，使单个触发器设计能够编码多个目标类别。

Result: 实验结果显示在多个数据集和模型架构上实现了高攻击成功率（最高100%），同时保持对干净数据的准确性。
...
TL;DR: MAFM^3是一个模块化框架，让单个基础模型通过轻量级模块组件扩展到医学成像的多个领域、任务和模态，实现高效的多任务和多模态适应。


<details>
  <summary>Details</summary>
Motivation: 医学影像数据稀缺，为每个领域、模态或任务预训练模型具有挑战性，需要一种统一且可扩展的框架来适应多任务和多模态需求。

Method: 通过轻量级模块化组件扩展单个基础模型，这些组件作为专门技能集，在推理时根据输入类型或临床目标灵活激活相应能力。

Result: 在胸部CT基础模型上验证，适应预后和分割模块后性能提升；整合PET扫描后，Dice得分比基线提高5%。

Conclusion: 配备模块化组件的基础模型不受初始训练范围限制，可发展为医学成像的多任务、多模态系统。
...
TL;DR: 本文提出了一种名为Guidance Score Distillation (GSD)的框架，利用预训练的视频扩散模型来增强3D高斯溅射在稀疏训练视图下的性能，通过深度扭曲引导和语义特征引导来校正生成方向，解决过拟合问题。


<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射在稀疏训练视图下容易过拟合，主要原因是缺乏中间视图的监督。受视频扩散模型成功的启发，希望利用其丰富的多视图一致性先验来改善这一问题。

Method: 提出GSD框架，基于Score Distillation Sampling思想，通过统一引导形式校正VDM的噪声预测结果，包括基于真实深度图的深度扭曲引导和基于语义图像特征的引导。

Result: 实验结果表明，该方法在多个数据集上优于现有方法。

Conclusion: GSD框架有效利用视频扩散模型的多视图一致性先验，通过深度和语义引导校正生成方向，显著提升了3D高斯溅射在稀疏视图下的表现。
...
TL;DR: 该研究探讨了多模态表示模型中的位置偏见问题，特别是在图像-文本检索任务中，发现文本编码器偏向输入开头，而图像编码器在开头和结尾都表现出偏见。
...
Result: 实验表明位置偏见在多模态模型中普遍存在，文本编码器偏向输入开头，图像编码器在开头和结尾都表现出偏见，这种偏见由位置编码方案、训练损失等多种因素共同导致。

Conclusion: 多模态模型中存在显著的位置偏见，且在不同模态中表现不同，这种偏见源于多种训练因素的组合效应。
...
<details>
  <summary>Details</summary>
Motivation: 现有的3D高斯泼溅模型在渲染细微连续视线变化时存在困难，需要更高保真的视线重定向方法来生成增强数据以提升视线估计器的泛化能力。
...
TL;DR: DoReMi是一个混合专家框架，通过联合建模领域感知专家分支和统一表示分支，解决多源点云领域差异导致的负迁移问题，在多个3D理解基准上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 解决3D深度学习在多领域泛化受限的问题，主要由于现有数据集规模有限和多源点云的高度异质性，不同传感器采集的点云在密度和噪声分布上存在显著差异，导致多领域融合时出现负迁移。
...
Conclusion: DoReMi展示了作为未来3D理解研究基础框架的强大潜力，能够有效解决多源点云的领域差异问题，实现专业知识和泛化知识的协同学习。
...
TL;DR: 本文提出了一种新颖的小样本风格编辑框架，通过构建包含五种不同风格的基准数据集，采用参数高效的混合专家低秩适应（MoE LoRA）方法，结合风格特定和风格共享路由机制来联合微调多种风格。
...
Method: 提出参数高效的MoE LoRA方法，包含风格特定路由和风格共享路由机制；通过新颖的度量引导方法自动确定每层的最佳秩；探索在DiT模型中插入LoRA的最佳位置；集成对抗学习和流匹配来指导扩散训练过程。

Result: 实验结果表明，所提出的方法在显著减少LoRA参数的情况下，优于现有的最先进方法。

Conclusion: 该框架能够有效解决小样本风格编辑问题，通过创新的MoE LoRA结构和训练策略实现了更好的性能表现。
...
TL;DR: GEODE是一个解决视觉语言模型在3D空间理解上双重瓶颈的新架构，通过解耦3D推理和数值生成，使用两个可插拔模块实现高效的空间推理。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在理解真实世界3D空间智能方面存在根本性困难，主要由于输入阶段的几何感知编码器计算昂贵与2D特征浅层化冲突，以及输出阶段离散分词器无法产生精确连续数值的双重瓶颈。

Method: 引入GEODE架构，包含两个专门模块：解耦推理模块(DRM)作为空间协处理器，通过交叉注意力对齐3D数据与2D视觉特征，并将空间思维链逻辑蒸馏为可注入的推理令牌；直接回归头(DRH)采用"嵌入即值"范式，将专用控制令牌路由到轻量级MLP进行精确连续回归。

Result: 1.5B参数的模型能够作为高级语义分发器运行，在空间推理性能上达到最先进水平，可与7B+模型相媲美。

Conclusion: GEODE通过解耦3D推理和数值生成的双模块架构，有效解决了视觉语言模型在3D空间理解上的双重瓶颈问题，实现了高效且精确的空间推理能力。
...
TL;DR: 本文提出了一种针对自闭症儿童的新型AI应用，通过社交感知粗到细框架实现注视目标检测，利用AGT数据集解决类不平衡问题，显著提升了面部注视检测性能。


<details>
  <summary>Details</summary>
Motivation: 自闭症儿童缺乏足够的专业支持来改善生活质量，通过AI自动检测注视目标可以测量联合注意力这一自闭症核心挑战，具有重要应用价值。

Method: 提出了社交感知粗到细框架，采用双路径架构，分别处理社交和非社交注视，通过上下文感知门模块引导，并利用AGT数据集解决类不平衡问题。

Result: 实验结果表明该框架在自闭症儿童注视目标检测中达到了新的最先进性能，显著优于现有方法，特别是在关键的面部注视少数类上表现突出。

Conclusion: 该研究为自闭症儿童的注视目标检测提供了有效的AI解决方案，通过社交上下文建模成功克服了数据集不平衡问题，在面部注视检测方面取得了显著进展。
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型虽然能生成逼真连贯的图像，但往往无法准确遵循文本中的数字指令，这揭示了语言与视觉表示之间的差距。研究发现模型内部已经编码了对数值正确性的潜在认知。

Method: 基于模型内部对计数准确性的隐含认知，提出CountSteer方法，在推理过程中引导模型的交叉注意力隐藏状态，无需额外训练。

Result: 实验表明CountSteer将对象计数准确率提高了约4%，同时保持了视觉质量。

Conclusion: CountSteer展示了通过利用模型内部信号实现更可控和语义可靠的文本到图像生成的简单而有效的途径。
...
TL;DR: 本文提出了一种新的视觉语言模型，通过将caption tokens分组来获取细粒度的语言表示，并与图像中的对象对齐，从而提升模型对视觉和语言的细粒度理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型主要关注图像块与语言token的对齐，但图像块对人类没有意义，单个token也不一定携带可接地的图像信息。需要将token分组来描述场景的不同方面，以获得对现实世界更好的细粒度理解。

Method: 提出一种模型架构，在模型中自动对caption tokens进行分组，以捕获细粒度的语言表示。这些表示与经过训练发现对象的图像编码器输出进行对齐，期望达到图像中对象级别的表示水平。

Result: 通过学习token分组，视觉语言模型在视觉和语言理解方面表现出更好的细粒度理解能力。模型发现的token组在定性和定量上都与文本中的可接地短语高度相似。

Conclusion: 通过将语言token分组并与图像对象对齐的方法，能够有效提升视觉语言模型的细粒度理解能力，且发现的token组与真实语言结构高度一致。
...
TL;DR: 该论文提出了一种模型无关的方法，通过交通场景图来增强基于语言的驾驶模型的性能，在LangAuto基准测试中显著提升了驾驶性能。
...
Method: 提出了一种模型无关的方法，将不同抽象层次和格式的交通场景图序列化，并通过结构化提示模板将其整合到模型中。

Result: 在LangAuto基准测试中，场景图条件化使LMDrive的驾驶分数提高了15.6%，BEVDriver提高了17.5%，即使测试时不需要场景图输入。
...
TL;DR: CORAL提出了一种协调性序数关系解剖学习方法，通过对比排序目标和序数目标来捕获体积医学图像中的局部和全局解剖结构，在有限标注设置下实现最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用硬二元阈值定义正负样本，丢弃了有价值的连续解剖相似性信息，且忽视了解剖进展的全局方向一致性，导致特征空间扭曲无法捕获跨患者的规范解剖流形。
...
Conclusion: 通过协调学习框架，CORAL能够捕获体积图像中的局部和全局结构，产生具有解剖信息的表示，从而有益于下游分割任务。
...
TL;DR: RTGaze是一种实时高质量的视线重定向方法，通过可控制的视线提示学习面部表示，并通过神经渲染进行解码，实现3D一致的视线重定向。
...
Result: 在多个数据集上实现了最先进的效率、重定向准确性和图像质量，处理速度达到约0.06秒/图像，比之前的3D感知方法快800倍。

Conclusion: RTGaze实现了实时、3D感知的视线重定向，具有显著的速度优势和高生成质量。
...
TL;DR: SimuFreeMark是一种无需噪声模拟的图像水印框架，通过利用图像低频分量的固有稳定性来抵抗传统信号处理和语义编辑攻击。
...
Result: SimuFreeMark在广泛的传统和语义攻击下优于最先进的方法，同时保持优异的视觉质量。

Conclusion: 该框架完全消除了训练过程中的噪声模拟需求，展示了低频分量在水印嵌入中的有效性。
...
TL;DR: 本文提出AUVIC框架，通过对抗性扰动实现多模态大语言模型中视觉概念的精确遗忘，解决数据隐私和版权问题，同时保持模型在相关实体上的性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型训练数据常包含敏感或受版权保护内容，引发数据隐私担忧。监管框架要求的'被遗忘权'推动了机器遗忘技术的发展，但视觉概念遗忘在MLLMs中研究不足。

Method: 提出AUVIC框架，应用对抗性扰动实现精确遗忘，有效隔离目标概念而不影响相似实体。构建VCUBench基准评估视觉概念遗忘效果。

Result: 实验结果表明，AUVIC在目标遗忘率方面达到最先进水平，同时对非目标概念的性能影响最小。

Conclusion: AUVIC框架成功解决了MLLMs中视觉概念遗忘的挑战，在保护数据隐私的同时保持了模型性能。
...
TL;DR: 本文提出了一种使用纯合成数据进行草莓6D姿态估计的方法，通过程序化Blender管道生成逼真合成数据，采用YOLOX-6D-Pose算法，在RTX 3090和Jetson Orin Nano上均取得良好性能。
...
Result: 模型在RTX 3090和Jetson Orin Nano上均达到可比较的精度，RTX 3090处理速度更快，Jetson Orin Nano更适合资源受限环境。模型能准确估计成熟和部分成熟草莓姿态，但对未成熟草莓检测存在挑战。

Conclusion: 该方法可轻松适配其他水果如苹果、桃子和李子，在农业自动化领域具有广泛适用性和重要影响。
...
TL;DR: YCB-Ev SD是一个用于6DoF物体姿态估计的标准分辨率事件相机合成数据集，包含50,000个事件序列，采用物理渲染和线性相机运动模拟生成，通过系统评估确定了最佳的事件表示方法。
...
Method: 基于物理渲染(PBR)场景和BOP方法学，使用模拟线性相机运动生成合成事件数据，系统评估不同事件表示方法对CNN推理的影响。

Result: 发现具有线性衰减的时间表面和双通道极性编码在姿态估计性能上表现最佳，显著优于指数衰减和单通道替代方案，极性信息对性能提升贡献最大。

Conclusion: 成功创建了首个全面的标准分辨率事件相机数据集，确定了最优的事件表示方法，为事件视觉研究提供了即用的基准资源。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D人体运动生成模型虽然重建精度高，但难以泛化到训练分布之外，这源于精确3D监督使模型过度拟合固定坐标模式而非学习3D结构和运动语义线索。

Method: 提出Motion-Lifting Residual Quantized VAE (ML-RQ)将2D运动序列映射到3D一致潜在空间，并设计了一套3D无关正则化目标来增强视图一致性、方向连贯性和物理合理性。

Result: 仅使用2D运动数据训练，Free3D能生成多样化、时间连贯且语义对齐的3D运动，性能达到甚至超过全3D监督的对比方法。
...
TL;DR: 本文提出了一种结合隐马尔可夫随机场(HMRF)理论和CNN分割的方法，在无需地面真实数据的情况下实现高精度分割，并提出了减少标注数据需求的预训练策略。


<details>
  <summary>Details</summary>
Motivation: 传统分割方法在精度和速度上存在不足，监督学习方法需要大量标注数据，而无监督方法分割时间较长且精度较差。需要结合两者的优势来解决这些问题。

Method: 集成HMRF理论和CNN分割，利用HMRF的邻域和类别分布概念，研究不同邻域项和组件对无监督HMRF损失的贡献，使用HMRF-UNet在PU泡沫结构的μCT图像数据集上进行分割。

Result: HMRF-UNet在无需地面真实数据的情况下实现了高分割精度，提出的预训练策略显著减少了训练分割模型所需的地面真实数据量。

Conclusion: 该方法成功结合了无监督学习和快速分割的优势，在材料表征提取任务中表现出色，为减少标注数据依赖提供了有效解决方案。
...
TL;DR: 本文提出了一种基于低秩稀疏原则的统一框架LSEF，用于解决视频情感计算中的模型不稳定和表征退化问题，通过分层结构机制分离情感基和瞬时波动。


<details>
  <summary>Details</summary>
Motivation: 视频情感计算面临模型不稳定和表征退化的挑战，主要由于缺乏分层结构机制来区分长期情感基和短期情感波动。

Method: 提出LSEF框架，包含三个即插即用模块：稳定性编码模块捕获低秩情感基，动态解耦模块分离稀疏瞬时信号，一致性集成模块重建多尺度稳定性与反应性一致性，并通过秩感知优化策略自适应平衡梯度平滑性和敏感性。
...
TL;DR: 提出了MicroVQA++，一个三阶段构建的大规模高质量显微镜视觉问答数据集，通过专家验证、异构图过滤和人工筛选确保数据质量，使4B参数的多模态大模型在显微镜推理任务上达到与GPT-5竞争的性能。
...
Result: 构建了包含大规模训练集和人工检查测试集的数据集，其Bloom认知水平困难样本分布超过MicroVQA基准。4B参数的多模态大模型在显微镜推理任务上达到与GPT-5竞争的性能，并在开源模型中达到最先进水平。

Conclusion: 通过精心设计的数据构建方法，即使是较小规模的多模态大模型也能在显微镜推理任务上达到优秀性能，证明了高质量数据的重要性。
...
Result: 评估表明MLLMs具备初步的DIQA能力，但存在评分不一致、失真误识别和严重程度误判等关键限制。思维链提示显著提升了所有层次的性能。
...
TL;DR: BOFA是一个用于类增量学习的框架，通过仅在CLIP的跨模态桥接层进行适配，无需额外参数或推理成本，利用正交低秩融合防止遗忘，并结合跨模态混合原型提升分类性能。


<details>
  <summary>Details</summary>
Motivation: 将CLIP应用于类增量学习面临两个主要挑战：(1) 适配下游任务需要额外可学习模块，增加模型复杂性和遗忘风险；(2) 现有方法未能充分利用多模态表示在视觉和文本模态整合方面的潜力。

Method: BOFA框架将所有模型适配限制在CLIP现有的跨模态桥接层，采用正交低秩融合机制将参数更新约束到与过去任务特征正交的低秩安全子空间，并使用结合稳定文本原型和视觉对应物的跨模态混合原型。

Result: 在标准基准测试上的广泛实验表明，BOFA在准确性和效率方面均优于现有方法。

Conclusion: BOFA通过桥接层正交融合适配，在无需数据回放的情况下实现了稳定的知识积累，在类增量学习中取得了优越的性能。
...
TL;DR: 本文提出自适应教学范式来解决视觉-脑电信号不对称对齐问题，通过让视觉模态动态调整知识结构来匹配脑电模态的能力，在零样本脑到图像检索任务上达到60.2%的top-1准确率。
...
Result: 实验表明，在WEAVE-100k上训练能够提升视觉理解、图像编辑和理解-生成协作能力，促进UMMs发展出新兴的视觉记忆能力，同时WEAVEBench评估揭示了当前方法在多轮上下文感知图像生成和编辑方面的持续局限性。
...
TL;DR: 本文提出了一个评估框架，用于区分文本到图像扩散模型中的识别（识别文化参考）和实现（如何描绘参考），重点关注多模态标志性现象，即图像和文本唤起文化共享关联的情况。


<details>
  <summary>Details</summary>
Motivation: 解决文本到图像扩散模型中泛化与记忆之间的模糊性，特别关注多模态标志性现象，即图像和文本唤起文化共享关联的情况，如标题唤起熟悉的艺术品或电影场景。

Method: 引入评估框架分离识别和实现两个维度，通过量化指标衡量；评估五个扩散模型在767个维基数据衍生的文化参考上；进行提示扰动实验使用同义词替换和字面图像描述；分析文化对齐与训练数据频率、文本独特性、参考流行度和创建日期的相关性。

Result: 框架比现有基于相似性的方法更有效地区分复制与转换；模型即使文本线索改变也经常复制标志性视觉结构；文化对齐不仅与训练数据频率相关，还与文本独特性、参考流行度和创建日期相关。

Conclusion: 扩散模型的价值不仅在于复制内容，还在于如何转换和重新情境化文化知识，推动评估超越简单的文本-图像匹配，实现更丰富的语境理解。
...
TL;DR: Hi-DREAM是一个受大脑启发的条件扩散框架，通过显式建模皮层层次结构来解码fMRI信号，将视觉区域分为早期/中期/晚期流，构建与U-Net深度对齐的多尺度皮层金字塔，实现高效可解释的图像重建。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散的解码器直接基于fMRI特征而不分析视觉信息在皮层中的组织方式，忽视了大脑的层次处理过程，模糊了不同视觉区域的作用。

Method: 提出Hi-DREAM框架：使用ROI适配器将fMRI分组为早期/中期/晚期流，转换为多尺度皮层金字塔；采用轻量级深度匹配的ControlNet在去噪过程中注入尺度特定提示。
...
TL;DR: VP-Bench是一个评估多模态大语言模型视觉提示感知能力的基准，包含两阶段评估框架，测试了28个模型在8种形状和355种属性组合下的表现。


<details>
  <summary>Details</summary>
Motivation: 现有基准缺乏对MLLMs解释视觉提示能力的系统评估，无法确定模型是否能有效识别和使用这种直观的人类提示方法。
...
TL;DR: 本文提出了一种重新设计视觉语言模型微调过程的方法，通过生成无偏、分布平衡且标注准确的合成数据来改善模型性能，在真实世界数据上取得了优于匹配设置微调的效果。


<details>
  <summary>Details</summary>
Motivation: 传统的视觉语言模型微调过程容易受到数据偏见、分布不平衡和标注错误的影响，导致过拟合和性能不平衡问题。现有方法在生成合成数据时缺乏对分布偏见和标注质量的控制。

Method: 1. 通过全面采样物体属性（颜色、形状、大小、位置）来自动构建无偏、平衡且标注准确的合成数据集；2. 使用该数据集微调最先进的视觉语言模型，并在绝对位置任务上评估向真实世界数据的性能迁移。

Result: 1. 在平衡合成数据上微调能在整个视觉场景中产生均匀性能，缓解常见偏见；2. 在合成刺激上微调显著提高了在真实世界数据（COCO）上的性能，优于在匹配设置下微调的模型。

Conclusion: 通过控制合成数据的生成和标注质量，可以有效解决传统微调过程中的偏见和分布不平衡问题，提升模型在真实世界场景中的泛化能力。
...
TL;DR: VoxTell是一个用于文本提示的体医学图像分割的视觉语言模型，能够将自由形式的文本描述映射到3D掩码，在多种模态上实现最先进的零样本性能。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够理解自由形式文本描述（从单词到完整临床句子）并进行3D医学图像分割的模型，以解决传统分割方法对预定义类别的依赖问题。

Method: 使用多阶段视觉语言融合在解码器层中，在多个尺度上对齐文本和视觉特征，在62K+ CT、MRI和PET体积数据上训练，涵盖1000多个解剖和病理类别。

Result: 在未见数据集上实现最先进的零样本性能，在熟悉概念上表现出色，同时能够泛化到相关的未见类别，展示了强大的跨模态迁移能力和对语言变化的鲁棒性。

Conclusion: VoxTell证明了视觉语言模型在医学图像分割中的有效性，能够从真实世界文本进行准确的实例特定分割，为临床应用提供了灵活的工具。
...
TL;DR: 提出了一种名为MaMOL的框架，通过双路由机制处理遥感多模态分类中的模态缺失问题，将模态缺失重新定义为多任务学习问题，实现了参数高效的自适应。


<details>
  <summary>Details</summary>
Motivation: 遥感多模态分类常因环境干扰、传感器故障或大气效应导致模态缺失，严重降低分类性能。现有两阶段自适应方法计算成本高且假设训练时具有完整多模态数据，限制了其在现实世界不完整情况下的泛化能力。

Method: MaMOL框架引入双路由机制：任务导向的动态路由器自适应激活不同缺失模式的专家，以及模态特定共享的静态路由器维持稳定的跨模态知识共享。通过轻量级专家更新和共享专家重用实现参数高效自适应。

Result: 在多个遥感基准测试中表现出优异的鲁棒性和泛化能力，在不同缺失率下都能保持良好性能，且计算开销最小。在自然图像数据集上的迁移实验验证了其可扩展性和跨领域适用性。

Conclusion: MaMOL为不完整多模态学习提供了一个通用且高效的解决方案，具有优异的性能表现和广泛的应用潜力。
...
TL;DR: 本文提出Prelim Attention Score (PAS)方法，利用LVLM对prelim tokens的注意力权重来检测物体幻觉，无需额外训练即可实时过滤幻觉预测。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型存在物体幻觉问题，研究发现许多幻觉预测中模型会忽略图像而依赖先前生成的prelim tokens来推断新物体。

Method: 引入Prelim Attention Score (PAS)，通过计算模型对prelim tokens的注意力权重来量化图像依赖性，该信号轻量且无需额外前向传播。

Result: PAS在多个模型和数据集上实现了最先进的物体幻觉检测性能，能够实时进行过滤和干预。

Conclusion: 利用被忽视的prelim tokens注意力信号，PAS提供了一种有效的训练无关的物体幻觉检测方法。
...
TL;DR: BRIDGE提出了一种轻量级的视觉语言模型融合模块，通过在编码器顶部添加少量双向交叉注意力层，在共享空间中跨模态对齐隐藏状态，同时保持编码器的非因果性和高效性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型要么在编码器早期融合特征，要么在后期比较池化嵌入，但两种模态的隐藏状态已经包含丰富的模态特定结构信息，直接对齐这些状态是更自然的跨模态匹配方式。
...
Conclusion: BRIDGE提供了一种高效且有效的视觉语言对齐方法，通过轻量级融合模块在保持编码器理解能力的同时实现跨模态对齐，生成任务可通过可选解码器干净解耦。
