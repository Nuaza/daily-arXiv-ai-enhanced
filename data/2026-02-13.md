...
TL;DR: 本文开发了一个基于深度学习的茶叶病害自动分类系统，使用TeaLeafBD数据集，通过对抗训练和数据增强等技术，实现了对7种茶叶健康状况（6种病害+健康叶片）的高精度识别，最高准确率达到93%。


<details>
  <summary>Details</summary>
Motivation: 茶叶是孟加拉国的重要经济作物，但叶片病害会影响产量和质量。传统人工检测方法耗时且易出错，因此需要开发自动化深度学习模型来高效准确地识别茶叶病害。

Method: 使用TeaLeafBD数据集（5,278张高分辨率图像，分为7类），采用数据预处理、数据分割、对抗训练、数据增强等技术。使用DenseNet201和EfficientNetB3两种深度学习模型进行分类，并应用Grad-CAM可视化技术分析模型预测依据。

Result: 实验结果显示，EfficientNetB3模型取得了最高的分类准确率93%，DenseNet201达到91%。对抗训练使模型对噪声或干扰输入具有更好的鲁棒性，Grad-CAM可视化帮助理解模型决策过程。

Conclusion: 提出的方法能够准确检测茶叶叶片病害，为现代农业管理提供了实用解决方案。深度学习模型在茶叶病害自动识别方面表现出色，有助于提高茶叶生产的效率和质量。
...
TL;DR: Active-Zero框架通过主动探索视觉环境而非被动交互，让视觉语言模型能够自主构建学习轨迹，在12个基准测试中显著提升推理和通用理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型的自对弈方法依赖于与静态图像集的被动交互，导致对初始数据集的强依赖和低效学习。模型无法主动寻找适合其当前能力水平的视觉数据，浪费计算资源在过于简单或过于困难的任务上。

Method: 提出Active-Zero框架，包含三个协同进化的智能体：1) Searcher从开放世界存储库中根据模型能力边界检索图像；2) Questioner合成校准的推理任务；3) Solver通过准确性奖励进行优化。形成闭环的自脚手架自动课程学习系统。

Result: 在Qwen2.5-VL-7B-Instruct模型上，Active-Zero在12个基准测试中，推理任务平均准确率达到53.97%（提升5.7%），通用理解达到59.77%（提升3.9%），持续优于现有自对弈基线方法。

Conclusion: 主动探索是实现可扩展和自适应自进化视觉语言系统的关键要素，Active-Zero框架通过主动环境探索显著提升了模型的学习效率和性能。
...
<details>
  <summary>Details</summary>
Motivation: 该项目旨在探索人工智能如何影响身体运动，揭示生成系统如何通过编排的动作编码社会文化偏见。通过AI、人类和机器人的沉浸式互动，探讨在AI也能移动、思考和留下痕迹的时代，作为人类意味着什么这一关键问题。

Method: 1. 从科幻小说中提取描述人机交互的句子；2. 使用大语言模型为每个摘录生成"做什么"和"不做什么"的配对提示；3. 通过基于扩散的文本到视频模型将这些提示转换为人类表演者的编舞指导和四足机器人的运动指令；4. 在镜面地板上进行表演，使用多摄像头运动追踪系统捕捉动作；5. 将数据重建为3D点云和运动轨迹，形成数字运动档案。

Result: 创建了一个数字运动痕迹档案，记录了AI生成的动作指导下人类和机器人的交互表演。通过这种多模态方法，揭示了生成系统如何通过编排的动作编码社会文化偏见，为理解AI对身体运动的影响提供了新的视角。

Conclusion: ReTracing作为一种新颖的方法，通过AI、人类和机器人的具身表演互动，探索了人工智能时代身体运动的意义。它不仅揭示了生成系统如何编码社会文化偏见，还提出了在AI也能移动、思考和留下痕迹的时代，人类身份和意义的重要哲学问题。
...
TL;DR: 该研究揭示了当前视频语言模型在理解视频内容、时间序列和运动方面的基本缺陷，提出了REVEAL诊断基准，包含五个压力测试来评估模型弱点。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究视频语言模型是否能够稳健地理解视频内容、时间序列和运动，发现现有模型在这方面存在严重缺陷，需要系统性的诊断工具来评估和改进。

Method: 提出了REVEAL诊断基准，包含五个控制性压力测试：时间期望偏差、语言捷径依赖、视频谄媚、相机运动敏感性、时空遮挡鲁棒性。同时提供了自动生成诊断示例的数据管道。

Result: 测试了领先的开源和闭源视频语言模型，发现这些模型存在严重问题：将倒放场景描述为正向、回答问题忽略视频内容、同意虚假声明、处理基本相机运动困难、在简单时空遮挡下无法聚合时间信息。人类在这些任务上表现优异。

Conclusion: 当前视频语言模型在理解视频的基本方面存在系统性缺陷，需要更强大的评估工具来推动模型改进。REVEAL基准和代码的发布将支持未来研究。
...
TL;DR: 提出一种从高质量3D模型生成合成图像的新流程，用于可重复、可量化的实验，比较虚拟相机参数和对象的真实值与重建估计值。


<details>
  <summary>Details</summary>
Motivation: 传统通过摄影测量从真实物体生成3D模型的方法存在多种设计选择，这些方法之间的差异主要依赖定性评估，缺乏可重复、可量化的实验框架来比较不同重建方法的性能。

Method: 开发了一种新颖的流程，从高质量3D模型生成合成图像，并通过程序化生成相机姿态。这创建了一个可控的实验环境，可以精确比较虚拟相机参数和对象的真实值与重建估计值。

Result: 该流程能够进行各种可重复、可量化的实验，可以系统地评估和比较不同摄影测量方法在相机参数估计和对象重建方面的性能。

Conclusion: 提出的合成图像生成流程为评估数字孪生生成方法提供了标准化、可量化的实验框架，能够客观比较不同摄影测量方法的性能差异。
...
<details>
  <summary>Details</summary>
Motivation: 当前选择性预测方法要么是临时性的架构修改（如SelectiveNet），要么是后验的概率输出分析（如softmax响应）。研究发现后验方法隐含生成的选择先验信息仅在推理阶段使用，而该信息在训练阶段同样重要
...
Conclusion: 通过将后验方法的选择先验信息整合到训练过程中，SYNC损失函数成功融合了临时性和后验选择性预测方法的优势，显著提升了选择性预测性能
...
TL;DR: 提出了一种将学习深度先验集成到VINS-Mono优化后端的新框架，通过强制仿射不变深度一致性和成对序数约束，在边缘设备计算限制内实现鲁棒的度量尺度恢复，显著提升低纹理环境下的视觉惯性里程计性能。


<details>
  <summary>Details</summary>
Motivation: 传统单目视觉惯性里程计系统在低纹理环境中表现不佳，稀疏视觉特征不足以进行准确的姿态估计。虽然密集单目深度估计可作为补充信息源，但基于Vision Transformer的复杂基础模型计算需求大，难以在边缘设备上实时部署。

Method: 提出了一种新颖框架，将学习深度先验直接集成到VINS-Mono优化后端。该方法强制仿射不变深度一致性和成对序数约束，通过基于方差的门控机制显式过滤不稳定伪影，严格遵循边缘设备的计算限制。

Result: 在TartanGround和M3ED数据集上的广泛实验表明，该方法在挑战性场景中防止发散，并带来显著的精度提升，绝对轨迹误差最多减少28.3%。

Conclusion: 该方法成功地将学习深度先验集成到视觉惯性里程计系统中，在边缘设备计算限制内实现了鲁棒的度量尺度恢复，显著提升了低纹理环境下的性能。
...
TL;DR: 本文提出了StreamSR数据集和EfRLFN模型，针对流媒体视频实时超分辨率的特殊挑战，通过YouTube数据构建代表性数据集，并设计高效实时模型提升压缩视频的超分辨率性能。


<details>
  <summary>Details</summary>
Motivation: 现有实时超分辨率方法在处理压缩视频内容时面临独特挑战，常用数据集不能准确反映流媒体特性，限制了当前基准测试的相关性。需要针对流媒体场景构建更准确的数据集和优化模型。

Method: 1) 从YouTube收集构建StreamSR数据集，涵盖多种视频类型和分辨率；2) 提出EfRLFN模型，集成高效通道注意力和双曲正切激活函数；3) 设计复合损失函数改善训练收敛；4) 对11种最先进的实时超分辨率模型进行基准测试。

Result: 1) 创建了具有代表性的流媒体视频数据集；2) EfRLFN模型在视觉质量和运行时性能上均有提升；3) 在其他模型上使用该数据集进行微调，在各种标准基准测试中都能获得显著的性能提升和良好的泛化能力。

Conclusion: StreamSR数据集填补了流媒体视频超分辨率评估的空白，EfRLFN模型通过新颖的架构设计在效率和视觉质量上取得平衡。该工作为流媒体场景的实时超分辨率研究提供了有价值的基准和工具。
...
<details>
  <summary>Details</summary>
Motivation: 艺术史文章通常讨论艺术品的整体和具体部分（如布局、图像学、物质文化），但观看艺术品时难以识别不同文章对该作品的论述，需要系统化的信息整合方法

Method: 1) 新颖的语料收集流水线；2) 使用低秩适应（LoRA）技术定制CLIP模型，使其适应艺术领域；3) 通过收集的语料进行弱监督训练得到PaintingCLIP模型

Result: 新模型PaintingCLIP在性能上超越了原始CLIP模型，能够为给定艺术品提供有效的上下文信息

Conclusion: 该流水线具有通用性，可轻松应用于众多人文学科领域，为艺术品研究提供了系统化的信息整合工具
...
TL;DR: Latent Forcing：一种改进潜在扩散模型的方法，通过在潜在空间和像素空间联合处理，实现高效的原生图像生成，达到扩散变换器像素生成的新SOTA


<details>
  <summary>Details</summary>
Motivation: 现有潜在扩散模型虽然能生成高质量图像，但失去了端到端建模的优势：在图像编码过程中丢弃信息、需要单独训练的编码器、以及对原始数据的辅助分布建模。需要一种既能保持潜在扩散效率，又能直接处理原始自然图像的方法。

Method: 提出Latent Forcing方法，通过联合处理潜在表示和像素，使用分别调整的噪声调度来排序去噪轨迹。潜在表示作为中间计算的"草稿纸"，在高频像素特征生成前进行计算。特别关注条件信号的顺序安排。

Result: 在ImageNet数据集上，Latent Forcing在扩散变换器像素生成方面达到了新的最先进水平。同时分析了条件信号顺序对REPA蒸馏、条件与非条件生成以及编码器重建质量与扩散能力关系的影响。

Conclusion: Latent Forcing通过简单的架构修改，实现了潜在扩散的效率优势，同时直接在原始自然图像上操作，为扩散模型提供了更有效的端到端解决方案。
...
TL;DR: 利用高分辨率CTA数据训练神经隐式函数，从各向异性的短轴CMRI中重建更准确、平滑的心脏形状，改善心脏形态分析


<details>
  <summary>Details</summary>
Motivation: 短轴心血管磁共振成像的各向异性特性限制了心脏形状分析的准确性，需要一种方法来克服这一限制

Method: 使用近各向同性的高分辨率CTA数据训练单个神经隐式函数，联合表示任何分辨率的CMRI心脏形状，特别针对右心室和心肌的重建

Result: 通过从重建形状中提取4腔切面进行评估，右心室Dice系数0.91±0.07，Hausdorff距离6.21±3.97mm；心肌Dice系数0.75±0.13，Hausdorff距离7.53±5.13mm
...
<details>
  <summary>Details</summary>
Motivation: 现有方法难以同时实现三个核心目标：背景保持、视点变化下的几何一致性以及用户可控变换。基于几何的方法需要显式3D重建且泛化能力差，而基于扩散的方法泛化好但缺乏细粒度几何控制。

Method: 将对象操作分解为两个阶段：对象移除和参考引导修复，并在统一扩散过程中编码。设计了多任务多阶段训练策略，分离背景、身份和姿态信号。引入了可扩展的真实世界数据集构建流程，生成带有估计相对相机姿态的配对图像/视频样本。

Result: 广泛实验表明，Ctrl&Shift在保真度、视点一致性和可控性方面达到了最先进的结果。这是第一个统一细粒度几何控制和真实世界泛化的对象操作框架，无需依赖任何显式3D建模。

Conclusion: Ctrl&Shift通过创新的两阶段扩散框架，成功解决了对象级操作中的几何一致性、背景保持和可控性难题，为电影后期制作、AR和创意编辑提供了强大工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有特征压缩方法通常需要为不同压缩比训练专门模型，缺乏灵活性和泛化能力，当需要新压缩比时必须重新训练。为解决这一限制，需要开发支持任意压缩比的灵活压缩框架。
...
Conclusion: ARFC框架有效解决了现有特征压缩方法缺乏灵活性的问题，通过单一模型支持任意压缩比，在资源受限的实际场景中表现出色，验证了其有效性和多功能性。
...
TL;DR: ImagineAgent框架通过认知推理与生成想象结合，解决开放词汇人-物交互中的跨模态幻觉和遮挡模糊问题，使用认知地图建模实体关系，动态调用工具增强视觉证据，在少量训练数据下实现SOTA性能。
...
Method: 提出ImagineAgent框架，创新性地构建认知地图来显式建模检测实体与候选动作之间的合理关系。动态调用工具（检索增强、图像裁剪、扩散模型）获取领域知识和丰富视觉证据，实现模糊场景下的跨模态对齐。提出平衡预测准确性和工具效率的复合奖励机制。

Result: 在SWIG-HOI和HICO-DET数据集上达到SOTA性能，仅需现有方法约20%的训练数据，验证了方法的鲁棒性和效率。

Conclusion: ImagineAgent通过认知推理与生成想象的协调，有效解决了开放词汇人-物交互中的跨模态对齐问题，在少量数据下实现了优越的性能，展示了框架的实用性和高效性。
...
TL;DR: 提出VasoMIM框架，结合血管解剖知识的掩码图像建模方法，配合XA-170K大规模X射线血管造影数据集，在多个下游任务上实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: X射线血管造影是心血管疾病的金标准成像方式，但当前深度学习方法受限于标注数据稀缺。虽然大规模自监督学习是潜在解决方案，但在该领域尚未充分探索，主要缺乏有效的自监督框架和大规模数据集

Method: 提出血管解剖感知的掩码图像建模框架VasoMIM，包含两个关键设计：1) 解剖引导的掩码策略，有针对性地掩码包含血管的补丁；2) 解剖一致性损失，保持原始图像和重建图像中血管结构的一致性。同时构建了XA-170K大规模预训练数据集
...
Conclusion: 提出的MFdiff模型成功解决了多模态PET图像恢复中的融合不一致和分布外数据不匹配问题，实现了高质量的PET图像恢复
...
<details>
  <summary>Details</summary>
Motivation: 传统基于失真的去噪方法在强噪声和分布偏移下往往产生过度平滑的重建结果，无法保持结构细节和感知真实性，需要一种能够平衡失真与感知质量的去噪框架。

Method: 提出了基于生成压缩的感知去噪框架：1）条件Wasserstein GAN压缩去噪器，显式控制率-失真-感知权衡；2）条件扩散重建策略，通过压缩潜在进行迭代去噪指导；使用熵编码潜在表示强制低复杂度结构，生成解码器通过LPIPS损失和Wasserstein距离等感知度量恢复纹理。

Result: 在合成和真实噪声基准测试中，该方法在保持竞争性失真性能的同时，实现了一致的感知改进；建立了加性高斯噪声下压缩最大似然去噪器的非渐近保证，包括重建误差和解码错误概率的界限。

Conclusion: 提出的生成压缩框架有效解决了感知去噪问题，通过压缩潜在表示强制结构简洁性，结合生成模型恢复真实纹理，在失真与感知质量之间取得了良好平衡，为图像去噪提供了新的有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型在视觉质量上已有显著提升，但超高清视频生成仍面临运动建模、语义规划和细节合成的复合挑战，需要更高效的解决方案。

Method: 采用三阶段架构：1) 低分辨率运动生成用于运动一致的潜在合成；2) 视频潜在上采样在潜在空间直接进行分辨率提升以减少内存和计算开销；3) 高分辨率内容细化集成低频和高频专家共同增强语义连贯性和细粒度细节生成。

Result: 大量实验表明LUVE在超高清视频生成中实现了卓越的逼真度和内容保真度，全面的消融研究进一步验证了每个组件的有效性。

Conclusion: LUVE通过创新的潜在级联架构和双频专家设计，有效解决了超高清视频生成的挑战，为高质量视频生成提供了新的解决方案。
...
TL;DR: PLOT-CT是一种用于低剂量CT重建的新框架，通过在预对数域应用Voronoi分解来分离噪声和结构信息，显著提升了重建精度。
...
Method: 提出PLOT-CT框架，在预对数正弦图上应用Voronoi分解，将数据解耦为不同的基础成分，分别嵌入独立的潜在空间。这种显式分解增强了模型学习判别特征的能力，直接通过减轻噪声和保留预对数域信息来提高重建精度。
...
<details>
  <summary>Details</summary>
Motivation: 涂鸦注释虽然降低了密集像素标注的成本，但存在噪声和不完整监督的问题。现有基于伪标签的方法受限于伪标签质量，需要改进伪标签的可靠性和空间一致性。

Method: PLESS通过层次化图像分区将图像划分为空间一致的区域层次结构，在语义一致区域内传播涂鸦信息来细化伪标签。该框架是模型无关的，可轻松集成到现有伪标签方法中。

Result: 在两个公开心脏MRI数据集（ACDC和MSCMRseg）上，对四种涂鸦监督算法进行实验，均显示分割准确性的持续改进。

Conclusion: PLESS是一种有效的伪标签增强策略，能提升涂鸦监督医学图像分割的性能，具有通用性和易集成性。
...
TL;DR: 提出GR-Diffusion框架，结合离散高斯表示的几何先验和扩散模型的生成能力，用于3D低剂量全身PET重建，通过分层引导机制提升图像质量。


<details>
  <summary>Details</summary>
Motivation: PET重建面临噪声放大、结构模糊和细节丢失等挑战，传统方法存在低频限制。离散高斯表示在计算机视觉中已显示潜力，但需要与生成模型结合以提升重建质量。
...
Conclusion: GR-Diffusion成功整合离散高斯表示的几何先验与扩散模型的生成能力，通过分层引导机制有效解决了PET重建中的噪声、模糊和细节丢失问题，为低剂量PET成像提供了高质量重建方案。
...
TL;DR: SToRM框架通过监督式令牌减少技术，在保持全令牌性能的同时，将多模态大语言模型的端到端自动驾驶计算成本降低高达30倍。


<details>
  <summary>Details</summary>
Motivation: 端到端自动驾驶系统在处理意外场景时需要依赖人类自然语言指令，多模态大语言模型能促进人车交互但计算资源需求大。现有视觉令牌减少方法常导致性能下降，需要在保持性能的同时实现高效计算。

Method: 提出SToRM监督令牌减少框架：1) 轻量级重要性预测器使用短期滑动窗口估计令牌重要性；2) 监督训练方法通过辅助路径从全令牌LLM传递获取伪监督信号；3) 锚点-上下文合并模块将令牌分为锚点和上下文，合并冗余上下文令牌以最小化信息损失。

Result: 在LangAuto基准测试中，SToRM在相同减少令牌预算下优于现有最先进的端到端驾驶MLLMs，保持全令牌性能的同时将计算成本降低高达30倍。

Conclusion: SToRM框架首次实现了在保持多模态大语言模型端到端驾驶性能的同时显著降低计算成本，为自动驾驶系统的高效人车交互提供了可行解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有生成方法在创建情感丰富的VR内容时存在局限：无法捕捉细微的情感语义，缺乏沉浸式体验所需的细粒度控制。需要一种能够理解并控制情感语义的生成框架。

Method: 提出EmoSpace框架：1) 使用视觉语言对齐学习动态可解释的情感原型；2) 采用分层情感表示和可学习原型；3) 开发可控生成流程，包括多原型指导、时间混合和注意力重加权；4) 支持情感图像外绘、风格化生成和VR全景生成等应用。
...
TL;DR: Clutt3R-Seg是一个用于杂乱场景中语言引导抓取的零样本3D实例分割管道，通过层次化实例树和语义线索处理遮挡和噪声掩码，在稀疏视角下显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 在杂乱环境中，遮挡、有限视角和噪声掩码会严重降低3D实例分割的可靠性，而这对语言引导的机器人操作至关重要。现有方法试图优化噪声掩码，但效果有限。

Method: 提出层次化实例树结构，将噪声掩码作为信息线索而非需要修正的误差。通过跨视角分组和条件替换，抑制过分割和欠分割，生成视角一致的掩码和鲁棒的3D实例。每个实例都带有开放词汇语义嵌入，支持自然语言指令的目标选择。针对多阶段任务中的场景变化，引入一致性感知更新机制，仅需单张交互后图像即可保持实例对应关系。

Result: 在合成和真实数据集上评估，并在真实机器人上验证。在所有设置中，Clutt3R-Seg在杂乱和稀疏视角场景下始终优于最先进的基线方法。在最具挑战性的重度杂乱序列中，AP@25达到61.66，比基线高2.2倍以上；仅用4个输入视角就超过了使用8个视角的MaskClustering方法2倍以上。

Conclusion: Clutt3R-Seg通过创新的层次化实例树方法，有效解决了杂乱环境中3D实例分割的挑战，显著提升了语言引导机器人操作的感知能力，特别是在稀疏视角和重度遮挡条件下表现出色。
...
TL;DR: 本文提出了颈戴式视角的视线估计新任务，收集了首个数据集，评估了Transformer模型GLC，并提出了视线越界分类和多视角协同学习两种扩展方法。


<details>
  <summary>Details</summary>
Motivation: 现有的自我中心视线估计主要关注头戴式摄像头，而其他视角（如颈戴式）的研究不足。颈戴式摄像头在实际应用中具有优势，但相关研究缺乏。

Method: 1) 收集首个颈戴式视线估计数据集（8名参与者，约4小时日常活动视频）；2) 评估Transformer模型GLC；3) 提出视线越界分类辅助任务；4) 提出多视角协同学习方法，联合训练头戴式和颈戴式模型，使用几何感知辅助损失。

Result: 视线越界分类任务能提升性能，优于标准微调；但多视角协同学习方法未带来性能增益。对结果进行了深入分析，讨论了颈戴式视线估计的启示。
...
TL;DR: 提出了一种轻量级、计算高效的次日野火蔓延预测工具TD-FusionUNet，通过可训练的Hadamard变换和离散余弦变换层在正交化潜在空间中捕捉频率分量，在保持高精度的同时显著减少参数数量。


<details>
  <summary>Details</summary>
Motivation: 开发适用于资源受限环境的实时野火预测工具，需要平衡预测精度和计算效率，传统方法参数过多不适合实时应用。

Method: 提出TD-FusionUNet模型，包含可训练的Hadamard变换和离散余弦变换层，实现二维变换以捕捉正交化潜在空间中的频率分量；采用随机边缘裁剪和高斯混合模型等预处理技术增强稀疏预火掩模表示。

Result: 在Google Research的Next-Day Wildfire Spread数据集和WildfireSpreadTS数据集上评估，TD-FusionUNet以37万参数获得0.591的F1分数，优于使用ResNet18编码器的UNet基线，同时参数数量显著减少。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到形状检索方法需要规范姿态且支持类别有限，限制了在现实世界中的应用，因为现实中的物体可能属于多样类别且以随机方向出现。

Method: 提出RI-Mamba模型：1) 定义全局和局部参考框架解耦姿态与几何；2) 使用希尔伯特排序构建具有几何结构的标记序列；3) 引入方向嵌入计算策略并通过特征线性调制重新整合；4) 采用跨模态对比学习与自动三元组生成进行训练。

Result: 在OmniObject3D基准测试中，RI-Mamba在超过200个物体类别和任意方向下实现了最先进的性能，展示了优越的表征能力和鲁棒性。

Conclusion: RI-Mamba是首个旋转不变的状态空间模型，能够有效处理任意方向的点云，为大规模文本到形状检索提供了实用的解决方案，代码已开源。
...
TL;DR: 开发了一种语义条件化的潜在扩散模型，用于生成受解剖循环和C臂位置控制的动脉期脑DSA图像，专家评估显示生成的图像具有临床真实感。


<details>
  <summary>Details</summary>
Motivation: 数字减影血管造影（DSA）在脑血管疾病诊断和治疗中起核心作用，但其侵入性和高采集成本严重限制了大规模数据收集和公共数据共享，因此需要开发能够生成合成DSA图像的方法。

Method: 开发了语义条件化的潜在扩散模型（LDM），使用编码解剖结构和采集几何的文本嵌入进行训练，基于99,349帧的单中心DSA数据集，通过解剖循环（前循环vs后循环）和标准C臂位置进行显式控制。

Result: 四位医学专家（包括两名神经放射科医生、一名神经外科医生和一名内科专家）使用5级Likert量表评估400张合成DSA图像，总体Likert评分在3.1-3.3之间，评分者间信度高（ICC(2,k)=0.80-0.87），Fréchet起始距离中位数为15.27，表明与真实DSA帧的分布相似性高。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D场景生成方法虽然使用大型语言模型，但缺乏领域特定推理、验证机制和模块化设计，导致控制性差和可扩展性不足。特别是在农业仿真领域，需要更专业、可靠和可扩展的自动化生成方案。

Method: 开发了模块化多LLM管道，整合3D资产检索、领域知识注入和Unreal渲染引擎API代码生成。采用混合策略结合few-shot提示、检索增强生成、微调和验证等LLM优化技术，实现结构化数据处理、中间验证和灵活扩展。

Result: 系统通过结构化提示和语义准确性指标进行评估。用户研究表明生成场景在真实感和熟悉度方面表现良好，专家对比显示相比手动场景设计显著节省时间。结果证实多LLM管道在自动化领域特定3D场景生成方面的有效性和可靠性。

Conclusion: 多LLM管道方法能够有效自动化农业领域的3D场景生成，提高可靠性和精确性。未来工作将扩展资产层次结构、探索实时生成，并将管道适配到农业以外的其他仿真领域。
...
TL;DR: GSO-SLAM：基于高斯场景表示的实时单目稠密SLAM系统，通过双向耦合视觉里程计和高斯泼溅，在EM框架下联合优化，实现实时重建和跟踪。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么将跟踪和建图与统一场景耦合导致计算成本高，要么与结构化跟踪框架松散集成引入冗余。需要一种既能实时运行又能保持高质量重建和跟踪精度的方法。

Method: 1. 双向耦合视觉里程计(VO)和高斯泼溅(GS)；2. 在期望最大化(EM)框架下制定联合优化，同时细化VO的半稠密深度估计和GS表示；3. 提出高斯泼溅初始化，利用图像信息、关键帧位姿和VO的像素关联生成接近最终高斯场景的近似。

Result: 1. 系统能够实时运行；2. 在重建场景的几何/光度保真度和跟踪精度方面达到最先进水平；3. 无需启发式方法。

Conclusion: GSO-SLAM通过双向耦合VO和GS，在EM框架下联合优化，实现了实时、高质量的单目稠密SLAM，在几何/光度保真度和跟踪精度方面表现优异。
...
TL;DR: 提出STVG-R1框架，通过视觉提示和强化学习解决时空视频定位中的幻觉问题，在多个基准测试中达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型中文本描述与视觉坐标之间的错位会导致幻觉问题，在时空视频定位等密集预测任务中尤为严重。现有方法通常关注增强视觉-文本对齐或附加辅助解码器，但这会引入额外可训练模块，导致显著的标注成本和计算开销。

Method: 提出新颖的视觉提示范式，将逐帧坐标预测重新表述为紧凑的实例级识别问题，为每个对象分配唯一的、时间一致的ID，并将这些ID作为视觉提示嵌入视频中。同时引入STVG-R1，这是首个用于时空视频定位的强化学习框架，使用任务驱动的奖励联合优化时间准确性、空间一致性和结构格式正则化。

Result: 在六个基准测试上的广泛实验证明了方法的有效性。STVG-R1在HCSTVG-v2基准测试上比基线Qwen2.5-VL-7B的m_IoU提高了20.9%，建立了新的SOTA。令人惊讶的是，STVG-R1在多对象指代视频对象分割任务上也表现出强大的零样本泛化能力，在MeViS上达到47.3%的J&F SOTA。

Conclusion: 提出的视觉提示范式和STVG-R1强化学习框架有效解决了时空视频定位中的幻觉问题，避免了跨模态坐标对齐的困难，在多个任务上实现了最先进的性能，并展示了良好的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 电商产品理解需要强大的多模态（文本、图像、结构化属性）理解能力。通用视觉语言模型虽然能够进行通用化的多模态潜在建模，但目前缺乏针对电商数据特点（属性中心、多图像、噪声多）的适配策略，且不能牺牲模型的通用性能。

Method: 通过大规模实验研究探索通用视觉语言模型的针对性适配方法，并提出一个新颖的全面评估套件，涵盖深度产品理解、严格指令遵循和动态属性提取三个维度。

Result: 研究表明，针对性的适配可以显著提升通用视觉语言模型在电商领域的性能，同时保持其广泛的跨模态能力。

Conclusion: 本文证明了通用视觉语言模型可以通过针对性适配有效应用于电商产品理解任务，并提出了一个全面的评估框架来衡量模型在该领域的表现。
...
TL;DR: 该论文提出了一种改进视觉对比解码的方法，通过构建对象对齐的辅助视图来减少多模态大语言模型中的物体幻觉问题。
...
Result: 在两个流行的物体幻觉基准测试中，该方法在两个不同的多模态大语言模型上都表现出一致的性能提升。

Conclusion: 通过构建对象对齐的辅助视图来改进视觉对比解码，可以有效减少多模态大语言模型中的物体幻觉问题，且计算开销小，具有较好的通用性。
...
<details>
  <summary>Details</summary>
Motivation: 实现空间智能需要超越视觉合理性，构建基于物理定律的世界模拟器。虽然编码LLM在静态3D场景生成方面取得了进展，但将其扩展到4D动态仍然是一个关键前沿。该任务面临两个基本挑战：多尺度上下文纠缠和语义-物理执行差距

Method: 1. 双流架构：解耦检索增强的对象生成与分层环境编排；2. 物理感知闭环机制：PostProcess Agent编写动态脚本，配合VLM-Motion Critic进行自我反思以迭代优化模拟代码

Result: 在Code4D基准测试中，Code2Worlds优于基线方法，获得41%的SGS增益和49%更高的丰富度，同时独特地生成了先前静态方法中缺乏的物理感知动态
...
TL;DR: Light4D是一个无需训练即可合成4D视频重照明的框架，通过解耦流引导和时间一致性注意力机制，在极端视角变化下保持时间一致性。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的生成模型在图像和视频重照明方面取得了进展，但扩展到4D重照明面临挑战，主要原因是缺乏配对的4D重照明训练数据，以及在极端视角变化下难以保持时间一致性。

Method: 提出了Light4D框架，包含两个核心技术：1）解耦流引导，这是一种时间感知策略，在潜在空间中有效注入光照控制同时保持几何完整性；2）时间一致性注意力，在IC-Light架构中开发，并结合确定性正则化来消除外观闪烁。

Result: 大量实验表明，该方法在时间一致性和光照保真度方面达到了竞争性性能，能够稳健处理从-90度到90度的相机旋转。

Conclusion: Light4D是一个无需训练即可实现4D视频重照明的有效框架，解决了现有方法在极端视角变化下时间一致性不足的问题。
...
TL;DR: 提出轻量级RGB-D融合框架，在少量数据（11.2k样本）上训练，通过深度先验提升分割性能，超越EfficientViT-SAM
...
Method: 提出轻量级RGB-D融合框架，增强EfficientViT-SAM模型。使用预训练深度估计器生成深度图，通过专用深度编码器在中间层与RGB特征融合

Result: 仅用11.2k样本（少于SA-1B的0.1%）训练，达到比EfficientViT-SAM更高的准确率，证明深度线索为分割提供强几何先验

Conclusion: 深度先验可以有效提升分割性能，在极少量数据上实现优于现有高效SAM模型的效果，为轻量级分割系统提供新思路
...
TL;DR: 本文提出了一种使用3D分形视频进行动作识别模型预训练的方法，通过改进分形生成算法解决了传统方法速度慢和多样性不足的问题。


<details>
  <summary>Details</summary>
Motivation: 合成数据集在深度学习中被视为替代真实标注数据的宝贵资源。公式驱动监督学习（FDSL）能够通过公式驱动方法（如分形或轮廓）提供无限数量的完美标注数据，且避免了人工标注、隐私和伦理问题。然而，传统的3D分形生成方法速度慢且产生退化的分形，限制了其在动作识别预训练中的应用。

Method: 1. 使用3D迭代函数系统（IFS）生成3D分形；2. 将分形进行时间变换形成视频序列；3. 提出"目标智能过滤"方法，解决分形生成速度和多样性问题；4. 将生成的分形视频用于动作识别模型的预训练。

Result: 提出的目标智能过滤方法实现了约100倍的采样速度提升，相比其他3D分形过滤方法，在下游任务性能上表现更优。同时发现，过度限制的分形生成方法虽然能产生美观的分形，但对下游任务性能不利。
...
TL;DR: 论文提出JEPA-VLA方法，通过整合预测性视频嵌入来改进视觉语言动作模型，解决现有VLA模型样本效率低和泛化能力有限的问题。
...
Method: 通过深入分析发现，视频预测嵌入（特别是V-JEPA 2）擅长灵活丢弃不可预测的环境因素并编码任务相关的时间动态。基于此观察，作者提出了JEPA-VLA方法，这是一种简单而有效的方法，能够自适应地将预测嵌入整合到现有的VLA模型中。

Result: 实验表明，JEPA-VLA在多个基准测试中带来了显著的性能提升，包括LIBERO、LIBERO-plus、RoboTwin2.0以及真实机器人任务。

Conclusion: 预测性视频嵌入能够有效补偿现有VLA模型中视觉表示的关键缺陷，通过整合这些嵌入可以显著提升VLA模型的性能，为解决样本效率低和泛化能力有限的问题提供了有效途径。
...
TL;DR: WorldTree提出统一的时空分解框架，通过时间划分树和空间祖先链实现从粗到细的优化，在单目动态重建中取得显著性能提升。
...
Method: 提出WorldTree框架：1) 时间划分树(TPT)基于继承式划分树结构实现层次时间分解的从粗到细优化；2) 空间祖先链(SAC)递归查询祖先层次结构，提供互补的空间动态，并在祖先节点间专门化运动表示。

Result: 在多个数据集上实验表明，该方法在NVIDIA-LS数据集上LPIPS指标提升8.26%，在DyCheck数据集上mLPIPS指标提升9.09%，优于现有最佳方法。

Conclusion: WorldTree通过统一的时空分解框架有效解决了单目动态重建中的挑战，实现了显著性能提升，为更实用的应用提供了解决方案。
...
TL;DR: DiffPlace是一个用于生成地点可控多视角图像的新框架，通过引入place-ID控制器实现背景建筑一致的城市场景生成，同时灵活修改前景物体和天气条件，显著提升视觉地点识别任务的训练效果。


<details>
  <summary>Details</summary>
Motivation: 当前的多视角扩散模型在生成城市街景时难以保持地点感知和背景一致性，限制了其在视觉地点识别任务中生成真实样本的有效性。需要一种能够控制地点特征同时保持背景一致性的生成方法。

Method: 提出DiffPlace框架，引入place-ID控制器，使用线性投影、感知器变换器和对比学习将地点ID嵌入映射到固定CLIP空间，使模型能够合成背景建筑一致的图像，同时灵活修改前景物体和天气条件。

Result: 通过定量比较和增强训练评估的广泛实验表明，DiffPlace在生成质量和视觉地点识别训练支持方面均优于现有方法，证明了生成模型在增强场景级和地点感知合成方面的潜力。

Conclusion: DiffPlace为自动驾驶中的地点识别提供了一种有价值的改进方法，展示了生成模型在提升场景级和地点感知合成能力方面的潜力，能够有效支持视觉地点识别任务。
...
TL;DR: 提出了DynaHOI-Gym平台和DynaHOI-10M基准测试，用于评估动态手-物交互场景，填补了现有基准测试主要关注静态物体的空白。


<details>
  <summary>Details</summary>
Motivation: 现有手部运动生成基准主要关注静态物体交互，缺乏对动态场景（移动目标和时间关键协调）的测试，需要建立专门的动态手-物交互评估平台和基准。

Method: 1. 开发DynaHOI-Gym统一在线闭环平台，包含参数化运动生成器和基于rollout的评估指标；2. 构建DynaHOI-10M大规模基准测试，包含1000万帧和18万条手部捕捉轨迹；3. 提出ObAct基线方法，通过时空注意力整合短期观察和当前帧来预测动作。

Result: 1. 创建了包含8个主要类别和22个细分子类别的动态目标运动分类体系；2. ObAct基线方法在位置成功率上实现了8.1%的提升；3. 提供了大规模动态手-物交互数据集和评估平台。

Conclusion: DynaHOI-Gym平台和DynaHOI-10M基准测试填补了动态手-物交互评估的空白，为相关研究提供了重要的基础设施和评估标准，推动了该领域的发展。
...
TL;DR: 提出了一种基于隐式神经表示和去噪扩散模型的心肌LGE图像及分割掩码合成框架，用于缓解数据稀缺问题，提升纤维化分割性能


<details>
  <summary>Details</summary>
Motivation: 晚期钆增强（LGE）成像是心肌瘢痕评估的临床标准，但标注数据集有限阻碍了自动分割方法的发展，需要解决数据稀缺问题

Method: 首先训练隐式神经表示（INRs）捕获LGE数据和相应心肌、纤维化掩码的连续空间表示，然后将其压缩为紧凑的潜在嵌入，最后使用扩散模型在该潜在空间生成新表示，解码为具有解剖一致分割掩码的合成LGE图像

Result: 在133个心脏MRI扫描上的实验表明，使用200个合成体积增强训练数据可将纤维化分割的Dice分数从0.509提升至0.524，改善了分割性能

Conclusion: 该方法提供了一种无需标注的数据增强方法，有助于缓解数据稀缺问题，代码已公开可用
...
TL;DR: 该研究评估了视觉语言模型在法语PDF转Markdown任务上的表现，创建了一个基于模型分歧采样的法语文档基准，并设计了针对具体失败模式的单元测试式评估方法。


<details>
  <summary>Details</summary>
Motivation: 文档解析是检索增强生成(RAG)管道的关键步骤，但现有基准多关注英语或中文，且对格式和线性化选择的惩罚过重，而这些对下游应用影响不大。需要专门针对法语文档的评估基准。

Method: 从60,000份文档中通过模型分歧采样选取困难页面，创建法语基准。采用单元测试式检查方法，针对文本存在性、阅读顺序和局部表格约束等具体失败模式，结合类别特定的规范化来减少仅展示层面的差异。

Result: 评估了15个模型，发现最强专有模型在手写体和表单处理上表现出更高的鲁棒性，而多个开源权重系统在标准印刷布局上仍具有竞争力。

Conclusion: 需要更细粒度的评估方法来准确衡量文档解析质量，特别是在处理法语等语言的复杂文档时。专有模型在特定困难任务上表现更好，但开源模型在常规任务上仍有竞争力。
...
<details>
  <summary>Details</summary>
Motivation: 在基于医学影像的关键决策支持系统中，AI辅助决策的可靠性与预测准确性同等重要。尽管深度学习模型在准确性方面表现出色，但经常存在校准不足的问题，表现为对错误预测过度自信。为了促进临床接受，模型需要以与预测正确性相关的方式量化不确定性，使临床医生能够识别不可靠的输出以供进一步审查。

Method: 提出一个通用的概率优化框架，基于贝叶斯深度学习。具体包括：1) 引入新颖的置信度-不确定性边界损失（CUB-Loss），对高置信度错误和低置信度正确预测施加惩罚，明确强制预测正确性与不确定性估计之间的对齐；2) 设计双温度缩放（DTS）策略进行后处理校准，进一步细化后验分布以提高直观可解释性。

Result: 在三个不同的医学影像任务上验证了该框架：肺炎自动筛查、糖尿病视网膜病变检测和皮肤病变识别。实证结果表明，所提出的方法在不同模态下实现了一致的校准改进，在数据稀缺场景下保持了稳健性能，并且在严重不平衡的数据集上仍然有效。

Conclusion: 该框架强调了其在真实临床部署中的潜力，通过强制预测正确性与不确定性估计之间的对齐，提高了医学影像AI模型的可靠性和临床实用性。
...
TL;DR: 提出SCoT框架，通过空间思维链将MLLMs的空间推理能力与扩散模型的生成能力结合，解决现有方法计算成本高或空间信息损失的问题


<details>
  <summary>Details</summary>
Motivation: 扩散模型在美学图像合成方面表现出色，但在复杂空间理解和推理方面存在困难。现有方法要么计算成本高（联合训练），要么依赖文本提示导致空间信息损失

Method: 提出空间思维链(SCoT)框架：1) 使用文本-坐标交替指令格式训练扩散模型增强布局感知；2) 利用先进MLLMs作为规划器生成综合布局计划，将其空间规划能力直接转移到生成过程

Result: 在图像生成基准测试中达到最先进性能，在复杂推理任务上显著优于基线方法，在图像编辑场景中也表现出强大效果

Conclusion: SCoT框架有效弥合了MLLMs的推理能力与扩散模型的生成能力之间的差距，提供了一种即插即用的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 新生儿复苏的准确记录对质量改进和临床指南遵循至关重要，但实践中仍未被充分利用。虽然之前的3D-CNN和Vision Transformer方法在检测关键活动方面显示出潜力，但在识别细粒度活动方面仍面临挑战。

Method: 研究探索使用本地视觉语言模型结合大语言模型，与监督学习的TimeSFormer基线进行比较。使用包含13.26小时新生儿复苏视频的模拟数据集，评估多种零样本VLM策略和带有分类头的微调VLM，包括低秩适应方法。

Result: 小型本地VLM存在幻觉问题，但使用LoRA微调后，F1分数达到0.91，显著超过了TimeSformer的0.70结果。

Conclusion: 生成式AI方法，特别是经过适当微调的视觉语言模型，在新生儿复苏视频活动识别方面具有显著优势，能够超越传统监督学习方法。
...
TL;DR: ReNoV框架利用外部视觉表征作为条件，通过专门的表征投影模块将其注入扩散过程，提升基于扩散的新视角合成的几何一致性和质量。
...
Method: 首先分析外部视觉表征在空间注意力中的对应能力，然后提出ReNoV框架，通过专门的表征投影模块将外部表征注入扩散过程，指导新视角合成。

Result: 实验表明该方法在重建保真度和修复质量方面均有显著提升，在标准基准测试中优于先前的基于扩散的新视角合成方法，并能从稀疏、无位姿的图像集合中进行鲁棒合成。

Conclusion: ReNoV框架通过利用外部表征的对应特性，有效提升了基于扩散的新视角合成的几何一致性和生成质量，为从稀疏图像集合中进行新视角合成提供了鲁棒解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统CCD/CMOS传感器动态范围低于70dB，在焊接电弧监测和抛光金属表面分析等强光环境下容易饱和，导致细节丢失和数字图像相关(DIC)测量误差
...
Conclusion: DMD系统提供高保真自适应HDR成像，克服传统传感器限制，在传统方法不适用的高光环境中具有光学计量和应力分析的强大潜力
...
TL;DR: GigaBrain-0.5M* 是一个通过世界模型强化学习训练的视觉-语言-动作模型，在复杂机器人操作任务上相比基线有显著性能提升，并能可靠执行长时程任务。


<details>
  <summary>Details</summary>
Motivation: 传统的视觉-语言-动作模型在场景理解和未来预测方面存在局限，而视频世界模型具有强大的时空推理和准确未来预测能力，因此将两者结合可以提升VLA模型性能。

Method: 基于GigaBrain-0.5（在超过10,000小时机器人操作数据上预训练）构建，通过RAMP（基于世界模型条件策略的强化学习）方法整合世界模型强化学习，实现跨任务鲁棒适应。

Result: RAMP相比RECAP基线在Laundry Folding、Box Packing和Espresso Preparation等挑战性任务上获得约30%的性能提升，GigaBrain-0.5M*展现出可靠的长时程执行能力，能一致完成复杂操作任务而无失败。

Conclusion: 基于世界模型的强化学习能显著提升VLA模型在复杂机器人操作任务上的性能，GigaBrain-0.5M*通过整合视频世界模型的时空推理能力，实现了鲁棒的跨任务适应和可靠的长时程执行。
...
<details>
  <summary>Details</summary>
Motivation: 数字产业对高质量、多样化的模块化3D资产有强烈需求，尤其是在用户生成内容领域。现有方法难以生成符合约束设计参数的模块化资产。

Method: 采用自回归Transformer架构，创新性地借鉴语言模型的模块序列化和解码技术，从在线平台收集的真实模块化资产数据进行训练。

Result: 初步结果表明AssetFormer能有效提升资产生成质量，简化专业开发和UGC场景中的资产创建流程。

Conclusion: 该工作提供了一个可扩展到多种模块化3D资产类型的灵活框架，为3D内容生成领域做出了贡献。
...
Method: PosterOmni框架通过高效的数据-蒸馏-奖励管道：1)构建覆盖六种任务类型的多场景图像到海报数据集；2)在局部和全局专家之间进行知识蒸馏以进行监督微调；3)应用统一的PosterOmni奖励反馈来联合对齐视觉实体保持和美学偏好。

Result: 实验表明PosterOmni显著提升了参考依从性、全局构图质量和美学和谐性，超越了所有开源基线，甚至超过了几个专有系统。

Conclusion: PosterOmni成功解决了图像到海报生成中局部编辑和全局创建的耦合需求，通过统一框架实现了实体保持编辑和概念驱动创建的协同优化。
...
Result: 仅使用13,000个Nano Banana pro演示微调FLUX，在提示跟随和美学基准上取得竞争性性能。框架有效泛化到离散图像和视频生成，并能作为鲁棒正则化器缓解基于奖励优化的奖励黑客问题。
...
TL;DR: TexSpot是一个基于扩散的纹理增强框架，通过新颖的Texlet表示方法解决现有3D纹理生成中的视图不一致问题，显著提升纹理质量和几何一致性。


<details>
  <summary>Details</summary>
Motivation: 当前主流的多视角扩散流水线存在视图不一致问题，现有表示方法如UV贴图在展开时会产生扭曲，而基于点的方法将纹理保真度与几何密度紧密耦合，限制了高分辨率纹理生成。

Method: 提出TexSpot框架，核心是Texlet表示方法，将基于点的3D纹理的几何表达能力与基于UV表示的紧凑性相结合。每个Texlet潜在向量通过2D编码器编码局部纹理块，并使用3D编码器聚合全局形状上下文。通过级联的3D到2D解码器重建高质量纹理块，实现Texlet空间学习。训练基于Texlet条件的扩散变换器来优化多视角扩散方法生成的纹理。
...
TL;DR: DreamID-Omni是一个统一的可控人中心音视频生成框架，通过对称条件扩散变换器和双重解耦策略解决多人场景中的身份-音色绑定失败问题，在多个任务上实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将基于参考的音视频生成、视频编辑和音频驱动视频动画等任务视为孤立目标，且在多人物场景中难以实现精确的身份和音色解耦控制。

Method: 提出对称条件扩散变换器整合异构条件信号；采用双重解耦策略：信号级的同步RoPE确保注意力空间绑定，语义级的结构化描述建立属性-主体映射；设计多任务渐进训练方案。

Result: 在视频、音频和音视频一致性方面实现全面的最先进性能，甚至超越领先的专有商业模型。

Conclusion: DreamID-Omni为可控人中心音视频生成提供了一个统一框架，解决了多人场景中的身份-音色绑定问题，并将开源代码以弥合学术研究与商业应用之间的差距。
...
TL;DR: DeepGen 1.0是一个轻量级5B参数的多模态统一模型，在图像生成和编辑任务上超越更大规模模型，通过创新的SCB对齐框架和三阶段训练策略实现高效性能


<details>
  <summary>Details</summary>
Motivation: 当前统一多模态模型通常需要巨大参数量（>10B），导致训练成本和部署开销过高。需要开发轻量级但性能强大的替代方案来降低研究门槛

Method: 1. 提出Stacked Channel Bridging（SCB）深度对齐框架，从多个VLM层提取层次特征并与可学习的"思考令牌"融合，为生成主干提供结构化、富含推理的指导
2. 设计三阶段数据中心训练策略：对齐预训练、联合监督微调、基于MR-GRPO的强化学习
3. 仅使用约5000万样本进行训练

Result: 在多个基准测试中取得领先性能：在WISE上超越80B HunyuanImage 28%，在UniREditBench上超越27B Qwen-Image-Edit 37%。训练稳定，避免视觉伪影，生成质量显著提升
...
TL;DR: UniDFlow是一个统一的多模态离散流匹配框架，通过任务特定低秩适配器解耦理解和生成，避免目标干扰和表示纠缠，并通过基于参考的多模态偏好对齐优化相同条件下的相对结果，无需大规模重新训练即可提高忠实度和可控性。


<details>
  <summary>Details</summary>
Motivation: 当前多模态系统在处理理解、生成和编辑任务时存在目标干扰和表示纠缠问题，需要统一的框架来同时支持这些任务，同时保持高忠实度和可控性。

Method: 1. 使用任务特定低秩适配器解耦理解和生成；2. 引入基于参考的多模态偏好对齐，在相同条件下优化相对结果；3. 采用离散流匹配框架统一多模态任务。

Result: 在八个基准测试中达到最先进性能，在零样本情况下展现出强大的泛化能力，包括修复、上下文图像生成、基于参考的编辑和组合生成等任务，尽管没有进行明确的任务特定训练。

Conclusion: UniDFlow提供了一个统一的多模态框架，能够有效解耦理解和生成任务，通过创新的偏好对齐机制提高忠实度和可控性，在多个任务上展现出卓越的性能和泛化能力。
...
Result: Monarch-RT在仅针对双向模型设计的现有稀疏基线中表现出高效性，在应用于最先进的Self-Forcing模型时，达到95%的注意力稀疏度且无质量损失。优化实现优于FlashAttention-2/3/4内核，在RTX 5090、H100和B200 GPU上获得1.4-11.8倍的加速。首次在单张RTX 5090上实现16FPS的真正实时视频生成。

Conclusion: Monarch-RT是首个针对实时视频生成的高性能稀疏注意力参数化方法，通过结构化注意力分解有效解决了实时视频生成中的计算瓶颈，为实时视频生成提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有统一多模态模型通常单次处理任务，无法迭代优化输出。复杂多模态任务（如空间组合、多对象交互、动态指令）需要分解指令、验证中间结果并进行迭代修正。测试时扩展在语言模型中已证明有效，但扩展到多模态模型仍是挑战。

Method: 提出UniT框架，结合智能数据合成、统一模型训练和灵活测试时推理，支持验证、子目标分解和内容记忆等认知行为。通过多模态思维链测试时扩展，使模型能够进行多轮推理。

Result: 1) 在短推理轨迹上训练的统一模型能泛化到更长的测试时推理链；2) 顺序思维链推理比并行采样更具可扩展性和计算效率；3) 在生成和编辑轨迹上训练能提升分布外视觉推理能力。
...
TL;DR: 提出了一种名为"渐进式语义错觉"的新型矢量草图任务，通过顺序添加笔画实现语义转换，并开发了Stroke of Surprise生成框架来优化矢量笔画，使其在不同绘制阶段满足不同的语义解释。


<details>
  <summary>Details</summary>
Motivation: 传统视觉错觉主要依赖于空间操作，如多视图一致性。本研究旨在将视觉字谜从空间维度扩展到时间维度，探索在绘制过程中通过笔画顺序实现语义转换的可能性。

Method: 提出了一个序列感知的联合优化框架，采用双分支分数蒸馏采样机制。核心是解决"双重约束"问题：初始笔画既要形成第一个概念，又要作为第二个概念的结构基础。引入了新颖的叠加损失来确保空间互补性而非遮挡。

Result: 实验表明，该方法在可识别性和错觉强度方面显著优于现有基线方法，成功实现了从空间到时间维度的视觉字谜扩展。

Conclusion: 通过引入渐进式语义错觉和Stroke of Surprise框架，成功将视觉错觉从静态空间操作扩展到动态时间维度，为矢量草图生成和视觉错觉研究开辟了新方向。
