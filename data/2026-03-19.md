...
Motivation: 现有的放射学报告生成方法通常是单模型强化学习或对独立训练模型进行后处理智能体化，缺乏对多个智能体的联合训练和整个智能体系统的优化，这限制了临床效能的提升。
...
Conclusion: MARL-Rad通过多智能体强化学习框架有效提升了放射学报告生成的临床效能，证明了联合训练多个智能体并优化整个系统的重要性。
...
TL;DR: 本文研究了基于惯性测量单元的在线手写识别，针对字符分布不均和书写者间差异问题，系统比较了子词分词和基于拼接的数据增强两种策略，发现它们分别有效处理书写者间和书写者内差异。


<details>
  <summary>Details</summary>
Motivation: 基于惯性测量单元的在线手写识别虽然能在不同书写表面上收集输入信号，但仍面临字符分布不均和书写者间差异的挑战。本研究旨在系统探索解决这些问题的有效策略。

Method: 研究系统调查了两种策略：1) 子词分词（特别是Bigram分词），用于处理书写者间差异；2) 基于拼接的数据增强，用于处理书写者内分布稀疏性。在OnHW-Words500数据集上进行实验，分别评估了书写者独立和书写者依赖两种分割方式。

Result: 在书写者独立分割中，Bigram分词显著提升性能，将词错误率从15.40%降至12.99%。在书写者依赖分割中，分词反而降低性能，但基于拼接的数据增强作为强大正则化器，将字符错误率降低34.5%，词错误率降低25.4%。分析显示短的低级分词有利于模型性能，且数据增强效果优于按比例延长训练。
...
TL;DR: 提出BHU框架，通过Top-K像素选择、MU-MIMO传输和Swin-large编码器，在降低85%通信开销的同时提升5%以上感知性能


<details>
  <summary>Details</summary>
Motivation: 多无人机协同感知面临海量视觉数据带来的通信延迟和资源效率挑战，需要通信高效的解决方案

Method: 采用Top-K机制选择信息量最大的像素进行稀疏化传输，通过MU-MIMO传输到地面服务器，使用Swin-large-based MaskDINO编码器提取BEV特征并进行协同融合，开发基于扩散模型的DRL算法联合优化无人机选择、稀疏化比率和预编码矩阵
...
TL;DR: 本文提出了一种将迁移学习与宽度学习系统（BLS）相结合的面部美感预测方法，通过EfficientNets提取特征并传输到BLS进行预测，有效解决了数据不足和过拟合问题。


<details>
  <summary>Details</summary>
Motivation: 面部美感预测面临两大挑战：1）缺乏大规模有效数据容易导致过拟合；2）面部外观多变和人类感知复杂性使得难以快速构建鲁棒有效的评估模型。迁移学习可以减少对大量数据的依赖并避免过拟合，而宽度学习系统能够快速完成模型构建和训练。

Method: 提出两种方法：1）E-BLS：基于迁移学习的CNN模型（使用EfficientNets）构建特征提取器，将提取的面部美感融合特征传输到BLS进行预测；2）ER-BLS：在E-BLS基础上，设计连接层将特征提取器与BLS连接起来。

Result: 实验结果表明，与现有的BLS和CNN方法相比，E-BLS和ER-BLS提高了面部美感预测的准确性，证明了所提方法的有效性和优越性。

Conclusion: 将迁移学习与宽度学习系统融合的面部美感预测方法不仅提高了预测精度，还能广泛应用于模式识别、目标检测和图像分类等领域。
...
TL;DR: 提出Script-to-Slide Grounding (S2SG)任务，利用LLM将脚本句子对应到幻灯片文本对象，实现自动化教学视频生成的第一步。


<details>
  <summary>Details</summary>
Motivation: 幻灯片视频在教育研究中广泛应用，但添加视觉效果的编辑过程非常耗时，需要自动化工具来简化这一过程。

Method: 提出Text-S2SG方法，使用大语言模型(LLM)将脚本句子对应到幻灯片中的文本对象，实现脚本到幻灯片的定位任务。

Result: 实验显示该方法达到高性能（F1分数：0.924），验证了LLM在此任务上的有效性。
...
TL;DR: AwaRes是一个空间按需视觉语言模型框架，通过低分辨率全局视图和工具调用检索高分辨率片段，解决了准确性与计算效率的权衡问题。
...
Result: 该方法在保持计算效率的同时提高了对细粒度视觉信息（如小文本）的识别准确性，解决了传统VLM中的分辨率权衡问题。

Conclusion: AwaRes框架通过空间按需处理机制，实现了视觉语言模型中准确性与计算效率的更好平衡，为需要细粒度视觉理解的任务提供了有效解决方案。
...
TL;DR: 该研究利用基础生成模型合成大规模面部行为数据集，通过语言模型实现面部运动的双向理解与生成：从3D面部参数生成语言描述，以及从文本提示生成面部运动序列。
...
Method: 1) 设计涵盖情感和头部动作的提示词套件，使用多个生成器合成约80小时面部视频；2) 提取每帧3D面部参数，构建大规模(提示词-参数)配对数据集；3) 通过两个互补任务训练语言模型：Motion2Language（从面部参数生成描述）和Language2Motion（从文本生成面部运动序列）。

Result: 实验表明，在该设置下语言模型能够很好地解释和合成面部运动，具有较强的泛化能力。这是首个将面部参数建模视为语言问题的工作。

Conclusion: 该研究建立了文本条件面部动画和运动理解的统一路径，首次将面部参数建模转化为语言问题，为文本引导的面部动画提供了新框架。
...
TL;DR: 本文提出了一种基于差异的多模态融合方法，通过显式测量视觉、音频和文本模态之间的冲突来识别视频中的矛盾/犹豫情绪，在BAH数据集上显著超越了基线性能。


<details>
  <summary>Details</summary>
Motivation: 针对第10届ABAW竞赛中的矛盾/犹豫视频识别挑战，需要有效处理多模态信息之间的不一致性，因为矛盾/犹豫情绪通常表现为不同模态之间的冲突或不协调。

Method: 提出基于差异的多模态融合方法：1) 视觉特征通过Py-Feat提取动作单元(AUs)；2) 音频特征通过Wav2Vec 2.0编码；3) 文本特征通过BERT编码。每个模态通过带注意力池化的BiLSTM处理并投影到共享嵌入空间，然后计算模态嵌入之间的绝对差异来显式捕捉跨模态冲突。

Result: 在BAH数据集验证集上获得Macro F1为0.6808，显著优于挑战基线0.2827。统计分析表明，动作单元(AUs)的时间变异性是识别矛盾/犹豫情绪的主要视觉判别因素。

Conclusion: 基于差异的多模态融合方法能有效捕捉矛盾/犹豫情绪特有的跨模态不一致性，动作单元的时间动态是关键的视觉线索，该方法在多模态情感识别中具有实用价值。
...
TL;DR: 本文提出了Omni IIE Bench基准测试，专门用于诊断指令式图像编辑模型在不同语义尺度任务上的编辑一致性，发现现有模型从低语义尺度向高语义尺度任务过渡时存在显著性能下降。


<details>
  <summary>Details</summary>
Motivation: 现有指令式图像编辑基准测试采用混合评估追求任务广度，但忽视了专业应用中一个关键失败模式：模型在不同语义尺度任务上的性能不一致性。需要专门诊断模型在实际应用场景中的编辑一致性。

Method: 提出了Omni IIE Bench基准测试，采用创新的双轨诊断设计：1）单轮一致性（包含属性修改和实体替换的共享上下文任务对）；2）多轮协调（涉及跨语义尺度的连续对话任务）。通过严格的多阶段人工筛选流程构建，包括计算机视觉研究生执行的质量标准和专业设计师进行的行业相关性审查。
...
TL;DR: 提出PhysQuantAgent框架用于真实世界物体质量估计，结合VisPhysQuant基准数据集，通过视觉提示方法提升VLM的质量推理能力


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在机器人感知和操作中应用广泛，但缺乏可靠的物理属性推理能力，特别是物体质量估计对于确定合适的抓握力和确保安全交互至关重要。现有基准大多未在真实感知条件下评估物理量估计。
...
Conclusion: 通过视觉提示增强VLM的输入可以显著改善物理属性推理能力，特别是物体质量估计，为机器人安全操作提供了重要技术支持。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统的批准依赖于在代表性真实交通场景中评估其行为。现有场景提取方法依赖异构定义，阻碍场景可比性；而场景聚类方法中，基于机器学习的方法缺乏可解释性且可能与领域知识不一致。

Method: 基于场景即规范概念实现标准化场景提取，开发领域知识引导的场景聚类流程，在高D数据集上进行实验验证。

Result: 实验证明场景可以被可靠地提取，领域知识可以有效地集成到聚类过程中，支持从高速公路数据记录中推导场景类别的标准化流程。
...
TL;DR: CineSRD：面向影视节目的多模态说话人注册与分割系统，解决开放世界说话人分割挑战


<details>
  <summary>Details</summary>
Motivation: 传统说话人分割系统主要针对会议、访谈等受限场景，而影视节目等开放世界场景存在说话人数量多、视听异步、环境复杂等挑战，需要新的解决方案

Method: 提出CineSRD统一多模态框架，利用视频、语音和字幕的视觉、声学和语言线索，先通过视觉锚点聚类注册初始说话人，再集成音频语言模型检测说话轮次，精炼标注并补充未注册的屏幕外说话人

Result: 在提出的视觉媒体说话人分割基准测试中表现优异，在传统数据集上也取得有竞争力的结果，验证了其在开放世界视觉媒体场景中的鲁棒性和泛化能力

Conclusion: CineSRD成功将说话人分割任务扩展到影视节目等开放世界视觉媒体领域，通过多模态融合有效解决了长视频理解、说话人数量多、视听异步等挑战，为复杂视听节目的说话人分析提供了有效解决方案
...
TL;DR: MAND是一个模态感知的多模态自我中心开放世界持续学习框架，通过自适应模态评分和模态特定表示稳定训练，显著提升新颖活动检测和已知类别分类性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖主logits进行新颖性评分，未能充分利用各模态的互补证据。由于RGB通常主导logits，IMU等其他模态线索被利用不足，且这种不平衡在灾难性遗忘下随时间恶化。

Method: 提出MAND框架：1) 推理时使用模态感知自适应评分(MoAS)，从能量分数估计样本级模态可靠性，自适应整合模态logits以更好利用互补模态线索进行新颖性检测；2) 训练时使用模态特定表示稳定训练(MoRST)，通过辅助头和模态特定logit蒸馏保持跨任务的模态特定判别性。

Result: 在公开多模态自我中心基准测试中，MAND将新颖活动检测AUC提升高达10%，已知类别分类准确率提升高达2.8%，优于现有最先进基线方法。

Conclusion: MAND通过模态感知方法有效解决了多模态自我中心开放世界持续学习中的模态不平衡问题，显著提升了新颖性检测和分类性能。
...
TL;DR: 提出Beyond Images自动数据增强管道，通过大规模图像检索、视觉内容文本化描述、多源描述融合生成实体对齐摘要，提升多模态知识图谱性能
...
Method: 三阶段管道：1) 大规模检索实体相关图像；2) 将所有视觉输入转换为文本描述，确保模糊图像贡献可用语义而非噪声；3) 使用大语言模型融合多源描述，生成简洁的实体对齐摘要。这些摘要替换或增强标准MMKG模型中的文本模态，不改变其架构或损失函数

Result: 在三个公共MMKG数据集和多个基线模型上观察到一致性能提升（总体Hits@1提升高达7%）。在具有视觉模糊标志和符号的实体子集上，将图像转换为文本带来显著改进（MRR提升201.35%，Hits@1提升333.33%）

Conclusion: 扩展图像覆盖范围并将模糊视觉内容转换为文本是提升MMKG完成性能的实用路径。同时发布了轻量级文本-图像一致性检查接口用于可选的有针对性审计
...
TL;DR: 本文针对资源受限环境下视觉语言模型(VLMs)部署的低延迟高吞吐需求，通过系统性能分析识别瓶颈，提出优化方案显著降低延迟，同时开发了ArgusVLM模型家族。


<details>
  <summary>Details</summary>
Motivation: 现有紧凑型视觉语言模型在推理速度上未能达到其较小参数量应有的加速效果，在资源受限环境中部署时需要低延迟和高吞吐量，但现有模型在这方面表现不足。

Method: 进行端到端效率分析，系统剖析推理过程识别主要瓶颈；基于发现开发针对紧凑型VLMs的优化方案；扩展紧凑型VLMs以支持结构化感知输出，构建ArgusVLM模型家族。

Result: 优化技术使InternVL3-2B的首令牌时间(TTFT)降低53%，SmolVLM-256M降低93%；ArgusVLM在多样化基准测试中表现优异，同时保持紧凑高效设计。

Conclusion: 提出的优化方案可广泛应用于不同VLM架构和常见服务框架，为构建高效VLM系统提供实用指导；ArgusVLM展示了紧凑型VLMs在保持高效性的同时实现强大性能的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在细粒度视觉语言推理方面存在不足，长链思维推理会暴露感知、推理、知识和幻觉等多种错误，而现有RLVR训练数据缺乏依赖视觉证据的复杂推理链，无法充分暴露这些弱点。

Method: 提出HopChain框架，合成多跳视觉语言推理数据用于VLMs的RLVR训练。每个多跳查询形成逻辑依赖的实例接地跳链，早期跳建立后续跳所需的实例、集合或条件，最终答案保持具体、明确的数字以便验证奖励。

Result: 将HopChain合成的多跳数据添加到原始RLVR数据中训练Qwen3.5模型，在24个基准测试（涵盖STEM与谜题、通用VQA、文本识别与文档理解、视频理解）中，20个测试表现提升。完整多跳查询比半多跳或单跳变体分别提高5.3和7.0个准确率点，在超长思维链推理中增益超过50个准确率点。

Conclusion: HopChain是合成多跳数据的有效、可扩展框架，能显著提升视觉语言模型的通用推理能力，多跳训练对增强长链视觉语言推理至关重要。
...
TL;DR: OpenQlaw是一个用于分析二维量子材料的智能编排系统，通过解耦视觉识别与推理过程，将专业的多模态大语言模型作为专门节点，为研究人员提供更自然、实用的交互体验。


<details>
  <summary>Details</summary>
Motivation: 现有领域特定的多模态大语言模型虽然能基于物理知识进行推理，但其输出过于冗长且注重认知透明度，导致认知过载，缺乏对研究人员实际交互的即时实用性。

Method: 基于NanoBot轻量级智能框架构建OpenQlaw系统，将QuPAINT物理感知多模态平台作为专门节点，核心大语言模型智能体编排领域专家MLLM，实现视觉识别与推理的解耦，并具备持久记忆功能。

Result: 系统能够动态处理用户查询，执行尺度感知的物理计算，生成隔离的视觉标注，并以自然方式回答，通过持久记忆保存物理尺度比例和样品制备方法，支持高效比较。
...
TL;DR: Astrolabe是一个针对蒸馏自回归视频模型的高效在线强化学习框架，通过前向过程RL公式和流式训练方案解决现有方法的内存和计算开销问题，显著提升生成质量。
...
Method: 1. 提出基于负感知微调的前向过程RL公式，通过对比推理端点的正负样本来建立隐式策略改进方向，无需反向过程展开。2. 为长视频对齐提出流式训练方案，通过滚动KV缓存逐步生成序列，仅在局部剪辑窗口应用RL更新，同时以前文为条件确保长程一致性。3. 集成多奖励目标，通过不确定性感知选择性正则化和动态参考更新来缓解奖励黑客问题。

Result: 大量实验表明，该方法在多个蒸馏自回归视频模型上一致地提升了生成质量，成为一个鲁棒且可扩展的对齐解决方案。

Conclusion: Astrolabe为蒸馏自回归视频模型提供了一个高效、可扩展的在线强化学习对齐框架，通过前向过程RL公式、流式训练方案和多奖励目标优化，有效解决了现有方法的计算瓶颈和奖励黑客问题。
...
TL;DR: PaAgent是一种基于多模态大语言模型的图像修复智能体，通过自演化的工具画像库和检索增强生成技术，能够更高效地选择适合的图像修复工具，特别是在复杂退化场景下表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有图像修复智能体通常缺乏对过去交互的洞察总结机制，导致需要穷举搜索最优修复工具，效率低下。为了解决这一问题，需要开发能够自动总结工具特性并智能选择工具的系统。

Method: 1. 构建自演化的工具画像库，通过总结修复图像、选择的修复工具和退化图像来丰富工具特性；2. 使用检索增强生成技术从画像库中检索相关信息来选择最优修复工具；3. 提出主客观强化学习策略，结合图像质量分数和语义洞察生成奖励，增强对复杂场景退化的感知能力。

Result: 在8个图像修复基准测试中，涵盖6种单一退化和8种混合退化场景，PaAgent在解决复杂图像修复任务方面表现出优越性。

Conclusion: PaAgent通过自演化的工具画像库和检索增强生成技术，有效解决了现有图像修复智能体缺乏洞察总结机制的问题，在复杂退化场景下能够更准确地选择修复工具，提升了图像修复的自动化水平。
...
TL;DR: 提出了一种使用RGB-D相机自动收集可变形物体3D数据集的低成本框架，通过识别3D关键点并加入运动一致性约束进行鲁棒跟踪，创建了包含6个物体、110分钟轨迹数据的大规模高质量数据集。
...
Method: 提出一种仅使用RGB-D相机的低成本自主框架，识别3D关键点并鲁棒跟踪其轨迹，通过加入运动一致性约束来产生时间平滑且几何一致的数据。

Result: TrackDeform3D在多种物体类别上与多个最先进的跟踪方法进行比较，在几何精度和跟踪精度方面均表现出持续改进。使用该框架创建了一个包含6个可变形物体、总计110分钟轨迹数据的高质量大规模数据集。

Conclusion: 该研究提供了一种经济有效的解决方案，用于收集可变形物体的3D数据，解决了现有数据收集方法的局限性，为下游任务（如动力学建模和运动规划）提供了有价值的资源。
...
TL;DR: 提出结合反事实生成与密集对比学习的DVD-CL和MVD-CL方法，用于图像分割任务，无需人工标注或利用AI生成标签，显著提升分割性能


<details>
  <summary>Details</summary>
Motivation: 图像分割依赖昂贵的人工标注数据集，而AI生成的银标准标签容易引入偏差。自监督学习需要大量图像但无需标注，但现有结合对比学习和反事实生成的方法主要针对分类任务，难以扩展到像素级任务

Method: 提出结合反事实生成与密集对比学习的双视图(DVD-CL)和多视图(MVD-CL)方法，以及利用银标准标注的监督变体。同时引入新的可视化算法CHRO-map（彩色高分辨率叠加图）

Result: 无标注的DVD-CL优于其他密集对比学习方法，使用银标准标签的监督变体比直接在银标准标注数据上训练表现更好，在挑战性数据上达到约94%的DSC

Conclusion: 通过反事实生成和银标准标注增强的像素级对比学习，能够提高对采集和病理变化的鲁棒性，为图像分割提供有效的预训练方法
...
TL;DR: 研究发现视觉语言模型家族内部存在相关性错误，导致集成效果受限，提出了三种家族感知方法显著提升性能


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型集成方法忽略了同一架构家族模型间的相关性错误，这种家族相关性会降低集成效果，甚至在某些问题上导致准确率降至0%
...
Result: 家族相关性错误使有效集成维度降至2.5-3.6个独立投票者，HFV在误导层恢复+18-26个百分点，QualRCCV在所有三个基准测试中首次击败校准投票，LCS获得最大提升：VQAv2 +0.68%、TextVQA +0.61%、GQA +2.45%，在VQAv2测试标准上达到87.83%

Conclusion: 视觉语言模型家族内部存在显著相关性错误，家族感知的集成方法能有效利用模型多样性，显著提升集成性能，其中学习候选评分方法表现最佳且稳定
...
TL;DR: MosaicMem是一种混合空间记忆方法，结合了显式和隐式记忆的优势，通过将图像块提升到3D空间实现可靠定位和针对性检索，同时保持模型的提示跟随生成能力。


<details>
  <summary>Details</summary>
Motivation: 视频扩散模型正从生成短片段发展为世界模拟器，需要保持相机运动、重访和干预下的一致性。现有方法存在局限性：显式3D结构能改善重投影一致性但难以描绘运动物体，而隐式记忆即使有正确姿态也常产生不准确的相机运动。

Method: 提出MosaicMem混合空间记忆，将图像块提升到3D空间实现可靠定位和针对性检索，通过patch-and-compose接口在查询视图中组合空间对齐的图像块，保留应持续的内容同时让模型修复应演变的部分。结合PRoPE相机条件和两种新的记忆对齐方法。

Result: 实验显示，相比隐式记忆有更好的姿态遵循性，相比显式基线有更强的动态建模能力。MosaicMem进一步实现了分钟级导航、基于记忆的场景编辑和自回归展开。

Conclusion: MosaicMem通过混合空间记忆方法有效解决了视频扩散模型中的空间记忆瓶颈，平衡了姿态遵循性和动态建模能力，为世界模拟器的发展提供了重要技术支撑。
...
TL;DR: BEV-SLD是一种基于场景地标检测的LiDAR全局定位方法，使用鸟瞰图图像发现场景特定模式作为地标，通过自监督学习实现鲁棒定位。


<details>
  <summary>Details</summary>
Motivation: 现有场景无关的定位方法可能无法充分利用场景特定信息，需要一种能够发现和利用场景特定模式作为地标的方法来提高全局定位的鲁棒性和准确性。

Method: 使用鸟瞰图图像在规定的空间密度下发现场景特定模式作为地标，通过一致性损失对齐可学习的全局地标坐标与每帧热图，确保跨场景的地标检测一致性。

Result: 在校园、工业和森林等多种环境中，BEV-SLD实现了鲁棒的定位性能，与最先进方法相比表现出色。

Conclusion: BEV-SLD通过自监督学习利用场景特定地标，提供了一种有效的LiDAR全局定位方法，在不同环境中都表现出良好的性能。
...
TL;DR: GazeOnce360是一个端到端模型，使用单个朝上鱼眼摄像头进行多人360度场景下的3D视线估计，通过双分辨率架构和旋转卷积处理鱼眼图像畸变。


<details>
  <summary>Details</summary>
Motivation: 传统视线估计方法依赖前向摄像头和受限视角，本文探索了从朝上鱼眼视角估计360度场景中多人3D视线方向这一未被充分研究的设置。

Method: 提出端到端模型，结合旋转卷积处理鱼眼畸变，使用眼标点监督，采用双分辨率架构融合全局低分辨率上下文和高分辨率局部眼部特征。

Result: 实验结果表明模型各组件均有效，证明了鱼眼摄像头在360度多人视线估计场景中的可行性和潜力。

Conclusion: 这项工作展示了基于鱼眼摄像头的360度视线估计在实际多人场景中的可行性，为相关研究提供了新方向。
...
TL;DR: 研究探索多模态大语言模型在隐私约束下进行虹膜呈现攻击检测的可行性，发现结合专家知识的提示工程能提升检测性能，超越传统CNN基线和人工检查。


<details>
  <summary>Details</summary>
Motivation: 虹膜呈现攻击检测面临实际挑战：无法收集未来未知攻击数据，收集多样化数据成本高昂，且生物识别数据共享存在隐私问题。由于新攻击向量快速出现需要适应性解决方案，研究在严格隐私约束下（禁止将生物识别数据发送到公共云服务）使用通用多模态大语言模型进行虹膜PAD的可行性。

Method: 使用预训练视觉变换器分析虹膜图像嵌入，发现MLLMs能自然聚类多种攻击类型。在聚类重叠的情况下，采用结合人类显著性的结构化提示（受试者识别的攻击指标口头描述）来消除歧义。在包含7种攻击类型、224张虹膜图像的IRB限制数据集上测试，仅使用大学批准服务（Gemini 2.5 Pro）或本地部署模型（如Llama 3.2-Vision）。

Result: Gemini结合专家知识提示的表现优于专门的CNN基线和人工检查员，而本地可部署的Llama模型达到接近人类的性能水平。预训练视觉变换器在未明确训练的情况下能自然聚类多种虹膜攻击类型。

Conclusion: 在机构隐私约束下可部署的多模态大语言模型为虹膜呈现攻击检测提供了可行路径，结合专家知识的提示工程能有效提升检测性能。
...
TL;DR: Patient4D：一种利用静止性先验的4D人体网格重建方法，专门针对手术增强现实场景中患者被手术单遮挡且相机连续移动的挑战性条件。


<details>
  <summary>Details</summary>
Motivation: 在手术增强现实场景中，麻醉患者被手术单遮挡，同时外科医生的头戴相机连续改变视角，这使得从单目视频恢复密集3D人体网格变得困难。现有的人体网格恢复方法通常针对直立、移动的主体和相对稳定的相机进行训练，在这种条件下性能会下降。

Method: 提出了Patient4D管道，结合图像级基础模型进行感知和轻量级几何机制来强制帧间时间一致性。两个关键组件：1) Pose Locking（姿态锁定）：使用稳定关键帧锚定姿态参数；2) Rigid Fallback（刚性回退）：通过轮廓引导的刚性对齐在严重遮挡下恢复网格。这些机制稳定预测同时保持与现有人体网格恢复模型的兼容性。

Result: 在4,680个合成手术序列和三个公共人体网格恢复视频基准上进行评估。在手术单遮挡下，Patient4D达到0.75的平均IoU，将失败帧从30.5%减少到1.3%，相比最佳基线方法有显著改进。

Conclusion: 利用静止性先验可以显著改善临床增强现实场景中的单目重建性能，Patient4D展示了在具有挑战性的手术条件下稳健重建的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 工业应用中可靠的产品识别至关重要，错误的匹配会导致昂贵的下游故障。当前需要评估现代视觉嵌入模型在真实工业环境中的实例级检索能力，为实践者和研究者提供参考。

Method: 采用统一的图像到图像检索协议，评估开源基础嵌入模型、专有多模态嵌入系统和领域特定视觉模型。使用工业数据集（制造、汽车、DIY、零售）和公共基准数据集，不进行后处理以隔离模型检索能力。

Result: 基准测试结果揭示了当代基础和统一嵌入模型在细粒度实例检索任务中的迁移能力，以及与专门为工业应用训练模型的比较表现，展示了各模型在真实约束和异构图像条件下的性能。

Conclusion: 该基准测试为实践者和研究者提供了当前视觉嵌入方法在生产级产品识别系统中的优势和局限性见解，有助于指导工业应用中的模型选择和系统设计。
...
<details>
  <summary>Details</summary>
Motivation: 多站点神经影像分析面临扫描仪引起的协变量偏移问题，即体素强度的边际分布随采集协议非线性变化，而解剖条件分布保持不变。这种偏移对影像组学可重复性特别有害，因为采集方差常常超过生物病理方差。现有方法要么在特征空间操作无法支持空间下游任务，要么受限于局部感受野无法建模全局强度相关性。

Method: 提出SA-CycleGAN-2.5D域适应框架，包含三个架构创新：1) 2.5D三平面流形注入，以O(HW)复杂度保持平面间梯度；2) U-ResNet生成器配备密集体素到体素自注意力，超越CNN的O(√L)感受野限制以建模全局扫描仪场偏置；3) 谱归一化判别器约束Lipschitz常数(K_D≤1)以实现稳定对抗优化。

Result: 在654名胶质瘤患者（BraTS和UPenn-GBM两个机构域）上评估，方法将最大均值差异(MMD)降低99.1%（从1.729降至0.015），域分类器准确率降至接近随机水平（59.7%）。消融实验证实全局注意力在异质到同质转换方向上具有统计显著性（Cohen's d=1.32, p<0.001）。

Conclusion: 通过结合2D效率和3D一致性，该框架生成体素级协调图像，保持肿瘤病理生理特征，实现可重复的多中心影像组学分析，有效解决扫描仪引起的协变量偏移问题。
...
TL;DR: EGS提出了一种基于强化学习的预算感知锚点采样器，用于动态场景高斯溅射重建，相比传统FPS采样，在显著减少锚点数量的同时保持甚至提升重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有动态场景高斯溅射重建方法通常使用固定数量的锚点（如8192个），无论场景复杂度如何，这导致在严格计算预算下过度分配计算资源，效率低下。

Method: 提出EGS（高效高斯流式处理），这是一个基于强化学习的预算感知锚点采样器，可替代传统的FPS采样。该策略联合选择锚点预算和信息丰富的锚点子集，利用高斯表示的空间特征来平衡重建质量和运行时间。

Result: 在动态多视角数据集上，EGS在质量-效率权衡方面始终优于FPS采样。在快速渲染模式下（仅256个锚点，比8192少32倍），PSNR提升0.52-0.61dB，运行速度比IGS@8192快1.29-1.35倍。在高质量优化模式下，EGS在显著降低锚点预算的情况下仍能与完整锚点基线竞争。

Conclusion: EGS作为一种即插即用的预算感知锚点采样器，能够有效替代传统FPS采样，在动态场景高斯溅射重建中实现更好的质量-效率平衡，特别适用于实时渲染和自由视点视频应用。
...
TL;DR: 该研究通过分层线性探测评估发现，MLLMs在分割任务中，适配器会导致表征质量下降，但LLM层通过注意力机制逐步恢复分割能力，正确分类的token能够引导错误分类的邻居token。


<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型（MLLMs）越来越多地应用于像素级视觉任务，但其内在的空间理解能力仍未被充分理解。研究者希望探究MLLMs在分割任务中的工作机制。

Method: 采用分层线性探测评估方法，分析整个MLLM流程（视觉编码器、适配器、LLM）。通过基于干预的注意力敲除分析测试交叉token注意力是否逐步细化视觉表征，并评估图像token间双向注意力对空间一致性的影响。

Result: 研究发现：1）适配器引入分割表征下降；2）LLM层通过注意力介导的细化逐步恢复，正确分类的token能够引导错误分类的邻居token；3）早期图像token位置的恢复受因果注意力限制，而图像token间的双向注意力能够缓解这一问题。

Conclusion: 这些发现为MLLMs如何处理视觉信息进行分割提供了机制性解释，为未来设计具有分割能力的模型提供了指导。
...
TL;DR: GigaWorld-Policy是一种基于世界-动作模型的动作中心化机器人策略学习方法，通过解耦动作预测和视频生成，实现了9倍推理速度提升和7%任务成功率提高。
...
Method: 提出动作中心化的WAM，学习2D像素-动作动态，支持高效动作解码。将策略训练分为两个耦合组件：基于当前观测预测未来动作序列，同时基于预测动作和相同观测生成未来视频。采用因果设计防止未来视频令牌影响动作令牌。

Result: 在真实机器人平台上，GigaWorld-Policy比领先的WAM基线Motus运行速度快9倍，任务成功率提高7%。相比pi-0.5，在RoboTwin 2.0上性能提升95%。

Conclusion: GigaWorld-Policy通过动作中心化设计和因果架构，解决了现有WAM的效率和性能瓶颈，实现了高效且准确的机器人策略学习，推理时可选择性地生成视频以加速动作预测。
...
TL;DR: 提出Layout Error Detection (LED)基准，用于评估文档布局分析中的结构错误检测能力，超越传统基于重叠的评估指标


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型和多模态模型在文档布局分析方面有所改进，但区域合并、分割和遗漏等结构错误仍然存在。传统的基于重叠的评估指标（如IoU、mAP）无法捕捉这些逻辑不一致性

Method: 定义了八种标准化错误类型（缺失、幻觉、尺寸错误、分割、合并、重叠、重复和错误分类），提供了定量规则和注入算法进行真实错误模拟。构建了LED-Dataset，设计了三个评估任务：文档级错误检测、文档级错误类型分类和元素级错误类型分类
...
TL;DR: 本文提出ConfusionBench，一个用于教育视频中识别和定位学生困惑的高质量基准数据集，通过多阶段筛选流程构建，包含平衡的困惑识别数据集和视频定位数据集，并提供了零样本基线评估。
...
Method: 提出实用的多阶段筛选流程，整合模型辅助筛选、研究者筛选和专家验证三个阶段，构建ConfusionBench基准数据集，包含平衡的困惑识别数据集和视频定位数据集。

Result: 实验结果显示，专有模型整体表现更好但倾向于过度预测过渡片段，开源模型更保守但更容易漏检。提出的学生困惑报告可视化可支持教育专家做出干预决策。

Conclusion: 通过多阶段筛选流程构建的ConfusionBench为教育视频中的困惑理解提供了更高质量的基准，所有数据集和相关材料将公开提供。
...
TL;DR: 提出两阶段框架解决故事可视化中的角色一致性问题：1）组共享注意力机制实现跨帧无损信息流；2）直接偏好优化对齐人类审美标准，在ViStoryBench基准上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有故事可视化方法在角色一致性和身份漂移方面存在不足，特别是在复杂交互和长叙事弧中。需要一种能保持严格角色身份和视觉风格一致性的解决方案。

Method: 提出两阶段框架：1）组共享注意力机制，在注意力层内实现无损跨样本信息流，结构性地编码跨帧身份对应关系；2）直接偏好优化，从整体偏好数据中学习，同时提升视觉保真度和身份保持。

Result: 在ViStoryBench基准上建立新的SOTA，显著优于强基线：角色身份一致性提升+10.0，风格一致性提升+18.7，同时保持高保真生成质量。

Conclusion: 该框架通过组共享注意力机制和直接偏好优化，有效解决了故事可视化中的角色一致性问题，实现了语义对齐、身份一致和高保真生成的三重目标。
...
<details>
  <summary>Details</summary>
Motivation: 当前MLLM智能体在长视频理解任务中面临挑战，这些任务具有高信息密度和长时域跨度。简单的任务分解和协作机制不足以处理长链推理任务，而基于嵌入的检索方法可能会丢失复杂问题的关键信息。

Method: Symphony采用多智能体系统，模拟人类认知模式：1）将长视频理解分解为细粒度子任务；2）引入基于反思的深度推理协作机制；3）提供基于VLM的接地方法分析任务并评估视频片段相关性，以定位具有隐含意图和大时域跨度的复杂问题。

Result: 实验结果显示，Symphony在LVBench、LongVideoBench、VideoMME和MLVU等基准测试中达到最先进性能，在LVBench上比先前最优方法提升5.0%。
...
TL;DR: R²VLM：一种用于长时程任务进度估计的循环推理视觉语言模型，通过迭代处理视频片段并维护全局思维链，在保持推理能力的同时降低计算成本。
...
Result: 在ALFRED和Ego4D自动生成的大规模数据集上训练，在进度估计及下游应用（包括进度增强策略学习、强化学习奖励建模和主动辅助）中表现优异，达到长时程任务进度估计的最新水平。

Conclusion: R²VLM通过循环推理框架有效解决了长视频处理的计算瓶颈，同时保持了复杂推理能力，在任务进度估计和相关应用中实现了最先进的性能。
...
TL;DR: MedSAD-CLIP：一种基于CLIP的医学异常检测与分割方法，通过Token-Patch交叉注意力机制和轻量级适配器，在有限标注数据下实现精确的病变定位和分割。


<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的医学异常检测方法通常依赖全局表示和弱监督，导致定位粗糙、分割质量有限。本研究旨在在临床实际场景中（有限但有意义的标注异常数据可用），通过监督方式适配CLIP以提升医学异常检测性能。

Method: 提出MedSAD-CLIP模型，采用Token-Patch交叉注意力机制（TPCA）利用细粒度文本-视觉线索改善病变定位；使用轻量级图像适配器和可学习提示令牌高效适配预训练CLIP编码器；设计基于边界的图像-文本对比损失增强正常与异常全局特征区分度。

Result: 在脑部、视网膜、肺部和乳腺四个多样化基准数据集上的实验表明，该方法在像素级分割和图像级分类任务上均优于现有最先进方法，展示了监督CLIP适配作为统一可扩展医学异常理解范式的潜力。

Conclusion: MedSAD-CLIP通过细粒度文本-视觉对齐和高效域适配，在有限标注数据下实现了优异的医学异常检测与分割性能，为临床诊断提供了有效的辅助工具。
...
TL;DR: OnlineHMR是一个完全在线的三维人体网格恢复框架，能够从单目视频实时重建世界坐标系下的人体轨迹和运动，满足在线处理的四个关键标准。
...
Conclusion: OnlineHMR是一个完全在线的框架，能够实时从单目视频中恢复三维人体网格和世界坐标轨迹，填补了现有离线方法的局限性，为交互式应用提供了可行的解决方案。
...
TL;DR: 该论文提出了一个用于资产检查3D重建的新数据集，包含合成场景、模拟检查轨迹和复杂表面条件，评估了现有方法在此领域的局限性。


<details>
  <summary>Details</summary>
Motivation: 资产检查需要高精度的3D模型来支持维护、维修和评估工作，但现有3D重建数据集缺乏资产检查特有的密集捕获轨迹和复杂表面条件（如反射、透明），难以评估相关方法。

Method: 创建了一个包含三个合成场景的新数据集，提供真实深度图、相机位姿和网格模型，模拟了资产检查的飞行轨迹，并包含非朗伯表面和不同表面条件变化。

Result: 评估了最先进的3D重建方法，结果显示当前方法在处理密集捕获轨迹和复杂表面条件方面存在显著困难，暴露了可扩展性差距。

Conclusion: 该数据集揭示了当前3D重建方法在资产检查领域的局限性，指出了可部署3D重建技术的新研究方向，为解决实际应用中的挑战提供了基准。
...
TL;DR: 本文提出MCoT-MVS方法，通过多模态思维链推理和多级视觉选择来改进组合图像检索，在CIRR和FashionIQ基准上达到SOTA性能。
...
Method: 提出MCoT-MVS框架：1) 利用多模态大语言模型对组合输入进行思维链推理，生成保留、移除和目标推断文本；2) 基于这些文本线索指导两个参考视觉注意力选择模块，提取参考图像的判别性补丁级和实例级语义；3) 设计加权分层组合模块，将多粒度视觉线索与修改文本及想象的目标描述融合，在统一嵌入空间中对齐组合查询与目标图像。

Result: 在CIRR和FashionIQ两个组合图像检索基准上的广泛实验表明，该方法持续优于现有方法，实现了新的最先进性能。

Conclusion: 通过多模态思维链推理和多级视觉选择机制，MCoT-MVS能够更准确地提取反映用户意图的视觉语义线索，有效减少视觉噪声干扰，从而显著提升组合图像检索的性能。
...
TL;DR: 提出Material Magic Wand工具，用于在无纹理网格中基于材料属性自动分组相似部件，减少手动选择重复结构的工作量。


<details>
  <summary>Details</summary>
Motivation: 许多真实世界形状（如松果鳞片、建筑窗户）包含具有相同材料但几何变化的重复结构。为这些网格分配材料时，需要逐个手动识别和选择重复部件，过程繁琐耗时。

Method: 提出Material Magic Wand工具，核心是部件编码器，为每个3D部件生成材料感知嵌入，同时考虑局部几何和全局上下文。使用监督对比损失训练模型，使相同材料的部件嵌入更接近，不同材料的嵌入更分离，从而实现通过检索相似嵌入进行部件分组。

Result: 构建了包含100个形状和241个部件级查询的基准数据集。通过大量实验验证了方法的有效性，并在交互式材料分配应用中展示了实用价值。

Conclusion: Material Magic Wand能够有效解决无纹理网格中的材料感知部件分组问题，显著减少艺术家在材料分配过程中的手动工作量。
...
TL;DR: 该研究发现视觉语言模型的安全对齐在视觉模态加入后减弱，图像能显著提高越狱成功率。研究识别出越狱方向，提出通过移除越狱相关偏移来防御的方法。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在集成视觉模态后表现出安全对齐减弱的问题，即使文本提示包含明确的恶意意图，添加图像也能显著提高越狱成功率。这引发了关于视觉模态如何影响模型安全机制的研究动机。

Method: 研究发现VLMs能在表示空间中清晰区分良性输入和有害输入，并识别出越狱样本形成的独特内部状态。通过识别越狱方向，定义越狱相关偏移作为图像诱导表示偏移沿该方向的分量。最后提出JRS-Rem防御方法，在推理时移除越狱相关偏移。

Result: 分析表明越狱相关偏移能可靠地表征越狱行为，为多样化的越狱场景提供统一解释。实验证明JRS-Rem方法在多种场景下提供强大的防御能力，同时在良性任务上保持性能。

Conclusion: 越狱并非源于无法识别有害意图，而是视觉模态将表示向特定越狱状态转移，导致无法触发拒绝机制。通过移除越狱相关偏移可以有效增强VLM的安全性。
...
TL;DR: InfoShot：一种面向长视频理解的镜头感知帧采样方法，通过信息论目标选择互补关键帧，平衡视频整体覆盖与关键短暂事件


<details>
  <summary>Details</summary>
Motivation: 现有帧采样方法在只能保留少量帧时，难以平衡广泛的视频覆盖与短暂但关键的事件，导致下游预测不可靠

Method: 首先将视频分割成语义一致的镜头，然后从每个镜头中选择两个互补关键帧：一个代表主要内容，一个捕捉镜头内的异常变化；基于信息论目标设计

Result: 在帧数限制下提高了异常检测命中率和下游视频问答准确率，在标准视频理解基准测试中匹配或优于强基线方法

Conclusion: InfoShot是一种任务无关的镜头感知帧采样器，无需重新训练即可有效保留视频整体上下文和决策关键时刻，为长视频理解提供了有效的帧采样解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有方法要么是单目RGB或RGBD方法，要么需要将单目生成结果转换为立体格式，存在立体一致性差、计算效率低等问题。StereoWorld旨在直接在RGB模态中学习双目几何，实现更高效、更一致的立体视频生成。

Method: 1. 统一的相机帧RoPE：通过相机感知的旋转位置编码增强潜在标记，实现相对、视角和时间一致的调节，同时通过稳定的注意力初始化保留预训练视频先验。2. 立体感知注意力分解：将完整的4D注意力分解为3D视图内注意力加水平行注意力，利用极线先验以更低的计算成本捕捉视差对齐的对应关系。

Result: 在基准测试中，StereoWorld相比强大的单目转换流程，在立体一致性、视差精度和相机运动保真度方面均有提升，生成速度提升3倍以上，视角一致性额外提升5%。此外，该模型支持无需深度估计或修复的端到端双目VR渲染，通过度量尺度深度基础增强具身策略学习，并兼容长视频蒸馏以实现扩展的交互式立体合成。

Conclusion: StereoWorld通过联合学习外观和双目几何，在RGB模态内实现了高效、一致的立体视频生成。其创新的相机帧RoPE和注意力分解方法不仅提升了性能，还降低了计算成本，为立体视觉生成、VR渲染和具身AI应用提供了新的可能性。
...
TL;DR: 提出一个利用基础模型解决材料分类数据稀缺问题的新框架，通过图像生成自动标注管道和先验知识融合策略，显著提升材料识别性能。


<details>
  <summary>Details</summary>
Motivation: 材料分类在计算机视觉和图形学中至关重要，但传统方法面临标注数据稀缺的挑战，限制了模型的准确性和泛化能力。尽管视觉语言基础模型提供了新途径，但现有解决方案在材料识别任务中效果仍不理想。

Method: 提出包含两个关键创新的框架：(1) 鲁棒的图像生成和自动标注管道，创建多样化的材料中心图像训练数据集，通过融合对象语义和材料属性的文本提示自动分配标签；(2) 先验知识融合策略，从视觉语言模型中提取信息，结合联合微调方法优化预训练视觉基础模型和VLM先验，在保持广泛泛化能力的同时适应材料特定特征。

Result: 在多个数据集上的广泛实验显示出显著改进。合成数据集有效捕捉了真实世界材料的特征，视觉语言模型先验知识的整合显著提升了最终性能。

Conclusion: 提出的框架有效解决了材料分类中的数据限制问题，通过结合图像生成自动标注和基础模型先验知识，显著提高了分类准确性，为材料识别任务提供了有效的解决方案。
...
TL;DR: 提出了一种利用手势语义作为归纳偏置的两阶段3D手部姿态估计框架，通过手势感知预训练和关节令牌Transformer提升单手势估计精度
...
Result: 在InterHand2.6M数据集上的实验表明，手势感知预训练相比最先进的EANet基线持续提升单手势估计精度，且这种优势可以在不修改架构的情况下跨模型迁移

Conclusion: 手势语义可以作为有效的归纳偏置提升3D手部姿态估计性能，提出的两阶段框架通过手势感知预训练和关节令牌Transformer实现了精度提升，且该方法的优势具有架构无关性
...
TL;DR: 提出一种基于运动自适应时间注意力机制的参数高效视频生成方法，在冻结的Stable Diffusion模型上仅添加少量参数即可实现高质量视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法通常对所有视频内容采用统一的时间注意力机制，忽略了不同运动幅度序列的差异需求。高运动序列需要局部帧间注意力来保留快速变化的细节，而低运动序列需要全局注意力来保持场景一致性。

Method: 1) 运动自适应时间注意力机制：根据估计的运动内容动态调整时间注意力感受野；2) 级联注入策略：在下采样和中间块中使用全局注意力进行语义稳定，在上采样块中使用运动自适应注意力进行细粒度优化；3) 时间相关噪声初始化和运动感知门控；4) 仅使用标准去噪目标，无需额外时间一致性损失。

Result: 仅添加25.8M可训练参数（占基础UNet的2.9%），在WebVid验证集上取得有竞争力的结果。消融研究揭示了噪声相关性与运动幅度之间的权衡关系，为推理时控制提供了实用方法。

Conclusion: 运动自适应时间注意力机制能够有效平衡视频生成中的细节保留和场景一致性，标准去噪目标本身已提供足够的时间正则化，无需额外损失函数。该方法参数效率高，为视频生成提供了实用的推理控制手段。
...
TL;DR: ASSR-EIC：基于任意尺度超分辨率的可变比特率极端图像压缩框架，通过单一模型支持灵活比特率控制


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型极端图像压缩方法需要为每个目标比特率训练单独模型，计算开销大；而联合超分辨率方法在超低比特率下因信息丢失严重而表现不佳，且固定超分辨率尺度无法适应不同比特率需求

Method: 提出ASSR-EIC框架：编码器端引入任意尺度下采样模块实现可控比特率压缩；解码器端采用基于扩散的联合退化感知任意尺度超分辨率解码器，利用压缩和重缩放感知扩散先验指导重建。设计了全局压缩-重缩放适配器提供整体比特率适应指导，局部压缩-重缩放调制器动态平衡生成性和保真度行为，实现细粒度比特率自适应细节恢复。还引入了双重语义增强设计

Result: 实验表明ASSR-EIC在极端图像压缩中达到最先进性能，同时支持灵活的比特率控制和自适应比特率相关重建

Conclusion: ASSR-EIC成功解决了现有方法的局限性，通过任意尺度超分辨率实现了可变比特率极端图像压缩，在单一模型中实现了高质量、高真实感的图像重建，具有实际部署价值
...
TL;DR: 论文提出针对指代图像分割中运动相关查询性能不足的问题，通过数据增强和多模态径向对比学习来提升模型对运动描述的理解能力，并建立了新的运动中心测试基准。


<details>
  <summary>Details</summary>
Motivation: 现有指代图像分割方法在处理运动相关查询时表现显著低于外观相关查询，需要专门解决模型对运动描述理解不足的问题。

Method: 1. 提出高效的数据增强方案，从原始描述中提取运动中心短语，让模型接触更多运动表达；2. 提出多模态径向对比学习，在融合的图像-文本嵌入上进行对比学习而非单模态表示。

Result: 方法在多个RIS模型上显著提升了运动相关查询的性能，同时在外观描述上保持竞争力。建立了新的运动中心测试分割和M-Bench基准。

Conclusion: 通过专门的数据增强和对比学习策略，有效解决了指代图像分割中运动相关查询的性能瓶颈问题，为运动中心查询提供了新的评估基准。
...
Method: 提出基于像素通量动态的像素运动奖励，捕捉瞬时和长期运动一致性；设计Smooth Hybrid Fine-tuning (SHIFT)框架，融合监督微调和优势加权微调，利用新颖的对抗优势提高收敛速度并防止奖励攻击

Result: 实验表明该方法能有效解决现代视频扩散模型监督微调中的动态度崩溃问题

Conclusion: SHIFT框架通过像素运动奖励和混合微调策略，显著提升了视频扩散模型的运动对齐能力，解决了微调后运动保真度下降的核心问题
...
TL;DR: 论文提出ECHO框架，解决交互式头部生成中面部行为缺乏上下文适当性和唇部同步受损的问题，通过长程上下文理解和空间感知解耦交叉注意力模块提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有交互式头部生成方法存在两个主要问题：1) 依赖短片段行为线索，缺乏长程上下文建模，导致生成的面部行为缺乏上下文适当性；2) 双轨信号（用户行为和预定义音频）的纠缠融合导致交叉信号干扰，可能损害说话时的唇部区域同步。

Method: 提出ECHO框架，包含两个关键组件：1) 长程上下文理解组件，促进行为基础动态和语言驱动情感语义的上下文理解；2) 块状空间感知解耦交叉注意力模块，在保持自音频驱动唇部发音的同时，自适应整合用户上下文行为线索用于非唇部面部区域，配合两阶段训练范式。
...
TL;DR: FACE-net是一个检索增强的情感视频描述框架，通过事实校准和情感增强来解决事实-情感偏置问题，使用统一架构协同挖掘事实-情感语义并提供自适应生成指导。


<details>
  <summary>Details</summary>
Motivation: 现有情感视频描述方法在全局情感线索感知和视频内容结合方面存在不足，难以处理事实-情感偏置问题，即不同样本在生成时对事实和情感需求不同。

Method: 提出检索增强框架FACE-net：1) 引入外部知识库检索相关句子增强语义信息；2) 通过不确定性估计进行事实校准，将检索信息拆分为主谓宾三元组并通过自/交叉精炼挖掘事实语义；3) 渐进式视觉情感增强模块利用校准后的事实语义作为专家，与视频内容和情感词典交互生成视觉查询和候选情感；4) 动态偏置调整路由模块预测和调整样本偏置程度。

Result: 该方法能够有效挖掘事实-情感语义，为生成提供自适应和准确的指导，突破了传统方法在所有样本学习中事实-情感描述的折衷倾向。

Conclusion: FACE-net通过统一的检索增强架构，协同处理事实和情感语义，有效解决了情感视频描述中的事实-情感偏置问题，提高了描述的质量和适应性。
...
Method: 提出AR-CoPO框架，将Neighbor GRPO对比视角适配到流式AR生成。引入分块级对齐机制：在随机选择的分块处构建邻域候选，分配序列级奖励，执行局部GRPO更新。并提出半在线策略训练策略，结合在线探索和参考轨迹回放缓冲区的利用。

Result: 在Self-Forcing上的实验表明，AR-CoPO在领域外泛化和领域内人类偏好对齐方面均优于基线，提供了真正对齐而非奖励黑客的证据。

Conclusion: AR-CoPO成功解决了流式自回归视频生成器在强化学习对齐中的特殊挑战，通过创新的分块级对齐和半在线训练策略实现了更好的生成质量和偏好对齐。
...
TL;DR: 该论文提出VirPro方法，通过视觉引导的概率提示学习增强弱监督单目3D目标检测，利用自适应提示库和多高斯提示建模捕捉场景感知表示，在KITTI基准上实现4.8%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 传统伪标签方法依赖真实标注，现有基于确定性语言提示的方法难以捕捉场景中个体的视觉多样性，限制了模型学习场景感知表示的能力。

Method: 提出视觉引导的概率提示学习(VirPro)，包含自适应提示库(APB)存储可学习的实例条件提示，多高斯提示建模(MGPM)将视觉特征融入文本嵌入以表达视觉不确定性，通过RoI级对比匹配增强模态对齐。

Result: 在KITTI基准上的大量实验表明，集成该预训练范式能带来显著性能提升，相比基线平均精度提升最高达4.8%。

Conclusion: VirPro方法通过自适应多模态预训练有效增强弱监督单目3D检测，能够捕捉场景视觉多样性并提升语义一致性，可无缝集成到多种现有框架中。
...
TL;DR: UniSAFE是首个针对统一多模态模型的系统级安全评估基准，涵盖7种I/O模态组合，包含6802个实例，揭示了多模态模型在多图像组合和多轮对话设置中的安全漏洞


<details>
  <summary>Details</summary>
Motivation: 统一多模态模型虽然具有强大的跨模态能力，但带来了单任务模型未观察到的安全风险。现有安全基准在任务和模态上碎片化，无法全面评估复杂系统级漏洞

Method: 开发UniSAFE基准，采用共享目标设计，将常见风险场景映射到特定任务的I/O配置中，支持跨任务安全失效的受控比较。包含6802个精心策划的实例

Result: 评估15个最先进的专有和开源UMMs，发现当前UMMs存在关键漏洞：多图像组合和多轮设置中的安全违规率更高，图像输出任务比文本输出任务更易受攻击

Conclusion: 研究结果强调了UMMs需要更强的系统级安全对齐。UniSAFE为全面评估多模态模型安全提供了重要工具，代码和数据已公开
...
Method: 创建包含1080个样本的基准测试，涵盖科学可视化到复杂符号表示等多种数字内容，每个样本都配有可执行参考代码。评估框架将性能解耦为感知保真度和符号精度两个维度。

Result: 评估显示领先的大型多模态模型之间存在显著性能差距，即使是当前最先进的模型在复杂场景中也难以保持结构完整性，表明多模态代码生成仍然是一个艰巨挑战。

Conclusion: Omni-I2C基准测试揭示了大型多模态模型在将复杂结构化图形转换为可执行代码方面的局限性，为未来模型改进提供了重要评估工具和方向。
...
TL;DR: 提出早期干预框架和混合低秩适应方法，解决多模态医学影像疾病识别中的信息融合不足和标注数据稀缺问题


<details>
  <summary>Details</summary>
Motivation: 当前多模态医学影像疾病识别方法面临两大挑战：1) 主流的"单模态嵌入后融合"范式无法充分利用多模态数据的互补和关联信息；2) 标注多模态医学图像稀缺且与自然图像存在显著领域偏移，阻碍了视觉基础模型在医学图像嵌入中的应用

Method: 提出早期干预框架：将一个模态作为目标，其余作为参考，利用参考模态的高层语义标记作为干预标记，在早期阶段引导目标模态的嵌入过程。同时提出混合低秩适应方法：使用一组不同秩的低秩适配器和权重松弛路由器，实现参数高效的视觉基础模型适应

Result: 在视网膜疾病、皮肤病变和膝关节异常分类三个公共数据集上的广泛实验验证了所提方法相对于多个竞争基线的有效性

Conclusion: 提出的早期干预框架和混合低秩适应方法能够有效解决多模态医学影像疾病识别中的信息融合不足和标注数据稀缺问题，在多种疾病分类任务上表现出优越性能
...
Result: 在ScanNet和Replica数据集上，UniSem在深度预测和开放词汇3D分割方面表现优异。使用16个输入视图时，相比强基线方法，深度Rel降低15.2%，开放词汇分割mAcc提高3.7%

Conclusion: UniSem通过联合优化深度精度和语义泛化，解决了稀疏无位姿图像3D重建中的几何不稳定和语义不完整问题，为语义感知的3D重建提供了有效解决方案
...
Method: 提出并行成本聚合范式PCA-Seg，包含专家驱动感知学习模块（多专家解析器提取多视角互补特征，系数映射器自适应学习像素特定权重），以及特征正交化解耦策略减少语义流和上下文流之间的冗余。

Result: 在8个基准测试上验证，PCA-Seg每个并行块仅增加0.35M参数，但实现了最先进的开放词汇语义和部件分割性能。

Conclusion: 并行成本聚合范式能有效缓解类别语义与空间上下文的知识干扰问题，使模型能从成本体积中捕获更丰富的视觉-语言对齐信息，为开放词汇分割任务提供了简单而有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 开集语义分割在遥感领域具有巨大潜力，但现有方法主要局限于晴朗天气的光学数据，在云层或雾霾污染条件下表现不佳。光学图像提供丰富的光谱语义信息，而合成孔径雷达（SAR）具有云层穿透能力，两者的互补优势为解决恶劣天气条件下的分割问题提供了可能。

Method: 提出MM-OVSeg多模态光学-SAR融合框架，包含两个关键设计：1）跨模态统一过程，用于多传感器表示对齐；2）双编码器融合模块，集成来自多个视觉基础模型的分层特征，实现文本对齐的多模态分割。该框架利用光学图像的丰富光谱语义和SAR的云层穿透结构线索。

Result: 大量实验表明，MM-OVSeg在不同云层条件下实现了优越的鲁棒性和泛化能力。该方法在恶劣天气条件下的开集语义分割任务中表现出色。

Conclusion: MM-OVSeg通过多模态光学-SAR融合有效解决了恶劣天气条件下的开集语义分割问题，为遥感领域的鲁棒分割提供了新思路。源代码和数据集已公开。
...
TL;DR: 提出Rel-Zero关系零水印框架，利用图像块间关系距离在AI编辑中的不变性，实现无需修改原图的内容认证，显著提升抗编辑鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 扩散模型等AI编辑技术威胁数字内容真实性。传统嵌入水印方法为保持鲁棒性会引入可见扰动，损害视觉保真度；现有零水印方法依赖全局特征，难以抵抗复杂编辑操作。

Method: 基于关键发现：AI编辑中单个图像块会发生显著变化，但块对之间的相对距离关系保持相对不变。利用这一特性，从编辑不变的关系距离中提取唯一零水印，无需修改原始图像。

Result: 大量实验表明，Rel-Zero相比现有零水印方法，在各种编辑模型和操作下都实现了显著提升的鲁棒性。
...
TL;DR: 视频监督微调（Video-SFT）能提升MLLMs的视频理解能力，但会损害静态图像性能，这种权衡与时间预算密切相关。


<details>
  <summary>Details</summary>
Motivation: 研究视频监督微调如何影响多模态大语言模型的视觉能力演化，特别是空间理解与时间理解之间的平衡关系。

Method: 系统研究Video-SFT在不同架构、参数规模和帧采样设置下对MLLMs的影响，分析时间预算与性能权衡的关系，并提出指令感知的混合帧策略。
...
TL;DR: ProGVC是一个基于渐进式传输的生成式视频压缩框架，通过多尺度残差令牌映射实现灵活码率适配，将渐进传输、高效熵编码和细节合成统一在单一编解码器中。


<details>
  <summary>Details</summary>
Motivation: 现有感知视频编解码器缺乏对可变码率和渐进传输的原生支持，其生成模块与熵编码耦合较弱，限制了码率降低。受视觉自回归模型中下一尺度预测的启发，需要开发统一渐进传输、高效熵编码和细节合成的框架。

Method: 提出ProGVC框架：1) 将视频编码为层次化多尺度残差令牌映射；2) 支持通过传输粗到细的尺度子集进行渐进式传输；3) 使用基于Transformer的多尺度自回归上下文模型估计令牌概率；4) 该模型既用于传输令牌的高效熵编码，也用于在解码器预测截断的细尺度令牌以恢复感知细节。

Result: 大量实验表明，ProGVC作为一种新的编码范式，在低码率下提供了有前景的感知压缩性能，同时提供了实用的可扩展性。

Conclusion: ProGVC成功地将渐进传输、高效熵编码和细节合成统一在单一编解码器中，实现了灵活码率适配和低码率下的良好感知压缩性能，为视频压缩提供了新的编码范式。
...
TL;DR: FrescoDiffusion：一种无需训练的方法，用于从单张复杂图像生成连贯的大格式图像到视频（I2V），通过增强分块去噪与预计算的潜在先验来解决4K超高清分辨率下的全局布局一致性问题。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散式图像到视频模型在处理超高清分辨率输入（如4K）时面临挑战：在模型原生分辨率下生成会丢失细节，而高分辨率分块去噪虽保留局部细节但破坏全局布局一致性。这在壁画动画场景中尤为严重，因为壁画包含许多不同角色、物体和语义不同的子场景，需要在时间上保持空间连贯性。

Method: FrescoDiffusion引入了一种无需训练的方法，通过增强分块去噪与预计算的潜在先验。首先在底层模型分辨率下生成低分辨率视频，并将其潜在轨迹上采样以获得捕捉长程时空结构的全局参考。对于4K生成，计算每个分块的噪声预测，并通过最小化模型输出空间中的加权最小二乘目标，在每个扩散时间步将这些预测与全局参考融合。该方法还提供了空间正则化变量，实现对运动区域的区域级控制。

Result: 在VBench-I2V数据集和提出的壁画I2V数据集上的实验表明，与分块基线相比，FrescoDiffusion在全局一致性和保真度方面有显著提升，同时计算效率高。正则化方法使得在创造性和一致性之间的权衡具有明确的可控性。

Conclusion: FrescoDiffusion通过结合预计算的潜在先验和分块去噪，有效解决了大格式图像到视频生成中的全局一致性难题，特别适用于壁画动画等复杂场景，在保持细节的同时确保了时空连贯性。
...
TL;DR: 提出一种特征保留的人脸匿名化框架，通过密集面部关键点保留表情，并通过轻量后处理模块确保光照和肤色一致性，在保持身份匿名的同时提升表情、光照和肤色的保真度。


<details>
  <summary>Details</summary>
Motivation: 社交媒体和大规模数据集中人脸图像的广泛共享引发了隐私担忧，现有生成式匿名化方法主要关注身份移除和图像真实感，但往往忽略了面部表情以及光照和肤色等光度一致性特征，而这些特征对于重光照、颜色恒常性、医疗或情感分析等下游任务至关重要。

Method: 扩展DeepPrivacy框架，引入密集面部关键点以更好地保留表情，并添加轻量级后处理模块确保光照方向和肤色的一致性。同时建立了专门评估表情保真度、光照一致性和颜色保持的量化指标。

Result: 在CelebA-HQ数据集上的实验表明，该方法生成的匿名化人脸在真实感方面有所提升，并且在表情、光照和肤色保真度方面显著优于现有最先进的基线方法。

Conclusion: 特征感知的匿名化对于实现更有用、更公平、更可信的隐私保护人脸数据至关重要，该方法为这一方向迈出了重要一步。
...
TL;DR: PanoVGGT：一个用于全景图像的等变Transformer框架，能够单次前向传播联合预测相机位姿、深度图和3D点云，解决了全景图像的非针孔畸变问题。


<details>
  <summary>Details</summary>
Motivation: 全景图像提供360°视野但在消费设备中日益普及，但其非针孔畸变给联合位姿估计和3D重建带来挑战。现有的前馈模型针对透视相机设计，在此场景下泛化能力差。

Method: 提出PanoVGGT，一个置换等变的Transformer框架，包含球面感知位置嵌入和全景特定的三轴SO(3)旋转增强，支持在球面域进行有效几何推理。为解决全局坐标系模糊性，引入随机锚定策略。同时贡献了PanoCity数据集。

Result: 在PanoCity和标准基准测试上的广泛实验表明，PanoVGGT实现了竞争性的精度、强大的鲁棒性和改进的跨域泛化能力。

Conclusion: PanoVGGT框架有效解决了全景图像的位姿估计和3D重建问题，通过球面感知设计和随机锚定策略，在多个数据集上表现出优越性能，代码和数据集将开源。
...
TL;DR: LoGSAM：基于放射科医生口述生成文本提示，利用基础模型进行脑肿瘤定位和分割的参数高效框架


<details>
  <summary>Details</summary>
Motivation: 现有脑肿瘤分割方法依赖任务特定的监督模型，受限于标注数据稀缺。需要开发能够利用放射科医生口述、减少标注依赖的自动化解决方案。

Method: 1) 使用Whisper ASR转录和翻译放射科医生口述；2) 通过否定感知临床NLP提取肿瘤特异性文本提示；3) 用LoRA适配的Grounding DINO进行文本条件肿瘤定位；4) 将预测边界框作为MedSAM的提示生成像素级肿瘤掩码。

Result: 在BRISC 2025上获得80.32%的Dice分数；使用德国放射科医生口述在12个未见MRI扫描上达到91.7%的病例级准确率；仅更新5%模型参数实现高效领域适应。

Conclusion: 通过智能利用预训练基础模型和最小参数更新，可以构建模块化的语音到分割管道，减少对大量标注数据的依赖，同时保持高性能。
...
TL;DR: Edit-As-Act：一个基于目标回归规划的3D室内场景编辑框架，将自然语言指令转化为符号化目标谓词，通过PDDL风格的动作语言进行物理一致的场景编辑


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇系统在3D场景编辑中存在局限性：要么重新生成大部分场景，要么依赖图像空间编辑破坏空间结构，导致全局意外变化或物理不一致布局。这些方法将编辑主要视为生成任务，而作者认为编辑应该是实现目标状态的最小动作序列

Method: 提出Edit-As-Act框架，将开放词汇场景编辑视为3D空间中的目标回归规划。给定源场景和自由形式指令，预测符号化目标谓词，在EditLang（受PDDL启发的动作语言）中规划，包含显式前提条件和效果编码支持、接触、碰撞等几何关系。语言驱动规划器提出动作，验证器强制执行目标导向性、单调性和物理可行性

Result: 在E2A-Bench基准测试（包含9个室内环境的63个编辑任务）上，Edit-As-Act在所有编辑类型和场景类别上显著优于先前方法，实现了指令保真度、语义一致性和物理合理性这三个现有范式无法同时满足的标准

Conclusion: 通过将推理与低级生成分离，Edit-As-Act能够产生可解释且物理一致的场景变换，为3D场景编辑提供了新的视角和有效解决方案
...
### [70] [ReLaGS: Relational Language Gaussian Splatting](https://arxiv.org/abs/2603.17605)
...
<details>
  <summary>Details</summary>
Motivation: 现有3D感知方法要么是物体中心的，要么需要昂贵的训练来进行物体间推理，难以实现跨分割、检索和关系理解等任务的统一3D感知与推理

Method: 1) 构建层次化语言蒸馏高斯场景；2) 高斯剪枝机制优化场景几何；3) 鲁棒多视角语言对齐策略聚合噪声2D特征为准确3D物体嵌入；4) 基于视觉语言标注和GNN关系推理构建开放词汇3D场景图

Result: 在开放词汇分割、场景图生成和关系引导检索等任务上验证了方法的有效性，实现了高效可扩展的开放词汇3D推理

Conclusion: ReLaGS框架通过联合建模层次语义和物体内/间关系，实现了无需场景特定训练的统一3D感知与推理，为开放词汇3D推理提供了高效可扩展的解决方案
...
TL;DR: S-VGGT是一种针对3D基础模型的计算加速方法，通过结构级冗余处理来减少全局注意力机制的二次计算成本，与token级加速方法正交且可结合使用。


<details>
  <summary>Details</summary>
Motivation: 前馈3D基础模型面临全局注意力带来的二次计算成本问题，限制了模型的可扩展性。现有的token级加速方法（如token合并）虽然能提供局部节省，但需要最近邻搜索引入额外开销，且未能解决密集捕获数据中占主导地位的结构冗余问题。

Method: S-VGGT在结构帧级别处理冗余问题。首先利用初始特征构建密集场景图来表征结构场景冗余并指导场景划分；然后使用该图将帧软分配到少量子场景中，确保平衡分组和平滑几何过渡；核心创新在于设计共享共同参考帧的子场景，建立并行几何桥梁，实现独立高效处理而无需显式几何对齐。

Result: 该方法通过结构重组从源头上削减全局注意力成本，提供强大的内在加速效果。S-VGGT与token级加速方法完全正交，可以无缝结合实现复合加速，同时不损害重建保真度。

Conclusion: S-VGGT通过结构级冗余处理有效解决了3D基础模型的计算瓶颈问题，提供了一种可与其他加速技术结合的创新解决方案，显著提升了模型的可扩展性和效率。
...
TL;DR: 提出一种两阶段跨模态框架，通过语言模型生成部件感知指令和几何一致性建模，解决开放词汇3D功能基础定位中的语义缺失和几何对齐问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在开放词汇泛化、细粒度几何对齐和部件级语义一致性方面存在挑战，需要改进语言驱动的3D功能基础定位技术。

Method: 两阶段框架：第一阶段使用大语言模型生成部件感知指令恢复缺失语义；第二阶段引入功能原型聚合（APA）捕获跨对象几何一致性，以及对象内关系建模（IORM）细化几何差异。

Result: 在新引入的基准和两个现有基准上进行广泛实验，相比现有方法表现出优越性能。

Conclusion: 提出的两阶段跨模态框架通过增强语义和几何表示，有效解决了开放词汇3D功能基础定位的关键挑战。
...
TL;DR: 本文提出了一种新的生成式中间帧合成方法，通过关键帧锚定注意力偏置和重缩放时间RoPE技术，解决了稀疏序列和大运动场景下的帧不一致、节奏不稳定和语义不对齐问题。


<details>
  <summary>Details</summary>
Motivation: 当序列变得稀疏且运动幅度增大时，现有的生成式中间帧合成模型会产生不一致的帧、不稳定的节奏和语义不对齐的问题。由于生成式中间帧合成涉及固定端点（首尾关键帧）和多种可能的路径，需要从关键帧和文本中获取额外指导来指定预期路径。

Method: 提出了两种关键技术：1）关键帧锚定注意力偏置，从关键帧和文本中为每个中间帧提供语义和时间指导；2）重缩放时间RoPE，使自注意力机制能更准确地关注关键帧，从而增强帧一致性。同时创建了首个专门用于文本条件生成式中间帧合成的基准测试TGI-Bench。

Result: 无需额外训练，该方法在短序列和长序列的各种挑战场景下，在帧一致性、语义保真度和节奏稳定性方面均达到了最先进的性能水平。

Conclusion: 通过关键帧锚定注意力偏置和重缩放时间RoPE，有效解决了生成式中间帧合成中的帧不一致、语义不对齐和节奏不稳定问题，在TGI-Bench基准测试中表现出色，为稀疏序列和大运动场景提供了可靠的解决方案。
...
Method: 提出了FINER基准（包括FINER-CompreCap和FINER-DOCCI），用于分析四种细粒度设置下的幻觉；并开发了FINER-Tuning方法，基于直接偏好优化在FINER启发数据上进行微调。

Result: 在四个前沿MLLMs上进行FINER-Tuning微调后，在自建基准上幻觉减少最高达24.2%（InternVL3.5-14B），同时在八个现有幻觉基准上性能提升，并在六个多模态基准上增强了通用能力。

Conclusion: FINER基准有效揭示了MLLMs在细粒度查询中的幻觉模式，FINER-Tuning方法能显著减少幻觉并提升多模态能力，为改善MLLMs的可靠性提供了有效解决方案。
...
TL;DR: 本文提出了一种实例感知的离散化框架，通过基于输入相关先验自适应分配时间步长，改进了扩散和流匹配模型的生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法采用全局共享的时间步长调度策略，无法考虑生成过程中实例特定的复杂性，这限制了性能。在合成数据上的控制实验揭示了全局调度在实例特定动态下的次优性。

Method: 提出实例感知离散化框架，基于输入相关先验学习自适应时间步长分配，将基于梯度的离散化搜索扩展到条件生成设置。

Result: 在合成数据、像素空间扩散、潜在空间图像和视频流匹配模型等多种设置中，该方法一致提高了生成质量，相比训练仅需边际调优成本，推理开销可忽略。

Conclusion: 实例感知离散化框架能够有效解决全局时间步长调度的局限性，为扩散和流匹配模型提供了一种高效的自适应离散化策略。
...
TL;DR: DeepCORO-CLIP是一个基于视频-文本对比学习的多视角冠状动脉造影基础模型，在内部和外部验证中表现出色，能够快速自动评估冠状动脉疾病。


<details>
  <summary>Details</summary>
Motivation: 冠状动脉造影是评估冠状动脉疾病的金标准，但视觉解读存在读者间差异。现有AI方法通常分析单帧或单一投影，主要关注狭窄程度，限制了全面的冠状动脉评估。

Method: 开发了DeepCORO-CLIP多视角基础模型，采用视频-文本对比学习训练，整合多个投影并使用基于注意力的池化进行研究级评估。在28,117名患者的203,808个血管造影视频上进行训练，并在外部数据集验证。

Result: 模型在显著狭窄检测方面内部AUROC为0.888，外部验证为0.89；与核心实验室定量冠状动脉造影相比，平均绝对误差为13.6%（临床报告为19.0%）。在慢性完全闭塞、冠状动脉内血栓和钙化检测方面表现优异。迁移学习可预测一年主要不良心血管事件（AUROC 0.79）和估计左心室射血分数（平均绝对误差7.3%）。

Conclusion: DeepCORO-CLIP为床旁自动化冠状动脉造影解读提供了基础框架，平均推理时间仅4.2秒，代码、数据、模型权重和部署基础设施均已公开发布。
...
TL;DR: 该论文研究使用成对闪光-非闪光接触式指纹采集作为轻量级主动传感机制，用于检测接触式指纹识别中的欺骗攻击，通过分析光照引起的差异特征来区分真实指纹与伪造攻击。


<details>
  <summary>Details</summary>
Motivation: 接触式指纹识别虽然卫生方便，但由于缺乏物理接触和传统活体检测线索，给欺骗检测带来了新挑战。现有方法大多依赖单图像采集和基于外观的特征，在不同设备、采集条件和欺骗材料之间泛化能力差。

Method: 采用成对闪光-非闪光接触式指纹采集作为主动传感机制，分析光照引起的差异特征，包括通道间相关性、镜面反射特性、纹理真实性和差分成像等可解释指标。

Result: 闪光照明增强了材料和结构相关特性（如脊线可见性、次表面散射、微观几何和表面油脂），而非闪光图像提供基线外观上下文。这些互补特征有助于区分真实指纹与打印、数字和模制欺骗攻击。

Conclusion: 光照感知分析有潜力提高接触式指纹呈现攻击检测的鲁棒性和可解释性，激励未来在成对采集和物理信息特征设计方面的研究。
...
TL;DR: 该研究提出了WeatherReasonSeg基准，用于评估视觉语言模型在恶劣天气条件下的推理分割能力，发现模型性能随天气严重程度单调下降且不同天气类型导致不同的脆弱性模式。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在推理分割方面表现出色，但现有基准主要基于理想条件下的高质量图像构建。当恶劣天气条件（如雨、雪、雾）严重降低视觉线索时，视觉语言模型能否保持可靠的推理分割能力成为一个关键问题。

Method: 提出了WeatherReasonSeg基准，包含两个互补组件：1）通过将合成天气应用于现有分割数据集构建可控推理数据集，支持细粒度鲁棒性分析；2）通过掩码引导的LLM提示生成语义一致的查询，构建真实世界恶劣天气推理分割数据集。评估范围涵盖五个推理维度：功能、应用场景、结构属性、交互和需求匹配。

Result: 对不同视觉语言模型的广泛实验揭示了两个关键发现：1）视觉语言模型性能随天气严重程度单调下降；2）不同天气类型导致不同的脆弱性模式。
...
TL;DR: AFSS提出了一种针对YOLO检测器的抗遗忘采样策略，通过动态评估训练图像的学习充分性，将图像分为易、中、难三个级别，采用不同的采样频率，实现训练加速同时提升精度。


<details>
  <summary>Details</summary>
Motivation: YOLO检测器虽然推理速度快，但训练过程仍然耗时，因为每个epoch都要处理所有训练图像，即使许多图像已经充分学习。这与"只看一次"的效率理念相矛盾，因此需要探索YOLO是否真的需要在每个epoch中看到所有训练图像。

Method: 提出抗遗忘采样策略(AFSS)：1) 通过检测召回率和精度的最小值衡量每张训练图像的学习充分性；2) 根据学习充分性将图像动态分为易、中、难三个级别；3) 易图像采用稀疏重采样，优先选择长时间未使用的图像；4) 中等图像部分选择，优先最近未使用的；5) 难图像每个epoch都完全采样；6) 定期更新学习充分性评估。

Result: 在MS COCO 2017、PASCAL VOC 2007等自然图像检测基准和DOTA-v1.0、DIOR-R等遥感检测数据集上，AFSS为YOLO系列检测器实现了超过1.43倍的训练加速，同时提高了检测精度。

Conclusion: AFSS通过智能采样策略有效解决了YOLO训练效率低下的问题，证明了YOLO不需要在每个epoch中看到所有训练图像，实现了训练加速和精度提升的双重目标。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型从图像到视频理解的过渡存在两个关键限制：1）现有数据集缺乏时间中心性，答案可以从孤立的关键帧推断而不需要整体时间整合；2）专有模型生成的训练数据在基本时间感知（如运动方向、速度判断）上存在系统性错误。

Method: 提出SynRL后训练框架，将时间理解分解为短期感知原语（速度、方向）和长期认知原语，通过基于代码的视频生成构建了7.7K CoT和7K RL样本，包含真实帧级标注。使用简单几何形状的合成视频训练模型学习时间原语。

Result: 在15个基准测试（时间定位、复杂推理和通用视频理解）上取得显著改进。仅用7.7K合成CoT样本就超越了Video-R1的165K真实世界样本性能。证明从抽象合成模式到复杂真实场景的有效迁移。

Conclusion: 通过精心设计的合成数据进行视频时间学习为视频后训练提供了新的范式，这是一种更经济高效的扩展路径，基础时间技能（如逐帧跟踪变化和速度比较）能够从合成模式有效迁移到真实世界。
...
TL;DR: SAM3在眼图像分割任务中表现不如SAM2，无论是视觉提示还是概念提示模式，且处理速度更慢，因此SAM2仍是眼图像分割的最佳选择。
...
TL;DR: DiffVP通过提取扫描图像与参考图像之间的语义差异作为视觉提示，指导大语言模型生成更准确的CT报告，避免冗余解剖背景干扰。


<details>
  <summary>Details</summary>
Motivation: 现有CT报告生成方法通常整体编码3D体积数据，无法区分信息性线索和冗余的解剖背景。受放射学认知减法启发，需要基于显式的高层语义差异而非绝对视觉特征来生成报告。

Method: 提出差分视觉提示（DiffVP），包含分层差异提取器捕获全局和局部语义差异到共享潜在空间，以及差异到提示生成器将这些信号转换为可学习的视觉前缀标记用于LLM条件化。

Result: 在两个大规模基准测试中，DiffVP持续优于先前方法，平均BLEU-1-4分别提升+10.98和+4.36，在RadGenome-ChestCT上进一步提升临床效能（F1分数0.421）。

Conclusion: DiffVP通过显式的高层语义差异作为结构化条件信号，隐式抑制不变解剖结构同时放大诊断相关视觉证据，无需显式病灶定位即可促进准确报告生成。
...
TL;DR: SARE是一个样本自适应推理框架，通过级联设计结合快速候选检索和细粒度推理，仅在必要时调用推理过程，并利用自反经验机制重用错误经验，在训练自由的细粒度视觉识别任务中实现最优性能并显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有方法在利用大型视觉语言模型进行细粒度视觉识别时面临两个基本限制：1) 对所有样本采用相同的推理流程，未考虑识别难度不均，导致准确性和效率次优；2) 缺乏整合和重用错误特定经验的机制，导致在类似挑战性案例上重复失败。

Method: SARE采用级联设计，结合快速候选检索与细粒度推理，仅在必要时调用推理过程。在推理过程中，引入自反经验机制，利用过去的失败经验在推理过程中提供可转移的判别性指导，无需参数更新。
...
<details>
  <summary>Details</summary>
Motivation: 现有通用视频扩散模型难以在360度视图中保持严格的几何一致性和外观稳定性，无法满足高质量3D重建需求，需要专门针对3D几何条件优化的解决方案。

Method: 将3D外观生成重构为几何条件视频扩散问题：给定3D网格，渲染编码多模态几何特征以像素级精度约束视频生成；设计多阶段重建流程，包含3D感知修复以完成自遮挡区域。

Result: 实验结果表明，TAPESTRY在视频一致性和最终重建质量方面均优于现有方法，生成的视频既可作为高质量动态预览，又可作为可靠的3D感知中间表示。

Conclusion: TAPESTRY框架能够从无纹理网格自动创建生产就绪的完整3D资产，通过几何条件视频扩散生成高质量旋转视频，有效支持纹理合成和神经渲染等下游任务。
...
TL;DR: C2P是一个无需提示的通用医学图像分割框架，通过将解剖知识分解为几何和语义表示，利用MLLM提取语义标记，引入显式监督的几何标记，生成动态核进行精确分割，并采用几何感知推理共识机制提升鲁棒性。
...
Method: 提出C2P框架：1）将解剖知识分解为几何和语义表示；2）利用MLLM提取高级医学概念为可学习语义标记；3）引入显式监督的几何标记以强制执行通用物理和结构约束；4）通过解耦标记与图像特征交互生成输入特定的动态核进行掩码预测；5）采用几何感知推理共识机制评估预测可靠性并抑制异常值。

Result: 在包含8个数据集、7种模态的统一基准测试中，该方法显著优于通用或单模型方法；统一模型展现出强大的泛化能力，不仅在涉及未见案例的零样本任务上取得优异结果，在跨模态的相似任务转移中也表现突出。
...
TL;DR: PC-CrossDiff是一个用于3D视觉定位的双任务框架，通过点级和聚类级差分注意力机制，在复杂多物体场景中提升定位精度，特别是在解析隐式空间线索方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有3D视觉定位方法在简单单物体场景中表现良好，但在真实世界常见的复杂多物体场景中性能严重下降。主要面临两个挑战：1) 对区分视觉相似物体的隐式定位线索解析不足；2) 对共现物体的动态空间干扰抑制效果不佳。

Method: 提出PC-CrossDiff统一双任务框架，包含：1) 点级差分注意力模块，通过文本与点云的双向差分注意力，自适应提取隐式定位线索；2) 聚类级差分注意力模块，建立分层注意力机制，增强定位相关空间关系，同时抑制模糊或无关空间关系。

Result: 在ScanRefer、NR3D和SR3D基准测试中达到最先进性能。特别是在ScanRefer的隐式子集上，3DREC任务的Overall@0.50分数提升了+10.16%，显示出强大的隐式空间线索解析能力。

Conclusion: PC-CrossDiff通过创新的双级差分注意力架构，有效解决了复杂多物体场景中的3D视觉定位挑战，显著提升了隐式定位线索的解析能力和空间干扰抑制效果，推动了3D视觉定位的实际应用部署。
...
Method: SCEP框架：1) 使用视觉编码器的CLS令牌作为全局参考；2) 将补丁特征聚类成连贯组；3) 使用融合度量对补丁评分（CLS引导的语义不匹配+频率和噪声异常）；4) 从每个聚类采样高置信度补丁并应用基于网格的NMS，生成证据包；5) 用证据包条件化冻结的LVLM进行预测

Result: 在多样化的基准测试中，SCEP无需LVLM微调即可超越强基线方法
...
Method: 提出CrowdGaussian统一框架：1）自监督适应管道处理遮挡，使预训练大型人体模型能从严重遮挡输入重建完整3D人体；2）自校准学习策略，通过混合身份保持样本与干净/损坏图像对，让单步扩散模型自适应地将粗糙渲染优化到最佳质量，输出可蒸馏回多人3D高斯溅射表示以提升质量
...
TL;DR: 本研究系统比较了三种深度学习模型在精准畜牧业行为分类中的应用：从头训练、冻结特征提取和参数高效微调（PEFT）。结果表明，基于DINOv3基础模型的PEFT方法（特别是QLoRA配置）在测试准确率（83.16%）和训练效率（5.8小时）方面均显著优于其他方法，且增加适配器容量能持续改善泛化性能而不导致过拟合。


<details>
  <summary>Details</summary>
Motivation: 精准畜牧业中的自动化行为分类面临高计算成本和有限标注数据的挑战。本研究旨在探索如何高效利用大规模基础模型（如DINOv3，67亿参数）进行农业图像分类，同时降低计算资源需求。

Method: 系统比较三种方法：1）从头训练（ResNet-18、ViT-Small）；2）冻结特征提取；3）参数高效微调（PEFT），包括QLoRA和DoRA两种技术。在PEFT中，评估了不同秩（8、16、64）和目标模块（q_proj vs 全线性层）的配置。使用2,160张训练图像和211,800张测试图像（测试训练比98:1）进行评估。

Result: PEFT方法显著优于其他方法：最佳QLoRA配置（全线性层，秩=64）达到83.16%测试准确率，仅需2.72%参数（183.0M）和5.8小时训练时间。相比之下，ResNet-18为72.87%（16.8小时），ViT-Small为61.91%（18.7小时），冻结DINOv3为76.56%（17.5小时）。DoRA达到类似准确率（83.14%）但需要更长时间（11.0小时）。增加适配器容量持续改善泛化性能而不导致过拟合。

Conclusion: 参数高效微调（PEFT）是将数十亿参数视觉基础模型应用于农业畜牧业的有效策略。研究结果表明，在农业图像分类中，欠拟合而非过拟合是主要挑战，增加适配器容量能持续提升性能。这为在农业应用中部署大规模视觉模型提供了实用指南。
...
TL;DR: 基于ResNet-50帧分类器和解剖引导时序事件解码的多标签胃肠道视频分析系统，通过类别加权和时序优化策略，在严重类别不平衡下提升了罕见病理检测性能。


<details>
  <summary>Details</summary>
Motivation: 解决胃肠道视频分析中多标签分类的挑战，特别是严重类别不平衡问题（罕见病理标签样本极少），以及帧级预测与官方标注时序事件之间的不匹配问题。

Method: 1. 使用ResNet-50作为帧分类器处理336x336分辨率帧；2. 采用裁剪类别正样本加权训练损失解决类别不平衡；3. 时序阶段结合GT风格帧级事件组合、解剖投票平滑、解剖病理门控和保守滞后解码器。

Result: 在挑战测试集上，时序mAP从0.3801提升到0.4303，显著改善了罕见病理类别的学习效果和时序事件预测的准确性。

Conclusion: 提出的多阶段胃肠道视频分析管道有效解决了类别不平衡和时序事件对齐问题，通过解剖引导的时序解码策略显著提升了多标签分类性能。
...
TL;DR: 提出QIG方法，通过集成梯度量化评估token敏感性，将LVLM量化粒度从模态级提升到token级，在W4A8和W3A16设置下显著提升精度


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型虽然在下游任务表现出色，但计算和内存开销巨大，阻碍实际部署。现有量化方法通常在模态级别测量token敏感性，无法捕捉复杂的跨token交互，也无法在token级别定量测量量化误差。

Method: 提出基于量化感知集成梯度的细粒度量化策略，利用集成梯度定量评估token敏感性，将量化粒度从模态级提升到token级，反映模态间和模态内的动态特性。

Result: 在多个LVLM模型和基准测试中，W4A8和W3A16设置下均提高了精度，且延迟开销可忽略。例如，在3位仅权重量化下，LLaVA-onevision-7B平均精度提升1.60%，与全精度版本的差距缩小到仅1.33%。

Conclusion: QIG方法通过细粒度的token级量化策略，有效解决了现有LVLM量化方法的局限性，在保持低延迟的同时显著提升了量化模型的精度。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频扩散模型通过循环帧处理实现高质量生成，但训练时需要在整个视频序列上累积激活，导致内存成本过高。这种限制使得使用像素级损失对长视频或高分辨率视频进行微调在计算上不可行。
...
Conclusion: ChopGrad通过截断反向传播有效解决了视频扩散模型训练中的内存瓶颈问题，实现了高效微调，同时保持了生成质量，在多个视频生成任务中优于现有最先进方法。
...
TL;DR: 论文提出Mask-to-Point (M2P)学习方法，利用视频对象分割(VOS)掩码标注来改进视觉基础模型(VFMs)的密集点跟踪能力，通过三种掩码约束实现弱监督表示学习。


<details>
  <summary>Details</summary>
Motivation: 当前基于静态图像预训练的视觉基础模型(VFMs)在捕捉视频中的密集时间对应关系方面存在固有不足，需要改进以适应点跟踪任务。

Method: 提出M2P学习方法，包含三种掩码约束：1) 局部结构一致性损失，利用Procrustes分析建模局部结构内点的凝聚运动；2) 掩码标签一致性损失，确保前景点严格匹配跨帧的前景区域；3) 掩码边界约束，显式监督边界点。

Result: M2P模型在仅使用3.6K VOS训练视频的情况下，显著优于基线VFMs。在TAP-Vid-DAVIS基准上，分别比DINOv2-B/14和DINOv3-B/16提升了12.8%和14.6%的性能。

Conclusion: M2P方法能够作为点跟踪任务的通用预训练模型，既可用于测试时优化，也可用于离线微调的TAP任务，展示了其在视频理解中的潜力。
...
TL;DR: 该论文提出了一种名为结构化激活引导（STAS）的训练免费方法，通过引导视频扩散变换器中罕见的高幅度隐藏状态尖峰（大规模激活）来提升视频生成质量。


<details>
  <summary>Details</summary>
Motivation: 尽管视频扩散变换器发展迅速，但如何以最小开销利用其内部模型信号来提升视频生成质量仍未被充分探索。大规模激活（MAs）作为视频扩散变换器中罕见的高幅度隐藏状态尖峰，其潜在价值尚未被挖掘。

Method: 研究发现大规模激活在不同视觉标记中存在一致的结构化模式：首帧标记的MA幅度最大，潜在帧边界标记（潜在空间中每个时间块的首尾部分）幅度次之，内部标记幅度相对适中。基于此观察，提出了结构化激活引导（STAS）方法，该方法无需训练，通过将首帧和边界标记的MA值引导至缩放的全局最大参考幅度来实现自我引导。

Result: STAS方法在不同文本到视频模型中均能一致提升视频质量和时间连贯性，同时引入的计算开销可忽略不计。

Conclusion: 大规模激活在视频扩散变换器中呈现出结构化模式，反映了模型对潜在空间时间分块的隐式优先级。利用这一模式的结构化激活引导方法能够有效提升视频生成质量，为视频扩散模型优化提供了新的方向。
...
TL;DR: 本文提出TINA攻击方法，证明当前文本到图像扩散模型的概念擦除技术只是切断了文本到图像的映射，而底层视觉知识仍然存在，存在安全隐患。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像扩散模型的概念擦除技术主要关注切断文本到图像的映射，但忽略了底层视觉知识仍然存在。这种文本中心的范式存在安全漏洞，需要从视觉角度重新审视概念擦除的有效性。

Method: 提出TINA（Text-free INversion Attack）方法，通过DDIM反演在无文本条件下探索视觉生成路径。该方法避免使用文本提示，从而绕过现有的文本中心防御机制，并通过优化过程克服无文本指导时的累积近似误差。

Result: 实验表明，TINA能够从经过最先进遗忘技术处理的模型中成功再生被擦除的概念。这证明当前的概念擦除方法只是掩盖了概念，而底层视觉知识仍然可以被访问和利用。

Conclusion: 当前的概念擦除范式存在根本性缺陷，仅仅切断文本到图像映射是不够的。需要开发直接针对内部视觉知识进行操作的新范式，才能真正实现概念的安全擦除。
...
TL;DR: 这篇综述系统梳理了视频理解领域，从三个互补视角组织文献：低层视频几何理解、高层语义理解以及统一视频理解模型，总结了从孤立任务特定方法向统一建模范式的转变趋势。


<details>
  <summary>Details</summary>
Motivation: 视频理解相比图像理解需要建模时间动态和演化的视觉上下文，对时空推理提出更高要求，是计算机视觉的基础问题。本文旨在通过结构化综述为这一领域提供系统概览。

Method: 将视频理解文献组织为三个互补视角：1) 低层视频几何理解（几何结构、运动分析等）；2) 高层语义理解（动作识别、事件检测等）；3) 统一视频理解模型（通用视频基础模型）。

Result: 通过整合这三个视角，提供了视频理解领域的连贯图谱，总结了关键建模趋势和设计原则，突出了从孤立任务特定流水线向统一建模范式的广泛转变。

Conclusion: 该综述为视频理解领域提供了系统化的组织框架，指出了构建鲁棒、可扩展和统一的视频基础模型面临的开放挑战，并展望了未来发展方向。
...
TL;DR: Omni-3DEdit是一个统一的基于学习的3D编辑模型，能够隐式泛化各种3D编辑任务，避免了传统方法需要任务特定规则和耗时迭代优化的问题。


<details>
  <summary>Details</summary>
Motivation: 现有指令驱动的3D编辑方法依赖2D模型指导3D表示的显式迭代优化，存在两个主要缺点：1）缺乏不同3D编辑任务的通用设计，因为3D几何的显式操作需要任务依赖规则；2）迭代优化过程非常耗时，通常需要数千次2D/3D更新调用。

Method: 构建数据流水线合成高质量配对多视图编辑样本；采用预训练生成模型SEVA作为主干，在序列空间中连接源视图潜在表示和条件标记；提出双流LoRA模块解耦不同视图线索，增强表示学习能力。

Result: 作为基于学习的模型，Omni-3DEdit无需耗时的在线优化，能够在一次前向传播中完成各种3D编辑任务，将推理时间从数十分钟减少到约两分钟。大量实验证明了该方法的有效性和效率。

Conclusion: Omni-3DEdit通过统一的学习框架解决了传统3D编辑方法在通用性和效率方面的局限性，为各种3D编辑任务提供了高效、隐式的解决方案。
...
TL;DR: 该研究全面评估了细胞分割基础模型（CellPoseSAM、CellSAM、μSAM）和通用分割基础模型（SAM、SAM2、SAM3）在多种显微数据集上的性能，并提出了自动提示生成（APG）策略来改进基于SAM的显微基础模型。


<details>
  <summary>Details</summary>
Motivation: 细胞分割是显微图像分析的基础任务。虽然已有多个基于Segment Anything Model（SAM）的细胞分割基础模型，但SAM2和SAM3等通用分割基础模型的最新进展需要系统性评估。研究旨在评估这些模型在显微数据上的表现，并开发改进策略。

Method: 1）在多样化的显微数据集上全面评估细胞分割基础模型（CellPoseSAM、CellSAM、μSAM）和通用分割基础模型（SAM、SAM2、SAM3）；2）提出新的实例分割策略——自动提示生成（APG），用于改进基于SAM的显微基础模型；3）以μSAM为基础模型，应用APG策略。

Result: APG策略能一致性地改进μSAM的分割结果，使其与最先进的CellPoseSAM模型竞争。研究为SAM风格模型在显微领域的适应策略提供了重要经验，并为创建更强大的显微基础模型提供了策略。

Conclusion: 该研究系统评估了当前细胞分割基础模型，提出的APG策略能有效改进基于SAM的显微分割模型。研究为显微图像分析领域提供了重要的模型评估和改进方法，代码已开源供社区使用。
...
TL;DR: 该研究比较了不同人类显著性先验（手绘标注、眼动热图、分割掩码和DINOv2嵌入）在开放集虹膜呈现攻击检测中的效果，发现去噪后的眼动热图在泛化性能上表现最佳。


<details>
  <summary>Details</summary>
Motivation: 人类感知先验在显著性引导的深度学习训练中显示出潜力，特别是在虹膜呈现攻击检测领域。然而，对于开放集虹膜PAD，最有效的人类显著性形式尚未得到充分探索。本研究旨在比较不同形式的显著性先验在开放集虹膜PAD任务中的效果。

Method: 通过一系列实验比较手绘标注、眼动热图、分割掩码和DINOv2嵌入，与最先进的深度学习基线模型进行对比。采用留一攻击类型出的范式进行开放集PAD评估，使用ROC曲线下面积和特定误判率下的攻击分类错误率作为评估指标。

Result: 在开放集PAD任务中，去噪后的眼动热图在泛化性能上表现最佳，在ROC曲线下面积和BPCER为1%时的APCER指标上均优于交叉熵基线。该研究还提供了训练模型、代码和显著性图以支持可重复性研究。

Conclusion: 去噪后的眼动热图是开放集虹膜呈现攻击检测中最有效的人类显著性先验形式，能够显著提升模型的泛化性能。该研究为虹膜PAD领域提供了有价值的实验比较和可复现的研究资源。
...
TL;DR: 论文提出EditSpilloverProbe框架，将图像编辑模型中的编辑溢出现象重新定义为评估模型世界知识的自然探针，通过系统评估发现编辑溢出反映了模型真实的世界理解能力而非注意力泄漏。


<details>
  <summary>Details</summary>
Motivation: 指令跟随的图像编辑模型在实践中存在编辑溢出现象——模型会修改指定区域之外的相关内容。这引发了一个根本问题：溢出是反映了模型真实的隐式世界理解，还是仅仅是注意力泄漏？

Method: 提出EditSpilloverProbe框架，包括：1）编辑溢出分类法（空间、语义、混合、随机）；2）自动检测和分类流程；3）基于真实中文文本编辑任务构建的基准数据集EditSpilloverBench；4）对5个代表性编辑模型进行系统评估。

Result: 1）不同架构的溢出率差异显著（3.49%到11.46%，相差3.3倍）；2）语义溢出数量反映模型世界理解能力——nano_banana产生最多语义溢出（每图27.8个），qwen_2511编辑控制最精确但语义溢出较少（每图16.3个），显示编辑控制与世界理解之间的权衡；3）空间衰减分析表明溢出区域密度随距离指数衰减，但语义相关溢出比例保持恒定（40%-58%），证明语义溢出反映真实世界理解而非空间扩散。

Conclusion: 编辑溢出现象可以作为评估图像编辑模型世界知识的有效探针，语义溢出反映了模型真实的世界理解能力，而非简单的注意力泄漏，这为理解模型内部表示提供了新视角。
...
TL;DR: 提出一个统一的身份感知音视频联合生成框架，支持多身份细粒度控制，通过数据自动提取和身份注入机制实现高保真个性化内容生成。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏一个开放可访问的框架，能够对多个身份的面部外观和声音音色进行细粒度控制，无法满足身份感知内容创作的需求。

Method: 1) 设计数据自动提取管道，从音视频模态中提取身份信息并配对标注；2) 提出灵活可扩展的身份注入机制，将面部外观和声音音色作为控制信号；3) 针对模态差异设计多阶段训练策略，加速收敛并增强跨模态一致性。

Result: 实验证明该框架具有优越性能，能够实现高保真且一致的身份个性化音视频生成。
...
<details>
  <summary>Details</summary>
Motivation: 开发低功耗、实时的3D感知系统，特别是在资源受限的嵌入式应用中，需要突破传统深度相机的功耗限制。

Method: 采用定制相机同时捕获两幅不同焦距的图像，在FPGA上实现系统级Verilog的差分离焦深度算法，通过算法改进克服低功耗传感器挑战，设计内存本地化处理流式深度计算。

Result: 实现了32.5FPS的480x400稀疏深度图输出，总功耗仅624mW，工作范围52厘米，成为文献中首个亚瓦特总功耗的被动式FPGA 3D相机。

Conclusion: SpiderCam展示了在低功耗FPGA上实现实时深度感知的可行性，为嵌入式3D视觉应用提供了高效解决方案。
...
TL;DR: 提出几何驱动的2D-3D-2D范式，自动生成大规模无人机RGB和热成像语义分割标注，构建SegFly数据集并建立基准模型


<details>
  <summary>Details</summary>
Motivation: 现有无人机RGB和RGB-T数据集规模小、多样性不足、标注成本高，且商用无人机难以实现准确的RGB-T对齐

Method: 利用高重叠航拍图像的多视角冗余性，将少量手动标注的RGB图像提升为语义3D点云，再重投影到所有视角，自动生成RGB和热成像的伪真值标注；同时使用3D几何作为中间对齐空间实现跨模态图像配准

Result: 自动生成97%的RGB标签和100%的热成像标签，标注准确率达91%和88%；实现87%的RGB-T配准精度；构建包含2万多张高分辨率RGB图像和1.5万多对几何对齐RGB-T对的SegFly数据集
...
TL;DR: 该论文提出了C-TRAIL多模态法律数据集和两阶段框架，用于将行车记录仪视频自动转化为交通事故责任判定和法律条款分析。
...
Method: 首先创建C-TRAIL多模态法律数据集，将行车记录仪视频与文本描述对齐到特定责任模式和中国交通法规。然后提出两阶段框架：1) 交通事故理解模块生成文本视频描述；2) 法律多智能体框架输出责任模式、法规集合和完整判决报告。

Result: 在C-TRAIL和MM-AU数据集上的实验结果表明，该方法优于通用和法律LLM以及现有基于智能体的方法，同时提供了透明可解释的法律推理过程。

Conclusion: 该研究填补了视频证据与法律分析之间的空白，通过多模态数据集和两阶段框架实现了从行车记录仪视频到法律责任判定的自动化处理，具有更好的性能和可解释性。
...
TL;DR: TransText：首个基于Alpha-as-RGB范式的图像到视频模型适配方法，用于生成层感知文本（字形）动画，无需重建底层VAE，保持RGB语义先验


<details>
  <summary>Details</summary>
Motivation: 现有方法将透明度通道作为额外潜在维度附加到RGB空间，需要重建底层VAE，但高质量透明字形数据稀缺，重新训练VAE计算成本高且可能破坏从海量RGB语料库学习的稳健语义先验，导致潜在模式混合

Method: 提出TransText框架，基于新颖的Alpha-as-RGB范式，通过潜在空间拼接将alpha通道嵌入为RGB兼容的视觉信号，联合建模外观和透明度，无需修改预训练生成流形，明确确保严格的跨模态（RGB和Alpha）一致性，防止特征纠缠

Result: TransText显著优于基线方法，生成连贯、高保真度的透明动画，具有多样化的细粒度效果

Conclusion: TransText为动态视觉设计提供了实用的层感知文本动画生成能力，通过创新的Alpha-as-RGB范式解决了现有方法的局限性，在保持预训练模型优势的同时实现了高质量的透明动画生成
...
TL;DR: LaDe是一个生成可编辑分层设计文档的潜在扩散框架，能够根据自然语言提示生成灵活数量的语义层，支持文本到图像、文本到分层设计、设计分解三种任务。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么限制输出固定层数，要么要求每层只包含空间连续区域，导致层数随设计复杂度线性增长，无法生成灵活数量的语义层。

Method: 结合三个组件：基于LLM的提示扩展器将简短用户意图转换为结构化分层描述；带有4D RoPE位置编码的潜在扩散变换器联合生成完整设计及其RGBA层；支持完整alpha通道的RGBA VAE解码每层。

Result: 在Crello测试集上，LaDe在文本到分层和图像到分层任务中优于Qwen-Image-Layered，通过GPT-4o mini和Qwen3-VL评估器验证了更好的文本到层对齐。

Conclusion: LaDe框架能够生成灵活数量的语义层，支持多种设计任务，在分层设计生成方面优于现有方法，实现了更好的可编辑性和语义一致性。
...
TL;DR: 该论文提出Robust-ComBat方法，解决传统ComBat在包含病理异常值的多站点扩散MRI数据协调中的失真问题，通过MLP实现稳健的异常值补偿。


<details>
  <summary>Details</summary>
Motivation: 传统ComBat方法假设数据呈高斯分布，但在临床实践中，神经疾病患者的扩散指标常偏离健康对照组，形成病理异常值，这些异常值会扭曲站点效应估计。由于大多数患者在接受脑成像时尚未确诊，难以从协调队列中排除，导致传统方法产生显著失真。

Method: 研究评估了10种异常值拒绝方法与4种ComBat变体在不同场景下的表现，发现许多过滤策略在病理存在时失效。提出使用简单的多层感知器（MLP）提供稳健的异常值补偿，实现可靠的协调同时保留疾病相关信号。

Result: 在7种神经疾病条件和包含高达80%神经疾病患者的真实多站点队列实验中，Robust-ComBat在所有ComBat变体上都一致优于传统统计基线，具有更低的协调误差。

Conclusion: Robust-ComBat方法能够有效处理包含病理异常值的多站点扩散MRI数据协调问题，在保持疾病信号的同时提供更可靠的协调效果，优于现有的异常值过滤策略。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法假设输入是无遮挡的——完全可见的主体，通常处于规范姿势，这排除了绝大多数真实世界视频中人物经常被家具、物体或其他人遮挡的情况。从这种视频中重建面临基本挑战：大部分身体区域可能从未被观察到，且每个姿势缺乏多视角监督。
...
Result: 在YouTube视频和具有显著遮挡的多视角捕获数据上评估，展示了最先进的重建质量。生成的化身足够稳健，可以用新颖姿势进行动画，并合成到使用手机视频捕获的3DGS场景中。

Conclusion: AHOY方法成功解决了从严重遮挡的单目视频重建完整可动画3D高斯化身的挑战，通过创新的幻觉监督和架构设计实现了高质量重建，为真实世界视频应用开辟了新可能性。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在3D场景空间推理中通常依赖计算成本高的3D表示（如点云或BEV地图），或缺乏物理基础来解决尺度和大小模糊性问题。本文旨在通过引入IMU采集的自我运动模态数据来增强MLLMs的3D理解能力。

Method: 提出Motion-MLLM框架，包含两个关键组件：1）级联运动-视觉关键帧过滤模块，利用IMU数据和视觉特征高效选择稀疏但具代表性的关键帧；2）非对称跨模态融合模块，其中运动标记作为中介，将自我运动线索和跨帧视觉上下文传递到视觉表示中。

Result: Motion-MLLM在3D场景理解和空间推理相关任务中取得显著改进。与基于视频帧和显式3D数据的SOTA方法相比，Motion-MLLM在显著减少开销的情况下（成本效益分别提高1.40倍和1.63倍），实现了相似甚至更高的准确性。

Conclusion: 通过将视觉内容基于物理自我运动轨迹，Motion-MLLM能够推理绝对尺度和场景中的空间关系，为MLLMs的3D空间推理提供了一种更高效、物理基础更强的方法。
...
TL;DR: DynaEdit是一种无需训练的通用视频编辑方法，基于预训练的文本到视频流模型，能够实现复杂的视频编辑，包括修改动作、插入与场景交互的对象以及引入全局效果。


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑方法存在局限性：训练模型难以处理复杂编辑任务，因为相关训练数据难以收集；而无需训练的方法只能进行结构和运动保持的编辑，无法修改运动或交互。需要一种能够处理复杂编辑的通用方法。

Method: DynaEdit基于最近提出的无反转方法，不干预模型内部结构，因此具有模型无关性。针对直接应用该方法到无约束编辑时出现的低频错位和高频抖动问题，作者分析了这些现象的原因，并提出了新的机制来克服这些问题。

Result: 通过大量实验证明，DynaEdit在复杂的基于文本的视频编辑任务上取得了最先进的结果，包括修改动作、插入与场景交互的对象以及引入全局效果。

Conclusion: DynaEdit是一种无需训练的通用视频编辑方法，能够解锁预训练文本到视频流模型的多样化视频编辑能力，解决了现有方法在处理复杂编辑任务时的局限性。
...
TL;DR: GMT是一个多模态Transformer框架，通过融合3D边界框几何、点云上下文、语义对象类别和目标末端姿态，生成真实且目标导向的6自由度物体操控轨迹。


<details>
  <summary>Details</summary>
Motivation: 合成可控的6自由度物体操控轨迹对于机器人交互至关重要，但现有方法依赖2D或部分3D表示，限制了完整场景几何的捕捉和轨迹精度。

Method: 提出GMT多模态Transformer框架，将轨迹表示为连续的6自由度姿态序列，采用定制化条件策略融合几何、语义、上下文和目标导向信息。

Result: 在合成和真实世界基准测试中，GMT超越了CHOIS和GIMO等最先进的人类运动和人物交互基线，在空间精度和方向控制方面取得显著提升。

Conclusion: 该方法为基于学习的操控规划建立了新基准，并在多样化物体和杂乱3D环境中展现出强大的泛化能力。
...
TL;DR: 提出LoST（语义层次分词）方法，通过语义显著性对3D形状进行分词，使早期token解码为完整形状，后续token细化细节，实现高效高质量的自回归3D生成


<details>
  <summary>Details</summary>
Motivation: 现有3D形状分词方法主要依赖几何层次细节（LoD）层次结构，这些空间层次结构token效率低且缺乏语义连贯性，不适合自回归建模
...
Result: LoST在几何和语义重建指标上大幅超越现有LoD-based 3D形状分词器，实现高效高质量的自回归3D生成，仅需先前AR模型0.1%-10%的token，并支持语义检索等下游任务

Conclusion: LoST通过语义层次分词解决了3D形状token效率低和语义连贯性问题，为自回归3D生成提供了更优的分词方案，显著提升了重建质量和生成效率
...
TL;DR: 提出无需训练、基于文本嵌入空间引导的连续可控图像编辑框架，通过大语言模型自动构建去偏对比提示对，在生成器文本编码空间计算引导向量，实现平滑连续编辑控制。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常需要额外训练或人工干预，本文旨在开发一种无需训练、轻量级的连续可控图像编辑方法，通过简单的文本嵌入空间引导实现平滑编辑控制。

Method: 1) 使用大语言模型自动构建去偏对比提示对；2) 在生成器文本编码空间计算引导向量；3) 提出弹性范围搜索程序确定有效引导幅度区间；4) 将缩放后的引导向量添加到输入提示表示中实现连续控制。

Result: 方法在连续编辑行为上与基于训练的方法相当，优于其他无需训练的方法；引入新的评估指标量化引导连续性；方法自然泛化到图像和视频生成等多种文本条件模态。

Conclusion: 简单的文本嵌入空间引导足以实现平滑连续的图像编辑控制，无需额外训练或人工干预；提出的轻量级框架在连续可控编辑方面具有竞争力，并能跨模态泛化。
...
TL;DR: EchoGen是一个统一的布局到图像生成和图像定位框架，通过联合训练这两个任务实现相互促进，采用渐进式训练策略解决优化挑战，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 图像定位任务具有强大的文本和布局理解能力，可以弥补布局到图像生成任务的相应限制；同时，从布局生成的图像在内容上具有高度多样性，可以增强图像定位的鲁棒性。联合训练这两个任务可以相互促进性能提升，但面临优化挑战。

Method: 提出渐进式训练策略：1) 并行多任务预训练阶段，使用共享token加速训练，为两个任务提供基本能力；2) 双重联合优化阶段，利用任务对偶性顺序整合两个任务，实现统一优化；3) 循环强化学习阶段，使用一致性约束作为奖励消除对视觉监督的依赖，通过GRPO策略显著增强模型的统一能力。

Result: 在布局到图像生成和图像定位基准测试中取得了最先进的结果，并显示出两个任务联合优化的明显协同增益。

Conclusion: EchoGen框架成功实现了布局到图像生成和图像定位的统一，通过渐进式训练策略解决了联合训练的优化挑战，证明了这两个任务可以相互促进，实现协同性能提升。
...
Method: 受人类空间认知启发，采用两个联合目标：全局布局重建构建场景结构整体表示，显式情境建模锚定自我中心视角；利用预训练3D基础模型提取的轻量级相机位姿先验确保几何一致性和度量尺度对齐

Result: 在基于语言的定位任务上达到最先进性能，在情境化和通用3D问答基准测试中优于现有的2D和视频方法

Conclusion: Loc3R-VLM的空间监督框架能够实现强大的3D理解能力，为2D视觉语言模型提供了先进的3D理解能力
...
TL;DR: SkeletonLLM通过可微分渲染器将骨架序列转换为视觉模态，使多模态大语言模型能够处理非视觉的骨架数据，并通过协同训练策略提升推理能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉-语言推理方面表现出色，但无法直接处理非视觉的结构化数据（如人体骨架）。现有方法要么将骨架动态压缩为有损特征向量，要么将运动量化为离散标记，这些方法在不同骨架格式间泛化能力差。

Method: 提出SkeletonLLM框架，核心是DrAction可微分渲染器，可将任意骨架序列转换为MLLM的原生视觉模态。采用协同训练策略：因果推理蒸馏从教师模型转移结构化推理能力，判别性微调增强混淆动作间的决策边界。

Result: SkeletonLLM在识别、描述、推理和跨格式迁移等多种任务上表现出强大的泛化能力，为将MLLM应用于非原生模态提供了可行路径。

Conclusion: 通过将骨架序列转换为视觉模态，SkeletonLLM实现了通用的骨架理解，为多模态大语言模型处理非视觉结构化数据开辟了新途径。
...
TL;DR: STTS是一种简单轻量的时空令牌评分模块，可在ViT和LLM中剪枝视觉令牌，无需文本条件或令牌合并，实现端到端训练，剪枝50%视觉令牌带来62%效率提升，性能仅下降0.7%。


<details>
  <summary>Details</summary>
Motivation: 现有令牌剪枝方法存在局限性：要么只在ViT中剪枝且仅适用于单模态感知任务，不适用于下游视觉语言任务；要么只在LLM中剪枝但需要复杂的文本条件令牌选择机制。需要一种统一、简单且有效的跨架构视觉令牌剪枝方法。

Method: 提出STTS（时空令牌评分）模块，通过辅助损失学习时间评分，通过LLM下游梯度学习空间评分，配合高效打包算法，在ViT和LLM中剪枝视觉令牌，无需文本条件或令牌合并，支持端到端训练。

Result: 在13个短视频和长视频QA任务上，剪枝50%视觉令牌，训练和推理效率提升62%，平均性能仅下降0.7%。效率增益随视频采样帧数增加而增加。长视频QA测试时缩放进一步带来0.5-1%性能提升。

Conclusion: STTS是一种新颖、简单而有效的统一跨架构视觉令牌剪枝技术，显著提升计算效率同时保持性能，特别适合视频任务中的时间冗余处理。
