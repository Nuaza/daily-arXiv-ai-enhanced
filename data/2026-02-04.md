...
TL;DR: WorldVQA是一个专门评估多模态大语言模型视觉世界原子知识的基准测试，旨在分离视觉知识检索与推理能力，严格衡量模型记忆的视觉事实。


<details>
  <summary>Details</summary>
Motivation: 当前评估方法往往将视觉知识检索与推理能力混为一谈，无法准确衡量模型真正记忆的视觉事实。需要建立一个能够严格测试模型视觉事实性、百科全书广度和幻觉率的基准。

Method: 设计WorldVQA基准，通过分层分类法（从常见头部类别对象到长尾稀有类别）评估模型对视觉实体的定位和命名能力，解耦知识检索与推理过程。

Result: WorldVQA基准能够严格测量多模态大语言模型的原子视觉世界知识，为评估当前和下一代前沿模型的视觉事实性、百科全书广度和幻觉率建立标准。

Conclusion: WorldVQA作为评估多模态大语言模型视觉世界原子知识的基准，能够更准确地衡量模型记忆的视觉事实，为模型视觉事实性评估提供严格标准。
...
TL;DR: 本文提出了AdaptMMBench基准，用于评估视觉语言模型的自适应多模态推理能力，通过动态识别任务难度和MCC指标来评估模式选择的合理性，解决了现有评估方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有自适应多模态推理评估方法存在三个主要问题：1）依赖静态难度标签，无法捕捉相对于不同模型能力的动态难度；2）简单指标无法区分自适应模式选择能力和一般性能；3）缺乏细粒度的过程分析。这些问题阻碍了对模型自适应推理能力的准确评估。

Method: 提出了AdaptMMBench基准，涵盖五个领域：真实世界、OCR、GUI、知识和数学任务，包含直接感知和复杂推理。采用马修斯相关系数（MCC）评估不同推理模式的选择合理性，通过动态识别基于模型能力边界的任务难度来分离元认知能力。同时支持多维度过程评估，包括关键步骤覆盖率、工具有效性和计算效率。

Result: 评估发现：1）自适应模式选择能力随模型容量扩展，但与最终准确率显著解耦；2）关键步骤覆盖率与性能表现一致；3）工具有效性在不同模型架构间存在高度不一致性。

Conclusion: AdaptMMBench为自适应多模态推理提供了更全面的评估框架，能够分离模式选择能力和一般性能，揭示模型在自适应推理中的关键特性，为未来研究提供了重要基准和分析工具。
...
TL;DR: 提出了一种基于深度学习的正则化端到端框架，用于光学相干断层扫描（OCT）的逆散射问题，同时重建光学参数图和去斑后的结构强度图像。


<details>
  <summary>Details</summary>
Motivation: OCT逆散射问题旨在恢复结构图像和内在组织光学特性（折射率、散射系数、各向异性），但由于衰减、散斑噪声和参数间的强耦合而具有挑战性。
...
TL;DR: SVD-ViT使用奇异值分解来增强Vision Transformers的前景特征学习，抑制背景噪声，提高分类性能


<details>
  <summary>Details</summary>
Motivation: Vision Transformers作为大规模基础模型，由于自注意力机制全局操作，缺乏显式区分前景与背景的机制，可能学习到不必要的背景特征和伪影，导致分类性能下降

Method: 提出SVD-ViT，包含三个组件：SPC模块、SSVA和ID-RSVD，通过奇异值分解提取和聚合捕获物体前景信息的奇异向量，抑制任务无关的背景噪声和伪影

Result: 实验结果表明该方法提高了分类准确率，有效学习了信息丰富的前景表示，同时减少了背景噪声的影响

Conclusion: SVD-ViT通过奇异值分解机制增强了Vision Transformers的前景特征学习能力，解决了传统ViT缺乏前景-背景区分机制的问题
...
<details>
  <summary>Details</summary>
Motivation: 传统手动标志标注耗时且存在观察者间差异，基于规则的方法通常针对特定几何或有限标志集。需要一种能利用不同物种同源骨骼进行转化研究的自动方法
...
TL;DR: 提出了一种无需标注或相机标定的自监督多视角视频匿名化框架，通过检索假阴性检测和自监督域适应，在手术室视频中实现超过97%的召回率。


<details>
  <summary>Details</summary>
Motivation: 手术室研究中使用视频数据需要隐私保护，现有方法面临两个可扩展性瓶颈：1) 需要手动标注每个新临床站点；2) 多摄像头设置需要重新标定。需要一种无需标注和标定的自动匿名化方法。

Method: 提出自监督多视角视频匿名化框架，包含全身人员检测和姿态估计。核心策略是通过时间上下文和多视角上下文"检索"假阴性检测，并进行自监督域适应。首先使用低阈值运行现成检测器收集候选检测，然后通过跟踪和自监督非标定多视角关联检索与高分检测一致的假阴性，这些恢复的检测作为伪标签迭代微调检测器，最后对检测到的人员进行姿态估计。

Result: 在模拟手术的4D-OR数据集和真实手术数据集上实验，方法有效性达到超过97%的召回率。使用伪标签训练实时全身检测器，获得可比性能，展示了方法的实际应用性。

Conclusion: 提出了一种无需标注或相机标定的自监督多视角视频匿名化框架，通过检索假阴性检测和自监督域适应，在手术室视频中实现了高召回率的隐私保护，具有实际应用价值。
...
TL;DR: ViThinker框架通过主动查询生成机制，让视觉语言模型能够自主触发专家对齐的视觉特征合成，解决了传统CoT方法因过早视觉到文本转换而丢失连续信息的问题。


<details>
  <summary>Details</summary>
Motivation: 传统CoT推理在视觉语言模型中表现不佳，因为过早的视觉到文本转换会丢弃几何和空间布局等连续信息。现有方法大多是被动处理预计算输入，而非主动寻找任务相关细节。

Method: 提出ViThinker框架，通过自主生成决策（查询）令牌来触发按需合成的专家对齐视觉特征。采用两阶段课程学习：首先将冻结专家蒸馏到模型参数中，然后通过稀疏惩罚学习任务驱动的查询机制。

Result: 在多个视觉中心基准测试中表现出一致的改进，验证了主动查询生成在感知基础和推理准确性方面优于被动方法。

Conclusion: ViThinker通过主动感知机制，使视觉语言模型能够发现每个推理步骤的最小充分感知，实现了比被动方法更好的性能。
...
<details>
  <summary>Details</summary>
Motivation: 医学影像诊断需要基于细微视觉差异进行推理，但现有方法依赖最近邻检索，返回冗余证据并强化单一假设，缺乏对比性证据选择机制

Method: 1) 引入对比性文档感知参考选择框架，使用ROCO嵌入和元数据平衡视觉相关性、嵌入多样性和来源可追溯性；2) 提出反事实对比推理框架，执行结构化成对视觉比较，使用基于边界的决策规则进行证据聚合

Result: 在MediConfusion基准测试中实现最先进性能，相对于先前方法将集合级准确率提高近15%，同时减少混淆并提升个体准确率

Conclusion: 通过对比性证据选择和结构化推理框架，显著提升了医学影像诊断的准确性和可靠性，为医学图像推理提供了系统化的解决方案
...
TL;DR: 现有基于变换的隐私保护人脸识别系统主要关注像素级重建防护，但FaceLinkGen攻击显示这些系统仍存在严重的身份信息泄露风险，攻击者无需恢复原始像素即可进行身份匹配和面部再生。


<details>
  <summary>Details</summary>
Motivation: 当前隐私保护人脸识别系统的评估主要关注像素级重建防护，使用PSNR和SSIM等指标，但这种重建中心的视角存在缺陷，无法真正衡量身份隐私保护效果。

Method: 提出FaceLinkGen攻击方法，直接从受保护的模板中执行身份链接/匹配和面部再生，无需恢复原始像素。在三种最新的PPFR系统上进行测试，包括近零知识场景。

Result: 在三种PPFR系统上，FaceLinkGen达到超过98.5%的匹配准确率和96%以上的再生成功率；在近零知识设置下，仍超过92%匹配准确率和94%再生成功率。

Conclusion: 像素失真指标与真实隐私保护之间存在结构性差距，视觉混淆无法有效保护身份信息，身份信息对攻击者和不可信服务提供商都存在严重泄露风险。
...
TL;DR: MARBLE是首个纯Mamba基础的多状态多实例学习框架，用于全切片图像分析，通过并行处理多放大级别并在线性时间状态空间模型中集成从粗到细的推理，显著提升了WSI分析的性能。


<details>
  <summary>Details</summary>
Motivation: 全切片图像分析面临千兆像素分辨率和层次化放大倍数的挑战，现有MIL方法通常只在单一尺度操作，而基于Transformer的方法存在二次注意力计算成本问题，需要更高效的多尺度处理方案。

Method: MARBLE采用纯Mamba基础架构，并行处理多个放大级别，在线性时间状态空间模型中集成从粗到细的推理，以最小参数开销捕获跨尺度依赖关系，提供可扩展的模块化替代注意力架构。

Result: 在五个公共数据集上的实验显示，MARBLE在AUC上提升高达6.9%，准确率提升20.3%，C-index提升2.3%，证明了其在多尺度WSI分析中的高效性和泛化能力。

Conclusion: MARBLE作为首个纯Mamba基础的多状态多实例学习框架，通过并行多尺度处理和线性时间序列建模，为全切片图像分析提供了高效、可扩展的解决方案，显著优于现有方法。
...
TL;DR: SRA-Seg通过语义特征对齐解决合成数据在医学图像分割中的领域差距问题，仅用10%真实标注数据和90%合成数据就能达到与使用真实未标注数据方法相当的性能。
...
Conclusion: SRA-Seg通过显式对齐合成和真实特征分布，有效解决了合成数据在医学图像分割中的领域差距问题，为有限标注数据场景下的医学图像分割提供了有效解决方案。
...
TL;DR: Nüwa是一个两阶段视觉令牌剪枝框架，通过保持空间完整性来提升视觉语言模型在视觉定位任务上的性能，同时在视觉问答任务上保持优异表现。


<details>
  <summary>Details</summary>
Motivation: 现有视觉令牌剪枝方法在视觉问答任务上表现良好，但在视觉定位任务上性能显著下降。研究发现，基于全局语义相似性和注意力分数的策略会丢失全局空间参考框架，而空间参考框架来源于令牌位置信息的交互。

Method: 提出Nüwa两阶段令牌剪枝框架：第一阶段在视觉编码器后应用分离、对齐和聚合操作（受群体智能算法启发）以保留信息丰富的全局空间锚点；第二阶段在LLM内执行文本引导的剪枝以保留任务相关的视觉令牌。

Result: 在多个VQA基准测试中达到SOTA性能（从94%提升到95%），在视觉定位任务上获得显著改进（从7%提升到47%）。

Conclusion: Nüwa通过保持空间完整性，在保持视觉问答性能的同时显著提升了视觉语言模型在视觉定位任务上的表现，证明了空间参考框架对视觉定位任务的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 胸部X光片的时间比较在临床放射学中至关重要，但目前没有方法能将视觉语言模型的单图像报告生成和视觉定位能力结合用于时间变化检测。

Method: 提出TRACE模型，给定先前的和当前的胸部X光片，联合执行时间比较、变化分类和空间定位，生成自然语言描述并用边界框坐标定位每个发现。

Result: TRACE展示了超过90%的定位准确率，消融研究发现只有当时间比较和空间定位联合学习时才能实现有效的变化检测，单独任一项都无法实现有意义的变化检测。

Conclusion: TRACE为这一具有挑战性的新任务奠定了基础，研究发现定位提供了时间推理所必需的空间注意力机制，变化检测能力只有在联合学习时才会出现。
...
TL;DR: 该研究推导了鱼眼立体视觉系统的深度和距离误差的解析表达式，专门考虑了在大角度情况下的精度问题。


<details>
  <summary>Details</summary>
Motivation: 鱼眼立体视觉系统在计算机视觉和机器人领域有广泛应用，但现有研究通常假设小角度近似，这在大角度情况下会导致显著的精度误差。需要建立更准确的误差模型来评估系统性能。

Method: 通过数学推导建立鱼眼立体视觉系统的深度和距离误差的解析表达式，特别考虑大角度情况下的几何关系，不依赖小角度近似假设。

Result: 得到了深度和距离误差随物体距离变化的精确解析表达式，这些表达式能够准确描述在大角度情况下的误差特性，为系统设计和性能评估提供了理论依据。

Conclusion: 该研究提供了鱼眼立体视觉系统误差分析的完整解析框架，特别适用于大角度场景，有助于提高系统精度评估和优化设计。
...
TL;DR: CAFT提出了一种分层图像-文本表示学习框架，通过跨域对齐全局和局部语义，解决了CLIP等大视觉语言模型在处理长文本描述时的局限性，无需像素级监督即可实现细粒度视觉-语言理解。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（如CLIP）将图像和文本作为整体对齐，难以处理长文本描述。细粒度视觉语言理解需要分层语义来捕捉全局上下文和局部细节，但语言层次结构（语法或语义）很少与视觉组织匹配，而纯视觉层次结构倾向于将场景分割为外观驱动的部分而缺乏语义焦点。

Method: 提出CAFT（森林与树的跨域对齐）框架：1）耦合细到粗的视觉编码器与分层文本Transformer；2）使用分层对齐损失，在匹配整体图像与整体描述的同时，偏向区域-句子对应关系；3）粗粒度语义从细粒度证据构建，而非脱离部分级基础的聚合。

Result: 在3000万图像-文本对上进行训练，CAFT在六个长文本检索基准测试中实现了最先进的性能，并表现出强大的扩展行为。实验表明，分层跨域对齐使细粒度、视觉基础的图像-文本表示能够在没有显式区域级监督的情况下出现。

Conclusion: CAFT通过分层跨域对齐全局和局部语义，成功解决了长文本描述下的视觉语言理解问题，无需像素级监督即可实现细粒度对齐，为视觉语言表示学习提供了新的有效方法。
...
TL;DR: SharpTimeGS是一个基于4D高斯表示的动态场景新视角合成框架，通过可学习的寿命参数实现静态和动态区域的时域自适应建模，支持4K分辨率下100FPS的实时渲染。


<details>
  <summary>Details</summary>
Motivation: 现有基于高斯表示的方法在平衡长期静态区域和短期动态区域的表示与优化方面存在困难，需要一种能够统一建模静态和动态区域且保持时域适应性的方法。

Method: 提出寿命感知的4D高斯框架，引入可学习的寿命参数将时间可见性从高斯衰减重新定义为平顶轮廓；寿命参数调节每个基元的运动，将运动幅度与时间持续时间解耦；设计寿命-速度感知的致密化策略来平衡静态和动态区域的优化。

Result: 在多个基准测试中达到最先进的性能，在单个RTX 4090上支持4K分辨率下100FPS的实时渲染，有效提高了长期稳定性而不牺牲动态保真度。

Conclusion: SharpTimeGS通过寿命感知的4D高斯表示，成功实现了静态和动态区域的统一时域自适应建模，在保持实时渲染性能的同时显著提升了新视角合成的质量。
...
TL;DR: Video-OPD：一种基于策略蒸馏的高效视频时序定位后训练框架，通过反向KL散度提供密集的token级监督，结合TVDF课程学习提升训练效率
...
Method: 提出Video-OPD框架：1）直接从当前策略采样轨迹，保持训练和推理分布对齐；2）使用前沿教师模型通过反向KL散度提供密集的token级监督；3）引入TVDF课程学习，迭代优先选择教师可靠且对学生信息量最大的轨迹
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态视觉语言系统通常以固定保真度运行，导致高成本的视觉信息检索和处理，需要一种能根据查询需求自适应选择信息保真度的优化方法。

Method: 采用两阶段流水线：1) 梯度提升回归器仅基于问题特征估计每个保真度的正确性概率；2) 等渗校准器精炼这些概率以实现可靠决策。系统选择最小成本且最大化期望效用的保真度。

Result: 在三个部署场景、五个数据集和六个参数规模从7B到235B的视觉语言模型上评估，VOILA能持续减少50-60%的计算成本，同时保持90-95%的全分辨率准确率。

Conclusion: 预检索保真度选择对于在资源约束下优化多模态推理至关重要，VOILA框架证明了通过自适应信息获取策略能显著降低成本同时保持高性能。
...
TL;DR: 该论文提出了一种通过统计归一化和反归一化策略来缓解卷积下采样过程中结构和纹理特征信息损失的方法，以改进图像修复质量。


<details>
  <summary>Details</summary>
Motivation: 现有图像修复方法在卷积下采样过程中不可避免地会丢失结构和纹理特征信息，导致上采样输出不理想。论文旨在系统性地探索结构和纹理特征图如何相互帮助来缓解这种信息损失。

Method: 采用统计归一化和反归一化策略，在卷积下采样过程中为重建提供指导。通过结构和纹理特征图的相互辅助来减少信息损失，并可以替换所有编码器。

Result: 实验结果表明该方法在256×256和512×512等不同分辨率图像上都优于现有技术，特别是在用该方法替换所有编码器时效果更显著。

Conclusion: 通过统计归一化和反归一化策略，结构和纹理特征图能够相互辅助缓解卷积下采样过程中的信息损失，从而提升图像修复质量。
...
TL;DR: 通过计算机视觉分析纽约市900多个交通摄像头数据，研究拥堵收费政策实施前后（2024年11月-2026年1月）的交通模式变化


<details>
  <summary>Details</summary>
Motivation: 评估纽约市拥堵收费政策对交通流量的实际影响，通过大规模交通摄像头数据分析政策效果

Method: 建立计算机视觉处理流水线，分析曼哈顿和纽约市900多个交通摄像头的视频数据，对比政策实施前后（2024年11月-2026年1月）的交通模式

Result: 建立了基准交通模式，识别出监测区域内车辆密度的系统性变化

Conclusion: 通过自动化交通摄像头数据分析，量化了纽约市拥堵收费政策对交通流量的影响，为政策评估提供了数据支持
...
TL;DR: MUSE是一个多智能体框架，通过迭代的规划-执行-验证-修订循环来生成长篇音视频故事，解决了现有方法在长序列生成中的语义漂移和身份不一致问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从简短用户提示生成长篇音视频故事时存在意图-执行差距，高级叙事意图需要在长时域上保持连贯的镜头级多模态生成。现有方法通常依赖前馈管道或仅提示优化，导致语义漂移和身份不一致。

Method: 将故事讲述形式化为闭环约束执行问题，提出MUSE多智能体框架，通过迭代的规划-执行-验证-修订循环协调生成。将叙事意图转化为对身份、空间构图和时间连续性的显式机器可执行控制，并在生成过程中应用有针对性的多模态反馈来纠正违规。

Result: 实验表明，与代表性基线相比，MUSE显著改善了长时域叙事连贯性、跨模态身份一致性和电影质量。

Conclusion: MUSE通过闭环约束执行和多智能体协调，有效解决了长篇音视频故事生成中的意图-执行差距问题，提高了生成质量。
...
<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在日常视觉任务上取得进展，但人类具备在全新情境中部署视觉推理能力的独特技能，而Bongard问题正是测试这种能力的经典挑战。现有方法难以解决这类需要抽象推理的问题。

Method: 采用神经符号方法：给定Bongard问题的假设解规则，利用LLM生成参数化的程序化规则表示，然后使用贝叶斯优化进行参数拟合。

Result: 方法在两种任务上评估：1) 给定真实规则时对Bongard问题图像进行分类；2) 从零开始解决Bongard问题。

Conclusion: 提出的神经符号方法能够有效解决Bongard视觉推理问题，结合了LLM的生成能力和贝叶斯优化的参数优化，为复杂视觉推理任务提供了新思路。
...
TL;DR: IVC-Prune：一种无需训练、提示感知的视觉token剪枝方法，通过保留隐式视觉坐标token和语义相关前景token，在减少约50%视觉token的同时保持≥99%原始性能。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在处理高分辨率视觉输入时推理成本过高。现有视觉token剪枝方法主要关注语义相关性，但往往会丢弃对空间推理至关重要的token。本文发现LVLMs通过旋转位置编码隐式建立视觉坐标系，其中特定token位置作为隐式视觉坐标对空间推理至关重要。

Method: 提出IVC-Prune方法：1）通过理论分析RoPE的数学特性识别IVC token（旋转矩阵近似单位矩阵或90°旋转矩阵的位置）；2）通过两阶段过程识别前景token：语义种子发现和基于值向量相似度的上下文细化；3）结合保留IVC token和语义相关前景token进行剪枝。

Result: 在四个代表性LVLMs和二十个多样化基准测试上的广泛评估表明，IVC-Prune减少约50%视觉token的同时，保持≥99%原始性能，甚至在多个基准上实现性能提升。

Conclusion: IVC-Prune通过识别和保留对空间推理至关重要的隐式视觉坐标token，解决了现有视觉token剪枝方法在空间推理任务上的局限性，为高效LVLM推理提供了有效的训练免费解决方案。
...
TL;DR: 提出GBO（高斯边界优化）框架，通过优化问题直接预测视频片段边界，无需启发式映射，显著提升弱监督视频时序定位性能。


<details>
  <summary>Details</summary>
Motivation: 现有弱监督视频时序定位方法使用高斯分布表示查询相关片段，但依赖启发式映射从高斯参数到片段边界，导致定位性能不理想。

Method: 提出GBO框架，通过求解平衡提案覆盖度和片段紧凑性的优化问题来预测边界，推导出闭式解并分析不同惩罚机制下的最优性条件。

Result: GBO显著提升定位性能，在标准基准测试中达到最先进结果，且具有训练无关性，兼容单高斯和混合提案架构。

Conclusion: GBO为弱监督视频时序定位提供了理论严谨且实用的推理框架，在多个提案方案上展现出高效性和泛化能力。
...
TL;DR: SKELEX是一个基于120万张多样化肌肉骨骼X光片训练的大规模基础模型，通过自监督学习实现，在12个下游诊断任务中表现优于基线，并具备零样本异常定位能力。


<details>
  <summary>Details</summary>
Motivation: 现有AI模型大多任务特定、依赖标注、泛化能力有限，临床需要一个能跨疾病和解剖区域的大规模肌肉骨骼X光基础模型，但公开数据集规模有限且多样性不足。

Method: 使用自监督学习在120万张多样化、条件丰富的肌肉骨骼X光图像上训练SKELEX基础模型，并开发了可解释的区域引导模型用于骨肿瘤预测。
...
TL;DR: 本文提出了一种新的语义匹配方法，用具有空间一致性的最优传输算法替代计算昂贵的Stable Diffusion特征，在保持DINOv2准确性的同时大幅提升效率。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的语义对应方法需要结合DINOv2和Stable Diffusion两种大型基础模型的特征，虽然性能优秀但计算成本极高。作者希望找到一种更高效的替代方案。

Method: 采用最优传输算法替代标准最近邻匹配，该算法包含Gromov Wasserstein空间平滑先验，能够提供与Stable Diffusion类似的空间一致性特性，同时仅使用DINOv2特征。

Result: 该方法显著提升了DINOv2基线的性能，与使用Stable Diffusion特征的最先进方法竞争甚至超越，同时计算效率提高了5-10倍。

Conclusion: 通过将空间一致性先验融入匹配算法而非依赖额外的特征提取，可以在保持语义对应质量的同时大幅降低计算成本，为实际应用提供了更高效的解决方案。
...
TL;DR: EvoAug：基于进化算法和生成模型的自动数据增强学习框架，通过构建分层组合增强的随机增强树，在细粒度分类和少样本学习中取得优异性能


<details>
  <summary>Details</summary>
Motivation: 传统数据增强方法（如裁剪、旋转）虽然能减少过拟合，但多样性有限。生成模型（如条件扩散模型、少样本NeRF）能合成更丰富真实的数据，但若增强策略与任务不匹配可能损害性能。需要自动化学习任务特定的最优增强策略。

Method: 提出EvoAug自动化增强学习流程：1）利用生成模型合成多样化数据；2）采用高效进化算法学习最优任务特定增强策略；3）引入随机增强树，分层组合增强操作，实现结构化自适应变换。

Result: 在细粒度分类和少样本学习任务中表现优异。即使在低数据场景下，发现的增强策略仍能与领域知识保持一致，验证了学习生成增强的潜力。

Conclusion: EvoAug展示了学习生成增强的潜力，为鲁棒模型训练开辟了新可能性。通过自动化学习任务特定增强策略，能有效利用生成模型的多样性优势，同时避免不匹配增强带来的性能下降。
...
TL;DR: 该论文提出了一种基于扩散模型的3D人体姿态估计框架，能够从2D图像中采样多个合理的3D姿态，解决了传统方法中的深度模糊性和遮挡问题。


<details>
  <summary>Details</summary>
Motivation: 3D人体姿态估计面临深度模糊和遮挡的挑战，导致任务不确定——同一张2D图像可能对应多个合理的3D姿态。传统方法通常假设存在确定性映射并估计单一姿态，而基于机器学习的方法需要大量配对2D-3D数据训练且泛化能力有限。

Method: 提出基于扩散模型的框架，采用条件生成引导方法：首先在仅使用3D数据训练的无条件扩散模型上采样，然后利用2D关键点检测器热图的梯度来引导采样，生成与2D图像一致的合理3D姿态分布。

Result: 在Human 3.6M数据集上使用best-of-m多假设评估，在不需配对2D-3D数据训练的方法中达到最先进性能。在MPI-INF-3DHP和3DPW数据集上展示了良好的泛化能力。框架还支持姿态生成和姿态补全等新任务。

Conclusion: 该扩散模型框架有效解决了3D姿态估计的不确定性问题，无需配对2D-3D训练数据，具有良好的泛化能力和任务灵活性，为3D人体姿态估计提供了新的概率化解决方案。
...
TL;DR: SwiftVLM是一种新的视觉语言模型剪枝方法，通过"旁路"机制保留未选择的视觉token供后续层重新评估，避免早期剪枝造成的关键信息丢失，在保持效率的同时显著提升细粒度任务的性能。
...
Method: 提出新的剪枝范式"旁路"，保留未选择的视觉token并转发到后续剪枝阶段重新评估。基于此提出SwiftVLM方法，在具有强视觉token选择能力的模型特定层执行剪枝，同时允许跨层独立剪枝决策，无需训练。

Result: 在多个视觉语言模型和基准测试上的实验表明，SwiftVLM始终优于现有剪枝策略，实现了更优的准确率-效率权衡，并展现出更可靠的视觉token选择行为。

Conclusion: SwiftVLM通过引入旁路机制解决了早期剪枝导致的关键信息丢失问题，为视觉语言模型的高效推理提供了一种简单有效的训练免费剪枝方案，在保持计算效率的同时显著提升了细粒度任务的性能。
...
TL;DR: 该研究提出了首个完全基于Kolmogorov-Arnold表示定理的深度模型，通过Share-activation KAN和Grad-Free Spline技术解决了深度KAN训练困难和内存消耗大的问题，在医学图像分割任务中实现了优于传统架构的性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度KAN模型由于训练困难和内存需求大，只能使用少量KAN层，限制了KAN在深度学习中的全面探索。本研究旨在克服这些限制，实现完全基于Kolmogorov-Arnold表示定理的深度模型。

Method: 提出了三种关键技术：1) Share-activation KAN (SaKAN) - 基于Sprecher变体的Kolmogorov-Arnold表示定理重新表述，简化参数化并增加训练样本密度；2) Grad-Free Spline - 消除样条梯度以减少内存使用和计算开销；3) ALL U-KAN - 首个完全KA基础的深度模型，用KA和KAonv层完全替代传统的FC和Conv层。

Result: 在三个医学图像分割任务上的评估表明，完全KA基础的架构优于部分KA基础架构和传统架构，实现了更高的分割精度。相比直接深度堆叠KAN，ALL U-KAN参数数量减少10倍，内存消耗降低20倍以上。

Conclusion: 该研究成功实现了首个完全基于Kolmogorov-Arnold表示定理的深度模型，证明了KA基础层可以完全替代传统深度学习架构，并展现出更优的学习能力，为深度KAN架构的探索开辟了新方向。
...
TL;DR: 本文提出了一种无需群体活动标注的人机协同适应方法，用于群体活动特征学习（GAFL），通过交互式微调提升群体活动视频检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常需要预定义的群体活动类别进行监督学习，而本文旨在开发无需群体活动标注的方法，通过人机协同交互来提升视频检索性能。

Method: 1. 采用自监督方式预训练群体活动特征空间；2. 设计交互式微调过程，通过用户标注正负样本来更新特征空间；3. 提出数据高效视频选择过程，从数据库中筛选视频供用户标注；4. 使用对比学习使正样本靠近查询视频，负样本远离查询视频。

Result: 在两个团队运动数据集上的综合实验验证了该方法显著提升了检索性能，消融研究也证明了人机协同适应中的多个组件对性能提升有贡献。

Conclusion: 本文提出的人机协同适应方法能够有效提升群体活动视频检索性能，无需群体活动标注，通过交互式微调和数据高效选择过程实现了性能的显著改进。
...
TL;DR: LSGQuant是一种针对一步扩散视频超分辨率模型的层敏感度引导量化方法，通过动态范围自适应量化器、方差导向层训练策略和量化感知优化，在保持接近原始模型性能的同时显著压缩模型大小和计算成本。


<details>
  <summary>Details</summary>
Motivation: 一步扩散模型在真实世界视频超分辨率中表现出色，但基于扩散变换器的大模型尺寸和高计算成本限制了实际应用。传统低比特量化方法在处理输入潜在空间的高动态范围和不同层行为多样性时效果有限。

Method: 提出LSGQuant方法：1) 动态范围自适应量化器(DRAQ)适应视频token激活；2) 通过层间统计分析估计层敏感度，实施方差导向层训练策略(VOLTS)；3) 引入量化感知优化(QAO)联合微调量化分支和保留的高精度分支。

Result: 大量实验表明，该方法在保持接近全精度原始模型性能的同时，显著超越了现有量化技术，实现了有效的模型压缩和计算成本降低。

Conclusion: LSGQuant为一步扩散视频超分辨率模型提供了一种有效的量化解决方案，通过层敏感度引导的量化策略解决了高动态范围和层行为多样性带来的挑战，在性能保持和计算效率之间取得了良好平衡。
...
Method: 提出VIRAL框架，将ICL视为通过视觉类比的条件生成（x_s:x_t::x_q:y_q）；采用角色感知多图像条件化调整冻结的Diffusion Transformer，并引入混合专家LoRA来减轻跨任务梯度干扰；构建大规模视觉上下文数据集

Result: VIRAL在实验中优于现有方法，验证了统一的视觉ICL范式能够处理大多数视觉任务，包括开放域编辑

Conclusion: 通过视觉类比和适应预训练图像编辑模型，VIRAL成功实现了视觉上下文学习，为处理多样化视觉任务提供了有效的统一框架
...
TL;DR: ConsisDrive是一个身份保持的驾驶世界模型，通过实例级时间一致性约束解决自动驾驶数据生成中的身份漂移问题，在nuScenes数据集上实现了最先进的驾驶视频生成质量。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要大规模高质量多视角驾驶视频数据，但现有世界模型在生成数据时存在身份漂移问题，即同一物体在不同帧中外观或类别发生变化，这源于缺乏实例级的时间约束。

Method: ConsisDrive包含两个核心组件：1）实例掩码注意力，通过实例身份掩码和轨迹掩码确保视觉标记仅与对应实例特征在时空维度交互；2）实例掩码损失，通过概率性实例掩码自适应强调前景区域，减少背景噪声同时保持场景保真度。

Result: ConsisDrive在nuScenes数据集上实现了最先进的驾驶视频生成质量，并在下游自动驾驶任务中展现出显著改进。

Conclusion: 通过引入实例级时间一致性约束，ConsisDrive有效解决了驾驶世界模型中的身份漂移问题，为自动驾驶提供了更高质量的数据生成解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 视觉跟踪领域存在性能与速度的矛盾：高性能跟踪器往往处理速度慢，难以在资源受限设备上部署。需要一种既能保持高性能又能实现高效执行的跟踪方法。

Method: 提出FARTrack框架，包含两个核心技术：1) 任务特定自蒸馏：通过逐层蒸馏任务特定token实现模型压缩，避免次优的手动师生层对分配；2) 帧间自回归稀疏化：顺序压缩多个模板，学习时间全局最优稀疏化策略，避免额外运行时开销。

Result: FARTrack在GOT-10k数据集上达到70.6%的AO（平均重叠率），实现实时跟踪。最快模型在GPU上达到343 FPS，在CPU上达到121 FPS，展现了出色的速度和竞争力性能。
...
TL;DR: Spiral RoPE是一种改进的二维旋转位置编码方法，通过将嵌入通道分组并分配到多个均匀分布的方向上，突破了标准轴向RoPE只能编码轴对齐方向的位置关系的限制，实现了多方向位置编码。


<details>
  <summary>Details</summary>
Motivation: 标准轴向二维RoPE将空间位置分解为水平和垂直分量，这限制了位置编码只能处理轴对齐方向的关系，而自然图像中存在大量倾斜的空间关系，这种方向约束成为视觉Transformer建模能力的根本限制。

Method: 提出Spiral RoPE方法，将嵌入通道划分为多个组，每个组与均匀分布的方向相关联。每个组根据图像块位置在其对应方向上的投影进行旋转，从而实现对水平和垂直轴之外的空间关系的编码。

Result: 在分类、分割和生成等多种视觉任务中，Spiral RoPE都一致提升了性能。注意力图的定性分析显示，Spiral RoPE在语义相关对象上表现出更集中的激活，并能更好地尊重局部对象边界。

Conclusion: Spiral RoPE通过实现多方向位置编码，克服了标准轴向二维RoPE的方向限制，证明了多方向位置编码在视觉Transformer中的重要性，为视觉任务提供了更有效的位置编码方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于事件的MLLMs通常采用密集图像式处理范式，忽视了事件流的时空稀疏性，导致计算成本高昂。需要开发更高效的事件处理模型。

Method: 1) 构建大规模场景多样的EventMind数据集（50万+指令集）；2) 自适应时间窗口聚合模块进行高效时间采样；3) 稀疏密度引导注意力模块提升空间令牌效率

Result: EventFlash相比基线（EventFlash-Zero）实现12.4倍吞吐量提升，同时保持可比性能；支持长达1000个bin的事件流处理，显著优于EventGPT的5-bin限制

Conclusion: EventFlash通过探索时空令牌稀疏化，有效减少数据冗余并加速推理，可作为基于事件视觉的高效基础模型
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要高质量的大规模多视角驾驶视频来训练鲁棒模型。现有世界模型虽然能生成逼真驾驶视频，但在实例级时间一致性和空间几何保真度方面存在不足，影响视频真实性和下游任务性能。

Method: 提出InstaDrive框架，包含两个核心组件：1) 实例流引导器：提取并传播实例特征跨帧，强制时间一致性，保持实例身份；2) 空间几何对齐器：改进空间推理，确保精确实例定位，显式建模遮挡层次。此外利用CARLA自动驾驶系统程序化随机模拟罕见但安全关键的驾驶场景。

Result: 在nuScenes数据集上实现了最先进的视频生成质量，并增强了下游自动驾驶任务的性能。同时通过CARLA模拟了多样化的安全关键场景，支持自动驾驶系统的严格安全评估。

Conclusion: InstaDrive通过实例感知机制有效解决了驾驶视频生成中的时间一致性和空间几何保真度问题，为自动驾驶模型训练提供了更高质量的合成数据，并支持全面的安全评估。
...
Method: 1) 创建LaVPR基准数据集，扩展现有VPR数据集并添加65万+自然语言描述；2) 研究多模态融合增强鲁棒性；3) 研究跨模态检索实现语言定位，使用LoRA和Multi-Similarity损失作为基线方法。

Result: 语言描述在视觉退化条件下带来一致性能提升，对小模型影响最显著；添加语言后，紧凑模型性能可媲美更大的纯视觉架构；跨模态检索基线方法显著优于标准对比方法。

Conclusion: LaVPR实现了既能抵抗现实世界随机性又适合资源受限部署的新型定位系统，为视觉-语言定位研究提供了重要基准。
...
TL;DR: 全局嵌入几何不能有效预测组合绑定能力，而功能敏感性（输入-输出雅可比矩阵）能可靠追踪该能力


<details>
  <summary>Details</summary>
Motivation: 现有表示学习假设全局良好分布的嵌入支持鲁棒和可泛化的表示，但全局几何可能对元素如何组合不敏感，需要研究这种局限性

Method: 测试21个视觉编码器中几何指标预测组合绑定的能力，对比标准几何统计量与功能敏感性（输入-输出雅可比矩阵）的相关性

Result: 标准几何统计量与组合绑定能力接近零相关，而功能敏感性可靠追踪该能力；分析显示现有损失函数明确约束嵌入几何但未约束局部输入-输出映射

Conclusion: 全局嵌入几何仅捕获表示能力的部分视图，功能敏感性是建模组合结构的关键补充维度
...
<details>
  <summary>Details</summary>
Motivation: 现有的伪标签基测试时自适应方法依赖扰动集成启发式方法（如dropout采样、测试时增强、高斯噪声），这些方法缺乏分布基础，产生不稳定的训练信号，容易导致错误累积和灾难性遗忘。

Method: 提出A3-TTA框架：1）使用类紧凑密度度量识别预测良好的目标域图像作为锚点；2）锚点作为稳定参考指导伪标签生成；3）通过语义一致性和边界感知熵最小化进行正则化；4）引入自适应指数移动平均策略减轻标签噪声并稳定模型更新。

Result: 在多域医学图像（心脏结构和前列腺分割）和自然图像上评估，A3-TTA相比源模型将平均Dice分数提高了10.40到17.68个百分点，优于多种最先进的TTA方法。在持续TTA中表现出色，在顺序目标域上保持高性能，具有较强的抗遗忘能力。

Conclusion: A3-TTA通过锚点引导监督构建可靠的伪标签，有效解决了测试时自适应中的不稳定训练信号问题，显著提升了分割性能，并在持续域适应中表现出强大的抗遗忘能力。
...
TL;DR: LEVIO是一个专为超低功耗计算平台优化的视觉惯性里程计（VIO）系统，能够在资源受限的硬件上实现实时6自由度运动跟踪，功耗低于100mW。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的VIO系统计算需求过高，无法在微无人机、智能眼镜等资源受限的硬件上运行，需要开发适合超低功耗平台的VIO解决方案。

Method: 采用硬件-软件协同优化方法，结合ORB特征跟踪和束调整等成熟VIO组件，设计计算效率高的架构，强调并行化和低内存使用，适用于嵌入式微控制器和低功耗SoC。

Result: 在并行处理的超低功耗RISC-V SoC上实现20FPS实时性能，功耗低于100mW，在公开VIO数据集上进行了基准测试，在效率和精度之间取得了良好平衡。

Conclusion: LEVIO为资源受限设备提供了可行的VIO解决方案，实现了超低功耗下的实时运动跟踪，开源实现促进了可重复性和采用。
...
TL;DR: FOCUS是一个基于基础模型的端到端自动化3D OCT视网膜疾病诊断框架，通过图像质量评估、异常检测和多疾病分类，将2D切片级预测智能整合为3D患者级诊断，在真实世界验证中表现优异。
...
Result: 在3,300名患者（40,672个切片）上训练测试，并在1,345名患者（18,498个切片）上进行外部验证，FOCUS在质量评估（99.01% F1）、异常检测（97.46% F1）和患者级诊断（94.39% F1）方面表现优异。真实世界验证显示稳定性能（F1：90.22%-95.24%），在人机比较中与专家表现相当甚至更优。

Conclusion: FOCUS实现了从图像到诊断的端到端自动化流程，代表了向无人眼科发展的关键进展，为自主筛查提供了经过验证的蓝图，可提高人群规模视网膜护理的可及性和效率。
...
TL;DR: 本文提出InvLBA，一种针对生成数据增强的隐式干净标签后门攻击方法，通过在潜在特征层面进行扰动，显著提高了攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 随着图像生成模型的快速发展，生成数据增强成为丰富训练图像的有效方法，特别是在小规模数据集场景下。然而，现有像素级干净标签后门攻击方法（如COMBAT）在生成图像上攻击成功率较低，这促使研究者探索超越像素级触发器，关注潜在特征层面的攻击方法。

Method: 提出InvLBA方法，通过潜在扰动进行隐式干净标签后门攻击。该方法在生成图像的潜在特征层面进行扰动，而不是传统的像素级触发器，从而提高了攻击的隐蔽性和成功率。

Result: 在多个数据集上的实验表明，InvLBA方法平均提高了46.43%的攻击成功率，同时几乎不影响干净准确率，并且对最先进的防御方法具有高鲁棒性。

Conclusion: InvLBA方法通过在潜在特征层面进行扰动，有效解决了生成数据增强场景下干净标签后门攻击成功率低的问题，为生成数据增强的安全性研究提供了新的视角和方法。
...
TL;DR: MedSAM-Agent是一个将医学图像分割重新定义为多步自主决策过程的框架，通过混合提示策略和两阶段训练管道，在6种医学模态和21个数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于多模态大语言模型的医学图像分割方法通常采用单轮、刚性的交互策略，缺乏过程级监督训练，无法充分利用交互工具的动态潜力，导致冗余操作。需要一种能够统一自主医学推理与稳健迭代优化的框架。

Method: 1. 将交互式分割重新定义为多步自主决策过程；2. 引入专家策划轨迹生成的混合提示策略，使模型能够内化类人决策启发式和自适应细化策略；3. 开发两阶段训练管道，集成多轮端到端结果验证与临床保真度过程奖励设计，促进交互简洁性和决策效率。

Result: 在6种医学模态和21个数据集上的广泛实验表明，MedSAM-Agent实现了最先进的性能，有效统一了自主医学推理与稳健的迭代优化。

Conclusion: MedSAM-Agent通过将交互式分割重新定义为多步自主决策过程，结合混合提示策略和两阶段训练管道，成功克服了现有方法的局限性，实现了更高效、更准确的医学图像分割。
...
Method: 提出PWAVEP净化框架：1）计算每个点的谱图小波域显著性得分和局部稀疏性得分；2）采用分层策略：消除最显著点（难以恢复的对抗异常点）；3）对中等显著点应用谱滤波，利用图小波变换衰减目标点的高频系数以抑制对抗噪声
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉分词器在组合性控制方面存在不足，无法有效支持通过token交换实现高级语义编辑。需要开发一种能够增强token组合性并确保解码器充分利用所有token的训练框架。

Method: 1. 使用token条件扩散解码器；2. 采用InfoGAN风格目标，训练识别模型预测用于条件解码的token；3. 在原始图像基础上，通过交换图像间token子集进行训练；4. 对无配对的交换生成应用流正则化器保持自然图像分布。

Result: 1. 在图像类别条件生成任务上达到最先进性能；2. 支持通过token交换实现图像高级语义编辑；3. 提出两个衡量token空间景观的指标，CompTok在这两个指标上均有改进；4. 能够支持最先进的生成器进行类别条件生成。

Conclusion: CompTok框架成功增强了视觉分词器的组合性，不仅提升了生成性能，还实现了通过token交换进行语义编辑的能力，为视觉表示学习提供了新的组合性增强方法。
...
TL;DR: Tiled Prompts框架通过为每个潜在图块生成特定提示，解决全局提示在图像和视频超分辨率中的不足，提供局部文本条件指导，提升感知质量和文本对齐。


<details>
  <summary>Details</summary>
Motivation: 现代超分辨率管道通常依赖潜在分块扩展到高分辨率，但单个全局标题会导致提示不足。粗粒度的全局提示经常错过局部细节（提示稀疏性）并提供局部不相关的指导（提示误导性），这些问题可能被无分类器指导放大。
...
TL;DR: Z3D：首个仅使用多视角图像、无需几何监督或物体先验的零样本3D视觉定位方法，通过高质量3D实例分割和基于提示的分割推理实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有零样本3D视觉定位方法存在显著性能瓶颈，需要探索仅基于多视角图像、无需几何监督或物体先验的解决方案。

Method: 提出Z3D通用定位流程：1）使用SOTA零样本3D实例分割生成高质量3D边界框提案；2）通过基于提示的分割推理充分利用现代视觉语言模型的全部能力；3）灵活支持相机位姿和深度图作为可选输入。

Result: 在ScanRefer和Nr3D基准测试中，Z3D在零样本方法中实现了最先进的性能表现。

Conclusion: Z3D证明了仅使用多视角图像就能实现有效的零样本3D视觉定位，为解决3D视觉定位中的几何监督依赖问题提供了新途径。
...
Method: 采用离散扩散框架，将HMER重新定义为迭代符号精炼过程而非序列生成；通过多步重掩码逐步精炼符号和结构关系；引入符号感知标记化和随机掩码相互学习机制来增强句法对齐和对书写多样性的鲁棒性。

Result: 在MathWriting基准测试中达到5.56% CER和60.42% EM，优于强Transformer和商业基线；在CROHME 2014-2023数据集上取得一致性能提升。

Conclusion: 离散扩散框架为结构感知的视觉识别提供了超越生成建模的新范式，通过消除因果依赖和改善结构一致性，显著提升了手写数学表达式识别性能。
...
TL;DR: 本文提出了一种多分辨率对齐（MRA）方法来解决基于相机的3D语义场景补全中的体素稀疏性问题，通过利用多分辨率3D特征之间的场景级和实例级对齐作为辅助监督。
...
Method: 提出多分辨率对齐（MRA）方法，包含三个模块：1）多分辨率视图变换器模块，将2D图像特征投影到多分辨率3D特征并进行场景级对齐；2）立方语义各向异性模块，识别每个体素的实例级语义显著性；3）关键分布对齐模块，选择关键体素作为实例级锚点，应用循环损失进行跨分辨率关键特征分布一致性的辅助监督。

Result: 该方法有效缓解了体素稀疏性问题，提高了基于相机的3D语义场景补全的性能。

Conclusion: 提出的多分辨率对齐方法通过利用场景级和实例级对齐作为辅助监督，成功解决了基于相机的3D语义场景补全中的体素稀疏性问题，为自动驾驶感知系统提供了更好的体素级场景感知基础。
...
<details>
  <summary>Details</summary>
Motivation: 癫痫FLAIR MRI中的FCD病变非常细微且稀少，导致传统的联合图像-掩码生成模型容易出现不稳定和记忆化问题，需要更稳定有效的生成方法。

Method: 提出SLIM-Diff模型，包含：(1) 共享瓶颈U-Net架构，通过2通道图像+掩码表示强制解剖结构和病变几何的紧密耦合；(2) 可调Lp目标函数进行损失几何调整，比较了DDPM风格的ε预测与L2损失，以及x0预测与不同Lp几何的效果。

Result: 实验表明：x0预测是联合合成的最强选择；分数次亚二次惩罚（L1.5）能提高图像保真度；而L2损失能更好地保留病变掩码形态。

Conclusion: SLIM-Diff通过紧凑的共享架构和可调损失几何，有效解决了FCD病变生成中的不稳定问题，为医学图像合成提供了更稳定可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度水印方法虽然共享相似的编码器-解码器架构，但在功能行为上差异很大。作者希望建立一个统一框架，从功能层面理解不同水印方法，并探索维度配置如何影响水印行为。

Method: 提出DiM框架，将水印信息建模为不同维度的载荷：一维二进制消息、二维空间掩码、三维时空结构。框架将水印问题形式化为维度感知的映射问题，通过改变嵌入和提取的维度配置来实现不同的水印功能。

Result: 实验表明，仅改变嵌入和提取的维度配置（无需架构更改）就能实现不同的水印能力：时空篡改定位、局部嵌入控制、以及帧顺序被打乱时的时序恢复。在视频领域的实例化展示了更广泛的维度映射可能性。

Conclusion: DiM框架从功能层面统一了现有水印方法，揭示了维度配置是决定水印行为的关键因素。相同维度映射保持载荷结构，支持细粒度控制；跨维度映射实现空间或时空定位。该框架为理解和设计水印系统提供了新视角。
...
TL;DR: SRL通过建立编码器与解码器之间的协同循环，解决了无监督物体中心学习中注意力图与重建图之间的表示差距问题，实现了状态最优性能。


<details>
  <summary>Details</summary>
Motivation: 传统基于槽的无监督物体中心学习模型存在编码器的高频注意力图与解码器的模糊重建图之间的根本冲突，这种差异导致恶性循环：编码器的噪声特征图迫使解码器平均可能性产生更模糊输出，而模糊重建图的梯度缺乏监督编码器特征所需的高频细节。

Method: 提出协同表示学习(SRL)，建立编码器与解码器相互精炼的良性循环。利用编码器的锐利度去模糊解码器输出中的语义边界，同时利用解码器的空间一致性去噪编码器特征。通过带有槽正则化目标的预热阶段稳定这一过程，初始为每个槽分配不同实体。

Result: SRL在视频物体中心学习基准测试中取得了最先进的结果，通过弥合编码器与解码器之间的表示差距，显著提升了性能。

Conclusion: SRL通过建立编码器与解码器之间的协同循环，成功解决了无监督物体中心学习中注意力图与重建图之间的表示差距问题，为相关领域提供了有效的解决方案。
...
TL;DR: Socratic-Geo是一个完全自主的多智能体框架，通过动态耦合数据合成与模型学习来解决MLLMs在几何推理上的瓶颈，仅用少量种子数据就超越了现有方法。
...
Result: 仅从108个种子问题开始，Socratic-Solver在六个基准测试中达到49.11分，仅使用基线四分之一的数据，超越强基线2.43分。Socratic-Generator在GenExam上达到42.4%，超越Seedream-4.0(39.8%)并接近Gemini-2.5-Flash-Image(43.1%)。

Conclusion: Socratic-Geo通过多智能体交互实现了数据合成与模型学习的动态耦合，有效解决了MLLMs几何推理的数据稀缺问题，在性能和效率上都取得了显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 基于流的模型的强化微调（RFT）对偏好对齐至关重要，但常常引入视觉幻觉，如过度优化的细节和语义不对齐。本文旨在探索视觉幻觉产生的原因并寻求减少这些幻觉的方法。

Method: 首先从统一视角研究RFT方法，揭示探索和利用两个核心问题：1）随机微分方程采样中探索有限，导致过度关注局部细节而忽略全局语义；2）策略梯度方法固有的轨迹模仿过程扭曲了模型的基础向量场及其跨步一致性。基于此，提出ConsistentRFT框架，包括动态粒度采样机制（DGR）来平衡全局语义和局部细节的探索，以及一致性策略梯度优化（CPGO）来保持模型一致性。

Result: 实验表明，ConsistentRFT显著减少了视觉幻觉，低层和高层感知幻觉分别平均减少49%和38%。在域外指标上优于其他RFT方法，相比FLUX1.dev提升了5.1%（而基线下降了-0.4%）。

Conclusion: ConsistentRFT框架通过解决RFT中的探索和利用问题，有效减少了视觉幻觉，提高了模型的稳定性和泛化能力，为基于流的模型的强化微调提供了更可靠的解决方案。
...
Method: 分层概念到外观引导框架：概念层使用VAE dropout训练策略，鼓励模型依赖VLM语义信号；外观层引入对应感知掩码注意力模块，限制文本token只关注匹配的参考区域
...
TL;DR: CoViP是一个解决视觉语言模型个性化响应问题的统一框架，通过强化学习后训练和增强生成技术，提升模型基于用户视觉经验生成个性化图像描述的能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型缺乏基于用户特定视觉经验生成个性化响应的能力，无法将新图像与用户积累的视觉-文本上下文关联起来，这限制了模型的个性化应用。

Method: 提出CoViP统一框架，将个性化图像描述作为核心任务，采用基于强化学习的后训练和描述增强生成技术，并引入诊断评估排除文本捷径解决方案。

Result: 实验表明现有开源和专有视觉语言模型存在显著局限性，而CoViP不仅改善了个性化图像描述能力，还在下游个性化任务中带来整体性能提升。

Conclusion: CoViP为实现稳健且可泛化的上下文视觉个性化提供了关键阶段，解决了视觉语言模型在关联用户视觉经验方面的核心挑战。
...
TL;DR: InlierQ是一种基于内点的后训练量化方法，通过梯度感知体积显著性评分区分异常值和信息性内点，使用EM算法拟合后验分布，抑制异常值同时保留有用特征，仅需64个校准样本，在2D/3D目标检测任务上显著降低量化误差。


<details>
  <summary>Details</summary>
Motivation: 目标检测计算需求巨大，部署时速度慢且功耗高，需要量化。但背景杂乱和传感器噪声等任务无关形态会导致冗余激活（异常值），这些异常值扩大激活范围并使分布偏向任务无关响应，使比特分配复杂化并削弱信息特征的保留。缺乏明确区分异常值的标准可能导致抑制异常值时意外丢弃有用信息。

Method: 提出InlierQ，一种以内点为中心的后训练量化方法，分离异常值和信息性内点。计算梯度感知体积显著性评分，将每个体积分类为内点或异常值，使用期望最大化（EM）算法在这些评分上拟合后验分布。这种方法抑制异常值同时保留信息特征。该方法无需标签、即插即用，仅需64个校准样本。

Result: 在COCO和nuScenes基准测试上的实验表明，InlierQ在基于相机（2D和3D）和基于LiDAR（3D）的目标检测中，能够持续降低量化误差。

Conclusion: InlierQ通过有效区分和抑制激活中的异常值，同时保留信息性内点特征，为计算机视觉中的目标检测任务提供了一种高效的后训练量化解决方案，显著改善了量化性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖昂贵的监督训练、强化学习或外部工具来处理表格图像推理，限制了效率和可扩展性。需要一种无需大量标注和外部工具的方法来适应表格推理

Method: 1. DiSCo框架：解耦结构-内容对齐，在多模态对齐过程中显式分离结构抽象和语义基础；2. Table-GLS框架：全局到局部结构引导推理，通过结构化探索和证据基础推理进行表格推理

Result: 在多个基准测试上的广泛实验表明，该框架能有效增强LVLM的表格理解和推理能力，特别是在未见过的表格结构上表现出良好的泛化能力

Conclusion: 提出的框架以最小标注和无外部工具的方式成功适应了大视觉语言模型到表格推理任务，解决了表格图像中复杂布局和结构-内容信息紧密耦合的挑战
...
TL;DR: 提出Logical Anomaly Classification (LAC)任务，将异常检测与细粒度违规分类统一为单步推理，并开发LogiCls视觉语言框架，通过子查询分解和指令合成解决工业图像中的逻辑异常分类问题。


<details>
  <summary>Details</summary>
Motivation: 传统异常检测仅做二元判断，无法指出具体违反的逻辑规则，对质量保证价值有限。工业图像中的逻辑异常（如对象数量、空间布局、组合关系违反约束）需要更细粒度的分类和解释。

Method: 提出LogiCls框架：1) 将复杂逻辑约束分解为可验证的子查询序列；2) 数据中心的指令合成管道生成思维链监督；3) 结合精确的定位标注和多样化的图像-文本增强；4) 采用难度感知重采样策略，重点关注挑战性子查询和长尾约束类型。

Result: 大量实验表明LogiCls能够提供鲁棒、可解释且准确的工业逻辑异常分类，既能预测违规类别，又能提供证据链。

Conclusion: LAC任务统一了异常检测和违规分类，LogiCls框架通过视觉语言模型和思维链监督实现了逻辑敏感的推理，为工业质量保证提供了更细粒度和可解释的解决方案。
...
TL;DR: 该论文提出了首个结合自回归和扩散模型的3D理解与生成统一框架，通过轻量级Transformer桥接LLM和3D扩散模型的特征空间，在保持各自模型先验的同时实现跨模态信息交互，在多个3D任务上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 大型多模态模型的快速发展激发了构建理解与生成统一框架的努力。虽然在2D领域取得了显著成功，但扩展到3D领域仍未被充分探索。现有尝试将3D任务统一到单一自回归范式会导致性能显著下降，因为强制信号量化和训练成本过高。关键洞察在于：核心挑战不是强制统一的自回归范式，而是在最小化损害各自内在能力的同时，实现生成与理解之间的有效信息交互，并利用预训练模型降低训练成本。

Method: 提出了首个结合自回归和扩散模型的3D理解与生成统一框架：采用自回归的下一个token预测范式进行3D理解，采用连续扩散范式进行3D生成。通过一个轻量级Transformer桥接大型语言模型的特征空间和3D扩散模型的条件空间，实现有效的跨模态信息交换，同时保持独立模型学习到的先验知识。

Result: 大量实验表明，该框架在多样化的3D理解和生成基准测试中实现了最先进的性能，同时在3D编辑任务中也表现出色。这些结果突显了统一的自回归+扩散模型作为构建更通用3D智能的有前景方向。

Conclusion: 该研究展示了结合自回归和扩散模型的统一框架在3D理解与生成任务中的有效性，为构建更通用的3D智能系统提供了新的方向。通过桥接不同范式而非强制统一，能够在保持各自优势的同时实现跨模态协同。
...
TL;DR: CDGS提出了一种预算约束的动态高斯泼溅框架，通过可微分预算控制器和自适应分配机制，在严格遵循硬件限制的同时实现最优渲染质量


<details>
  <summary>Details</summary>
Motivation: 动态高斯泼溅技术面临内存消耗与渲染质量之间的根本矛盾：无约束的密集化导致内存消耗过大不适用于边缘设备，而启发式剪枝方法在预设高斯预算下无法达到最优渲染质量

Method: 提出约束动态高斯泼溅(CDGS)框架，将动态场景重建建模为预算约束优化问题；引入可微分预算控制器，基于多模态统一重要性评分融合几何、运动和感知线索；采用静态与动态元素解耦优化和自适应分配机制；实施三阶段训练策略和双模式混合压缩方案

Result: CDGS严格遵循硬件约束（误差<2%），推动率失真性能的帕累托前沿，在变化容量限制下实现最优渲染质量，相比最先进方法实现超过3倍的压缩

Conclusion: CDGS通过预算约束优化框架解决了动态高斯泼溅的内存-质量权衡问题，为边缘设备上的高质量4D重建提供了可行的解决方案
...
TL;DR: 本文研究了四种跨图像数据增强策略（CutMix、CarveMix、ObjectAug和AnatoMix）在多器官分割任务中的应用，发现CutMix、CarveMix和AnatoMix能显著提升分割性能，其中CutMix效果最佳且鲁棒性强。


<details>
  <summary>Details</summary>
Motivation: 多器官分割是临床常规任务，深度学习模型需要大量标注数据，但临床数据稀缺。传统数据增强策略局限于图像内操作，而跨图像和对象级的数据增强策略在多器官分割任务中尚未充分探索。

Method: 研究了四种跨图像数据增强策略：CutMix、CarveMix、ObjectAug和AnatoMix，在两个器官分割数据集上进行实验，并与未使用数据增强的nnUNet基准模型进行比较。

Result: CutMix、CarveMix和AnatoMix相比未使用数据增强的nnUNet，平均Dice分数分别提升了4.9、2.0和1.9。结合传统数据增强策略后，性能可进一步提升。CutMix表现出最强的鲁棒性，即使生成"错误"图像也能提升分割性能。

Conclusion: 跨图像数据增强策略能有效提升多器官分割性能，其中CutMix是最简单且鲁棒的策略。研究为多器官分割任务提供了新的数据增强基准。
...
Result: 在多个基准测试中，ELIQ始终优于现有无标签方法，无需修改即可从AIGC场景泛化到UGC场景，为持续进化的生成模型提供可扩展的无标签质量评估途径。

Conclusion: ELIQ为不断进化的生成模型提供了可扩展的无标签质量评估框架，在视觉质量和提示对齐方面表现出色，具有良好的泛化能力。
...
TL;DR: SlowFocus机制通过识别问题相关时间段进行密集采样，结合多频混合注意力，提升视频大语言模型的细粒度时序理解能力，在多个基准测试中表现优异。
...
Method: 提出SlowFocus机制：1)基于问题识别相关时间段；2)在该段进行密集采样提取局部高频特征；3)使用多频混合注意力模块聚合局部高频细节与全局低频上下文；4)配套训练策略增强时序定位和详细时序推理能力；5)建立FineAction-CGR基准评估细粒度时序理解。

Result: 在现有公开视频理解基准和提出的FineAction-CGR基准上，SlowFocus机制均表现出优越性能，显著提升了视频大语言模型的细粒度时序理解能力。

Conclusion: SlowFocus机制通过在不牺牲帧级视觉token质量的前提下显著提升等效采样频率，有效解决了视频大语言模型在细粒度视频理解方面的局限性，为视频理解研究提供了新方向。
...
TL;DR: DeepTopo-Net：一种结合拓扑感知建模与频率解耦感知的水下伪装目标检测新框架，针对深海细长生物拓扑断裂和透明生物特征提取问题，在首个高分辨率(2K)海洋垂直分层基准GBU-UCOD上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 水下伪装目标检测面临极端视觉相似性挑战，现有方法难以处理深海细长生物的拓扑断裂问题和透明生物的细微特征提取。

Method: 提出DeepTopo-Net框架，包含：1) Water-Conditioned Adaptive Perceptor (WCAP)，使用黎曼度量张量动态变形卷积采样场以应对物理退化；2) Abyssal-Topology Refinement Module (ATRM)，通过骨架先验保持细长目标的结构连通性；3) 引入首个高分辨率(2K)海洋垂直分层基准GBU-UCOD。

Result: 在MAS3K、RMAS和提出的GBU-UCOD数据集上的广泛实验表明，DeepTopo-Net实现了最先进的性能，特别是在保持复杂水下图案的形态完整性方面表现突出。

Conclusion: DeepTopo-Net通过拓扑感知建模和频率解耦感知有效解决了水下伪装目标检测的关键挑战，特别是在深海环境中细长生物和透明生物的检测问题上取得了显著进展。
...
TL;DR: 提出Refer-Agent，一种基于多智能体协作和交替推理-反思机制的零样本视频指代分割方法，无需大规模监督微调即可超越现有方法


<details>
  <summary>Details</summary>
Motivation: 当前基于大规模监督微调的方法数据依赖性强、可扩展性有限，而零样本方法性能显著落后。需要一种既灵活又高性能的解决方案

Method: 提出多智能体协作系统，包含：1) 粗到细的帧选择策略确保多样性和文本相关性；2) 动态焦点布局自适应调整视觉焦点；3) 链式反思机制通过提问者-回答者对生成自我反思链，验证中间结果并为下一轮推理提供反馈

Result: 在五个挑战性基准测试中显著优于最先进方法，包括基于监督微调的模型和零样本方法。系统灵活，无需额外微调成本即可快速集成新的多模态大语言模型

Conclusion: Refer-Agent通过多智能体协作和交替推理-反思机制，为视频指代分割提供了一种既高性能又灵活的零样本解决方案，解决了现有方法的局限性
...
TL;DR: EB-JEPA是一个开源库，用于通过联合嵌入预测架构学习表示和世界模型，支持从图像到视频再到动作条件世界模型的学习，提供模块化实现和单GPU训练示例。


<details>
  <summary>Details</summary>
Motivation: 为了解决生成模型在表示学习中的局限性，开发一个能够学习语义丰富特征、避免表示崩溃，并能扩展到视频和动作条件世界模型的框架，同时降低研究门槛。

Method: 使用联合嵌入预测架构在表示空间而非像素空间进行预测，提供模块化实现，包括图像级自监督学习、视频多步预测和动作条件世界模型，通过正则化组件防止表示崩溃。

Result: 在CIFAR-10上获得91%的表示探测准确率，在Moving MNIST上展示多步预测能力，在Two Rooms导航任务中达到97%的规划成功率，验证了各正则化组件的重要性。

Conclusion: EB-JEPA成功展示了JEPA框架在表示学习和世界建模中的有效性，为研究社区提供了可访问的工具，证明了从图像到视频再到动作条件模型的扩展可行性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的训练免费视频理解方法存在严重的视觉冗余和高计算开销问题，特别是处理长视频时。基于CLIP相似度的关键帧选择策略容易产生偏见，可能遗漏关键帧，导致视频理解效果不佳。

Method: KTV采用两阶段框架：第一阶段通过聚类帧级视觉特征进行问题无关的关键帧选择，得到紧凑、多样且具有代表性的帧子集；第二阶段对每个选中的关键帧进行关键视觉token选择，基于token重要性和冗余性剪枝，显著减少输入LLM的token数量。

Result: 在Multiple-Choice VideoQA任务上的实验表明，KTV优于最先进的训练免费基线方法，同时使用显著更少的视觉token（例如，60分钟10800帧的视频仅用504个视觉token），在MLVU-Test基准上达到44.8%的准确率。在某些基准测试中，KTV甚至超过了一些需要训练的方法。
...
<details>
  <summary>Details</summary>
Motivation: 语义分割需要大量像素级标注数据，合成图像虽能提供标注但存在域差距问题。风格迁移可减少这种差异，但传统方法难以选择有效的变换组合和顺序。

Method: 将风格迁移建模为流水线排序问题，使用多目标遗传算法优化，平衡结构一致性和风格相似性。采用成对图像度量进行快速评估，避免传统分布度量需要大量图像生成的限制。

Result: 在GTA5到Cityscapes和ACDC的合成到真实域适应任务中，进化算法能生成适应不同目标的多样化增强流水线，有效提升语义分割性能。

Conclusion: 将风格迁移作为排序问题用进化算法优化是有效的，高效度量方法使该空间搜索可行，为合成到真实域适应提供了新方法。
...
TL;DR: 该论文提出了首个稀疏部分弱监督定向目标检测框架，通过少量稀疏弱标注数据和大量未标注数据，在遥感图像中实现高效的目标检测，显著降低了标注成本。


<details>
  <summary>Details</summary>
Motivation: 遥感领域中目标分布密集、类别多样导致标注成本极高，现有定向目标检测方法在标注效率方面仍有提升空间。为了应对大规模标注的挑战，需要开发能够利用少量弱标注数据和大量未标注数据的高效方法。

Method: 提出了SPWOOD框架，包含三个关键创新：1) SOS-Student模型在稀疏标注设置下从背景中分离未标注目标，并从方向或尺度无关的弱标注中学习方向和尺度信息；2) 多级伪标签过滤策略利用模型多层级预测的分布信息；3) 独特的稀疏分区方法确保每个类别得到平等处理。

Result: 在DOTA和DIOR数据集上的大量实验表明，该框架相比传统定向目标检测方法取得了显著的性能提升，提供了高成本效益的解决方案。

Conclusion: 该研究提出的稀疏部分弱监督定向目标检测框架能够有效利用稀疏弱标注和未标注数据，在遥感目标检测任务中实现了性能与标注成本的良好平衡，为大规模遥感图像分析提供了实用解决方案。
...
TL;DR: MM-SCALE是一个大规模多模态道德对齐数据集，通过5点标量评分和显式模态标注来提升视觉语言模型的道德判断能力。
...
Result: 在MM-SCALE上微调的视觉语言模型相比基于二元信号的训练，获得了更高的排序保真度和更稳定的安全校准性能。

Conclusion: 通过从离散监督转向标量监督，MM-SCALE框架提供了更丰富的对齐信号和更精细的多模态道德推理校准，显著提升了视觉语言模型的道德判断能力。
...
<details>
  <summary>Details</summary>
Motivation: 传统工业异常检测方法面临挑战：无监督方法需要手动阈值，监督方法因数据稀缺和类别不平衡而过拟合，且都受限于"一种异常一个模型"的限制

Method: 提出RIAS范式，利用文本描述指导检测；引入MVTec-Ref数据集，包含多样化指代表达式，重点关注异常模式；提出DQFormer基准模型，采用双查询令牌和语言门控多级聚合

Result: RIAS能够从文本描述生成精确掩码，无需手动阈值，使用通用提示通过单个模型检测多样化异常，实验证明其有效性

Conclusion: RIAS范式将工业异常检测推向开放集能力，通过语言指导实现更灵活、高效的异常检测
...
TL;DR: 提出RegionReasoner强化学习框架，通过显式引用边界框实现多轮视觉推理，结合全局-局部一致性奖励提升推理准确性和空间定位精度


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型大多依赖单步或纯文本推理，缺乏跨多轮视觉上下文迭代精炼理解的能力，限制了复杂视觉推理任务的性能

Method: 提出RegionReasoner强化学习框架：1) 要求每个推理轨迹显式引用对应的参考边界框实现接地推理；2) 通过全局-局部一致性奖励保持语义连贯性，从全局场景描述和区域级描述中提取关键对象和名词，与推理轨迹对齐；3) 结合定位保真度和全局-局部语义对齐的结构化奖励进行优化

Result: 在检测和分割任务上的实验表明，RegionReasoner-7B配合新引入的RegionDial-Bench基准，显著提升了多轮推理准确性、空间定位精度和全局-局部一致性，为该新兴研究方向建立了强基线

Conclusion: RegionReasoner框架通过强化接地推理和语义一致性，有效解决了多轮视觉推理的挑战，为迭代式视觉理解提供了新的解决方案和评估基准
...
TL;DR: LIVE提出了一种新的长时域交互视频世界模型，通过循环一致性目标限制误差累积，无需教师蒸馏，在长时域视频生成上达到SOTA性能。
...
Method: LIVE通过循环一致性目标强制限制误差累积：首先从真实帧进行前向展开，然后应用反向生成过程重建初始状态，在重建的终端状态上计算扩散损失，从而约束长时域误差传播。同时引入统一框架视角和渐进训练课程来稳定训练。

Result: 实验表明LIVE在长时域基准测试中达到最先进性能，能够生成超出训练展开长度的稳定、高质量视频。

Conclusion: LIVE通过循环一致性约束有效解决了长时域视频生成中的误差累积问题，无需教师蒸馏，为长时域交互视频世界模型提供了新的解决方案。
...
TL;DR: 提出自动化框架，将静态动漫插图转换为可操控的2.5D模型，替代需要手动分割和遮挡区域艺术"幻觉"的专业工作流程
...
Result: 能够生成高保真、可操控的模型，适用于专业的实时动画应用，能够解决动漫角色复杂分层问题（如交错发丝）
...
TL;DR: REPA-G是一种利用自监督模型对齐表征进行推理时条件控制的扩散模型框架，通过优化相似性目标在生成过程中实现多尺度语义引导


<details>
  <summary>Details</summary>
Motivation: 虽然自监督模型表征对齐已被证明能改进扩散模型训练，但其在推理时条件控制方面的潜力尚未充分探索。现有方法主要依赖模糊的文本提示或粗糙的类别标签，缺乏灵活精确的控制手段。

Method: 提出表征对齐引导（REPA-G）框架，利用预训练特征提取器提取的条件表征，在推理时通过优化相似性目标（势函数）来引导去噪过程。支持从单块纹理匹配到全局图像特征标记的多尺度控制，并能扩展到多概念组合。

Result: 在ImageNet和COCO数据集上的定量结果显示，该方法能生成高质量、多样化的图像。理论分析表明该引导方法能够从势函数诱导的倾斜分布中采样。

Conclusion: REPA-G提供了一种完全在推理时操作的灵活精确条件控制方法，可作为模糊文本提示或粗糙类别标签的有效替代方案，实现了多尺度的语义引导和多概念组合。
...
TL;DR: RAWDet-7是一个包含约2.5万训练和7.6千测试RAW图像的大规模数据集，用于研究RAW图像处理中的物体检测和描述任务，支持模拟4/6/8位量化评估。


<details>
  <summary>Details</summary>
Motivation: 传统视觉模型基于经过ISP处理、为人类感知优化的RGB图像训练，这会丢弃对机器推理有用的传感器级信息。RAW图像保留了未处理的场景数据，能够为物体检测和描述提供更丰富的线索。

Method: 构建RAWDet-7数据集，包含来自不同相机、光照条件和环境的RAW图像，按照MS-COCO和LVIS标准密集标注了7个物体类别，并提供从对应高分辨率sRGB图像中提取的物体级描述。

Result: 创建了一个大规模RAW图像数据集，支持在模拟4位、6位和8位量化条件下评估物体检测性能、描述质量和细节保留，为低比特RAW图像处理研究提供基准。

Conclusion: RAWDet-7数据集填补了RAW图像处理研究领域的空白，为研究物体级信息在RAW图像处理和低比特量化下的保留提供了重要资源，有助于推动基于传感器级信息的机器视觉模型发展。
...
TL;DR: 提出基于人眼视网膜和初级视觉皮层的仿生视觉接口FOVI，将视网膜式可变分辨率传感器阵列转换为均匀密集的V1式传感器流形，通过kNN卷积实现高效计算，显著降低高分辨率视觉任务的计算成本。


<details>
  <summary>Details</summary>
Motivation: 人类视觉具有中央凹特性，分辨率从中心向外围递减，这种主动感知机制能高效处理大视野场景。而传统计算机视觉系统采用均匀分辨率编码，处理全视野高分辨率图像时计算效率低下，需要仿生解决方案。

Method: 提出FOVI接口，模拟视网膜到V1的映射：1）将视网膜式可变分辨率阵列转换为均匀密集的V1式传感器流形；2）在传感器流形上定义k最近邻感受野；3）通过核映射技术实现kNN卷积。展示了两种应用：端到端kNN卷积架构和基于DINOv3 ViT的中央凹适应模型（使用LoRA低秩适应）。

Result: 模型在保持竞争力的性能下，计算成本仅为非中央凹基线的几分之一，为高分辨率第一人称视觉的高效可扩展主动感知开辟了新途径。代码和预训练模型已开源。

Conclusion: FOVI成功将人类视觉的中央凹特性引入计算机视觉系统，通过仿生设计实现了高效的高分辨率视觉处理，为主动感知系统提供了有前景的技术路径。
...
TL;DR: QVLA提出首个面向具身控制的动作中心量化框架，通过通道级比特分配策略，在保持98.9%性能的同时仅需29.2%显存，相比LLM量化方法性能提升22.6%


<details>
  <summary>Details</summary>
Motivation: 视觉-语言-动作模型计算需求巨大，难以部署在资源受限的机器人平台。现有LLM量化方法优先考虑数据保真度，但忽略了微小动作偏差会累积成灾难性任务失败的问题

Method: 提出QVLA框架，采用通道级比特分配策略，直接测量量化每个通道到不同比特宽度时的最终动作空间敏感性，通过全局优化统一量化和剪枝（0比特）

Result: 在LIBERO基准测试中，OpenVLA-OFT量化版本仅需原始模型29.2%显存，保持98.9%原始性能，速度提升1.49倍，相比SmoothQuant性能提升22.6%
...
<details>
  <summary>Details</summary>
Motivation: 神经外科中，图像引导神经外科系统依赖术前MRI定位手术靶点和路径，但硬脑膜打开后脑移位会使术前MRI失效，需要术中MRI更新以补偿脑移位，提高导航精度和手术效果

Method: 提出NeuralShift模型，基于U-Net架构，仅使用术前MRI预测颞叶切除术中的脑移位。使用目标配准误差（TRE）评估切除侧和中线解剖标志点的位移精度，用DICE分数比较预测的术中掩模与术中MRI衍生的掩模

Result: 模型能准确预测脑的全局变形（DICE 0.97）和局部位移（TRE低至1.12mm），有效补偿颞叶切除手术中的大范围脑移位
...
<details>
  <summary>Details</summary>
Motivation: 自回归图像生成面临的关键挑战是如何在并行采样独立位置的同时，通过序列条件建模相互依赖关系。现有方法要么通过多尺度金字塔在尺度间进行条件建模，要么通过规则分区或随机排序在单张图像内并行采样。

Method: 提出基于渐进棋盘格的固定排序方法，在每个尺度上从均匀间隔的区域并行采样，在四叉树细分的每个层级保持完全平衡。这种方法实现了尺度间和尺度内的有效条件建模。

Result: 研究发现，在平衡设置下，只要总序列步骤数恒定，广泛的尺度放大因子都会产生相似结果。在类别条件ImageNet上，该方法在使用较少采样步骤的情况下，实现了与具有相似模型容量的最新自回归系统相竞争的性能。

Conclusion: 渐进棋盘格排序为多尺度自回归图像生成提供了一种灵活高效的采样策略，能够在保持生成质量的同时减少采样步骤。
...
TL;DR: DualSpeed框架通过快速-慢速双模式训练解决MLLMs训练效率问题，快速模式使用视觉token剪枝提升效率，慢速模式保持训练-推理一致性，实现2.1-4.0倍加速且性能保留99%以上


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型训练效率低下，现有方法主要关注减少模型规模或可训练参数。视觉token剪枝在推理阶段有效，但直接应用于训练会导致训练-推理不匹配问题

Method: 提出DualSpeed双速训练框架：1) 快速模式作为主模式，集成现有VTP方法减少视觉token，使用模式隔离器隔离模型行为；2) 慢速模式作为辅助模式，在完整视觉序列上训练保持一致性；3) 通过自蒸馏让慢速模式从充分训练的快速模式学习

Result: 实验显示DualSpeed将LLaVA-1.5训练加速2.1倍，LLaVA-NeXT训练加速4.0倍，同时保持超过99%的性能表现

Conclusion: DualSpeed框架成功解决了MLLMs训练效率问题，通过双模式设计既实现了训练加速，又避免了训练-推理不匹配，为高效多模态模型训练提供了新方向
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散编辑模型缺乏平滑控制文本引导编辑强度的机制，标准的分类器自由引导在编辑模型中无法实现从输入到编辑结果的平滑过渡

Method: 提出自适应原点引导方法，用身份条件化的自适应原点调整标准引导原点，通过身份指令对应身份操作，根据编辑强度在身份预测和标准无条件预测之间插值

Result: 在图像和视频编辑任务上评估，相比当前基于滑块的编辑方法，提供了更平滑和一致的控制，无需每次编辑的特殊处理或依赖专门数据集

Conclusion: AdaOr方法通过调整引导原点实现了对文本引导编辑强度的连续控制，解决了现有扩散编辑模型在编辑强度控制上的局限性
...
Method: 结合3D有符号距离函数和密度场学习与事件监督，引入球谐编码处理视角依赖效应

Result: 显著优于现有方法，平均Chamfer距离降低34%，平均绝对误差降低31%

Conclusion: EventNeuS首次实现了事件相机的高精度3D表示学习，为事件相机3D重建开辟了新方向
