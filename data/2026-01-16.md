...
TL;DR: 比较DenseNet121和EfficientNet-B0两种CNN架构在儿童肺炎X光片自动检测中的性能，EfficientNet-B0表现更优，准确率达84.6%，结合可解释性技术增强临床可信度。
...
Method: 使用5,863张儿童胸片公开数据集，进行归一化、调整大小和数据增强预处理。在相同训练设置下微调DenseNet121和EfficientNet-B0（使用预训练ImageNet权重）。使用Grad-CAM和LIME进行模型可解释性分析。
...
<details>
  <summary>Details</summary>
Motivation: 传统评估指标如BLEU或ROUGE无法有效衡量跨模态信息覆盖率，特别是文本段落与关键帧序列之间的信息比较，需要新的评估框架。

Method: 提出视频摘要信息损失(ViSIL)评分，基于视觉语言模型推理，通过信息论框架量化视频信息未被摘要捕获的程度。

Result: ViSIL评分与人类和VLM在视频问答任务上的表现呈显著统计相关性；通过ViSIL进行摘要选择可在信息损失和处理速度间实现帕累托最优，比文本摘要提升7%的VQA准确率。

Conclusion: ViSIL提供了一个统一的跨模态评估框架，能够有效衡量视频摘要的信息覆盖率，并在信息保留和处理效率之间实现优化平衡。
...
TL;DR: TuneCLIP是一个自监督微调框架，能够在现有自监督数据集上提升开源CLIP模型在各种下游任务中的性能，避免性能下降问题。


<details>
  <summary>Details</summary>
Motivation: CLIP作为多模态表示学习的基石，通常需要从头训练数十亿样本才能提升性能，成本极高。本研究探索能否仅使用现有自监督数据集提升开源CLIP模型在各种下游任务中的通用性能，而不是针对单一任务的监督微调。

Method: TuneCLIP包含两个关键组件：1）基于理论分析的热身阶段，恢复优化统计量以减少冷启动偏差；2）微调阶段，优化新的对比损失函数以减轻对假阴性对的惩罚。

Result: TuneCLIP在不同模型架构和规模上都能一致提升性能。特别是将SigLIP（ViT-B/16）等领先开源模型的ImageNet及相关分布外基准提升高达+2.5%，在高度竞争的DataComp基准上提升+1.2%，为高效的后预训练适应设立了新的强基线。

Conclusion: TuneCLIP成功解决了从开源CLIP模型开始应用标准训练协议时常见的性能下降问题，提供了一种有效的自监督微调框架，能够在不需要从头训练的情况下显著提升CLIP模型在各种任务中的通用性能。
...
Method: 开发了VibrantSR生成式超分辨率框架，从10米分辨率的Sentinel-2影像估计0.5米分辨率的冠层高度模型，利用季节性合成数据实现连续监测

Result: 在美国西部22个EPA三级生态区域评估中，VibrantSR对冠层高度≥2米的区域实现了4.39米的平均绝对误差，优于Meta(4.83米)、LANDFIRE(5.96米)和ETH(7.05米)等卫星基准方法

Conclusion: 虽然基于航空影像的VibrantVS(2.71米MAE)在精度上仍有优势，但VibrantSR能够在不需要昂贵且时间频率低的航空采集的情况下，实现大陆尺度的操作性森林监测和碳核算
...
TL;DR: OT-Drive：基于最优传输的多模态融合框架，用于非结构化环境中的可通行区域分割，在分布外场景下具有强泛化能力
...
Method: 提出最优传输驱动的多模态融合框架，设计场景锚点生成器分解场景信息为联合分布，构建可泛化的语义锚点；设计基于最优传输的多模态融合模块，将RGB和表面法线特征传输到语义锚点定义的流形上
...
TL;DR: 本文指出当前视觉语言模型在空间关系理解方面存在盲点，主要原因是CLIP风格的图像编码器将图像展平为1D序列，丢失了2D结构信息。作者通过研究替代训练目标和2D位置编码来提升模型的空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型虽然发展迅速，但在捕捉空间关系方面存在明显不足。这主要是因为它们通常采用CLIP风格的图像编码器，训练过程中将图像展平为1D补丁序列，丢弃了理解空间关系所需的2D结构信息。这种空间感知能力的缺失限制了模型在需要空间基础的应用（如机器人和具身AI）中的表现。

Method: 作者从两个方面进行改进研究：(1) 探索使用替代训练目标的图像编码器；(2) 引入2D位置编码。这些架构选择旨在更好地保留和处理图像的2D空间结构信息。

Result: 实验结果表明，通过上述架构改进，模型在多个基准测试中的空间推理能力得到了显著提升。

Conclusion: 空间感知能力是视觉语言模型设计中缺失的重要维度，也是限制其在需要空间基础应用中表现的关键瓶颈。通过改进图像编码器的训练目标和引入2D位置编码，可以有效提升模型的空间推理能力。
...
TL;DR: DR²Seg是一个自奖励框架，通过两阶段策略解决推理分割中的过度思考问题，提升推理效率和分割精度


<details>
  <summary>Details</summary>
Motivation: 现有推理分割方法存在过度思考问题，生成冗长的推理链会干扰多模态大语言模型中的目标定位
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注视频中事件、物体和场景存在的幻觉，而忽略了事件关系幻觉。视频大语言模型在处理密集事件关系推理时存在困难，常常依赖先验知识而非充分利用帧级线索。

Method: 提出了VERHallu基准，专注于事件间的因果、时序和子事件关系，包含关系分类、问答和反事实问答三种任务。同时提出了关键帧传播策略，在中间层重新分配帧级注意力以增强多事件理解。

Result: 实验表明当前最先进的VideoLLMs在密集事件关系推理方面表现不佳，虽然对关键事件有较强的定位能力，但常常忽略周围的子事件，导致对事件关系的理解不完整和不准确。提出的KFP策略能有效缓解事件关系幻觉且不影响推理速度。

Conclusion: 事件关系幻觉是视频大语言模型的一个重要问题，需要专门的评估基准和缓解策略。关键帧传播策略通过重新分配注意力机制，能够在不影响效率的情况下改善模型对复杂事件关系的理解能力。
...
TL;DR: 提出DiCo框架，通过解耦概念表示实现层次化的跨模态对齐，用于文本到图像行人重识别任务，在多个数据集上取得竞争性性能并提升可解释性。
...
Method: 提出DiCo框架，采用共享的基于槽位的表示，每个槽位作为跨模态的部分级锚点，并进一步分解为多个概念块，实现互补属性的解耦，同时保持图像与文本之间一致的部分级对应关系。

Result: 在CUHK-PEDES、ICFG-PEDES和RSTPReid数据集上的大量实验表明，该框架与最先进方法相比具有竞争性性能，同时通过显式的槽位和块级表示增强了可解释性，实现更细粒度的检索结果。

Conclusion: DiCo框架通过层次化解耦的概念表示有效解决了文本到图像行人重识别中的模态鸿沟和细粒度对应问题，在保持性能竞争力的同时提升了模型的可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 水下成像面临波长相关光衰减、悬浮颗粒散射、浑浊模糊和非均匀照明等挑战，传统相机难以获取真实运动信息。事件相机具有微秒分辨率和动态范围优势，但缺乏真实水下光学与准确光流配对的训练数据。

Method: 使用基于物理的射线追踪渲染水下RGBD序列，通过现代视频到事件转换管道生成逼真的事件数据流，包含密集的真实光流、深度和相机运动信息。
...
TL;DR: CoF-T2I模型将视频生成中的链式帧推理引入文本到图像生成，通过渐进式视觉细化过程提升图像质量
...
TL;DR: 该论文提出了一种基于流引导几何平流的3D高斯泼溅框架，将后印象派艺术风格（强调几何抽象而非表面纹理）应用于3D场景重建，通过提取2D绘画中的方向流场并反向传播到3D空间，实现与场景拓扑一致的笔触对齐。


<details>
  <summary>Details</summary>
Motivation: 现有3D风格迁移方法大多将几何视为刚性基底，仅进行表面纹理投影，这与后印象派艺术"在本质中寻求夸张"的核心原则相悖。后印象派强调几何抽象作为主要表达手段，而非照片级细节。因此需要开发能够真正体现后印象派风格化的3D方法。

Method: 提出流引导几何平流框架：1) 从2D绘画中提取方向流场并反向传播到3D空间；2) 调整高斯基元形成与流场对齐的笔触，无需显式网格先验；3) 采用亮度-结构解耦策略，将几何变形与颜色优化分离；4) 使用基于投影的网格无关流引导机制。

Result: 该方法能够在3D高斯泼溅中实现表达性结构变形，直接由绘画运动而非光度约束驱动。通过几何抽象作为主要表达手段，能够更真实地再现后印象派风格化效果。

Conclusion: 该研究提出了一种新的3D风格迁移范式，将后印象派艺术的几何抽象原则应用于3D场景重建。通过流引导几何平流框架，实现了基于绘画运动的结构变形，并通过VLM-as-a-Judge评估框架解决艺术风格化的主观性评估问题，为艺术驱动的3D内容创作提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有广义类别发现方法依赖伪标签或两阶段聚类，缺乏从实例特定噪声中显式解耦类别定义信号的机制。需要从信息论角度重新思考GCD问题，解决表示空间中的噪声干扰问题。

Method: 基于信息瓶颈原理，提出InfoSculpt框架，通过最小化双重条件互信息目标：1）在标注数据上使用类别级CMI学习已知类的紧凑判别表示；2）在所有数据上使用实例级CMI通过压缩增强诱导噪声来提取不变特征。

Result: 在8个基准测试上的广泛实验验证了InfoSculpt的有效性，表明该信息论方法能够产生解耦且鲁棒的潜在空间，保留类别信息同时丢弃噪声细节。

Conclusion: 从信息论角度重新构建GCD问题，提出的InfoSculpt框架通过双重CMI目标系统性地塑造表示空间，在已知和未知类别分类任务中取得显著效果，为开放世界应用提供了新思路。
...
TL;DR: MathDoc是首个针对真实高中数学试卷文档级信息提取的基准，包含3609个有真实噪声的问题，专门评估模型在文档质量差时的拒绝能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注干净文档或通用布局分析，忽略了数学问题的结构完整性以及模型主动拒绝不完整输入的能力。真实场景中数学试卷存在严重视觉噪声，需要专门基准来评估模型在退化文档条件下的可靠性。

Method: 提出MathDoc基准，包含3609个精心策划的真实高中数学试题，包含实际噪声和不可识别样本。采用多维评估框架，涵盖题干准确性、视觉相似度和拒绝能力三个维度。

Result: 实验表明，包括Qwen3-VL和Gemini-2.5-Pro在内的SOTA MLLMs在端到端提取方面表现良好，但在拒绝不可读输入方面持续失败，反而产生自信但无效的输出。
...
TL;DR: 该论文提出了一个超越单提示融合的多组合协作融合框架，通过生成三个上下文表示分支来整合不同高质量提示组合的信息，使用MULTI-VQGAN架构联合解释多源信息，在多种视觉任务上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前主流的"检索-提示"方法通常只选择单个最佳视觉提示，丢弃了其他合适候选的宝贵上下文信息。即使最近的工作探索了将top-K提示融合为单一增强表示，也只是将多个丰富信号压缩为一个，限制了模型的推理能力。需要更全面、协作的融合方法来充分释放这些多样化上下文的潜力。

Method: 提出了一个新颖框架，从单提示融合转向多组合协作融合。方法生成三个上下文表示分支，每个分支通过整合不同高质量提示组合的信息形成。这些互补的指导信号被输入到提出的MULTI-VQGAN架构中，该架构专门设计用于联合解释和利用来自多个源的协作信息。

Result: 在多种任务（包括前景分割、单目标检测和图像着色）上的广泛实验表明，该方法具有强大的跨任务泛化能力、有效的上下文融合能力，能够比现有方法产生更稳健和准确的预测。

Conclusion: 多组合协作融合方法能够更好地利用多样化上下文信息，超越传统的单提示或简单融合方法，在视觉上下文学习中实现更强大的性能和更准确的预测。
...
TL;DR: 提出端到端视觉上下文学习框架，通过自适应融合模块聚合多提示信息，引入布局特定MLP解耦布局先验，采用双向微调机制增强融合模块与修复模型协作，在多个视觉任务上表现优异。
...
Method: 1) 自适应融合模块聚合多个提示的关键模式和标注信息，形成更精确的上下文提示；2) 引入排列特定的轻量级MLP，将布局先验与核心模型解耦；3) 采用双向微调机制，交换查询和提示的角色，鼓励模型从融合上下文中重建原始提示。

Result: 在前景分割、单目标检测和图像着色等任务上的实验表明，该方法取得了优异结果，并展现出强大的跨任务泛化能力。

Conclusion: 提出的端到端视觉上下文学习框架通过多提示融合、布局先验解耦和双向微调机制，有效解决了现有方法的局限性，在多个视觉任务上实现了卓越性能。
...
Result: 在收集的828例CT扫描的肺癌数据集和其他公共基准测试上进行了广泛实验，VQ-Seg方法超越了现有的最先进方法，证明了其有效性。

Conclusion: VQ-Seg通过向量量化和量化扰动模块解决了传统dropout方法的超参数敏感问题，结合双分支架构和基础模型指导，在半监督医学图像分割中实现了更有效的正则化和更好的性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态潜在推理通常依赖外部监督（如辅助图像），忽略了内在的视觉注意力动态。研究发现存在关键的感知鸿沟：学生模型经常模仿教师的文本输出，但关注完全不同的视觉区域，实际上依赖语言先验而非基于感知的推理。

Method: 提出LaViT框架，通过让学生模型自回归地重建教师的视觉语义和注意力轨迹（在文本生成之前），并采用课程感知门控机制来防止捷径学习，从而对齐潜在视觉思维而非静态嵌入。

Result: LaViT显著增强了视觉基础能力，在复杂推理任务上实现了高达+16.9%的性能提升，使一个紧凑的3B参数模型能够超越更大的开源变体和GPT-4o等专有模型。

Conclusion: 通过强制模型在生成文本之前先重建视觉语义和注意力轨迹，LaViT有效解决了多模态推理中的感知鸿沟问题，实现了更可靠的视觉基础推理。
...
TL;DR: 该论文提出了视频异常推理(VAR)新任务，将视频异常分析从描述性理解提升到结构化多阶段推理，并构建了包含8,641个视频和50,000+样本的大规模数据集，提出了基于感知-认知-行动链式思维的结构化标注方法。
...
Method: 1. 定义视频异常推理(VAR)新任务，要求模型在回答异常相关问题前进行渐进式推理；2. 构建包含8,641个视频的大规模数据集，采用感知-认知-行动链式思维(PerCoAct-CoT)进行结构化标注；3. 提出异常感知组相对策略优化方法增强弱监督下的推理可靠性；4. 开发端到端MLLM模型Vad-R1-Plus，支持自适应分层推理和风险感知决策。

Result: 构建了包含8,641个视频和50,000+样本的大规模视频异常推理数据集，是视频异常领域最大的数据集之一。提出的Vad-R1-Plus模型在VAR任务上超越了开源和专有基线模型，有效提升了MLLM在视频异常推理方面的能力。

Conclusion: 该研究通过定义VAR任务、构建结构化数据集和开发端到端模型，成功将视频异常分析从描述性理解提升到结构化多阶段推理，为视频异常检测和理解领域提供了新的研究方向和评估基准。
...
TL;DR: RAG-3DSG通过重新拍摄引导的不确定性估计和检索增强生成技术，显著提升了开放词汇3D场景图的生成精度和速度。
...
Method: 提出RAG-3DSG方法：1）通过重新拍摄引导的不确定性估计来减少聚合噪声；2）利用可靠低不确定性物体支持物体级检索增强生成；3）采用动态下采样映射策略加速跨图像物体聚合。

Result: 在Replica数据集上的实验表明，RAG-3DSG显著提高了3D场景图中节点标注的准确性，同时将映射时间减少了三分之二。

Conclusion: RAG-3DSG通过不确定性估计和检索增强生成技术，有效解决了开放词汇3D场景图生成中的准确性和效率问题，为机器人操作和导航等下游任务提供了更好的结构化语义表示。
...
Method: 1. 引入前馈的Mesh2Gaussian先验模型（MGPM）快速初始化高斯化身；2. 设计测试时生成适应阶段，使用真实和合成图像作为监督；3. 提出渲染引导的单步扩散增强器，基于高斯化身渲染恢复缺失的视觉细节，避免传统全扩散去噪策略的缓慢和幻觉问题。
...
Method: 提出DepthDirector框架，使用显式3D表示的深度视频作为相机控制引导；设计视图-内容双流条件机制，将源视频和目标视角下的变形深度序列注入预训练视频生成模型；采用轻量级LoRA视频扩散适配器进行训练；构建大规模多相机同步数据集MultiCam-WarpData。

Result: 实验表明DepthDirector在相机可控性和视觉质量方面均优于现有方法。

Conclusion: DepthDirector通过深度引导和双流条件机制，有效解决了精确相机控制中的内容一致性问题，为视频重渲染提供了新方法。
...
TL;DR: 本文提出一个针对复杂视频问答的框架，通过查询/选项预处理、领域特定微调、时序思维链提示和后处理，在HD-EPIC VQA基准上达到41.6%准确率


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在复杂视频问答基准（如HD-EPIC VQA）上表现不佳，主要面临模糊查询/选项、长程时序推理能力差、输出非标准化等问题

Method: 提出集成框架：1) 查询/选项预处理；2) 领域特定的Qwen2.5-VL微调；3) 新颖的时序思维链提示用于多步推理；4) 鲁棒的后处理机制

Result: 在HD-EPIC VQA基准上达到41.6%的准确率，显著优于现有方法，展示了整体管道优化在视频理解任务中的重要性

Conclusion: 研究表明，对于复杂的视频理解任务，需要采用整体管道优化策略，而不仅仅是改进单个组件。提出的框架为视频问答系统提供了有效的解决方案，代码和微调模型已开源
...
<details>
  <summary>Details</summary>
Motivation: 在观看演示时，听众需要在演讲者叙述和幻灯片内容之间来回切换，这造成了听觉与视觉的脱节，特别是在快节奏或内容密集的演示中，难以吸收关键细节。

Method: 引入一种自动识别和突出显示最相关幻灯片区域的方法，通过分析口语内容并与幻灯片中的文本或图形元素进行匹配，确保听众听到的内容与他们需要关注的内容之间更好的同步。

Result: 探索了解决该问题的不同方法，并评估了它们的成功和失败案例。代码和数据集已在GitHub上公开。
...
TL;DR: 该论文提出了HRA（分层细化攻击），一种针对视觉语言预训练模型的多模态通用对抗攻击框架，通过样本级和优化级双重细化来生成通用对抗扰动，解决了现有攻击方法计算开销大的问题。
...
Method: HRA在样本级和优化级两个层次上细化通用对抗扰动：1）图像模态：将对抗样本解耦为干净图像和扰动，分别处理以更有效地破坏跨模态对齐；引入ScMix增强策略多样化视觉上下文；2）文本模态：结合句内和句间重要性度量识别全局有影响力的词作为通用文本扰动；3）优化路径：利用历史和估计未来梯度的时间层次结构避免局部极小值。

Result: 在各种下游任务、VLP模型和数据集上的广泛实验证明了所提出的通用多模态攻击方法的优越性。

Conclusion: HRA框架通过分层细化策略实现了对VLP模型的高效通用对抗攻击，在保持攻击效果的同时显著降低了计算开销。
...
TL;DR: ROMA是一个实时全模态助手，通过同步多模态单元处理连续音频-视频流，使用轻量级speak head解耦响应触发与生成，在主动监控和被动问答任务上均表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有全多模态大语言模型在流式音频-视频理解方面存在局限：要么模态支持不完整，要么缺乏自主主动监控能力，无法实现统一的反应式和主动式交互。

Method: 1) 将连续输入处理为同步多模态单元，对齐密集音频与离散视频帧以处理粒度不匹配；2) 引入轻量级speak head解耦响应触发与生成，确保精确触发而无任务冲突；3) 使用精心策划的流式数据集和两阶段课程学习，逐步优化流式格式适应和主动响应能力。

Result: 在12个基准测试上的广泛实验表明，ROMA在主动任务（警报、叙述）上达到最先进性能，在反应式设置（问答）上具有竞争力，验证了其在统一实时全多模态理解中的鲁棒性。

Conclusion: ROMA通过创新的流式处理架构和训练策略，成功解决了现有全多模态模型在流式音频-视频理解中的局限性，实现了统一的反应式和主动式交互能力。
...
TL;DR: 提出T2G范式，让LLM编码器先推理并重写用户提示，再用重写后的提示作为扩散条件，提升图像生成的事实一致性、语义对齐和视觉真实感。


<details>
  <summary>Details</summary>
Motivation: 现有T2I扩散模型仅将LLM用作文本编码器，未充分利用其推理能力来推断文本提示应生成什么视觉内容，导致生成过于字面化。

Method: 1. 通过轻量级监督微调激活LLM编码器的"思考-重写"模式；2. 使用Dual-GRPO联合优化LLM编码器和扩散主干，LLM编码器基于图像奖励进行推理和知识回忆，扩散主干生成语义一致、视觉连贯的图像。
...
Conclusion: 通过HPE-Bench基准测试和基于MLLM的统一框架，成功解决了文本引导人体姿态编辑中的评估问题，为姿态不一致性提供了细粒度分析工具。
...
TL;DR: DiffCR是一个基于扩散模型的图像压缩框架，通过一致性先验精炼实现高效高保真图像重建，相比现有方法在比特率和速度上都有显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散先验的图像压缩方法存在采样过程缓慢和比特分配次优的问题，这是由于碎片化的训练范式导致的。需要开发更高效、更高质量的扩散基图像压缩框架。

Method: 提出DiffCR框架，核心包括：1) 频率感知跳跃估计(FaSE)模块，精炼预训练潜在扩散模型的ε预测先验；2) 频率解耦注意力(FDA)机制，在不同时间步对齐压缩潜在表示；3) 轻量级一致性估计器，通过保持扩散采样的语义轨迹实现快速两步解码。

Result: 在不更新骨干扩散模型的情况下，DiffCR相比最先进的扩散基压缩基线实现了：27.2%的BD-rate(LPIPS)节省和65.1%的BD-rate(PSNR)节省，以及超过10倍的加速。

Conclusion: DiffCR通过一致性先验精炼有效解决了扩散基图像压缩中的效率和质量问题，实现了显著的比特率节省和速度提升，为高效高保真图像压缩提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在端到端OCR中的成就表明文本信息可以进行低损失压缩。先前工作将Transformer输入渲染为图像进行预填充，虽然减少了token数量，但无法在token-by-token推理阶段节省计算或内存成本。因此需要研究在预填充和推理阶段都能节省token的全局上下文压缩方法。

Method: 提出VIST2模型，将输入文本块与其视觉编码交错排列，在预上下文中仅依赖视觉token预测下一个文本token分布。具体方法包括：将文本块渲染为草图图像，采用多阶段训练策略（从课程调度的光学语言建模预训练开始，然后是模态交错指令微调），并在0.6B到8B规模上进行实验探索。

Result: 在4倍压缩比下，VIST2模型在长文本写作任务上显著优于基线模型，平均实现：首token生成速度提升3倍，内存使用减少77%，FLOPS减少74%。

Conclusion: VIST2通过全局上下文压缩方法有效解决了现有部分压缩方法的局限性，在保持性能的同时显著提升了长文本生成的效率和资源利用率，为文本信息压缩提供了新思路。
...
TL;DR: 提出一种缺失感知的多模态生存预测框架，整合CT、WSI和临床数据用于NSCLC生存建模，通过基础模型特征提取和缺失感知编码策略，实现不完全模态下的中间融合，无需丢弃患者数据。


<details>
  <summary>Details</summary>
Motivation: NSCLC的精准生存预测需要整合异质临床、影像和组织病理信息，但多模态深度学习面临小队列规模和模态缺失的挑战，常被迫进行完整病例过滤或激进插补，限制了临床适用性。

Method: 使用基础模型进行模态特定特征提取，采用缺失感知编码策略，提出中间融合架构，该架构对缺失模态具有设计上的鲁棒性，允许模型利用所有可用数据而无需在训练或推理时丢弃患者。

Result: 实验结果显示中间融合持续优于单模态基线以及早期和晚期融合策略，WSI与临床模态融合表现最强（C-index 73.30）。模态重要性分析显示模型具有自适应行为，自动降低信息较少模态（如CT）的权重。

Conclusion: 提出的缺失感知多模态生存框架能够有效整合不完全的模态信息，为NSCLC生存预测提供临床适用的解决方案，通过自适应权重分配优化预测性能。
...
TL;DR: 提出一种计算框架，将多时间帧荧光显微镜视频融合成单张高质量图像，保留生物内容并提升细胞计数44%


<details>
  <summary>Details</summary>
Motivation: 荧光显微镜用于活体生物样本分析，但受噪声、时间变异性和信号振荡影响，限制了记录数据的实用性

Method: 开发独特计算框架，整合多时间帧信息到单张高质量图像，结合来自不同计算机视觉领域的可解释技术

Result: 在111种配置和具有挑战性的心脏细胞2D单层数据集上评估，相比先前方法平均提升44%的细胞计数

Conclusion: 该框架能生成保留和增强显微镜帧质量和信息的复合图像，适用于需要将多时间图像堆栈融合成高质量2D图像的其他成像领域
...
TL;DR: Lunar-G2R：首个从地形几何直接推断空间变化反射率的方法，用于月球表面高保真渲染和视觉导航
...
Method: 提出Lunar-G2R框架，使用U-Net结合可微分渲染，直接从月球数字高程模型预测空间变化的BRDF参数，无需多视角图像或专用硬件

Result: 在Tycho陨石坑测试中，光度误差降低38%，PSNR和SSIM更高，感知相似性改善，能捕捉均匀模型缺失的精细反射变化

Conclusion: 这是首个直接从地形几何推断空间变化反射率模型的方法，为高保真月球渲染和视觉导航提供了新途径
...
TL;DR: 该研究提出了一种基于视觉语言模型推理的城市社会语义分割方法，通过引入新的数据集SocioSeg和推理框架SocioReasoner，解决了传统分割模型在社会定义类别（如学校、公园）上的识别难题。


<details>
  <summary>Details</summary>
Motivation: 当前先进的分割模型能够可靠地分割由物理属性定义的实体（如建筑物、水体），但在社会定义类别（如学校、公园）上仍然存在困难。城市表面包含丰富的语义实体，对这些实体进行分割对下游应用至关重要。

Method: 提出了SocioReasoner视觉语言推理框架，模拟人类识别和标注社会语义实体的过程，通过跨模态识别和多阶段推理实现。采用强化学习优化这个不可微分的过程，激发视觉语言模型的推理能力。同时引入了Urban Socio-Semantic Segmentation数据集SocioSeg，包含卫星图像、数字地图和分层结构组织的像素级标签。

Result: 实验表明该方法在现有最先进模型基础上取得了显著提升，并表现出强大的零样本泛化能力。数据集和代码已在GitHub上开源。
...
TL;DR: MERGETUNE提出了一种新的持续微调范式，通过线性模式连接指导，在已微调的模型基础上继续微调，恢复预训练知识，提升基础-新颖类别的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型微调会导致预训练知识的灾难性遗忘，现有方法主要关注减轻遗忘过程，但遗忘往往不可避免。本文提出在零样本模型已经适应后恢复预训练知识的新范式。

Method: 提出MERGETUNE方法，基于线性模式连接指导，在已微调模型的基础上继续微调可训练参数，寻找同时连接零样本模型和微调模型的低损失路径。使用二阶近似替代原始LMC约束，避免大规模数据重放。

Result: MERGETUNE将CoOp的基础-新颖泛化调和平均提升5.6%，在DTD和EuroSAT数据集上首次超越CLIP性能。在鲁棒微调评估中，LMC合并模型超越集成基线，与零样本模型集成后获得进一步增益和SOTA结果。

Conclusion: MERGETUNE提供了一种简单、模型无关的持续微调策略，能够有效恢复微调过程中丢失的预训练知识，提升模型泛化性能，且无需增加参数或架构修改。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于车载相机的高精地图构建方法存在深度感知有限和遮挡导致精度下降的问题，需要一种能够克服这些限制的在线地图构建方案。

Method: SatMap方法整合卫星地图与多视角相机观测，利用卫星图像提供的鸟瞰视角全局先验（包括车道级语义和纹理信息），直接预测矢量化的高精地图。

Result: 在nuScenes数据集上，SatMap相比纯相机基线提升34.8% mAP，相比相机-LiDAR融合基线提升8.5% mAP，在长距离和恶劣天气条件下也表现出优势。

Conclusion: SatMap通过融合卫星地图作为全局先验，有效缓解了深度模糊和遮挡问题，显著提升了在线高精地图构建的性能，为下游预测和规划模块提供了更好的支持。
...
TL;DR: FUSE-Bike平台和BikeActions数据集：首个从骑行者视角捕捉密集共享空间VRU行为的开放感知平台和多模态数据集


<details>
  <summary>Details</summary>
Motivation: 当前研究主要关注车辆视角下的行人过街行为，但密集共享空间中的VRU交互行为研究不足，特别是从骑行者视角理解VRU意图对自动驾驶和移动机器人安全至关重要

Method: 开发FUSE-Bike开放感知平台（配备两个LiDAR、摄像头和GNSS），从骑行者视角采集高保真近距离数据；创建BikeActions多模态数据集（852个标注样本，5个动作类别）；评估最先进的图卷积和基于Transformer的模型

Result: 建立了首个从骑行者视角理解VRU行为的性能基准，公开发布了完整数据集、数据管理工具、开放硬件设计和基准代码
...
TL;DR: 该研究使用FFA-Net和AECR-Net两种深度学习架构来增强烟雾和雾霾环境下模拟仪表图像的可见性，以支持应急响应中的自动仪表读数。


<details>
  <summary>Details</summary>
Motivation: 在雾霾和烟雾环境中拍摄的图像可见度降低，这给基础设施监控带来挑战，并阻碍紧急服务在关键情况下的工作。准确的仪表数据解读对应急响应人员具有重要价值。

Method: 研究采用FFA-Net和AECR-Net两种深度学习架构来增强受雾霾和烟雾影响的仪表图像。由于缺乏模拟仪表图像的基准数据集，研究使用虚幻引擎生成了包含14,000多张图像的新合成数据集。模型采用80%训练、10%验证、10%测试的数据划分。

Result: 对于合成雾霾数据集，SSIM和PSNR指标分别达到约0.98和43dB，与最先进结果相当。AECR-Net相比FFA-Net获得更稳健的结果。虽然合成烟雾数据集的结果较差，但训练模型仍取得有趣成果。烟雾图像增强更困难，因为烟雾具有不均匀性和高密度特性。
...
TL;DR: 该论文提出WMReward方法，利用世界模型的物理先验作为奖励，在推理时引导多个去噪轨迹，显著提升视频生成的物理合理性，在ICCV 2025物理IQ挑战赛中取得第一名。
...
Conclusion: 这项工作证明了使用潜在世界模型来提升视频生成物理合理性的可行性，超越了特定的实例化或参数化方法，为视频生成的物理合理性改进提供了新的方向。
...
TL;DR: DeepUrban是一个新的无人机数据集，专注于密集城市交通场景，用于提升轨迹预测和规划基准测试性能。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶系统的预测和规划能力依赖于强大的基准测试，但现有基准测试缺乏密集交通场景，这对于理解复杂道路使用者交互至关重要。
...
Conclusion: DeepUrban数据集填补了密集城市交通场景的空白，能够有效提升自动驾驶系统的预测和规划能力，为高级建模和仿真任务提供支持。
...
TL;DR: 该论文提出了一种基于约当曲线定理的拓扑感知分割评估方法，通过数字拓扑和同调理论验证分割掩码的结构一致性，为传统评估指标提供了补充。


<details>
  <summary>Details</summary>
Motivation: 传统分割评估指标（像素级、区域基、边界聚焦）往往难以捕捉分割的结构和拓扑一致性。在医学影像或对象分割等实际应用中，边界的小误差、空洞或碎片化预测可能导致高评分，但掩码无法保持对象的全局形状或连通性。这凸显了传统指标的局限性：无法评估预测分割是否将图像划分为有意义的内外区域。

Method: 基于约当曲线定理提出拓扑感知分割概念，并适应数字平面使用。定义"Jordan-segmentatable mask"概念，即确保图像域拓扑分离为两个连通分量的二进制分割。通过数字拓扑和同调理论分析分割掩码，从掩码中提取4-曲线候选，使用贝蒂数验证其拓扑有效性。当候选形成数字4-曲线且β₀=β₁=1时（等价于其补集恰好分裂为两个8-连通分量），掩码被视为Jordan-segmentatable。

Result: 该框架提供了一个数学严谨的无监督准则，用于评估分割掩码的结构一致性。通过结合数字约当理论和同调不变量，该方法为标准评估指标提供了有价值的替代方案。

Conclusion: 在需要保持拓扑正确性的应用中，这种基于约当曲线定理和同调理论的拓扑感知分割评估方法能够有效补充传统评估指标的不足，为分割质量提供更全面的结构一致性评估。
...
Method: 使用全自动流程构建数据集：(1)利用V-JEPA 2嵌入进行分层时间分割；(2)生成组织为"标题树"的多层次帧和片段标题；(3)通过GPT-OSS-120B推理模型在多轮自我精炼过程中聚合证据，输出结构化标注（简要/详细动作、执行者、简要/详细标题）。

Result: Action100M包含从120万互联网教学视频（总时长14.6年）中提取的约1亿个时间定位片段，具有开放词汇动作监督和丰富标题。在Action100M上训练VL-JEPA显示出持续的数据规模改进效果，并在多种动作识别基准测试中表现出强大的零样本性能。

Conclusion: Action100M为视频理解和世界建模的可扩展研究建立了新的基础，展示了大规模开放词汇动作数据集在提升模型性能方面的重要价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：基于网格的3D方法能建模双人对话但缺乏真实纹理，基于大模型的2D方法能产生自然外观但计算成本过高，而基于3D高斯泼溅的方法虽然高效真实但仅限于单人说话且忽略社交关系。虚拟现实中多轮对话场景需要既真实又考虑社交关系的说话头生成技术。

Method: 首先从语音驱动基于网格的3D面部运动，然后将3D高斯绑定到网格面片上以渲染高保真2D头像视频。提出社交感知模块，通过可学习查询机制将社交关系（血缘/非血缘、平等/不平等）编码为高层嵌入。设计三阶段训练范式，并构建了包含语音-网格-图像三元组及社交关系标注的RSATalker数据集。

Result: 大量实验表明，RSATalker在真实性和社交感知方面都达到了最先进的性能。该方法在保持高效渲染的同时，能够生成考虑社交关系的逼真说话头视频。

Conclusion: RSATalker是首个利用3D高斯泼溅技术实现社交感知说话头生成的框架，成功解决了现有方法在真实纹理、计算成本和社交关系建模方面的局限性，为虚拟现实中的多轮对话场景提供了有效的解决方案。代码和数据集将公开发布。
...
TL;DR: Molmo2是一个开源视频语言模型家族，在视频理解、计数、描述和像素级定位任务上表现优异，通过7个新视频数据集和2个多图像数据集实现，无需依赖专有模型数据。
...
Method: 收集了7个新视频数据集和2个多图像数据集，包括详细视频描述数据集、自由形式视频问答数据集、复杂查询对象跟踪数据集和创新视频指向数据集。采用高效打包和消息树编码方案，使用双向注意力机制和新颖的令牌权重策略。

Result: 8B模型在短视频、计数和描述任务上优于同类开源模型，在长视频任务上具有竞争力。在视频定位任务上显著优于Qwen3-VL等开源模型，在某些任务上甚至超过Gemini 3 Pro等专有模型。

Conclusion: Molmo2为开源社区提供了先进的视频语言模型基础，展示了无需依赖专有模型数据也能实现卓越性能，特别是在像素级定位能力方面取得了突破性进展。
...
Method: 提出CoMoVi协同生成框架，包含：1）有效的2D人体运动表示以继承预训练视频扩散模型的强大先验；2）双分支扩散模型，通过相互特征交互和3D-2D交叉注意力耦合人体运动和视频生成；3）构建CoMoVi数据集，包含大规模真实世界人体视频及文本和运动标注。
...
TL;DR: CURVE是一个针对多元文化和多语言视频推理的挑战性基准测试，包含18个全球地区的文化视频和人工标注，要求模型具备深层次的文化情境理解能力。


<details>
  <summary>Details</summary>
Motivation: 当前视频模型基准测试主要基于西方中心数据和英语，存在显著偏见。需要建立一个多元文化和多语言的视频推理基准来评估模型在真实世界文化多样性中的表现。

Method: 创建CURVE基准：1）收集18个全球地区的文化视频；2）提供完全人工生成的高质量标注，包括复杂问题、答案和多步推理步骤，均使用本地语言；3）利用推理轨迹构建基于证据的图；4）提出使用这些图识别推理中细粒度错误的迭代策略。

Result: 评估显示，最先进的视频大语言模型在CURVE上表现显著低于人类水平，主要错误源于对文化元素的视觉感知不足。

Conclusion: CURVE基准揭示了当前视频模型在文化理解和推理方面的局限性，为未来研究提供了重要的评估工具，强调需要开发能够理解视觉文化背景的模型。
...
TL;DR: 该研究创建了一个包含超过13,200个地面甲虫标本的多模态数据集，通过高分辨率成像和数字测量，为AI驱动的自动特征提取和生态研究提供基础。


<details>
  <summary>Details</summary>
Motivation: 全球特征数据库严重偏向脊椎动物和植物，限制了地面甲虫等高多样性群体的全面生态分析。虽然NEON拥有大量地面甲虫标本，但主要是物理收藏，限制了广泛的研究访问和大规模分析。

Method: 对来自美国大陆和夏威夷30个站点的13,200多个NEON地面甲虫标本进行高分辨率成像数字化，包括数字测量每个标本的鞘翅长度和宽度，并与手动测量进行验证。
...
Method: 提出随机补丁选择(SPS)：对每一帧随机掩码部分补丁描述符，不输入策略模型，同时保持剩余补丁的空间布局，让策略学习基于对特定令牌不敏感的特征。

Result: 在所有OOD场景中超越SOTA，平均提升6.2%，闭环仿真中最高提升20.4%，速度提升2.4倍；训练的9个系统中有8个超越先前SOTA，且无需调优即可迁移到真实物理车辆。

Conclusion: SPS通过提供场景的不同随机但完整的视图，使策略学习基于对特定补丁不敏感的特征，显著提升自动驾驶策略的鲁棒性、泛化性和效率。
...
<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型存在严重的视觉特征瓶颈，仅将视觉编码器输出连接到LLM输入，这种静态架构限制了LLM与分层视觉知识的全面对齐，无法准确整合局部细节与全局语义进行连贯推理
...
Method: 1. 使用松弛训练目标：模型在身份参考图像、目标内在属性文本描述、背景图像和物体掩码的条件下，可以同时改变内在和外在属性；推理时重用原始背景和掩码以限制外在变化。2. 视觉命名实体（VNEs）：细粒度视觉身份类别，通过视觉语言模型从大规模公共图像数据集中自动提取VNE标签和内在属性描述，实现可扩展的身份保持监督。

Result: Alterbute在保持身份的同时编辑物体内在属性方面优于现有方法。

Conclusion: Alterbute通过松弛训练目标和视觉命名实体的结合，实现了既能保持物体身份又能有效编辑内在属性的图像编辑方法，解决了现有方法的局限性。
...
TL;DR: WildRayZer是一个自监督框架，用于解决动态环境中的新视角合成问题，通过分析-合成测试分离静态和瞬态内容，在动态场景中实现高质量的新视角生成。


<details>
  <summary>Details</summary>
Motivation: 动态环境中的新视角合成面临挑战，因为相机和物体都在移动，破坏了静态模型依赖的多视角一致性，导致重影、幻觉几何和不稳定的姿态估计。
...
Conclusion: WildRayZer通过自监督方式有效解决了动态环境中的新视角合成问题，同时构建了大规模动态数据集D-RE10K，为动态场景理解提供了重要基准。
