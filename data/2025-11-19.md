...
TL;DR: 该论文提出了一个解决开放集识别中过度自信问题的框架，通过参数扰动估计预测不确定性，并使用两阶段分类器来更好地区分已知和未知类别。


<details>
  <summary>Details</summary>
Motivation: 开放集识别中，当未知样本与已知类别语义相似时，模型会对其赋予过高的置信度，导致误分类为已知类别，这种现象称为过度自信，削弱了开放集识别的性能。
...
TL;DR: 提出了一种用于少样本视频目标检测的新方法，通过选择性传播高置信度目标特征来保持时间一致性，无需复杂的目标管建议，在多个数据集上取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 解决传统检测方法需要大量训练数据的问题，应对视频中目标遮挡和外观变化带来的时间一致性挑战，避免计算昂贵的区域建议机制。

Method: 采用目标感知的时间建模方法，包含过滤机制选择性传播高置信度目标特征，结合少样本训练的检测和分类头，实现高效特征传播。

Result: 在5-shot设置下，AP提升分别为：FSVOD-500 3.7%、FSYTV-40 5.3%、VidOR 4.3%、VidVRD 4.5%。在1-shot、3-shot和10-shot配置下也表现出改进。

Conclusion: 该方法通过目标感知的时间建模有效解决了少样本视频目标检测中的时间一致性和泛化问题，实现了显著的性能提升，且不依赖复杂的目标管建议。
...
Method: 采用流匹配范式实现直接概率传输；收集多模型融合结果作为先验，使用任务感知选择函数筛选可靠伪标签；引入融合精炼模块采用分治策略识别、分解和增强退化组件；集成弹性权重巩固和经验回放机制支持多任务学习。

Result: 在多种融合任务中取得竞争性性能，显著提高采样效率，保持轻量级模型设计。

Conclusion: 该方法为多模态图像融合提供了高效统一的解决方案，在保持性能的同时显著提升采样效率，并支持多任务学习场景。
...
TL;DR: 本文提出了一种新颖的两阶段无轨迹碰撞检测框架，通过扩散模型生成合理的未来路段地图并与监控地图对比来检测碰撞。


<details>
  <summary>Details</summary>
Motivation: 为了解决轨迹获取和车辆跟踪的限制，直接使用记录个体级交通动态数据的路段地图进行碰撞检测，开发实时碰撞检测系统以提升主动安全管理和交通效率。

Method: 提出两阶段框架：第一阶段使用基于扩散的Mapfusion模型，通过噪声到正常的过程生成路段地图，结合时序嵌入组件捕获动态特征，并通过ControlNet融入背景上下文；第二阶段通过比较监控路段地图与扩散模型生成结果来检测碰撞。

Result: Mapfusion能够基于学习的运动模式成功生成真实的路段演化地图，在不同采样间隔下保持鲁棒性。在真实世界碰撞数据上的实验表明该两阶段方法能有效准确检测碰撞。

Conclusion: 该研究提出的两阶段无轨迹碰撞检测框架通过扩散模型生成路段地图并与实际监控数据对比，能够有效实现实时碰撞检测，为主动安全管理提供了可行方案。
...
TL;DR: 该论文提出了一种基于面部微动态分析的被动痴呆筛查方法，通过分析眨眼动态、嘴部运动、注视变异性和头部微调等面部运动特征，实现无需语言或文本的早期神经认知变化检测。
...
Method: 通过稳定面部信号，将面部微运动转换为可解释的时间序列，平滑处理后提取短窗口的统计特征。分析运动在不同通道的分布而非幅度，使每个通道的影响透明化。

Result: 在YT DemTalk数据集上，消融实验显示注视变异性和嘴部/下颌动态是最具信息量的线索，轻量级分类器取得了AUROC 0.953、AP 0.961、F1分数 0.851和准确率0.857的优异性能。

Conclusion: 面部时间微动态分析为痴呆筛查提供了一种无需语言、可扩展的方法，能够捕捉自然面部行为，且可跨设备、主题和文化转移，无需临床医生或研究人员在录制过程中的主动干预。
...
TL;DR: 本文提出了Gen-ViRe基准测试框架，用于评估视频生成模型在链式帧推理(CoF)中的认知能力，填补了现有基准只关注保真度而对多步规划、算法逻辑等核心推理能力评估不足的空白。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成基准主要评估视觉质量和文本对齐，无法衡量模型在连续物理世界模拟中的链式帧推理能力，这阻碍了对模型真实认知能力的系统理解和改进指导。

Method: 基于认知科学和实际AI应用，将CoF推理分解为6个认知维度和24个子任务，通过多源数据整理、最小提示协议和混合VLM辅助评估来量化视频模型的推理能力。

Result: 实验发现SOTA系统在视觉质量和实际推理深度之间存在显著差距，为推进真正世界模拟器建立了基准和诊断工具。

Conclusion: Gen-ViRe提供了首个对视频模型作为推理器的定量评估，揭示了当前模型的局限性，为开发更强大的世界模拟器提供了系统指导。
...
TL;DR: 该论文提出基于排序的损失函数来解决基于热图的人体姿态估计中的三个主要问题：MSE损失无法有效提升关节定位精度、热图的空间和类别不平衡、以及损失函数与评估指标之间的不一致性。


<details>
  <summary>Details</summary>
Motivation: 解决传统热图方法中MSE损失无法专注于峰值定位、热图不平衡以及损失函数与mAP评估指标不一致的问题。

Method: 提出基于排序的损失函数，通过理论分析和实验验证，证明该损失函数能提高置信度分数与定位质量之间的相关性。

Result: 在COCO、CrowdPose和MPII三个数据集上验证了方法的有效性，RSPose在COCO-val集上达到79.9 mAP，优于之前的最优方法，同时将SimCC Resnet-50的性能提升了1.5 AP。

Conclusion: 提出的基于排序的损失函数是首个与mAP评估指标对齐的人体姿态估计损失函数，能显著提升模型性能。
...
TL;DR: 本文提出了碰撞声源分割（CS3）新任务，旨在根据音频条件从碰撞视频帧中分割出产生碰撞声音的物体。针对该任务提出了弱监督方法，利用基础模型和手部物体线索，在两个新基准上取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 受人类多感官感知能力启发，希望让机器能够通过碰撞声音识别产生声音的物体，特别是在自我中心视角下，声音清晰但视觉场景杂乱、物体小、交互短暂的情况。

Method: 提出弱监督的音频条件分割方法，利用CLIP和SAM2基础模型，并整合自我中心线索（如手部物体）来识别可能的碰撞声源物体。

Result: 在提出的两个CS3基准（EPIC-CS3和Ego4D-CS3）上，方法比竞争基线在mIoU指标上分别提升了3倍和4.7倍。

Conclusion: CS3任务具有挑战性但可行，提出的弱监督方法能有效识别碰撞声源物体，为多模态感知研究提供了新方向。
...
TL;DR: 本文提出了一种几何基础的绝对姿态回归方法，通过将问题重新表述为几何表示回归，从图像直接回归3D几何表示，然后通过可微分求解器恢复6-DoF相机姿态。


<details>
  <summary>Details</summary>
Motivation: 传统的绝对姿态回归模型作为黑盒直接回归6-DoF姿态，容易记忆训练视图而非理解3D场景几何。本文旨在通过引入几何先验来改进这一范式。

Method: 提出几何表示回归范式，显式预测两个解耦的几何表示：光线束方向估计相机旋转，对应点图估计相机平移。使用可微分确定性求解器从这些几何组件恢复最终姿态。

Result: 在7-Scenes和Cambridge Landmarks数据集上实现了最先进的性能，验证了建模逆渲染过程对通用绝对姿态估计的鲁棒性。

Conclusion: 通过将学习到的视觉到几何映射与最终姿态计算分离，引入强几何先验，显式解耦旋转和平移预测显著提升了性能，为通用绝对姿态估计提供了更稳健的路径。
...
TL;DR: 本文提出了H-CNN-ViT模型用于膀胱癌复发预测，该模型通过分层门控注意力机制融合CNN和ViT的局部与全局特征，在多序列MRI数据集上达到78.6%的AUC。


<details>
  <summary>Details</summary>
Motivation: 膀胱癌复发率高达78%，术后监测至关重要。多序列增强MRI是常用检测方法，但术后组织变化使判读困难。AI辅助诊断工具虽有潜力，但缺乏专门的复发评估数据集阻碍了进展。

Method: 提出H-CNN-ViT分层门控注意力多分支模型，基于上下文需求选择性加权全局(ViT)和局部(CNN)路径特征，实现平衡的目标特征融合。多分支架构独立处理每个模态，确保最佳捕获和整合各成像通道特性。

Result: 在专门构建的多序列多模态MRI数据集上评估，H-CNN-ViT达到78.6%的AUC，超越现有最先进模型。

Conclusion: 本文贡献包括：1）构建了专门用于膀胱癌复发预测的多序列MRI数据集；2）提出的H-CNN-ViT模型通过分层特征融合策略显著提升了复发预测性能。
...
TL;DR: 提出了QwenCLIP框架，用大语言模型替换CLIP的文本编码器，通过可学习提示增强跨模态对齐，解决了长放射学报告表示受限的问题。


<details>
  <summary>Details</summary>
Motivation: CLIP的文本编码器仅支持77个token，无法有效表示信息丰富的长放射学报告；现有领域特定编码器仍受限于512token输入长度和浅层语义理解。

Method: 用基于大语言模型的嵌入模块（如Qwen3-Embedding）替换CLIP文本编码器，引入可学习提示增强跨模态对齐，利用LLM的扩展上下文窗口和丰富表示能力。

Result: QwenCLIP能够从长格式临床文本中捕获全面的医学语义，显著改善了医学图像-文本对齐和在放射学基准测试中的下游性能。

Conclusion: QwenCLIP通过LLM增强的文本编码有效解决了长放射学报告表示问题，提升了医学视觉语言任务的性能。
...
TL;DR: 本文提出了一种结合EfficientNet和VGG19的混合模型架构，用于腰椎退行性变的DICOM图像分类，通过伪牛顿增强层和稀疏诱导特征减少层提升特征选择和表示能力。


<details>
  <summary>Details</summary>
Motivation: 传统迁移学习方法在医学图像高维背景下存在局限性，特别是在详细解剖特征提取方面不足，需要开发更有效的自动诊断工具。

Method: 采用EfficientNet和VGG19混合架构，集成伪牛顿增强层进行特征权重优化，稀疏诱导层去除冗余特征，形成多层框架。

Result: 模型性能显著提升，达到精度0.9、召回率0.861、F1分数0.88、损失0.18、准确率88.1%，优于基准模型EfficientNet。

Conclusion: 该架构克服了传统迁移学习的限制，为医学图像自动诊断工具的发展提供了有效解决方案。
...
TL;DR: 本文研究了医学解剖分割中的数据规模缩放规律，分析了15个语义任务和4种成像模态，发现更大数据集能显著提升分割性能。提出了基于图像配准的变形增强方法，提高了数据利用效率，超越了标准幂律缩放趋势。


<details>
  <summary>Details</summary>
Motivation: 医学解剖分割领域的数据缩放规律研究不足，且医学图像具有解剖结构拓扑同构的特点，这为研究变形增强策略对数据缩放规律的影响提供了动机。

Method: 分析了15个语义任务和4种成像模态的分割缩放规律；评估了随机弹性变形和配准引导变形两种增强策略；提出了基于图像配准的测地子空间生成可微映射的新颖图像增强方法。

Result: 实验结果表明，配准和生成变形增强方法显著提高了数据利用效率；提出的生成变形方法在性能和收敛速度上表现更优，超越了标准幂律缩放趋势且无需额外数据。

Conclusion: 这项工作为理解医学图像分割的可扩展性和拓扑变异影响提供了见解，有助于以更低的标注和计算成本开发更高效的模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有数字血液病理学方法（包括单任务、视觉语言、WSI优化和单细胞血液学模型）无法提供统一的、多任务、多模态的推理能力来应对数字血液病理学的复杂性。

Method: 基于Hema-Former多模态模块构建Uni-Hema模型，该模块在层次级别桥接视觉和文本表示，支持检测、分类、分割、形态学、掩码语言建模和视觉问答等不同粒度的任务。

Result: Uni-Hema在多种血液学任务上实现了与单任务单数据集模型相当或更优的性能，同时在单细胞级别提供可解释的、形态相关的见解。
...
<details>
  <summary>Details</summary>
Motivation: 解决短暂沟壑自动检测的挑战：短暂的时间周期使传统计算机视觉方法难以检测，且缺乏准确的标注数据限制了机器学习方法的应用。

Method: 使用视觉语言模型嵌入知识，采用教师-学生模型，教师从VLM的噪声标签中学习，学生通过教师生成的标签和噪声感知损失函数进行弱监督学习。
...
TL;DR: SAE-MCVT是首个可扩展的实时多摄像头车辆跟踪框架，通过边缘设备处理视频流并传输轻量级元数据到中央工作站进行跨摄像头关联，在2K 15FPS视频流上保持实时运行。


<details>
  <summary>Details</summary>
Motivation: 现有MCVT研究过度关注准确性而忽略实时性能和可扩展性，这在城市规模应用中随着摄像头数量增加变得愈发重要。
...
Method: 从学校教科书中收集交通标志和道路安全规范的示意图数据集，在零样本设置下评估模型能力。

Result: 初步结果显示这些模型在安全推理方面表现不佳，存在性能差距。

Conclusion: 多模态LLMs在道路安全概念理解方面存在明显不足，需要进一步研究来弥合人类学习与模型解释之间的差距。
...
TL;DR: 本文提出了一种基于课程学习的相对策略优化方法（CuRPO），通过利用CoT长度和gIoU奖励作为复杂度指标，从简单到复杂逐步组织训练数据，以解决强化学习微调CoT推理在视觉定位任务中性能下降的问题。


<details>
  <summary>Details</summary>
Motivation: 研究发现基于强化学习的CoT推理微调在视觉定位任务中可能降低性能，特别是当CoT输出变得冗长或复杂时。同时，增加数据集规模并不总是能提升性能，因为数据复杂度存在差异。

Method: 提出Curriculum-based Relative Policy Optimization (CuRPO)训练策略，使用CoT长度和广义交并比(gIoU)奖励作为复杂度指标，从简单到困难逐步组织训练数据。

Result: 在RefCOCO、RefCOCO+、RefCOCOg和LISA数据集上的实验表明，CuRPO始终优于现有方法（包括Visual-RFT），在RefCOCO上实现了高达+12.52 mAP的显著改进。该方法在少样本学习场景下也表现出优异的效率和鲁棒性。

Conclusion: CuRPO是一种有效的训练策略，能够显著提升视觉定位任务的性能，特别是在处理具有模糊和复杂文本描述的任务时表现突出，且在小样本场景下仍能保持强大的定位性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决视频帧数据集中由于连续帧相似性导致的信息泄露问题，提高数据集划分的可靠性

Method: 基于聚类的帧选择策略，先对视觉相似的帧进行分组，然后再划分训练集、验证集和测试集

Result: 该方法能够产生更具代表性、更平衡和更可靠的数据集划分

Conclusion: 基于聚类的帧选择策略有效缓解了视频帧数据集中的信息泄露问题，提高了数据集划分的质量
...
<details>
  <summary>Details</summary>
Motivation: 探索在视觉Transformer中注入通用归纳偏置的新方法，通过使用程序生成的无视觉语义内容数据进行预训练，以增强模型的抽象计算能力。

Method: 使用形式语法等简单算法生成无视觉语义内容的数据，在预训练阶段绕过视觉补丁嵌入机制，让模型内化抽象计算先验，随后进行标准的基于图像的训练。

Result: 在ImageNet-1k上，仅用1%的训练预算用于程序生成数据，最终准确率提高超过1.7%，效果相当于使用28%的ImageNet-1k数据。
...
TL;DR: 提出CrossTrainer方法，通过发现跨运动项目的技能属性（如平衡、控制、手部位置），训练多模态语言模型为视频生成可操作的反馈和熟练度评级，在跨运动和同运动场景下相比现有技术提升达60%。
...
Result: 在多个数据集上验证，在跨运动和同运动设置下相比现有技术提升达60%，视频表示泛化能力显著优于现有技术。
...
TL;DR: RISE是首个基于单静态雷达的室内场景理解基准系统，通过利用多径反射的几何线索，联合实现布局重建和物体检测，在保护隐私的同时提升几何推理能力。
...
Method: 提出双角度多径增强方法，显式建模到达角和离开角来恢复次生反射；采用仿真到现实的层次扩散框架，将碎片化的雷达响应转化为完整的布局重建和物体检测。

Result: 在布局重建方面将Chamfer距离降低60%至16厘米，实现了首个基于毫米波的物体检测，达到58%的IoU；构建了包含5万帧数据的大规模基准数据集。
...
TL;DR: 开发了一个2.5D上下文感知模型，用于自动识别MRI图像的解剖平面方向（轴向、冠状面和矢状面），准确率达到99.49%，比2D方法减少60%错误。该模型在脑肿瘤检测任务中通过元数据增强将准确率从97.0%提升至98.0%。


<details>
  <summary>Details</summary>
Motivation: 人类能轻松识别MRI图像的解剖平面方向，但自动化系统难以完成此任务。缺失平面方向元数据会复杂化分析、增加异质数据集合并时的域偏移，并降低诊断分类器的准确性。

Method: 采用2.5D上下文感知模型，利用多切片信息避免单一切片的模糊性，实现稳健的特征学习。在3D切片序列和静态2D图像上训练模型，并与2D参考模型进行比较。

Result: 2D参考模型准确率为98.74%，2.5D方法提升至99.49%，错误减少60%。在脑肿瘤检测任务中，通过基于不确定性评分的门控策略使用元数据增强预测，准确率从97.0%提升至98.0%，误诊减少33.3%。

Conclusion: 2.5D上下文对MRI平面方向识别至关重要。开发的模型显著提高了准确性，在脑肿瘤检测中验证了其效用，并已集成到开源交互式Web应用中。
...
TL;DR: LINGUAL框架通过自然语言指令实现医学图像分割的主动学习，无需人工干预即可执行子任务序列，在主动域适应中性能优于传统主动学习方法，同时减少约80%的标注时间。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割中边界模糊和模糊不清的问题使得传统主动学习仍然具有挑战性、劳动密集型和认知要求高。语言指导提供了一种有效的替代方案，需要最少的专家努力，同时绕过了分割中精确边界描绘的认知要求高的任务。

Method: 引入LINGUAL框架：接收专家的自然语言指令，通过上下文学习将其转换为可执行程序，并自动执行相应的子任务序列，无需任何人工干预。

Result: 在主动域适应（ADA）中，LINGUAL实现了与主动学习基线相当或更优的性能，同时估计标注时间减少了约80%。

Conclusion: LINGUAL框架通过自然语言指令有效解决了医学图像分割中的标注挑战，显著提高了效率并降低了认知负担。
...
Method: 基于自监督模型对RandomResizedCrop增强的鲁棒性，定义基础评分函数量化图像嵌入对Haar小波分解提取的高频方向扰动的敏感性。将图像缩放到模型输入大小的倍数，分割成小块，计算每个块的基础评分，最终通过平均所有块的评分得到检测分数。

Result: 在真实数据集和23种不同生成模型生成的图像上验证，WaRPAD始终实现竞争性性能，对测试时损坏表现出强鲁棒性，且适用于各种自监督模型。

Conclusion: WaRPAD是一种有效的无需训练AI生成图像检测方法，利用自监督模型的通用特性，在不同分辨率和领域的图像上表现稳定，具有广泛适用性。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有方法在服装变形过程中导致的纹理失真问题，以及缺乏对生成模型细粒度属性控制的能力，满足电子商务中真实可控的时尚图像生成需求。

Method: 提出免变形框架直接外推分割的服装，采用区域自适应解耦注意力机制和链式掩码注入策略，自适应预测每个细粒度文本属性的生成区域。

Result: 实验验证了该框架相比现有最先进方法的优越性能，能够高质量地保留服装细节并实现细粒度控制。
...
Conclusion: SCAR克服了基于向量量化的预填充方法的语义限制和高成本，同时保持了其灵活性和通用性，可推广到next-token和next-set两种AR范式。
...
TL;DR: CORE提出了一种新的视觉token压缩方法，通过对象分割生成语义先验，将视觉token合并为紧凑的对象中心表示，并采用质心引导排序机制恢复空间顺序，在保持高性能的同时大幅提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有视觉token压缩方法缺乏高层语义理解，导致次优合并、信息冗余或上下文丢失，无法有效处理大视觉语言模型中视觉token随图像分辨率二次增长带来的计算和内存成本问题。

Method: 使用高效分割解码器生成对象掩码作为语义先验，指导视觉token合并为对象中心表示，并引入质心引导排序机制恢复空间顺序。

Result: 在六个权威基准测试中达到固定率压缩的最新水平，在自适应率设置下实现显著效率提升，即使仅保留2.2%视觉token仍能维持97.4%的基线性能。

Conclusion: 对象中心表示在大视觉语言模型的高效处理中具有优越性，CORE方法在保持性能的同时大幅降低了计算成本。
...
TL;DR: 提出ZS-TMS新范式，通过预训练生成引擎直接合成任务特定分类器的全部参数，仅需单张示例图像和临床文本描述，无需任务特定训练即可部署使用。
...
Method: 使用语义引导参数合成器(SGPS)，输入最少的多模态任务信息(如图像和文本描述)，通过预训练生成引擎直接合成轻量级分类器的全部权重参数。

Result: 在ISIC 2018皮肤病变数据集和自定义罕见疾病数据集上的评估表明，在1-shot和5-shot分类任务中显著优于现有few-shot和zero-shot学习方法，达到新的最先进水平。

Conclusion: 为AI驱动的诊断工具快速开发和部署开辟了新途径，特别适用于数据严重受限的罕见疾病场景。
...
TL;DR: 本文提出DEER-3D框架，通过错误驱动的3D场景编辑生成针对性反事实数据，解决3D-LLMs在语言到视觉空间元素精确接地方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有3D-LLMs在语言到3D视觉空间元素的精确接地方面存在限制，主要源于训练数据偏向语言推理而非空间理解，以及稀缺的3D资源导致固有接地偏差未解决。

Method: 提出DEER-3D框架，采用"分解、诊断评估、编辑、重新训练"工作流程。当检测到3D-LLM接地失败时，首先诊断谓词级错误（如属性或空间关系），然后执行最小化的谓词对齐3D场景编辑（如重新着色或重新定位），生成针对性反事实监督进行迭代模型微调。

Result: 在多个3D接地和场景理解任务的基准测试中，通过迭代优化，所有评估数据集都显示出持续的改进。

Conclusion: DEER-3D证明了针对性、错误驱动的场景编辑在弥合3D LLMs中语言推理能力与空间接地之间的差距方面具有有效性。
...
TL;DR: 本文提出了一种基于二元交叉熵（BCE）的三方协同学习框架BCE3S，用于解决长尾识别任务中特征紧凑性和类间可分性的平衡问题，通过联合学习、对比学习和均匀学习三个组件实现更好的性能。
...
Method: BCE3S框架包含三个组件：1）基于BCE的联合学习，通过多个Sigmoid解耦特征与不平衡分类器向量之间的度量；2）基于BCE的对比学习，进一步提高特征的类内紧凑性；3）基于BCE的均匀学习，平衡分类器向量之间的可分性。

Result: 在CIFAR10-LT、CIFAR100-LT、ImageNet-LT和iNaturalist2018等多个长尾数据集上的广泛实验表明，BCE3S训练的长尾识别模型不仅实现了更高的特征紧凑性和可分性，还平衡了分类器的可分性，达到了最先进的性能。

Conclusion: BCE3S框架通过三方协同学习有效解决了长尾识别中的特征学习和分类器平衡问题，在多个基准数据集上实现了优越的性能。
...
TL;DR: FAPE-IR是一个用于全合一图像恢复的频率感知规划和执行框架，使用冻结的多模态大语言模型作为规划器生成频率感知恢复计划，并通过基于扩散的执行器动态选择高频或低频专家进行图像恢复。


<details>
  <summary>Details</summary>
Motivation: 现有的全合一图像恢复方法通常依赖任务特定设计或潜在路由策略，难以适应现实世界中各种复杂退化情况，需要更统一和可解释的解决方案。

Method: 使用冻结的MLLM作为规划器分析退化图像并生成频率感知恢复计划；通过LoRA-MoE模块在基于扩散的执行器中动态选择高频或低频专家；引入对抗训练和频率正则化损失来提升恢复质量和减少伪影。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频编辑方法需要用户提供精确的空间位置和时间边界描述，这在用户通过语义属性或对象关系的隐式查询进行概念化编辑时不实用。

Method: RIVER通过数字孪生表示解耦推理和生成，使用大语言模型进行多跳推理生成结构化指令，然后指导基于扩散的编辑器执行像素级修改，使用强化学习训练。

Result: RIVER在提出的RVEBenchmark上表现最佳，在VegGIE和FiVE两个视频编辑基准测试中也达到了最先进的性能，超越了六个基线方法。
...
Result: 在KITTI数据集上以极低参数量（3M）在高分辨率和低分辨率下均达到最先进性能，相比轻量级方法在多个指标上有显著提升，在Nvidia Jetson Orin上实现49 FPS实时推理。

Conclusion: RTS-Mono成功解决了自监督单目深度估计模型的轻量化与性能平衡问题，为实际应用提供了可行的解决方案。
...
TL;DR: 提出了一种名为A²GC-VPR的不对称聚合视觉位置识别方法，通过行-列归一化平均和单独边缘校准实现不对称匹配，并结合几何约束提升空间感知能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于最优传输的聚合方法在图像特征和聚类中心分布差异较大时，由于Sinkhorn算法对称处理源和目标边缘分布而效果受限。

Method: 采用行-列归一化平均与单独边缘校准实现不对称匹配，通过可学习坐标嵌入引入几何约束，计算融合特征相似性的兼容性分数。

Result: 在MSLS、NordLand和Pittsburgh数据集上的实验结果表明该方法具有优越性能，验证了其在提升匹配准确性和鲁棒性方面的有效性。

Conclusion: 提出的A²GC-VPR方法通过不对称聚合和几何约束，有效解决了视觉位置识别中特征分布差异问题，显著提升了匹配性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决文本到图像扩散模型在微调时学习不期望概念的问题，这对于偏见缓解、防止恶意适应、属性解缠和扩散策略的通用微调等下游任务至关重要。

Method: 通过保持用户提示嵌入与不期望概念不对齐来实现正则化，使用语言描述来指定不期望概念以调节适应过程。

Result: 实验结果表明，Coffee能够有效防止文本到图像模型在微调过程中学习指定的不期望概念，并优于现有方法。

Conclusion: Coffee提供了一种无需额外训练、通过语言描述灵活控制不期望概念学习的方法，在可控微调方面表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有视频系统只能检测事故发生但无法分析行人行为认知阶段的问题，利用视觉语言模型提升对事故演变过程的理解。

Method: 采用四阶段统一框架：多视角视频采集、行人行为阶段分割、阶段感知多视角推理、分层合成与诊断推理，使用两个专用VLM模型进行阶段分割和多视角分析。
...
TL;DR: Wave-Former是一种利用毫米波信号进行3D形状重建的新方法，能够穿透遮挡物重建被完全遮挡的日常物体的高精度3D形状。


<details>
  <summary>Details</summary>
Motivation: 毫米波信号能够穿透常见遮挡物并从隐藏物体反射，这为在机器人、增强现实和物流等领域开发新应用提供了可能。传统毫米波重建方法存在覆盖范围有限和噪声高的问题。

Method: 采用三阶段流水线：提出候选几何表面、使用基于transformer的形状补全模型（专门为毫米波信号设计）、进行熵引导的表面选择。该方法完全使用合成点云进行训练。

Result: 在与最先进基线的对比中，Wave-Former将召回率从54%提升到72%，同时保持85%的高精度。

Conclusion: Wave-Former展示了从合成数据训练到真实世界数据的出色泛化能力，为遮挡物体3D重建提供了有效解决方案。
...
TL;DR: 本文提出了RiSe框架来解决多模态人脸反欺诈(FAS)在跨域部署时的性能下降问题，通过非对称不变风险最小化(AsyIRM)和多模态协同解耦(MMSD)分别应对模态表示不变风险和模态协同不变风险。


<details>
  <summary>Details</summary>
Motivation: 多模态FAS方法在未见域中比单模态方法遭受更严重的性能下降，主要由于两个被忽视的风险：模态表示不变风险（表示在域偏移下是否保持泛化性）和模态协同不变风险（模型过拟合到域特定的模态间相关性）。

Method: RiSe框架包含两个核心组件：AsyIRM在径向空间中学习不变球形决策边界以适应非对称分布，同时保留角度空间中的域线索；MMSD通过跨样本混合和解耦的自监督任务增强内在、可泛化的模态特征。

Result: 理论分析和实验验证表明，RiSe在跨域性能上达到了最先进水平。

Conclusion: RiSe框架有效解决了多模态FAS的跨域泛化问题，通过处理表示和协同不变风险，显著提升了在未见域中的性能表现。
...
TL;DR: MVI-Bench是首个专门评估误导性视觉输入对大型视觉语言模型稳健性影响的基准测试，包含1,248个专家标注的VQA实例，涵盖视觉概念、属性和关系三个层次，并提出了细粒度评估指标MVI-Sensitivity。
...
Method: 基于基本视觉原语设计MVI-Bench，构建三个层次误导性视觉输入分类，收集1,248个专家标注的VQA实例，并开发MVI-Sensitivity细粒度评估指标。

Result: 对18个最先进LVLM的实证结果显示它们对误导性视觉输入存在显著脆弱性。

Conclusion: MVI-Bench为开发更可靠和稳健的LVLM提供了可操作的见解，填补了误导性视觉输入评估的重要空白。
...
TL;DR: 该论文提出了一种面向MLLMs的自适应令牌压缩方法，通过对象级令牌合并策略，仅使用10%的令牌即可达到原始模型96%的性能，解决了补丁级令牌化带来的计算和内存负担问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型使用补丁级令牌化会导致图像令牌数量呈二次增长，带来巨大的计算和内存负担，且与人类视觉认知系统不一致，导致幻觉和计算冗余。

Method: 提出对象级令牌合并策略进行自适应令牌压缩，该方法与人类视觉系统保持一致，通过智能合并令牌来减少计算量。

Result: 在多个综合基准测试中，该方法平均仅使用10%的令牌即可达到原始模型96%的性能，在压缩比和性能平衡方面优于相关工作。

Conclusion: 提出的自适应令牌压缩方法有效解决了MLLMs的计算效率问题，在保持性能的同时显著减少了计算资源需求。
...
TL;DR: DoGCLR是一个基于博弈论的自监督对比学习框架，用于骨架动作识别。它通过动态优势博弈构建正负样本，使用时空双权重定位机制识别关键运动区域，并采用熵驱动策略管理负样本库，在NTU RGB+D和PKU-MMD数据集上取得了最先进的性能。
...
Method: 提出DoGCLR框架：1）将正负样本构建建模为动态优势博弈；2）时空双权重定位机制识别关键运动区域并指导区域增强；3）熵驱动优势策略管理记忆库，保留高熵负样本并替换低熵样本。
...
TL;DR: UniSER是一个通用的软效应修复模型，能够处理多种图像退化问题（如镜头光晕、雾霾、阴影、反射等），通过大规模数据集和定制训练流程，在单一框架中实现优于专业模型和通用模型的修复效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么是专门针对单一退化问题的专业模型，缺乏可扩展性；要么是通用模型需要详细提示且难以在细粒度任务中实现稳健的移除或保持场景身份。软效应的共同本质是半透明遮挡，这为构建通用修复模型提供了基础。

Method: 构建了380万对的大规模数据集，包含新颖的物理合理数据以填补公共基准的空白；采用定制训练流程，微调扩散变换器从多样化数据中学习稳健的修复先验，并集成细粒度掩码和强度控制。

Result: UniSER在软效应修复任务中显著优于专业模型和通用模型，能够在真实场景中实现稳健、高保真的修复效果。

Conclusion: 通过利用软效应的共同本质和构建大规模多样化数据集，UniSER证明了在单一框架中处理多种图像退化问题的可行性，为通用图像修复提供了新的方向。
...
Method: 提出全局视角标记化器(GloTok)，包括码本直方图关系学习方法将预训练模型在整个数据集上建模的语义转移到语义码本，以及残差学习模块恢复细粒度细节以减少量化引起的重建误差。
...
TL;DR: 首个完全由自动驾驶模式在真实世界采集的端到端基准数据集，包含100+小时自然驾驶数据、32,727个关键帧，用于评估自动驾驶车辆的行为安全性。


<details>
  <summary>Details</summary>
Motivation: 现有数据集大多由人工驾驶或未明确驾驶模式采集，只能用于感知和预测的早期训练，无法评估自动驾驶黑盒控制下的真实行为安全性。

Method: 从市场多个量产自动驾驶车型收集自然驾驶数据，分割成关键帧，包含同步摄像头图像、高精度GNSS/IMU数据、车辆轨迹、2D标注和丰富场景属性。

Result: 数据集包含32,727个关键帧，定位精度0.8厘米，提供过去6秒和未来5秒的20Hz车辆轨迹，使用端到端运动规划模型在自动驾驶帧上达到1.4米的平均位移误差。

Conclusion: 该数据集为自动驾驶行为分析和安全评估提供了可持续的研究基础，每周新增10+小时数据，能够有效评估自动驾驶系统的真实安全性。
...
<details>
  <summary>Details</summary>
Motivation: 主动脉病理需要及时干预，现有方法忽视了分层解剖关系且难以处理血管结构中的严重类别不平衡问题。

Method: 采用课程学习策略和新型分形softmax进行分层语义学习，通过从简单到复杂逐步学习解剖约束，并使用两阶段推理策略加速模型。

Result: 在验证集上，分层语义损失将nnU-Net ResEnc M的Dice分数提高了11.65%；在测试集上比基线方法高5.6%的Dice分数，推理速度提升达5倍。

Conclusion: 该方法在分割精度和效率上均有显著提升，适用于实时临床应用，代码已公开。
...
TL;DR: DetGain是一种专门用于目标检测的在线数据筛选方法，通过估计每张图像对数据集平均精度的边际扰动来选择信息量大的训练样本。
...
Result: 在COCO数据集上使用多个代表性检测器进行的实验显示准确率持续提升。DetGain在低质量数据下表现出强鲁棒性，并能与知识蒸馏技术有效结合进一步提升性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有扩散方法在视频重建中的问题：基于图像扩散的方法会产生时间伪影，而原生视频扩散模型的迭代后验采样速度过慢，无法满足实时应用需求。

Method: 将预训练的双向视频扩散模型（教师）蒸馏为因果自回归学生模型，仅需教师扩散模型和已知退化算子，无需外部配对数据；使用LeanVAE替换VAE并通过教师空间正则化蒸馏方案提升吞吐量。
...
Method: 通过构建基于粗糙恢复受控退化的逆问题，在文本到图像扩散中进行后验引导采样，包含前向测量和后向测量约束。

Result: 实验表明，该方法能够生成与提示对齐的结果，并在盲人脸恢复任务中优于现有方法。

Conclusion: 测量约束采样方法有效地解决了盲人脸恢复中的一对多问题，实现了多样化且与文本提示对齐的高质量重建。
...
TL;DR: WeSTAR是一个参数高效的弱监督自训练适应框架，通过结构自监督、语义感知层次归一化、弱监督排序注释和权重正则化来提升单目深度估计基础模型在未知领域的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 虽然基础模型在单目深度估计的零样本泛化方面取得了进展，但在获得下游任务数据后，如何进一步提升这些模型的性能成为一个自然的问题。

Method: 采用密集自训练目标作为结构自监督的主要来源；引入语义感知层次归一化，利用实例级分割图进行更稳定的多尺度结构归一化；使用成对深度排序注释作为成本效益高的弱监督；应用权重正则化损失来锚定LoRA更新。

Result: 在现实和损坏的分布外数据集上的广泛实验表明，WeSTAR始终能提高泛化能力，并在各种基准测试中达到最先进的性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统GNSS定位在GNSS拒止环境中经常失效，导致协作感知中特征对齐困难，需要解决无GNSS条件下的协作感知问题。

Method: 1. 提出轻量级姿态生成器PGC估计紧凑姿态和置信度表示；2. 开发姿态感知时空对齐变换器PASTAT进行置信度感知的空间对齐并捕获时间上下文；3. 创建V2VLoc仿真数据集支持LiDAR定位和协作检测任务。

Result: 在V2VLoc数据集上的广泛实验表明，该方法在GNSS拒止条件下达到最先进性能，在真实世界V2V4Real数据集上的扩展实验验证了PASTAT的有效性和泛化能力。

Conclusion: 提出的GNSS-free协作感知框架通过PGC和PASTAT组件，在无GNSS条件下实现了鲁棒的协作感知性能，并通过仿真和真实数据集验证了方法的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像篡改检测基准存在内容多样性不足、生成模型覆盖范围窄和可解释性不够的问题，限制了检测方法的泛化能力和解释能力。

Method: 构建包含45万张图像的ManipBench基准，涵盖25个先进图像编辑模型和12种篡改类别；提出基于多模态大语言模型的ManipShield，采用对比LoRA微调和任务特定解码器实现统一检测。

Result: 在ManipBench和多个公共数据集上的实验表明，ManipShield实现了最先进的性能，并对未见过的篡改模型表现出强泛化能力。

Conclusion: ManipBench和ManipShield为解决AI编辑图像检测提供了全面的基准和有效的解决方案，将促进该领域的发展。
...
TL;DR: 本文提出了基于高斯泼溅的低秩张量表示框架(GSLR)，用于解决传统t-SVD方法在表示多维图像时的两个关键限制：潜在张量近似粗糙和变换矩阵使用固定基原子无法精确捕捉局部高频信息。
...
Method: 提出GSLR框架，利用定制的2D高斯泼溅生成潜在张量，1D高斯泼溅生成变换矩阵，两者在该表示框架中不可或缺且互补，具有强大的表示能力，特别是对局部高频信息。

Result: 在多维图像恢复的广泛实验中，GSLR始终优于最先进的方法，特别是在捕捉局部高频信息方面表现突出。

Conclusion: GSLR框架能够紧凑且连续地表示多维图像，通过2D和1D高斯泼溅的互补使用，显著提升了局部高频信息的表示能力。
...
TL;DR: VLM3D是一个通用框架，利用大型视觉语言模型作为可微分的语义和空间评判器，通过双查询评判信号评估语义保真度和几何一致性，显著提升了文本到3D生成的质量。
...
Method: 提出VLM3D框架，重新利用大型视觉语言模型作为强大的可微分语义和空间评判器，核心贡献是基于VLM的是/否对数概率的双查询评判信号，用于评估语义保真度和几何一致性。

Result: 在优化型流程中作为奖励目标显著优于现有方法；在前馈流程中作为测试时指导模块，能够主动引导迭代采样过程纠正严重空间错误。

Conclusion: VLM3D建立了一个原则性且可推广的路径，将VLM丰富的基于语言理解的语义和空间知识注入到多样化的3D生成流程中。
...
Method: NeuralBoneReg包含两个模块：学习术前骨模型的隐式神经无符号距离场，以及基于MLP的配准模块，通过生成变换假设来执行全局初始化和局部细化。

Result: 在三个多模态数据集上的评估显示，NeuralBoneReg在所有数据集上达到或超过现有方法，在UltraBones100k上平均RRE/RTE为1.68°/1.86 mm，在UltraBones-Hip上为1.88°/1.89 mm，在SpineDepth上为3.79°/2.45 mm。

Conclusion: NeuralBoneReg在解剖结构和模态间表现出强大的泛化能力，为计算机辅助骨科手术提供了鲁棒和准确的跨模态对齐。
...
TL;DR: Gen3d是一种从单张图像生成高质量、广范围通用3D场景的新方法，通过RGBD图像生成初始点云，维护和扩展世界模型，并优化高斯溅射表示来生成3D场景。
...
Method: 从RGBD图像生成初始点云，维护和扩展世界模型，最终通过优化高斯溅射表示来完成3D场景生成。

Result: 在多个数据集上的广泛实验表明，该方法在生成世界模型和合成高保真、一致的新视角方面具有强大的泛化能力和优越性能。

Conclusion: Gen3d方法能够从单张图像生成高质量的3D场景，在生成世界模型和新视角合成方面表现出色，具有很好的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决医学图像分割中数据隐私和专家标注成本高的问题，同时应对联邦半监督学习中伪标签可靠性依赖本地模型强度、客户端设备计算资源有限导致模型轻量化或异构化的挑战。
...
Conclusion: SAM-Fed通过利用分割基础模型指导轻量级客户端，有效解决了联邦半监督学习中的伪标签质量和模型容量限制问题，在医学图像分割任务中表现出优越性能。
...
TL;DR: Diff-NAF是一种针对超稀疏视图条件下多源静态CT的迭代框架，结合神经衰减场表示和双分支条件扩散模型，通过角度先验引导的投影合成和扩散驱动的重用投影细化，逐步提升投影完整性和重建质量。


<details>
  <summary>Details</summary>
Motivation: 多源静态CT在时间敏感应用中具有快速重建优势，但实际系统常受限于超稀疏视图采样，导致重建质量显著下降。传统方法在超稀疏视图条件下效果不佳，需要新的解决方案。

Method: 提出Diff-NAF迭代框架：1）使用超稀疏视图投影训练初始神经衰减场；2）通过角度先验引导的投影合成策略生成新投影；3）利用扩散驱动的重用投影细化模块优化投影；4）将细化后的投影作为伪标签加入训练集进行迭代优化。

Result: 在多个模拟3D CT体积和真实投影数据上的实验结果表明，Diff-NAF在超稀疏视图条件下实现了最佳性能，显著提升了重建质量。

Conclusion: Diff-NAF通过迭代细化策略有效解决了超稀疏视图条件下多源静态CT的重建挑战，为时间敏感应用提供了高质量的CT重建解决方案。
...
TL;DR: Dental3R是一个用于稀疏口腔内照片的3D重建方法，通过几何感知配对策略和基于小波的正则化，解决了传统方法在远程正畸中面临的姿态估计不稳定和重建细节丢失的问题。
...
Method: 提出Dental3R流程：首先使用几何感知配对策略(GAPS)选择高价值图像对，稳定几何初始化；然后在3D高斯泼溅模型上应用小波正则化目标，通过离散小波变换保持频带保真度。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法的泛化性受限，依赖设备特定训练、大量标注数据和受控扫描条件，且原始口腔网格存在牙弓姿态变化、几何不完整和缺乏纹理线索等问题。

Method: 提出几何感知的牙弓展平模块将原始3D网格标准化为空间对齐的多视图投影，构建包含层次化牙齿本体论、牙列阶段策略和临床语义的牙科知识库来约束符号推理空间。

Result: 在1060个正畸病例上验证，在牙齿计数、解剖分区、牙列阶段分类和临床状况识别方面表现鲁棒，比监督流程和VLM基线具有更高准确性和稳定性。
...
Method: 首先构建U-Midline牙弓轴线建立统一解剖坐标系，然后通过表面DRR生成类似X光片的投影，最后使用对称双向Chamfer距离和分层优化策略进行2D相似变换配准。

Result: 在34个专家标注的临床病例上评估，结果显示显著减少标志点误差（特别是后牙区域）、下颌更紧密的分散分布，以及在曲线级别获得低Chamfer距离和可控Hausdorff距离。
...
TL;DR: ARC-Chapter是首个基于百万级长视频章节数据训练的大规模视频章节划分模型，通过双语、时间定位和分层标注，在视频内容结构化方面取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 随着时长视频（如讲座、播客、纪录片）的普及，对高效内容结构化的需求日益增长，但现有方法受限于小规模训练和粗粒度标注，难以泛化到长视频中的细致过渡。

Method: 构建了双语英中章节数据集，统一ASR转录、场景文本、视觉描述为多层次标注；设计了GRACE评估指标，结合多对一段落重叠和语义相似性；通过数据规模和标注强度的扩展实现性能提升。

Result: ARC-Chapter在F1分数上比之前最佳方法提升14.0%，在SODA分数上提升11.3%，并在YouCook2等下游任务的密集视频描述中改善了最先进水平。

Conclusion: ARC-Chapter通过大规模数据训练和创新的评估指标，在视频章节划分任务上建立了新的最先进水平，并展现出优秀的可迁移性。
...
TL;DR: 本文提出了一种基于图像的3D高斯泼溅方法，通过结合标准3DGS渲染的基础颜色和从邻近训练图像学习到的残差，显著提升了渲染质量，特别是高频细节和视角相关效果。
...
Method: 提出Image-Based Gaussian Splatting方法，将每个像素颜色建模为标准3DGS渲染的基础颜色与从邻近训练图像学习到的残差的组合，利用高分辨率源图像进行精细细节和视角特定颜色建模。

Result: 在标准NVS基准测试中，该方法在渲染质量上显著优于先前的Gaussian Splatting方法，且没有增加存储占用。

Conclusion: 该方法通过结合基础渲染和学习残差，实现了高质量的高频细节和准确视角相关效果渲染，同时保持了存储效率。
...
Method: 使用Flutter框架和Google ML Kit开发移动应用Bapp，通过45名真实患者的视频进行临床验证，由眼科专家手动标注眨眼作为基准真值，采用精确率、召回率和F1分数等标准指标评估性能。

Result: Bapp在临床验证中表现出优异性能：精确率98.4%、召回率96.9%、总体准确率98.3%，证实其作为眼睑运动监测工具的可靠性。

Conclusion: Bapp是传统手动眨眼计数的有前景替代方案，支持临床环境中的连续眼部健康监测和术后评估，具有便携性和可及性优势。
...
Result: 在多个草图任务（对象定位、计数、图像检索、视觉问答）上的综合评估显示，O3SLM实现了最先进的性能，显著优于现有LVLMs。

Conclusion: O3SLM通过专门的数据集和训练方法，显著提升了大型视觉语言模型对草图的理解和推理能力。
...
TL;DR: 提出了一种肩部展示评估算法，用于自动评估生物特征身份文档中肩部姿势的合规性，该方法仅使用两个肩部标志点的3D坐标来量化肩部偏转和滚动角度。


<details>
  <summary>Details</summary>
Motivation: 国际生物特征身份文档标准要求严格符合姿势要求，包括肩部正面展示，但现有自动化质量评估文献缺乏针对这一特定属性的定量评估方法。

Method: 使用常见姿态估计框架提供的两个肩部标志点的3D坐标，量化肩部偏转和滚动角度，开发肩部展示评估算法。

Result: 在121张肖像图像数据集上评估，SPE得分与人工标注显示出强Pearson相关性（r≈0.80），使用改进的Error-versus-Discard方法分析确认了其在识别不合规样本方面的有效性。
...
### [71] [BEDLAM2.0: Synthetic Humans and Cameras in Motion](https://arxiv.org/abs/2511.14394)
*Joachim Tesch,Giorgio Becherini,Prerana Achar,Anastasios Yiannakidis,Muhammed Kocabas,Priyanka Patel,Michael J. Black*

Main category: cs.CV

TL;DR: BEDLAM2.0是一个新的数据集，用于改进从视频中推断3D人体运动的方法，特别是在世界坐标系中估计人体运动。相比BEDLAM数据集，它增加了相机和相机运动的多样性、身体形状、动作、服装、头发、3D环境的真实感，并添加了鞋子。


<details>
  <summary>Details</summary>
Motivation: 从视频中推断3D人体运动是一个具有挑战性的问题，特别是当存在人体和相机运动时，需要将人体运动估计在世界坐标系中。该领域进展受限的主要原因是缺乏包含真实人体和相机运动的地面真实数据的丰富视频数据。

Method: 开发了BEDLAM2.0数据集，它比流行的BEDLAM数据集在多个重要方面进行了改进：引入更多样化和真实的相机和相机运动，增加身体形状、动作、服装、头发和3D环境的多样性和真实感，并添加了BEDLAM中缺失的鞋子。

Result: BEDLAM2.0在训练估计世界坐标系中人体运动的方法方面显著优于BEDLAM。比较在BEDLAM和BEDLAM2.0上训练的最先进方法发现，BEDLAM2.0显著提高了准确性。

Conclusion: BEDLAM2.0是一个比BEDLAM更好的数据集，特别适用于训练在世界坐标系中估计人体运动的方法。为研究目的提供了渲染视频、地面真实身体参数和相机运动，以及3D资产和第三方链接。
...
### [72] [Stage Aware Diagnosis of Diabetic Retinopathy via Ordinal Regression](https://arxiv.org/abs/2511.14398)
*Saksham Kumar,D Sridhar Aditya,T Likhil Kumar,Thulasi Bikku,Srinivasarao Thota,Chandan Kumar*

Main category: cs.CV

TL;DR: 该论文提出了一个基于序数回归的糖尿病视网膜病变检测框架，使用APTOS-2019眼底图像数据集，通过预处理方法提取相关特征，在APTOS数据集上取得了0.8992的QWK分数，创造了新的基准。
...
Conclusion: 该序数回归框架在糖尿病视网膜病变检测方面表现出色，为及时筛查和预防失明提供了有效的技术方案。
...
### [73] [Language as an Anchor: Preserving Relative Visual Geometry for Domain Incremental Learning](https://arxiv.org/abs/2511.14401)
*Shuyi Geng,Tao Zhou,Yi Zhou*

Main category: cs.CV

TL;DR: LAVA是一个新颖的领域增量学习框架，通过基于文本的参考锚点实现相对对齐，解决传统方法中的领域干扰和知识碎片化问题。


<details>
  <summary>Details</summary>
Motivation: 解决领域增量学习中存在的两难困境：统一视觉空间导致领域间干扰和语义扭曲，而隔离领域特定参数则造成知识碎片化。

Method: 使用语言锚定的视觉对齐方法，通过类名间的语义相似性定义相对几何结构，指导每个新领域的视觉表示保持一致的相对几何关系。

Result: 在标准领域增量学习基准测试中，LAVA相比现有最先进方法实现了显著的性能提升。

Conclusion: LAVA通过基于文本的相对对齐机制，有效解决了领域增量学习中的知识保留和跨领域知识重用问题。
...
### [74] [Cranio-ID: Graph-Based Craniofacial Identification via Automatic Landmark Annotation in 2D Multi-View X-rays](https://arxiv.org/abs/2511.14411)
...
<details>
  <summary>Details</summary>
Motivation: 法医颅面识别和生物医学应用中，颅骨测量标志点很重要，但传统方法耗时且需要专业知识，现有自动标注方法缺乏大规模验证研究，可靠性不足。

Method: 1) 使用训练的YOLO-pose模型自动标注2D头骨X射线和光学图像中的标志点；2) 将标志点表示为图结构，使用跨注意力机制和最优传输框架进行跨模态图匹配。

Result: 在S2F和CUHK数据集上的广泛实验表明，该方法在可靠性和准确性方面均有显著提升，在法医科学中的头骨到面部和素描到面部匹配中表现有效。

Conclusion: Cranio-ID框架通过自动标注和跨模态图匹配，显著提高了颅骨标志点定位的可靠性和准确性，为法医颅面识别提供了有效的解决方案。
...
### [75] [Learning to See Through a Baby's Eyes: Early Visual Diets Enable Robust Visual Intelligence in Humans and Machines](https://arxiv.org/abs/2511.14440)
*Yusen Cai,Bhargava Satya Nunna,Qing Lin,Mengmi Zhang*

Main category: cs.CV

TL;DR: 该研究通过模拟婴儿视觉发育的阶段性特征（灰度到彩色、模糊到清晰、保持时间连续性）训练自监督学习模型，发现这种'视觉饮食'策略能增强模型的鲁棒性，并表现出与生物发育相似的模式。


<details>
  <summary>Details</summary>
Motivation: 探索婴儿视觉发育的阶段性特征（低清晰度、色彩退化、时间连续性）如何为机器视觉系统提供生态优势，以及这种'视觉饮食'策略是否能增强模型的鲁棒性和生物对齐性。

Method: 提出CATDiet方法，在物体中心视频上训练自监督学习模型，模拟婴儿视觉的三个约束：灰度到彩色(C)、模糊到清晰(A)、保持时间连续性(T)。进一步提出CombDiet，先用CATDiet初始化，再进行标准训练。

Result: 所有CATDiet变体在物体识别任务中表现出增强的鲁棒性，模型还展现出与生物发育对齐的模式，包括类似猕猴V1区突触密度的神经可塑性变化，以及类似婴儿视觉悬崖反应的行为。CombDiet在物体识别和深度感知任务上优于标准自监督学习。

Conclusion: 婴儿早期视觉体验的发育进程为理解机器中稳健视觉智能的出现提供了一个强大的逆向工程框架，表明阶段性视觉输入策略能有效提升模型的鲁棒性和生物对齐性。
...
### [76] [DIR-TIR: Dialog-Iterative Refinement for Text-to-Image Retrieval](https://arxiv.org/abs/2511.14449)
*Zongwei Zhen,Biqing Zeng*

Main category: cs.CV

TL;DR: DIR-TIR框架通过对话精炼和图像精炼两个模块，实现交互式对话文本到图像检索，显著提高目标图像命中精度。
...
Method: 使用Dialog Refiner Module主动查询用户提取关键信息并生成精确描述，Image Refiner Module识别生成图像与用户意图之间的感知差距，减少视觉语义差异。

Result: 在多个图像数据集上的实验表明，该方法显著优于仅使用初始描述的基线方法，检索精度更高且交互体验更好。

Conclusion: DIR-TIR框架通过对话驱动的渐进式精炼，在文本到图像检索任务中实现了优越的控制性、容错性和检索性能。
...
### [77] [Learning Subglacial Bed Topography from Sparse Radar with Physics-Guided Residuals](https://arxiv.org/abs/2511.14473)
*Bayu Adhi Tama,Jianwu Wang,Vandana Janeja,Mostafa Cham*

Main category: cs.CV

TL;DR: 提出了一种物理引导的残差学习框架，通过预测BedMachine先验上的床厚残差来重建冰下床地形，结合多尺度物理约束和深度学习，在格陵兰冰盖区域取得了优于传统方法的性能。


<details>
  <summary>Details</summary>
Motivation: 冰下床地形的准确测量对冰盖建模至关重要，但雷达观测数据稀疏且分布不均，需要开发能够有效利用物理约束和深度学习的方法来填补数据空白。

Method: 使用DeepLabV3+解码器和标准编码器（如ResNet-50），结合轻量级物理和数据项进行训练，包括多尺度质量守恒、流向总变分、拉普拉斯阻尼、厚度非负性、斜坡先验一致性项，以及基于置信度图调制的掩码Huber拟合雷达拾取。

Result: 在两个格陵兰子区域上，该方法在测试核心区域实现了强健的精度和高结构保真度，优于U-Net、Attention U-Net、FPN和普通CNN。

Conclusion: 残差-先验设计结合物理约束，能够产生空间一致、物理合理的床地形，适用于领域偏移下的操作制图。
...
### [78] [Segmentation-Aware Latent Diffusion for Satellite Image Super-Resolution: Enabling Smallholder Farm Boundary Delineation](https://arxiv.org/abs/2511.14481)
...
TL;DR: SEED-SR方法通过结合条件潜在扩散模型和地理空间基础模型，在分割感知的潜在空间中执行超分辨率，实现了20倍尺度因子的农场边界分割，显著提升了实例和语义分割性能。


<details>
  <summary>Details</summary>
Motivation: 解决小农户农场边界分割的挑战，传统方法使用高分辨率图像但更新频率低，需要结合低分辨率图像进行更频繁的监测，但现有Ref-SR方法无法满足大尺度因子需求且会平滑关键特征。
...
Conclusion: SEED-SR方法能够以前所未有的20倍尺度因子生成分割图，有效解决了小农户农场边界分割的挑战，为农业应用提供了更好的监测能力。
...
### [79] [Enhancing End-to-End Autonomous Driving with Risk Semantic Distillaion from VLM](https://arxiv.org/abs/2511.14499)
...
<details>
  <summary>Details</summary>
Motivation: 解决当前自动驾驶系统在泛化能力方面的限制，特别是处理未见场景和不同传感器配置的能力，同时避免混合系统的不一致性和端到端方案的高计算成本。

Method: 提出Risk Semantic Distillation框架，引入RiskHead插件模块，从视觉语言模型中蒸馏因果风险估计到鸟瞰图特征，生成可解释的风险注意力图。

Result: 在Bench2Drive基准测试中，RSD在复杂和不可预测的驾驶条件下表现出色，感知和规划能力均有显著提升。

Conclusion: RSD通过增强BEV表示的风险注意力，有效提升了自动驾驶系统在复杂动态环境中的泛化能力和人机驾驶行为对齐度。
...
TL;DR: 提出PAMM模型，利用状态空间模型增强多任务密集预测中的任务互连性，通过双状态空间参数专家集成任务特定参数先验，并在NYUD-v2和PASCAL-Context基准测试中验证有效性。
...
Method: 提出参数感知Mamba模型(PAMM)，利用状态空间模型的丰富可扩展参数增强任务互连性，包含双状态空间参数专家来集成任务特定参数先验，并通过多方向希尔伯特扫描方法构建多角度特征序列。

Result: 在NYUD-v2和PASCAL-Context基准测试上的广泛实验证明了所提方法的有效性。

Conclusion: PAMM框架通过状态空间模型有效增强了多任务密集预测中的任务互连性，实现了精确的多任务交互和全局任务先验集成。
...
TL;DR: D-PerceptCT是一种受人类视觉系统启发的LDCT图像增强架构，通过集成语义先验和多尺度特征来增强感知相关特征，避免过度平滑导致细节丢失。


<details>
  <summary>Details</summary>
Motivation: 现有LDCT增强方法往往高估噪声并过度平滑，导致关键细节丢失。需要一种能够增强或保留感知相关特征的方法，为放射科医生提供解剖结构和病理细节清晰可见的CT图像。

Method: 提出D-PerceptCT架构，包含两个主要模块：1)视觉双路径提取器(ViDex)，集成预训练DINOv2模型的语义先验和局部空间特征；2)全局-局部状态空间块，捕获长距离信息和多尺度特征。还提出深度感知相关性损失函数(DPRLF)，基于人类对比敏感度。

Result: 在Mayo2016数据集上的广泛实验表明，D-PerceptCT在LDCT增强方面优于现有最先进方法，能更好地保留LDCT图像中的结构和纹理信息。

Conclusion: D-PerceptCT通过整合人类视觉系统原理和语义感知，有效提升了LDCT图像质量，为放射诊断提供了更好的视觉支持。
...
TL;DR: DeCo-VAE通过解耦视频内容为关键帧、运动和残差三个组件，为每个组件学习专用潜在表示，实现紧凑的视频潜在表征，避免冗余建模。


<details>
  <summary>Details</summary>
Motivation: 现有视频变分自编码器通常忽略帧间内容相似性，导致潜在建模冗余。本文旨在通过解耦视频内容实现紧凑的潜在表示。

Method: 将视频内容解耦为关键帧、运动和残差三个组件，为每个组件设计专用编码器，使用共享3D解码器保持时空一致性，并采用解耦适应策略进行顺序训练。

Result: 大量定量和定性实验表明，DeCo-VAE在视频重建性能上表现优异。

Conclusion: DeCo-VAE通过解耦视频内容和专用组件编码，实现了更紧凑和高效的视频潜在表示，显著提升了视频重建质量。
...
TL;DR: 提出了一种在共享空间中表示多个神经符号距离函数(SDF)的方法，通过结合泛化学习和过拟合学习策略的优势，用更紧凑的潜表示恢复高保真几何细节。


<details>
  <summary>Details</summary>
Motivation: 传统的神经SDF在单个形状或场景上表现良好，但在分析多个SDF时存在困难，因为潜空间编码信息有限且会丢失几何细节。

Method: 结合泛化学习和过拟合学习策略的优势，提出新的采样策略来提高训练效率并消除其他SDF影响造成的伪影。

Result: 在广泛使用的基准测试上进行了数值和视觉评估，验证了设计有效性，在表示能力和紧凑性方面优于最新方法。

Conclusion: 该方法能够用更紧凑的潜表示恢复更多高保真几何细节，为多个SDF的表示提供了有效解决方案。
...
TL;DR: 本文提出了一种无需物体先验的动态手-物体交互场景建模方法，基于3D高斯泼溅技术，通过交互感知的高斯模型、动态场构建和渐进式优化策略，实现了高质量的几何和外观重建。


<details>
  <summary>Details</summary>
Motivation: 现有方法在建模手-物体交互场景时面临相互遮挡、边缘模糊等挑战，且通常依赖物体先验知识。本文旨在无需物体先验的情况下，同时建模几何和外观。

Method: 1) 引入交互感知的手-物体高斯模型，采用分段线性假设优化参数；2) 将手部信息融入物体变形场，构建交互感知动态场；3) 提出渐进式优化策略，分别处理动态区域和静态背景；4) 设计显式正则化确保平滑运动过渡和物理交互真实性。

Result: 实验表明，该方法超越了现有的基于动态3D-GS的方法，在手-物体交互重建方面达到了最先进的性能。

Conclusion: 所提出的交互感知动态3D高斯泼溅方法能够有效建模复杂的手-物体交互场景，无需物体先验知识，在几何和外观重建方面表现出色。
...
TL;DR: ForensicFlow是一个三模态深度伪造检测框架，通过融合RGB、纹理和频率证据来检测视频Deepfake，在Celeb-DF数据集上达到AUC 0.9752的优异性能。
...
Method: 提出三模态框架：RGB分支提取全局视觉不一致性，纹理分支检测细粒度融合伪影，频率分支识别周期性频谱噪声；采用注意力机制进行时序池化和自适应融合。

Result: 在Celeb-DF数据集上达到AUC 0.9752、F1-Score 0.9408和准确率0.9208，优于单流基线方法。消融实验验证了分支协同作用，Grad-CAM确认了检测焦点。

Conclusion: 通过全面的特征融合，该框架对细微伪造具有更强的鲁棒性，为深度伪造检测提供了更有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于显著性映射的方法（如遮挡、GradCAM、相关性传播）仅能突出显示单个切片中对预测贡献最大的区域，缺乏对模型全局行为的理解，限制了在临床实践中的应用。

Method: 开发基于聚类的可解释性技术，通过聚类分析来可视化模型的全局行为模式，同时提供细粒度信息。

Result: 在现有前列腺癌检测模型上评估了该技术的性能，证明了其有效性。聚类结果不仅有助于理解模型，还能增强对其操作的信心。

Conclusion: 该方法能够促进数字病理模型在临床实践中更快地被采纳，通过提供全局视角和细粒度信息来增强模型的可解释性和可信度。
...
<details>
  <summary>Details</summary>
Motivation: 处理音频-视频令牌序列在OmniLLMs中造成了显著的计算瓶颈，现有令牌压缩方法尚未满足多模态令牌联合压缩的新兴需求。

Method: 首先识别关键音频令牌，然后计算每个时间组的音频保留分数以捕捉信息密度，动态指导视频令牌剪枝，并使用交错时空方案压缩视频令牌。
...
TL;DR: 提出了一个深度学习框架用于WMH分割和定位，评估显示区域WMH体积在疾病分类中优于全局病变负担，结合脑萎缩指标可进一步提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有WMH自动分割方法大多只提供全局病变负荷，忽视了不同白质区域的空间分布，而区域WMH量化可能对神经退行性疾病的早期诊断和分层有重要价值。

Method: 使用深度学习框架进行稳健的WMH分割和定位，在公共数据集和独立ADNI队列中评估，量化解剖定义区域内的WMH负荷，并结合脑结构体积评估诊断价值。

Result: 预测的病变负荷与参考WMH估计一致，区域WMH体积在疾病分类中始终优于全局病变负担，与脑萎缩指标结合后性能进一步提升，AUC值高达0.97。
...
Method: 采用共享-特定编码器-解码器架构，包含两种自蒸馏策略：层次化模态自蒸馏机制减少语义差异，渐进式模态组合蒸馏方法通过模拟模态丢失增强鲁棒性。

Result: 在公共脑肿瘤分割基准测试中，CCSD在各种模态缺失场景下均达到最先进性能，具有强大的泛化能力和稳定性。

Conclusion: CCSD框架能够有效解决多模态MRI中的模态缺失问题，为临床脑肿瘤分割提供了一种灵活且鲁棒的解决方案。
...
TL;DR: 本研究评估表格数据和基于MRI的表示在预测阿尔茨海默病认知衰退中的贡献，特别关注transformer衍生的MRI嵌入。结果显示临床特征在识别高风险极端情况方面表现最佳，而transformer MRI嵌入对稳定性标志更敏感。
...
Method: 引入基于动态时间规整聚类的轨迹感知标记策略，通过无监督重建在协调和增强的MRI数据上训练3D视觉Transformer，获得保留解剖结构的嵌入，随后使用传统机器学习分类器和深度学习头进行评估。

Result: 临床和体积特征在预测轻度和重度进展方面达到约0.70的最高AUC，而ViT模型的MRI嵌入在区分认知稳定个体方面最有效，AUC为0.71。所有方法在异质性中度组中都表现不佳。

Conclusion: 临床特征在识别高风险极端情况方面表现优异，而基于transformer的MRI嵌入对稳定性标志更敏感，这为AD进展建模的多模态融合策略提供了动机。
...
<details>
  <summary>Details</summary>
Motivation: 骨健康不良是重要的公共卫生问题，低骨密度会增加骨折风险，这是骨质疏松症的关键特征。现有方法需要改进多模态数据的有效融合和临床显著性病例的优先处理。

Method: 提出XAttn-BMD框架，使用新颖的双向交叉注意力机制动态整合图像和元数据特征，采用加权平滑L1损失函数处理骨密度不平衡并优先考虑临床显著性病例。

Result: 在Hertfordshire Cohort Study数据上的实验表明，该模型在回归泛化性和鲁棒性方面优于基线模型。交叉注意力融合相比简单特征拼接，MSE降低16.7%，MAE降低6.03%，R2得分提高16.4%。

Conclusion: 该研究证明了通过交叉注意力整合多模态数据在股骨颈骨密度估计中的有效性，模型在临床相关阈值下的二元分类表现展示了其在真实场景中的应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的预测算法大多独立处理每个切片而忽略3D结构，而现有的3D感知方法不可生成且扩展性差，需要开发能够充分利用3D结构信息且可扩展的算法。

Method: 在共享特征空间中检索相邻切片上的形态对应斑点，将跨切片上下文融合到轻量级ControlNet中；引入结合学习到的零膨胀负二项分布先验和空间经验先验的3D一致性先验；使用全局注意力块实现线性扩展。
...
TL;DR: 本文提出了一种名为Stereo Geometry-Texture Alignment的方法，通过联合优化渲染质量和几何估计，在稀疏视图场景下实现更准确和详细的表面重建，同时保持高质量的新视角合成性能。
...
Method: 提出了Stereo Geometry-Texture Alignment方法，将渲染质量与几何估计相结合；同时引入Pseudo-Feature Enhanced Geometry Consistency，通过结合训练视图和未见视图来强制多视图几何一致性，有效缓解稀疏监督导致的过拟合。

Result: 在DTU、BlendedMVS和Mip-NeRF360数据集上的大量实验表明，该方法达到了最先进的性能水平。

Conclusion: 该方法能够在稀疏视图场景下重建更准确和详细的表面，同时保持高质量的新视角渲染，有效解决了现有方法在稀疏监督下的过拟合问题。
...
TL;DR: SLAM-AGS是一个针对计算细胞学的多任务预训练框架，通过弱监督相似性目标和自监督对比学习来应对实例级标签稀缺和阳性样本率极低的挑战，在低阳性率下显著提升下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 计算细胞学面临两个主要挑战：实例级标签获取成本高且不可靠，以及阳性样本率极低。需要开发能够有效利用弱监督信息并在低阳性率下保持稳定性能的方法。

Method: 提出SLAM-AGS框架，联合优化弱监督相似性目标（处理阴性样本）和自监督对比目标（处理阳性样本），使用自适应梯度手术解决任务梯度冲突，并将预训练编码器集成到基于注意力的多实例学习聚合器中。

Result: 在公开骨髓细胞学数据集上，当阳性率从10%降至0.5%时，SLAM-AGS在袋级F1分数和Top 400阳性细胞检索方面优于其他预训练方法，在低阳性率下提升最显著。

Conclusion: 解决梯度干扰能够实现稳定的预训练，并在下游任务中获得更好的性能，特别是在低阳性率场景下。
...
TL;DR: 提出了一种基于时间特征增强的简单有效方法，用于多普勒全息图中的动静脉分割，通过脉冲分析特征使传统U-Net能够利用时间动态信息。


<details>
  <summary>Details</summary>
Motivation: 传统视网膜血管分割方法仅关注空间信息，忽略了多普勒全息数据的时间丰富性，需要利用时间动态信息进行准确的动静脉分割。

Method: 使用标准分割架构，结合专用脉冲分析流程提取的时间特征，使传统U-Net能够利用时间动态信息。

Result: 该方法实现了与更复杂的注意力或迭代模型相当的性能，证明了时间分辨预处理能够充分发挥深度学习在多普勒全息中的应用潜力。

Conclusion: 时间分辨预处理能够解锁深度学习在多普勒全息中的全部潜力，为视网膜血流动力学的定量探索开辟了新视角。
