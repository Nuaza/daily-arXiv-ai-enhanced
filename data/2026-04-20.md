...
TL;DR: 论文提出"缩放一致性"概念，利用多步缩放GUI定位流程中的中间预测作为置信度信号，无需额外计算成本即可评估预测可靠性。


<details>
  <summary>Details</summary>
Motivation: 当前多步缩放GUI定位流程中，中间预测结果在坐标重映射后通常被丢弃。作者发现这些中间输出包含有用的置信度信号——缩放一致性，即模型第二步预测与裁剪中心之间的距离，这可以作为评估预测可靠性的几何指标。

Method: 提出缩放一致性作为置信度度量，定义为模型第二步预测与裁剪中心之间的几何距离。在理想条件下证明该量是第一步空间误差的线性估计器，并在两个VLM模型（KV-Ground-8B和Qwen3.5-27B）上验证其与预测正确性的相关性。

Result: 缩放一致性在两种VLM模型中都显示出与预测正确性的显著相关性（AUC = 0.60；Spearman rho = -0.14，p < 10^{-6}；rho = -0.11，p = 0.0003）。相关性虽小但在不同模型、应用类别和操作系统中保持一致。作为概念验证，使用缩放一致性在专家模型和通用模型之间进行路由，捕获了16.5%的oracle性能差距。

Conclusion: 缩放一致性是一种简单有效的置信度度量，无需校准即可在不同架构的VLM之间直接比较，为多步GUI定位流程提供了有价值的免费置信度信号。
...
TL;DR: 提出一种通用知识蒸馏方法，通过冻结较弱教师模型、仅在早期训练阶段应用蒸馏，当学生模型达到并超越教师性能时关闭蒸馏，实现视觉学习任务训练加速。


<details>
  <summary>Details</summary>
Motivation: 大规模视觉学习的训练成本日益成为限制因素。现有知识蒸馏方法主要用于模型压缩或最终精度提升，本研究探索如何利用蒸馏来加速强学生模型的训练过程。

Method: 提出通用即插即用方案：1) 冻结一个较弱的教师模型；2) 仅在训练早期阶段应用知识蒸馏；3) 当学生模型达到并超越教师性能水平时关闭蒸馏。

Result: 在ImageNet和CIFAR分类任务中，该方法显著提前达到目标阈值，最高实现4.8倍的epoch加速。在COCO目标检测任务中实现1.7倍epoch加速，在CIFAR-10扩散生成任务中实现2.5倍目标FID提前达到。

Conclusion: 该方法被验证为视觉学习的通用加速机制，能够在多种视觉任务中显著减少训练时间，为大规模视觉学习提供有效的训练加速解决方案。
...
TL;DR: 本文研究了自回归生成模型中token结构对测试时搜索能力的影响，发现粗到细的1D有序tokenizer比传统2D网格结构更有利于通过验证器引导生成过程。
...
Method: 使用图像生成作为测试平台，比较粗到细有序tokenizer与传统2D网格结构，通过控制实验研究不同搜索算法（best-of-N、束搜索、前瞻搜索）与token结构的交互作用。

Result: 粗到细有序token模型展现出更好的测试时扩展行为；利用有序结构，仅通过测试时搜索（无需训练AR模型）就能实现免训练的文本到图像生成；系统分析了不同验证器和AR先验的作用。

Conclusion: token结构显著影响推理时的可扩展性，粗到细有序结构更有利于测试时搜索引导生成，为AR模型的测试时扩展提供了实用指导。
...
TL;DR: FreqFlow是一种频率感知的流匹配方法，通过时间依赖的自适应加权将频率信息显式整合到生成过程中，改善图像生成中低频结构和高频细节的协调建模。
...
Method: 提出频率感知流匹配（FreqFlow），采用双分支架构：1）频率分支分别处理低频和高频分量以捕获全局结构和细化纹理边缘；2）空间分支在潜在域合成图像，由频率分支输出指导。通过时间依赖的自适应加权显式整合频率信息。

Result: 在ImageNet-256类别条件生成基准上，FID达到1.38，优于之前的扩散模型DiT（提升0.79 FID）和流匹配模型SiT（提升0.58 FID），实现了最先进的性能。

Conclusion: 通过显式整合频率信息，FreqFlow能有效建模大规模连贯性和细粒度细节，低频条件强化全局结构，高频条件增强纹理保真度和细节清晰度，显著提升图像生成质量。
...
<details>
  <summary>Details</summary>
Motivation: 传统上专家需要手动评估数千个亚毫米尺寸样品的横截面特征，这个过程既耗时又主观。需要自动化方法来提高TRISO燃料微观图像分析的效率和准确性。

Method: 提出UA-Net深度学习框架，采用多阶段预训练策略：首先从ImageNet学习通用图像表示，然后在来自不同辐照实验和AGR-5/6/7粒子横截面的TRISO微观图像上进行微调。集成元模型进行不确定性预测以识别TRISO图像中的小缺陷。

Result: 在102张图像的测试集上，UA-Net实现了95.5%的平均交并比(mIoU)和97.3%的平均精度(mP)。元模型实现了91.8%的特异性和93.5%的敏感性，在检测错误分类方面表现出色。对新TRISO图像的定性评估也显示出提取层区域的高准确性。

Conclusion: UA-Net框架能够准确分割TRISO燃料微观图像的特征区域，并通过不确定性预测有效识别小缺陷，为TRISO燃料性能分析提供了高效、客观的自动化工具。
...
Method: 构建多中心数据集（PadChest和NIH Chest X-ray），包含超过14.5万张图像，所有开发和测试集由放射科医生标注。定义两个核心任务：30个已知类别的稳健多标签分类，以及6个未见（分布外）罕见疾病类别的开放世界泛化。

Result: 视觉语言基础模型改善了分布内和零样本性能，但在多中心偏移下检测罕见发现仍然具有挑战性。研究揭示了头尾性能差异、校准问题和跨中心泛化差距。

Conclusion: 该研究为在现实长尾和开放世界临床条件下开发和评估AI系统提供了基础，强调了罕见病理检测和多中心泛化的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 预测患者大脑的演变有助于早期干预、预后和治疗规划。现有的潜在扩散模型依赖自注意力模块，计算成本高，需要更高效的架构来建模大脑结构的时间变化。

Method: CLIMB使用状态空间模型架构替代传统自注意力模块，大幅降低计算开销。引入高斯对齐自编码器提取符合先验分布的潜在表示，避免传统变分自编码器的采样噪声。以基线MRI扫描和采集年龄为基础输入，结合预测年龄、性别、疾病状态、遗传信息和脑结构体积等多条件变量增强时间建模。

Result: 在ADNI数据集（1,390名参与者的6,306个MRI扫描）上训练和评估，生成图像与真实MRI扫描的结构相似性指数达到0.9433，相比现有方法有显著改进。

Conclusion: CLIMB框架能够高效建模大脑结构的时间演变，在保持高质量图像合成的同时大幅降低计算成本，为医学影像分析和临床决策提供有力工具。
...
TL;DR: 本文提出CBA框架解决无监督可见光-红外视频行人重识别问题，通过因果干预预热和原型引导的不确定性细化来提升跨模态对齐效果。


<details>
  <summary>Details</summary>
Motivation: 现有的VVI-ReID方法严重依赖昂贵的跨模态标注进行全监督学习，限制了可扩展性。无监督学习方法直接使用通用预训练编码器会导致身份判别能力弱和模态偏差强，造成严重的模态内身份混淆和跨模态聚类粒度不平衡。

Method: 提出因果引导对齐框架：1）因果干预预热：利用时间身份一致性和跨模态身份一致性进行序列级因果干预，抑制模态和运动引起的虚假相关性；2）原型引导的不确定性细化：采用粗到细的对齐策略解决跨模态粒度不匹配问题，在可靠可见光原型指导下重组欠聚类的红外表示。

Result: 在HITSZ-VCM和BUPTCampus基准测试上的大量实验表明，CBA在扩展到USL-VVI-ReID设置时显著优于现有的USL-VI-ReID方法。

Conclusion: CBA框架通过利用视频先验知识，有效解决了无监督可见光-红外视频行人重识别中的身份混淆和跨模态粒度不平衡问题，为全天候监控提供了可扩展的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有开放词汇遥感图像分割研究存在数据集碎片化、训练多样性不足、缺乏反映真实地理空间应用需求的评估基准等问题，需要更全面、应用导向的基准来评估开放世界泛化能力。
...
Conclusion: 研究强调了现实基准设计的重要性以及基于扰动的迁移方法在开放词汇遥感图像分割中的有效性，为地理空间应用提供了更实用的评估框架。
...
TL;DR: 本文优化了基于单目视频的3D生物力学分析流程，使其能在仅CPU的消费级硬件上高效运行，实现了2.47倍的吞吐量提升和59.6%的总运行时间减少。


<details>
  <summary>Details</summary>
Motivation: 当前大多数研究级的3D生物力学分析流程依赖GPU加速，限制了在消费级硬件和低资源环境中的部署。需要开发能在仅CPU硬件上高效运行的解决方案，以实现更广泛的可访问性和可扩展性。

Method: 基于MonocularBiomechanics框架，通过性能分析驱动的系统优化，包括：模型初始化结构重构、消除磁盘I/O序列化、改进CPU并行化等，优化单目3D生物力学分析流程。

Result: 在消费级工作站（AMD Ryzen 7 9700X CPU）上，处理吞吐量提高了2.47倍，总运行时间减少了59.6%，初始化延迟降低了4.6倍。生物力学输出与基准实现高度一致（平均关节角度偏差0.35°，相关系数r=0.998）。

Conclusion: 研究结果表明，研究级的视觉生物力学分析流程可以在仅CPU的消费级硬件上部署，实现可扩展的运动评估，提高了生物力学分析的可访问性和部署灵活性。
...
TL;DR: 该研究正式定义了无人机推理分割任务，构建了包含10k高分辨率航拍图像的DRSeg基准数据集，并提出了PixDLM模型作为统一基线，为无人机推理分割研究奠定基础。


<details>
  <summary>Details</summary>
Motivation: 推理分割从地面场景扩展到遥感图像，但无人机数据面临独特挑战：倾斜视角、超高分辨率和极端尺度变化。需要专门解决无人机推理分割问题。

Method: 1. 正式定义无人机推理分割任务，将语义需求组织为空间、属性和场景级三个推理维度；2. 构建DRSeg大规模基准数据集，包含10k高分辨率航拍图像，配备跨三种推理类型的思维链问答监督；3. 提出PixDLM（像素级多模态语言模型）作为统一基线方法。

Result: 在DRSeg基准上建立了强基线结果，突显了无人机推理分割的独特挑战，为未来研究提供了坚实基础。

Conclusion: 该研究通过定义任务、构建基准数据集和提出基线模型，为无人机推理分割领域建立了系统框架，解决了倾斜视角、高分辨率和尺度变化等独特挑战，推动了该领域的发展。
...
TL;DR: 本文提出HyCal方法，通过结合余弦相似度和马氏距离来缓解异构域不平衡数据中的"域引力"问题，在跨学科可变少样本类增量学习基准上取得更好效果。


<details>
  <summary>Details</summary>
Motivation: 现有少样本类增量学习方法假设同质域和平衡数据分布，但现实世界中数据来自异构学科，存在样本可用性不平衡和视觉复杂度变化的问题。作者发现"域引力"现象——数据在异构域间的不平衡导致过表示或低熵域对嵌入空间产生不成比例的影响，导致原型漂移和低表示域性能下降。

Method: 提出混合原型校准(HyCal)，一种无需训练的方法，结合余弦相似度和马氏距离来捕捉互补的几何特性——方向对齐和协方差感知的幅度，从而在不平衡异构条件下产生稳定的原型。该方法在冻结的CLIP嵌入上操作。

Result: 实验表明HyCal能有效缓解域引力问题，在不平衡跨域增量学习中优于现有方法，同时保持效率。

Conclusion: HyCal方法通过几何特性组合解决了异构域不平衡数据中的原型漂移问题，为现实世界中的跨学科少样本类增量学习提供了有效解决方案。
...
TL;DR: 提出了一种基于Noisier2Inverse的自监督光声断层成像重建方法，无需真实图像标签即可处理有限尺寸探测器导致的图像模糊问题。


<details>
  <summary>Details</summary>
Motivation: 光声断层成像中，有限尺寸探测器会导致重建图像系统性地模糊。虽然可以通过补偿有限探测器效应获得更清晰的图像，但监督学习方法通常需要真实图像标签，这在实践中往往难以获得。

Method: 提出自监督重建方法，包含三个关键组件：(1) 将问题重新表述为角度去模糊的PAT特定建模；(2) 在极坐标域中利用已知角度点扩散函数的Noisier2Inverse公式；(3) 新颖的基于统计的早停规则。

Result: 该方法在不使用监督数据的情况下，始终优于其他替代方法，性能接近监督基准，同时在实际有限尺寸探测器采集数据中保持实用性。

Conclusion: 提出的自监督方法有效解决了有限尺寸探测器效应，无需真实图像标签，为实际光声断层成像应用提供了实用解决方案。
...
TL;DR: P³T是一种参数高效的提示调优方法，用于预训练的3D视觉语言模型，通过点级提示和文本提示实现任务特定适应，同时保持泛化能力。


<details>
  <summary>Details</summary>
Motivation: 预训练3D点云模型在下游任务适应中，传统全微调方法计算成本高且存储密集，而现有提示调优方法容易过拟合，损害泛化能力。

Method: 提出原型点级提示调优(P³T)，包含两个组件：1)点提示器为输入点云生成实例感知的点级提示；2)文本提示器将可学习提示嵌入输入文本。同时引入原型损失减少类别内方差，增强嵌入空间对齐。

Result: 在分类和少样本学习任务中，P³T匹配或优于全微调性能，在跨数据集设置下对数据偏移表现出鲁棒的泛化能力。

Conclusion: P³T是一种参数高效的3D视觉语言模型调优方法，既能实现任务特定适应，又保持模型泛化能力，解决了传统微调计算成本高和现有提示调优过拟合的问题。
...
TL;DR: 提出LP²DH框架，通过局部保持的像素差异哈希将高维STLBP特征编码为紧凑二进制码，在动态纹理识别基准上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: STLBP作为动态纹理描述符存在维度极高的问题，现有方法在三个正交平面上提取特征牺牲了平面间相关性，需要更好的特征编码方法

Method: 提出LP²DH框架：1) 在全时空邻域联合编码像素差异；2) 通过局部保持嵌入保持PDV的局部结构；3) 使用曲率搜索策略在Stiefel流形上联合优化哈希矩阵和二进制码；4) 哈希后应用字典学习将二进制向量编码为码字，生成直方图作为最终特征
...
<details>
  <summary>Details</summary>
Motivation: 传统基于外观的视觉说话人识别方法对环境变化敏感，而基于唇部运动的识别具有行为驱动的稳定性优势。但传统帧相机存在运动模糊和动态范围限制，难以捕捉细粒度唇部动态。事件相机能更好地捕捉快速运动细节，因此提出NeuroLip框架来利用事件数据实现鲁棒的跨场景唇部运动识别。

Method: 1) 时间感知体素编码模块：采用自适应事件加权；2) 结构感知空间增强器：抑制噪声同时保留垂直结构运动信息；3) 极性一致性正则化机制：保留事件极性编码的运动方向线索。在DVSpeaker数据集（50个受试者，4种视角和光照场景）上评估。

Result: NeuroLip在匹配场景下达到接近完美的准确率，在跨场景泛化方面表现优异：在未见视角下超过71%准确率，在低光照条件下接近76%准确率，比现有代表性方法至少高出8.54%。

Conclusion: NeuroLip证明了事件相机在捕捉细粒度唇部动态方面的优势，通过提出的三个核心模块实现了鲁棒的跨场景视觉说话人识别。DVSpeaker数据集为相关研究提供了系统评估基准，代码和数据已开源。
...
TL;DR: 提出一种无监督扩散自编码器，用于修复手持眼底图像中的伪影，无需配对监督即可提升图像质量和诊断准确性


<details>
  <summary>Details</summary>
Motivation: 手持眼底成像设备虽然提高了眼科诊断和疾病筛查的可及性，但其图像常存在闪光反射、曝光变化和运动模糊等伪影，这些伪影会降低图像质量并影响下游分析。现有生成模型大多依赖配对监督或预定义的伪影结构，难以适应手持眼底图像中常见的非结构化退化

Method: 提出一种无监督扩散自编码器，将上下文编码器与去噪过程相结合，学习语义上有意义的表示用于伪影修复。模型仅在高质量桌面眼底图像上训练，但能够推断并修复受伪影影响的手持采集图像

Result: 通过定量和定性评估验证了修复效果，在未见数据集和多种伪影条件下，诊断准确率提升至81.17%

Conclusion: 该方法能够有效修复手持眼底图像中的伪影，提高图像质量和诊断准确性，无需配对监督即可适应非结构化退化
...
TL;DR: STBIR框架通过结合手绘草图的轮廓信息和文本的颜色纹理信息，实现细粒度图像检索，显著优于现有方法


<details>
  <summary>Details</summary>
Motivation: 手绘草图捕捉复杂结构轮廓但缺乏颜色纹理，文本提供颜色纹理但缺少空间轮廓，两者具有互补性，需要解决模态差距问题以实现细粒度图像检索

Method: 提出STBIR框架：1) 课程学习驱动的鲁棒性增强模块处理不同质量查询；2) 基于类别知识的特征空间优化模块提升表征能力；3) 多阶段跨模态特征对齐机制缓解对齐挑战；4) 构建细粒度STBIR基准数据集

Result: 大量实验表明，STBIR框架在细粒度图像检索任务上显著优于现有最先进方法

Conclusion: 通过结合草图和文本的互补信息，STBIR框架有效解决了跨模态细粒度图像检索问题，为后续研究提供了数据支持和参考框架
...
TL;DR: RefereeBench是首个评估多模态大语言模型作为自动体育裁判能力的大规模基准，涵盖11项运动、925个视频和6,475个问答对，测试五种核心裁判能力。当前最先进模型准确率仅约60%，显示其距离可靠体育裁判仍有很大差距。


<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型在通用视频理解方面表现出色，但其在支持专业化、基于规则的决策制定方面的能力尚未得到充分探索。本研究旨在填补这一空白，专门评估MLLMs作为自动体育裁判的能力。

Method: 引入RefereeBench基准，涵盖11项不同运动，包含925个精心挑选的视频和6,475个人工标注的问答对。基准评估五种核心裁判能力：犯规存在性判断、犯规和处罚分类、犯规和处罚推理、实体感知以及时间定位。所有标注均由人工完成，确保基于真实裁判逻辑和多模态证据的高质量标注。

Result: 对最先进MLLMs的广泛评估显示，即使是Doubao-Seed-1.8和Gemini-3-Pro等最强模型，准确率也仅达到约60%，而最强的开源模型Qwen3-VL仅达到47%。模型虽然能识别事件和涉及实体，但在规则应用和时间定位方面表现不佳，且经常在正常片段上过度判罚犯规。

Conclusion: 当前模型距离成为可靠的体育裁判仍有很大差距。RefereeBench基准突出了未来MLLMs需要更好地整合领域知识和多模态理解，以推进可信赖的AI辅助裁判和更广泛的多模态决策制定。
...
<details>
  <summary>Details</summary>
Motivation: 现有概念瓶颈模型存在两个关键限制：1) 预训练偏差问题，表现为粒度不对齐或过度依赖结构先验；2) 使用二元交叉熵损失时独立处理每个概念，忽略了概念间的互斥性，导致对齐效果不佳。

Method: 提出CoAt-CBM框架：1) 使用可学习的概念级视觉查询自适应获取细粒度概念级视觉嵌入；2) 生成概念得分向量；3) 引入新的概念对比优化方法，指导模型处理概念得分的相对重要性，使概念预测能忠实反映图像内容并改善对齐。

Result: 大量实验表明，CoAt-CBM在性能上持续优于最先进的方法，实现了更好的图像-概念对齐。

Conclusion: CoAt-CBM通过自适应细粒度概念对齐和概念对比优化，有效解决了现有概念瓶颈模型的局限性，提高了模型的解释性和对齐效果。
...
<details>
  <summary>Details</summary>
Motivation: 现有语义表示难以同时满足语言对齐、空间精确和可扩展性的要求，将像素级语义密集传播到3D空间会产生大量冗余，导致大规模场景中存储和查询效率低下。

Method: 提出PLAF框架，首先在2D图像中提取密集且准确的像素级语言对齐特征，保持开放词汇表达能力；然后设计高效的语义存储和查询方案，显著减少2D和3D领域的冗余。

Result: 实验结果表明PLAF为准确高效的开放词汇3D场景理解提供了强大的语义基础，代码已公开。

Conclusion: PLAF通过像素级语言对齐特征提取和高效语义存储方案，解决了现有3D场景理解中语义表示的局限性，实现了准确且高效的开放词汇3D场景理解。
...
TL;DR: 提出基于课程学习启发的shuffle反馈学习方法，利用图像级标签生成更高质量的组织病理学伪语义分割掩码


<details>
  <summary>Details</summary>
Motivation: 组织病理学分割需要像素级标注，但获取成本高。使用图像级分类标签可以扩大数据规模，但现有CAM方法只能识别小区域，无法生成足够的伪掩码用于分割
...
Conclusion: 通过shuffle反馈学习方法，能够利用图像级标签有效生成高质量伪分割掩码，解决了病理图像分割中标注数据稀缺的问题
...
TL;DR: SGMRI-VQA是一个包含41,307个问答对的医学视觉语言模型基准，专注于多帧MRI空间推理和视觉定位，要求模型在检测、定位、计数/分类和描述任务中进行透明推理。


<details>
  <summary>Details</summary>
Motivation: 当前医学视觉语言模型缺乏透明推理和空间证据，现有基准主要评估2D图像，忽略了临床影像的体素特性，其中发现可能跨越多个帧或仅出现在少数切片上。

Method: 基于fastMRI+数据集的专家放射科医生标注，构建了包含临床对齐思维链追踪和帧索引边界框坐标的问答对基准，任务按检测、定位、计数/分类和描述分层组织。

Result: 对10个视觉语言模型进行基准测试，发现Qwen3-VL-8B通过边界框监督的监督微调相比强大的零样本基线能持续提升定位性能，表明有针对性的空间监督是通向临床推理的有效路径。

Conclusion: SGMRI-VQA基准填补了医学视觉语言模型在空间推理和视觉定位方面的空白，展示了空间监督对提升临床推理能力的重要性，为开发更透明、可解释的医学AI系统提供了方向。
...
TL;DR: 该论文提出了一种基于令牌动态的方法来改善视觉语言模型的信息流，通过识别重要的视觉令牌并减少无关视觉令牌的干扰，从而提升模型的感知能力。


<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型能够捕捉到与问题相关的正确图像区域，但常常无法产生正确答案。研究发现这种错位可能源于视觉语言模型中信息流的次优化，即文本令牌对无关视觉令牌分配了过多注意力，导致错误答案。

Method: 提出基于令牌动态的方法来确定视觉令牌的重要性：在不同解码阶段表现出不同激活模式的视觉令牌被视为重要令牌。通过调制推理过程中的信息流，确保文本令牌仅与重要的视觉令牌关联，消除无关区域的干扰。

Result: 该方法在多种代表性开源视觉语言模型上进行了评估，涵盖视觉问答、视觉定位与计数、光学字符识别和物体幻觉等多个数据集。结果显示该方法显著提升了基线模型的性能。

Conclusion: 通过调制视觉语言模型中的信息流，减少文本令牌对无关视觉令牌的注意力分配，可以有效提升模型的感知能力，解决模型能够定位正确区域却产生错误答案的问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习标定模型在适应开放世界新场景时存在灾难性遗忘问题，而简单的回放策略无法有效缓解这一挑战。需要一种能够持续适应序列化开放世界操作场景的标定方法。

Method: 提出持续手眼标定框架，包含两个核心组件：1) 空间感知回放策略(SARS)，构建几何均匀的回放缓冲区，用信息量最大的视角替换冗余相邻帧；2) 结构保持双重蒸馏(SPDD)，将定位知识分解为粗略场景布局和精细姿态精度，分别进行蒸馏以缓解两类遗忘。

Result: 在多个公开数据集上的实验表明，该方法具有显著的抗场景遗忘性能，能够在保持对新场景适应能力的同时，维持对过去场景的标定精度。

Conclusion: 该框架通过空间感知回放和结构化蒸馏，有效解决了手眼标定中的持续学习问题，使机器人能够在开放世界环境中持续积累多场景标定能力。
...
TL;DR: 首个针对自我中心屏幕视角电影情感理解的数据集ESE，解决了传统电影情感分析在机器人现实观看场景中的领域迁移问题。


<details>
  <summary>Details</summary>
Motivation: 现有电影情感理解研究主要基于电影原片，而机器人等实体智能体通常通过自我中心屏幕视角感知电影，存在视角扭曲、尺度变化、光照变化和环境干扰等域偏移问题，限制了跨域泛化能力。

Method: 构建了ESE数据集，包含224个在受控条件下捕获的自我中心屏幕视角电影预告片，28,667个时间对齐的关键帧，采用置信度感知的多标签标注协议处理情感模糊性。建立了多模态长上下文情感推理框架，建模时序视觉证据、叙事摘要、压缩历史上下文和音频线索。

Result: 实验显示严重的领域差距：在电影原片上训练的模型在现实自我中心屏幕视角观察中Macro-F1从27.99降至16.69。在ESE上训练显著提高了现实观看条件下的鲁棒性。该方法与强大的闭源多模态模型相比具有竞争力。

Conclusion: 领域特定数据和长上下文多模态推理对现实世界电影情感理解至关重要，ESE数据集填补了现有研究空白，为实体智能体的情感理解提供了重要基准。
...
TL;DR: SSFT是一个轻量级光谱-空间融合Transformer模型，通过分解表示学习为光谱和空间两条路径，利用交叉注意力整合互补信息，在HSI-Benchmark上以不到2%的参数实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 高光谱成像虽然能捕捉丰富的光谱特征，但由于高维度、光谱冗余、标记数据有限和强域偏移等问题，学习鲁棒分类器具有挑战性。特别是在地球观测之外，标记的高光谱数据往往稀缺且不平衡，需要紧凑的通用模型来适应不同的采集条件。

Method: 提出轻量级光谱-空间融合Transformer（SSFT），将表示学习分解为光谱和空间两条路径，通过交叉注意力整合它们以捕捉互补的波长依赖信息和结构信息。

Result: 在异构多数据集基准HSI-Benchmark上取得最先进的整体性能，排名第一，同时使用的参数不到先前领先方法的2%。在更大的SpectralEarth基准上，SSFT尽管尺寸紧凑仍保持竞争力。消融研究表明光谱和空间路径都至关重要，其中空间建模贡献最大，且SSFT在没有数据增强的情况下仍保持鲁棒性。

Conclusion: SSFT通过分解表示学习和交叉注意力机制，实现了高效的光谱-空间特征融合，为高光谱分类提供了一个参数高效且性能优越的解决方案，能够适应多样化的采集条件和应用场景。
...
TL;DR: TICoE是一个文本-图像协同擦除框架，通过连续凸概念流形和分层视觉表示学习，实现精确的概念移除，同时保留无关内容，提升文本到图像生成的安全性和可控性。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像生成模型由于大规模训练数据集的隐含偏见，可能产生不安全或不期望的内容。现有的概念擦除方法存在权衡：文本方法通常无法完全抑制概念，而简单的图像引导方法可能过度擦除无关内容。

Method: 提出TICoE（文本-图像协同擦除）框架，采用连续凸概念流形和分层视觉表示学习，实现精确且忠实的概念移除。该方法通过文本和图像的协同作用，在移除目标概念的同时保持无关的语义和视觉内容。

Result: 在多个基准测试上的实验表明，TICoE在概念移除精度和内容保真度方面超越了先前的方法。作者还引入了面向保真度的评估策略来客观衡量擦除后的可用性。

Conclusion: TICoE框架能够实现更安全、更可控的文本到图像生成，通过精确的概念擦除同时保持内容保真度，为生成模型的安全应用提供了有效解决方案。
...
TL;DR: CBC-SLP是一个多模态遥感语义分割模型，通过结构化潜在投影方法同时保留模态不变和模态特定信息，在完整和缺失模态情况下均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有多模态分割模型通过学习共享表示处理缺失模态，但这种方法会损害模态特定的互补信息，降低所有模态可用时的性能。需要一种既能有效利用互补信息，又能在随机模态缺失时保持鲁棒性的方法。

Method: 提出结构化潜在投影方法，将潜在表示结构化为共享和模态特定组件，并根据随机模态可用性掩码自适应地将这些信息传输到解码器。这种方法作为架构归纳偏置直接融入模型结构，而非通过损失项强制实现。

Result: 在三个多模态遥感图像数据集上的实验表明，CBC-SLP在完整和缺失模态场景下均优于最先进的多模态模型。实证证明该方法能够恢复共享表示中可能无法保留的互补信息。

Conclusion: CBC-SLP通过结构化潜在投影有效解决了多模态语义分割中模态缺失问题，同时保留了模态不变和模态特定信息，在多种场景下均表现出优越性能。
...
TL;DR: 提出Adaptive Head Synthesis (AHS)方法，解决现有头像替换技术在全身图像、多样头部姿态和表情方面的局限性，通过头部重演合成数据增强策略实现更好的真实世界应用效果。


<details>
  <summary>Details</summary>
Motivation: 当前头像替换技术主要依赖面部裁剪数据，视角有限，难以处理多样头部表情、发型变化以及面部区域外的自然融合，限制了其真实世界应用。

Method: 提出Adaptive Head Synthesis (AHS)方法，处理完整上半身图像，包含多样头部姿态和表情。采用新颖的头部重演合成数据增强策略，克服自监督训练限制，无需配对训练数据即可增强对不同面部表情和方向的泛化能力。

Result: AHS在挑战性真实场景中表现优异，产生视觉一致的结果，在各种头部方向和发型下保持身份和表情保真度。特别在剧烈表情变化时保持面部身份，以及在显著头部姿态变化时忠实保留配饰方面表现出色。

Conclusion: AHS方法有效解决了现有头像替换技术的局限性，通过自适应头部合成和数据增强策略，实现了在真实世界场景中更鲁棒、更自然的头像替换效果。
...
TL;DR: Splats in Splats++ 是一个统一的隐写框架，可在3D高斯泼溅表示中嵌入高容量3D/4D内容，实现不可感知的版权保护，同时保持渲染效率和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 随着3D高斯泼溅技术的普及，保护3DGS资产的版权变得至关重要。现有的隐写框架难以平衡安全高容量数据嵌入与资产实用性，常常破坏原生渲染流程或易受结构扰动攻击。

Method: 基于球谐函数频率分布分析，提出重要性分级的SH系数加密方案；引入哈希网格引导的不透明度映射机制，结合梯度门控不透明度一致性损失，强制原始场景与隐藏场景之间的严格空间属性耦合。

Result: 方法显著优于现有方法，达到6.28 db更高的消息保真度，3倍更快的渲染速度，对GSPure等3D目标结构攻击具有卓越鲁棒性，并能泛化到2D图像嵌入、4D动态场景隐写等下游任务。

Conclusion: Splats in Splats++ 提供了一个管道无关的隐写框架，可在3DGS表示中无缝嵌入高容量内容，实现不可感知的版权保护，同时保持渲染效率和对抗攻击的鲁棒性。
...
Method: 提出UniEditBench统一基准，包含9种图像操作和8种视频操作的结构化分类；通过蒸馏高容量MLLM（Qwen3-VL-235B-A22B Instruct）构建轻量级4B/8B评估器，从结构保真度、文本对齐、背景一致性、自然度、时空一致性等多个维度进行评分。

Result: 蒸馏后的评估器与人类判断保持高度一致，同时相对于教师模型大幅降低了部署成本。基准测试为现代视觉编辑方法提供了实用且可复现的评估协议。

Conclusion: UniEditBench解决了视觉编辑评估的碎片化问题，通过统一的基准和轻量级评估器实现了跨范式公平比较，为视觉编辑领域提供了标准化的评估框架。
...
TL;DR: PolarMAE：针对超声图像的预训练框架，通过渐进视觉语义筛选、声学边界区域约束和极坐标纹理协同掩码，解决超声图像的数据冗余、扇形局部性和极坐标波束形成特性，在胎儿超声解释任务中实现SOTA性能。
...
Conclusion: PolarMAE是针对超声图像特性量身定制的高效预训练框架，通过解决超声特有的挑战，在胎儿超声智能解释任务中表现出卓越性能，为产前诊断提供了有效的无监督学习解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 视频扩散模型已成为高质量视频生成的主流方法，但其实际部署受到严重推理成本的限制。与图像生成相比，视频合成在时空token增长和迭代去噪方面计算量倍增，使得注意力机制和内存流量成为实际应用中的主要瓶颈。

Method: 提出了一个统一的分类框架，将现有方法组织为四种主要范式：步骤蒸馏、高效注意力、模型压缩和缓存/轨迹优化。基于这一分类，分别分析了这四种范式的算法趋势，并研究了不同设计选择如何针对两个核心目标：减少函数评估次数和最小化单步开销。

Result: 这是关于高效视频扩散模型的首次全面综述，为研究人员和工程师提供了该领域的结构化概述和新兴研究方向。论文系统性地回顾了现有方法，并提出了统一的分类框架。

Conclusion: 讨论了开放挑战和未来方向，包括复合加速下的质量保持、硬件-软件协同设计、鲁棒的实时长序列生成，以及标准化评估的开放基础设施。该工作为高效视频扩散模型领域提供了系统性的研究框架。
...
TL;DR: 该论文提出神经Gabor溅射方法，通过为每个高斯基元添加轻量级MLP来建模单个基元内的颜色变化，结合频率感知的密集化策略控制基元数量，以解决3DGS在处理高频细节时基元数量激增的问题。


<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射(3DGS)在3D重建和新视角合成方面表现出色，具有训练快速、实时渲染和便于后处理等优点。但存在一个关键缺陷：对于具有高频外观细节的场景，基元数量会急剧增加，因为每个基元只能表示单一颜色，每个尖锐的颜色过渡都需要多个基元。

Method: 1. 神经Gabor溅射：为每个高斯基元添加轻量级多层感知机(MLP)，使单个基元能够建模广泛范围内的颜色变化。2. 频率感知密集化策略：基于频率能量选择不匹配的基元进行剪枝和克隆，以控制基元数量。

Result: 该方法能够准确重建具有挑战性的高频表面。在标准基准测试（如Mip-NeRF360）和高频数据集（如棋盘格图案）上进行了广泛实验，并通过全面的消融研究验证了其有效性。

Conclusion: 提出的神经Gabor溅射方法通过增强单个基元的表达能力，有效解决了3DGS在处理高频细节时基元数量激增的问题，实现了对高频表面的准确重建。
...
TL;DR: SENSE是首个基于立体视觉的开放词汇语义分割方法，通过结合立体图像对和几何线索，显著提升了开放词汇语义分割的空间精度和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇语义分割方法通常依赖单视角图像，在遮挡和物体边界附近的空间精度不足，特别是在动态环境中需要更灵活的语义理解能力。

Method: 提出SENSE方法，利用立体视觉和视觉语言模型，通过立体图像对引入几何线索，在PhraseStereo数据集上进行训练，结合语义和几何推理。

Result: 在PhraseStereo数据集上比基线方法提升2.9%平均精度，比最佳竞争方法提升0.76%；在Cityscapes上相对提升3.5% mIoU，在KITTI上相对提升18%。

Conclusion: SENSE通过联合语义和几何推理，实现了从自然语言到准确场景理解的能力，对自主机器人和智能交通系统具有重要意义。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本引导图像编辑方法通常采用竞争范式，编辑和重建分支各自独立优化目标，导致语义冲突和不可预测结果，缺乏分支间的协调。

Method: 提出CoEdit框架：1) 空间上引入双熵注意力操纵，量化分支间的方向熵相互作用，将注意力控制重新表述为和谐最大化问题；2) 时间上提出熵潜在精炼机制，动态调整潜在表示；3) 提出保真度约束编辑评分作为复合评估指标。

Result: 在标准基准测试中，CoEdit在编辑质量和结构保持方面均表现出优越性能，能够更有效地定位可编辑和可保留区域，减少累积编辑错误。

Conclusion: CoEdit通过将注意力控制从竞争转变为协同竞争谈判，实现了空间和时间维度的编辑和谐，增强了多媒体信息利用，促进了视觉和文本模态间更有效的交互。
...
TL;DR: MMGait是一个综合多模态步态识别基准，整合了5种异构传感器的12种模态数据，包含725个对象的334,060个序列，支持单模态、跨模态和多模态评估，并提出了统一这三种范式的Omni多模态步态识别任务及基线模型OmniGait。


<details>
  <summary>Details</summary>
Motivation: 现有步态识别方法主要依赖RGB模态，在需要多模态协作和跨模态检索的真实场景中存在局限性，需要更全面的多模态基准来推动该领域发展。

Method: 提出了MMGait多模态步态基准，整合了RGB相机、深度相机、红外相机、LiDAR扫描仪和4D雷达系统五种传感器的数据；提出了Omni多模态步态识别任务，旨在统一单模态、跨模态和多模态三种范式；提出了OmniGait基线模型，学习跨不同模态的共享嵌入空间。

Result: MMGait基准包含12种模态和334,060个序列，来自725个对象；OmniGait模型在多模态步态识别中表现出有前景的性能；所有资源（基准、代码库、预训练检查点）均已公开。

Conclusion: MMGait为多模态步态识别研究提供了全面的基准，OmniGait模型展示了统一不同步态识别范式的可行性，有助于推动步态识别在真实复杂场景中的应用。
...
TL;DR: 本文提出IA-CLAHE方法，通过自适应估计每个局部区域的对比度限制参数，解决传统CLAHE的过增强问题，无需特定任务训练数据即可实现零样本泛化。


<details>
  <summary>Details</summary>
Motivation: 传统CLAHE在计算机视觉任务中被广泛使用，但其固定的对比度限制参数clip limit不考虑每个局部区域的直方图分布，常常导致过增强问题。

Method: 提出IA-CLAHE方法，训练一个轻量级的clip limits估计器，通过CLAHE的可微分扩展实现端到端优化。该方法学习将输入图像直方图映射到域不变均匀分布，无需预搜索真实clip limits或特定任务数据集。

Result: 实验结果表明，IA-CLAHE在提升识别性能的同时，也改善了人类感知的视觉质量，且不需要任何特定任务的训练数据。

Conclusion: IA-CLAHE通过自适应估计局部区域的clip limits，有效解决了传统CLAHE的过增强问题，实现了零样本泛化，在提升计算机视觉任务性能的同时改善了视觉质量。
...
TL;DR: 开发了一个名为Breakout-picker的深度学习框架，通过结合负样本训练和方位对称性验证，显著降低井壁崩落自动识别的误报率。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在井壁崩落自动识别中存在高误报率问题，经常将非崩落特征（如天然裂缝、键槽、测井伪影）错误分类为崩落，影响地应力分析的准确性。

Method: 提出Breakout-picker框架，采用两种策略降低误报：1）在训练中引入非崩落特征的负样本（天然裂缝、键槽、测井伪影）；2）对候选崩落进行方位对称性验证，排除不符合对称特征的结果。
...
TL;DR: 该研究首次全面评估了13种可解释AI方法在头颈癌预后预测中的表现，使用24个指标从忠实性、鲁棒性、复杂性和合理性四个维度进行评估，发现Integrated Gradients和DeepLIFT在多个维度表现最佳。


<details>
  <summary>Details</summary>
Motivation: 虽然AI技术在头颈癌预后预测中显示出潜力，但AI模型的可解释性不足阻碍了其临床应用。以往研究通常经验性地选择可解释AI方法，缺乏系统性的评估和比较。

Method: 研究在HECKTOR挑战赛的多中心数据集上，首次全面评估了13种可解释AI方法，使用24个评估指标覆盖四个维度：忠实性、鲁棒性、复杂性和合理性。

Result: 实验结果显示不同可解释AI方法在评估维度上存在显著差异，其中Integrated Gradients和DeepLIFT在忠实性、复杂性和合理性方面持续获得高排名。

Conclusion: 该研究强调了全面评估可解释AI方法的重要性，为头颈癌预后预测的临床应用提供了指导，并可扩展到其他医学影像任务中。
...
TL;DR: 本文发现扩散概率模型存在SNR-t偏差问题，即推理阶段去噪样本的信噪比与其对应时间步长不匹配，导致误差累积影响生成质量。作者提出了一种简单有效的差分校正方法来解决这一问题。


<details>
  <summary>Details</summary>
Motivation: 扩散概率模型在各种生成任务中表现出色，但作者观察到这些模型普遍存在SNR-t偏差问题。在训练阶段，样本的信噪比与其时间步长严格耦合，但在推理阶段这种对应关系被破坏，导致误差累积并损害生成质量。
...
Conclusion: SNR-t偏差是扩散模型中的一个重要问题，提出的差分校正方法能有效缓解这一问题，显著提升生成质量，为扩散模型的改进提供了新思路。
...
TL;DR: Mind's Eye基准测试评估多模态大语言模型的视觉认知能力，发现其在视觉空间推理方面显著落后于人类表现


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在视觉语言基准测试上取得了显著进展，但其视觉认知和视觉空间推理能力仍未被充分理解。研究者希望创建一个系统性的评估框架来深入探究这些模型的视觉推理能力

Method: 研究者开发了"Mind's Eye"基准测试，包含8个视觉认知任务，采用选择题形式，基于A-R-T分类法（抽象、关系、转换）。这些任务灵感来源于经典人类智力测试，评估流体智力的核心过程，如模式归纳、类比关系映射和心理转换。评估了多种闭源和开源MLLM，并与人类参与者表现进行对比

Result: 人类参与者平均准确率达到80%，而表现最好的MLLM准确率仍低于50%。错误分析揭示了三个主要失败原因：视觉注意力分配不足、内部感知操作能力有限、对底层视觉概念的抽象能力较弱

Conclusion: 当前多模态大语言模型在视觉空间推理能力方面存在显著局限，与人类表现差距明显。研究强调了需要更基于认知科学的评估框架来推动MLLM在视觉推理方面的发展
...
TL;DR: 研究发现多模态推理模型在空间智能任务中存在严重缺陷，CoT提示会降低视觉空间推理性能，模型存在捷径学习和幻觉问题


<details>
  <summary>Details</summary>
Motivation: 尽管基于思维链的多模态推理模型在数学和逻辑问题解决方面取得了革命性进展，但作者发现这些模型在广义空间智能方面存在严重不足，需要系统评估其空间推理能力

Method: 对17个模型在13个空间基准测试上进行全面评估，使用CoT提示方法，并通过创新的No-Image++消融实验来检测模型的捷径学习和幻觉问题

Result: CoT提示在视觉空间推理中持续降低模型性能，多模态推理模型和CoT提示的MLMs存在严重的捷径学习问题，即使图像缺失也会从文本先验中幻觉出视觉细节

Conclusion: 文本仅CoT方法在空间任务中效果有限，挑战了现有方法的有效性，强调需要开发以视觉为中心的新型推理范式
...
TL;DR: TableSeq是一个端到端的图像表格识别框架，将表格结构识别、内容识别和单元格定位统一为单一序列生成任务，通过HTML标签、文本内容和坐标标记的混合输出实现一体化处理。


<details>
  <summary>Details</summary>
Motivation: 传统表格识别方法通常需要外部OCR、多个解码器或复杂的后处理流程，导致系统复杂且效率低下。TableSeq旨在通过统一的序列生成框架简化这一过程，避免这些额外组件。

Method: TableSeq采用轻量级高分辨率FCN-H16编码器，结合最小化结构先验头和单层Transformer编码器，将表格识别任务转化为单一序列生成问题，输出包含HTML标签、单元格文本和离散化坐标标记的混合序列。

Result: 在多个标准基准测试中取得优异性能：PubTabNet上达到95.23 TEDS/96.83 S-TEDS，FinTabNet上达到97.45 TEDS/98.69 S-TEDS，SciTSR上CAR协议下达到99.79/99.54/99.66的精确率/召回率/F1分数。在PubTables-1M的GriTS评估中也保持竞争力。

Conclusion: TableSeq提供了一个实用且可复现的单流基线框架，统一了表格识别任务，避免了复杂的外部组件。该框架还能扩展到基于索引的表格查询任务，展示了良好的泛化能力。多标记预测技术进一步提升了推理速度。
...
TL;DR: 流匹配模型在数据剪枝、架构变更和训练配置变化下表现出惊人的稳定性，即使剪掉50%的数据集，生成样本的质量和多样性仍能保持，潜在表示也基本不变。
...
Result: 流匹配模型表现出极强的稳定性：即使剪掉50%的数据集，生成样本的质量和多样性仍能保持；潜在表示变化微小，相同种子在不同训练条件下生成视觉相似的输出；架构和训练配置变化下模型同样保持稳定。

Conclusion: 流匹配模型在各种扰动下具有惊人的稳定性，这有助于解释其在实践中的可靠性，并表明其成功不完全依赖于特定架构或大规模数据集。
...
TL;DR: 使用YOLOv12深度学习模型对急性髓系白血病(AML)细胞进行多分类，通过细胞和细胞核两种分割方法，结合Hue通道和Otsu阈值技术预处理图像，最终在细胞分割结合Otsu阈值的方法上取得了99.3%的验证和测试准确率。


<details>
  <summary>Details</summary>
Motivation: 急性髓系白血病(AML)是最具生命威胁的血液癌症之一，由于不同类型细胞之间的视觉相似性，其准确分类一直是一个具有挑战性的任务。本研究旨在解决AML细胞的多类别分类问题。

Method: 采用YOLOv12深度学习模型进行AML细胞分类。使用了两种分割方法：基于细胞特征和基于细胞核特征。在分类前，应用Hue通道和Otsu阈值技术对图像进行预处理。

Result: 实验结果表明，在细胞分割基础上应用Otsu阈值处理的YOLOv12模型取得了最佳性能，验证准确率和测试准确率均达到99.3%。

Conclusion: YOLOv12模型结合适当的图像预处理技术（特别是细胞分割和Otsu阈值处理）能够有效解决AML细胞多分类问题，达到很高的分类准确率，为AML的准确诊断提供了有前景的技术方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统自监督学习方法（如MoCo、DINO）通过捕捉对图像变换不敏感的特征来获得鲁棒表示，这在识别独立于外观的对象时有效。但在天气分析、自动驾驶等应用中，外观本身（如雨痕、雪粒、大气散射）是重要的判别信号，忽略这些信息存在风险。

Method: ST-STORM采用混合自监督学习框架，通过门控机制明确分离两个潜在流：内容分支通过JEPA方案和对比目标追求稳定的语义表示，对外观变化保持不变性；风格分支通过特征预测和重建在对抗约束下捕捉外观特征（纹理、对比度、散射等）。

Result: 在多个任务上评估：风格分支有效隔离复杂外观现象（Multi-Weather数据集F1=97%，ISIC 2024挑战赛F1=94%且仅用10%标注数据），同时不降低内容分支的语义性能（ImageNet-1K F1=80%），并改善了关键外观特征的保留。

Conclusion: ST-STORM成功将外观作为语义模态解耦，在需要外观信息的关键应用中提供了有效的解决方案，既保持了传统自监督学习的语义表示能力，又增强了对外观特征的捕捉能力。
...
TL;DR: DenTab数据集包含2000张牙科估算表图像，用于评估表格识别和视觉问答，发现现有模型在算术推理方面存在不足，提出了Table Router Pipeline来改进算术可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有表格识别和视觉问答数据集大多基于干净的数字源或渲染表格，无法充分反映实际行政文档中的噪声条件，需要更贴近真实应用场景的评估资源。

Method: 引入DenTab数据集（2000张牙科估算表图像，带高质量HTML标注），评估16个系统（14个视觉语言模型和2个OCR基线），并提出Table Router Pipeline结合VLM和基于规则的执行器来改进算术推理。

Result: 实验显示，即使使用真实HTML表格输入，现有模型在多步算术和一致性问题上仍存在推理失败；Table Router Pipeline能在不训练的情况下显著提高算术可靠性。

Conclusion: DenTab数据集填补了真实噪声条件下表格识别和视觉问答评估的空白，Table Router Pipeline为解决表格算术推理问题提供了有效途径，强调了结构恢复与可靠推理之间的差距。
...
TL;DR: Polyglot：基于扩散模型的个性化多语言语音驱动面部动画框架，通过联合条件化语言和个体风格，无需预定义标签即可在多语言场景中生成逼真动画。


<details>
  <summary>Details</summary>
Motivation: 现有语音驱动面部动画模型大多基于单语言数据训练，难以适应现实世界的多语言场景。语言影响语音学、节奏、语调和面部表情，同时个体差异也塑造说话风格，但现有方法通常只依赖语言特定或说话人特定的条件化，无法建模二者的交互作用。

Method: 提出Polyglot统一扩散架构，使用转录本嵌入编码语言信息，从参考面部序列提取风格嵌入捕捉个体说话特征。无需预定义语言或说话人标签，通过自监督学习实现跨语言和说话人的泛化。通过联合条件化语言和风格，捕捉节奏、发音和习惯性面部动作等表达特征。

Result: 实验表明在单语言和多语言设置下均取得改进性能，为语音驱动面部动画中的语言和个人风格建模提供了统一框架，生成时间连贯且逼真的动画。

Conclusion: Polyglot通过联合建模语言和个体风格，解决了多语言语音驱动面部动画的挑战，无需预定义标签即可生成逼真动画，为现实世界多语言应用提供了有效解决方案。
...
TL;DR: 提出一种结合生物启发式伪标签和生态先验知识的半监督深度学习方法，用于解决高光谱树种分类中标签有限、光谱混合和生态异质性等挑战，在真实森林数据集上取得5.6%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 高光谱树种分类面临三大挑战：1) 标签数据有限且类别不平衡；2) 光谱混合问题（不同树种的光谱特征重叠）；3) 生态异质性（生态系统间的变异性）。传统方法仅依赖光谱特征，忽略了植被的生物和结构特性（如冠层结构和种间相互作用）。

Method: 提出生物启发的半监督深度学习方法：1) 整合多传感器数据（高光谱成像HSI和机载激光扫描ALS）；2) 在预计算的冠层图上应用生物启发式伪标签，降低训练成本；3) 利用大语言模型从可靠来源自动提取物种共栖生态先验知识，编码为共栖矩阵；4) 将生态先验知识融入伪标签策略，引入专家知识。

Result: 在真实森林数据集上的实验表明，该方法比最佳基准方法提升了5.6%的分类准确率。对共栖先验知识的专家评估显示其准确性很高，差异不超过15%。

Conclusion: 该方法通过整合多传感器数据、生物启发式伪标签和生态先验知识，有效解决了高光谱树种分类的关键挑战，实现了低训练成本下的准确分类，为生态遥感提供了新思路。
...
TL;DR: 提出Motion-Adapter模块，解决文本到动作扩散模型生成复合动作时的灾难性遗忘和注意力崩溃问题，通过解耦交叉注意力图作为结构掩码来生成更忠实连贯的复合动作。


<details>
  <summary>Details</summary>
Motivation: 当前文本到动作扩散模型在生成复合动作（多个并发动作整合为连贯全身序列）时存在两个关键限制：灾难性遗忘（早期动作被后期动作覆盖）和注意力崩溃（交叉注意力机制中特征融合过度）。现有方法依赖过于详细的文本描述、显式身体部位指定或大型语言模型，导致物理结构和运动机制的语义表示不足，无法自然整合如"边走边打招呼"等行为。

Method: 提出Motion-Adapter，一个即插即用模块，通过计算解耦的交叉注意力图来指导文本到动作扩散模型生成复合动作。这些解耦的注意力图在去噪过程中作为结构掩码，避免不同动作特征之间的过度融合和干扰。

Result: 大量实验表明，该方法在各种文本提示下能够一致地生成更忠实和连贯的复合动作，超越了现有最先进的方法。

Conclusion: Motion-Adapter模块有效解决了文本到动作扩散模型在生成复合动作时的关键限制，通过解耦注意力机制实现了更好的动作合成质量，为自然行为整合提供了有效解决方案。
...
TL;DR: 神经CAD-Edit是首个基于专家CAD工程师收集的3D CAD模型编辑基准，通过捕捉专业设计师在CAD软件中的实际操作视频来收集真实编辑请求，而不是依赖文本条件。


<details>
  <summary>Details</summary>
Motivation: 现有3D CAD编辑方法主要依赖文本条件，缺乏真实世界中专业设计师的实际编辑行为数据。需要建立基于真实CAD工程师操作的专业基准来评估和推动3D CAD编辑技术的发展。

Method: 收集10位专业CAD设计师在实际CAD软件中操作时的视频数据，包括他们的语音、指向和绘图行为。将这些真实编辑请求作为基准，用于评估领先的基础模型与人类CAD专家的编辑能力对比。

Result: 在自动指标和人工评估中都发现基础模型与人类CAD专家之间存在巨大性能差距。即使最佳的基础模型（GPT 5.2）在人工接受度测试中也比CAD专家低53%（绝对值）。

Conclusion: 神经CAD-Edit基准揭示了当前3D CAD编辑技术的挑战，为未来3D CAD编辑方法和基础模型的发展提供了坚实的评估基础。
...
TL;DR: 本文提出了一种三阶段渐进式阴影去除方法，在CVPR2026 NTIRE WSRD+挑战赛中获胜。该方法基于OmniSR架构，结合RGB外观、DINOv2语义引导和几何线索，通过收缩约束目标稳定多阶段优化，在多个数据集上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 解决图像阴影去除问题，特别是在多阶段渐进式框架中，如何有效整合多种视觉线索（语义、几何）并稳定训练过程，以在阴影去除挑战中取得更好的性能。

Method: 1. 三阶段渐进式阴影去除管道，基于OmniSR架构，将去阴影视为迭代直接细化过程；2. 结合RGB外观、冻结的DINOv2语义引导以及单目深度和表面法线的几何线索；3. 引入收缩约束目标，确保级联中重建误差非递增以稳定多阶段优化；4. 分阶段训练流程：从WSRD预训练到WSRD+监督，再到WSRD+ 2026适应，使用余弦退火检查点集成。
...
Conclusion: 提出的三阶段渐进式阴影去除方法通过整合多种视觉线索和创新的收缩约束优化策略，在阴影去除任务中取得了最先进的性能，赢得了NTIRE 2026挑战赛，并在多个数据集上表现出色。
...
TL;DR: 提出了一种新颖的饱和感知空间变异性盲图像去模糊框架，专门处理高动态范围和低光照条件下饱和像素带来的挑战


<details>
  <summary>Details</summary>
Motivation: 在高动态范围和低光照条件下，饱和像素会给图像去模糊带来特殊挑战，现有方法在处理饱和区域时容易产生伪影（如振铃效应），需要专门的方法来准确估计饱和区域的真实辐射度

Method: 1) 基于模糊强度和接近饱和程度对图像进行分割；2) 利用预先估计的光扩散函数来减轻杂散光效应；3) 使用暗通道先验准确估计饱和区域的真实辐射度；4) 避免引入振铃等伪影

Result: 在合成和真实世界数据集上的实验评估表明，该框架在各种场景下都能改善去模糊效果，相比最先进的饱和感知方法和通用方法表现出更优越的性能

Conclusion: 该框架具有适应性强、性能优越的特点，能够与现有和新兴的盲图像去模糊技术进行潜在集成，为解决饱和像素相关的去模糊问题提供了有效方案
...
Method: 提出AIFIND方法：1) 设计基于伪影驱动的语义先验生成器，从低级伪影线索建立固定坐标系；2) 通过伪影-探针注意力将语义锚点注入图像编码器，显式约束易变的视觉特征与稳定的语义锚点对齐；3) 自适应决策协调器通过保持语义锚点的角度关系来协调分类器，维持跨任务的几何一致性。

Result: 在多个增量协议上的广泛实验验证了AIFIND的优越性。

Conclusion: AIFIND通过语义锚点实现了对增量人脸伪造检测中特征空间的显式约束，有效解决了特征漂移和灾难性遗忘问题。
...
### [57] [Dental Panoramic Radiograph Analysis Using YOLO26 From Tooth Detection to Disease Diagnosis](https://arxiv.org/abs/2604.16231)
*Khawaja Azfar Asif,Rafaqat Alam Khan*

Main category: cs.CV

TL;DR: 本研究首次应用YOLOv26模型实现全景X光片的自动牙齿检测、FDI编号和牙病分割，在牙齿计数和疾病分割任务上均取得优异性能。
...
Method: 使用Roboflow对DENTEX数据集进行预处理和增强，得到1082张牙齿计数图像和1040张疾病分割图像。在Google Colab上训练5个YOLOv26-seg变体，采用800x800分辨率和迁移学习。

Result: YOLOv26m-seg模型在牙齿计数任务上表现最佳：精确率0.976，召回率0.970，边界框mAP50为0.976，比YOLOv8x基线提升4.9%精确率和3.3% mAP50。YOLOv26l-seg在疾病分割任务上获得边界框mAP50 0.591和掩码mAP50 0.547，阻生牙检测精度最高（0.943）。
...
TL;DR: 提出基于YOLOv8n和RexNet-150的两阶段考试作弊检测系统，准确率达0.95，比基线提升13%，推理时间13.9ms/样本，兼顾技术性能与伦理考量。
...
Method: 采用两阶段框架：1) 使用YOLOv8n模型定位考场图像中的学生；2) 裁剪检测区域，通过微调的RexNet-150模型分类为正常或作弊行为。使用10个独立来源共273,897个样本的数据集进行训练。

Result: 系统达到0.95准确率、0.94召回率、0.96精确率和0.95 F1分数，比视频作弊检测基线准确率0.82提升13%。平均推理时间13.9ms/样本，适合大规模部署。

Conclusion: 该研究为开发实时、可扩展、符合伦理的开源解决方案奠定了基础。系统通过私密方式向学生反馈结果，避免公开羞辱。未来可整合音频数据和连续帧进一步提升准确性。
...
Result: 在三个常用公共数据集上达到最先进性能，显著超越先前工作；跨数据集评估显示良好泛化能力；可视化验证了分量解耦效果

Conclusion: CollideNet通过分层Transformer架构有效处理视频多尺度特性，在碰撞时间预测任务中取得突破性进展，具有良好泛化能力
...
TL;DR: 提出FFR框架：通过冻结的教师模型识别缺失时空依赖并提供证据补丁，学生模型在增强上下文下重新回答，结合GRPO和RIR奖励进行训练，实现视频推理的准确率提升。


<details>
  <summary>Details</summary>
Motivation: 现有视频推理方法存在局限性：基于策略的自我探索受限于模型知识边界，混合重放需要仔细正则化，动态上下文方法需要精心预训练且受限于小模型能力。而大模型在指令跟随和多模态理解方面表现优异，能够为小模型提供更丰富的上下文。

Method: 提出FFR框架：1) 冻结的教师模型识别缺失的时空依赖关系，从原始视频中提取最小证据补丁（时间戳、区域等）；2) 学生在增强上下文下重新回答问题；3) 采用chosen-rollout方案集成到GRPO中进行训练；4) 提出RIR奖励函数，同时优化答案正确性和依赖对齐性。

Result: 在多个相关基准测试中展示了持续的准确率提升和强大的泛化能力。该方法在保持策略探索的同时，沿着因果有意义的方向引导训练，对训练堆栈的改动最小。

Conclusion: FFR框架通过教师模型提供证据补丁的观察级干预，结合GRPO和RIR奖励，有效提升了视频推理性能，实现了更好的准确率和泛化能力，同时保持了训练过程的简洁性。
...
TL;DR: 该论文系统评估了多种最先进的视觉语言模型在仅使用地面视角图像进行国家级别地理定位的零样本性能，发现模型间存在显著差异，揭示了语义推理在粗粒度地理定位中的潜力及当前模型在捕捉细粒度地理线索方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统图像地理定位方法主要依赖基于检索的地点识别或基于几何的视觉定位流程。尽管视觉语言模型在多模态任务中展现出强大的零样本推理能力，但其在地理推断方面的性能尚未得到充分探索。本研究旨在填补这一空白，评估VLMs在国家级别图像地理定位中的表现。

Method: 采用零样本设置，基于提示进行国家预测，不依赖图像匹配、GPS元数据或任务特定训练。在三个地理多样性数据集上测试多个最先进的视觉语言模型，评估其鲁棒性和泛化能力。

Result: 结果显示不同模型之间存在显著差异，表明语义推理在粗粒度地理定位中具有潜力，但当前VLMs在捕捉细粒度地理线索方面存在局限性。这是首次针对现代VLMs在国家级别地理定位任务的集中比较。

Conclusion: 该研究为多模态推理与地理理解交叉领域的未来研究奠定了基础，展示了VLMs在零样本地理定位任务中的潜力与局限，为后续改进提供了基准和方向。
...
TL;DR: 该研究提出了CrossMath基准来评估视觉语言模型的跨模态推理能力，发现当前VLMs主要依赖文本推理而非视觉证据，并通过微调训练集显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在多种下游任务中表现出色，但尚不清楚其优越性能是源于真正的视觉基础推理，还是主要依赖其文本骨干的推理能力。需要系统性地测量这种跨模态推理能力。

Method: 提出了CrossMath基准，为每个问题构建文本、图像和图文三种格式，确保任务相关信息完全一致。通过人类标注验证对齐质量，有效隔离模态特定的推理差异。使用该基准评估最先进的VLMs，并创建CrossMath训练集进行微调。

Result: 评估发现：文本与视觉推理之间存在显著性能差距；VLMs在纯文本输入下表现优异，但加入视觉数据（图文）反而经常降低性能；这表明当前VLMs主要在文本空间进行推理，对视觉证据的依赖有限。微调CrossMath训练集后，所有模态的推理性能都显著提升，并在两个通用视觉推理任务上获得稳健增益。

Conclusion: 当前视觉语言模型主要依赖文本推理能力，而非真正的视觉基础推理。通过精心设计的跨模态对齐训练集进行微调，可以有效提升模型的视觉推理能力，使其更好地利用视觉证据进行推理。
...
TL;DR: MoIR是一种多模态信息路由方法，通过识别信息不足的token并从强模态补充信息，在融合前减少模态间的信息差异，从而缓解多模态模型中的模态主导问题。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型存在模态主导问题，即预测过度依赖单一模态。传统方法主要通过调整注意力分配来解决，但注意力只能决定模型关注哪里，无法补充缺失或模糊的信息。现实世界中不同模态的信息密度和信噪比不同，单纯调整注意力无法解决信息不足的根本问题。
...
Conclusion: 明确修改跨模态信息是缓解多模态推理模型中模态主导问题的有效且互补的策略。MoIR通过信息层面的融合方法，在融合前减少信息差异，为解决模态主导问题提供了新的思路。
...
TL;DR: 该研究提出了VEFX-Dataset（大规模人工标注的视频编辑数据集）、VEFX-Reward（专门用于视频编辑质量评估的奖励模型）和VEFX-Bench（标准化编辑系统比较基准），解决了当前视频编辑领域缺乏大规模标注数据和专业评估工具的问题。


<details>
  <summary>Details</summary>
Motivation: 随着AI辅助视频创作日益实用，指令引导的视频编辑对于精炼生成或拍摄的素材以满足专业要求变得至关重要。然而，该领域仍然缺乏大规模人工标注的完整编辑示例数据集和用于比较编辑系统的标准化评估工具。现有资源受限于规模小、缺少编辑输出或缺乏人工质量标签，而当前评估通常依赖于昂贵的人工检查或非专门针对编辑质量的通用视觉语言模型。

Method: 1. 构建VEFX-Dataset：包含5,049个视频编辑示例，涵盖9个主要编辑类别和32个子类别，每个示例在三个解耦维度上标注：指令遵循、渲染质量和编辑排他性。2. 开发VEFX-Reward：专门用于视频编辑质量评估的奖励模型，联合处理源视频、编辑指令和编辑后的视频，通过序数回归预测每个维度的质量分数。3. 发布VEFX-Bench：包含300个精选视频-提示对的基准，用于标准化比较编辑系统。
...
Conclusion: 该研究通过提供大规模标注数据集、专业评估模型和标准化基准，为指令引导的视频编辑领域建立了重要的基础设施。VEFX-Reward显示出比现有方法更好的人类对齐性，而基准测试结果突显了当前视频编辑系统在多个质量维度上仍需改进的挑战。
...
TL;DR: 论文提出了AnimalHaze3k合成数据集和IncepDehazeGan架构，用于去除野生动物图像中的雾霾，显著提升下游检测任务的性能。


<details>
  <summary>Details</summary>
Motivation: 大气雾霾严重降低了野生动物图像质量，阻碍了计算机视觉在动物检测、追踪和行为分析等保护应用中的效果。

Method: 1) 创建AnimalHaze3k合成数据集，包含3,477张雾霾图像，基于1,159张清晰野生动物照片通过物理模拟管道生成；2) 提出新颖的IncepDehazeGan架构，在GAN框架中结合inception模块和残差跳跃连接。

Result: IncepDehazeGan达到最先进性能（SSIM: 0.8914, PSNR: 20.54, LPIPS: 0.1104），比竞争方法SSIM提高6.27%，PSNR提高10.2%。去雾图像使YOLOv11检测mAP提升112%，IoU提升67%。
...
Method: 提出FineCog-Nav框架，将导航分解为语言处理、感知、注意力、记忆、想象、推理和决策等细粒度模块，每个模块由中等规模的基础模型驱动，配备角色特定提示和结构化输入输出协议

Result: FineCog-Nav在指令遵循、长视野规划和未见环境泛化方面持续优于零样本基线方法

Conclusion: 细粒度认知模块化方法对零样本无人机导航具有有效性，为复杂导航任务提供了可解释且协作的解决方案
...
Method: 1. 将布局生成建模为自回归过程，在3D空间中显式建模物体间的几何关系和物理约束；2. 提出适配的3D扩散模型，整合场景、物体和指令信息；3. 采用双引导自展开蒸馏机制提高效率和空间准确性。

Result: 在LayoutVLM基准测试中，LaviGen实现了优越的3D布局生成性能：物理合理性比现有最佳方法高19%，计算速度提高65%。
