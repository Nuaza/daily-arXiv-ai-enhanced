...
Motivation: NeRF在实现逼真新视角合成方面表现出色，但由于体积渲染过程中需要密集的射线采样，导致计算效率低下。传统方法通常采用均匀采样或简单启发式方法，难以根据场景特性自适应分配采样点。

Method: 提出SAC-NeRF强化学习框架，将采样过程建模为马尔可夫决策过程，RL智能体学习基于场景特性的采样策略。包含三个关键技术组件：1) 提供不确定性估计的高斯混合分布颜色模型；2) 平衡质量、效率和一致性的多组件奖励函数；3) 处理环境非平稳性的两阶段训练策略。

Result: 在Synthetic-NeRF和LLFF数据集上的实验表明，SAC-NeRF能够减少35-48%的采样点，同时保持渲染质量在密集采样基线的0.3-0.8 dB PSNR范围内。

Conclusion: 虽然学习到的策略是场景特定的，且RL框架相比简单启发式方法增加了复杂性，但这项工作证明了数据驱动的采样策略能够发现难以手动设计的有效模式，为NeRF的效率优化提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探索视觉语言模型在帮助盲人和低视力人群导航方面的潜力，评估现有模型在基础视觉技能和实际导航场景中的表现，为开发辅助技术提供指导。
...
Result: 研究结果显示：GPT-4o在所有任务中表现最佳，特别是在空间推理和场景理解方面；开源模型在细微推理和复杂环境适应性方面表现不佳；常见挑战包括在杂乱环境中准确计数困难、空间推理存在偏见、倾向于优先关注物体细节而非空间反馈。

Conclusion: 尽管存在局限性，视觉语言模型在寻路辅助方面仍具有潜力，需要更好地与人类反馈对齐并改进空间推理能力。该研究为开发者提供了关于当前VLM优势和局限性的可操作见解，指导如何有效将VLM集成到辅助技术中，同时解决关键限制以增强可用性。
...
TL;DR: 提出EfficientWSI (eWSI)方法，通过结合参数高效微调(PEFT)和多实例学习(MIL)，实现全切片图像(WSI)的端到端训练，无需昂贵的领域特定预训练。


<details>
  <summary>Details</summary>
Motivation: 全切片图像分析对肿瘤诊断至关重要，但WSI极高分辨率使端到端训练困难。现有方法使用ImageNet预训练的特征提取器缺乏领域特异性，而领域特定预训练又计算昂贵且缺乏任务特异性。

Method: 提出eWSI方法，将参数高效微调(PEFT)与多实例学习(MIL)结合，实现WSI任务的端到端训练。在Camelyon16、TCGA和BRACS数据集上评估七个WSI级任务。

Result: eWSI结合ImageNet特征提取器达到强大分类性能，匹配或超越使用领域特定特征提取器的MIL方法。结合领域特定特征提取器时，在大多数情况下进一步改善性能，显示其能捕捉任务特定信息。
...
TL;DR: MM-SafetyBench++是一个专门评估多模态大语言模型上下文安全性的基准测试，通过为每个不安全图像-文本对创建对应的安全版本来测试模型是否能根据上下文细微差异调整安全行为。EchoSafe是一个无需训练的自反思框架，通过记忆库积累安全洞察来提升上下文感知推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM安全研究主要关注检测和拒绝明显不安全的输入，但忽略了上下文安全性问题。模型需要能够区分看似相似但安全意图截然不同的场景，这种细微的上下文差异识别能力对实际应用至关重要。

Method: 1) 构建MM-SafetyBench++基准：为每个不安全图像-文本对创建安全对应版本，通过最小修改翻转用户意图但保持上下文含义；2) 提出EchoSafe框架：维护自反思记忆库积累安全洞察，在推理时检索相关历史经验集成到当前提示中，实现上下文感知推理和安全行为的持续演化。

Result: 在多个多模态安全基准测试上的广泛实验表明，EchoSafe始终实现卓越性能，为推进MLLM的上下文安全性建立了强有力的基线。所有基准数据和代码均已开源。

Conclusion: 该工作强调了上下文安全评估的重要性，提出的MM-SafetyBench++基准和EchoSafe框架为解决MLLM的上下文安全挑战提供了有效工具，推动了多模态大语言模型安全性的进一步发展。
...
TL;DR: MATCH是一种多视角高斯配准方法，能在0.5秒内从标定多视角图像直接预测对应的高斯溅射纹理，无需数据预处理，使头像创建速度比现有方法快10倍。


<details>
  <summary>Details</summary>
Motivation: 现有最先进的多视角头像方法需要耗时的头部跟踪和昂贵的头像优化，通常需要超过一天的总创建时间。需要一种更快速、更高效的头部头像创建和编辑方法。

Method: 使用基于transformer的端到端模型，在模板网格的固定UV布局中预测高斯溅射纹理。引入新颖的配准引导注意力块，其中每个UV贴图标记仅关注描绘其对应网格区域的图像标记，提高了效率和性能。

Result: MATCH在新视角合成、几何配准和头部头像生成方面优于现有方法，使头像创建速度比最接近的竞争基线快10倍。支持表情转移、免优化跟踪、语义编辑和身份插值等应用。

Conclusion: MATCH通过多视角高斯配准实现了高质量头部头像的快速创建和编辑，显著减少了处理时间，同时保持了高质量结果，为头像生成和编辑提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的端到端多视角多目标跟踪方法虽然精度高，但缺乏原则性的不确定性建模，且与训练配置紧密耦合，限制了在不同传感器布局、模态或数据集上的泛化能力，需要重新训练才能适应新场景。

Method: ModTrack将学习限制在检测和特征提取阶段，将每个传感器的输出简化为校准的位置-协方差对(z,R)；通过跨视角聚类和精度加权融合得到统一估计(ẑ,Ŕ)；使用反馈耦合、身份感知的高斯混合概率假设密度滤波器配合隐马尔可夫运动模式进行身份维护，处理漏检和严重遮挡。

Result: 在WildTrack数据集上达到95.5 IDF1和91.4 MOTA，超过所有先前模块化方法21分以上，与最先进的端到端方法相当；相同的跟踪核心无需修改即可迁移到MultiviewX和RadarScenes数据集，仅需替换感知模块即可扩展到新领域和传感器模态。

Conclusion: ModTrack提供了一种模块化、可泛化的多视角多目标跟踪解决方案，在保持与端到端方法相当性能的同时，提供了跨模态、传感器无关的部署灵活性，并具备可追踪的不确定性建模能力。
...
TL;DR: ConflictAwareAH是一个多模态框架，通过提取视频、音频和文本表示，利用模态间冲突特征来检测矛盾情感状态，显著提升了矛盾情感检测的准确性。


<details>
  <summary>Details</summary>
Motivation: 矛盾情感状态（如说一套做一套）在临床环境中具有重要价值，但机器识别困难，因为关键证据存在于不同模态之间的不一致性中。现有基于文本的方法倾向于过度检测矛盾情感，难以确认其不存在。

Method: 使用三个预训练编码器提取视频、音频和文本表示；通过模态嵌入之间的逐元素绝对差异构建成对冲突特征；采用文本引导的后期融合策略，将纯文本辅助头与完整模型在推理时融合。

Result: 在BAH数据集上，该方法在标记测试集上达到0.694 Macro F1，在私有排行榜上达到0.715，比已发布的多模态基线高出10多个点；仅使用单个GPU训练时间不到25分钟。

Conclusion: ConflictAwareAH通过显式建模模态间冲突特征，有效解决了矛盾情感检测问题，显著提升了性能，特别是在确认矛盾情感不存在方面表现突出。
...
TL;DR: AdaRAG-CT通过自适应检索增强框架解决3D CT报告生成中的病理覆盖不全问题，通过补充文本信息弥补视觉表示的维度瓶颈，在CT-RATE基准上实现最佳临床效果。


<details>
  <summary>Details</summary>
Motivation: 现有3D CT自动报告生成存在病理覆盖不全的问题，研究发现这是由于对比性3D CT嵌入存在严重的维度集中现象（512维中仅有2个有效维度），形成了视觉表示的瓶颈，限制了生成和检索性能。

Method: 提出AdaRAG-CT自适应检索增强框架，通过受控检索引入补充文本信息，并在生成过程中选择性整合这些信息，以补偿视觉表示的瓶颈。

Result: 在CT-RATE基准测试中，AdaRAG-CT将Clinical F1从0.420（CT-Agent）提升到0.480（+6个点），达到最先进的临床效果。消融研究证实检索和生成组件都对改进有贡献。

Conclusion: 3D CT报告生成的主要瓶颈在于视觉表示的维度集中，而非语言模型规模。通过自适应检索增强框架引入补充文本信息可以有效补偿这一瓶颈，显著提升临床效果。
...
TL;DR: FEEL数据集首次将力传感器手套数据与第一人称视角视频配对，包含约300万帧厨房环境中的自然操作数据，45%的帧涉及手-物体接触，用于物理动作理解。


<details>
  <summary>Details</summary>
Motivation: 力是驱动物理交互的根本原因，是物理动作理解的关键基础要素。现有数据集缺乏力测量数据，限制了物理动作理解的研究。

Method: 开发压阻式力传感器手套收集同步的力测量和第一人称视频数据，构建FEEL大规模数据集。应用于两个任务：1) 接触理解（时间接触分割和像素级接触物体分割）；2) 动作表示学习（以力预测作为自监督预训练目标）。

Result: 在时间接触分割任务上达到最先进水平，在像素级分割任务上获得竞争性结果且无需手动标注。在EPIC-Kitchens、SomethingSomething-V2、EgoExo4D和Meccano等数据集上的动作理解任务中，通过FEEL预训练显著提升了迁移性能。

Conclusion: 力测量数据对于物理动作理解至关重要，FEEL数据集为接触理解和动作表示学习提供了有价值的资源，证明了力作为自监督信号的有效性。
...
TL;DR: 提出一个两阶段框架，结合无监督疾病发现与自监督解耦，从3D医学形状中分离病理变化与生理衰老，无需诊断标签即可发现疾病模式并实现可控合成。
...
Method: 提出两阶段框架：第一阶段训练隐式神经模型学习稳定形状嵌入，通过聚类获得伪疾病标签；第二阶段在紧凑变分空间中使用伪疾病标签和真实年龄标签进行解耦，通过结合协方差和监督对比损失的多目标解耦损失强制分离和可控性。

Result: 在ADNI海马体和OAI远端股骨形状上，实现了接近监督方法的性能，在解耦和重建方面优于最先进的无监督基线，同时支持高保真重建、可控合成和基于因子的可解释性。

Conclusion: 该框架能够有效分离3D医学形状中的病理变化与生理衰老，无需诊断标签即可发现疾病模式，为开发可解释的生物标志物和患者分层提供了有效工具。
...
TL;DR: EvoIQA是一个基于遗传编程的完全可解释符号回归框架，能够生成用于图像质量评估的显式、人类可读数学公式，在保持可解释性的同时达到与最先进深度学习模型相当的性能。


<details>
  <summary>Details</summary>
Motivation: 传统图像质量评估（IQA）指标通常处于两个极端：要么是僵化的手工数学模型，要么是缺乏可解释性的"黑盒"深度学习架构。为了弥合这一差距，需要开发既具有高性能又完全可解释的IQA方法。

Method: 提出EvoIQA框架，基于遗传编程进行符号回归，从VSI、VIF、FSIM和HaarPSI等指标中提取丰富的终端集，将结构、色度和信息论退化映射到可观察的数学方程中，生成人类可读的数学公式。

Result: 演化出的GP模型在预测与人类视觉偏好之间表现出强一致性，不仅超越了传统手工指标，而且与DB-CNN等复杂深度学习模型达到性能相当的水平，证明了不需要为了最先进性能而牺牲可解释性。

Conclusion: EvoIQA框架成功解决了IQA领域可解释性与性能之间的权衡问题，通过符号回归生成完全可解释的数学公式，同时达到最先进的性能水平，为可解释AI在图像质量评估中的应用提供了新途径。
...
TL;DR: NAMD是一个预测肺结节进展的多模态扩散模型框架，通过生成1年随访CT图像，结合基线扫描和电子健康记录，实现肺结节恶性预测，性能接近真实随访扫描。
...
Method: 提出NAMD框架，包含结节对齐的潜在空间（潜在距离对应结节属性变化）和基于LLM的控制机制，利用扩散模型生成随访CT图像，结合基线扫描和患者电子健康记录。
...
<details>
  <summary>Details</summary>
Motivation: 胸部CT扫描自动诊断在临床部署中面临两个持续挑战：采集站点间的分布偏移和人口亚组间的性能差异。需要同时解决这两个问题以提高诊断的鲁棒性和公平性。

Method: 结合轻量级MobileViT-XXS切片编码器和两层SliceTransformer聚合器进行体积推理，使用KL正则化的Group DRO优化目标，自适应地加权表现不佳的采集中心和人口亚组，防止组权重崩溃。

Result: 在任务1（COVID-19分类）中，最佳配置达到挑战F1分数0.835，比已发表的最佳挑战结果提升5.9分；在任务2（肺病理识别）中，Group DRO（α=0.5）达到平均每性别宏F1分数0.815，比最佳挑战结果提升11.1个百分点，女性鳞状细胞癌F1分数比Focal Loss基线提升17.4分。

Conclusion: 提出的KL正则化Group DRO框架能有效同时解决多站点分布偏移和人口亚组公平性问题，在COVID-19分类和肺病理识别任务中均显著优于现有方法，特别改善了代表性不足亚组的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大型视觉语言模型剪枝方法通常以统一方式处理不同模态的校准数据，忽视了模态特异性行为，这导致无法准确处理文本和视觉标记在剪枝中的不同行为。

Method: 提出ATV-Pruning方法：1）通过解耦文本和视觉权重系统研究两者对剪枝操作的敏感性；2）自适应构建校准池，包含所有文本标记和部分视觉标记；3）设计层自适应选择策略来识别重要视觉标记。

Result: 研究发现：1）文本通路对剪枝更敏感，应通过文本标记校准；2）视觉通路冗余度高，可承受50%稀疏度。实验表明ATV-Pruning在标准多模态基准测试中优于现有方法。

Conclusion: ATV-Pruning通过考虑文本和视觉通路的非对称行为，实现了更准确的大型视觉语言模型剪枝，为轻量化多模态模型提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 单个自我中心图像通常只能捕捉地板的一小部分，而完整的可遍历性地图对室内导航等应用更有价值。需要解决单视角鸟瞰图地板补全的问题。

Method: 创建了FlatLands数据集，包含来自6个现有数据集的17,656个真实室内场景的270,575个观测，提供对齐的观测、可见性、有效性和地面真实BEV地图。建立了包含分布内和分布外评估协议的基准测试，比较了免训练方法、确定性模型、集成方法和随机生成模型。

Result: 构建了完整的数据集和基准测试框架，能够为不确定性感知的室内地图构建和生成式补全提供严格的测试平台。实现了端到端的单目RGB到地板地图的完整流程。

Conclusion: FlatLands为单视角鸟瞰图地板补全任务提供了全面的数据集和评估基准，支持不确定性感知的室内地图构建和生成式补全研究，特别适用于具身导航应用。
...
TL;DR: ViT-AdaLA：一种将视觉基础模型的先验知识有效迁移到线性注意力ViT的新框架，通过注意力对齐、特征对齐和监督微调三阶段解决线性注意力ViT训练难题


<details>
  <summary>Details</summary>
Motivation: 基于ViT的视觉基础模型在多种视觉任务上表现出色，但存在二次复杂度问题，限制了其在长序列上的可扩展性。现有的线性注意力方法需要从头训练，计算资源消耗大，而针对大语言模型解码器开发的线性化方法在ViT上迁移效果不佳。

Method: 提出ViT-AdaLA三阶段框架：1) 注意力对齐阶段：将普通线性注意力与原始softmax注意力在每个块中对齐，近似softmax注意力的行为；2) 特征对齐阶段：微调线性化ViT，使其最后一层特征与冻结的softmax VFM教师模型对齐，减轻残差近似误差的累积；3) 监督微调阶段：将适应的先验知识通过监督微调迁移到下游任务。

Result: 在分类和分割任务上的大量实验表明，ViT-AdaLA在各种最先进的线性注意力对应方法上表现出有效性和通用性。

Conclusion: ViT-AdaLA框架成功解决了将视觉基础模型先验知识迁移到线性注意力ViT的挑战，通过三阶段对齐方法有效减少了计算复杂度，同时保持了模型性能。
...
TL;DR: 该论文提出了一种新的语义感知上采样方法USU，用于解决传统插值方法在可解释AI归因图上产生的伪影问题，通过质量重分布算子保持归因信号的完整性。


<details>
  <summary>Details</summary>
Motivation: 传统上采样方法（如双线性和双三次插值）是为自然图像设计的，不适用于归因图。这些方法会通过混叠、振铃和边界渗漏系统性地破坏归因信号，产生虚假的高重要性区域，从而误导对模型推理的理解。

Method: 提出了通用语义感知上采样（USU），将归因上采样重新定义为质量重分布问题而非插值问题。该方法使用比率形式的质量重分布算子，可证明地保持归因质量和相对重要性顺序。基于特征归因的公理化传统，形式化了四个忠实上采样的期望特性，并证明了插值方法在结构上违反了其中三个。

Result: 在具有已知归因先验的模型上进行受控实验验证了USU的形式化保证；在ImageNet、CIFAR-10和CUB-200上的评估确认了一致性的忠实性改进，并产生了定性上更优越、语义更连贯的解释。

Conclusion: USU通过将上采样重新定义为由模型推导的语义边界指导的质量重分布问题，解决了传统插值方法在归因图上的根本缺陷，提供了更忠实、语义更连贯的解释，推动了可解释AI中归因方法的可靠性。
...
Method: 引入体积一致的隐式模型，将符号距离函数重建与神经微分同胚流耦合，学习共享的规范模板。使用雅可比行列式和双调和惩罚等体积正则化来抑制局部折叠并促进全局一致的变形。

Result: 在活体胎盘MRI扫描实验中，相比基于表面的隐式基线方法，该方法在几何保真度和体积对齐方面表现更好，产生了解剖学可解释且拓扑一致的扁平化结果，适合群体分析。
...
TL;DR: 提出了一种用于驾驶场景解析的无监督域自适应框架，通过正则化语义特征结构来缩小合成数据到真实数据的域差距，显著提升真实场景下的解析性能。


<details>
  <summary>Details</summary>
Motivation: 驾驶场景解析对自动驾驶至关重要，但像素级标注成本高昂，合成数据成为替代方案。然而，合成数据训练的模型在真实场景中表现不佳，现有域自适应方法主要关注全局特征对齐而忽略了特征空间的语义结构，导致类间语义关系建模不足，限制了模型的泛化能力。

Method: 提出了一种新颖的无监督域自适应框架，通过类特定原型强制实施类间分离和类内紧凑性，增强特征簇的区分性和结构一致性。采用基于熵的噪声过滤策略提高伪标签的可靠性，并使用像素级注意力机制进一步细化特征对齐。

Result: 在代表性基准测试上的广泛实验表明，该方法持续优于最近的最先进方法，验证了语义结构保持对于驾驶场景解析任务中鲁棒的合成到真实自适应的重要性。

Conclusion: 该研究强调了在驾驶场景解析任务中保持语义结构对于实现鲁棒的合成到真实自适应的重要性，提出的框架通过正则化语义特征结构显著提升了真实场景下的解析性能。
...
TL;DR: Interact3D是一个从单图像生成3D组合物体的框架，通过3D引导场景、两阶段组合管道和基于VLM的自主优化来解决遮挡和物体间空间关系保持的挑战。
...
Method: 1. 利用生成先验创建高质量个体资产和统一的3D引导场景；2. 两阶段组合管道：通过全局到局部几何对齐固定主要物体，使用可微SDF优化集成后续几何体；3. 基于VLM的闭环自主优化策略，通过多视角渲染分析、制定纠正提示并迭代自校正。

Result: 实验表明Interact3D成功生成了具有碰撞感知的组合物体，提高了几何保真度并保持了一致的空间关系。

Conclusion: Interact3D框架能够从单图像生成物理上合理的交互式3D组合物体，有效解决了遮挡和物体间空间关系保持的挑战。
...
TL;DR: Parallel-ICL：一种并行处理长上下文演示的推理算法，通过分块并行处理和加权集成，在保持性能的同时显著提升多模态上下文学习的推理速度


<details>
  <summary>Details</summary>
Motivation: 多模态上下文学习中增加演示数量能提升性能，但Transformer的二次计算成本导致推理延迟显著增加，需要在准确性和效率之间取得平衡

Method: 提出Parallel-ICL算法：1）将长演示上下文划分为多个短块；2）并行处理这些块；3）在logit层面使用加权Product-of-Experts集成来近似全上下文输出；4）基于聚类分块最大化块间多样性；5）基于相似性编译上下文按查询相关性加权预测
...
TL;DR: LICA是一个包含155万张多层图形设计组合的大规模数据集，提供层次化组件结构和丰富的元素元数据，支持图形设计的结构化理解和生成研究。


<details>
  <summary>Details</summary>
Motivation: 当前图形设计研究缺乏大规模、结构化的数据集，难以支持对设计层次结构和组件关系的深入理解。现有数据集通常只提供渲染后的图像，缺少设计元素的层次化表示和元数据，限制了结构化生成和编辑任务的研究。

Method: 构建LICA数据集，包含1,550,244个多层图形设计组合，每个设计表示为层次化组合的组件（文本、图像、矢量、组元素），并配有丰富的元素元数据（空间几何、排版属性、透明度、可见性等）。数据集涵盖20个设计类别和971,850个独特模板，还包含27,261个动画布局，标注了组件关键帧和运动参数。

Result: LICA建立了图形设计研究的新范式，支持层感知修复、结构化布局生成、可控设计编辑和时间感知生成建模等任务。通过将设计表示为组合层和关系的系统，支持直接在设计结构而非像素级别操作的模型研究。

Conclusion: LICA数据集为图形设计的结构化理解和生成提供了大规模、多层次的数据支持，开辟了新的研究方向，特别是通过引入图形设计视频作为新的挑战，推动了视觉语言模型在动态设计理解方面的发展。
...
Method: 提出了OneWorld框架，核心是3D统一表示自编码器（3D-URAE），利用预训练的3D基础模型，通过注入外观和提取语义来增强其几何中心特性。引入标记级跨视图对应（CVC）一致性损失来显式强制跨视图结构对齐，并提出流形漂移强制（MDF）来缓解训练-推理暴露偏差，通过混合漂移和原始表示来塑造鲁棒的3D流形。
...
TL;DR: 该研究质疑CLIP类对比语言-图像训练模型在纯图像任务中表现不佳是因为模态内（图像-图像）未对齐的假设，通过理论和实验证明这种未对齐并非主要原因，任务模糊性才是关键因素。


<details>
  <summary>Details</summary>
Motivation: 最近的研究认为CLIP类对比语言-图像训练产生的嵌入在纯图像任务中表现不佳，主要理论是跨模态（语言-图像）对齐损失忽略了模态内（图像-图像）对齐，导致图像间距离校准不良。本研究质疑这一模态内未对齐假设。

Method: 1. 重新审视模态内未对齐假设的理论基础，证明图像嵌入距离不存在所谓的自由度；2. 对经验指标进行分析，比较语言-图像训练模型（CLIP、SigLIP）和图像-图像训练模型（DINO、SigLIP2）；3. 在常用的模态内任务（检索和少样本分类）上进行实验验证。

Result: 1. 理论分析表明图像嵌入距离不存在假设的自由度；2. 经验指标显示语言-图像训练模型和图像-图像训练模型产生相似结果，表明观察到的现象并非语言-图像模型特有的未对齐问题；3. 实验证实解决任务模糊性而非假设的未对齐才是获得最佳结果的关键。

Conclusion: CLIP类模型在纯图像任务中的表现不佳并非由于模态内未对齐，而是任务模糊性问题。这一发现挑战了现有理论，为改进多模态模型在单模态任务上的性能提供了新方向。
...
TL;DR: NanoGS是一个无需训练、轻量级的3D高斯泼溅简化框架，通过局部成对合并减少原语数量，保持渲染质量，降低存储和传输成本。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅技术需要数百万个原语，导致显著的存储和传输成本。现有压缩方法通常依赖GPU密集型后训练优化和校准图像，限制了实际部署。

Method: 将简化问题形式化为稀疏空间图上的局部成对合并。使用质量保持矩匹配来近似一对高斯原语，并通过原始混合与其近似之间的原则性合并成本评估合并质量。限制合并候选到局部邻域并高效选择兼容对。

Result: NanoGS显著减少了原语数量，同时保持了高渲染保真度。该方法可直接在现有高斯泼溅模型上运行，在CPU上高效执行，并保持标准3DGS参数化，可与现有渲染管道无缝集成。

Conclusion: NanoGS为高斯泼溅简化提供了一个高效实用的解决方案，无需训练和GPU密集型优化，便于实际部署。
...
TL;DR: PathGLS是一个用于评估病理学视觉语言模型的无参考评估框架，通过三个维度（Grounding、Logic、Stability）量化模型幻觉和鲁棒性，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在计算病理学中具有巨大潜力，但由于缺乏能够检测细微失败（如幻觉）的可靠自动评估指标，其临床广泛应用受到限制。

Method: 提出PathGLS框架，从三个维度评估病理学VLM：1) Grounding：细粒度视觉-文本对齐；2) Logic：使用自然语言推理的蕴含图一致性；3) Stability：对抗性视觉语义扰动下的输出方差。支持斑块级和全切片图像级分析。

Result: 在多个数据集上验证PathGLS的优越性：在Quilt-1M数据集上，PathGLS对幻觉报告的敏感性下降达40.2%，而BERTScore仅为2.1%。与专家定义的临床错误层次结构验证显示，PathGLS的Spearman秩相关系数ρ=0.71，显著优于基于LLM的方法（Gemini 3.0 Pro：ρ=0.39）。

Conclusion: PathGLS是一个强大的无参考评估指标，能够直接量化幻觉率和领域转移鲁棒性，为在私有临床数据集上基准测试VLM和指导安全部署提供了可靠标准。
...
<details>
  <summary>Details</summary>
Motivation: 当前目标检测器在处理分布外物体时存在严重问题，常常将这些异常物体误判为背景而完全忽略。现有方法通常依赖复杂架构或辅助分支，且缺乏统一处理分布内和分布外物体的框架。

Method: 提出SynOE-OD框架，利用Stable Diffusion等强生成模型和开放词汇目标检测器生成语义上有意义的物体级数据作为训练时的异常样本。通过迁移学习建立强的分布内任务性能，并增强检测模型对分布外物体的检测鲁棒性。

Result: 在已建立的分布外物体检测基准测试中取得了最先进的平均精度，而像GroundingDINO这样的开放词汇检测器在街景中检测分布外物体的零样本性能有限。

Conclusion: SynOE-OD框架能够使单个检测器同时检测分布内和分布外物体，解决了现有方法中分布外物体被静默忽略的问题，为统一处理分布内和分布外物体检测提供了有效方案。
...
TL;DR: QICA框架通过数量感知与空间聚合的协同，解决零样本物体计数中的数量意识不足和空间不敏感问题，在多个数据集上表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有零样本物体计数方法将计数视为粗粒度检索任务，缺乏细粒度数量意识，同时存在空间不敏感和模型适应过程中的特征空间扭曲问题，导致泛化能力下降。

Method: 提出QICA框架，包含协同提示策略(SPS)通过数值条件提示适应视觉和语言编码器；成本聚合解码器(CAD)直接在视觉-文本相似度图上操作，通过空间聚合防止过拟合；以及多级数量对齐损失(L_MQA)确保整个流程的数量一致性。

Result: 在FSC-147数据集上表现出竞争性性能，在CARPK和ShanghaiTech-A的零样本评估中验证了对未见领域的优越泛化能力。

Conclusion: QICA通过数量感知与空间聚合的协同，有效解决了零样本物体计数中的关键挑战，实现了更好的数量意识和跨领域泛化能力。
...
TL;DR: EPOFusion是一种曝光感知的红外与可见光图像融合模型，专门解决过曝光区域信息丢失问题，通过引导模块提取过曝光区域的细粒度红外特征，使用迭代解码器渐进增强融合图像质量。


<details>
  <summary>Details</summary>
Motivation: 实际场景中经常出现过曝光现象，导致关键视觉信息丢失。现有的红外与可见光融合方法在高度明亮区域表现不佳，需要专门针对过曝光问题的解决方案。

Method: 提出EPOFusion曝光感知融合模型：1）引入引导模块帮助编码器从过曝光区域提取细粒度红外特征；2）设计包含多尺度上下文融合模块的迭代解码器，渐进增强融合图像；3）使用自适应损失函数动态约束融合过程，在不同曝光条件下平衡模态信息。

Result: 实验表明EPOFusion优于现有方法，能在过曝光区域保持红外线索，同时在非过曝光区域实现视觉忠实的融合，提升了视觉保真度和下游任务性能。作者还构建了首个红外与可见光过曝光数据集IVOE。

Conclusion: EPOFusion有效解决了红外与可见光融合中的过曝光问题，通过曝光感知的设计在保持红外线索的同时实现高质量融合，为相关应用提供了实用解决方案。
...
Conclusion: DualPrim的加减法设计在不牺牲紧凑性或可微性的情况下增加了表示能力，为3D重建提供了一种结构化且实用的解决方案。
...
TL;DR: 比较三种数据增强方法在类别不平衡场景下的表现：传统变换、FastGAN和LoRA微调的Stable Diffusion 1.5。发现FastGAN在小样本条件下不仅效果差，反而会加剧分类器偏见，而Stable Diffusion表现最佳。


<details>
  <summary>Details</summary>
Motivation: 生成模型常用于解决AI训练中的类别不平衡问题，但它们在低数据条件下的失败模式尚未被充分理解。本研究旨在系统评估不同数据增强策略在细粒度动物分类任务中的表现，特别是在小样本场景下。

Method: 使用Oxford-IIIT Pet数据集，人工制造8个代表性不足的品种。比较三种增强策略：传统图像变换、FastGAN和LoRA微调的Stable Diffusion 1.5。通过特征嵌入分析（t-SNE）研究图像分布特性，并在消费级GPU上运行所有实验。

Result: FastGAN增强在极低训练集规模下不仅效果不佳，反而显著增加分类器偏见（偏见差距增加+20.7%，Cohen's d = +5.03，p = 0.013）。t-SNE分析显示FastGAN为严重少数类生成的图像形成紧密的孤立聚类，表明存在模式崩溃。Stable Diffusion表现最佳，获得最高宏F1分数（0.9125±0.0047），并将偏见差距相对未增强基线降低13.1%。

Conclusion: 在每类20-50个训练图像以下的样本规模边界，GAN增强可能变得有害。Stable Diffusion配合LoRA微调是处理类别不平衡问题的有效方法，而FastGAN在小样本条件下需谨慎使用。研究为生成模型在数据增强中的应用提供了重要指导。
...
<details>
  <summary>Details</summary>
Motivation: 统一多模态模型（UMMs）的视觉生成组件通常依赖于低效的训练范式和稀缺的高质量文本-图像配对数据，这成为主要瓶颈。研究者希望解决这两个问题，提高训练效率和数据利用率。
...
Conclusion: IOMM框架不仅提高了训练效率，还实现了最先进的性能，证明了仅使用未标注图像数据进行视觉生成组件预训练的有效性，为统一多模态模型的训练提供了更高效、数据友好的解决方案。
...
TL;DR: EFF-Grasp：基于流匹配的物理感知灵巧抓取生成框架，将抓取合成重新表述为确定性ODE过程，通过无训练物理感知能量引导实现高效稳定生成


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的灵巧抓取生成方法通常采用随机微分方程（SDE）公式，需要大量去噪步骤且轨迹不稳定，可能导致物理不可行的抓取

Method: 提出EFF-Grasp框架：1）将抓取合成重新表述为确定性常微分方程（ODE）过程，通过平滑概率流实现高效稳定生成；2）引入无训练物理感知能量引导策略，使用适应显式物理能量函数定义能量引导目标分布，通过局部蒙特卡洛近似估计引导项
...
<details>
  <summary>Details</summary>
Motivation: 4D点云视频理解面临两个主要挑战：不同帧率下的时间尺度偏差，以及不规则点云中的分布不确定性。现有CNN或Transformer方法受限于有限的感受野或二次计算复杂度，且忽略了这些隐式失真。

Method: 提出GATS框架，包含两个互补模块：1) 不确定性引导高斯卷积(UGGC)：将局部高斯统计和不确定性感知门控融入点卷积，在密度变化、噪声和遮挡下实现鲁棒的邻域聚合；2) 时间缩放注意力(TSA)：引入可学习缩放因子来归一化时间距离，确保帧划分不变性和跨不同帧率的一致速度估计。

Result: 在主流基准测试中取得显著性能提升：MSR-Action3D（+6.62%准确率）、NTU RGBD（+1.4%准确率）、Synthia4D（+1.8% mIoU）。相比Transformer方法，提供了更高效、更原则性的不变4D点云视频理解范式。

Conclusion: GATS框架通过显式解决分布不一致性和时间尺度偏差，为4D点云视频理解提供了一个统一且鲁棒的解决方案，在准确性、鲁棒性和可扩展性方面优于现有方法。
...
TL;DR: 该论文调查了主要学术期刊对AI生成图像的政策，分析了出版商的关键关切，并提出了使用AI生成科学图像的最佳实践指南。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI能够制作出版质量的科学图像，但学术出版商对此制定了不一致且模糊的政策，需要明确AI生成图像在学术出版中的合规使用方式。

Method: 调查了Nature、Science、Cell Press、Elsevier、PLOS等主要期刊和出版商的AI图像政策，分析其关注点，并基于SciDraw等AI工具的实际案例提出指导原则。

Result: 发现出版商主要担忧可重复性、作者署名和视觉错误信息等问题，但通过适当的披露和质量控制，AI生成图像可以加速科学传播而不损害学术诚信。

Conclusion: AI生成图像在学术出版中具有潜力，但需要制定明确的披露指南和质量控制措施，以确保透明和合规的使用。
...
TL;DR: 提出一种无监督、架构无关的条件化策略，通过预训练的细胞核分割基础模型生成连续细胞概率图作为显式输入先验，结合方差保持正则化项，从常规免疫组化合成多重免疫荧光图像，显著提升细胞核计数保真度和感知质量。


<details>
  <summary>Details</summary>
Motivation: 多重免疫荧光(mIF)技术成本高、流程复杂，限制了临床常规应用。现有的虚拟染色方法虽然能从普通免疫组化(IHC)合成mIF通道，但主要优化像素级保真度，未显式约束细胞核形态，这在病理学中具有重要临床意义，因为细胞核计数、形状或空间排列的细微扭曲会直接影响Ki67增殖指数等量化终点，几个百分点的误差就可能导致治疗相关风险分类的变化。

Method: 引入无监督、架构无关的条件化策略：1) 使用预训练的细胞核分割基础模型生成连续细胞概率图作为显式输入先验；2) 添加方差保持正则化项，匹配局部强度统计以保持合成荧光通道中的细胞级异质性。软先验保留了二值化阈值处理中丢失的梯度级边界信息，提供更丰富的条件化信号而无需任务特定调优。

Result: 在Pix2Pix（U-Net和ResNet生成器）、确定性回归U-Net和条件扩散模型上，使用两个独立数据集进行的对照实验表明，仅通过该方法修改即可在细胞核计数保真度和感知质量方面获得一致改善。

Conclusion: 该方法通过注入细胞核形态先验和保持细胞级异质性，显著提升了从常规免疫组化合成多重免疫荧光图像的质量，特别是细胞核计数的准确性，这对于临床病理量化分析具有重要意义。代码将在接受后公开。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在传统图像理解方面表现出色，但在360°图像感知方面研究不足。360°图像能捕捉整个周围环境，但存在几何失真和复杂空间关系等挑战，需要专门的评估和改进方法。

Method: 提出了360Bench基准测试（包含7K分辨率360°图像和7个子任务的VQA基准），并设计了Free360框架——一种基于场景图的免训练方法，通过模块化推理步骤、自适应球形图像变换和统一图表示来提升360°图像理解能力。

Result: 实验显示现有MLLMs在360°图像感知方面存在不足，而Free360框架能持续提升基础MLLM的性能，为360° VQA任务提供了强大的免训练解决方案。

Conclusion: 该研究填补了MLLMs在360°图像感知领域的空白，提出的360Bench基准和Free360框架为评估和改进360°图像理解能力提供了有效工具，代码和数据集将在接受后公开。
...
TL;DR: KidsNanny是一个两阶段多模态内容审核架构，用于儿童安全保护。第一阶段使用视觉Transformer和物体检测器进行视觉筛查（11.7毫秒），第二阶段通过OCR和7B语言模型进行文本推理（总延迟120毫秒）。在UnsafeBench数据集上表现优于现有基线模型。


<details>
  <summary>Details</summary>
Motivation: 针对儿童安全保护，需要高效的多模态内容审核系统。现有方法要么只关注视觉信息，要么处理速度较慢。需要开发既能处理视觉内容又能理解文本上下文，同时保持低延迟的解决方案。

Method: 采用两阶段架构：第一阶段结合视觉Transformer和物体检测器进行快速视觉筛查，输出文本描述而非原始像素；第二阶段使用OCR提取文本，并应用7B参数的语言模型进行上下文推理。这种设计避免了将原始像素传输到第二阶段，提高了效率。

Result: 第一阶段达到80.27%准确率和85.39% F1分数（11.7毫秒），优于59.01%-77.04%的视觉基线。完整流水线达到81.40%准确率和86.16% F1分数（120毫秒），优于ShieldGemma-2（64.80%准确率，1136毫秒）和LlavaGuard（80.36%准确率，4138毫秒）。在文本依赖型图像上表现尤其出色，达到100%召回率。

Conclusion: KidsNanny展示了基于OCR的文本推理在检测文本嵌入威胁方面的优势，同时保持了较低的延迟。该架构为高效的多模态内容审核提供了有前景的方向，特别是在需要同时处理视觉和文本信息的儿童安全保护场景中。
...
TL;DR: ECHO是一个边缘-云框架，用于人形机器人的语言驱动全身控制。云端扩散模型根据文本指令生成运动参考，边缘强化学习跟踪器在机器人上闭环执行，无需推理时重定向，实现零硬件微调。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够直接从自然语言指令控制人形机器人全身运动的系统，避免传统方法中需要从人体模型重定向到机器人形态的复杂过程，实现高效、安全的实时控制。

Method: 采用边缘-云架构：云端使用基于扩散的文本到运动生成器（1D卷积UNet+CLIP文本特征），生成紧凑的38维机器人原生运动表示；边缘部署强化学习跟踪器（教师-学生范式+证据适应模块），结合形态对称约束和域随机化实现sim-to-real迁移；配备自主跌倒恢复机制。

Result: 在重定向的HumanML3D基准测试中表现优异（FID 0.029，R-Precision Top-1 0.686），在Unitree G1人形机器人上成功执行多样化文本指令，无需硬件微调，保持高运动安全性和轨迹一致性。

Conclusion: ECHO框架成功实现了从自然语言到人形机器人全身控制的端到端管道，通过紧凑的机器人原生表示消除了重定向需求，结合云生成和边缘执行的架构实现了高效、安全的实时控制。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在SVG生成任务中存在泛化能力有限、代码输出路径冗余、缺乏显式推理等问题，需要更系统的方法来提升生成质量。
...
Conclusion: CTRL-S通过思维链强化学习框架，系统提升了SVG生成的推理能力和生成质量，为SVG生成任务提供了有效的解决方案。
...
TL;DR: S-VAM是一种通过单次前向传播实现实时高保真视觉预测的视频动作模型，解决了现有方法无法同时保证实时推理和高保真预测的问题。


<details>
  <summary>Details</summary>
Motivation: 当前视频动作模型要么依赖缓慢的多步视频生成，要么依赖噪声的单步特征提取，无法同时保证实时推理和高保真预测，限制了机器人学习在复杂操作任务中的应用。

Method: 提出S-VAM模型，通过单次前向传播预测连贯的几何和语义表示；采用自蒸馏策略，将多步去噪的结构化生成先验压缩到单步推理中；使用视觉基础模型从扩散模型生成的多步视频中提取教师目标，轻量级解耦器作为学生直接学习从噪声单步特征到这些目标的映射。
...
TL;DR: Leveling3D：一种将前馈3D重建与几何一致生成相结合的新方法，通过几何感知的调平适配器修复外推新视角中的伪影区域，提升3D重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用扩散模型修复损坏的渲染结果，但缺乏几何考虑，无法填充外推视角中的缺失区域。需要一种能够同时进行重建和生成的整体解决方案。

Method: 提出Leveling3D流程，包含几何感知的调平适配器（轻量级技术，对齐扩散模型内部知识与前馈模型的几何先验）、调色板过滤训练策略和测试时掩码细化。通过修复3D表示中欠约束区域导致的外推新视角伪影，提升重建质量。

Result: 在公共数据集上实现了SOTA性能，包括新视角合成和深度估计任务。增强的外推新视角可作为3D高斯溅射的输入，进一步提升3D重建质量。

Conclusion: Leveling3D成功整合了前馈3D重建与几何一致生成，通过几何感知的调平适配器有效修复外推视角中的伪影，实现了同时重建和生成的整体解决方案，在多个任务上达到最先进性能。
...
TL;DR: PureCLIP-Depth是一种完全无提示、无解码器的单目深度估计模型，直接在CLIP嵌入空间中运行，通过概念信息而非几何特征实现深度估计。


<details>
  <summary>Details</summary>
Motivation: 探索一种基于概念信息而非传统几何特征的单目深度估计新方法，利用CLIP嵌入空间中的概念表示来实现深度估计。
...
TL;DR: 提出EDP-SAM和XMask方法，通过生成掩码监督和空间分离学习，实现半监督人群计数与实例分割的统一框架


<details>
  <summary>Details</summary>
Motivation: 传统基于点的标注在密集场景中存在局限性，难以学习细粒度结构语义，需要利用丰富的未标注数据提升性能

Method: 1. 提出EDP-SAM基于最近邻排除圆约束生成掩码监督；2. 提出XMask通过判别性掩码目标强制空间分离；3. 使用高斯平滑和可微分中心采样提升特征连续性和训练稳定性；4. 构建半监督人群计数框架，使用实例掩码先验作为伪标签

Result: 在ShanghaiTech A、UCF-QNRF和JHU++数据集上（使用5%、10%、40%标注数据）实现最先进的半监督分割和计数性能

Conclusion: 提出的端到端模型有效弥合了计数与实例分割之间的差距，在统一框架内实现了优越性能
...
TL;DR: 提出混合框架解决白细胞分类中的类别不平衡问题，整合生成式修复、Transformer集成和生物启发式细化步骤，在WBCBench 2026挑战中取得0.77139的Macro-F1分数
...
Conclusion: 该方法在严重不平衡条件下表现出强大的性能，突显了将生物先验知识整合到深度学习中对血液学图像分析的价值
...
TL;DR: SEVEX框架通过自动化语义探索发现任务级视觉提示，显著提升大型视觉语言模型的感知能力，超越传统工具选择方法。
...
Method: 提出自动化语义探索框架SEVEX，采用基于抽象概念空间的搜索空间、新颖性引导的选择算法和语义反馈驱动的构思过程，通过智能体驱动实验实现高效多样的视觉提示探索。

Result: 在BlindTest和BLINK基准测试中，SEVEX在任务准确性、推理效率、探索效率和探索稳定性方面显著优于基线方法，发现了超越传统工具使用的复杂反直觉视觉策略。

Conclusion: SEVEX框架通过自动化、任务级的视觉提示探索，为增强LVLM感知提供了新范式，能够发现传统方法难以获得的复杂视觉策略，显著提升模型性能。
...
TL;DR: EVPV提出了一种显式视觉前提验证方法，通过视觉清单和约束提取来解耦感知不确定性和逻辑评估，提升视觉语言过程奖励模型的验证准确性。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言过程奖励模型（VL-PRMs）作为黑盒评判器存在感知与推理的纠缠问题，导致系统性误报（奖励幻觉视觉前提）和漏报（惩罚正确接地陈述），影响重排序和错误定位的准确性。

Method: 提出显式视觉前提验证（EVPV）框架：1）策略生成步骤级视觉清单，明确所需视觉事实；2）约束提取器从输入图像独立推导结构化视觉约束；3）将清单声明与约束匹配计算标量视觉可靠性信号；4）通过可靠性门控校准PRM步骤奖励，在可靠性低时衰减视觉依赖步骤的奖励，可靠性高时保留奖励。

Result: 在VisualProcessBench和六个多模态推理基准测试中，EVPV显著提升了步骤级验证质量，并持续超越强基线模型的最佳N重排序准确率。通过向提取约束注入受控损坏，观察到性能单调下降，为约束保真度和显式前提验证提供了因果证据。

Conclusion: EVPV通过显式验证视觉前提，有效解耦了感知不确定性和逻辑评估，无需每步工具调用即可提升视觉语言过程奖励模型的验证性能，为多模态推理系统提供了更可靠的评估框架。
...
TL;DR: AW-MoE框架通过集成混合专家系统，利用图像引导的天气感知路由和统一双模态增强，显著提升了恶劣天气条件下的3D目标检测性能，相比现有方法有约15%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有方法在训练时简单组合所有天气样本，忽略了不同天气场景下的数据分布差异，导致性能冲突。需要一种能够处理恶劣天气条件下数据分布差异的鲁棒3D目标检测方法。

Method: AW-MoE框架创新性地将混合专家系统集成到天气鲁棒的多模态3D目标检测方法中。包含：1) 图像引导的天气感知路由，利用图像特征在不同天气条件下的优越区分性和对场景变化的稳定性进行精确天气分类；2) 基于准确分类选择最相关的K个天气特定专家来处理数据差异；3) 统一双模态增强，在保持场景真实性的同时同步增强LiDAR和4D雷达双模态数据。

Result: 在真实世界数据集上的实验表明，AW-MoE在恶劣天气性能上比最先进方法提高了约15%，同时推理开销可忽略不计。将AW-MoE集成到现有基线检测器中，其性能提升超过了当前最先进方法。

Conclusion: AW-MoE通过处理不同天气条件下的数据分布差异，实现了恶劣天气下鲁棒的3D目标检测，证明了其有效性和强大的可扩展性。该方法为自动驾驶在复杂天气条件下的感知提供了有效解决方案。
...
TL;DR: 该论文提出FG-SGL框架，通过细粒度和类别级语义联合指导视觉-语言模型感知微手势的局部运动，解决微手势识别中类间差异细微的挑战。
...
Method: 提出细粒度语义指导学习框架，包含FG-SA模块（细粒度语义指导局部运动特征学习）和CP-A模块（类别级语义增强特征可分性）。构建了包含四个细化语义维度的人工标注细粒度文本数据集，并设计了多级对比优化策略进行粗到细的联合优化。

Result: 实验表明FG-SGL取得了有竞争力的性能，验证了细粒度语义指导对微手势识别的有效性。

Conclusion: 细粒度语义指导能够有效提升视觉-语言模型对微手势局部运动的感知能力，改善微手势识别性能。
...
TL;DR: 提出基于几何的奖励模型，利用预训练的几何基础模型通过跨帧重投影误差评估多视角一致性，解决视频扩散模型中的几何不一致问题


<details>
  <summary>Details</summary>
Motivation: 视频扩散模型在训练过程中缺乏明确的几何监督，导致生成的视频中出现物体变形、空间漂移和深度违反等不一致伪影。现有几何指标在像素空间测量不一致性，但像素强度可能引入额外噪声

Method: 1) 提出基于几何的奖励模型，利用预训练几何基础模型通过跨帧重投影误差评估多视角一致性，采用点对点方式计算误差；2) 引入几何感知采样策略，过滤低纹理和非语义区域，专注于几何意义明确且对应关系可靠的区域；3) 通过两种互补途径应用奖励模型：通过SFT或强化学习对双向模型进行后训练，或通过测试时缩放对因果视频模型进行推理时优化

Result: 实验结果表明，基于几何的奖励设计相比其他变体具有更好的鲁棒性。通过支持高效的推理时缩放，该方法为增强开源视频模型提供了实用解决方案，无需大量计算资源进行重新训练

Conclusion: 提出的几何奖励模型能有效解决视频扩散模型中的几何不一致问题，通过点对点误差计算和几何感知采样提高了鲁棒性，为视频生成模型的几何对齐提供了实用且高效的解决方案
...
TL;DR: 提出LTS-FS框架，通过定位幻觉相关层并稀疏化特征引导来减少LVLM幻觉，同时保持通用性能


<details>
  <summary>Details</summary>
Motivation: 现有特征引导方法在所有层上统一应用，忽略了层间差异，可能干扰与幻觉无关的层，导致通用任务性能下降
...
Conclusion: LTS-FS通过定位幻觉相关层并稀疏化特征引导，实现了更精确的幻觉缓解，避免了通用性能下降
...
TL;DR: EverTale是一个连续故事角色定制系统，通过统一LoRA模块实现角色持续适应，使用MLLM质量门控确保角色保真度，并采用角色感知区域聚焦采样解决多角色生成中的身份退化问题。


<details>
  <summary>Details</summary>
Motivation: 当前故事可视化方法难以同时实现准确的角色定制、语义对齐和新身份的持续集成，需要解决角色定制、语义对齐和连续身份集成的协同问题。

Method: 1. All-in-One-World Character Integrator：通过统一LoRA模块实现连续角色适应，无需每个角色的优化模块；2. Character Quality Gate via MLLM-as-Judge：通过思维链推理确保角色适应过程的保真度；3. Character-Aware Region-Focus Sampling：解决多角色故事可视化中的身份退化和布局冲突问题。

Result: 实验结果表明，EverTale在单角色和多角色故事可视化任务上都优于广泛的对比方法，实现了更优的性能表现。

Conclusion: EverTale通过创新的角色集成器、质量门控和采样策略，成功解决了连续故事角色定制中的关键挑战，为故事世界模拟提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有微表情AU检测方法通常从整个面部图像/视频中学习AU特征，这与AU固有的局部性相冲突，导致对AU区域的感知不足。实际上，每个AU独立对应特定的局部面部肌肉运动（局部独立性），而在特定情绪状态下，某些AU之间存在固有的依赖关系（全局依赖性）。

Method: 提出Micro-AU CLIP框架，将AU检测过程分解为局部语义独立性建模（LSI）和全局语义依赖性建模（GSD）。在LSI中设计了Patch Token Attention（PTA），将AU区域内的多个局部特征映射到同一特征空间；在GSD中提出了Global Dependency Attention（GDA）和Global Dependency Loss（GDLoss）来建模不同AU之间的全局依赖关系。此外，针对CLIP在微语义对齐方面的固有局限性，设计了microAU对比损失（MiAUCL）来通过视觉和文本特征的细粒度对齐学习AU特征。

Result: 实验结果表明，Micro-AU CLIP能够充分学习细粒度的微表情AU特征，实现了最先进的性能。该方法还能以无情绪标签的方式有效应用于微表情识别。

Conclusion: 本文探索了独立性到依赖性模式的有效性，提出的Micro-AU CLIP框架通过局部语义独立性建模和全局语义依赖性建模，成功解决了微表情AU检测中的区域感知不足问题，为细粒度真实情绪分析提供了客观线索。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散先验的4D场景重建方法通常独立处理帧或逐视角处理，缺乏时空协同，导致跨摄像头的空间错位和序列中的时间漂移问题

Method: 采用交错扩散transformer架构，包含专门设计的块来显式建模时间依赖性和跨摄像头空间一致性；通过历史上下文条件生成和几何感知训练损失，确保修复的视角符合统一的3D几何结构

Result: 在Waymo、nuScenes和PandaSet数据集上的广泛评估表明，DriveFix在重建和新视角合成方面实现了最先进的性能

Conclusion: DriveFix通过确保驾驶场景的时空一致性，显著减少了伪影并实现了高保真纹理的一致传播，为真实世界部署的鲁棒4D世界建模迈出了重要一步
...
TL;DR: 该论文开发了一个基于XGBoost的预测模型，利用多组学数据和机器学习技术预测肺癌药物敏感性，并通过SHAP和DeepSeek大语言模型解释特征重要性，实现个性化治疗方案的制定。


<details>
  <summary>Details</summary>
Motivation: 传统肺癌治疗方法（手术、化疗、放疗）因癌症异质性而效果有限，需要基于患者个体遗传信息和生活方式制定个性化治疗方案。AI深度学习可以分析大数据发现癌症早期迹象、肿瘤类型和治疗前景。
...
Result: 开发了一个能够预测肺癌药物敏感性的预测模型，通过SHAP分析确定了影响药物反应的关键分子和细胞特征，DeepSeek提供了关于重要基因或通路的生物学解释，支持模型的可预测性。

Conclusion: 该研究成功整合了多组学数据、机器学习和大语言模型，为肺癌个性化治疗提供了数据驱动的决策支持系统，能够根据患者特定遗传特征预测药物反应，优化治疗方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统前馈方法依赖大量训练数据但仍缺失细节，现有扩散方法利用生成先验但难以从合成域迁移到真实域。需要一种既能保留细节、又能从合成场景泛化到真实场景、且数据效率高的单目深度估计方法。

Method: 提出两阶段先验到几何确定性调度：先验阶段使用谱门控蒸馏转移低频真实先验，几何阶段使用谱门控一致性增强高频保真度并利用合成真值进行细化。两阶段共享权重，采用高到低时间步调度。

Result: 实验结果表明Iris在单目深度估计性能上取得显著提升，具有强大的野外泛化能力，能够保留细节并高效利用有限训练数据。

Conclusion: Iris框架通过将真实世界先验集成到扩散模型中，实现了细节保留、合成到真实场景的强泛化以及数据高效的单目深度估计，为相关领域提供了新的解决方案。
...
TL;DR: 本文提出了一种基于LiDAR点云的3D人体姿态估计框架HOIL，通过人-物交互学习解决交互区域的空间模糊性和类别不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶中基于LiDAR点云的人体理解至关重要，但现有方法大多忽视了人-物交互的潜力。主要挑战包括：1）人-物交互导致人与物体点之间的空间模糊性，导致交互区域关键点预测错误；2）交互与非交互身体部位的点数存在严重类别不平衡，手、脚等频繁交互区域在LiDAR数据中观测稀疏。

Method: 提出HOIL框架：1）人-物交互感知对比学习（HOICL）增强人与物体点之间的特征区分度；2）接触感知部分引导池化（CPPool）自适应重新分配表示能力，压缩过表示点同时保留交互身体部位的信息点；3）可选的基于接触的时序细化，利用时序接触线索细化每帧关键点估计。

Result: HOIL框架有效利用人-物交互解决了交互区域的空间模糊性和类别不平衡问题，代码将开源。

Conclusion: 通过人-物交互学习，本文提出的HOIL框架能够从LiDAR点云中实现鲁棒的3D人体姿态估计，解决了现有方法在交互区域面临的挑战。
...
TL;DR: 提出基于深度学习的框架，使用YOLO架构结合HiResCAM技术，实现姬蜂总科寄生蜂的自动分类识别，准确率超过96%，并通过可视化验证模型关注分类相关的解剖学特征。


<details>
  <summary>Details</summary>
Motivation: 姬蜂总科寄生蜂的准确分类鉴定对生物多样性评估、生态监测和生物防治至关重要，但形态相似性高、体型小、种间差异细微，使得人工鉴定劳动密集且依赖专家经验。

Method: 提出基于YOLO架构的深度学习框架，集成高分辨率类激活映射（HiResCAM）增强可解释性，使用3556张高分辨率膜翅目标本图像数据集，主要包含姬蜂科、茧蜂科、蜜蜂科和胡蜂科。

Result: 模型性能评估显示准确率超过96%，具有强大的形态变异泛化能力。HiResCAM可视化证实模型关注翼脉、触角分节和后腹部结构等分类相关解剖区域，验证了学习特征的生物学合理性。
...
<details>
  <summary>Details</summary>
Motivation: 水下图像增强对水下平台提供可靠视觉信息至关重要，但现有高性能方法通常依赖复杂架构，难以部署到水下设备上；而轻量级方法往往牺牲质量换取速度，且难以处理严重退化的水下图像。

Method: 1. 自适应加权通道补偿模块：以绿色通道为参考锚点，实现红蓝通道的动态颜色恢复
2. 多分支重参数化扩张卷积：训练时采用多分支融合，推理时进行结构重参数化，以低计算开销实现大感受野表示
3. 统计全局颜色调整模块：基于统计先验优化整体颜色性能

Result: 在八个数据集上的实验表明，该方法在七个评估指标上达到最先进性能。模型仅含3,880个推理参数，推理速度达409 FPS，在多样化环境条件下将UCIQE分数提升了29.7%。在ROV平台上的部署和下游任务中的性能提升进一步验证了其实时水下任务优势。

Conclusion: 该研究提出了一种高效的水下图像增强框架，在保持轻量化的同时实现了高质量颜色恢复，为实时水下任务提供了可行的解决方案，具有实际部署价值。
...
TL;DR: InViC是一个轻量级插件框架，通过提取问题相关的视觉提示令牌来增强医学VQA中的图像证据利用，减少模型依赖语言先验的捷径回答。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在医学VQA中常出现"捷径回答"问题，即过度依赖语言先验或数据集偏差，而忽视关键的视觉证据，这降低了临床可靠性，特别是在需要关注细微影像发现时。

Method: 提出InViC框架：1) Cue Tokens Extraction模块将密集视觉令牌提炼为K个问题相关的提示令牌；2) 两阶段微调策略：第一阶段使用提示瓶颈注意力掩码，强制模型通过提示通路获取视觉信息；第二阶段恢复标准因果注意力，训练模型联合利用视觉和提示令牌。

Result: 在三个公开医学VQA基准测试(VQA-RAD、SLAKE、ImageCLEF VQA-Med 2019)上评估，InViC在零样本推理和标准LoRA微调基础上均有持续改进，证明意图感知视觉提示与瓶颈训练是提高医学VQA可信度的实用有效策略。

Conclusion: InViC通过提取意图感知的视觉提示令牌并采用两阶段瓶颈训练策略，有效增强了医学VQA中基于图像的答案生成能力，提高了模型的临床可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉标记器主要将图像映射到固定2D空间网格，专注于像素级恢复，这阻碍了捕获具有紧凑全局语义的表示。需要一种能够捕捉高级语义的压缩表示方法。
...
Conclusion: SemTok通过语义一维标记化有效解决了现有视觉标记器的局限性，验证了语义1D标记化的有效性，代码将开源。
...
TL;DR: DermaFlux：基于rectified flow的文本到图像生成框架，通过合成皮肤病图像解决临床数据集稀缺和类别不平衡问题，提升皮肤病分类性能。


<details>
  <summary>Details</summary>
Motivation: 皮肤病分类系统受限于大型、多样且标注良好的临床数据集稀缺，导致良恶性病变类别不平衡，从而降低了模型的泛化性能。

Method: 基于Flux.1构建DermaFlux框架，使用参数高效的LoRA在大型临床图像数据集上进行微调，通过Llama 3.2生成符合皮肤病学标准（不对称性、边界不规则性、颜色变化）的文本描述来构建图像-文本对。

Result: DermaFlux生成多样且具有临床意义的皮肤病图像，在小规模真实数据集上提升二元分类性能达6%，在合成图像训练时比基于扩散的方法提升9%。仅用2500张真实图像和4375张DermaFlux合成样本训练的ViT达到78.04%准确率和0.859 AUC，超越现有最佳皮肤病模型8%。

Conclusion: DermaFlux通过文本到图像生成有效缓解皮肤病数据集稀缺问题，显著提升分类性能，为临床皮肤病诊断提供了有前景的解决方案。
...
TL;DR: 提出一种利用对称光源布置的线性求解方法，用于近光源光度立体视觉，无需初始化和完整标定即可获得表面法线和深度的闭式解。


<details>
  <summary>Details</summary>
Motivation: 传统近光源光度立体视觉方法通常采用非凸优化，需要深度初始化和精确的光源标定，这在实际应用中存在困难。本文旨在降低这些要求，提出更实用的解决方案。

Method: 通过布置多组对称的近光源对，利用对称光源排列的特性，推导出表面法线和深度的闭式线性解。该方法仅要求光源关于任意点对称分布，即使整个空间偏移未标定也能工作。

Result: 实验表明该方法在形状恢复精度上达到了与最先进的标定近光源光度立体方法相当的结果，同时显著减少了对仔细深度初始化和光源标定的要求。

Conclusion: 提出的线性求解方法通过利用对称光源布置，为近光源光度立体视觉提供了一种无需初始化和完整标定的实用解决方案，在保持精度的同时大大降低了应用门槛。
...
<details>
  <summary>Details</summary>
Motivation: 蛋白质标记物和组织病理学图像的联合预后潜力尚未充分探索，主要由于蛋白质表达谱的高成本和有限可用性。需要一种高效整合这两种模态信息的方法来改善癌症生存风险预测。

Method: 提出HGP-Mamba框架：1) 蛋白质特征提取器(PFE)利用预训练基础模型从全切片图像(WSIs)直接提取高通量蛋白质嵌入；2) 局部交互感知Mamba(LiAM)用于细粒度特征交互；3) 全局交互增强Mamba(GiEM)促进幻灯片级别的整体模态融合，捕捉复杂的跨模态依赖关系。

Result: 在四个公共癌症数据集上的实验表明，HGP-Mamba实现了最先进的性能，同时与现有方法相比保持了优越的计算效率。

Conclusion: HGP-Mamba通过从组织学图像生成蛋白质特征，有效整合了形态学和分子信息，为数据高效的癌症生存风险预测提供了一种有前景的解决方案。
...
TL;DR: SF-Mamba：一种新型视觉Mamba模型，通过辅助补丁交换和批量折叠技术解决传统Mamba在视觉任务中的局限性，在保持计算效率的同时实现双向信息流


<details>
  <summary>Details</summary>
Motivation: 传统视觉Transformer存在二次复杂度问题，而Mamba虽然计算效率高，但其循环扫描机制限制了图像补丁间的非因果交互，且在短令牌长度下计算速度较慢，需要更高效的视觉编码器
...
Conclusion: SF-Mamba通过重新设计扫描操作和计算效率，实现了真正高效的视觉编码器，为视觉Mamba模型的发展提供了新方向
...
TL;DR: HGFNet是一种用于高光谱图像分类的新型架构，结合了3D卷积特征提取和频率域全局滤波，通过三种互补的频率变换和自适应焦点损失来解决现有方法的可扩展性和类别不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有高光谱图像分类方法存在根本性限制：基于Transformer的模型由于自注意力的二次复杂度导致可扩展性差，而基于傅里叶变换的方法通常依赖2D空间FFT，忽略了高光谱数据固有的关键波段间光谱依赖性。

Method: 提出Hybrid GFNet (HGFNet)，整合局部3D卷积特征提取和通过GFNet风格块的频率域全局滤波。引入三种互补的频率变换：光谱傅里叶变换（沿光谱轴的1D FFT）、空间傅里叶变换（空间维度的2D FFT）和空间-空间傅里叶变换（光谱和空间维度的3D FFT）。还包含自适应焦点损失(AFL)来处理类别不平衡。

Result: 该方法能够实现高效稳健的空间-光谱表示学习，通过3D卷积层捕获细粒度局部空间-光谱结构，同时傅里叶基全局滤波模块有效建模长程依赖并抑制噪声。

Conclusion: HGFNet通过整合局部3D卷积和频率域全局滤波，解决了现有高光谱图像分类方法的可扩展性和光谱依赖性建模不足的问题，同时通过自适应焦点损失缓解了类别不平衡问题，实现了更全面的高维频率建模。
...
<details>
  <summary>Details</summary>
Motivation: 当前浮游生物图像识别主要依赖监督学习方法，需要大量人工标注数据，而现代成像仪器采集的光学测量数据（散射和荧光剖面）在识别中未被充分利用。作者希望利用这些多模态数据，通过自监督学习减少对人工标注的依赖。

Method: 受CLIP启发，使用二元监督信息（判断图像和剖面是否来自同一颗粒）训练图像和剖面两种模态的编码器。识别时结合少量已知物种的标注图库和k-NN分类器，构建能够同时利用图像和剖面信息的跨模态识别模型。

Result: 该方法仅需极少量标注图像即可实现高识别准确率，且性能优于仅使用图像的自监督基线方法，证明了跨模态自监督学习的有效性。

Conclusion: 自监督跨模态协调策略能够有效利用多模态浮游生物数据，显著减少对人工标注的依赖，为大规模浮游生物识别提供了实用且高效的解决方案。
...
Method: 引入IRIS基准数据集，包含220个在受控实验室条件下拍摄的4K 60fps真实世界视频，涵盖单体和多体动力学系统，每个系统都配有独立测量的真实参数、不确定性估计和控制方程。定义了标准化评估协议，包括参数准确性、可识别性、外推性、鲁棒性和控制方程选择。

Result: 评估了多个基线方法，包括多步物理损失公式和四种互补的方程识别策略（VLM时间推理、描述-分类提示、CNN分类和路径标记），建立了所有IRIS场景的参考性能，并揭示了系统性的失败模式。

Conclusion: IRIS为无监督物理参数估计提供了首个全面的真实世界基准，数据集、标注、评估工具包和所有基线实现均已公开发布，将推动该领域的未来研究。
...
TL;DR: 该论文首次定义了雨滴和反射统一去除任务(UR³)，构建了真实拍摄的RDRF数据集，并提出了基于扩散模型的DiffUR³框架来同时去除雨滴和反射，在基准测试和实际图像中都取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 在雨天透过玻璃或挡风玻璃拍摄时，雨滴和反射经常同时出现，严重降低图像可见度。这个实际问题缺乏关注且急需解决，现有的去雨滴、去反射或一体化模型都无法有效处理这种复合退化问题。

Method: 首先正式定义了雨滴和反射统一去除任务(UR³)，构建了真实拍摄的RDRF数据集作为新基准。然后提出了基于扩散模型的DiffUR³框架，利用强大的生成先验，通过特定的目标设计来同时去除两种退化类型。

Result: 广泛的实验表明，该方法在构建的基准测试和具有挑战性的实际图像上都取得了最先进的性能。RDRF数据集和代码将在论文被接受后公开。

Conclusion: 该研究首次解决了雨滴和反射同时存在的复合退化问题，通过定义新任务、构建高质量数据集和提出基于扩散模型的解决方案，为这一实际问题提供了有效的技术方案。
...
TL;DR: Evo-Retriever是一个检索框架，通过LLM引导的课程演化和多视角-路径协作，解决视觉语言模型在异构文档中的跨模态检索问题，在ViDoRe V2和MMEB数据集上达到SOTA性能。
...
Method: 1. 多视角图像对齐：通过多尺度和多方向视角增强细粒度匹配；2. 双向对比学习策略：生成"困难查询"并建立互补学习路径，用于视觉和文本消歧以重新平衡监督；3. LLM元控制器：将模型状态摘要输入LLM，利用专家知识自适应调整训练课程，促进模型演化。
...
TL;DR: GAP-MLLM提出了一种几何对齐预训练范式，通过视觉提示联合任务和多级渐进融合模块，激活多模态大语言模型的几何感知能力，显著提升3D视觉任务的性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在语义推理方面表现出色，但在仅使用RGB输入时缺乏3D空间感知能力。现有方法通常采用简单的特征拼接，并在没有几何特定监督的情况下直接优化下游任务，导致几何表示未被充分激活。

Method: 提出GAP-MLLM几何对齐预训练范式：1）引入视觉提示联合任务，强制模型同时预测稀疏点图和语义标签以激活几何感知；2）设计多级渐进融合模块，采用令牌级门控机制自适应整合几何先验而不抑制语义推理。

Result: 在3D视觉定位、3D密集描述和3D视频目标检测等任务上，GAP-MLLM显著提升了几何特征融合效果，并持续增强了性能表现。

Conclusion: 通过几何对齐预训练范式，GAP-MLLM成功激活了多模态大语言模型的几何感知能力，解决了现有方法中几何表示未被充分激活的问题，为3D视觉任务提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 一步生成框架（如流映射模型）显著提高了图像生成效率，但确保这些强大生成器安全性的机器遗忘问题尚未探索。现有的扩散遗忘方法依赖于多步迭代去噪过程，与一步模型不兼容。

Method: 基于非平衡最优传输理论，将遗忘问题形式化为遗忘成本（抑制目标类别）与f-散度惩罚（通过松弛边际约束保持整体生成保真度）之间的权衡。使被遗忘类别的概率质量平滑地重新分配到剩余类别，而不是坍缩为低质量或噪声样本。

Result: 在CIFAR-10和ImageNet-256上的实验结果表明，该框架在遗忘成功率（PUL）和保留质量（u-FID）方面表现优异，显著优于基线方法。

Conclusion: UOT-Unlearn为一步生成模型提供了一个即插即用的类别遗忘框架，通过非平衡最优传输理论实现了有效的类别遗忘，同时保持了整体生成质量。
...
TL;DR: VIEW2SPACE是一个用于稀疏多视角推理的多维度基准测试，通过物理仿真生成大规模数据，评估发现现有模型在多视角推理上表现接近随机猜测，提出的Grounded Chain-of-Thought方法能显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的多视角推理需要整合稀疏离散的局部观测，但现有研究主要关注单图像或密集视频场景，且缺乏大规模带精确几何和语义标注的多视角数据。

Method: 利用物理仿真构建多样化的高保真3D场景，生成精确的每视角元数据，创建VIEW2SPACE基准测试和可扩展的训练分割，提出Grounded Chain-of-Thought with Visual Evidence方法。

Result: 现有最先进的视觉语言和空间模型在多视角推理上表现接近随机猜测，提出的方法在中等难度下显著提升性能，并能泛化到真实世界数据，在跨数据集评估中优于现有方法。

Conclusion: 多视角推理仍是一个未解决的挑战，虽然几何感知在足够可见度下可通过缩放提升，但跨稀疏视角的深度组合推理仍是根本性难题，需要更先进的推理方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统的手动和原始2D边缘检测方法在爆轰胞格分割和测量方面存在局限性，需要更精确的自动化方法来解决这一长期存在的挑战。

Method: 基于图论的无训练算法，使用分割模型从3D压力迹线（爆轰晶格）中提取细胞模式，通过图基工作流程处理3D模拟数据。

Result: 在生成数据上分割误差为2%；3D模拟数据显示椭圆形细胞沿波传播轴排列，偏差为17%；体积的更大离散反映了线性变异性的立方放大。

Conclusion: 该图基框架对多种细胞几何形状具有普适性，是爆轰分析的实用工具，为未来三重点碰撞研究奠定了坚实基础，但对高度复杂细胞模式的分割和量化仍具挑战性。
...
<details>
  <summary>Details</summary>
Motivation: 尽管3D高斯泼溅（3DGS）在场景表示和视觉定位方面表现出色，但其姿态优化的鲁棒性对初始相机姿态和重建几何非常敏感。本文识别了两个主要的不确定性来源：姿态先验不确定性（来自回归或检索模型的单点估计）和几何不确定性（由3DGS重建不完美导致，会传播到PnP求解器中）。这些不确定性会扭曲重投影几何并破坏优化稳定性。

Method: 提出了一种重定位框架，结合蒙特卡洛姿态采样和基于费舍尔信息的PnP优化。该方法明确考虑了姿态和几何不确定性，无需重新训练或额外监督。通过蒙特卡洛采样处理姿态先验不确定性，使用费舍尔信息矩阵来量化几何不确定性对PnP求解的影响。

Result: 在多样化的室内外基准测试中，该方法一致提高了定位精度，并显著增强了在姿态和深度噪声下的稳定性。相比现有方法，能够更好地处理不确定性带来的挑战。

Conclusion: 通过显式建模姿态和几何不确定性，提出的框架有效解决了3DGS姿态优化中的鲁棒性问题，为视觉定位和姿态细化提供了更稳定可靠的解决方案。
...
TL;DR: 使用变分自编码器（VAE）和期望最大化（EM）方法解决扫描透射电子显微镜（STEM）的校准问题，通过模拟数据学习图像的低维表示，实现比现有方法更快速、更一致的校准效果。


<details>
  <summary>Details</summary>
Motivation: 电子显微镜校准面临挑战：基于图像调整参数以克服光学像差，但诊断图像高维且噪声大，且单个图像无法确定最优参数。现有方法大多只提取标量值，无法充分利用图像信息。

Method: 使用变分自编码器（VAE）在模拟数据上训练，学习图像的低维表示；然后采用期望最大化（EM）方法联合估计校准参数到编码表示的映射模型及最优校准参数，解决模拟到现实的差距问题。

Result: 在真实STEM上验证，该方法比现有方法显著更快、更一致，估计误差减少2倍，且需要更少的观测次数。证明了VAE在图像信息压缩方面的潜力。

Conclusion: VAE-EM框架为自动化STEM校准提供了重要进展，该框架也适用于存在模拟训练数据与现实差距、且非单射映射会阻碍唯一解的其他逆问题。
...
TL;DR: 提出SAE方法，利用语义分割量化视觉注意力不确定性，检测并减少大视觉语言模型中的物体幻觉问题
...
Conclusion: SAE方法通过量化视觉注意力不确定性，有效检测和缓解大视觉语言模型中的物体幻觉问题，提高了模型的可靠性，特别是在实际机器人应用场景中
...
TL;DR: 基于Transformer的深度学习模型直接从原始活细胞视频预测癌细胞命运，准确率达0.94，可提前10小时预测细胞结局，并提供可解释的生物学洞察。


<details>
  <summary>Details</summary>
Motivation: 理解非遗传因素对细胞命运的决定机制对癌症治疗至关重要，因为遗传相同的细胞在相同治疗条件下可能产生不同结局。现有方法依赖预定义特征，需要直接从原始图像序列预测细胞命运并解释预测依据。

Method: 提出基于Transformer的深度学习模型，直接从化疗处理下的长期活细胞记录原始图像序列预测细胞命运，无需预定义形态或分子特征。开发了全面的可解释性框架，包括注意力机制和掩码实验，分析时间动态和形态线索。

Result: 模型在细胞命运预测上达到平衡准确率0.94和F1分数0.93。注意力实验显示预测信号不仅存在于最终帧，可靠预测可提前10小时。分析揭示了有丝分裂和凋亡序列中预测信息的独特时间分布，以及细胞形态和p53信号在决定细胞结局中的作用。

Conclusion: 基于注意力的时序模型能够准确预测细胞命运，同时提供对细胞决策非遗传决定因素的生物学可解释洞察。该方法为理解细胞异质性和开发精准癌症治疗提供了新工具。
...
TL;DR: 基于视频扩散Transformer架构的3D形状物理材质生成方法，通过几何和文本描述联合生成多种材质属性，兼容主流创作工具。


<details>
  <summary>Details</summary>
Motivation: 为3D形状生成物理准确的材质是内容创作中的重要挑战，需要同时处理多种材质属性并保持物理一致性。

Method: 使用视频扩散Transformer架构，以几何和文本描述为条件，联合建模多种材质属性；引入定制变分自编码器将多模态材质编码到紧凑潜在空间，减少token数量。

Result: 能够根据文本提示为3D形状生成高质量的物理材质，支持基础颜色、粗糙度、金属性、高度图等多种属性，兼容常见内容创作工具。

Conclusion: 该方法通过联合建模多材质属性和紧凑潜在表示，实现了高效、高质量的物理材质生成，为3D内容创作提供了实用工具。
...
TL;DR: Face2Scene：一个两阶段图像修复框架，利用人脸作为感知参考来估计图像退化并指导整个场景的修复，解决了现有方法只修复人脸或忽略退化线索的问题。


<details>
  <summary>Details</summary>
Motivation: 现有参考式人脸修复模型只关注面部区域，忽略了身体和背景的退化；而全场景修复器又经常完全忽略退化线索，导致预测不确定和视觉伪影。需要一种能利用人脸退化信息来指导整个场景修复的方法。

Method: 提出两阶段修复框架：1）首先使用参考式人脸修复模型从退化图像和身份参考中重建高质量面部细节；2）从修复-退化的人脸对中提取人脸衍生的退化编码，捕获噪声、模糊、压缩等退化属性，然后转换为多尺度退化感知标记；3）这些标记条件化扩散模型，在单步中修复包括身体和背景在内的整个场景。

Result: 大量实验证明，该方法相比最先进的方法具有优越的有效性。

Conclusion: Face2Scene通过将人脸作为感知参考来估计退化并指导全场景修复，解决了现有方法的局限性，实现了更全面和高质量的图像修复。
...
TL;DR: REFORGE是一个黑盒红队框架，通过对抗性图像提示评估图像生成模型反学习（IGMU）的鲁棒性，揭示当前方法在多模态对抗攻击下的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 图像生成模型（IGMs）在实现高保真内容生成的同时，也带来了版权复制和有害内容生成等风险。图像生成模型反学习（IGMU）旨在无需完全重新训练即可移除有害概念，但其在黑盒设置下对抗图像侧威胁的鲁棒性尚未得到充分研究。

Method: REFORGE采用基于笔触的图像初始化，通过交叉注意力引导的掩码策略优化扰动，将噪声分配到概念相关区域，平衡攻击效果和视觉保真度。

Result: 在代表性反学习任务和防御方法上的广泛实验表明，REFORGE显著提高了攻击成功率，同时实现了更强的语义对齐和更高的效率。

Conclusion: 研究结果揭示了当前IGMU方法存在的持续脆弱性，强调需要开发针对多模态对抗攻击的鲁棒性感知反学习方法。
...
TL;DR: 该研究将人脑的共线性视觉感知原理引入计算机视觉，探索其在工业检测等应用中的潜力，发现共线性对晶圆、纳米材料等结构化图像有显著提升效果，但对ImageNet等自然图像效果有限。


<details>
  <summary>Details</summary>
Motivation: 人脑的共线性感知原理（对空间对齐边缘的增强）在真实世界中的作用尚不明确，在计算机视觉和工程应用中的利用更是未充分探索。本研究旨在将这一原理引入计算机视觉，探索其潜在应用价值。

Method: 开发了原型模型来体现共线性原理，系统测试并在四个用例中进行基准测试：1）结合深度学习的晶圆缺陷检测；2）纳米材料缺陷识别（结合深度学习和显著性模型）；3）遮挡处理；4）ImageNet测试。

Result: 晶圆缺陷检测：错误率从6.5%降至5.26%，性能提升1.24倍；纳米材料缺陷识别：错误率从21.65%降至6.64%，性能提升3.2倍；遮挡处理显示有益效果；ImageNet测试效果不明显。确定了共线性适用的场景（晶圆、纳米技术、遮挡）和不适用的场景（ImageNet）。

Conclusion: 共线性原理特别适用于工业应用，当感兴趣图像结构是人造且包含线条时效果显著。该研究为计算机视觉提供了新工具，旨在捕捉人类视觉处理的优势。
...
TL;DR: 该论文提出了ACPV-Net框架，用于从航拍图像一次性生成包含所有土地覆盖类别的完整矢量地图，解决了多类别多边形化中的拓扑不一致问题。


<details>
  <summary>Details</summary>
Motivation: 现有多边形化方法通常是针对单一类别的，扩展到多类别时会导致拓扑不一致问题，如重复边缘、间隙和重叠。需要一种能够一次性生成所有类别多边形表示的方法。

Method: 提出了ACPV-Net统一框架，包含语义监督条件机制（SSC）将语义感知与几何基元生成耦合，以及拓扑重建机制通过设计强制共享边缘一致性。

Result: 在Deventer-512基准测试中，ACPV-Net在强制严格拓扑约束的同时，在所有类别的多边形质量上超越了所有单类别基线方法。在WHU-Building单类别任务上也取得了最佳报告结果。

Conclusion: ACPV-Net成功解决了多类别多边形矢量化的拓扑一致性问题，为从航拍图像生成完整矢量地图提供了一种高效、准确的解决方案。
...
Method: 提出TCATSeg框架，结合局部几何特征与全局语义上下文；引入一组稀疏但物理意义明确的超点来捕捉全局语义关系；创建包含400个牙科模型的新数据集，包括正畸前样本。

Result: 大量实验表明，TCATSeg在3D牙科模型分割任务上优于现有最先进方法，特别是在处理复杂牙齿排列和相似形状牙齿时表现更佳。
...
TL;DR: 该研究系统分析了多模态大语言模型在无约束人脸验证任务中生成解释的可靠性，发现即使模型做出正确判断，其解释也常依赖无法验证或虚构的面部属性，缺乏视觉证据支持。


<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型被提出用于为人脸识别决策生成自然语言解释，但在无约束人脸图像上的解释可靠性尚未得到充分探索。研究者希望系统分析这些模型在极端姿态变化和监控图像等挑战性场景下的解释质量。

Method: 研究使用IJB-S数据集对无约束人脸验证任务进行系统分析，特别关注极端姿态变化和监控图像。研究还探讨了整合传统人脸识别系统信息（如分数和决策）的影响，并引入基于似然比的框架来评估文本解释的证据强度。

Result: 结果显示，即使多模态大语言模型做出正确的验证决策，其解释也经常依赖无法验证或虚构的面部属性，缺乏视觉证据支持。虽然整合传统人脸识别系统信息提高了分类验证性能，但并未一致地产生可信的解释。

Conclusion: 当前多模态大语言模型在可解释人脸识别方面存在根本性局限，需要在生物识别应用中建立原则性评估框架来衡量解释的可靠性和可信度。
...
TL;DR: FlowComposer：首个系统研究流匹配在组合零样本学习中的应用，通过显式组合流和泄漏引导增强解决现有方法的两大局限


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型的组合零样本学习方法存在两大根本局限：1）隐式组合构建，仅通过token拼接或分支提示调优实现组合；2）残留特征纠缠，不完美的解耦导致属性、对象和组合特征相互污染。这些问题限制了当前CZSL模型的泛化能力。

Method: 提出FlowComposer框架：学习两个原始流将视觉特征传输到属性和对象文本嵌入，以及一个可学习的组合器将它们的速度场显式融合成组合流。此外设计了泄漏引导增强方案，重用泄漏特征作为辅助信号。
...
<details>
  <summary>Details</summary>
Motivation: 现有的场景图异常关系检测方法存在性能限制，需要更高效、鲁棒且能利用现实世界语义知识的解决方案。作者旨在开发一种基于学习的方法，能够有效检测场景图中的异常关系，特别是在处理同义词等语义变化时保持稳定性能。

Method: 采用多模态方法，使用语言模型嵌入场景图中的对象和关系标记以利用现实世界语义知识。构建归一化流模型学习从对象-关系-对象三元组到简单基分布（通常是高斯分布）的双射变换，通过似然估计进行异常检测。

Result: 在包含办公室和餐厅场景的SARD数据集上评估，相比当前最先进模型，AUROC结果提升约10%，同时速度快5倍。消融研究显示模型具有优越的鲁棒性和通用性，特别是在处理同义词时，BUSSARD保持稳定性能而基线模型有17.5%的偏差。

Conclusion: BUSSARD展示了基于学习的方法在场景图关系异常检测中的强大潜力，通过结合语言模型的语义知识和归一化流的概率建模，实现了高效、鲁棒的异常检测，为场景理解任务提供了有前景的解决方案。
...
TL;DR: StyleExpert：基于专家混合的语义感知风格化框架，能够处理从浅层纹理到深层语义的多样化风格，超越现有仅关注颜色变换的方法
...
Conclusion: StyleExpert通过语义感知的专家混合框架，成功解决了现有风格化方法在语义和材质细节处理上的局限性，实现了从浅层纹理到深层语义的多样化风格处理
...
TL;DR: HeBA提出了一种异构瓶颈适配器，通过模态特定的结构归纳偏置来改进视觉语言模型的微调，在11个少样本基准测试中达到新的最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有大规模视觉语言模型（如CLIP）在下游任务适配时采用"一刀切"的架构方法，忽视了视觉和文本模态的结构差异：图像具有空间局部性，而文本具有语义密度。这种同质化处理限制了模型性能。

Method: HeBA框架包含三个关键创新：1）异构性：视觉token通过2D深度可分离卷积处理以保持空间相关性，文本token通过密集线性投影处理以捕捉语义关系；2）瓶颈正则化：采用压缩瓶颈（D→D/4）强制学习紧凑、鲁棒的特征；3）主动梯度初始化：使用Kaiming初始化策略确保足够的初始梯度流，加速收敛而不损害预训练知识。

Result: 在广泛的实验中，HeBA的架构专业化设计实现了优越的稳定性和准确性，在11个少样本基准测试中建立了新的最先进性能。

Conclusion: HeBA通过引入模态特定的结构归纳偏置，解决了视觉语言模型适配中的同质化问题，其异构瓶颈设计在保持预训练知识的同时实现了更好的下游任务性能。
...
TL;DR: SPDDA是一种针对高光谱图像单源域泛化的数据增强方法，通过考虑高光谱图像的光谱特性（设备依赖的通道数变化和相邻通道混合），在保持真实性和多样性之间取得平衡，提升跨域分类性能。


<details>
  <summary>Details</summary>
Motivation: 高光谱图像虽然具有丰富的光谱信息，但其高维度和传感器变异性使其对跨域分布差异敏感，影响分类性能。现有单源域泛化方法在数据增强中存在真实性与多样性的权衡问题：盲目增强可能产生不现实的样本，而过度强调真实性又会抑制多样性。

Method: 提出光谱特性驱动的数据增强方法SPDDA：1) 光谱多样性模块：沿光谱维度重采样源域数据，生成不同光谱通道数的样本；2) 通道自适应光谱混合器：通过建模通道间相似性避免固定增强模式；3) 空间-光谱协同优化机制：联合优化空间保真约束和光谱连续性自约束，自适应调整光谱自约束权重以防止光谱过平滑并保持空间结构。

Result: 在三个遥感基准数据集上的广泛实验表明，SPDDA优于现有最先进的方法。

Conclusion: 通过显式考虑高光谱图像的内在特性，SPDDA在数据增强的真实性和多样性之间取得了更好的平衡，有效提升了高光谱图像在单源域泛化场景下的跨域分类性能。
...
TL;DR: Fast-WAM提出了一种无需在测试时显式预测未来的世界动作模型，通过保留训练时的视频建模但跳过测试时的未来预测，实现了实时性能（190ms延迟）且保持竞争力。


<details>
  <summary>Details</summary>
Motivation: 现有WAMs采用"想象后执行"范式，在测试时需要迭代视频去噪，导致显著的延迟。研究旨在探究WAMs是否真的需要在测试时进行显式未来想象，或者其优势主要来自训练时的视频建模。

Method: 提出Fast-WAM架构，在训练时保留视频协同训练，但在测试时跳过未来预测。实例化多个Fast-WAM变体以进行受控比较，分离训练时视频建模和推理时显式未来生成的作用。

Result: Fast-WAM在仿真基准（LIBERO和RoboTwin）和真实世界任务上均取得与最先进方法竞争的结果，无需具身预训练。运行延迟仅190ms，比现有"想象后执行"WAMs快4倍以上。研究发现跳过视频协同训练会导致更大的性能下降。

Conclusion: 视频预测在WAMs中的主要价值可能在于改善训练时的世界表示，而非在测试时生成未来观测。Fast-WAM展示了通过简化推理流程实现实时性能的可行性。
...
TL;DR: x²-Fusion提出了一种基于事件相机边缘空间的多模态融合方法，将图像、LiDAR和事件数据统一到共享的潜在表示中，用于联合估计2D光流和3D场景流。


<details>
  <summary>Details</summary>
Motivation: 现有方法在异构特征空间中处理多模态数据（图像、LiDAR、事件），缺乏共享的潜在空间，导致跨传感器不匹配问题未解决，融合过程复杂。事件相机提供的时空边缘信号可以作为内在边缘场来锚定统一表示。

Method: 提出Event Edge Space（事件边缘空间）作为统一的潜在表示，将图像和LiDAR特征显式对齐到这个共享空间中。采用可靠性感知的自适应融合来估计模态可靠性并强调稳定线索，同时使用跨维度对比学习来紧密耦合2D光流和3D场景流。

Result: 在合成和真实基准测试中，x²-Fusion在标准条件下达到了最先进的精度，在具有挑战性的场景下实现了显著改进。

Conclusion: 通过将多模态融合重新定义为表示统一，利用事件衍生的时空边缘作为边缘中心同质空间，x²-Fusion有效解决了跨传感器不匹配问题，简化了融合复杂性，提升了动态场景理解的性能。
...
TL;DR: HMAR提出了一种用于医学图像检索的自适应分层框架，通过双专家机制分别处理全局特征和局部特征，结合对比学习和滑动窗口匹配，在无需边界框标注的情况下实现细粒度检索，性能优于现有方法。
...
Method: HMAR采用混合专家架构，包含双专家机制：Expert0提取全局特征进行整体相似性匹配，Expert1学习位置不变的局部表示用于精确病变区域检索。使用两阶段对比学习策略避免昂贵的边界框标注，滑动窗口匹配算法在推理时实现密集局部比较，并通过KAN层生成哈希码进行高效汉明距离搜索。

Result: 在RadioImageNet-CT数据集（16种临床模式，29,903张图像）上的实验显示，HMAR在64位和128位哈希码上分别达到0.711和0.724的mAP，相比最先进的ACIR方法分别提升了0.7%和1.1%。

Conclusion: HMAR通过分层模态感知专家和动态路由机制，有效解决了医学图像检索中的细粒度检索需求，在无需边界框标注的情况下实现了精确的病变区域检索，性能优于现有方法。
...
TL;DR: Search2Motion是一个无需训练的图像到视频生成框架，通过目标帧控制和早期注意力机制实现物体运动编辑，无需轨迹、边界框或掩码等额外输入。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要轨迹、边界框、掩码或运动场等额外输入，且现有基准测试混淆了物体运动和相机运动，缺乏专门评估物体运动编辑的方法。

Method: 采用目标帧控制策略，利用首尾帧运动先验；通过语义引导的物体插入和鲁棒背景修复构建目标帧；利用早期自注意力图预测物体和相机动态，并提出ACE-Seed轻量搜索策略提升运动保真度。

Result: 在FLF2V-obj和VBench基准测试中一致优于基线方法，无需前瞻采样或外部评估器即可提升运动保真度。

Conclusion: Search2Motion提供了一种无需训练、无需额外输入的物体运动编辑框架，通过目标帧控制和早期注意力机制实现了高质量的物体重定位，同时保持了场景稳定性。
...
TL;DR: 该研究提出了一个用于课堂学习的高通量实时多智能体情感计算框架，通过情绪状态监测提升教学效果，系统在三个学校测试中表现出色，检测50张人脸达25FPS，课堂参与度分类准确率达88%。
...
Result: 系统在三个教育机构测试中表现稳健：可同时检测50张人脸（25FPS），课堂参与状态分类总体准确率达88%，学生、教师和家长反馈积极，认为改善了课堂互动和教学适应性。

Conclusion: 该研究建立了实用的基于物联网的情绪感知学习环境框架，并引入了"课堂情绪数据集"促进进一步验证和研究，为情感计算在教育领域的应用提供了可行方案。
...
TL;DR: 提出一种从视频扩散模型生成不一致帧中重建高质量3D场景的方法，通过非刚性对齐和全局优化实现3D一致性


<details>
  <summary>Details</summary>
Motivation: 视频扩散模型生成的帧虽然质量高，但缺乏3D一致性，难以重建3D世界。需要解决帧间不一致问题，实现从视频到3D场景的转换

Method: 1) 使用几何基础模型将每帧提升为像素级3D点云；2) 提出定制化的非刚性迭代帧到模型ICP进行初始对齐；3) 全局优化进一步锐化点云；4) 利用点云作为3D重建初始化，提出逆变形渲染损失从不一致视图中创建高质量3D环境

Result: 方法能够从视频扩散模型生成不一致帧中重建出比基线方法更高质量的3D场景，有效将视频模型转化为3D一致的世界生成器

Conclusion: 提出的方法成功解决了视频扩散模型帧间3D不一致问题，通过非刚性对齐和全局优化实现了高质量3D场景重建，为从视频生成3D一致世界提供了有效解决方案
...
Method: 提出CIRCLES框架，通过属性引导的组合图像检索主动构建演示集，检索反事实风格示例，使模型能够隐式推理属性与结果之间的因果关系

Result: 在四个多样化数据集上的实验表明，CIRCLES在多种架构上一致优于现有方法，特别是在小规模模型上，在信息稀缺条件下表现尤为突出，同时检索到更多样化和因果信息丰富的示例
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶3D感知严重依赖大规模数据收集和人工标注，但在不同城市和地区部署时变得不切实际。现代城市越来越多地部署路边单元，这引发了一个自然问题：城市本身能否帮助训练车辆？

Method: 提出基础设施教学的无标签3D感知范式：1) RSUs作为固定无监督教师从无标签数据中学习本地3D检测器；2) RSUs向经过车辆广播预测结果；3) 车辆聚合这些预测作为伪标签监督来训练独立的自我检测器；4) 测试时无需基础设施或通信支持

Result: 在CARLA多智能体环境中进行概念可行性研究，使用CenterPoint检测器，该方法达到82.3% AP的车辆检测性能，而完全监督的自我检测器上限为94.4%。系统分析了每个阶段，评估了可扩展性，并展示了与现有自我中心无标签方法的互补性
...
TL;DR: SuCor是一种使用最优传输理论校正EPI图像磁化率几何畸变的新方法，通过反向相位编码EPI对和Wasserstein-2重心位移建模，自动选择正则化参数，在HCP数据集上表现优于FSL TOPUP。


<details>
  <summary>Details</summary>
Motivation: 回波平面成像（EPI）中的磁化率诱导几何畸变是功能磁共振成像（fMRI）和扩散磁共振成像（dMRI）中的常见问题，需要有效的校正方法。现有方法如FSL TOPUP存在性能限制，需要更准确、自动化的校正方案。

Method: SuCor使用最优传输理论，通过反向相位编码EPI对，将每个畸变场列建模为相反极性强度剖面之间的Wasserstein-2重心位移。在谱域中使用弯曲能量惩罚进行正则化，通过Morozov差异原理自动选择正则化强度，无需手动调参。

Result: 在人类连接组计划（HCP）数据集上，SuCor与T1结构图像的体积互信息达到0.341，优于FSL TOPUP的0.317。计算效率高，在单CPU核心上仅需约12秒运行时间。

Conclusion: SuCor是一种有效、自动化的EPI畸变校正方法，基于最优传输理论，在准确性和计算效率方面均优于现有方法，为神经影像分析提供了改进的畸变校正工具。
...
Method: 1) 引入基于顶点帧的新范式，从稳定伪装状态的顶点帧分类真实情绪；2) 提出双流独立解耦框架，分离真实和伪装情绪特征；3) 设计解耦损失组，包括两个分类损失分别学习真实情绪和伪装表情特征，以及希尔伯特-施密特独立性损失增强特征独立性。

Result: 实验证明基于顶点帧的范式更具挑战性，提出的解耦框架提高了识别性能。

Conclusion: 通过顶点帧范式和双流独立解耦框架，能更有效地从稳定伪装状态中识别真实情绪，避免伪装情绪的干扰。
...
TL;DR: 本文提出GDPO方法，将强化学习整合到一步生成式图像超分辨率模型中，通过噪声感知扩散模型和属性感知奖励函数提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习主要应用于多步生成式图像超分辨率，而一步生成式ISR由于随机性有限而研究不足。现有RL方法如DPO需要离线生成正负样本对，样本数量有限；GRPO仅计算整张图像的似然，忽略了ISR中至关重要的局部细节。

Method: 提出Group Direct Preference Optimization (GDPO)：1) 引入噪声感知的一步扩散模型，通过不等时间步策略将噪声添加时间步与扩散时间步解耦；2) 将GRPO原理整合到DPO中，计算在线生成样本的组相对优势进行模型优化；3) 设计属性感知奖励函数，基于平滑和纹理区域的统计动态评估每个样本的分数。

Result: 实验证明了GDPO在增强一步生成式ISR模型性能方面的有效性。

Conclusion: GDPO成功将强化学习整合到一步生成式图像超分辨率训练中，通过创新的噪声感知模型和组相对优化策略，有效提升了ISR性能。
...
TL;DR: 本文系统研究了视觉协同去噪方法，提出了V-Co框架，识别出四个关键要素：完全双流架构、结构定义的无条件预测、感知漂移混合损失和RMS特征重标定，显著提升了像素空间扩散模型的性能。


<details>
  <summary>Details</summary>
Motivation: 像素空间扩散模型虽然能实现高质量生成，但缺乏对高层视觉结构的显式建模能力。现有的视觉协同去噪方法设计选择混杂，不清楚哪些要素真正关键，需要系统研究来明确有效视觉协同去噪的核心要素。

Method: 提出V-Co框架，采用统一的JiT（即时）架构进行系统研究。通过控制变量法分离设计选择，研究双流架构、无条件预测结构、损失函数设计和特征标定等要素对视觉协同去噪效果的影响。

Result: 在ImageNet-256上，V-Co在可比模型大小下超越了底层像素空间扩散基线和先前强像素扩散方法，同时使用更少的训练轮次，验证了四个关键要素的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有动物深度估计模型大多基于无度量尺度的数据集训练，这限制了图像模型的验证能力。动物（特别是野生动物）的深度估计和3D重建面临挑战，需要包含度量信息的真实世界数据。

Method: 提出WildDepth多模态数据集，包含从家养到野外环境的多种动物类别，提供同步的RGB图像和LiDAR数据。建立深度估计、行为检测和3D重建的基准测试套件。

Result: 多模态数据使用使深度估计的RMSE提升高达10%，RGB-LiDAR融合使3D重建的Chamfer距离改善12%。数据集和基准测试的发布促进了跨领域鲁棒多模态感知系统的发展。

Conclusion: WildDepth数据集填补了动物深度估计领域缺乏度量尺度数据的空白，通过多模态融合显著提升了深度估计和3D重建的精度，为跨领域泛化的鲁棒感知系统提供了基础。
...
TL;DR: 该研究系统评估了三种数据为中心的方法在处理遥感数据标签噪声方面的表现，通过在不同噪声水平下测试这些方法，证明了它们在识别噪声标签和提升任务性能方面的价值。


<details>
  <summary>Details</summary>
Motivation: 遥感数据集中普遍存在标签噪声问题，但该领域对标签噪声的自动化处理方法关注不足，缺乏对数据为中心方法的系统性分析，这些方法不仅能处理标签噪声，还能明确识别和隔离噪声标签。

Method: 研究选取了三种数据为中心的方法，在两个基准数据集上注入不同噪声类型（10-70%噪声水平），分析这些方法过滤标签噪声的效果及其对任务性能的影响。

Result: 分析结果明确证明了数据为中心方法在标签噪声识别和任务性能提升两方面的价值，提供了根据不同设置和目标选择最佳方法的见解，并指出了将数据为中心方法应用于遥感数据时仍需研究的方向。

Conclusion: 该研究在桥接数据为中心标签噪声方法的方法论建立与其在遥感领域实际应用方面迈出了一步，为遥感数据处理中的标签噪声问题提供了系统性的解决方案框架。
...
TL;DR: 该研究发现视觉Transformer（ViT）存在位置偏差问题，特别是在材料科学等领域的零样本适应中，通过使用ALiBi相对位置编码进行微调来减少这种偏差，同时保持模型的语义能力。


<details>
  <summary>Details</summary>
Motivation: 视觉Transformer（特别是像DINOv2这样的特征基础模型）虽然能学习丰富的表示，但其架构选择（如位置编码）可能导致模型表现出与语义内容无关的位置偏差和伪影。这在材料科学等领域造成零样本适应困难，因为这些领域的图像通常是均匀微观结构的横截面（没有首选方向）。

Method: 通过线性探测研究ViT中的位置偏差，发现这种偏差存在于多种目标和位置编码中。然后通过微调模型使用ALiBi相对位置编码来减少这种偏差。

Result: 经过ALiBi微调的模型保留了理想的通用语义能力，其无偏特征可以成功用于复杂显微镜图像的可训练分割任务。

Conclusion: 位置偏差是ViT模型中的一个普遍问题，但可以通过使用ALiBi相对位置编码进行微调来有效减少，同时保持模型在下游任务中的性能，特别是在材料科学等需要位置无关表示的领域中。
...
TL;DR: M^3提出了一种增强多视图基础模型的方法，通过添加匹配头实现精细密集对应，并集成到单目高斯溅射SLAM中，显著提升了未标定单目视频流的姿态估计和场景重建精度。


<details>
  <summary>Details</summary>
Motivation: 未标定单目视频流重建面临挑战，需要高精度姿态估计和动态环境中的高效在线优化。现有方法中，多视图基础模型以前馈方式估计姿态，产生的像素级对应缺乏几何优化所需的精度。

Method: M^3在多视图基础模型中添加专用匹配头以实现精细密集对应，将其集成到鲁棒的单目高斯溅射SLAM框架中，并引入动态区域抑制和交叉推理内参对齐来增强跟踪稳定性。

Result: 在多种室内外基准测试中，M^3在姿态估计和场景重建方面达到最先进精度。相比VGGT-SLAM 2.0，ATE RMSE降低64.3%；在ScanNet++数据集上，PSNR比ARTDECO高出2.11 dB。

Conclusion: M^3通过增强多视图基础模型的匹配能力并集成到SLAM框架中，有效解决了未标定单目视频流重建的精度瓶颈，在姿态估计和场景重建方面均取得显著改进。
...
<details>
  <summary>Details</summary>
Motivation: 现有VSR方法在推理时像黑盒子，用户无法可靠地修正意外伪影，只能接受模型输出。需要一种允许用户交互控制的方法。

Method: 提出关键帧条件化的潜空间-像素两阶段训练管道，融合低分辨率视频潜空间与稀疏编码的高分辨率关键帧潜空间，学习跨空间传播和细节细化。

Result: 在多个VSR基准测试中，在CLIP-IQA、DOVER和MUSIQ指标上分别比基线提升24.6%、21.8%和5.6%，同时展示出更好的时间一致性。

Conclusion: SparkVSR实现了可控制的关键帧驱动视频超分辨率，并且是一个通用的交互式视频处理框架，可应用于老电影修复和视频风格迁移等任务。
...
TL;DR: 提出MessyKitchens数据集和Multi-Object Decoder方法，用于改进杂乱场景中的多物体3D重建，解决物体穿透和接触不准确的问题。


<details>
  <summary>Details</summary>
Motivation: 单目3D场景重建虽然已有进展，但将复杂场景分解为独立3D物体仍面临挑战，包括物体多样性、频繁遮挡和复杂关系。机器人学和动画应用需要物理上合理的场景重建，要求物体遵循非穿透和真实接触的物理原则。

Method: 1) 引入MessyKitchens数据集，提供杂乱真实场景的高保真物体级标注（3D形状、姿态和准确接触）；2) 基于SAM 3D单物体重建方法，扩展Multi-Object Decoder（MOD）进行联合物体级场景重建。

Result: MessyKitchens数据集在配准精度和物体间穿透方面显著优于先前数据集。MOD方法在三个数据集上的多物体重建均表现出对现有技术的持续显著改进。

Conclusion: 通过新数据集和MOD方法，推进了物体级场景重建，为物理上合理的3D场景重建提供了更好的基准和工具。
...
TL;DR: SegviGen利用预训练3D生成模型的结构化先验，通过部件着色实现3D部件分割，在有限监督下显著超越现有方法


<details>
  <summary>Details</summary>
Motivation: 现有3D部件分割方法存在两个主要问题：基于2D先验的方法存在跨视图不一致和边界模糊问题；原生3D判别分割方法需要大规模标注数据和大量训练资源。SegviGen旨在利用预训练3D生成模型的结构化先验，以更高效的方式实现3D部件分割。

Method: SegviGen通过编码3D资产并在几何对齐重建的活跃体素上预测部件指示性颜色来实现分割。该方法支持交互式部件分割、完整分割以及带2D引导的完整分割的统一框架。
...
TL;DR: 视频生成模型中的推理能力主要沿扩散去噪步骤展开，而非先前认为的跨帧序列推理。研究发现模型在早期去噪步骤探索多种候选解，逐步收敛到最终答案，这一过程被称为"步骤链"机制。


<details>
  <summary>Details</summary>
Motivation: 挑战先前关于视频生成模型中推理机制的传统假设。先前工作认为推理通过跨视频帧的序列展开（帧链机制），但本文研究发现这一假设不成立，需要揭示视频模型中推理能力的真实工作机制。

Method: 通过定性分析和针对性探测实验，研究视频生成模型的推理过程。具体包括：1）分析推理沿扩散去噪步骤的展开方式；2）识别关键推理行为；3）揭示扩散变换器中的功能专业化；4）提出基于相同模型不同随机种子的潜在轨迹集成策略。

Result: 发现推理主要沿扩散去噪步骤展开（步骤链机制），而非跨帧序列。识别出三个关键推理行为：工作记忆、自我纠正与增强、感知先于行动。揭示扩散变换器中存在自演化的功能专业化。提出的训练免费集成策略能改善推理能力。

Conclusion: 视频生成模型中的推理能力通过扩散去噪步骤的步骤链机制实现，而非传统的帧链机制。这一发现为理解视频模型推理提供了系统框架，为未来研究利用视频模型固有推理动态作为智能新基质奠定了基础。
...
TL;DR: 该论文提出了一种基于相机姿态的交互式游戏世界模型，通过将用户动作映射到6自由度相机姿态来提升动作控制精度和长期3D一致性。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散变换器方法在交互式游戏世界建模中存在两个主要问题：1）动作控制精度不足，将用户动作视为抽象条件信号，忽略了动作与3D世界之间的几何耦合关系；2）长期3D一致性差，无法保持长时间导航中的空间一致性。

Method: 1）定义基于物理的连续动作空间，在Lie代数中表示用户输入以推导精确的6自由度相机姿态，通过相机嵌入器注入生成模型确保动作对齐；2）使用全局相机姿态作为空间索引检索相关历史观测，实现长期导航中几何一致的位置重访。

Result: 构建了包含3000分钟真实人类游戏数据的大规模数据集，标注了相机轨迹和文本描述。实验表明，该方法在动作可控性、长期视觉质量和3D空间一致性方面显著优于现有最先进的交互式游戏世界模型。

Conclusion: 相机姿态作为统一的几何表示能够有效联合支撑即时动作控制和长期3D一致性，为交互式游戏世界建模提供了更精确和一致的方法。
