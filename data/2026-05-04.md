...
TL;DR: 本文提出AIDA（SF-MIDA）框架，通过自适应中间域生成与伪镜像正则化，解决多源和无源场景下行人重识别的域适应问题。


<details>
  <summary>Details</summary>
Motivation: 现有中间域方法（如IDM、IDM++）依赖固定混合策略和联合源-目标域访问，无法适用于多源和无源场景，导致性能下降。

Method: 提出自适应中间域适应框架（AIDA/SF-MIDA）：1）多源中间域生成器合成多样化中间表征；2）基于模型不确定性和训练稳定性的反馈信号动态调控特征混合与正则化强度；3）伪镜像正则化策略保持域扰动下的身份一致性。

Result: 在域泛化和无源设置下的广泛实验证明该框架的有效性。

Conclusion: AIDA通过动态自适应机制克服了固定混合策略的局限，为多源和无源域适应行人重识别提供了有效方案。
...
TL;DR: 本文提出了一套面向神经形态平台（Intel Loihi 2）的通用SNN检测架构设计方法，并在帧基和事件基数据集上进行了目标检测基准测试。结果表明，Loihi 2上的SNN能以最低的动态能耗实现实时检测，结合蒸馏训练可恢复87-100%的ANN检测精度，验证了神经形态系统在边缘端能效实时目标检测中的潜力。


<details>
  <summary>Details</summary>
Motivation: 能效受限平台（如无人机、移动机器人）上的实时目标检测至关重要，SNN+神经形态硬件被认为比传统ANN更节能，但缺乏系统的SNN检测架构设计方法及实际神经形态处理器上的部署与基准测试。

Method: 提出通用SNN检测架构设计方法，并进行工程适配部署于Loihi 2；采用ANN-to-SNN蒸馏感知训练；在帧基和事件基数据集上，与Jetson Orin Nano、Jetson Nano B01、Apple M2上的ANN进行对比。

Result: Loihi 2上的SNN实现了实时检测，单次推理动态能耗最低；功耗优于所有对比平台；蒸馏训练使SNN恢复87-100%的ANN检测精度，无蒸馏时精度下降11-27%；Jetson Orin Nano推理速率更高。

Conclusion: 神经形态系统在边缘端能效实时目标检测方面具有显著潜力，Loihi 2在能耗和功耗上优势明显，蒸馏训练有效弥合了SNN与ANN之间的精度差距。
...
TL;DR: 提出了一种利用单目巡检影像对非合作空间物体进行神经隐式表面重建的流程，成功应用于真实在轨巡检视频数据。


<details>
  <summary>Details</summary>
Motivation: 现有神经隐式表面重建方法多局限于合成数据或已知相机位姿的硬件在回路数据，缺乏对真实在轨巡检影像中非合作空间物体的重建能力。

Method: 采用基于分割的背景去除实现相机位姿估计，并引入每帧曝光变化的 photometric correction，结合神经隐式表面重建管线。

Result: 在STS-119任务的ISS巡检影像和H-IIA火箭上面级的在轨巡检影像上成功实现重建，发现背景去除对位姿估计至关重要，光照特性影响阴影区域重建效果。

Conclusion: 该管线有效解决了真实在轨影像中的背景变化和曝光变化问题，为非合作空间物体的在轨重建提供了可行方案。
...
TL;DR: VkSplat 是一个完全基于 Vulkan compute 实现的高性能、跨厂商 3D Gaussian Splatting 训练管线，相比 CUDA+PyTorch 基线实现了 3.3 倍加速和 33% 的显存降低，同时保持渲染质量并兼容不同 GPU 厂商。


<details>
  <summary>Details</summary>
Motivation: 现有 3DGS 训练管线存在性能瓶颈和厂商兼容性限制，缺乏高性能且跨平台的实现方案。

Method: 完全使用 Vulkan compute 实现 3DGS 训练管线，并采用多种优化策略提升性能与降低显存占用。

Result: 相比 CUDA+PyTorch 基线实现 3.3 倍速度提升和 33% 显存减少，且保持渲染质量不变，兼容多厂商 GPU。

Conclusion: VkSplat 是首个基于 Vulkan 的全栈 3DGS 训练管线，达到了当前最优性能，证明了 Vulkan 在 3DGS 训练中的潜力。
...
TL;DR: 针对AR/辅助设备的自我中心姿态估计，标准共形预测（CP）在困难帧上存在约30%的条件覆盖差距。本文提出基于DINOv2-Bridge的自适应CP方法，仅需单源参与者训练即可跨参与者迁移，将困难帧（Q4）覆盖率从~0.75提升至~0.93，同时保持整体90%目标覆盖率。


<details>
  <summary>Details</summary>
Motivation: AR和辅助设备中的自我中心姿态估计不仅需要准确预测，更需要保证的不确定性区域。标准共形预测（CP）虽然提供无需重新训练的保证，但在困难帧上存在严重的条件覆盖差距（最难的25%帧仅覆盖约60%），这在实际应用中是不可接受的。

Method: 提出DINOv2-Bridge自适应CP方法：两阶段难度估计器，仅使用单源参与者训练，测试时无需任何图像即可跨参与者迁移。同时引入测地线SE(3)非一致性分数，比欧氏评分更能识别物理上更困难的帧。

Result: 在EPIC-Fields数据集上（12名参与者、3个预测器、3个时间范围，共108组评估），标准CP在Q4帧上仅覆盖~60%，存在约30%的条件覆盖差距。测地线SE(3)分数与欧氏分数在Q4上仅有15-26%重叠，测地线Q4帧的地面真实相机位移高出2-3倍。提出的方法将Q4覆盖率从~0.75提升至~0.93。

Conclusion: DINOv2-Bridge自适应CP有效解决了自我中心姿态估计中困难帧的条件覆盖差距问题，实现了跨参与者的零样本迁移，为AR和辅助设备提供了更可靠的姿态估计保障。
...
TL;DR: MAEPose是一种基于掩码自编码的毫米波雷达人体姿态估计方法，直接处理光谱视频流，无需预提取中间表示，利用无标签数据进行预训练，在三个数据集上优于现有方法最高达22.1%。


<details>
  <summary>Details</summary>
Motivation: 现有毫米波雷达人体姿态估计方法依赖预提取的稀疏点云或光谱图等中间表示，丢弃了雷达视频流中丰富的时空信息，且信号处理增加系统复杂度；同时现有方法多为端到端有监督学习，无法利用无标签原始视频流学习通用表示。

Method: 提出MAEPose方法，直接对毫米波光谱视频进行掩码自编码预训练，学习时空运动感知的通用表示；利用热力图解码器进行多帧姿态估计预测。

Result: 在三个数据集上采用留一人交叉验证，MAEPose在MPJPE指标上优于最先进基线最高达22.1%（p<0.05）；零样本旁观者干扰下仅增加6.5%误差；消融实验证实预训练和热力图解码器均有显著贡献；模态分析表明Range-Doppler视频输入优于Range-Azimuth及其融合，且计算成本更低。

Conclusion: MAEPose通过掩码自编码直接处理雷达光谱视频流，有效利用无标签数据学习通用表示，实现了更优的姿态估计性能和鲁棒性，且具有隐私保护优势。
...
TL;DR: 提出了一种名为Remote SAMsing的开源流水线，在不修改SAM2模型或需要训练数据的情况下，解决了SAM2在遥感大场景中面临的质量-覆盖权衡和图块碎片化问题，将覆盖率从30-68%提升到91-98%。


<details>
  <summary>Details</summary>
Motivation: SAM2在自然图像上表现优异，但应用于遥感大场景时存在两个问题：(1) 掩码生成器存在内在的质量-覆盖权衡——严格阈值得到精确掩码但覆盖率低，放松阈值则牺牲质量；(2) 大图像必须分块处理，导致物体在块边界处被碎片化。

Method: (1) 多轮次算法：在每个图块上反复运行SAM2，每轮将已接受的掩码涂黑，在覆盖率增长停滞时才放松质量阈值，确保先捕获最精确的掩码；(2) 上下文填充和无参数最佳匹配合并机制，重构跨图块边界的碎片化物体。

Result: 在7个场景（5cm到4.78m GSD）上，覆盖率从30-68%提升到91-98%；建筑检测Det@0.5达95%，车辆达82-93%；分割边界精度比SLIC和Felzenszwalb基线高3-8倍；图块大小可作为隐式尺度参数，从1000降到250时Det@0.5从56%升至85%；在19.4亿像素的波茨坦马赛克上实现97%覆盖率且无质量下降。

Conclusion: Remote SAMsing是一个无需训练数据、不修改SAM2的通用流水线，有效解决了SAM2在遥感大场景中的覆盖率和碎片化问题，且泛化能力强，能够处理生产级大尺度遥感图像。
...
TL;DR: 本文通过构建可控的MOSAIC数据集，系统研究了文本到图像扩散模型在多目标生成中失败的根本原因，发现场景复杂度而非概念不平衡是主导因素，计数在低数据场景下最难学习，且组合泛化随着训练中概念组合的缺失而崩溃。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型在视觉保真度上表现优异，但在多目标生成中不可靠。尽管已有大量经验证据表明这些失败，其根本原因仍不明确。本文旨在探究数据本身对这一限制的影响程度。

Method: 引入MOSAIC（Multi-Object Spatial relations, AttrIbution, Counting）可控数据集生成框架，在不同数据集规模下设置两种训练范式：概念泛化（单个概念在训练中出现但分布不均）和组合泛化（特定概念组合被系统性地排除），训练扩散模型进行分析。

Result: 场景复杂度（而非概念不平衡）是多目标生成失败的主导因素；计数任务在低数据场景下最难学习；随着训练中更多概念组合被排除，组合泛化性能崩溃。

Conclusion: 这些发现揭示了扩散模型在多目标组合生成方面的根本性局限，亟需更强的归纳偏置和精心设计的数据来提升其鲁棒性。
...
TL;DR: 本文系统研究了基于Vision Transformer（ViT）的时空植被像素分类方法，在保持分类性能的同时，计算效率比传统多时相CNN提升一个数量级，参数复杂度不随时间序列长度增加，为资源受限的物候监测提供了可扩展的解决方案。


<details>
  <summary>Details</summary>
Motivation: 现有基于多时相CNN的物候监测方法存在架构僵化、难以扩展至长时间序列、需要大空间上下文窗口等局限，亟需一种计算高效且可扩展的时空分类方法。

Method: 对Vision Transformer进行了七个关键设计维度的消融研究，包括数据归一化、光谱排列、边界处理、空间上下文窗口形状与大小、令牌化策略、位置编码和特征聚合策略，并在巴西塞拉多生物群落的两个数据集（航空影像和近地表影像）上评估。

Result: ViT方法在保持竞争性分类性能的同时，FLOPs降低一个数量级，且参数复杂度不随时间序列长度变化（恒定），而CNN基线呈线性增长。
...
TL;DR: 本文提出GeoSR-Bench，首个将超分辨率（SR）与下游地球观测任务（如土地覆盖分类、基础设施测绘等）直接结合的基准数据集。实验表明，传统SR指标（PSNR/SSIM）与下游任务性能相关性差甚至负相关，揭示了将下游任务纳入SR模型评估的必要性。


<details>
  <summary>Details</summary>
Motivation: 现有超分辨率研究主要使用PSNR、SSIM等保真度指标评估模型，但这些指标无法反映超分辨率图像在下游实际任务（如土地覆盖分类、生物量估算、变化检测等）中的真实效用，存在评估与实用脱节的问题。

Method: 构建GeoSR-Bench基准数据集，包含约36,000个位置的空间对齐、时间对齐、质量受控的图像对，分辨率从500m到0.6m。在此基础上，对GAN、Transformer、神经算子、扩散模型等9种SR模型进行270种实验设置（涵盖2个跨平台SR任务、3个下游任务模型、5个下游任务）的综合评估。

Result: 传统SR指标（PSNR/SSIM）的提升往往与下游任务性能增益不相关，甚至存在负相关关系，表明这些指标对选择适用于下游任务的优越模型指导有限。

Conclusion: 需要将下游任务整合到超分辨率模型的开发与评估过程中，而非仅依赖传统保真度指标。
...
TL;DR: OSCAR是一个在线自我校准框架，利用LVLM自身的生成-判别差距，通过蒙特卡洛树搜索和双粒度奖励机制构建偏好数据，迭代优化模型以减少幻觉，无需依赖更强模型的外部监督。


<details>
  <summary>Details</summary>
Motivation: 现有的偏好对齐方法通常依赖GPT等更强模型蒸馏的离线监督，导致学生模型被迫对齐超出自身感知能力的细节，产生"监督-感知不匹配"问题，模型学会猜测而非真正看见。

Method: 提出OSCAR框架，识别并利用LVLM的生成-判别差距（模型在判别性验证上比开放式生成更准确），结合蒙特卡洛树搜索与双粒度奖励机制构建在线偏好数据，通过直接偏好优化（DPO）迭代式微调模型。

Result: 在幻觉评测基准上达到最先进性能，同时提升通用多模态能力。

Conclusion: OSCAR通过在线自我校准有效解决了监督-感知不匹配问题，在不依赖外部强模型的情况下减少幻觉，证实了利用模型自身判别能力进行自我提升的可行性。
...
TL;DR: 提出PAD（Pose-Aware Diffusion），一种端到端扩散框架，通过在观察空间直接合成3D几何体，解决解耦范式中的空间不匹配和姿态歧义问题，实现高保真姿态对齐的3D资产生成。


<details>
  <summary>Details</summary>
Motivation: 传统解耦式"先标准后旋转"范式存在空间不匹配和变换歧义，导致生成姿态对齐的3D物体具有挑战性。

Method: 将单目深度图反投影为部分点云，作为3D几何锚点显式注入扩散模型，直接在观察空间端到端合成3D几何体，抛弃标准姿态假设。

Result: 在几何对齐和图像到3D对应方面优于现有最先进方法，并能自然地扩展到组合式3D场景重建，保持精确的空间布局。

Conclusion: PAD通过原生在观察空间生成几何体，从根本上解决了姿态歧义问题，是生成高保真姿态对齐3D资产的有效框架。
...
TL;DR: 本文提出CURE-OOD基准，系统评估癌症生存预测中由成像采集协变量偏移引起的分布外检测问题，发现协变量偏移会显著降低生存预测性能，且传统分类导向的OOD检测方法在此任务中可能失效。


<details>
  <summary>Details</summary>
Motivation: 癌症患者迫切想知道生存期，但现有生存预测方法在成像采集变化导致的分布偏移下可靠性不足，且缺乏系统研究OOD检测的基准。

Method: 构建CURE-OOD基准，定义基于扫描参数的训练集、分布内测试集和分布外测试集，覆盖四个生存预测任务，并引入HazardDev作为生存感知的OOD检测基线。

Result: 协变量偏移显著降低生存预测性能；主流分类OOD检测器在生存预测中可能失效。

Conclusion: CURE-OOD能系统分析分布偏移对下游生存性能及OOD可检测性的影响，为未来鲁棒生存预测研究提供基础。
...
TL;DR: 本文提出了一种基于流匹配（flow matching）模型的遥感影像超分辨率方法，实现了Sentinel-2影像4倍超分辨率（10米→2.5米），在感知质量与光谱保真度间取得了良好平衡，并生成了覆盖美国本土的2.5米4波段超分辨率影像产品（超1.58万亿像素），在土地覆盖分类任务中达到了89.11%的总体精度。


<details>
  <summary>Details</summary>
Motivation: 在卫星影像超分辨率中，光谱保真度与感知质量之间存在常见的权衡问题。现有扩散模型和GAN方法难以同时兼顾二者，作者希望探索流匹配模型能否更有效地平衡这一矛盾，从而生成高质量超分辨率影像。

Method: 使用流匹配（flow matching）模型对Sentinel-2的10米可见光和近红外波段进行4倍超分辨率，采用120,851对同日获取的10米Sentinel-2与2.5米重采样NAIP影像作为训练数据，利用Euler方法单步采样和Midpoint二阶求解器进行推理，并与扩散模型和Real-ESRGAN进行对比。

Result: 流匹配模型在单步Euler采样中像素精度优于扩散和Real-ESRGAN；使用Midpoint求解器仅需20步即可生成感知真实的超分辨率影像；基于2025年Sentinel-2年度合成数据生成了超1.58万亿像素的2.5米4波段CONUS产品；在切萨皮克湾流域2020-2025年土地覆盖分类中，基于25,000个地面真值点评估达到89.11%总体精度。

Conclusion: 流匹配是遥感影像超分辨率中一种有效的生成式建模方法，优于扩散模型和GAN方法，能够在推理时动态调整感知-失真权衡而无需重新训练，对需要高空间精度的地理空间应用具有重要推广价值。
...
TL;DR: 针对DeepSeek-OCR模型的视觉token冗余问题，提出RTPrune两阶段剪枝方法，通过优先保留高范数token和最优传输合并剩余token，实现高效推理，在OmniDocBench上达到99.47%准确率和1.23倍加速。


<details>
  <summary>Details</summary>
Motivation: DeepSeek-OCR中视觉token存在冗余的文本和结构信息，现有VLM剪枝方法无法保留文本保真度，需要针对OCR任务的token剪枝方法。

Method: 提出RTPrune两阶段剪枝：第一阶段优先保留捕获关键文本/结构信息的高范数视觉token；第二阶段基于最优传输理论配对合并剩余token；并引入动态剪枝比，根据token相似度和文本密度自适应调整。

Result: 在OmniDocBench上达到99.47%准确率，预填充速度提升1.23倍，token保留率为84.25%，实现SOTA性能。

Conclusion: RTPrune通过洞察DeepSeek-OCR的两阶段阅读轨迹，实现高效准确的token剪枝，为OCR任务提供了更好的效率-精度平衡。
...
TL;DR: 本文提出SIMON框架，通过显著性感知的多视角方法解决EEG-to-image检索中的中心偏置问题，在THINGS-EEG数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有EEG-to-image检索方法假设固定中心视图，与人类基于内容的注意力机制存在几何-语义分离，导致视觉特征与EEG响应不匹配。

Method: 提出SIMON框架，包含显著性感知采样（SAS）用于选择注视中心，结合前景分割和显著性预测生成强调目标区域、抑制背景杂波的foveated视图。

Result: 在THINGS-EEG上，被试内Top-1准确率69.7%，被试间19.6%，均超越现有基线。在采样粒度、EEG通道拓扑和视觉/脑编码器骨干上均表现鲁棒。

Conclusion: 显著性感知的多视角融合策略有效缓解了中心偏置问题，提升了零样本EEG-to-image检索性能。
...
TL;DR: BOLT是一种轻量级即插即用模块，通过以自我为中心的教师蒸馏在线适应异构协同感知中的邻居特征，无需预部署协调或联合训练，显著提升了准备自由的异构协同感知性能。


<details>
  <summary>Details</summary>
Motivation: 现有异构协同感知方法依赖离线联合训练或定制适配等预处理，这在现实场景中不实用（智能体由不同开发者独立训练，偶然在线相遇），导致直接跨智能体融合性能甚至不如单智能体感知。

Method: 提出BOLT，一种轻量级即插即用模块，采用ego-as-teacher蒸馏策略在线适应邻居特征：利用高置信度的自我感知特征引导跨智能体特征域对齐，同时允许邻居在自我低置信度区域贡献特征。仅需自我预测，无需真实标签。

Result: 仅0.9M可训练参数，在准备自由设置下相比未适应的融合，AP@50提升高达32.3点。在DAIR-V2X和OPV2V数据集上，不同编码器对和融合策略下均一致优于仅自我感知结果。

Conclusion: BOLT有效解决了准备自由异构协同感知中的特征对齐问题，无需预部署协调即可实现显著的性能提升，具有实用性和普适性。
...
TL;DR: 本文提出LIMSSR框架，利用大语言模型在不依赖完整训练数据的情况下处理多模态学习中的模态缺失问题，通过序列推理、上下文感知模态补全和多维表示融合等策略，在动作质量评估任务上显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有不完整多模态学习方法通常假设训练时模态完整（"上帝视角"），这在实际场景中不现实。本文旨在解决训练阶段模态也不完整这一更具挑战性的问题，避免对完整数据的依赖。

Method: 将不完整多模态学习重新定义为条件序列推理任务；利用大语言模型的语义推理能力，通过Prompt引导的上下文感知模态补全和多维表示融合来推断潜在语义；引入掩码感知双路径聚合机制动态校准推理不确定性，缓解幻觉问题。

Result: 在三个动作质量评估数据集上，LIMSSR显著优于现有最先进方法，且无需依赖完整训练数据。

Conclusion: LIMSSR为数据高效的多模态学习建立了新范式，证明了在训练阶段模态缺失条件下利用大语言模型进行有效学习的可行性。
...
TL;DR: 提出MACF，一种端到端多智能体协作框架，通过解耦感知预算与视频复杂度，利用潜在通信协议实现可扩展的视频理解，在多种基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 多模态大模型在长视频任务中受限于感知上下文预算，现有基于规则的方法存在信息丢失、成本高、依赖文本中间表示等问题。

Method: 将视频分割为片段分配给局部预算的智能体，通过智能体原生潜在通信协议实现整体推理，采用课程学习策略逐步强化语义对齐、证据总结和跨智能体协调。

Result: 在多种视频理解基准测试中，在相同预算约束下，MACF一致优于最先进的多模态大模型和多智能体系统。

Conclusion: MACF的潜在协作机制能够在不丢失视觉保真度的前提下，实现高效且可扩展的长视频理解。
...
TL;DR: 提出CT-Lite框架，通过JPEG压缩CT图像和特征蒸馏技术，在显著降低计算资源和参数量的情况下，实现接近无损输入的胸部异常检测性能。


<details>
  <summary>Details</summary>
Motivation: 医疗AI部署受限于高计算复杂度和体积数据的资源密集处理，JPEG压缩CT体积可用于低资源部署和高效数据传输，但压缩会导致信息损失。

Method: 提出FAST蒸馏框架（Gram矩阵注意力风格保留+双注意力特征对齐）将高保真CT表征迁移到压缩输入编码器；提出SFP（基于Block Tensor Train分解的参数高效投影层）；结合SigLIP多模态对齐目标构建CT-Lite对比学习管道。

Result: 在CT-RATE、NIDCH和Rad-ChestCT三个数据集上，CT-Lite的AUROC仅比无损输入基线低5-7%，同时参数显著减少。

Conclusion: 证明了在资源受限条件下，通过压缩输入和高效蒸馏可实现接近无损性能的AI临床评估，为低资源部署场景铺平了道路。
...
TL;DR: 本文提出一个以实例级有效感受野（iERF）为核心的统一可解释性框架，从局部（SRD）、全局（CAFE）和机制（ICAT）三个层面统一了视觉模型的可解释性分析，并在多种主流视觉模型上验证了其高保真度与鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉模型可解释性方法在局部、全局和机制层面各自独立，缺乏统一的分析单元和分析框架，导致对模型"证据从何而来、编码了什么、如何组合计算"等问题无法给出连贯一致的答案。

Method: 以点级特征向量（PFV）及其实例级有效感受野（iERF）为核心分析单元；局部层面提出共享比率分解（SRD）传播iERF构造类判别显著性图；全局层面提出概念锚定特征解释（CAFE），利用iERF作为语义标签解释潜在向量；机制层面提出层间概念图及层间概念归因（ICAT），量化概念间影响。

Result: 在ResNet50、VGG16、ViTs上，该框架在忠实度和鲁棒性上均优于基线方法，成功解释了分散的稀疏自编码器特征，并揭示了正确分类、误分类和对抗样本中的主导概念路径。

Conclusion: 基于iERF的框架提供了一条从像素到概念再到决策的连贯、有证据支持的可解释性地图，统一了视觉模型不同层面的可解释性分析。
...
TL;DR: 提出一种结合细粒度人工标注与粗粒度VLM弱标签的主动学习框架，通过实例级标签分配和噪声建模，在有限标注预算下提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有主动学习仍依赖大量人工标注，VLM在细粒度任务上表现不稳定，但能提供准确的粗粒度标签。利用这一特性，将粗粒度VLM弱标签与细粒度人工标注结合，降低人工标注成本。

Method: 1. 观察到VLM在细粒度识别任务中，粗粒度标签准确但细粒度标签不可靠；2. 提出主动学习框架，对每个实例进行动态标签分配，结合人工细粒度标签与VLM粗粒度弱标签；3. 利用少量可信完整标签对VLM生成标签中的系统噪声进行建模。

Result: 在CUB200和FGVC-Aircraft数据集上，所提框架在相同标注预算下持续优于现有主动学习方法。

Conclusion: 通过合理利用VLM的粗粒度标签能力并建模其噪声，可有效减少主动学习中对人工细粒度标注的依赖，提升标注效率。
...
TL;DR: 本文研究时间分辨率对零样本高速人体动作语义理解的影响，通过剑道动作识别实验证明高帧率（120Hz）比低帧率能显著提升语义可分性和表征稳定性。


<details>
  <summary>Details</summary>
Motivation: 高速、不常见的动作因标注困难，难以收集足够的有监督数据，亟需零样本方法实现语义理解；然而时间分辨率对高速动作零样本理解的影响尚未被充分探索。

Method: 提出无训练流水线：预训练视频-语言模型用于语义表征 + 大语言模型进行成对动作推理。在剑道数据集上，以120Hz、60Hz、30Hz三种帧率进行受控实验，并分析基于跟踪的人体关节信息在完整/部分观测场景下的作用。

Result: 高时间分辨率（120Hz）显著提升零样本设置下的语义可分性；最近类原型策略的定量评估表明高速视频能提供更稳定、可解释的语义表征。

Conclusion: 时间分辨率对无训练动作识别至关重要，高速感知可增强零样本语义理解能力，为快速动作的语义理解研究提供了新方向。
...
TL;DR: 本文提出GOR-IS框架，通过将场景分解为内在组件并显式建模光传输，实现了物理一致且视觉连贯的3D物体移除，在LPIPS和PSNR上分别提升13%和2dB。


<details>
  <summary>Details</summary>
Motivation: 现有3D物体移除方法虽然提升了修复一致性，但忽略了全局光照效应，导致物理上不合理的结果；同时难以处理视角依赖的非朗伯表面，导致修复不可靠。

Method: 将场景分解为内在组件（材质与光照），显式建模光传输以保持全局光照一致性；提出内在空间修复模块，直接在材质和光照域进行操作，以应对非朗伯表面挑战。

Result: 在合成和真实数据集上，相比现有方法，LPIPS提升13%，PSNR提升2dB，物理一致性和视觉连贯性均显著改善。

Conclusion: GOR-IS通过内在空间分解与光传输建模，有效解决了物体移除中的光照一致性和非朗伯表面问题，是实现物理真实3D编辑的重要进展。
...
TL;DR: 本文提出了一种端到端的联合训练框架，将视觉分词器与自回归生成模型共同优化，使得生成结果可以直接监督分词器的训练，相比传统的两阶段训练方法取得了更优的生成效果，在ImageNet 256x256上达到了1.48的FID分数。


<details>
  <summary>Details</summary>
Motivation: 传统的自回归图像建模采用两阶段方法，先训练视觉分词器将图像压缩为潜在表示，再单独训练生成模型，两者缺乏联合优化，导致分词器的训练目标与生成任务之间存在偏差。

Method: 设计了一个端到端的训练流程，联合优化图像重建与生成任务，使生成结果的损失可以直接反向传播到分词器；同时探索利用视觉基础模型来改进一维分词器在自回归建模中的表现。

Result: 在ImageNet 256x256图像生成任务上取得了1.48的FID分数，达到了当时的最高水平（state-of-the-art），且无需使用分类器引导采样。

Conclusion: 端到端的联合训练框架能有效弥合分词器与生成模型之间的优化目标差异，视觉基础模型的引入进一步提升了一维分词器的表征能力，为自回归图像生成提供了一种更高效的新范式。
...
TL;DR: 提出PhysiGen，一种计算高效的碰撞感知物理约束优化策略，用于减少多人交互生成中的身体穿透问题。


<details>
  <summary>Details</summary>
Motivation: 现有文本驱动的3D人体运动生成模型在多人交互序列中普遍存在身体相互穿透问题，严重影响真实感和可用性，而之前的方法要么忽略该问题，要么引入计算昂贵的网格级损失函数。

Method: 将高分辨率人体网格简化为几何基元以降低碰撞检测成本，识别碰撞区域作为优化方向指导，设计即插即用的物理约束优化策略PhysiGen。

Result: 跨数据集和跨模型实验表明，PhysiGen能有效减少相互穿透，显著提升视觉连贯性和物理合理性，优于现有方法。

Conclusion: PhysiGen是一种通用、高效且即插即用的碰撞感知优化方法，可集成到现有交互生成模型中提升生成质量。
...
TL;DR: 本文提出IdentiFace框架，通过多模态输入和迭代生成管道解决嫌疑人面部生成中的条件模糊和采样方差问题，在身份检索方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统素描画像效率低、质量差，而现有扩散模型存在文本到图像的条件模糊性和单次生成的采样方差问题，难以生成可识别的嫌疑人面部。

Method: 提出IdentiFace框架：(1)多模态输入设计增强条件控制；(2)迭代生成管道实现可识别特征调整；同时引入面部身份损失函数和两个任务特定数据集。

Result: 在合成数据集和真实场景中的实验表明，IdentiFace在身份检索等指标上优于现有方法，展现出强大的实际应用潜力。

Conclusion: IdentiFace有效解决了嫌疑人面部生成的关键问题，为刑侦实践提供了可行的技术方案。
...
TL;DR: 本文研究扩散模型中输入噪声的特性，发现低频成分决定图像全局结构和色彩，高频成分控制细节，并提出一种无需训练的简单方法，通过操纵低频噪声来引导图像生成。


<details>
  <summary>Details</summary>
Motivation: 白色高斯噪声虽能保证生成多样性，但缺乏可解释性，难以控制和预测图像的特定视觉属性。

Method: 分析噪声频率特性，利用低频图像先验对低频噪声进行简单操作，从而在不影响高频细节的前提下引导生成过程。

Result: 低频噪声主导全局结构与色彩，高频噪声决定细节；通过低频噪声操控可有效重建低频视觉线索。

Conclusion: 提出一种简单、无需训练、计算开销小的噪声操控方法，能够在保持生成多样性的同时控制图像的整体结构和颜色。
...
TL;DR: 本文通过引入单目深度和法线先验来增强2DGS，提出深度引导的高斯初始化策略和基于聚类的退化高斯剪枝技术，在DTU数据集上实现了网格重建的最新成果，同时保持了高质量的新视角合成。


<details>
  <summary>Details</summary>
Motivation: 2DGS虽然能进行视图一致且几何精确的表面重建，但对高斯原始体的初始化敏感，依赖SfM初始化在困难图像集上效果不佳。

Method: 1) 引入单目深度和法线先验；2) 提出深度引导的高斯初始化策略；3) 提出基于聚类的退化高斯剪枝技术。

Result: 在DTU数据集上，网格重建达到最先进水平，同时保持高质量的新视角合成。

Conclusion: 融合单目先验能显著提升2DGS的几何精度和鲁棒性。
...
TL;DR: 本文发现视觉-语言模型（VLM）的视觉模态是绕过安全对齐的未被充分探索的攻击面，提出了四种视觉攻击方法，实验表明视觉攻击成功率远高于文本攻击，揭示了安全训练无法自动泛化到视觉模态的跨模态对齐漏洞。


<details>
  <summary>Details</summary>
Motivation: 现有VLM的安全对齐主要集中在文本模态，视觉模态作为攻击面被忽视，作者希望揭示视觉输入可以绕过文本安全训练的限制。

Method: 提出了四种视觉攻击方法：(1) 视觉密码（将有害指令编码为视觉符号序列），(2) 有害对象替换（如炸弹替换为香蕉），(3) 图像中有害文字替换为无害词，(4) 视觉类比谜题。在六个前沿VLM上评估攻击效果。

Result: 视觉攻击有效绕过了安全对齐，例如视觉密码在Claude-Haiku-4.5上攻击成功率达40.9%，而等效文本密码仅10.7%，表明文本安全训练无法自动泛化到视觉模态。

Conclusion: 鲁棒的VLM对齐需要将视觉视为安全后训练的一等目标，仅依赖文本模态的安全训练是不够的。
...
TL;DR: 提出双软提示微调（DSPT）方法，通过内在梯度抑制机制解决CLIP模型在标签噪声下的提示微调鲁棒性问题，无需超参数即可实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: CLIP的提示微调对标签噪声高度敏感，噪声样本会产生不成比例的大梯度，破坏预训练先验知识。现有方法依赖复杂架构或手工超参数，缺乏轻量级鲁棒方案。

Method: 提出Double-Softmax Prompt Tuning (DSPT)，通过顺序概率归一化构建自适应饱和区，自动抑制高错误率噪声样本的梯度，同时保留信息性更新。

Result: 在多种噪声基准上达到最先进的鲁棒性，超越使用复杂架构和手工超参数的方法。

Conclusion: 将传统训练瓶颈"梯度消失"转化为原则性的噪声过滤机制，为标签噪声下的提示微调提供了简单有效的即插即用方案。
...
TL;DR: 本文研究如何将V2X通信中的目标级信息集成到3D目标检测中，并提出噪声感知训练策略以提升系统对V2X不完美条件（如延迟、定位误差、低渗透率）的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 车载传感器存在视距和视野限制，在遮挡或能见度差时可能失效；V2X通信虽能补充感知，但现实中的不完美条件（如噪声、丢失）会影响系统可靠性，需研究如何鲁棒地融合V2X信息。

Method: 基于nuScenes数据集，从地面真值模拟目标级V2X消息并注入噪声和对象丢失；将消息转换为专用BEV输入，融合到BEVFusion风格检测器中；提出噪声感知训练策略和显式置信度编码。

Result: 理想条件下NDS达0.80；但基于理想数据训练的模型对V2X过度依赖且脆弱；噪声感知训练策略在严重噪声和低渗透率下仍能保持性能增益。

Conclusion: 目标级V2X信息能显著提升检测性能，但需通过噪声感知训练和置信度编码来增强对现实V2X不完美条件的鲁棒性。
...
TL;DR: 提出了FaithEIR，一个基于扩散模型的极端图像缩放框架，通过可逆变换、自适应细节先验和语义嵌入，在16倍以上缩放因子下实现优于现有方法的重建保真度和感知质量。


<details>
  <summary>Details</summary>
Motivation: 现有极端图像缩放方法在16倍以上缩放因子下难以保持语义一致的结构并生成逼真的细节，由于低分辨率到高分辨率映射存在严重的不适定性问题。

Method: 1) 受奇异值分解启发，提出可学习的可逆变换，实现潜在空间中的可逆下采样和上采样；2) 提出自适应细节先验（高频字典），捕捉训练数据中常见结构的经验平均值以补偿量化信息损失；3) 设计轻量级像素语义嵌入器，为预训练扩散模型提供语义条件。

Result: 在极端缩放因子（16倍以上）下，FaithEIR在重建保真度和感知质量上 consistently 优于现有最先进方法。

Conclusion: FaithEIR通过可逆变换、自适应细节先验和语义条件嵌入，有效解决了极端图像缩放中的结构保持和细节生成问题，代码已开源。
...
TL;DR: 提出CMTA框架，通过捕捉AI生成视频中跨模态时序伪影（语义对齐异常稳定）来实现检测，在四个数据集上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有AI视频检测方法仅关注单模态或时空伪影，忽略了视觉-文本跨模态空间中语义对齐的时序稳定性这一重要线索。

Method: 利用BLIP生成帧级图像描述，CLIP提取视觉-文本表示；粗粒度分支用GRU建模跨模态对齐的时序波动，细粒度分支用Transformer编码器捕捉帧间细微变化。

Result: 在GenVideo、EvalCrafter、VideoPhy、VidProM四个数据集的40个子集上达到新SOTA，跨生成器泛化性能优异。

Conclusion: AI生成视频存在跨模态时序伪影（异常稳定的语义轨迹），CMTA通过多粒度时序建模有效利用该特征实现高精度检测。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在从自然语言自动生成可执行的Blender代码时，存在频繁的语法错误和几何不一致对象的问题，需要一种无需微调且可立即部署的解决方案。

Method: 构建了包含50个物体类别、500个专家验证示例的多模态数据集（文本、代码、图像），采用检索增强生成机制，在生成时检索语义相似的示例来辅助代码生成。

Result: 在四种大语言模型上，编译成功率从40.8%提升至70.0%，CLIP语义归一化对齐度从0.41提升至0.77。

Conclusion: BlenderRAG有效提升了Blender代码生成的准确性和语义一致性，且无需微调或专用硬件，具有良好的可部署性。
...
TL;DR: UniVidX是一个统一的视频生成框架，利用视频扩散模型先验，通过随机条件掩码、解耦门控LoRA和跨模态自注意力三种设计，实现多模态间的任意方向条件生成，在RGB视频与内在图、RGBA层等任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法针对每种任务单独训练模型，输入-输出映射固定，限制了跨模态相关性的建模能力。

Method: 提出UniVidX框架，包含三大核心设计：(1) 随机条件掩码(SCM)实现任意方向条件生成；(2) 解耦门控LoRA(DGL)保护VDM先验；(3) 跨模态自注意力(CMSA)促进模态间信息交换。

Result: 在UniVid-Intrinsic和UniVid-Alpha两个实例化模型上，各任务均达到与SOTA方法竞争的性能，且在少于1000个视频训练下仍能鲁棒泛化到真实场景。

Conclusion: UniVidX证明了统一多模态框架可以有效利用VDM先验实现灵活的视频生成，且数据效率高。
...
TL;DR: 提出一种基于初始噪声优化的免训练可控3D修复方法，通过反向传播近似和频谱参数化实现高保真修复。


<details>
  <summary>Details</summary>
Motivation: 在结构化3D潜在扩散框架中，底层几何结构在扩散过程早期建立且对初始噪声高度敏感，导致修复和编辑任务中难以保持与现有上下文的严格对齐。

Method: 在结构化3D潜在扩散框架中优化初始噪声：利用基于整流流模型的反向传播近似更新初始噪声，并设计频谱参数化以实现鲁棒高效的结构化3D潜在优化。

Result: 在上下文一致性和提示对齐方面一致优于代表性免训练修复基线，验证了初始噪声控制作为3D修复独立维度的有效性。

Conclusion: 初始噪声控制可成为3D修复中独立于传统采样轨迹操作的维度和方法。
...
TL;DR: 本研究利用深度学习模型从彩色眼底照片中预测12种阿尔茨海默病相关风险因素，发现眼底照片确实编码了与AD风险因素相关的视网膜结构特征，尤其是在视神经头和视网膜血管区域，且这些特征在AD发病前数年已表现出差异。


<details>
  <summary>Details</summary>
Motivation: 已知系统性、代谢性和生活方式因素与AD存在流行病学和生物标志物关联，但彩色眼底照片是否包含对应这些AD风险域的视网膜结构特征尚不清楚。本研究旨在探究深度学习能否从眼底照片中预测AD风险因素，并表征其背后的视网膜结构。

Method: 使用UK Biobank的62,876张眼底照片（来自44,501名参与者），训练深度学习模型预测12个AD相关因素（6个分类变量+6个连续变量）。评估模型性能、模型显著性图以及显著性衍生分数（CAM-Score），并与视网膜形态测量学进行比较。同时比较AD发病前平均8.55年的病例与匹配对照组的分数差异。

Result: 深度学习模型性能为：分类变量AUROC在0.5654-0.9480之间，连续变量R²在-0.0291-0.7620之间，优于大多数形态测量-机器学习模型。显著性图一致地突出了视神经头和视网膜血管等生物学意义区域，与形态测量变异一致。多个显著性衍生分数在AD发病前病例与对照组之间存在显著差异。

Conclusion: 眼底照片编码了与AD风险因素相关的视网膜特征。尽管不具备诊断意义，但深度学习衍生的视网膜表征可能揭示反映AD潜在脆弱性的生物学上有意义的风险相关结构变化。
...
TL;DR: 本文提出DMDSC框架，通过自适应动态边界方法解决医学影像中的类别不平衡问题，在开放集识别任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有Deep Simplex Classifier等方法在处理医学影像数据时使用统一边界，未能考虑类别频率差异导致的密度不均问题，限制了模型在极端类别不平衡场景下的开放集识别性能。

Method: 提出DMDSC框架，采用动态边界方法，根据标签频率自动调整类别特定的边界，对稀有病理类别施加更高惩罚并强制更紧致的特征聚类，以抵消数据不平衡的影响。

Result: 在BloodMNIST、OCTMNIST、DermaMNIST和BreaKHis等多个医学基准数据集上的实验表明，所提框架优于现有最先进方法。

Conclusion: DMDSC通过自适应动态边界有效缓解了医学影像中类别不平衡对开放集识别的挑战，为临床环境下的可靠未知样本拒绝提供了更好的解决方案。
...
TL;DR: 本文提出基于Vision Transformer的ViTCG框架，利用高光谱遥感影像的空间-光谱联合建模，将AOD反演误差降低62%，优于现有基础模型。


<details>
  <summary>Details</summary>
Motivation: 传统AOD反演方法依赖辐射传输模型和查找表，计算开销大且噪声敏感；现有数据驱动方法未能充分利用高光谱影像的空间-光谱一致性，导致反演结果空间不连续。

Method: 提出ViTCG框架，采用Vision Transformer结合通道分组（Channel-wise Grouping）的空间回归策略，以高光谱大气顶辐射为输入，联合建模空间上下文与光谱信息。

Result: 基于PACE辐射观测数据验证，相比Prithvi等最先进基础模型，均方误差降低62%，且生成空间连续一致的AOD场。

Conclusion: 首次将基础AI模型应用于AOD反演任务，ViTCG通过联合空间-光谱建模显著提升反演精度与空间一致性，为高光谱遥感气溶胶监测提供了新范式。
...
TL;DR: PhysEdit提出了一种自适应推理框架，通过复杂度自适应推理深度（CARD）和空间推理掩码（SRM）两个模块，在无需重训练的情况下实现图像编辑指令的差异化处理，在保持编辑质量的同时获得1.18倍的加速。


<details>
  <summary>Details</summary>
Motivation: 现有推理型图像编辑器对所有编辑指令（如颜色替换、物体插入、物理动作编辑）采用统一的固定推理方案，忽略了不同编辑在空间覆盖和推理深度上的异构需求，缺乏自适应性。

Method: 提出PhysEdit框架，核心包含两个推理时模块：(1) CARD模块根据指令和参考图像预测编辑复杂度，自适应分配推理步数Nr和推理token长度r；(2) SRM模块从交叉注意力中提取指令条件化的空间先验，将推理限制在语义相关区域。

Result: 在ImgEdit Basic-Edit Suite（737个样本）上，PhysEdit比强推理基线加速1.18倍（64.3s vs 76.1s），指令遵循度略有提升（CLIP-T 0.2283 vs 0.2266，+0.7%），身份保持基本持平（CLIP-I 0.8246 vs 0.8280）。外观编辑上加速达1.52倍。

Conclusion: 自适应推理是图像编辑效率提升的关键自由度，CARD的自适应分配是加速的主要来源，消融实验验证了各模块的独立贡献。
...
TL;DR: 本文提出GSACP，一种基于批次内点锚定特征亲和传播的端到端红外小目标检测方法，无需外部伪标签演化，在SIRST3数据集上以极低虚警率实现了有竞争力的性能。


<details>
  <summary>Details</summary>
Motivation: 现有单点监督红外小目标检测方法依赖显式、离线的伪标签构建（如多阶段主动学习或物理驱动掩码生成），流程复杂且成本高。本文探索一种更简洁的替代方案：通过在线批次内的点锚定特征亲和传播直接生成点到掩码的监督信号。

Method: 提出GSACP端到端测试框架，利用局部图像先验门控的硬间隔特征亲和度直接监督检测器，完全消除外部标签演化循环。针对自参考传播漂移问题，采用局部EMA教师解耦、硬背景对比分离和自适应支持几何等单变量消融分析进行系统性隔离。

Result: 在SIRST3数据集上，GSACP-Final达到0.6674 mIoU，相比PAL方法虚警伪影（Fa）相对降低38%，建立了超低虚警工作区间。

Conclusion: 通过系统解构端到端范式，本文展示了批次内特征传播在虚警抑制至关重要的部署场景中是一种紧凑有效的替代方案。
...
TL;DR: 本文提出了基于量子计算的Sobel边缘检测和Harris角点检测方法，比较了FRQI和QPIE两种量子图像编码方式，在无噪声模拟环境下取得了与经典方法一致的检测结果，其中QPIE表现更稳定。


<details>
  <summary>Details</summary>
Motivation: 探索量子计算在经典图像处理任务（边缘检测和角点检测）中的可行性，并比较不同量子图像编码方法的效果。

Method: 利用FRQI和QPIE两种量子图像编码方法；提出基于lag-2差分的量子梯度计算方案；结合经典后处理步骤减少误检。

Result: 量子电路输出与经典Sobel和Harris算子结果一致；QPIE比FRQI更稳定、更连贯；梯度计算在电路层面高效，但整体成本仍由状态制备、测量和经典后处理主导。

Conclusion: 该工作展示了经典边缘和角点检测方法的可行且可扩展的量子实现，但并非端到端的加速，在NISQ硬件上性能可能受噪声和测量限制影响。
...
TL;DR: 本文提出了Place Pulse-Gaze数据集，将街景图像与眼动追踪和感知标签结合，并设计了基于注视引导的城市感知框架，验证了注视行为对主观城市感知建模的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有计算模型直接从街景图像建模城市感知，忽略了人类形成这些判断的感知过程（如注视行为）。

Method: 构建Place Pulse-Gaze数据集（街景图像+眼动追踪+感知标签），提出Gaze-Guided Urban Perception Framework，包含三种设置：仅用注视信息、注视融合显式语义场景表征、注视融合隐式丰富视觉表征。

Result: 注视本身携带有效的预测信号；将注视与场景表征融合后，在语义和丰富视觉表征下均能进一步提升主观城市感知的预测性能。
...
TL;DR: LightKV是一种通过跨模态消息传递和文本提示引导，在预填充阶段压缩视觉token KV缓存的方法，仅用55%的视觉token即可将KV缓存减半、计算量减少40%，并保持模型性能。


<details>
  <summary>Details</summary>
Motivation: LVLMs中KV缓存因视觉token数量过多导致显存开销巨大，现有方法缺乏文本提示的引导。

Method: 利用视觉token嵌入间的冗余性，通过文本提示引导的跨模态消息传递在预填充阶段逐步压缩视觉token。

Result: 仅保留55%的原始视觉token，KV缓存减半，计算量减少40%，在8个LVLMs和8个基准数据集上显著优于现有基线。

Conclusion: LightKV通过提示感知的视觉token压缩，有效降低KV缓存显存开销，同时保持通用性能。
...
TL;DR: 本文提出GMGaze，一种基于多尺度Transformer的全局条件化目光估计方法，通过语义原型条件化、早期统一融合和稀疏专家混合模块，在四个公开数据集上达到SOTA性能，平均角度误差分别为2.49°、3.22°、10.16°和1.44°。


<details>
  <summary>Details</summary>
Motivation: 现有CNN、Transformer和CLIP-based目光估计方法存在三大挑战：图像特征的晚期融合导致信息丢失、缺乏因子感知条件化、以及容量扩展不切实际。本文旨在解决这些问题，实现更准确高效的目光估计。

Method: 1) 语义原型条件化：利用四个原型库（光照、背景、头部姿态、外观）调制CLIP全局图像嵌入，生成两个互补的上下文偏置全局token；2) 早期统一融合：将CLIP patch、CNN token与全局token在第一层融合，避免晚期融合的信息丢失；3) 稀疏专家混合模块：为每个token提供条件计算容量，避免均匀增加密集参数；4) 对抗域适应：引入特征分离损失，使两个全局token去相关，提升跨域适应能力。

Result: 在MPIIFaceGaze、EYEDIAP、Gaze360、ETH-XGaze上分别达到2.49°、3.22°、10.16°、1.44°的平均角度误差，在域内设置中全面超越基线；在跨域评估中在两个标准迁移路线上取得SOTA结果。

Conclusion: GMGaze通过多尺度Transformer架构、语义原型条件化和早期融合策略，有效解决了现有目光估计方法的关键挑战，在域内和跨域场景均取得了显著优于现有方法的性能。
...
TL;DR: GenLIP是一种用于多模态大语言模型的视觉编码器生成式预训练框架，通过直接预测视觉token对应的语言token来实现视觉-语言对齐，方法简单、可扩展且性能优异。


<details>
  <summary>Details</summary>
Motivation: 现有视觉编码器与自回归语言模型的对齐不够直接，通常依赖对比学习或额外的文本解码器，流程复杂且扩展性受限。

Method: 采用标准语言建模目标，直接让ViT从视觉token预测语言token，无需对比批次构建或额外文本解码器，单个Transformer联合建模视觉和文本token，并支持多分辨率图像继续预训练。

Result: 在Recap-DataComp-1B的8B样本上训练，用更少的预训练数据达到或超越了强基线；多分辨率继续预训练后，在OCR和图表理解等细节敏感任务上进一步提升。

Conclusion: GenLIP为MLLM提供了一种简洁、可扩展且高性能的视觉编码器预训练方案，是强有力的基础框架。
...
TL;DR: 提出后验增强流匹配（PAFM），通过用近似后验的期望替代单目标监督，降低训练梯度方差，缓解流坍塌问题，在多个图像生成基准上提升最高3.4 FID。


<details>
  <summary>Details</summary>
Motivation: 标准流匹配（FM）在高维图像训练中，每个样本仅监督单一轨迹和中间点，导致训练信号极其稀疏且方差大，引发流坍塌——模型记忆特定源-目标配对，无法泛化。

Method: 将难解的后验分解为中间状态在假设终点下的似然与终点的条件先验之积，利用重要性采样构建多个候选目标的混合分布，用期望替代单目标监督，得到原始FM目标的无偏估计并降低梯度方差。

Result: 在SiT-B/2和SiT-XL/2两种模型规模、SiT和MMDiT两种架构、ImageNet和CC12M两种条件下，PAFM相比FM提升最高3.4 FID50K，计算开销近乎不变。

Conclusion: PAFM在理论上保证无偏估计的同时有效降低方差，是一种通用且高效的流匹配改进方法。
