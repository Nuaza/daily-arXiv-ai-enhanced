...
TL;DR: DOne框架通过解耦结构理解和元素渲染，解决VLM在Design-to-Code中的整体瓶颈问题，显著提升布局复杂设计的代码生成质量
...
Conclusion: DOne通过解耦结构理解和元素渲染的方法，有效解决了VLM在复杂设计到代码转换中的瓶颈问题，显著提升了生成代码的质量和生产力
...
TL;DR: Look Twice (LoT)是一个无需训练、在推理时改进多模态大语言模型使用视觉和文本证据的框架，通过注意力模式识别相关证据并用提示标记高亮，提升知识型视觉问答性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在回答需要外部知识的图像问题时，难以识别最相关的视觉和文本证据，需要整合可能有噪声或部分相关的文本证据，同时定位图像中的细粒度视觉信息。

Method: 利用模型注意力模式估计查询相关的视觉区域和检索到的文本元素，然后基于这些高亮证据生成答案。通过轻量级的提示级标记高亮选定的线索，鼓励模型在生成过程中重新关注相关证据。

Result: 在多个基于知识的视觉问答基准测试中，相比零样本多模态大语言模型取得了持续改进。在纯视觉中心和幻觉导向基准上的额外评估进一步表明，仅视觉证据高亮也能在没有文本上下文的环境中提升模型性能。

Conclusion: Look Twice框架无需额外训练或架构修改，通过推理时改进多模态大语言模型对多模态证据的利用方式，有效提升了知识密集型视觉问答任务的性能。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在通用基准上表现出色，但在医学影像领域缺乏鲁棒性，存在跨数据集干扰和对数据体制敏感的问题。在临床工作流中，数据和任务按顺序到达，简单的持续训练会导致灾难性遗忘。

Method: 提出MedQwen模型，采用参数高效的频谱路由专家混合架构，从预训练权重的非重叠奇异值分解段初始化每个专家，引入残差补偿和缩放方案，实现稳定的专家专业化和分布偏移下的一致路由。

Result: 在23个医学数据集上（涵盖视觉问答、报告生成、放射学分类和幻觉缓解），MedQwen实现了强大可靠的性能：在零样本分类上接近全参数微调，可训练参数减少339倍；将顺序遗忘减少到约5%，而强基线模型性能下降20-50%。

Conclusion: MedQwen通过创新的参数高效架构设计，成功解决了医学视觉语言模型中的跨数据集干扰、数据体制敏感性和灾难性遗忘问题，为临床工作流中的持续学习提供了有效解决方案。
...
TL;DR: 本文提出了一种基于视觉Transformer的模型，用于从美式足球训练视频中检测危险擒抱动作，并在扩展的数据集上实现了比先前基线更好的性能。


<details>
  <summary>Details</summary>
Motivation: 在接触性运动中及早识别危险动作能够及时干预并提高运动员安全性。美式足球中的擒抱技术不当可能导致严重伤害，因此需要自动检测危险擒抱的方法来支持教练进行伤害预防。

Method: 使用基于视觉Transformer的模型，结合处理类别不平衡的训练方法。数据集包含733个单人擒抱假人视频片段，每个片段围绕首次接触点进行时间定位，并使用标准擒抱技术评估(SATT-3)的撞击区域组件进行标注。

Result: 在交叉验证中获得了0.67的危险召回率和0.59的危险F1分数。相对于先前在较小数据集上的基线（危险召回率0.58，危险F1分数0.56），危险召回率提高了超过8个百分点。
...
TL;DR: 使用合成蹦床姿态数据集微调姿态估计模型，显著提升极端姿态下的2D和3D姿态估计精度


<details>
  <summary>Details</summary>
Motivation: 蹦床体操涉及极端人体姿态和罕见视角，现有姿态估计模型在这些情况下表现不佳，需要专门的数据集来提升性能

Method: 开发合成蹦床姿态数据集生成流程：从动作捕捉数据拟合到参数化人体模型，生成多视角真实图像，用此数据微调ViTPose模型

Result: 在2D姿态估计上达到最先进水平，显著缩小常见姿态与极端姿态之间的性能差距；在3D姿态估计上，MPJPE降低12.5毫米，相比预训练ViTPose模型提升19.6%

Conclusion: 合成数据可以有效解决极端姿态估计问题，通过针对性的数据集微调可以显著提升姿态估计模型在特定领域（如蹦床体操）的性能
...
TL;DR: 本文提出了一种基于自助法的注意力正则化方法，通过统计重采样技术量化注意力分数的不确定性，去除噪声产生的虚假注意力，提高ViT注意力图的可解释性。


<details>
  <summary>Details</summary>
Motivation: 视觉Transformer的注意力分数通常非零且分散，导致注意力图噪声大、可解释性差。需要量化注意力分数的不确定性，获得正则化的注意力分数以去除噪声影响。

Method: 提出注意力正则化方法：1）通过重采样输入特征生成注意力分数的自助分布；2）基于该分布估计注意力分数的显著性和后验概率；3）去除由噪声产生的虚假注意力，实现稀疏化和收缩。

Result: 在自然图像和医学图像上，该方法能有效去除噪声引起的虚假注意力，显著提高注意力图的稀疏性和收缩性。通过模拟和真实数据集进行了定量评估，证明其有效性。

Conclusion: 自助法可作为实用的正则化工具，用于提高ViT注意力分数作为解释的可信度。该方法为注意力机制提供了统计基础，增强了视觉Transformer的可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 研究CNN模型中的纹理表示是否与人类纹理感知对齐，特别是那些被认为是更好视觉系统模型的CNN是否具有更接近人类的纹理表示

Method: 比较不同CNN模型中特征相关性捕获的感知内容，并将其与Brain-Score测量的模型与哺乳动物视觉系统的感知对齐程度进行比较

Result: 令人惊讶地发现，CNN作为视觉系统模型的传统质量衡量标准与其与人类纹理感知的对齐程度之间没有关联

Conclusion: 纹理感知涉及不同于基于物体识别训练的CNN方法通常建模的机制，可能依赖于上下文信息的整合
...
TL;DR: AffordTissue是一个用于预测手术中工具-动作特定组织可操作性区域的多模态框架，通过密集热图在胆囊切除术中实现安全手术自动化。


<details>
  <summary>Details</summary>
Motivation: 当前手术动作自动化方法在临床部署中存在挑战：现有方法对器械在组织表面交互位置的预测能力有限，缺乏显式的条件输入来强制执行工具-动作特定的安全交互区域。

Method: 结合了时间视觉编码器（捕捉多视角下的工具运动和组织动态）、语言条件（实现跨不同器械-动作对的泛化）和DiT风格解码器（用于密集可操作性预测）的多模态框架。

Result: 在103个胆囊切除术的15,638个视频片段上建立了首个组织可操作性基准，实验显示相比视觉语言模型基线有显著改进（20.6像素ASSD vs. 60.2像素），任务特定架构优于大规模基础模型。

Conclusion: 通过预测工具-动作特定的组织可操作性区域，AffordTissue为安全手术自动化提供了显式的空间推理，可能解锁对适当组织区域的显式策略指导，并在器械偏离预测安全区域时实现早期安全停止。
...
TL;DR: GRAZE是一个无需训练的流程，用于在美式足球训练视频中定位"首次接触点"（FPOC），即球员首次接触训练假人的帧，无需接触示例标签，在真实世界杂乱场景中达到高精度定位。


<details>
  <summary>Details</summary>
Motivation: 美式足球训练产生大量视频，但关键交互只占长视频的短暂窗口。可靠的生物力学分析需要时空定位来识别交互实体和接触开始时间。现有方法在杂乱场景、相机运动、多个相似装备运动员以及冲击时快速姿态变化等挑战下效果有限。

Method: GRAZE采用无训练流程：1) 使用Grounding DINO发现候选球员-假人交互；2) 通过运动感知的时间推理进行精炼；3) 使用SAM2作为显式的像素级接触验证器，而非仅依赖检测置信度。候选发现与接触确认的分离使方法对杂乱场景和冲击附近的不稳定定位具有鲁棒性。

Result: 在738个擒抱训练视频上，GRAZE在97.4%的片段中产生有效输出，在77.5%的所有片段中将FPOC定位在±10帧内，在82.7%的所有片段中定位在±20帧内。这表明无需任务特定训练即可在真实世界训练素材中实现帧级准确的接触开始定位。

Conclusion: GRAZE展示了在无约束训练视频中实现帧级准确接触开始定位的可行性，无需接触示例标签。该方法通过分离候选发现和接触确认，对杂乱场景和冲击附近的不稳定定位具有鲁棒性，为美式足球训练的生物力学分析提供了实用解决方案。
...
TL;DR: 提出基于稀疏体素光栅化的新框架，解决3D高斯泼溅在开放词汇3D场景理解中的空间模糊性和多级语义模糊性问题，在细粒度查询任务上表现优异。
...
Method: 提出使用稀疏体素光栅化作为结构化、不相交的几何表示，通过单目深度和法线先验进行正则化，建立稳定的几何基础。实现确定性、置信感知的特征注册过程，并利用基础模型AM-RADIO的密集对齐特性解决多级模糊性，避免分层训练的计算开销。

Result: 在开放词汇3D对象检索和点云理解基准测试中达到最先进性能，特别是在细粒度查询任务上表现优异，而传统注册方法通常在这些任务上失败。

Conclusion: 稀疏体素光栅化框架有效解决了3D高斯泼溅方法中的空间模糊性和语义模糊性问题，通过结构化几何表示和密集对齐特性，在细粒度3D场景理解任务上取得了显著改进。
...
TL;DR: EgoFlow：基于流匹配的框架，从第一人称视频生成物理一致的6DoF物体轨迹，结合混合Mamba-Transformer-Perceiver架构和梯度引导推理，在多个数据集上超越现有方法。
...
Result: 在HD-EPIC、EgoExo4D和HOT3D等真实世界数据集上，EgoFlow在准确性、泛化性和物理真实性方面优于基于扩散和Transformer的基线方法，碰撞率降低高达79%，并在未见场景上表现出强大的泛化能力。

Conclusion: 研究结果突显了基于流的生成建模在可扩展和物理基础的第一人称运动理解方面的潜力，实现了无需后处理过滤或额外监督的连贯可控运动生成。
...
TL;DR: 提出一个通用的人体姿态数据分析流程，用于从复杂姿态时间序列中提取有意义的运动模式，适用于多种实验场景。
...
Method: 开发了一个通用分析流程，结合原则性预处理、降维和基于递归的时间序列分析，以量化运动动态的时间结构，支持线性和非线性特征描述。

Result: 通过三个案例研究展示了流程的灵活性：涵盖面部和全身运动、2D和3D数据、个体与多智能体行为，证明同一分析工作流可以适应不同场景提取理论上有意义的见解。

Conclusion: 该通用分析流程能够有效处理复杂姿态时间序列，为从自然环境中捕捉的人类运动数据中提取有意义的行为模式提供了系统化方法。
...
TL;DR: UniRecGen统一框架整合稀疏视图3D建模中的重建与生成范式，在共享规范空间中协调两者冲突，实现重建保真度与生成合理性的平衡。


<details>
  <summary>Details</summary>
Motivation: 稀疏视图3D建模面临重建保真度与生成合理性之间的根本矛盾：前馈重建方法效率高、输入对齐好但缺乏全局先验导致结构不完整；基于扩散的生成方法能提供丰富几何细节但多视图一致性差。

Method: 提出UniRecGen统一框架，将重建与生成范式整合到单一协作系统中。通过共享规范空间对齐坐标空间、3D表示和训练目标的冲突，采用解耦协作学习保持训练稳定性并实现推理时的无缝协作。重建模块提供规范几何锚点，扩散生成器利用潜在增强条件来细化和补全几何结构。

Result: 实验结果表明，UniRecGen在从稀疏观测创建完整一致的3D模型方面，实现了卓越的保真度和鲁棒性，优于现有方法。

Conclusion: UniRecGen成功整合了重建与生成两种范式，通过共享规范空间和解耦协作学习解决了稀疏视图3D建模中的基本矛盾，为创建完整一致的3D模型提供了有效解决方案。
...
TL;DR: TAG框架通过非参数纹理恢复解决热成像中的鬼影效应，首次实现夜间人脸表情恢复和3D拓扑对齐，超越现有HADAR技术


<details>
  <summary>Details</summary>
Motivation: 热成像在夜视中至关重要，但受到鬼影效应的根本限制。虽然传统方法依赖数据后处理，但HADAR技术开启了超光谱计算热成像的新前沿。然而，现有HADAR仅适用于材料均匀的有限场景，而材料非均匀性在现实世界中普遍存在，因此需要通用的抗鬼影成像解决方案。

Method: 提出TAG（热抗鬼影）通用计算热成像框架，利用超光谱光子流进行非参数纹理恢复。该方法能够处理材料非均匀性问题，实现高保真夜视成像。

Result: TAG在实验中首次实现了鬼影人脸的表情恢复，这是长期以来公认的典型鬼影现象。TAG不仅在各种场景中普遍优于HADAR，还揭示了材料非均匀性的影响，阐明了HADAR的有效性边界。研究还展示了昼夜面部纹理和表情恢复，首次实现了热3D拓扑对齐和情绪检测。

Conclusion: 这项工作为高保真计算夜视建立了通用基础，在自主导航、侦察、医疗保健和野生动物监测等领域具有潜在应用价值。
...
TL;DR: PBSeg：针对无人机图像语义分割的轻量级原型注意力框架，在保持分割质量的同时显著降低计算复杂度


<details>
  <summary>Details</summary>
Motivation: 无人机图像语义分割面临极端尺度变化、复杂边界和边缘设备计算资源有限等挑战。现有方法要么计算开销大（如Transformer），要么难以捕捉高分辨率航拍场景的细节（轻量级方法）。
...
Method: ReFlow包含完整规范空间构建模块用于增强静态和动态区域初始化，以及基于分离的动态场景建模模块解耦静态和动态组件。核心是自校正流匹配机制，包括全流匹配将3D场景流与时间变化的2D观测对齐，以及相机流匹配强制静态对象的多视角一致性。
...
TL;DR: VideoZeroBench：一个用于长视频问答的分层基准测试，通过严格的时空证据验证来评估视频多模态大语言模型的实际推理能力，暴露了表面答案正确性与真实证据推理之间的显著差距。


<details>
  <summary>Details</summary>
Motivation: 当前视频多模态大语言模型的评估存在两个关键局限：(1) 夸大的分数掩盖了细粒度视觉理解和推理的缺陷；(2) 答案正确性通常在没有验证模型是否识别支持其预测的精确时空证据的情况下进行测量。需要更严格的评估方法来衡量真正的基于证据的推理能力。
...
Result: 实验显示，即使在标准端到端QA设置下（Level-3），Gemini-3-Pro正确回答的问题也不到17%。当施加定位约束时，性能急剧下降：在需要正确答案和准确时空定位的情况下（Level-5），没有模型超过1%的准确率，大多数模型无法实现任何正确的定位预测。这暴露了表面答案正确性与真实证据推理之间的显著差距。

Conclusion: 基于证据的视频理解仍然是长视频问答的瓶颈。VideoZeroBench揭示了当前模型的局限性，为未来基于证据的视频推理研究提供了见解。基准测试和代码将公开提供。
...
Method: 提出梯度对齐交替学习(GAAL)范式，采用交替单模态学习和共享分类器来解耦多模态梯度并促进交互；设计基于不确定性的跨模态梯度手术，选择性对齐跨模态梯度，使共享参数对所有模态都有益
...
<details>
  <summary>Details</summary>
Motivation: 现有无人机视角地理定位方法依赖卫星图像进行训练，但在卫星数据不可用或受限的实际部署场景中受到限制。需要开发不依赖卫星图像的训练方法。

Method: 1) 使用3D高斯泼溅从多视角无人机图像重建密集3D场景；2) 通过PCA引导的正射投影生成几何归一化的伪正射影像；3) 轻量级几何引导修复获得纹理完整的无人机侧视图；4) 从无人机数据学习Fisher向量聚合模型，测试时用于编码卫星瓦片进行跨视角检索。

Result: 在University-1652和SUES-200数据集上的实验表明，SFT框架显著优于卫星无关泛化基线方法，并缩小了与使用卫星图像训练方法的性能差距。

Conclusion: 提出的卫星无关训练框架通过无人机侧3D重建和伪正射影像生成，实现了不依赖卫星数据的跨视角地理定位，为实际部署提供了可行的解决方案。
...
TL;DR: SHOE是一个新的开放词汇人-物交互检测评估框架，使用语义相似度替代传统精确匹配，通过分解动词和对象组件并利用多个LLM计算相似度，更符合人类判断。


<details>
  <summary>Details</summary>
Motivation: 传统评估指标（如mAP）将HOI类别视为离散分类标签，无法准确评估语义有效但词汇不同的预测（如"靠在沙发上" vs "坐在沙发上"），限制了开放词汇预测的评估能力。
...
TL;DR: ROSETTA：一种通过角度损失和特征范数损失来平衡ID分类和OOD检测的鲁棒开集测试时适应方法，解决了熵最小化和最大化之间的内在冲突。


<details>
  <summary>Details</summary>
Motivation: 开集测试时适应中，协变量偏移会影响ID样本，同时需要检测OOD样本。现有方法结合熵最小化（保持ID性能）和熵最大化（增强OOD检测）存在内在冲突，导致ID分类和OOD检测之间的权衡问题。

Method: 提出ROSETTA方法：1）分析熵最大化在OSTTA中的局限性；2）引入角度损失来调节特征范数大小；3）使用特征范数损失来抑制csOOD样本的logits，从而改进OOD检测。

Result: 在CIFAR-10-C、CIFAR-100-C、Tiny-ImageNet-C和ImageNet-C上实现了强大的OOD检测能力，同时保持高ID分类性能。在Cityscapes数据集上的实验验证了该方法在真实世界语义分割中的有效性，在HAC数据集上的结果证明了其在不同开集TTA设置中的适用性。

Conclusion: ROSETTA通过创新的角度损失和特征范数损失，有效解决了开集测试时适应中ID分类和OOD检测之间的权衡问题，在各种数据集和实际应用中表现出色。
...
TL;DR: F3DGS是一个用于去中心化多智能体3D重建的联邦3D高斯溅射框架，通过共享几何骨架和可见性感知聚合解决分布式机器人系统中的通信开销和几何不一致问题。


<details>
  <summary>Details</summary>
Motivation: 现有的3D高斯溅射（3DGS）管道假设集中访问所有观测数据，这在分布式机器人设置中适用性有限，因为智能体独立运行且集中数据聚合可能受限。直接扩展集中式训练到多智能体系统会引入通信开销和几何不一致问题。

Method: F3DGS首先通过注册多个客户端本地合并的LiDAR点云来构建共享几何骨架，初始化全局3DGS模型。在联邦优化期间，高斯位置固定以保持几何对齐，每个客户端仅更新外观相关属性（协方差、不透明度、球谐系数）。服务器使用可见性感知聚合这些更新，根据每个客户端观察每个高斯的频率加权其贡献。

Result: 实验表明，F3DGS在实现与集中式训练相当的重建质量的同时，能够支持跨智能体的分布式优化。作者收集了包含同步LiDAR、RGB和IMU测量的多序列室内数据集进行评估。
...
<details>
  <summary>Details</summary>
Motivation: 现有VLA模型对物理可实现对抗攻击的鲁棒性研究不足。现有攻击方法要么不具代表性（语言扰动），要么物理真实性有限（2D视觉攻击）。3D对抗纹理作为更物理可行且破坏性强的威胁，但标准3D模拟器缺乏从VLA目标函数到物体外观的可微分优化路径

Method: 提出Tex3D框架：1）前景-背景解耦（FBD）：通过双渲染器对齐实现可微分纹理优化，同时保持原始模拟环境；2）轨迹感知对抗优化（TAAO）：优先处理行为关键帧，并使用基于顶点的参数化稳定优化

Result: 在模拟和真实机器人实验中，Tex3D显著降低了VLA模型在多个操作任务中的性能，任务失败率高达96.7%。实证结果揭示了VLA系统对物理基础3D对抗攻击的关键脆弱性

Conclusion: Tex3D是首个在VLA模拟环境中端到端优化3D对抗纹理的框架，成功展示了VLA系统对物理基础3D对抗攻击的脆弱性，强调了鲁棒性感知训练的必要性
...
TL;DR: 该论文提出了一种利用稀疏自编码器和基础模型特征自动提取生物形态特征的方法，构建了包含8万条特征标注的Bioscan-Traits数据集，为大规模生态学研究提供了可扩展的解决方案。


<details>
  <summary>Details</summary>
Motivation: 生物形态特征是理解生物与环境相互作用的关键，但目前提取这些特征主要依赖专家手动标注，过程缓慢且成本高昂，限制了在大规模生态研究中的应用。主要瓶颈在于缺乏连接生物图像与特征级标注的高质量数据集。

Method: 1. 在基础模型特征上训练稀疏自编码器，获得单语义、空间定位的神经元，这些神经元能持续激活有意义的形态部位；2. 引入特征标注流程：定位显著区域并使用视觉语言提示生成可解释的特征描述；3. 构建Bioscan-Traits数据集，包含19K昆虫图像的80K特征标注。

Result: 1. 构建了Bioscan-Traits数据集，涵盖BIOSCAN-5M中的19K昆虫图像和80K特征标注；2. 人类评估确认生成的形态描述具有生物学合理性；3. 通过全面的消融研究评估设计敏感性，系统变化关键设计选择并测量其对特征描述质量的影响。

Conclusion: 通过模块化流水线而非昂贵的手动工作来标注特征，提供了一种可扩展的方法：将生物学上有意义的监督注入基础模型，实现大规模形态分析，并弥合生态相关性与机器学习实用性之间的差距。
...
Result: 在单张RTX 5090 GPU上，每个新场景扩展步骤需要9秒生成，随后3秒进行运动对齐和运动场更新，实现了具有全局一致环境动态的交互式4D世界生成。
...
Method: 提出包含两个关键模块的框架：1) 道路感知补丁增强(RAPA)：将分割的目标补丁增强到道路区域，保持3D几何一致性；2) 原型过滤(PBF)：通过原型相似性和深度不确定性过滤预测，生成高质量伪标签
...
Method: 利用莫尔效应，当相机拍摄紧凑双层光栅结构时会产生干涉条纹。推导出莫尔运动不变性，证明条纹相位与光栅图像位移通过光学几何线性耦合，与观看距离和光栅结构无关。验证器从视频中提取这两个信号并测试它们的相关性。

Result: 在多个最先进的AI生成器和真实拍摄视频上进行验证，发现真实视频和AI生成视频产生显著不同的相关特征，表明该方法能够有效区分两者。

Conclusion: 确定性光学现象可以作为物理基础的、可验证的签名来对抗AI生成视频，为视频真实性认证提供了鲁棒的方法。
...
TL;DR: DynaVid是一个视频合成框架，通过使用计算机图形渲染的光流作为合成运动数据来训练视频扩散模型，解决了动态运动生成和精细运动控制的问题。


<details>
  <summary>Details</summary>
Motivation: 当前视频扩散模型在合成高度动态运动或需要精细运动控制的视频时表现不佳，主要原因是常用训练数据集中这类示例稀缺。

Method: 采用两阶段生成框架：1）运动生成器合成光流表示的运动；2）运动引导的视频生成器基于运动条件生成视频帧。使用计算机图形渲染的光流作为合成运动数据，将运动与外观解耦。

Result: 在剧烈人体运动生成和极端相机运动控制这两个具有挑战性的场景中，DynaVid在动态运动生成和相机运动控制的真实感和可控性方面都有显著提升。

Conclusion: 通过利用合成运动数据（特别是光流表示）并将运动生成与视频生成解耦，DynaVid能够从合成数据中学习动态运动模式，同时保持真实世界视频的视觉真实性。
...
Method: 1. 设计解耦表示机制，去除非必要的环境风格干扰，引导模型专注于提取跨场景共享的语义内在特征；2. 使用权重融合策略在参数空间中动态整合新旧环境知识，无需存储历史数据即可适应新分布，同时最大程度保留旧环境的判别能力。

Result: 在多个标准基准数据集上的广泛实验表明，该方法在完全无样本和无域ID的设置下显著减少了灾难性遗忘，其准确率优于现有最先进方法。

Conclusion: 该方法为具身多媒体系统提供了一种鲁棒的连续环境适应解决方案，通过解耦表示和权重融合实现了在未知交互场景中的有效增量学习，解决了现有方法的局限性。
...
TL;DR: 该论文提出频率域适应(FDA)和谐波约束最优传输(HOT)框架，用于解决远程光电容积描记(rPPG)技术中的域适应问题，提高模型在不同数据集间的鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 远程光电容积描记(rPPG)技术在实际部署中面临域偏移导致的性能显著下降问题。现有深度学习方法虽然在单个数据集上表现良好，但容易过拟合到外观相关因素（如光照、相机特性、颜色响应等），这些因素在不同域间变化很大。

Method: 提出频率域适应(FDA)策略，通过转移编码域依赖外观特征的低频频谱成分，使rPPG模型学习对外观变化的鲁棒性，同时保留心脏诱导信号。进一步提出谐波约束最优传输(HOT)，利用心脏信号的谐波特性来指导原始表示和FDA转移表示之间的对齐。

Result: 广泛的跨数据集实验表明，提出的FDA和HOT框架有效增强了rPPG模型在不同数据集间的鲁棒性和泛化能力。

Conclusion: 频率域适应和谐波约束最优传输为rPPG领域提供了一种原则性的域适应策略，能够有效解决外观变化带来的域偏移问题，提高模型的实用部署能力。
...
TL;DR: Director：一种统一的时空高斯表示，联合建模人体表演、高保真渲染和实例级语义，实现时间一致的4D重建，同时支持实例分割和开放词汇查询。
...
Method: 1. 利用时间对齐的实例掩码和MLLM生成的句子嵌入，通过两个MLP解码器监督高斯语义特征学习；2. 将2D光流与4D高斯连接，微调运动参数以提高时间稳定性；3. 引入几何感知的SDF约束和表面连续性正则化项。
...
TL;DR: 提出UnderEraser方法，通过外部知识蒸馏和内部上下文注意力机制，在视频中移除目标物体及其引发的副作用（阴影、反射等），保持时空一致性


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的视频物体移除方法难以消除物体引发的副作用（如阴影、反射、光照变化），同时保持整体连贯性，这源于对目标物体及其与场景交互的物理和语义理解不足

Method: 1. 外部：通过蒸馏方案将视觉基础模型中物体与其引发效果的关系迁移到视频扩散模型；2. 内部：提出帧间上下文交叉注意力机制，将每个去噪块锚定在目标区域周围信息丰富的未掩码上下文中

Result: 大量实验证明该方法达到最先进性能，并建立了首个真实世界视频物体移除基准，促进未来研究和社区发展

Conclusion: 外部和内部指导共同使模型能够理解目标物体、其引发的效果和全局背景上下文，实现清晰连贯的物体移除
...
<details>
  <summary>Details</summary>
Motivation: 现有方法要么在估计共享注意力时不检测实际关注群体，要么假设图像中只有一个共享注意力点，这限制了共享注意力检测的实际应用并影响性能表现。
...
Method: 1. 前向过程：引入摊销定点求解器，通过强制连续时间步的速度一致性来隐式拉直前向轨迹；2. 后向过程：引入轨迹插值，自适应混合目标编辑和源重建速度；3. 自适应掩码机制：使用概念引导分割和源-目标速度差异来空间约束编辑信号。

Result: 在FLUX.1-dev和Stable Diffusion 3.5 Medium上的实验表明，SteerFlow比现有方法实现了更好的编辑质量，并能自然扩展到多轮编辑范式而不积累漂移。

Conclusion: SteerFlow是一个具有强理论保证的模型无关编辑框架，通过隐式轨迹拉直和自适应轨迹插值，在保持源图像保真度的同时实现了高质量的文本引导图像编辑。
...
TL;DR: 本文提出了一种几何感知的无人机地理定位框架，通过显式建模3D场景几何，将粗粒度位置识别和细粒度姿态估计统一到单个推理流程中，显著提升了GNSS拒止环境下的跨视角定位精度。


<details>
  <summary>Details</summary>
Motivation: 在GNSS拒止环境中，无人机倾斜视角图像与正交卫星地图之间存在严重的几何差异，现有方法通常采用解耦的检索-姿态估计流程，将透视畸变视为外观噪声而非显式几何变换，这限制了定位精度。

Method: 提出几何感知的无人机地理定位框架：1）使用视觉几何基础Transformer从多视角无人机图像序列重建局部3D场景；2）渲染虚拟鸟瞰图表示，将无人机视角正交校正以对齐卫星图像；3）引入卫星注意力块，隔离每个卫星候选与重建无人机场景的交互，防止候选间干扰同时保持线性计算复杂度。

Result: 在重新校准的University-1652数据集和SUES-200数据集上的大量实验表明，该方法显著优于现有最先进基线，实现了鲁棒的米级定位精度，并在复杂城市环境中表现出更好的泛化能力。

Conclusion: 通过显式建模3D场景几何并统一位置识别和姿态估计，提出的几何感知框架有效解决了跨视角地理定位中的几何差异问题，为GNSS拒止环境下的无人机定位提供了更准确、更鲁棒的解决方案。
...
TL;DR: 该研究构建了US-365K超声图像-文本数据集和超声诊断分类法，提出了语义感知对比学习框架Ultrasound-CLIP，在超声图像分类和检索任务上取得SOTA性能。
...
Method: 1. 构建US-365K大规模超声图像-文本数据集（36.5万配对样本，52个解剖类别）；2. 建立超声诊断分类法（UDT），包含层次解剖分类和诊断属性框架；3. 提出Ultrasound-CLIP框架，引入语义软标签和语义损失；4. 构建基于UDAF文本表示的异构图模态，支持病变-属性关系结构化推理。

Result: 在患者级数据分割下，该方法在分类和检索基准测试中达到最先进性能，同时在零样本学习、线性探测和微调任务中表现出强大的泛化能力。

Conclusion: 该研究通过构建大规模超声数据集和结构化知识框架，开发了专门针对超声模态的语义感知对比学习模型，有效解决了现有通用模型在超声数据上的局限性。
...
TL;DR: 提出一种轻量级架构和训练策略，使用自监督学习特征作为视频扩散模型的通用条件信号，实现视频域迁移和3D到视频生成，通过解耦外观与其他特征实现鲁棒控制。


<details>
  <summary>Details</summary>
Motivation: 当前视频模型主要用作生成渲染器，通常通过感知、几何或简单语义信号进行条件控制。自监督学习获得的高维特征虽然包含丰富的场景信息，但由于风格、光照和语义信息纠缠在一起，限制了其生成能力。需要探索如何将这些特征作为预训练视频扩散模型的通用条件信号。

Method: 引入轻量级架构和训练策略，解耦外观特征与其他需要保留的特征（如几何、语义），使模型能够鲁棒地控制外观变化（如风格化和重光照）。同时证明低空间分辨率可以通过更高的特征维度来补偿，从而提高从显式空间表示进行生成渲染的可控性。

Result: 该方法成功应用于视频域迁移和从3D生成视频等任务，能够实现外观变化（如风格化、重光照）的鲁棒控制，同时保持场景的其他特征不变。通过高维特征补偿低空间分辨率，提高了生成渲染的可控性。

Conclusion: 自监督学习特征可以作为预训练视频扩散模型的通用条件信号，通过适当的架构设计和训练策略解耦外观与其他特征，能够实现视频域迁移和3D到视频生成等应用，为生成渲染提供了更灵活的控制能力。
...
TL;DR: 该研究指出大型视觉语言模型在解决需要多步推理的谜语问题时存在严重缺陷，即使具备必要的视觉和语言组件，但缺乏连接这些组件的认知推理能力。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在显式视觉识别方面表现出色，但在处理需要复杂多步推理的问题时存在认知鸿沟，特别是当视觉输入仅作为线索而非答案时，模型难以解决信息未明确描绘的问题。

Method: 研究者引入了RebusBench基准测试，包含1,164个谜语谜题，用于评估模型整合感知和知识的能力。评估了包括Qwen、InternVL和LLaVA在内的最先进模型，测试了模型缩放和上下文学习的效果。

Result: 评估结果显示模型性能严重不足：精确匹配率低于10%，语义准确率低于20%。模型缩放和上下文学习均未带来显著改进，表明模型虽然具备必要的视觉和语言组件，但缺乏连接这些组件的认知推理能力。

Conclusion: 当前大型视觉语言模型在解决需要神经符号推理能力的问题时存在根本性缺陷，成功解决谜语谜题所需的认知工作流程（提取属性、检索先验知识、抽象映射）超出了现有模型的能力范围。
...
TL;DR: DriveDreamer-Policy：统一的驾驶世界-动作模型，将深度生成、未来视频生成和运动规划集成到单一模块化架构中，通过几何感知的世界表示提升自动驾驶性能


<details>
  <summary>Details</summary>
Motivation: 现有世界-动作模型（WAM）主要关注2D外观或潜在表示建模，缺乏对物理世界具身系统至关重要的几何基础。需要开发具有几何感知能力的统一驾驶模型，以产生更连贯的未来预测和更明智的驾驶动作。
...
Conclusion: DriveDreamer-Policy通过集成深度生成、未来视频生成和运动规划，在统一框架中实现了几何感知的世界建模，显著提升了自动驾驶系统的性能和鲁棒性。显式深度学习为视频想象提供了补充优势并提高了规划鲁棒性。
...
TL;DR: FSKD框架通过知识蒸馏将LiDAR森林结构信息迁移到RGBI图像，实现单模态高分辨率森林结构监测，在零样本CHM预测上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 机载LiDAR虽然能提供高精度森林结构数据，但成本高、频率低，限制了大规模应用。需要开发基于低成本RGBI图像的高分辨率森林结构监测方法

Method: 提出LiDAR-to-RGBI知识蒸馏框架：多模态教师融合RGBI图像与LiDAR平面指标和垂直剖面，通过交叉注意力；RGBI-only SegFormer学生学习复制这些输出，实现单模态预测

Result: 在德国萨克森州384平方公里森林数据上训练，8个地理独立测试区评估：零样本CHM预测MedAE 4.17m，R²=0.51，IoU 0.87，MAE比基线提升29-46%；多模态融合比纯RGBI训练提升10-26%性能

Conclusion: FSKD框架能同时预测CHM、PAI、FHD，突破了现有单目CHM估计器的局限；支持时间不匹配数据（冬季LiDAR+夏季RGBI），为德国数字孪生等国家项目提供可扩展的20cm分辨率监测方案
...
TL;DR: 提出了一种利用轮式里程计实现时间一致性单目深度估计的框架，通过光流三角化估计相机位姿和稀疏深度，使用递归贝叶斯估计度量尺度，再对预训练深度估计基础模型的相对深度进行重缩放。


<details>
  <summary>Details</summary>
Motivation: 现有单目深度估计方法在连续帧间难以保持时间一致性，导致深度估计抖动，且在深度范围突变时容易失败。为解决这些问题，需要开发能够保持稳定和连贯深度预测的方法。

Method: 利用移动机器人的轮式里程计，通过连续帧间的光流进行三角化估计相机位姿和稀疏深度。使用稀疏深度估计更新度量尺度的递归贝叶斯估计，然后将该尺度应用于预训练深度估计基础模型预测的相对深度进行重缩放。

Result: 在KITTI、TartanAir、MS2和自建数据集上评估，展示了鲁棒且准确的深度估计性能。

Conclusion: 提出的方法能够实现时间一致的单目深度估计，解决了现有方法在连续帧间深度估计不一致的问题，为自动驾驶和移动机器人感知系统提供了更稳定的深度信息。
...
### [42] [A deep learning pipeline for PAM50 subtype classification using histopathology images and multi-objective patch selection](https://arxiv.org/abs/2604.01798)
...
<details>
  <summary>Details</summary>
Motivation: 乳腺癌具有高度异质性，PAM50基因特征作为分类标准可实现个性化治疗。但分子检测成本高昂，需要开发从病理图像直接预测亚型的方法

Method: 结合NSGA-II多目标优化算法和蒙特卡洛dropout不确定性估计，联合优化斑块信息量、空间多样性、不确定性和斑块数量。使用ResNet18提取特征，自定义CNN头进行分类

Result: 在TCGA-BRCA内部数据集上F1分数0.8812，AUC 0.9841；在外部CPTAC-BRCA验证集上F1分数0.7952，AUC 0.9512
...
<details>
  <summary>Details</summary>
Motivation: 现有基于群体的策略优化方法在大型多模态模型中表现出潜力，但当回答具有相似正确性时，往往存在奖励方差低的问题，导致优势估计弱或不稳定。需要一种方法来丰富奖励信号并促进更稳定的策略更新。

Method: STRIVE通过构建每个输入视频的多个时空变体，并在文本生成和视觉变体上进行联合归一化。引入重要性感知采样机制，优先选择与输入问题最相关的帧，同时保持时间覆盖。通过结构化视觉扰动扩展群体比较，超越语言多样性。

Result: 在六个具有挑战性的视频推理基准测试（VideoMME、TempCompass、VideoMMMU、MMVU、VSI-Bench和PerceptionTest）中，相对于强大的强化学习基线，在多个大型多模态模型上展示了一致的改进。
...
TL;DR: SafeRoPE：针对基于transformer的扩散模型（如MMDiT）的轻量级安全生成框架，通过分析注意力机制发现不安全语义集中在可解释的低维子空间，通过扰动RoPE来抑制不安全内容生成
...
Result: 实验表明SafeRoPE在平衡有害内容缓解和效用保持方面达到SOTA性能，能有效抑制不安全语义生成，同时保持生成保真度和良性内容质量

Conclusion: SafeRoPE为基于transformer的扩散模型提供了一种轻量级、细粒度的安全生成框架，通过头级RoPE扰动实现了精确的不安全内容抑制，同时保持了生成质量
...
<details>
  <summary>Details</summary>
Motivation: 语言预训练模型和视觉预训练模型的异常参数比例差异显著，使得跨模态（语言和视觉）适应比跨域适应更具挑战性。以往研究多关注跨域迁移，认为语言预训练模型不适合下游视觉任务，但本文挑战了这一假设。

Method: 提出随机标签桥接训练方法，作为模态适应学习器，无需人工标注。该方法通过桥接训练阶段对齐大语言模型参数与视觉任务，并探索部分桥接训练策略，保留LLM中某些具有强基础特性的层。

Result: 桥接训练能有效使大语言模型参数适应视觉基础任务。研究发现部分桥接训练通常更有优势，因为LLM中的某些层具有强大的基础特性，即使不针对视觉任务进行微调也保持有益。

Conclusion: 该研究为直接利用语言预训练参数于视觉模型开辟了新途径，突出了部分桥接训练作为跨模态适应的实用路径的潜力，挑战了语言预训练模型不适合视觉任务的假设。
...
TL;DR: 提出了一种用于医学图像严重程度分类的半监督域自适应方法，通过排名分数对齐源域和目标域，解决类别边界模糊问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理医学图像严重程度分类时面临挑战，因为严重程度分类具有自然有序的类别标签，导致类别边界模糊，使得域自适应变得复杂。

Method: 提出了一种新颖的方法，通过基于类别顺序的排名学习排名分数来对齐源域和目标域。具体包括跨域排名（Cross-Domain Ranking）来跨域排名样本对，以及连续分布对齐（Continuous Distribution Alignment）来对齐排名分数分布。

Result: 在溃疡性结肠炎和糖尿病视网膜病变分类任务上的实验验证了该方法的有效性，成功对齐了类别特定的排名分数分布。

Conclusion: 该方法能够有效处理医学图像严重程度分类中的域自适应问题，通过排名分数对齐解决了有序类别标签带来的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 纹理3D网格同时表示几何、拓扑和外观，但其不规则结构给基于深度学习的语义分割带来挑战。现有方法通常直接处理网格而忽略丰富的纹理信息，需要一种能够有效利用纹理和几何信息的方法。

Method: 1. 纹理感知Transformer直接从每个网格面关联的原始像素学习；2. 新的分层学习方案用于多尺度特征聚合；3. 纹理分支将所有面级像素汇总为可学习token；4. 几何描述符与纹理token融合；5. 使用两阶段Transformer块(TSTB)处理，支持局部和全局信息流。

Result: 在Semantic Urban Meshes (SUM)基准测试中达到81.9% mF1和94.3% OA；在新构建的文化遗产数据集（带损坏类型三角形级标注的纹理屋顶瓦片）上达到49.7% mF1和72.8% OA，显著优于现有方法。

Conclusion: 提出的纹理感知Transformer模型能够有效利用3D网格中的纹理和几何信息，在语义分割任务上取得优异性能，为文化遗产保护等应用提供了有力工具。
...
<details>
  <summary>Details</summary>
Motivation: 传统向量量化方法（VQ-VAE、VQ-GAN）学习的离散表示具有位置依赖性：代码在空间上排列且上下文纠缠，需要自回归或扩散先验来建模依赖关系。本研究探讨位置信息是否对空间对齐数据的离散表示是必要的。

Method: 提出位置无关向量量化自编码器（PI-VQ），约束潜在代码不携带位置信息；引入基于最优二分图匹配的匹配量化算法，相比朴素最近邻量化提高3.5倍的有效瓶颈容量；利用学习代码的组合结构实现基于插值的采样。

Result: 在CelebA、CelebA-HQ和FFHQ数据集上评估，获得了具有竞争力的精度、密度和覆盖度指标；位置无关表示鼓励代码捕获全局语义特征，无需学习先验即可直接进行图像插值。

Conclusion: 位置无关表示在可分离性和可解释性方面存在权衡，但为离散表示学习提供了新方向，能够实现单次前向传播合成新颖图像，并指出了未来研究的多个方向。
...
<details>
  <summary>Details</summary>
Motivation: 尽管高斯泼溅(GS)技术在CT重建中表现优异，但其优势不足以推动从成熟重建算法的过渡。现有GS方法仍存在显著局限性，需要更快速、灵活的解决方案。

Method: 提出FaCT-GS框架，通过深度优化体素化和光栅化流程，实现快速CT重建。改进的体素化能够快速将高斯拟合到现有体积数据，可作为先验或压缩表示。

Result: FaCT-GS在标准512×512投影上比现有最佳GS CT重建快4倍以上，在2k投影上快13倍以上。能够良好扩展投影和输出体积大小。

Conclusion: FaCT-GS解决了GS在CT重建中的主要限制，提供了快速、灵活的重建框架，可作为现有体积的压缩表示或重建先验，显著提升了计算效率。
...
TL;DR: 研究发现当前最先进的视觉语言模型在基本几何变换下存在系统性脆弱性，缺乏空间不变性和等变性，导致在简单旋转、缩放等变换下识别性能大幅下降


<details>
  <summary>Details</summary>
Motivation: 尽管现代视觉语言模型在语义任务上表现出色，但它们在基本几何变换下的鲁棒性尚未得到充分研究。本研究旨在揭示这些模型在空间推理方面的系统性缺陷

Method: 通过系统评估方法，在不同视觉领域（符号草图、自然照片、抽象艺术）测试VLMs在几何变换（旋转、缩放、恒等变换）下的表现，涵盖不同架构、模型容量和提示策略

Result: 当语义内容变得稀疏时，VLMs性能急剧下降；这种脆弱性在不同架构、模型容量和提示策略中普遍存在，表明当前模型在语义理解和空间推理之间存在系统性差距

Conclusion: 当前视觉语言模型缺乏强大的几何基础，需要在未来的多模态系统中加强空间不变性和等变性的学习，以弥补语义理解与空间推理之间的差距
...
Method: 提出双损失训练框架：1）边界回归损失通过单通道边界预测提升时间定位精度；2）基于CDF的片段级正则化损失通过匹配预测片段和真实片段的累积分布来鼓励片段内结构一致性。该框架与架构无关，可作为训练时损失函数集成到现有TAS模型中。

Result: 在三个基准数据集上，该方法提高了片段级一致性和边界质量，在三种不同模型上都获得了更高的F1和Edit分数。帧级准确率基本保持不变，表明精确分割可以通过简单的损失设计实现，而非更重的架构或推理时优化。
...
TL;DR: MAR-MAER是一种创新的分层自回归框架，通过度量感知嵌入正则化和概率潜在模型，解决了文本到图像生成中图像质量不高和模糊提示处理困难的问题。


<details>
  <summary>Details</summary>
Motivation: 自回归模型在文本到图像生成中面临两大挑战：生成的图像质量可能不符合人类期望，以及难以处理具有多种有效解释的模糊提示。

Method: 提出MAR-MAER分层自回归框架，包含度量感知嵌入正则化方法（使用轻量级投影头和自适应核回归损失函数）和条件变分模块（在分层令牌生成过程中引入受控随机性）。

Result: 在COCO和新开发的模糊提示基准测试中，MAR-MAER在度量一致性和语义灵活性方面表现优异，相比基线Hi-MAR模型，CLIPScore提升+1.6，HPSv2提升+5.3，对模糊输入能产生更广泛的输出多样性。

Conclusion: MAR-MAER通过结合度量感知嵌入正则化和概率潜在建模，有效提升了文本到图像生成的质量和模糊提示处理能力，在人类评估和自动指标上都取得了显著改进。
...
TL;DR: HieraVid是一个分层剪枝框架，通过分段、帧和层三个层次逐步动态减少视频冗余，在保留30%令牌的情况下实现SOTA性能。
...
Method: 基于视频的分段-帧结构和LLM单向传播多模态信息的观察，提出三层剪枝：1) 分段级-时间分段和空间合并；2) 帧级-相似帧联合剪枝保留多样性；3) 层级-随着LLM层数增加逐渐减少冗余。

Result: 在四个广泛使用的视频理解基准测试中，仅保留30%令牌的情况下，HieraVid实现了新的SOTA性能，分别保持了LLaVA-Video-7B和LLaVA-OneVision-7B超过98%和99%的性能。

Conclusion: HieraVid通过分层剪枝有效减少了视频令牌的计算负担，同时保持了高性能，为视频大语言模型的高效部署提供了有效解决方案。
...
Method: 提出基于图的空间分布优化方法：1) 基于证据下界(ELBO)的自适应致密化策略；2) 透明度感知的渐进式剪枝策略；3) 基于图的特征编码模块，通过特征引导的点位移调整空间分布。

Result: 相比原始3DGS，GS^2仅使用约12.5%的高斯点就实现了更高的PSNR。在渲染质量和内存效率方面均优于所有基线方法。

Conclusion: GS^2通过优化高斯点的空间分布，实现了紧凑的高斯表示，在显著减少内存消耗的同时保持甚至提升了渲染质量。
...
TL;DR: 研究发现文本到图像生成模型存在低成本的提示词越狱攻击漏洞，通过简单的自然语言提示就能绕过安全过滤器，攻击成功率高达74.47%


<details>
  <summary>Details</summary>
Motivation: 文本到图像生成模型广泛应用于创意工具和在线平台，虽然依赖安全过滤器和审核管道来阻止有害内容，但现有系统仍然存在安全漏洞。本研究旨在揭示这些模型对低成本的提示词越狱攻击的脆弱性。

Method: 提出系统的提示词越狱攻击研究，引入视觉越狱技术分类学，包括艺术重构、材料替换、伪教育框架、生活方式美学伪装和模糊动作替换等策略。这些策略通过将不安全意图隐藏在良性语义上下文中来利用提示审核和视觉安全过滤的弱点。

Result: 在多个最先进的文本到图像系统中评估这些攻击，发现简单的语言修改就能可靠地绕过现有安全措施并生成受限图像。在所有测试模型和攻击类别中，攻击成功率(ASR)高达74.47%。

Conclusion: 研究结果突显了表面级提示过滤与检测生成媒体系统中对抗意图所需的语义理解之间存在关键差距。现有安全措施无法有效识别隐藏在良性语义上下文中的恶意意图。
...
Result: 在RRSIS-D和RISBench两个基准测试上的广泛实验表明，ProVG一致优于现有方法，达到了新的最先进性能。

Conclusion: ProVG通过解耦语言表达和渐进式跨模态调制，有效利用了细粒度的语言线索，显著提升了遥感视觉定位的准确性，为处理遥感图像中的大尺度变化和复杂语言描述提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在形状合成中难以平衡细节捕捉、计算效率和模型简洁性，需要一种能够同时优化这些因素的新框架。

Method: 1) 在表面内部体积中优化放置参考点，最大化稀疏性、中心性和表面可见性；2) 通过光线投射采样可见距离场；3) 使用快速球谐变换计算SH系数；4) 应用可配置低通滤波和基于邻近度的局部一致性约束进行细化。

Result: SHARC在重建精度和时间效率方面均优于现有最先进方法，同时保持了模型的简洁性。

Conclusion: SHARC通过球谐函数表示距离场和优化参考点放置，实现了高效、精确的形状合成，为几何处理提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 红外与可见光视频融合在智能监控和低光监测中至关重要，但现有方法要么侧重于帧级增强而时间建模有限，要么依赖重时空聚合而牺牲高频细节，难以同时保持时间稳定性和空间细节。

Method: FTPFusion将特征表示分解为高频和低频分量进行协同建模：高频分支执行稀疏跨模态时空交互以捕捉运动相关上下文和互补细节；低频分支引入时间扰动策略增强对复杂视频变化（如闪烁、抖动、局部错位）的鲁棒性；设计偏移感知时间一致性约束来显式稳定时间扰动下的跨帧表示。

Result: 在多个公共基准测试上的广泛实验表明，FTPFusion在空间保真度和时间一致性方面均优于现有最先进方法。

Conclusion: FTPFusion通过频率感知分解、稀疏跨模态交互和时间扰动策略，有效解决了红外与可见光视频融合中的时空稳定性和细节保留问题，在多个指标上表现出优越性能。
...
Method: 1) 用KAN卷积替换ResNet中的卷积层，实现自适应特征提取；2) 使用Gram多项式作为激活函数，适合SAR数据捕获复杂非线性关系；3) 采用参数共享策略，每个通道共享参数，减少参数和计算量

Result: 在MSTAR、FUSAR-Ship和SAR-ACD数据集上分别达到99.09%、93.01%和97.26%的准确率；在1024×1024 MSTAR数据集上，相比VGG16减少82.90倍FLOPs和163.78倍参数

Conclusion: Light-ResKAN为边缘设备上的SAR图像识别提供了高效解决方案，在保持高精度的同时显著降低了计算复杂度和参数数量
...
TL;DR: 提出了一种基于几何驱动的多光斑检测与匹配方法，将角膜反射光斑视为结构化星座而非独立斑点，提高了眼动追踪系统的可复现性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前角膜反射光斑检测通常作为启发式方法嵌入在更大的眼动追踪系统中，这使得不同硬件设置下的可复现性较差。需要一种更系统、可复现的方法来处理多光斑检测和匹配问题。

Method: 提出了一种2D几何驱动的星座式多光斑检测与匹配流程，包括相似性-布局对齐（SLA）过程，将星座匹配技术适配到多LED眼动追踪的特定约束中。框架结合了受控过检测、自适应候选回退、外观感知评分和可选的语义布局先验，同时保持检测和对应关系的明确分离。

Result: 在公开的多LED数据集上评估，该系统在噪声条件下提供了稳定的身份保持对应关系。作者发布了代码、预设和评估脚本，支持透明的复现、比较和数据集标注。

Conclusion: 该研究提出了一种可复现、几何驱动的多光斑检测与匹配框架，通过星座式方法提高了眼动追踪系统的鲁棒性和可移植性，为眼动追踪研究提供了更好的工具支持。
...
TL;DR: KnowMVG是一个用于医学视觉定位的知识先验和全局-局部注意力增强框架，通过编码医学知识嵌入和联合利用粗粒度全局信息与细粒度局部线索，显著提升了医学图像中诊断相关短语的定位精度。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在医学视觉定位任务中空间定位精度不足，主要原因是仅依赖潜在嵌入而缺乏显式的定位先验知识。作者从注意力机制的角度分析这一局限性，旨在增强解码过程中的空间感知能力。

Method: 提出KnowMVG框架，包含：1）知识增强提示策略，将短语相关的医学知识编码为紧凑嵌入；2）全局-局部注意力机制，联合利用粗粒度全局信息和细粒度局部线索来指导精确区域定位。该设计在不引入额外文本推理开销的情况下，桥接高级语义理解和细粒度视觉感知。

Result: 在四个MVG基准测试上的广泛实验表明，KnowMVG始终优于现有方法，在AP50指标上获得3.0%的提升，在mIoU指标上获得2.6%的提升，超越了先前的最先进方法。定性和消融研究进一步验证了每个组件的有效性。

Conclusion: KnowMVG通过知识先验和全局-局部注意力增强，有效解决了医学视觉定位中空间精度不足的问题，为临床决策提供了更可靠的视觉证据支持，在医学图像理解领域具有重要应用价值。
...
TL;DR: 该研究探索了能否直接从波束成形前的每天线距离-多普勒测量中学习有意义的空间结构，而不需要先构建角度域表示。实验使用48虚拟天线的汽车雷达，通过双啁啾共享权重编码器进行端到端训练，以鸟瞰图占用率作为几何探测指标。


<details>
  <summary>Details</summary>
Motivation: 传统汽车雷达感知流程通常先通过波束成形构建角度域表示，再应用学习模型。本研究旨在探索是否可以直接从波束成形前的每天线距离-多普勒测量中学习空间结构，避免显式的角度域构建和手工信号处理阶段。

Method: 使用6发射×8接收（48虚拟天线）的商用汽车雷达，采用A/B啁啾序列调频连续波传输方案。通过双啁啾共享权重编码器对波束成形前的每天线距离-多普勒张量进行端到端训练。监督信号来自激光雷达，采用可见性感知的跨模态监督，明确建模雷达视场和基于射线的可见性分析。

Result: 通过啁啾消融实验（仅A、仅B、A+B）、距离带分析和物理对齐基线评估，结果表明可以直接从波束成形前的每天线距离-多普勒张量中学习空间结构，无需显式的角度域构建或手工信号处理阶段。

Conclusion: 空间结构可以直接从波束成形前的每天线距离-多普勒测量中学习，这为汽车雷达感知提供了一种新的表示学习方法，避免了传统角度域构建的局限性。
...
TL;DR: 该研究提出了ECAC基准数据集和RSRS混合训练框架，用于提升早期儿童教育图像描述的专业性和准确性，并开发了KinderMM-Cap-3B模型，在专业玩具识别方面显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有图像描述方法在早期儿童教育领域面临两大挑战：缺乏大规模领域特定数据集导致描述泛化不精确，以及传统训练范式在提升专业物体描述能力方面存在局限。

Method: 1) 构建ECAC大规模基准数据集，包含256,121张真实世界图像，配有专家级描述和细粒度标签；2) 提出TTS评估协议专门衡量专业物体命名准确性；3) 设计RSRS混合训练框架，动态切换强化学习和监督微调，将零奖励困难样本重定向到监督学习。

Result: 开发的KinderMM-Cap-3B模型在TTS指标上达到51.06分，显著优于现有最先进基线，同时保持优秀的描述质量，展示了在专业教育应用中的潜力。

Conclusion: ECAC数据集和RSRS训练框架有效解决了早期儿童教育图像描述的专业性和准确性挑战，为专业教育应用提供了有力的技术基础。
...
TL;DR: 该研究提出AMIMV方法，通过非对称多图像多视图增强策略解决医学影像分类中的数据稀缺和类别不平衡问题，并在11个MedMNIST数据集上评估了8种自监督学习方法在长尾分布下的表现。


<details>
  <summary>Details</summary>
Motivation: 医学影像分析面临两大挑战：1）缺乏大量标注训练数据；2）数据不平衡问题，即常见类别数据丰富而罕见类别数据稀缺。虽然自监督学习（SSL）方法在一定程度上解决了第一个问题，但SSL方法在医学图像分类中对不平衡数据的鲁棒性研究较少。

Method: 1）扩展了先前提出的MIMV方法，引入新的增强策略构建非对称多图像多视图（AMIMV）对，同时解决数据稀缺和数据集不平衡问题；2）进行数据分析评估AMIMV在不同程度类别不平衡下的鲁棒性；3）在11个MedMNIST数据集上评估8种代表性SSL方法在长尾分布和有限监督下的表现。

Result: 在MedMNIST数据集上的实验结果显示，在retinaMNIST上提升4.25%，在tissueMNIST上提升1.88%，在DermaMNIST上提升3.1%。

Conclusion: AMIMV方法能有效应对医学影像分类中的数据稀缺和类别不平衡问题，在多个医学影像数据集上取得了显著性能提升，为不平衡医学数据下的自监督学习提供了有效解决方案。
...
TL;DR: MAVFusion是一个红外与可见光视频融合框架，通过运动感知稀疏交互机制，在保持高质量融合的同时显著提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法多为静态图像融合，无法有效处理视频帧间运动；现有视频融合方法虽能提升时间一致性，但计算成本高。需要一种既能保持时间一致性又能高效处理视频的融合方法。

Method: 提出端到端视频融合框架MAVFusion，采用运动感知稀疏交互机制：利用光流识别多模态序列中的动态区域，自适应地将计算密集的跨模态注意力分配到这些稀疏区域；对静态背景区域使用轻量级弱交互模块保持结构和外观完整性。

Result: 在多个红外与可见光视频基准测试中达到最先进性能，在640×480分辨率下达到14.16 FPS的速度，同时保持时间一致性和细节完整性。
...
Method: 使用nnU-Net v2框架构建深度学习模型，利用多模态mpMRI数据（T2加权、DWI、ADC）进行训练，在PI-CAI数据集的981个病例上进行训练，采用5折交叉验证和外部验证评估性能。

Result: 模型在交叉验证中平均Dice得分为0.96±0.00，在外部测试集（Hospital La Fe的54名患者）上为0.82，显著优于通用方法TotalSegmentator的0.15分。

Conclusion: 该研究强调了任务特异性、多模态分割策略的重要性，证明了所提方法可可靠集成到临床研究流程中，模型已容器化并作为即用推理工具提供。
...
TL;DR: MyEgo是首个评估多模态大语言模型在自我中心视频中个性化问答能力的基准数据集，包含541个长视频和5K个关于"我的物品"、"我的活动"、"我的过去"的问题。研究发现当前最先进的MLLMs在该任务上表现不佳，准确率远低于人类水平，表明自我定位和长期记忆能力是当前模型的瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在理解自我中心视频中相机佩戴者（即"我"）方面的能力尚未得到系统评估。为了推动个性化辅助系统的发展，需要建立一个专门评估模型理解、记忆和推理相机佩戴者相关信息的基准。

Method: 创建了MyEgo数据集，包含541个长视频和5K个个性化问题，涵盖三个类别："我的物品"、"我的活动"、"我的过去"。对多种MLLMs变体进行了基准测试，包括开源vs专有模型、思考vs非思考模型、小vs大规模模型，并分析了显式推理、模型缩放和相关证据提供对性能的影响。

Result: 顶级闭源模型（如GPT-5）准确率约46%，开源模型（如Qwen3-VL）约36%，分别比人类性能低近40%和50%。显式推理和模型缩放未能带来一致改进。提供相关证据时模型性能有所提升，但随时间推移增益下降，表明模型在跟踪和记忆"我"和"我的过去"方面存在局限。

Conclusion: 自我定位和长期记忆能力对于自我中心视频中的个性化问答至关重要。当前MLLMs在这方面存在显著不足，MyEgo数据集和分析为相关研究提供了重要基准，有望推动自我中心个性化辅助系统的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于文本描述的3D场景生成方法在短文本条件下存在物理合理性差和细节不足的问题，主要原因是过度依赖明确的物体组合和空间关系语义线索，需要增强3D推理能力，特别是先验整合和空间锚定

Method: 1. 多视角场景先验增强：通过聚合多视角结构知识丰富欠指定的文本输入；2. 功能感知布局定位：利用区域功能定位进行隐式空间锚定，进行分层布局推理；3. 迭代反射-修正方案：通过自修正实现渐进式结构合理性优化

Result: 在短文本条件下的3D室内场景生成任务上，该方法优于现有方法

Conclusion: SDesc3D通过多视角结构先验和区域功能推理，有效提升了短文本条件下3D室内场景生成的物理合理性和语义一致性
...
TL;DR: 提出了交互式跟踪新范式，创建了首个大规模交互式跟踪基准InteractTrack，评估发现现有跟踪器在交互场景中表现不佳，并提出了基于动态记忆机制的IMAT基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有视觉跟踪器主要在非交互、发射后不管的模式下运行，无法适应需要人机协作的真实世界场景，因此需要引入交互式跟踪新范式。

Method: 1) 创建InteractTrack基准：包含150个视频、密集边界框标注和时间戳语言指令；2) 提出评估协议并评估25个代表性跟踪器；3) 提出IMAT基线方法，采用动态记忆机制从用户反馈中学习并更新跟踪行为。

Result: 评估显示最先进的跟踪方法在交互场景中表现失败，传统基准上的强性能无法迁移到交互场景；IMAT基线方法为交互式跟踪提供了有效解决方案。
...
TL;DR: Curia-2是医学影像基础模型的改进版本，通过优化预训练策略和表征质量，首次将架构扩展到十亿参数级别，并在2D和3D评估基准上超越现有模型。


<details>
  <summary>Details</summary>
Motivation: 医学影像快速增长导致放射科医生工作量不可持续，需要开发基础模型来减轻负担。现有模型在CT和MRI分析方面仍有优化空间，需要更好地学习复杂放射学体积数据。

Method: 基于Curia框架改进预训练策略和表征质量，首次将架构扩展到十亿参数级别的视觉Transformer，支持多模态CT和MRI数据。同时将CuriaBench评估基准重构为2D切片模型和3D体积模型两个独立评估轨道。

Result: Curia-2在视觉任务上超越所有基础模型，在临床复杂任务（如病灶检测）上与视觉语言模型竞争相当。模型权重将公开以促进进一步研究。

Conclusion: Curia-2通过改进的预训练策略和十亿参数架构，显著提升了医学影像基础模型的性能，为放射学数据分析提供了更强大的工具，并通过标准化评估框架推动了该领域的发展。
...
TL;DR: 多模态大语言模型的视觉注意力存在惯性问题，早期解码步骤后注意力基本静止，无法支持认知推理所需的组合理解，现有方法对此类认知幻觉无效。


<details>
  <summary>Details</summary>
Motivation: 现有幻觉缓解方法主要针对物体存在或属性的感知幻觉，但对于需要对象间关系推理的认知幻觉效果不佳。研究发现视觉注意力存在惯性，一旦在早期解码步骤中稳定后基本保持静止，无法动态支持关系推理。

Method: 提出无需训练的惯性感知视觉激发（IVE）方法，将认知推理建模为视觉注意力的动态响应。IVE选择相对于历史注意力趋势动态涌现的视觉标记，同时区分表现出惯性行为的标记。引入惯性感知惩罚机制，防止注意力过度集中和局部区域的持久性。

Result: 大量实验表明，IVE在各种基础MLLMs和多个幻觉基准测试中均有效，特别是在认知幻觉方面表现突出。

Conclusion: 视觉注意力惯性是多模态大语言模型中认知幻觉的关键因素，IVE方法通过打破这种惯性模式，有效提升了模型对组合推理的能力，为认知幻觉缓解提供了新的解决方案。
...
TL;DR: Resonance4D是一个物理驱动的4D动态模拟框架，通过双域运动监督和全参数物理恢复，在单消费级GPU上实现高保真4D模拟，大幅降低计算成本和内存占用。
...
Method: 提出Resonance4D框架，将3D高斯泼溅与物质点法耦合，通过双域运动监督（空间结构一致性和频域谱一致性）和全参数物理恢复（零样本文本提示分割与模拟引导初始化）来实现高效物理模拟。

Result: 在合成和真实场景实验中，Resonance4D实现了强大的物理保真度和运动一致性，同时将峰值GPU内存从超过35GB降低到约20GB，可在单消费级GPU上运行。

Conclusion: Resonance4D通过创新的双域运动监督和全参数优化方法，解决了现有4D动态模拟的计算效率和物理真实性问题，为高质量物理驱动模拟提供了实用解决方案。
...
TL;DR: ProDiG是一个渐进式扩散引导框架，通过合成中间高度视图和几何感知的因果注意力模块，将航拍3D表示逐步转换为地面级保真度，解决了航拍到地面极端视角变化下的3D场景重建问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从纯航拍图像生成地面视图和连贯3D场景模型时面临挑战，包括极端视角变化、缺少中间观测和大尺度变化。现有方法要么后处理渲染产生几何不一致结果，要么依赖多高度地面真值数据（通常难以获取）。高斯溅射和基于扩散的细化方法在小变化下有效，但在大航拍到地面差距下失败。

Method: ProDiG（渐进式高度高斯溅射）是一个扩散引导框架，逐步将航拍3D表示转换为地面级保真度。包含两个核心模块：1）几何感知因果注意力模块，将极线结构注入参考视图扩散中；2）距离自适应高斯模块，根据相机距离动态调整高斯尺度和不透明度，确保在大视角差距下的稳定重建。

Result: 在合成和真实世界数据集上的大量实验表明，ProDiG能生成视觉逼真的地面级渲染和连贯的3D几何，在视觉质量、几何一致性和对极端视角变化的鲁棒性方面显著优于现有方法。

Conclusion: ProDiG通过渐进式扩散引导框架，在不需额外地面真值视角的情况下，实现了从航拍到地面视图的几何一致转换，解决了极端视角变化下的3D场景重建难题。
...
TL;DR: DR-Seg提出了一种解耦与校正框架，用于遥感图像的开集语义分割，通过分离CLIP特征为语义主导和结构主导子空间，在保持语义完整性的同时增强边界细节。


<details>
  <summary>Details</summary>
Motivation: 遥感开集语义分割需要语言对齐识别和精细空间划分。CLIP具有语义泛化能力但缺乏结构细节，现有方法引入DINO特征但将CLIP视为单一语义空间，无法定位结构增强需求，可能破坏CLIP语义完整性。

Method: 提出DR-Seg框架：1）基于CLIP特征通道功能异质性，将其解耦为语义主导和结构主导子空间；2）在DINO指导下通过先验驱动图校正模块注入高保真结构先验形成精炼分支；3）不确定性引导自适应融合模块动态集成精炼分支和原始CLIP分支进行最终预测。

Result: 在八个基准测试上的综合实验表明，DR-Seg建立了新的最先进性能。

Conclusion: DR-Seg通过解耦CLIP特征并针对性增强结构信息，有效解决了遥感开集语义分割中语言对齐识别与精细空间划分的平衡问题，实现了语义完整性和边界细节的优化。
...
TL;DR: LinkS²Bench是首个评估视觉语言模型在无人机-卫星协同空间智能方面的综合基准，包含17.9k高质量问答对，覆盖12个细粒度任务，揭示了现有模型在跨视图动态对齐上的显著瓶颈。
...
Method: 构建LinkS²Bench基准，将1022分钟动态无人机视频与覆盖200+平方公里高分辨率卫星图像关联；通过LMM辅助流程和严格人工标注，创建17.9k高质量问答对，涵盖感知、定位、关系和推理四个维度的12个细粒度任务；设计跨视图对齐适配器来缓解对齐瓶颈。

Result: 评估18个代表性视觉语言模型发现，与人类基线存在显著差距，准确跨视图动态对齐是关键瓶颈；跨视图对齐适配器显著提升模型性能；微调实验证明LinkS²Bench在推进复杂空间推理模型适应方面的潜力。

Conclusion: LinkS²Bench填补了评估视觉语言模型在无人机-卫星协同空间智能方面的空白，揭示了跨视图动态对齐的关键挑战，提出的对齐适配器有效提升性能，为推进复杂空间推理的视觉语言模型发展提供了重要基准。
...
TL;DR: 论文提出了一种针对空间不平衡图像数据的自编码器改进方法，通过自熵损失和样本传播机制来提升对罕见空间位置的重建质量。


<details>
  <summary>Details</summary>
Motivation: 在医学成像、生物学和物理学等领域，图像内容的空间采样通常不均匀，信息丰富的模式在特定坐标上很少出现，而背景在这些位置占据主导。自编码器倾向于学习主导模式，导致对罕见空间输入的重建模糊，丢失细粒度细节。
...
Conclusion: 该方法强调了批次中数据表示的重要性，并在无监督图像重建中突出了罕见样本的价值。研究结果展示了针对空间不平衡问题的专门解决方案的有效性。
...
TL;DR: IndoorCrowd是一个多场景室内人群数据集，包含31个视频（9,913帧），用于人体检测、实例分割和多目标跟踪，在四个校园地点采集，包含人工验证的实例分割掩码和跟踪标注。
...
Method: 在四个校园地点（ACS-EC、ACS-EG、IE-Central、R-Central）收集31个视频，提供人工验证的实例分割掩码。使用620帧子集评估三个基础模型自动标注器（SAM3、GroundingSAM、EfficientGroundingSAM），使用2,552帧子集支持多目标跟踪。建立检测、分割和跟踪基线模型。

Result: 数据集包含9,913帧图像，评估了自动标注器的性能（Cohen's κ、AP、精度、召回率、掩码IoU）。场景分析显示难度差异显著：ACS-EC场景（79.3%密集帧，平均实例尺度60.8像素）最具挑战性。建立了YOLOv8n、YOLOv26n、RT-DETR-L与ByteTrack、BoT-SORT、OC-SORT的基线结果。

Conclusion: IndoorCrowd数据集填补了大规模真实室内人群数据集的空白，提供了多任务标注和基准测试，揭示了场景特性（人群密度、尺度、遮挡）对算法性能的重要影响。
...
TL;DR: WISE提出了一种高效推理范式，通过"思考两次"原则（一次学习，一次加速），在推理时生成简洁的理性分析，大幅减少计算成本，同时保持高性能。


<details>
  <summary>Details</summary>
Motivation: 现有思维链推理方法在语言引导分割任务中虽然性能显著，但生成冗长理性分析导致计算成本过高，限制了实际应用。

Method: WISE训练模型生成结构化序列：简洁理性→最终答案→详细解释。通过自蒸馏目标强化语义保真度和简洁性，推理时使用WISE-S策略注入简洁性指令，仅生成简洁理性。

Result: 在ReasonSeg基准测试中实现58.3 cIoU的零样本性能，同时将平均推理长度从112个token减少到23个，压缩近5倍。

Conclusion: WISE通过将详细推理内化为简洁形式，实现了高效且高性能的推理范式，为大型多模态模型的实际应用提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 构建高质量的多语言和非英语视觉语言模型面临的主要障碍是其他语言的视觉问答数据集在规模和领域覆盖上都有限。英语可以通过聚合现有VQA资源构建数据集，但这一策略不适用于其他语言。

Method: 收集异构源数据（图像、图像-文本对、PDF文档），通过多种策略生成VQA对：基于VLM的问答生成、翻译和文本渲染。构建了包含约920万个实例的日语多模态数据集。

Result: 使用Jagle训练的22亿参数模型在日语任务上表现强劲，在十个日语评估任务中平均得分超过InternVL3.5-2B，接近Qwen3-VL-2B-Instruct（差距在5分以内）。结合Jagle和FineVision训练不仅不会降低英语性能，反而比单独使用FineVision表现更好。

Conclusion: Jagle数据集有效解决了日语VLM训练数据稀缺问题，通过创新的数据生成策略构建了大规模日语多模态数据集，显著提升了日语VLM性能，同时不影响英语能力。作者发布了数据集、模型和代码以促进可复现性和未来研究。
...
TL;DR: 提出了一种全自动、可扩展的方法来系统评估虚拟人渲染中的肤色保真度，通过肤色提取、纹理重着色、实时渲染和定量色彩分析的工作流程，发现肤色提取策略存在表型依赖性，深色肤色的色度误差更高。


<details>
  <summary>Details</summary>
Motivation: 虚拟人渲染中准确再现面部肤色对于真实感、身份保持和公平性至关重要，但大多数可访问的虚拟形象创建流程依赖于缺乏色彩校准的摄影输入，这会引入不一致性和偏见。

Method: 提出全自动工作流程，整合肤色和光照提取、纹理重着色、实时渲染和定量色彩分析。使用芝加哥面部数据库图像，比较基于脸颊区域采样和全脸分析多维掩码的肤色提取策略，结合TRUST框架进行光照隔离。将提取的肤色应用于MetaHuman纹理并在多种光照配置下渲染，在CIELAB色彩空间中使用ΔE度量和个体类型学角度进行客观评估。

Result: 生成并分析了约19,848个渲染实例，结果显示提取策略存在表型依赖性行为，深色肤色的色度误差始终更高。

Conclusion: 该方法无需人工干预，计算成本低，支持大规模评估，揭示了肤色保真度评估中的系统性偏差，为改进虚拟人渲染的肤色再现提供了量化框架。
...
TL;DR: 论文提出Multi-Object Affordance Grounding under Intent-Driven Instructions新任务，要求在多物体点云中根据隐式语言意图预测正确物体的功能掩码，并构建首个基准数据集CompassAD和解决方案CompassNet。


<details>
  <summary>Details</summary>
Motivation: 现实场景中多个物体可能具有相同功能（如刀和剪刀都能"切苹果"），但只有一个是任务合适的。现有3D功能方法回避这一挑战，仅评估孤立单物体且常提供明确类别名称。需要解决多物体场景中基于隐式意图的功能定位问题。

Method: 提出CompassNet框架：1) Instance-bounded Cross Injection (ICI)模块，限制语言-几何对齐在物体边界内，防止跨物体语义泄漏；2) Bi-level Contrastive Refinement (BCR)模块，在几何组和点级别强制区分，增强目标与混淆表面的辨别。

Result: 构建CompassAD基准数据集，包含30个混淆物体对、16种功能类型、6,422个场景、88K+查询-答案对。实验显示在可见和未见查询上均达到最先进结果。机器人操作器部署证实能有效转移到真实世界多物体场景的抓取任务。

Conclusion: 论文形式化了多物体功能定位新任务，解决了混淆物体对场景中的功能选择问题。提出的CompassNet框架通过ICI和BCR模块有效处理语言-几何对齐和区分混淆表面，为机器人理解隐式意图提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有显式思维链方法虽然能改善多模态嵌入，但存在推理开销大、将丰富多模态证据压缩为狭窄文本瓶颈的问题

Method: PLUME采用潜在推理框架，用短自回归潜在状态展开替代显式思维链；引入语义锚点引导的转换适配器指导不同推理轨迹；采用渐进式显式到潜在课程训练策略

Result: 在78个任务的MMEB-v2基准测试中，PLUME优于显式思维链基线，推理步骤从数百个生成token减少到少于10个潜在步骤，推理速度提升30倍以上
...
TL;DR: FlowSlider是一种无需训练的连续图像编辑方法，通过分解流编辑更新为保真项和转向项，实现平滑的滑块式强度控制。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的滑块方法通常依赖辅助模块和合成监督，需要额外训练开销，且滑块行为与训练分布耦合，在编辑或域分布偏移时可靠性降低。

Method: 提出FlowSlider方法，将Rectified Flow中的FlowEdit更新分解为：(1)保真项：作为源条件稳定器，保持身份和结构；(2)转向项：驱动语义向目标编辑过渡。几何分析和经验测量显示这两项近似正交，通过仅缩放转向项而保持保真项不变来实现稳定的强度控制。

Result: FlowSlider无需后训练即可提供平滑可靠的控制，在各种任务中提高了连续编辑质量。

Conclusion: FlowSlider是一种无需训练的连续图像编辑方法，通过正交分解实现稳定的滑块式控制，避免了现有方法对训练分布的依赖问题。
...
TL;DR: GroundVTS：一种用于视频时间定位的视觉大语言模型架构，通过查询引导的细粒度视觉令牌采样机制，聚焦信息最丰富的时序片段，提升视频时间定位性能。
...
Method: 提出Grounded Visual Token Sampling (GroundVTS)架构：1）使用细粒度的查询引导机制在输入LLM前过滤视觉令牌，保留关键时空信息；2）引入渐进优化策略，使LLM适应视觉特征的非均匀分布。

Result: 在三个标准VTG基准测试中，GroundVTS优于现有方法：在时刻检索任务上mIoU提升7.7个百分点，在高光检测任务上mAP提升12.0个百分点。

Conclusion: GroundVTS通过聚焦信息丰富的时序片段和渐进优化策略，有效提升了视频大语言模型的时间定位能力，为视频理解任务提供了更有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有统一模型需要像素解码作为视觉理解与生成之间的桥梁，这种分离的视觉表示既低效又低效。需要一种更有效的跨模态推理和生成方法。

Method: 引入LatentUM，在共享语义潜在空间中表示所有模态，消除了像素空间中介，使视觉理解与生成在相同表示空间中直接交互。

Result: LatentUM在视觉空间规划基准上达到最先进性能，通过自反思提升视觉生成质量，支持在共享语义潜在空间中预测未来视觉状态的世界建模。

Conclusion: 共享语义潜在空间表示显著提高了计算效率，减轻了编解码器偏差，增强了跨模态对齐，为实现灵活的交错跨模态推理和生成提供了有效途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有遥感变化检测方法通常假设固定的标签空间，无法响应用户自定义的概念查询。在完全无需训练的设置下，密集概念响应直接跨时间比较存在困难：外观变化、弱跨概念竞争以及许多土地覆盖类别的空间连续性导致噪声、碎片化和语义不可靠的变化证据。
...
Result: 在四个基准测试中（包括建筑导向和多类设置），CoRegOVCD始终优于先前最强的无需训练基线，F1_C分数提升2.24到4.98个百分点，在SECOND数据集上达到六类平均47.50%的F1_C分数。

Conclusion: CoRegOVCD通过后验校准和一致性正则化，有效解决了开放词汇变化检测中的语义可比性问题，无需训练即可实现可靠的概念查询变化检测，显著提升了性能。
...
TL;DR: 该论文指出面部动作单元检测中标准的主体排他交叉验证存在可测量的随机方差，在BP4D+数据集上产生±0.065的平均F1噪声，并建议采用留一数据集交叉验证以获得更稳定结果。


<details>
  <summary>Details</summary>
Motivation: 面部动作单元检测研究中，主体排他交叉验证是标准评估协议，但报告的改进往往很小。作者发现交叉验证本身引入了可测量的随机方差，这可能导致对模型性能的误判。

Method: 1. 在BP4D+数据集上进行重复3折主体排他分割，测量经验噪声水平；2. 使用留一数据集协议在五个AU数据集上评估跨数据集鲁棒性；3. 比较F1等操作点指标与AUC等阈值无关指标的波动性。

Result: 1. 重复3折分割产生±0.065的平均F1经验噪声，低流行度AU的波动更大；2. F1等操作点指标比AUC波动更大，模型排名可能因不同折叠分配而改变；3. 留一数据集协议揭示了单数据集交叉验证中不可见的域级不稳定性。

Conclusion: 交叉验证中经常报告的改进可能落在协议方差范围内。留一数据集交叉验证能产生更稳定和可解释的结果，建议作为更可靠的评估协议。
...
TL;DR: 本文提出了一种反射生成方法，通过向基础扩散模型注入反射位置和外观的先验信息，并采用类型感知的模型设计，在首个大规模物体反射数据集DEROBA上进行训练，实现了物理一致且视觉逼真的反射效果。
...
Conclusion: 本文提出了一种有效的反射生成方法，通过注入先验信息和类型感知设计，在构建的大规模数据集上训练，成功实现了高质量、物理一致的反射效果生成，填补了反射生成研究领域的空白。
...
<details>
  <summary>Details</summary>
Motivation: Transformer架构已成为NLP和CV的共享骨干，但理解这些模型如何工作仍然具有挑战性，特别是在视觉设置中。现有可解释性工具通常关注孤立组件或专家导向分析，缺乏对完整推理流程的引导式端到端理解。

Method: 开发了ViT-Explainer系统，提供Vision Transformer推理的集成可视化，包括动画演示、分块级注意力覆盖图和视觉适配的Logit Lens，支持引导和自由探索两种模式。

Result: 对6名参与者的用户研究表明，ViT-Explainer易于学习和使用，能够帮助用户解释和理解Vision Transformer的行为。

Conclusion: ViT-Explainer填补了Vision Transformer可解释性工具的空白，通过交互式可视化系统为用户提供了端到端的模型理解支持。
...
<details>
  <summary>Details</summary>
Motivation: 针对真实驾驶场景中车道线检测的鲁棒性需求，需要同时捕获空间和时间信息，现有方法在计算复杂度和性能平衡方面存在改进空间。

Method: 提出两种模型：1）使用3D-ResNet编码器和PINet解码器，结合FPN和自注意力机制增强多尺度特征表示；2）引入ROI检测头选择性关注车道相关区域，降低计算复杂度。

Result: 在TuSimple数据集上，第二种模型达到93.40%准确率，显著减少假阴性，相比现有2D和3D基线方法，参数量更少、延迟更低，性能更优。

Conclusion: 提出的模型适合集成到高级驾驶辅助系统（ADAS），具有向完整车道辅助系统（LAS）扩展的潜力，在真实驾驶场景中表现出鲁棒性能。
...
TL;DR: UniDriveVLA通过专家解耦的混合Transformer架构，解决了自动驾驶中VLA模型空间感知与语义推理的冲突问题，在多个任务上取得了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶中的VLA模型面临空间感知与语义推理的困境：直接采用2D VLM空间感知有限，而增强3D表示又会损害VLM的原生推理能力。这种困境源于空间感知和语义推理在共享模型参数中的耦合优化。

Method: 提出UniDriveVLA统一驾驶VLA模型，基于混合Transformer架构，通过专家解耦解决感知-推理冲突。包含三个专家：驾驶理解专家、场景感知专家和动作规划专家，通过掩码联合注意力协调。结合稀疏感知范式和三阶段渐进训练策略，在保持语义推理能力的同时提升空间感知。

Result: 在nuScenes的开环评估和Bench2Drive的闭环评估中达到最先进性能。在3D检测、在线建图、运动预测和驾驶导向VQA等广泛任务上表现出色，展示了作为自动驾驶统一模型的广泛适用性。

Conclusion: UniDriveVLA通过专家解耦架构成功解决了自动驾驶VLA模型中的感知-推理冲突，实现了空间感知和语义推理的平衡，为自动驾驶提供了一个强大的统一模型框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有零样本骨架动作识别方法依赖显式的骨架-文本对齐，但在动作名称无法充分描述细粒度动态特征以及未见类别语义混淆时表现脆弱。需要更鲁棒的方法来处理这些挑战。

Method: 提出SCALE框架，将ZSAR建模为类条件能量排序。使用文本条件变分自编码器，冻结文本表示参数化潜在先验和解码器。引入语义和置信度感知的列表能量损失，强调语义相似的困难负样本并融入后验不确定性。还使用潜在原型对比目标来对齐后验均值与文本导出的潜在原型。

Result: 在NTU-60和NTU-120数据集上的实验表明，SCALE持续优于先前的VAE和对齐基线方法，同时与基于扩散的方法保持竞争力。

Conclusion: SCALE通过能量排序框架和语义感知的损失函数，有效解决了零样本骨架动作识别中的语义混淆和细粒度动态描述不足的问题，提供了一种轻量级且鲁棒的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 基础视觉Transformer在需要细粒度空间理解的任务中存在局限性，特别是在密集预测场景中，现有方法通常使用滑动窗口策略处理高分辨率图像，虽然提高了准确性但计算成本显著。

Method: 提出SPAR方法，通过特征回归损失将精细步长的滑动窗口教师模型的空间推理能力蒸馏到单次推理学生模型中，无需架构更改或像素级监督，实现分辨率无关的密集特征提取。

Result: 在开放词汇分割任务中，SPAR将单次推理基线提升了高达10.5 mIoU，甚至超过了教师模型的性能，证明了在高分辨率推理中的有效性。

Conclusion: SPAR通过知识蒸馏方法实现了高效的高分辨率推理，解决了基础视觉Transformer在细粒度空间理解任务中的局限性，为密集预测任务提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有安全方法通常依赖于模型微调或精选数据集，这会降低生成质量或限制可扩展性。需要一种无需训练、保持生成质量的安全控制方法

Method: 利用视觉语言基础模型作为现成的监督信号，通过梯度反馈在推理时引导生成。将安全引导表述为基于能量的采样问题，在每个采样步骤注入干净的潜在估计

Result: 在NSFW红队测试基准上达到最先进的鲁棒性，有效实现多目标引导，同时在良性非目标提示上保持高质量生成

Conclusion: 该框架为基础模型作为语义能量估计器提供了原则性方法，实现了可靠且可扩展的文本到图像生成安全控制，兼容扩散模型和流匹配模型
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在文本和图像理解与生成方面表现优异，但扩展到3D领域面临挑战，主要原因是高质量3D数据稀缺，导致3D合成约束不足。现有方法通常依赖间接管道，先在2D中编辑，然后通过优化提升到3D，牺牲了几何一致性。

Method: 提出Omni123模型，将文本、图像和3D表示为共享序列空间中的离散标记。采用交错X-to-X训练范式，协调异构配对数据集上的多样化跨模态任务，无需完全对齐的文本-图像-3D三元组。通过在自回归序列中遍历语义-视觉-几何循环，联合强制执行语义对齐、外观保真度和多视图几何一致性。

Result: 实验表明，Omni123显著改善了文本引导的3D生成和编辑，展示了向多模态3D世界模型扩展的可扩展路径。

Conclusion: Omni123通过统一文本到2D和文本到3D生成，利用2D数据作为几何先验，解决了3D数据稀缺问题，为构建多模态3D世界模型提供了有前景的方向。
...
TL;DR: 提出了一种快速表面配准方法，将表面网格视为概率测度，使用切片Wasserstein距离度量差异，并开发了AdamFlow优化算法在概率空间进行优化。
...
Method: 1) 将表面网格表示为概率测度；2) 使用具有对数线性计算复杂度的切片Wasserstein距离度量网格差异；3) 提出AdamFlow优化方法，将Adam优化器从欧几里得空间推广到概率空间，用于最小化切片Wasserstein距离。

Result: 1) 理论分析了AdamFlow的渐近收敛性；2) 在多种解剖结构的仿射和非刚性表面配准中，实证证明了AdamFlow的优越性能。

Conclusion: 该方法通过将表面配准重新表述为分布优化问题，结合高效的切片Wasserstein距离和AdamFlow优化器，实现了既高效又鲁棒的表面配准，在医学图像解剖形状分析中具有重要应用价值。
...
TL;DR: VOID是一个视频对象移除框架，专注于处理对象移除后需要改变下游物理交互的复杂场景，通过生成物理上一致的替代结果来实现更真实的视频编辑。


<details>
  <summary>Details</summary>
Motivation: 现有视频对象移除方法在处理对象遮挡内容、阴影和反射等外观层面问题表现良好，但当移除的对象与其他对象有显著交互（如碰撞）时，现有模型无法修正这些物理交互，导致结果不真实。

Method: 1) 使用Kubric和HUMOTO生成包含反事实对象移除的配对数据集；2) 推理时使用视觉语言模型识别受移除对象影响的场景区域；3) 用视频扩散模型生成物理一致的替代结果。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D化身建模面临关键权衡：多视角工作室数据能实现高保真但泛化性差；大规模野外数据模型泛化性好但质量低。需要同时实现高保真度和大规模泛化能力。

Method: 采用预训练/后训练范式：先在100万野外视频上预训练学习外观和几何的广泛先验，然后在高质量策划数据上进行后训练以增强表现力和保真度。模型能泛化到不同发型、服装和人口特征，同时提供精细的面部表情和手指级关节控制。

Result: LCA实现了高保真度的全身3D化身建模，能前馈式泛化到世界规模人群，支持高效推理。模型展现出对重光照、宽松服装支持等新兴泛化能力，以及对风格化图像的零样本鲁棒性。

Conclusion: LCA通过大规模预训练与高质量后训练相结合，成功解决了3D化身建模中保真度与泛化性的根本矛盾，为大规模高质量化身建模提供了有效解决方案。
...
TL;DR: 论文提出了场景式视觉定位新任务和RSC基准，包含段落级描述而非简单指称表达，需要理解角色、意图和关系，比现有基准更具挑战性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉定位基准主要评估图像区域与字面指称表达的对齐，模型通常可以通过匹配显著命名类别来成功。作者探索了更具挑战性的场景式视觉定位，目标需要从角色、意图和关系上下文中推断而非显式命名。

Method: 引入Referring Scenario Comprehension (RSC)基准，包含段落长度的文本描述对象角色、用户目标和上下文线索，包括对干扰对象的故意引用。提出ScenGround方法，结合监督预热和难度感知强化学习的课程推理方法。

Result: RSC包含约31k训练样本、4k域内测试样本和3k分布外分割。实验显示场景式查询暴露了当前模型在标准基准中未揭示的系统性失败，课程训练提高了在挑战性切片上的性能并迁移到标准基准。

Conclusion: 场景式视觉定位比传统视觉定位更具挑战性，RSC基准和ScenGround方法为这一新设置提供了参考点，揭示了当前模型的局限性并展示了改进方向。
...
TL;DR: Steerable Visual Representations：通过早期融合文本到视觉编码器中，实现可用自然语言引导的视觉特征表示，既能聚焦任意感兴趣物体，又保持底层表示质量。


<details>
  <summary>Details</summary>
Motivation: 现有预训练视觉Transformer（如DINOv2、MAE）的特征关注最显著视觉线索，无法引导到次要概念；而多模态LLM虽可文本引导，但特征偏向语言中心化，失去通用视觉任务有效性。

Method: 提出可引导视觉表示新类别，通过轻量级交叉注意力将文本直接注入视觉编码器层（早期融合），而非像CLIP等模型在编码后融合文本与视觉特征（后期融合）。

Result: 可引导视觉特征能聚焦图像中任意目标物体，同时保持底层表示质量；在异常检测和个性化物体识别任务上匹配或优于专用方法，展现对分布外任务的零样本泛化能力。

Conclusion: 通过早期文本融合实现可引导视觉表示，解决了现有视觉表示无法文本引导和多模态LLM特征语言中心化的问题，在保持通用视觉任务有效性的同时实现精确概念引导。
...
TL;DR: ModMap是一个原生多视角多模态的3D异常检测与分割框架，通过跨模态特征映射和视角相关特征调制，在SiM3D基准上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法独立处理不同视角，缺乏对多视角和多模态数据的有效整合。需要一种能够同时处理多个视角和多种模态（如深度信息）的3D异常检测框架。

Method: 1. 采用跨模态特征映射范式，学习跨模态和跨视角的特征映射；2. 通过特征调制显式建模视角依赖关系；3. 引入跨视角训练策略，利用所有可能的视角组合；4. 通过多视角集成和聚合进行异常评分；5. 为工业数据集训练并公开了基础深度编码器。

Result: 在SiM3D基准测试中，ModMap取得了最先进的性能，大幅超越了先前的方法。SiM3D是首个用于3D异常检测和分割的多视角多模态基准。

Conclusion: ModMap通过原生多视角多模态设计、跨视角特征映射和显式视角关系建模，为3D异常检测和分割提供了有效的解决方案，在工业数据集上表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有合成数据集的真实性和时间一致性有限，阻碍了生成式逆渲染和正向渲染在真实世界场景中的扩展。需要弥合这一领域差距，提升渲染系统的实际应用能力。

Method: 1. 使用新颖的双屏拼接捕获方法从视觉复杂的AAA游戏中提取数据；2. 构建包含400万连续帧（720p/30 FPS）的大规模动态数据集，包含同步的RGB和五个G-buffer通道；3. 提出基于视觉语言模型（VLM）的评估协议，用于在没有真实标注的情况下评估逆渲染性能。

Result: 1. 数据集显著提升了双向渲染能力：支持鲁棒的野外几何和材质分解，以及高保真的G-buffer引导视频生成；2. 在数据集上微调的逆渲染器实现了优越的跨数据集泛化能力和可控生成；3. VLM评估结果与人类判断高度相关；4. 结合工具包，正向渲染器允许用户通过文本提示编辑AAA游戏的G-buffer风格。

Conclusion: 该研究通过从AAA游戏中构建大规模动态数据集，有效解决了生成式渲染领域的真实性和时间一致性问题。提出的VLM评估协议为无真实标注的逆渲染评估提供了可靠方法，整体方案显著提升了双向渲染的实际应用能力。
...
TL;DR: ActionParty是一个用于生成式视频游戏的动作可控多主体世界模型，解决了现有视频扩散模型中动作绑定问题，能够同时控制最多7个玩家在46个不同环境中互动。
...
Method: 提出主体状态令牌作为潜在变量持续捕捉每个主体的状态，通过空间偏置机制联合建模状态令牌和视频潜在表示，从而解耦全局视频帧渲染与个体动作控制的主体更新。

Result: 在Melting Pot基准测试中，ActionParty成为首个能够同时控制最多7个玩家在46个不同环境中的视频世界模型，在动作跟随准确性和身份一致性方面显著提升，并能通过复杂交互实现鲁棒的自回归主体追踪。

Conclusion: ActionParty通过引入主体状态令牌和空间偏置机制，成功解决了多主体视频扩散模型中的动作绑定问题，为生成式视频游戏提供了强大的多主体世界建模能力。
