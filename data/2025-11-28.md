...
TL;DR: MoGAN是一个专注于提升视频生成中运动真实性的后训练框架，通过光学流判别器和分布匹配正则化器，显著改善视频扩散模型的运动质量，同时保持视觉保真度。


<details>
  <summary>Details</summary>
Motivation: 现有的视频扩散模型在帧级保真度上表现良好，但在运动一致性、动态性和真实性方面存在不足，常产生抖动、重影或不合理的动态效果。标准去噪MSE目标缺乏对时间一致性的直接监督。
...
TL;DR: Qwen3-VL是Qwen系列中最强大的视觉语言模型，支持256K令牌的交错上下文，在多个多模态基准测试中表现优异。模型家族包括密集型和混合专家变体，具备更强的纯文本理解、长上下文理解和高级多模态推理能力。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够无缝整合文本、图像和视频，并在纯文本理解、长上下文处理和多模态推理方面都表现卓越的视觉语言模型，以满足现实工作流程中图像基础推理、智能决策和多模态代码智能的需求。

Method: 引入三个关键架构升级：增强的交错MRoPE用于改进时空建模；DeepStack集成以加强视觉语言对齐；基于文本的时间对齐方法，从T-RoPE演进到显式文本时间戳对齐。

Result: 在可比较的令牌预算和延迟约束下，Qwen3-VL在密集型和混合专家架构中都实现了卓越性能，在MMMU、MathVista和MathVision等综合评估中表现出领先性能。

Conclusion: Qwen3-VL有望成为现实工作流程中图像基础推理、智能决策和多模态代码智能的基础引擎，为多模态AI应用提供强大支持。
...
<details>
  <summary>Details</summary>
Motivation: AI模型在日常设备中普及，但预测错误会降低用户体验。现有解决方案主要关注错误检测，很少提供有效的纠正机制，特别是在资源受限的设备上。

Method: 系统包含两个关键组件：(1) 服务器端管道，利用知识蒸馏将基础模型的鲁棒特征表示转移到设备兼容架构；(2) 设备端机制，通过原型适应实现超高效错误纠正。

Result: 在Food-101和Flowers-102数据集上，单样本场景下实现超过50%的错误纠正率，同时保持极低的遗忘率（小于0.02%）和可忽略的计算开销。

Conclusion: 通过Android演示应用验证了系统在实际场景中的实用性，证明了该系统的有效性和可行性。
...
<details>
  <summary>Details</summary>
Motivation: 长时动作质量评估（如花样滑冰、艺术体操）需要建模长期时序动态并保持对上下文混淆因素的鲁棒性。现有方法依赖昂贵标注或单向时序建模，易受伪相关性和不稳定长期表示的影响。

Method: 提出CaFlow框架，包含因果反事实正则化（CCR）模块和双向时间条件流（BiT-Flow）模块。CCR以自监督方式解耦因果和混淆特征，通过反事实干预增强因果鲁棒性；BiT-Flow通过前向和后向动态建模与循环一致性约束生成更平滑一致的表示。

Result: 在多个长时AQA基准测试上的广泛实验表明，CaFlow达到了最先进的性能。

Conclusion: CaFlow通过整合反事实去混淆和双向时序建模，有效解决了长时动作质量评估中的时序动态建模和上下文混淆问题，实现了优越的性能。
...
TL;DR: Multi-Crit是一个评估多模态模型作为评判者的基准，专注于测试模型遵循多样化细粒度评估标准的能力，涵盖开放式生成和可验证推理任务。


<details>
  <summary>Details</summary>
Motivation: 当前大型多模态模型（LMMs）越来越多地被用作多模态评估系统的评判者，但它们在遵循多样化、细粒度评估标准方面的能力尚未得到充分探索。
...
TL;DR: 提出ADVLA框架，通过在视觉编码器投影到文本特征空间的特征上直接应用对抗扰动，在低幅度约束下有效破坏下游动作预测，注意力引导使扰动既集中又稀疏。


<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击方法需要昂贵的端到端训练且通常生成明显的扰动补丁，ADVLA旨在解决这些限制。

Method: 在视觉编码器投影到文本特征空间的特征上直接应用对抗扰动，引入三种策略增强敏感性、强制稀疏性和集中扰动，结合Top-K掩码修改少于10%的补丁。

Result: 在L∞=4/255约束下，ADVLA实现接近100%的攻击成功率，扰动集中在关键区域，整体图像中几乎不可察觉，单步迭代仅需约0.06秒，显著优于传统基于补丁的攻击。
...
TL;DR: 本文首次系统研究仅通过相机轨迹（而非像素）来感知视频内容的可行性，提出CamFormer对比学习框架，将相机姿态轨迹投影到与自然语言对齐的嵌入空间，证明相机轨迹是揭示视频内容的重要信号。


<details>
  <summary>Details</summary>
Motivation: 探索是否可以不依赖像素信息，仅通过相机运动轨迹来理解视频内容，研究"如何移动"能否揭示"在做什么"或"在观察什么"。
...
TL;DR: Canvas-to-Image是一个统一的框架，将文本提示、主题参考、空间排列、姿态约束和布局注释等多种控制信号整合到单一画布界面中，通过复合画布图像实现多模态控制，显著提升了生成图像的保真度和控制精度。


<details>
  <summary>Details</summary>
Motivation: 现代扩散模型在生成高质量多样化图像方面表现出色，但在处理高保真度的组合和多模态控制方面仍存在困难，特别是当用户同时指定文本提示、主题参考、空间排列、姿态约束和布局注释时。

Method: 提出Canvas-to-Image框架，将多样控制信号编码为单一复合画布图像，使模型能够直接解释以进行集成视觉空间推理。通过策划多任务数据集和提出多任务画布训练策略，在统一学习范式中优化扩散模型以联合理解和整合异构控制。

Result: 在具有挑战性的基准测试中，包括多人组合、姿态控制组合、布局约束生成和多控制生成，Canvas-to-Image在身份保持和控制遵循方面显著优于最先进的方法。

Conclusion: Canvas-to-Image通过统一框架实现了对多种控制模态的联合推理，而不依赖特定任务的启发式方法，在推理过程中能够很好地泛化到多控制场景。
