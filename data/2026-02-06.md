...
TL;DR: 提出了一种通过增强视觉条件化来改善Vision-Language-Action模型性能的训练框架，无需修改架构或收集额外数据


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在扩展到动作空间时会出现视觉-动作错位问题，导致动作预测对当前视觉状态的依赖较弱，产生不可靠的动作输出

Method: 提出两步训练框架：1) 通过偏好优化在轨迹跟随任务上对齐动作预测与视觉输入；2) 通过潜在空间蒸馏在监督微调中将增强的对齐转移到指令跟随任务

Result: 方法在离散OpenVLA和连续OpenVLA-OFT设置中均改善了视觉条件化和任务性能，获得了稳定的性能提升

Conclusion: 通过显式加强视觉条件化可以有效解决VLA模型中的视觉-动作错位问题，提高模型的可靠性和任务表现
...
<details>
  <summary>Details</summary>
Motivation: 基于图像的饮食评估对于准确便捷地监测个人健康、预防和管理慢性疾病及肥胖至关重要，但面临从2D图像估计食物三维尺寸的核心挑战。

Method: 探索了多种策略：1) 使用深度图等辅助输入；2) 多视角图像输入；3) 基于模板匹配的模型方法；4) 深度学习技术，包括单目图像或图像与辅助输入的组合。

Result: 论文综述了不同策略在准确估计食物分量方面的应用和效果，但未提供具体的实验结果数据。

Conclusion: 基于图像的饮食评估需要克服从2D图像估计3D食物尺寸的挑战，多种技术策略已被开发并应用于提高分量估计的准确性。
...
TL;DR: 提出了一种名为VCR的方法来识别大型多模态模型中的视觉概念，用于审计医疗任务中模型的性能差异和视觉特征依赖


<details>
  <summary>Details</summary>
Motivation: 在医疗等安全关键领域确保机器学习模型的可靠性需要审计方法，以发现模型缺陷。大型多模态模型在医疗任务中表现出不同人口亚组间的性能差异，需要理解其视觉概念依赖

Method: 提出Visual Concept Ranking (VCR)方法，用于识别大型多模态模型中的重要视觉概念。该方法生成关于不同视觉特征依赖的假设，并通过手动干预进行验证。主要应用于皮肤恶性病变分类任务，并补充了胸部X光片和自然图像的实验

Result: 发现大型多模态模型在医疗任务中表现出不同人口亚组间的意外性能差异。VCR方法能够识别模型依赖的视觉概念，并通过手动干预验证了这些假设

Conclusion: VCR方法为审计大型多模态模型在医疗任务中的行为提供了有效工具，有助于理解模型的视觉概念依赖和性能差异，提高模型在安全关键领域的可靠性
...
TL;DR: ARGaze：一种基于自回归解码的在线第一人称视线估计方法，将视线预测重新定义为序列预测任务，利用当前视觉特征和最近视线目标的固定长度上下文窗口，实现了因果性和有限资源流式推理。


<details>
  <summary>Details</summary>
Motivation: 在线第一人称视线估计对增强现实和辅助技术至关重要，但缺乏明确的头部或眼睛信号，需要从稀疏的间接线索（如手-物交互和显著场景内容）推断当前视觉注意力。作者观察到在目标导向活动中，视线具有强时间连续性，最近的视线位置为预测下一个视线提供了有力先验。

Method: ARGaze方法将视线估计重新定义为序列预测任务：在每个时间步，Transformer解码器通过条件化（i）当前视觉特征和（ii）最近视线目标估计的固定长度视线上下文窗口来预测当前视线。这种设计确保了因果性，并支持有限资源的流式推理。

Result: 在多个第一人称基准测试的在线评估中实现了最先进的性能，广泛的消融实验验证了自回归建模和有限视线历史对鲁棒预测的关键作用。
...
TL;DR: 该论文系统评估了视觉手部追踪模型在戴手套情况下的性能，发现现有裸手模型因外观差异而性能下降，提出了AirGlove方法通过有限数据泛化到新手套设计。


<details>
  <summary>Details</summary>
Motivation: 传感手套在遥操作和机器人策略学习中很重要，但基于传感器的追踪受信号质量和校准影响，而基于视觉的方法在裸手上表现良好但在手套手上性能未充分探索，需要解决手套外观差异带来的性能下降问题。

Method: 提出AirGlove方法，利用现有手套数据学习手套表示，通过有限数据将学习到的表示泛化到新的手套设计，在零样本和微调设置下评估视觉手部追踪模型在手套手上的性能。

Result: 实验显示现有裸手模型在传感手套上性能显著下降，而AirGlove能有效将手部姿态模型泛化到新的手套设计，相比其他方案获得显著性能提升。

Conclusion: AirGlove通过利用现有手套数据学习可泛化的手套表示，解决了手套外观差异问题，为传感手套的视觉追踪提供了有效解决方案。
...
TL;DR: 提出基于扩散模型的图像修复方法，用于逆转水下摄影中的对比度降低、空间模糊和波长相关色彩失真等问题，在小型高质量数据集上训练后能生成高感知一致性的512x768图像。


<details>
  <summary>Details</summary>
Motivation: 水下摄影面临对比度降低、空间模糊和波长相关色彩失真等固有挑战，这些问题会掩盖海洋生物的活力，特别是环保摄影师通常需要繁重的后期处理流程来纠正这些失真。

Method: 开发了一个图像到图像的管道，通过引入合成退化管道，并使用基于扩散的生成方法来学习逆转其效果。在Keith Ellenbogen的小型高质量环保摄影数据集上进行训练和评估。

Result: 所提出的方法在合成512x768图像时实现了高感知一致性和强泛化能力，使用约1100万参数的模型，在约2500张图像上从头开始训练。

Conclusion: 基于扩散模型的方法能有效逆转水下图像退化，为水下摄影提供了一种高效的图像修复解决方案，减少了繁重的后期处理需求。
...
TL;DR: PoseGaussian是一个基于姿态引导的高斯泼溅框架，用于高质量人体新视角合成，通过姿态信息优化深度估计和时间一致性，实现实时渲染和SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 解决动态人体场景中的挑战，如关节运动和严重自遮挡，同时保持高斯泼溅框架的效率优势。现有方法仅将姿态用作条件或变形参考，未能充分利用姿态信息。

Method: 提出姿态引导的高斯泼溅框架：1) 将人体姿态作为结构先验，与颜色编码器融合以细化深度估计；2) 使用专用姿态编码器处理姿态作为时间线索，增强帧间时间一致性；3) 构建完全可微分的端到端训练流程。

Result: 在ZJU-MoCap、THuman2.0和内部数据集上验证，达到SOTA性能：PSNR 30.86、SSIM 0.979、LPIPS 0.028。框架保持100 FPS的实时渲染效率。

Conclusion: PoseGaussian通过将姿态信号嵌入几何和时间阶段，显著提升了动态人体新视角合成的鲁棒性和泛化能力，同时保持了高斯泼溅的高效性，为实时高质量人体渲染提供了有效解决方案。
...
TL;DR: 该论文提出了一种新方法，将视频生成模型重新用作奖励模型，通过对比学习训练生成模型作为基于能量的模型来评估视频质量，显著减少了人工标注需求。


<details>
  <summary>Details</summary>
Motivation: 当前基于视觉语言模型的视频奖励建模方法难以捕捉细微的时间动态，需要大量人工标注数据，效果有限。

Method: 提出Generative-Transformer-based Self-Supervised Video Judge模型，将最先进的视频生成模型转化为时间感知的奖励模型。通过将生成模型重新表述为基于能量的模型，使用对比目标训练，并通过潜在空间扰动（时间切片、特征交换、帧重排）创建具有挑战性的合成负样本来防止模型利用表面差异。

Result: 在GenAI-Bench和MonteBench上实现了最先进的性能，仅使用30K人工标注，比现有基于VLM的方法减少了6到65倍的标注需求。

Conclusion: 生成模型可以作为强大的时间感知奖励模型，通过基于能量的模型框架和精心设计的对比学习策略，能够准确评估视频质量，显著降低人工标注成本。
...
<details>
  <summary>Details</summary>
Motivation: 现有神经编解码器在超低码率下性能显著下降，而基于语义先验的生成压缩方法存在语义保真度与感知真实度之间的权衡问题：显式表示方法保留内容结构但缺乏细粒度纹理，隐式方法能合成视觉细节但可能导致语义漂移。

Method: 提出统一框架，以无需训练的方式整合显式和隐式表示：1）在扩散模型中基于显式高层语义进行条件生成；2）使用反向信道编码隐式传递细粒度细节；3）引入可插拔编码器，通过调制隐式信息灵活控制失真-感知权衡。

Result: 在Kodak、DIV2K和CLIC2020数据集上的实验表明，该框架实现了最先进的率-感知性能，在DISTS BD-Rate指标上分别超过DiffC方法29.92%、19.33%和20.89%。

Conclusion: 提出的统一框架成功弥合了显式和隐式表示之间的鸿沟，在超低码率条件下实现了语义保真度和感知真实度的平衡，为生成压缩提供了有效的解决方案。
...
TL;DR: E.M.Ground：一种新颖的视频大语言模型，通过事件令牌聚合、Savitzky-Golay平滑和多粒度帧特征聚合，显著提升时序视频定位性能。
...
Method: 提出E.M.Ground模型，包含三个关键创新：1) 引入特殊<event>令牌，聚合查询事件所有帧的信息以保持语义连续性；2) 使用Savitzky-Golay平滑减少跨时间戳的令牌-帧相似度噪声；3) 采用多粒度帧特征聚合增强匹配可靠性和时序理解，补偿压缩导致的信息损失。

Result: 在多个基准数据集上的广泛实验表明，E.M.Ground始终以显著优势超越最先进的视频大语言模型。

Conclusion: E.M.Ground通过关注整体和连贯的事件感知，有效解决了时序视频定位中的语义连续性问题，为视频理解任务提供了更准确的定位能力。
...
TL;DR: MPA通过数据增强和策略优化，从数据和策略两个角度渐进式地将少样本分割能力适应到目标域，显著提升了跨域少样本分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有跨域少样本分割方法由于目标域样本数量有限、多样性不足，以及源域模型在目标域初始少样本能力较弱且存在较大域差距，导致目标样本利用效率低，适应效果受限。

Method: 提出多视图渐进适应方法：1) 数据角度：采用混合渐进增强，通过累积强增强生成更多样、更复杂的视图，创建逐渐具有挑战性的学习场景；2) 策略角度：设计双链多视图预测，通过顺序和并行学习路径在广泛监督下充分利用这些渐进复杂视图。

Result: 实验表明MPA能有效将少样本能力适应到目标域，大幅超越现有最优方法（+7.0%）。

Conclusion: 通过联合强制不同复杂视图间的预测一致性，MPA实现了对目标域既鲁棒又准确的适应，解决了跨域少样本分割中的关键挑战。
...
TL;DR: 本文提出了一种名为条件点稀疏化（CPS）的训练免费方法，用于解决基于SAM的少样本分割在跨域场景中的性能下降问题，通过自适应稀疏化密集匹配点来提升跨域分割精度。


<details>
  <summary>Details</summary>
Motivation: 现有基于SAM的训练免费少样本分割方法依赖密集点匹配作为提示，但在跨域少样本分割（CD-FSS）中性能显著下降，主要原因是大的域偏移破坏了SAM学习的点-图像交互，且点密度在这种条件下起着关键作用。

Method: 提出条件点稀疏化（CPS）方法，这是一种训练免费的方法，基于参考样本自适应地引导SAM对跨域图像的交互。利用参考图像的真实掩码作为可靠指导，自适应地稀疏化密集匹配点，从而实现更准确的分割结果。

Result: 大量实验表明，CPS在多种CD-FSS数据集上优于现有的基于SAM的训练免费方法。

Conclusion: CPS通过自适应稀疏化密集匹配点，有效解决了基于SAM的少样本分割在跨域场景中的性能问题，为跨域少样本分割提供了有效的训练免费解决方案。
...
TL;DR: 结合局部邻域感知补丁特征与归一化流模型，通过适配器模块弥合预训练特征提取器与工业图像之间的差距，提升压铸缺陷检测的准确率


<details>
  <summary>Details</summary>
Motivation: 压铸在工业中至关重要，但表面缺陷是影响质量控制的主要问题。计算机视觉技术可用于自动化缺陷检测，但现有方法在工业应用中的效率和准确性有待提升
...
Conclusion: 该方法展示了计算机视觉和深度学习技术在提升压铸行业检测能力方面的潜力，为工业质量控制提供了有效的无监督异常检测方案
...
TL;DR: 提出一种主动标签清洗方法，通过主动学习选择最有价值的噪声样本进行专家重新标注，有效降低电子致密物检测中的标注成本


<details>
  <summary>Details</summary>
Motivation: 肾小球疾病中电子致密物自动检测面临高质量标注数据稀缺的问题，众包标注虽然降低了成本但引入了标签噪声，需要高效的去噪方法

Method: 使用主动学习方法选择最有价值的噪声样本进行专家重新标注，构建高精度清洗模型；标签选择模块利用众包标签与模型预测之间的差异进行样本选择和实例级噪声分级
...
TL;DR: 本文提出一种新的FSCIL方法：冻结特征提取器，微调原型。通过双校准机制（类特定偏移和任务感知偏移）优化决策区域，在保持参数效率的同时提升性能。


<details>
  <summary>Details</summary>
Motivation: 传统FSCIL方法使用冻结预训练特征提取器生成静态类原型，但存在表示偏差问题。现有的基于提示的微调方法在数据极度稀缺情况下，模型吸收新信息和增强全局判别能力的能力有限。本文认为FSCIL的主要挑战不是特征获取，而是在静态高质量特征空间中优化决策区域。

Method: 提出原型微调框架，将静态质心演变为动态可学习组件。采用双校准方法：类特定偏移和任务感知偏移，协同工作以提升对持续增量类的判别能力。保持特征提取器冻结，仅微调原型参数。

Result: 在多个基准测试中取得了优越性能，同时仅需要极少的可学习参数。

Conclusion: 通过冻结特征提取器并微调原型，在静态高质量特征空间中优化决策区域，是解决FSCIL问题的有效方法。双校准机制能够显著提升模型对增量类的判别能力，同时保持参数效率。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在通用多模态检索中表现出巨大潜力，但处理大量视觉令牌时会产生巨大的计算成本，阻碍了实际应用

Method: 采用两个协同支柱：(1) 高效的MLLM架构，包含视觉令牌压缩以减少推理延迟和内存占用；(2) 多阶段渐进训练策略，包括继续预训练、大规模对比预训练与困难负样本挖掘，以及基于MLLM-as-a-Judge的任务感知微调

Result: 模型在保持高效推理的同时，性能大幅超越现有方法，实现了效率与性能的双重突破

Conclusion: Magic-MM-Embedding系列模型在通用多模态嵌入任务中实现了高效与高性能的平衡，为实际应用提供了可行的解决方案
...
Method: 提出Fast-SAM3D训练免费框架，包含三个异构感知机制：1) 模态感知步长缓存，解耦结构演化与布局更新；2) 联合时空令牌雕刻，聚焦高熵区域细化；3) 频谱感知令牌聚合，自适应解码分辨率

Result: 实验表明Fast-SAM3D实现高达2.67倍的端到端加速，且保真度损失可忽略，为高效单视图3D生成建立了新的帕累托前沿

Conclusion: 通过系统分析SAM3D推理动态并针对性解决其多层次异构性，Fast-SAM3D成功实现了显著加速，推动了高效3D重建的实际应用
...
TL;DR: FlashBlock是一种用于块扩散生成模型的优化技术，通过重用块外注意力输出来减少计算开销，提高生成长序列内容的效率。


<details>
  <summary>Details</summary>
Motivation: 当前块扩散方法在生成长序列内容时，即使使用KV缓存和块级因果推理，仍面临因重复计算注意力而带来的显著开销问题。作者发现块扩散中存在跨步冗余特性，即块外注意力输出在扩散步骤间保持稳定，这为优化提供了机会。

Method: 提出FlashBlock缓存机制，重用稳定的块外注意力输出，减少注意力计算和KV缓存访问。该方法与稀疏注意力正交，可作为补充的残差重用策略，在激进稀疏化下显著提升模型精度。

Result: 在扩散语言模型和视频生成任务上的实验表明，FlashBlock实现了最高1.44倍的token吞吐量提升和1.6倍的注意力时间减少，同时对生成质量影响可忽略不计。

Conclusion: FlashBlock通过利用块扩散中的跨步冗余特性，有效提升了生成长序列内容的效率，为扩散模型的推理优化提供了新的思路。
...
TL;DR: PAIR框架将概念擦除从简单移除重构为一致性保持的语义重对齐，通过不安全-安全配对数据实现细粒度擦除，在移除有害内容的同时保持结构和语义一致性。
...
Method: 提出PAIR框架：1) 从不安全输入生成结构语义保真的安全对应物，形成不安全-安全配对多模态数据；2) 配对语义重对齐：使用配对数据将目标概念显式映射到语义对齐的安全锚点；3) Fisher加权DoRA初始化：使用配对数据初始化参数高效的低秩适应矩阵，鼓励生成安全替代方案同时选择性抑制不安全概念。

Result: 大量实验表明，该方法显著优于现有最先进基线，实现了有效的概念擦除，同时保持了结构完整性、语义连贯性和生成质量。

Conclusion: PAIR框架通过将概念擦除重构为一致性保持的语义重对齐，使用不安全-安全配对数据实现了细粒度概念擦除，在移除目标概念的同时保持了整体语义一致性。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于原型的表示学习方法存在两个根本缺陷：静态同质性假设（所有类别使用固定的表示资源）和学习-推理脱节（推理时丢弃丰富的原型质量知识），这些缺陷限制了模型的能力和性能。
...
Conclusion: APEX通过解决原型方法中的两个根本缺陷，为OOD检测提供了一个更有效的自适应原型框架，显著提升了检测性能。
...
TL;DR: HIVE框架通过层次化视觉线索注入实现多模态潜在推理，避免依赖显式语言思维链，在潜在空间中进行迭代推理优化


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型采用"快速思考"范式，依赖端到端生成或显式语言思维链，存在效率低、冗长和幻觉问题。需要更稳健的推理机制在潜在空间中无缝整合多模态信号。

Method: 提出HIVE框架：1)递归扩展Transformer块创建内部循环进行迭代推理优化；2)通过层次化视觉线索注入将全局场景上下文到细粒度区域细节直接融入潜在表示；3)在潜在对齐空间中执行多步推理。

Result: 广泛评估表明：1)结合视觉知识时测试时间扩展有效；2)整合层次化信息显著增强模型对复杂场景的理解能力。

Conclusion: HIVE框架通过层次化视觉线索注入实现多模态潜在推理，为多模态大语言模型提供了更稳健、高效的"慢思考"推理范式，避免依赖显式语言思维链的局限性。
...
TL;DR: 论文揭示了OOD检测中的"简单性悖论"：现有SOTA模型能识别语义细微的OOD样本，但对结构差异明显但语义简单的样本或高频传感器噪声存在严重"几何盲区"，并提出D-KNN框架解决这一问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于特征的后处理方法在OOD检测中取得了显著进展，但研究发现这些模型存在一个反直觉的"简单性悖论"：模型对语义细微的OOD样本敏感，但对结构差异明显但语义简单的样本或高频传感器噪声存在严重"几何盲区"，这源于深度特征空间中的"语义霸权"现象。

Method: 提出D-KNN框架，这是一种无需训练、即插即用的几何解耦方法。通过正交分解显式分离语义成分和结构残差，并引入双空间校准机制重新激活模型对弱残差信号的敏感性。

Result: D-KNN有效打破了语义霸权，在CIFAR和ImageNet基准测试中建立了新的SOTA性能。在解决简单性悖论方面，将FPR95从31.3%降至2.3%；在处理高斯噪声等传感器故障时，将检测性能(AUROC)从基线79.7%提升至94.9%。

Conclusion: 通过神经崩溃视角揭示了语义霸权现象的数学本质，提出并验证了D-KNN框架能有效解决OOD检测中的简单性悖论，显著提升模型对结构分布偏移的检测能力。
...
TL;DR: CityGenAgent：一个基于自然语言的层次化程序化生成高质量3D城市的框架，通过分解为区块程序和建筑程序，结合监督微调和强化学习，实现语义对齐、视觉质量和可控性


<details>
  <summary>Details</summary>
Motivation: 当前3D城市自动生成方法在高质量资产创建、可控性和操作方面存在不足，需要开发能够支持自然语言交互的高保真城市生成框架

Method: 将城市生成分解为区块程序和建筑程序两个可解释组件，采用两阶段学习策略：1）监督微调训练BlockGen和BuildingGen生成符合模式约束的有效程序；2）强化学习设计空间对齐奖励和视觉一致性奖励

Result: 综合评估显示，相比现有方法，CityGenAgent在语义对齐、视觉质量和可控性方面表现优越，为可扩展的3D城市生成建立了坚实基础

Conclusion: CityGenAgent通过自然语言驱动的层次化程序化生成方法，成功解决了3D城市生成中的高保真资产创建、可控性和操作问题，支持自然语言编辑和操作
...
TL;DR: SAIL框架通过自增强迭代学习，仅需少量人工标注偏好对，让扩散模型作为自己的老师进行自我改进，无需外部奖励模型，大幅减少人工标注需求。
...
Method: 提出SAIL（自增强迭代学习）框架：从少量人工标注偏好对开始，采用闭环方式运行。模型逐步生成多样样本，基于其演化理解进行自我标注偏好，使用自增强数据集进行自我精炼。引入排名偏好混合策略平衡探索与初始人类先验的遵循，防止灾难性遗忘。

Result: 在多个基准测试中，SAIL始终优于最先进方法，仅需现有方法6%的偏好数据。实验表明扩散模型具有显著的自我改进能力，当被恰当利用时，可以有效替代大规模人工标注和外部奖励模型。

Conclusion: 扩散模型拥有强大的自我改进能力，通过SAIL框架可以仅用最少人工反馈实现有效对齐，无需外部奖励模型或大规模标注数据集，为扩散模型对齐提供了高效解决方案。
...
Method: 引入VRIQ基准，包含抽象谜题风格和自然图像推理两类任务。通过诊断探针分析失败原因，针对特定感知类别（形状、计数、位置、3D/深度等）设计细粒度诊断问题。

Result: 在抽象谜题上准确率约28%（接近随机），自然图像任务45%。工具增强推理仅带来有限改进。失败原因分析：56%仅因感知问题，43%感知和推理双重问题，仅1%纯推理问题。

Conclusion: 当前VLMs即使是增强了视觉推理工具，仍然是不可靠的抽象推理器，主要受限于感知能力。该基准和分析为改进多模态系统的视觉推理提供了原则性基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有文档解析领域存在两个主要问题：一是模型碎片化，用户需要面对数十个专业模型的选择困难；二是传统两阶段方法依赖轴对齐边界框，无法有效处理扭曲或拍摄的文档。

Method: 采用两阶段方法：第一阶段联合进行文档类型分类（数字文档vs拍摄文档）和布局分析；第二阶段采用混合解析策略，对拍摄文档进行整体页面级解析，对数字文档进行基于布局锚点的元素级并行解析。

Result: 在DocPTBench、OmniDocBench和自建RealDoc-160基准测试中表现优异：在OmniDocBench上整体提升14.78分，拍摄文档错误减少91%，同时通过并行处理保持高效推理。

Conclusion: Dolphin-v2通过创新的混合解析策略和增强功能，显著提升了文档解析能力，特别是对拍摄文档的处理效果，为文档解析领域提供了更统一、更强大的解决方案。
...
TL;DR: 提出Parallel Swin Transformer-Enhanced Med2Transformer架构，用于从MRI生成合成CT图像，以支持MRI-only放疗计划，解决传统方法需要MRI-CT配准的复杂性和不确定性。


<details>
  <summary>Details</summary>
Motivation: MRI在放疗中缺乏电子密度信息，无法直接用于剂量计算，现有工作流程依赖MRI和CT联合采集，增加了配准不确定性和程序复杂性。合成CT生成可以支持MRI-only计划，但由于MRI-CT关系的非线性和解剖变异性而具有挑战性。

Method: 提出Parallel Swin Transformer-Enhanced Med2Transformer，一种3D架构，集成卷积编码与双Swin Transformer分支，以建模局部解剖细节和长距离上下文依赖。采用多尺度移位窗口注意力与分层特征聚合来提高解剖保真度。

Result: 在公共和临床数据集上的实验表明，相比基线方法，该方法具有更高的图像相似度和改进的几何精度。剂量学评估显示临床可接受的性能，平均靶区剂量误差为1.69%。

Conclusion: 提出的Parallel Swin Transformer-Enhanced Med2Transformer架构能够有效从MRI生成合成CT图像，支持MRI-only放疗计划，减少对CT扫描的依赖，简化放疗工作流程。
...
<details>
  <summary>Details</summary>
Motivation: 现有线性梯度匹配方法在数据集蒸馏中存在计算和内存开销大的问题，需要加载大量真实图像并对合成图像进行多轮可微分增强，效率低下。

Method: 提出统计流匹配框架，通过对齐原始数据中目标类别中心到非目标类别中心的恒定统计流来优化合成图像；同时提出分类器继承策略，重用原始数据集上训练的分类器进行推理。

Result: 方法在性能上达到或优于现有最先进方法，同时GPU内存使用降低10倍，运行时间缩短4倍；分类器继承策略仅需极轻量级线性投影器和少量存储，实现显著性能提升。
...
TL;DR: 提出MAD框架，通过VAE学习解耦潜在空间，在保持生物合理性的前提下实现可控的细胞核特征编辑，解决传统方法因忽略特征相关性导致的非真实伪影问题。


<details>
  <summary>Details</summary>
Motivation: 病理组学特征虽然丰富，但许多特征（如"二阶矩"）难以解释，限制了其临床实际应用。现有条件扩散模型通常假设特征独立，而病理组学特征本质相关，导致编辑单个特征时可能产生非真实的伪影。

Method: 提出Manifold-Aware Diffusion (MAD)框架：1) 使用变分自编码器(VAE)学习解耦潜在空间；2) 在潜在空间中正则化特征轨迹，确保编辑目标特征时自动调整相关属性以保持在真实细胞分布内；3) 用优化后的特征指导条件扩散模型合成高保真图像。

Result: 实验表明该方法能够在编辑病理组学特征时导航特征流形，在条件特征编辑方面优于基线方法，同时保持结构一致性。

Conclusion: MAD框架通过流形感知的特征编辑，实现了可控且生物合理的细胞核编辑，解决了传统方法因忽略特征相关性导致的非真实伪影问题，提高了病理组学特征的可解释性和实用性。
...
TL;DR: 该研究提出了TSBOW数据集，这是一个针对各种极端天气条件下被遮挡车辆检测的交通监控基准数据集，包含超过32小时的真实世界交通数据和数百万标注帧。


<details>
  <summary>Details</summary>
Motivation: 全球变暖加剧了极端天气事件的频率和严重性，这降低了CCTV信号和视频质量，同时扰乱了交通流，增加了交通事故率。现有数据集通常只包含轻度雾霾、雨和雪，无法捕捉极端天气条件。

Method: 研究引入了TSBOW数据集，包含超过32小时来自人口密集城市地区的真实世界交通数据，包括超过48,000个手动标注和320万个半标注帧，涵盖八个交通参与者类别。

Result: 建立了TSBOW的对象检测基准，突出了遮挡和恶劣天气带来的挑战。数据集包含多样化的道路类型、尺度和视角，为智能交通系统发展提供了关键资源。
...
TL;DR: 提出了一种无外部数据的OOD检测方法，通过几何引导的虚拟异常合成和双粒度语义损失，在长尾分布下实现优于现有方法的检测性能。


<details>
  <summary>Details</summary>
Motivation: 长尾分布下的OOD检测具有挑战性，因为尾部类别样本稀缺导致特征空间决策边界模糊。现有SOTA方法依赖大规模外部数据集进行异常暴露，但这在实际部署中因数据获取成本和隐私问题而不可行。

Method: 提出了几何引导的虚拟异常合成(GOS)策略，使用von Mises-Fisher分布在超球面上建模统计特性，在特征空间低似然环形区域进行方向采样生成虚拟异常。同时引入双粒度语义损失(DGS)，利用对比学习最大化ID特征与合成边界异常之间的区分度。

Result: 在CIFAR-LT等基准测试上的大量实验表明，该方法优于使用外部真实图像的SOTA方法。

Conclusion: 提出了一种完全消除对外部数据集依赖的无数据框架，在长尾分布下实现了卓越的OOD检测性能，解决了实际部署中的数据获取和隐私问题。
...
TL;DR: 提出Disco框架，通过邻接感知的协作染色方法解决密集重叠细胞实例分割问题，基于细胞邻接图的色数特性分析，发现真实细胞图多为非二分图且含大量奇数环，传统2染色理论不足，而高色数模型又存在冗余和优化困难。


<details>
  <summary>Details</summary>
Motivation: 现有基于轮廓检测和距离映射的方法在处理复杂密集细胞区域时面临挑战，基于图染色的方法提供了新范式，但其在具有密集重叠和复杂拓扑的真实场景中的有效性尚未得到验证。

Method: 提出Disco框架，基于"分而治之"原则，结合数据驱动的拓扑标记策略和约束深度学习系统。包含"显式标记"策略（递归分解细胞图并隔离"冲突集"）和"隐式消歧"机制（通过强制不同实例间的特征差异性来解析冲突区域的模糊性）。

Result: 发布了大规模数据集GBC-FS 2025，包含高度复杂和密集的亚细胞核排列；首次系统分析了四个不同数据集中细胞邻接图的色数特性，发现大多数真实细胞图是非二分图，具有高比例的奇数长度环（主要是三角形）。

Conclusion: Disco框架通过邻接感知的协作染色方法有效解决了密集重叠细胞实例分割问题，为复杂组织中的细胞分割提供了新的解决方案，验证了图染色范式在真实复杂场景中的有效性。
...
TL;DR: NeVStereo是一个结合NeRF渲染和立体视觉的框架，能够从多视角RGB图像中联合估计相机位姿、深度、新视角合成和表面重建，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：前馈系统专注于端到端匹配和几何预测但不输出新视角合成；基于神经渲染的方法能提供高质量新视角合成和详细几何，但通常假设固定相机位姿且对位姿误差敏感。因此需要单一框架能够从随意拍摄的视图中同时提供准确位姿、可靠深度、高质量渲染和精确3D表面。

Method: NeVStereo结合了基于NeRF的新视角合成生成立体友好渲染、置信度引导的多视角深度估计、NeRF耦合的束调整用于位姿优化，以及迭代细化阶段同时更新深度和辐射场以提高几何一致性。该设计缓解了NeRF常见的表面堆叠、伪影和位姿-深度耦合问题。

Result: 在室内、室外、桌面和航空基准测试中，NeVStereo实现了持续强大的零样本性能：深度误差降低达36%，位姿精度提高10.4%，新视角合成保真度提高4.5%，并达到最先进的网格质量（F1 91.93%，Chamfer距离4.35毫米）。
...
TL;DR: 提出基于潜在扩散模型的LD-SLRO方法，用于高反射率物体三维重建中的条纹图像恢复


<details>
  <summary>Details</summary>
Motivation: 高反射率、低粗糙度物体的三维重建在条纹投影轮廓术中面临挑战，镜面反射和间接照明导致条纹图案严重失真或丢失
...
Conclusion: LD-SLRO方法有效解决了高反射物体三维重建中的条纹失真问题，在条纹恢复质量和重建精度上优于现有方法
...
TL;DR: 提出Stable Velocity框架，通过方差分析改进流匹配：训练时使用StableVM降低方差，VA-REPA增强监督；推理时利用低方差区域特性实现StableVS加速采样，在多个大模型上实现2倍以上加速且不损失质量。


<details>
  <summary>Details</summary>
Motivation: 传统流匹配方法依赖单样本条件速度，导致训练目标方差高，优化不稳定且收敛慢。本文通过分析方差特性，发现先验附近的高方差区域和数据分布附近的低方差区域，基于此提出改进方案。

Method: 1) Stable Velocity Matching (StableVM)：无偏方差减少目标；2) Variance-Aware Representation Alignment (VA-REPA)：在低方差区域自适应增强辅助监督；3) Stable Velocity Sampling (StableVS)：利用低方差区域动力学可闭式简化的特性，实现无需微调的采样加速。

Result: 在ImageNet 256×256以及SD3.5、Flux、Qwen-Image、Wan2.2等大型预训练文本到图像/视频模型上实验，训练效率显著提升，低方差区域内采样速度提高2倍以上，且不降低样本质量。

Conclusion: 通过方差分析提出的Stable Velocity框架，系统性地解决了流匹配方法的高方差问题，在训练稳定性和采样效率方面均取得显著改进，为大规模生成模型提供了有效的优化方案。
...
Method: 开发参数化3D缺陷网格模型，创建包含合成缺陷的数字孪生对象，通过基于物理的蒙特卡洛模拟生成合成检测数据

Result: 能够生成可变且任意规模的合成数据集，包含足够数量的稀有缺陷样本，同时生成像素级完美标注

Conclusion: 该方法为工业无损检测的机器学习训练提供了可控、可扩展的合成数据生成方案，可应用于多种检测方法
...
<details>
  <summary>Details</summary>
Motivation: 持续学习中灾难性遗忘是一个关键挑战。作者发现Vision Transformers中的注意力漂移是导致灾难性遗忘的主要原因，即学习新任务后对先前学习视觉概念的注意力发生显著偏移。受人类视觉系统选择性注意力的神经科学启发，需要设计方法来约束这种注意力漂移。

Method: 提出注意力保持框架，通过修改反向传播梯度来约束注意力漂移：1) 使用层间展开机制提取先前任务的注意力图，生成实例自适应的二值掩码；2) 学习新任务时，应用这些掩码将先前注意力区域相关的梯度置零，防止已学习视觉概念被破坏。为兼容现代优化器，梯度掩码过程通过按比例缩放参数更新来保持相对幅度。

Result: 实验和可视化结果表明该方法能有效缓解灾难性遗忘并保持视觉概念。在多种持续学习场景中实现了最先进的性能，并展现出强大的泛化能力。

Conclusion: 注意力漂移是Vision Transformers中灾难性遗忘的主要来源，通过梯度掩码机制约束注意力漂移能有效缓解遗忘问题，该方法在持续学习中表现出优越性能。
...
Result: 在物体目标导航任务中，在4个数据集上平均成功率比无训练基线提升7%，零样本设置提升5%。在HM3D_v0.1和HM3D_OVON数据集上分别提升8%和6%。在MP3D和HM3D_OVON数据集上不仅超越所有无训练方法，还超过了监督微调方法。

Conclusion: Memory-Execute-Review框架有效解决了视觉语言导航中成功率和泛化能力的平衡问题，在多个数据集上实现了全面领先，为具身智能提供了重要技术支撑。
...
Result: 在SOMA-1M上进行监督训练显著提升了图像匹配、图像融合、SAR辅助云去除和跨模态翻译等四项视觉任务的性能，其中多模态遥感图像匹配达到当前SOTA水平
...
TL;DR: 该报告提供了PFSeq鱼眼序列数据集，并评估了鱼眼图像在自标定场景下的最佳特征检测与描述算法，但属于2014年的未发表草稿。


<details>
  <summary>Details</summary>
Motivation: 解决鱼眼图像自标定中的"鸡与蛋"问题：需要准确投影模型来优化特征检测，同时又需要良好特征来校准投影模型。研究旨在找到车载鱼眼相机（朝向天顶）在自标定场景下的最佳特征检测与描述方法。

Method: 提供PFSeq（Photorealistic Fisheye Sequence）数据集，并进行全面的实验评估。报告比较了多种标准特征检测与描述算法在鱼眼图像上的性能，但未包含专门为全向图像设计的算法。

Result: 报告提供了详细的实验结果，评估了不同特征检测与描述算法在鱼眼图像自标定任务中的表现。数据集已公开可用，但具体算法性能结果未在摘要中详细说明。

Conclusion: 该工作为鱼眼图像处理提供了有价值的数据集和基准测试，但由于是2014年的未发表草稿，且未包含专门的全向图像算法，其结论需要谨慎参考。报告强调了鱼眼视觉里程计和立体视觉在城市场景中的应用潜力。
...
TL;DR: VGGT-Motion是一个无需标定的单目SLAM系统，通过运动感知子图构建和锚点驱动的Sim(3)配准，解决了长序列中的尺度漂移问题，实现了千米级轨迹的高效全局一致性。
...
Method: 1. 运动感知子图构建机制：使用光流指导自适应分区，修剪静态冗余，封装转弯以实现稳定的局部几何；2. 锚点驱动的直接Sim(3)配准策略：利用上下文平衡的锚点实现免搜索的像素级密集对齐和高效闭环，无需昂贵的特征匹配；3. 轻量级子图级位姿图优化：以线性复杂度强制全局一致性，支持可扩展的长距离操作。

Result: 实验表明，VGGT-Motion显著提高了轨迹精度和效率，在零样本、长距离无标定单目SLAM中实现了最先进的性能。

Conclusion: VGGT-Motion通过创新的运动感知子图构建和锚点驱动配准，有效解决了长序列SLAM中的尺度漂移问题，为无需标定的单目SLAM系统提供了高效且鲁棒的全局一致性解决方案。
...
TL;DR: SSG是一种无需训练、推理时引导的方法，通过强调目标高频信号（语义残差）来纠正VAR模型在推理时的层次结构漂移，保持粗到细的生成特性。


<details>
  <summary>Details</summary>
Motivation: 视觉自回归模型在推理时由于容量限制和误差累积，其粗到细的层次结构会发生漂移，导致训练和推理不一致的问题。

Method: 提出SSG（Scaled Spatial Guidance），通过强调目标高频信号（语义残差）来引导生成过程；使用DSE（Discrete Spatial Enhancement）在频域中锐化和隔离语义残差；该方法无需训练，适用于各种基于离散视觉token的VAR模型。

Result: SSG在保真度和多样性方面带来一致提升，同时保持低延迟，揭示了粗到细图像生成中未开发的效率潜力。

Conclusion: 从信息论角度分析VAR模型的层次漂移问题，提出SSG方法有效缓解训练-推理差异，为粗到细图像生成提供了高效解决方案。
...
TL;DR: 本文系统评估了相机、LiDAR及融合方法在3D行人检测中的性能，发现融合方法在复杂场景下表现最优，但对传感器错位敏感；相机方法性能最差且易受遮挡和噪声影响。


<details>
  <summary>Details</summary>
Motivation: 3D行人检测在机器人、工业监控和安全应用中至关重要。现有研究多集中于自动驾驶场景，本文旨在评估不同传感器配置在多样化室内外场景中的检测性能和鲁棒性。

Method: 使用JRDB数据集，在多样化室内外场景中系统评估三种代表性模型：BEVDepth（相机）、PointPillars（LiDAR）和DAL（相机-LiDAR融合）。分析它们在遮挡、距离变化、传感器损坏和错位等情况下的行为。

Result: 融合方法在所有场景中表现最佳，尤其在挑战性场景下优势明显。DAL对传感器错位和某些LiDAR损坏敏感。相机方法性能最低，对遮挡、距离和噪声最为敏感。

Conclusion: 传感器融合能显著提升3D行人检测性能，但系统仍存在对传感器错位和损坏的脆弱性。需要进一步研究解决这些系统固有的弱点。
...
Method: 1. 针对运动冗余：通过掩码注意力层限制到局部邻域，避免计算不必要距离的图像区域交互权重；2. 针对梯度冗余：设计重用先前扩散步骤梯度的优化方案，跳过不必要的梯度计算。

Result: FastVMT平均实现了3.43倍的加速，同时没有降低生成视频的视觉保真度或时间一致性。

Conclusion: 通过识别和消除运动冗余与梯度冗余这两种计算冗余，FastVMT显著提升了视频运动迁移的效率，为实时视频生成提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集主要关注家用或消费产品，使用合成、干净的桌面数据集，或在受控实验室环境中捕获的对象，缺乏真实工业场景的数据。需要创建专门针对工业机器人的真实应用相关测试平台。

Method: 创建包含5种新物体类型的RGB-D数据集，具有挑战性特性，在真实工业装配环境中捕获。数据集分为经典集（4.6k图像，6k标注姿态）和扩展集（支持模型无关和序列方法），首次提供RGB-D静态上机序列。

Result: 数据集包含多样复杂度的场景，从简单到挑战性场景，包括单物体和多物体场景，以及同一物体的多个实例。在代表性最先进方法上评估显示该领域仍有改进空间。

Conclusion: IndustryShapes为工业机器人中的6D姿态估计提供了真实且应用相关的基准测试平台，填补了实验室研究与实际部署之间的差距，并展示了该领域仍需进一步改进。
...
TL;DR: PIRATR是一个端到端的3D物体检测框架，用于机器人应用中的点云数据。它扩展了PI3DETR，能够直接从受遮挡的点云数据中联合估计多类别6自由度姿态和类别特定的参数化属性，实现几何定位和任务相关属性的估计。


<details>
  <summary>Details</summary>
Motivation: 解决机器人应用中需要同时进行3D物体几何定位和估计任务相关参数化属性的问题，弥合低级几何推理和可操作世界模型之间的差距，为动态机器人环境中的可扩展、仿真训练的感知系统铺平道路。

Method: 采用端到端的3D物体检测框架，基于PI3DETR扩展，使用模块化、类别特定的头部设计，能够直接从受遮挡的点云数据中联合估计多类别6自由度姿态和类别特定的参数化属性。3D模型根据预定义的简单规则进行调整。

Result: 在自动化叉车平台上验证，专注于三个结构和功能不同的类别：起重机夹具、装载平台和托盘。完全在合成环境中训练，PIRATR能够有效泛化到真实的室外LiDAR扫描，无需额外微调即可达到0.919的检测mAP。

Conclusion: PIRATR建立了一个新的姿态感知、参数化感知范式，弥合了低级几何推理和可操作世界模型之间的差距，为可扩展的、仿真训练的感知系统在动态机器人环境中的部署铺平了道路。
...
TL;DR: ViGT是一种用于自动驾驶的视觉隐式几何变换器，能够从多视角摄像头估计连续3D占据场，无需校准，支持多种传感器配置，采用自监督训练，在多个数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 开发一个可扩展、架构简单、能泛化到不同传感器配置的自动驾驶基础几何模型，解决现有方法需要校准、依赖标注数据、难以适应多种传感器设置的问题。

Method: 采用校准无关架构，从多视角摄像头输入估计连续3D占据场（BEV表示），使用自监督训练方法，利用同步的图像-LiDAR数据对，无需人工标注，在五个大规模自动驾驶数据集上混合训练。

Result: 在点图估计任务上达到最先进的性能，在所有评估基线中获得最佳平均排名；在Occ3D-nuScenes基准测试中，性能与监督方法相当。

Conclusion: ViGT展示了作为自动驾驶基础几何模型的潜力，通过校准无关架构和自监督训练，实现了可扩展性和跨传感器配置的泛化能力，为多种几何任务提供了通用表示。
...
<details>
  <summary>Details</summary>
Motivation: 当前图像编辑技术实现了高度可控和语义感知的内容修改，给篡改定位带来前所未有的挑战。现有的AI生成伪造定位方法主要关注修复式篡改，对最新的指令驱动编辑范式无效。

Method: 构建了大规模数据集LocateEdit-Bench，包含231K编辑图像，涵盖四种前沿编辑模型和三种常见编辑类型。开发了两种多指标评估协议来评估现有定位方法。

Result: 建立了专门针对指令驱动图像编辑的基准数据集，填补了现有方法在这一新兴领域的评估空白，为未来伪造定位方法的发展奠定了基础。

Conclusion: 该工作为跟上图像编辑技术发展的步伐建立了基础，促进了未来有效伪造定位方法的发展。数据集将在论文接受后开源。
...
<details>
  <summary>Details</summary>
Motivation: 现有开放词汇语义分割方法主要依赖图像级预训练的视觉语言模型，导致空间对齐不精确，在模糊或杂乱场景中产生错误分割。大多数方法缺乏强对象先验和区域级约束，导致对象幻觉或漏检问题。
...
Conclusion: LoGoSeg通过集成对象先验、区域对齐和双流融合，有效解决了开放词汇语义分割中的空间对齐不精确和对象幻觉问题，无需外部掩码提议或额外数据集，实现了高效准确的像素级标注。
...
<details>
  <summary>Details</summary>
Motivation: 传统敏感性分析工具（条件数分析、欧几里得扰动论证、费舍尔信息界）无法解释单个图像特征如何影响位姿估计，也不能说明为什么动态或不一致的观测会不成比例地扭曲现代SLAM和结构从运动系统。需要一种能够量化单个测量贡献的理论框架。

Method: 将影响函数理论扩展到矩阵李群，推导出SE(3)上左平凡化M估计量的固有扰动算子。通过曲率算子和可观测子空间的李代数结构，构建几何可观测性指数（GOI），该指数允许沿可观测曲率主方向进行谱分解。

Result: GOI揭示了弱可观测性与放大敏感性之间的直接对应关系。在总体机制下，GOI与SE(3)上的费舍尔信息几何一致，产生了单测量版本的克拉美-罗界。该谱机制解释了经典退化情况（如纯旋转和消失视差）以及沿弱曲率方向的动态特征放大。

Conclusion: GOI提供了一个几何一致的测量影响描述，统一了条件数分析、费舍尔信息几何、影响函数理论和动态场景可检测性。由于这些量直接出现在高斯-牛顿流程中，曲率谱和GOI还产生了轻量级、无需训练的诊断信号，用于识别动态特征和检测弱可观测性配置，无需修改现有SLAM架构。
...
TL;DR: EgoPoseVR是一个用于VR中准确自我中心全身姿态估计的端到端框架，通过融合头显运动信号和RGB-D观测，解决了现有方法的时间不稳定性和实时性不足问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于头戴摄像头的自我中心姿态估计方法在应用于VR头显时面临时间不稳定、下半身估计不准确和缺乏实时性能等挑战，需要更准确、稳定的全身姿态跟踪解决方案。

Method: 提出EgoPoseVR端到端框架，通过双模态融合管道整合头显运动信号和自我中心RGB-D观测；使用时空编码器提取帧级和关节级表示，通过交叉注意力融合互补运动线索；使用运动学优化模块施加HMD信号约束。

Result: 实验结果表明EgoPoseVR优于现有最先进的自我中心姿态估计模型；用户研究显示在准确性、稳定性、体现感和未来使用意愿方面获得显著更高的主观评分。

Conclusion: EgoPoseVR能够实现稳健的全身姿态跟踪，为准确VR体现提供了实用解决方案，无需额外身体穿戴传感器或房间尺度跟踪系统。
...
TL;DR: CAViT提出了一种双注意力架构，用动态的通道注意力替代ViT中的静态MLP，通过空间自注意力后接通道自注意力的方式实现内容感知的特征交互，在提升性能的同时减少了参数量和计算量。


<details>
  <summary>Details</summary>
Motivation: 传统Vision Transformers中的通道混合机制是静态的，依赖于固定的多层感知机，缺乏对输入内容的适应性。这限制了模型根据全局图像上下文动态调整特征表示的能力。

Method: 提出CAViT架构，在每个Transformer块中依次执行空间自注意力和通道自注意力，用动态的注意力机制替代静态的MLP，实现内容感知的token混合策略。

Result: 在五个基准数据集（涵盖自然和医学领域）上验证，CAViT比标准ViT基线准确率提升高达+3.6%，同时参数量和FLOPs减少超过30%。定性注意力图显示更清晰、语义更丰富的激活模式。

Conclusion: CAViT通过统一的、内容感知的token混合策略增强了表示表达能力，在不增加深度或复杂性的情况下提升了性能，验证了注意力驱动特征交互的有效性。
...
TL;DR: 提出了一种针对非经典模型的多实例鲁棒拟合方法，通过基于模型到数据误差的估计器和元启发式优化器，能够从噪声数据中重建多个非经典模型实例。


<details>
  <summary>Details</summary>
Motivation: 现有鲁棒拟合方法主要针对经典模型（如直线、圆、平面），对非经典模型（如螺旋曲线、程序化角色模型、自由曲面）的处理方法较少，且现有方法主要关注重建单个非经典模型实例，缺乏多实例重建能力。

Method: 将多实例拟合任务公式化为优化问题，包含估计器和优化器两部分。提出基于模型到数据误差的新型估计器，无需预定义误差阈值即可处理异常值。由于该估计器对模型参数不可微，采用元启发式算法作为优化器寻找全局最优解。

Result: 实验结果表明，该方法在各种非经典模型上均表现出有效性，能够成功从噪声数据中重建多个非经典模型实例。

Conclusion: 该方法填补了非经典模型多实例鲁棒拟合的研究空白，提出的估计器-优化器框架能够有效处理噪声数据和异常值，为复杂非经典模型的多实例重建提供了可行解决方案。
...
TL;DR: XSIM是一个用于自动驾驶的传感器仿真框架，通过扩展3DGUT splatting并引入广义滚动快门建模，能够渲染动态环境中的复杂传感器失真，特别解决了球形相机（如LiDAR）的投影问题。
...
Result: 在Waymo Open Dataset、Argoverse 2和PandaSet等多个自动驾驶数据集上评估，XSIM始终优于现有基线方法，在所有数据集上实现了最先进的性能。

Conclusion: XSIM提供了一个统一灵活的传感器仿真框架，能够准确渲染动态环境中的复杂传感器失真，特别解决了球形相机的关键边缘情况，实现了改进的几何一致性和逼真的外观表现。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于基础模型的手术视频分析方法主要依赖像素级重建目标，导致模型容量浪费在烟雾、镜面反射、流体运动等低层次视觉细节上，而非对手术理解至关重要的语义结构。

Method: 基于视频联合嵌入预测架构(V-JEPA)，提出三个针对手术视频的技术创新：1)运动引导的潜在预测以优先处理语义重要区域；2)时空亲和力自蒸馏以强制关系一致性；3)特征多样性正则化以防止纹理稀疏手术场景中的表示崩溃。使用UniSurg-15M数据集进行大规模预训练。
...
TL;DR: 论文探索了在人格识别中使用更细粒度的人格层次结构（细微特征和方面特征），相比传统的广泛特质分数，能显著提升从视听交互数据中识别人格的准确性。


<details>
  <summary>Details</summary>
Motivation: 传统人格识别模型依赖广泛特质分数作为真实标签，但相似的特质分数可能通过多样化的、依赖情境的行为表现出来。由于训练数据有限，这种依赖广泛特质分数的方法在泛化性方面面临挑战。

Method: 使用UDIVA v0.5数据集，训练了一个基于Transformer的模型，该模型包含跨模态（视听）和跨主体（二元组感知）的注意力机制。研究探索了大五人格模型中更细粒度的层次结构：方面特征和细微特征。

Result: 细微特征级别的模型在多个交互场景中始终优于方面特征和特质级别的模型，将均方误差降低了高达74%。

Conclusion: 在人格识别中利用更细粒度的人格层次结构（特别是细微特征）可以显著提升从视听交互数据中预测人格的准确性，为解决传统方法的泛化问题提供了有效途径。
...
TL;DR: ShapeUP是一个可扩展的图像条件3D编辑框架，通过监督式的潜在空间到潜在空间的转换实现精确的3D编辑，解决了现有方法在视觉可控性、几何一致性和可扩展性之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有的3D编辑框架面临视觉可控性、几何一致性和可扩展性之间的困难权衡：基于优化的方法速度过慢，多视图2D传播技术存在视觉漂移问题，而无需训练的潜在操作方法受限于冻结先验且无法从规模扩展中受益。

Method: ShapeUP将编辑定义为原生3D表示中的监督式潜在空间到潜在空间转换。它基于预训练的3D基础模型，通过监督训练适应编辑任务。具体使用3D Diffusion Transformer (DiT)学习从源3D形状、编辑后的2D图像到对应编辑3D形状的三元组直接映射，实现图像作为提示的细粒度视觉控制。

Result: ShapeUP在身份保持和编辑保真度方面一致优于当前有训练和无训练的基线方法，实现了隐式的无掩码定位，同时保持与原始资产的严格结构一致性。

Conclusion: ShapeUP为原生3D内容创作提供了一个稳健且可扩展的范式，通过图像条件编辑框架解决了3D精确操纵的挑战，同时保持了生成先验的优势。
...
TL;DR: 提出了一种多任务学习框架，通过三个自监督时序理解任务增强点监督时序动作定位模型的时序理解能力
...
Result: 在四个基准数据集上的广泛实验结果表明，该方法相比多个最先进方法具有更好的效果

Conclusion: 首次显式探索时序一致性用于点监督动作定位，通过自监督时序理解任务有效提升了模型的时序理解能力和动作定位性能
...
TL;DR: 提出AGFF-Embed方法，通过自适应融合全局和细粒度语义信息，提升多模态大语言模型嵌入性能


<details>
  <summary>Details</summary>
Motivation: 现有CLIP和MLLM嵌入模型只能捕捉全局语义信息，而复杂场景需要同时处理全局和细粒度元素的混合感知模式，需要兼容的融合机制

Method: 提出AGFF-Embed方法：1) 提示MLLM生成关注不同维度语义信息的多个嵌入；2) 自适应平滑聚合这些嵌入；3) 结合显式梯度放大技术实现批次内困难负样本增强

Result: 在MMEB和MMVP-VLM基准测试中，AGFF-Embed在通用和细粒度理解方面均达到最先进的性能

Conclusion: AGFF-Embed通过自适应融合全局和细粒度感知信息，有效提升了多模态嵌入模型在复杂场景下的理解能力
...
TL;DR: DepthPrior是一个利用深度信息作为先验知识而非融合特征的框架，通过深度感知的训练和推理策略，显著提升小目标和远距离目标的检测性能，无需修改检测器架构。


<details>
  <summary>Details</summary>
Motivation: 小目标和远距离目标检测面临尺度变化、低分辨率和背景干扰等挑战，对安全关键应用至关重要。深度信息能改善检测，但现有方法需要复杂的架构修改。需要一种不改变检测器架构就能利用深度信息的方法。

Method: 提出DepthPrior框架，将深度作为先验知识而非融合特征：训练时使用深度损失加权(DLW)和深度损失分层(DLS)；推理时使用深度感知置信度阈值(DCT)。唯一开销是初始深度估计成本。

Result: 在四个基准数据集(KITTI、MS COCO、VisDrone、SUN RGB-D)和两个检测器(YOLOv11、EfficientDet)上验证，小目标检测性能提升达+9% mAP$_S$和+7% mAR$_S$，推理恢复率高达95:1(真检测vs假检测)。

Conclusion: DepthPrior在不增加额外传感器、不修改架构、不产生性能开销的情况下，利用深度先验知识显著提升小目标和远距离目标检测性能，为安全关键应用提供可靠解决方案。
...
Conclusion: FMPose3D通过流匹配和ODE实现高效多假设生成，结合后验期望聚合获得准确预测，为单目3D姿态估计提供了高效且性能优越的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有的跨域行人重识别方法通常采用复杂架构处理域差异，但研究发现具有风格多样性的单摄像头数据能带来更好的泛化。虽然单摄像头数据易于收集，但缺乏跨视角变化导致复杂性不足。
...
TL;DR: Allocentric Perceiver是一种无需训练的VLM增强策略，通过几何专家恢复3D状态并建立目标中心参考系，将心理旋转从隐式推理转为显式计算，显著提升空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 随着视觉语言导航/动作等空间任务需求增长，VLM的全中心感知能力受到关注，但VLM在处理需要显式视角转换的全中心空间查询时仍然脆弱，这些查询需要在目标中心框架而非观察相机视角下进行推理。

Method: 使用现成的几何专家从单张或多张图像恢复度量3D状态，实例化与指令语义意图对齐的查询条件全中心参考框架，将重建几何确定性地转换到目标框架，并用结构化、几何基础的表示提示骨干VLM。

Result: 在多个骨干模型上的空间推理基准测试中，全中心任务获得一致且显著的提升（约10%），同时保持强大的自我中心性能，超越了空间感知微调模型以及最先进的开源和专有模型。

Conclusion: Allocentric Perceiver通过将心理旋转从隐式推理转移到显式计算，有效增强了VLM的全中心空间推理能力，为空间任务提供了强大的训练免费解决方案。
...
TL;DR: 论文提出SparseVideoNav方法，首次将视频生成模型应用于超越视野导航任务，通过生成稀疏未来轨迹实现20秒视野的快速推理，相比传统方法获得27倍加速和2.5倍成功率提升。


<details>
  <summary>Details</summary>
Motivation: 现实世界导航应该基于简单高层意图而非详细语言指令，但现有LLM方法因依赖短视野监督而短视，视频生成模型天然受益于长视野监督，适合超越视野导航任务。

Method: 引入视频生成模型到该领域，提出SparseVideoNav方法，通过生成稀疏未来轨迹实现20秒视野的子秒级推理，相比未优化版本获得27倍加速。

Result: 在真实世界零样本实验中，SparseVideoNav在超越视野导航任务上达到最先进LLM基线2.5倍的成功率，首次在挑战性夜间场景实现该能力。

Conclusion: 视频生成模型因其长视野监督优势适合超越视野导航，SparseVideoNav通过稀疏轨迹生成实现高效推理，为现实世界自主导航提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 蒸馏自回归扩散模型能实现实时短视频合成，但在长序列生成中存在严重误差累积。现有测试时优化方法对图像或短视频有效，但在长序列中因不稳定奖励景观和蒸馏参数超敏感性而失效

Method: 提出测试时校正方法，利用初始帧作为稳定参考锚点，沿采样轨迹校准中间随机状态，无需训练即可集成到各种蒸馏模型中

Result: 实验表明TTC方法能无缝集成多种蒸馏模型，以可忽略的开销扩展生成长度，在30秒基准测试中匹配资源密集型训练方法的生成质量
...
TL;DR: 提出一种轻量级离散扩散轮廓细化管道，用于低数据条件下的鲁棒边界检测，在医学影像KVASIR数据集上优于多个SOTA基线，推理速度提升3.5倍


<details>
  <summary>Details</summary>
Motivation: 不规则和半透明物体的边界检测在医学影像、环境监测和制造中很重要，但这些应用通常面临标记数据稀缺和现场计算资源有限的问题。现有研究多关注分割掩码与真实标签的对齐，而边界检测任务在低数据条件下研究不足

Method: 使用带自注意力层的CNN架构作为核心，以分割掩码为条件，迭代去噪稀疏轮廓表示。引入多个新颖改进：简化扩散过程、定制模型架构、最小化后处理，能在少于500张训练图像的数据集上生成密集、孤立的轮廓

Result: 在医学影像数据集KVASIR上优于多个SOTA基线，在HAM10K和自定义野火数据集Smoke上具有竞争力，同时推理帧率提升3.5倍

Conclusion: 提出了一种高效、轻量级的边界检测方法，特别适用于低数据条件，在多个数据集上表现出色且推理速度快，为解决不规则和半透明物体边界检测问题提供了有效方案
...
<details>
  <summary>Details</summary>
Motivation: 准确的3D心脏结构表示对于解剖和功能定量分析至关重要。当前2D经胸超声心动图(TTE)只能提供有限的平面视图，需要从稀疏的2D图像重建完整的3D心脏形状，以实现更精确的腔室量化分析。

Method: 使用神经隐式函数从稀疏CTA平面重建心脏腔室和左心室心肌的3D形状。方法包括：1）模拟标准心尖2D TTE视图的平面；2）训练多层感知器从CTA的3D分割中学习形状先验；3）测试时通过联合优化潜在代码和刚性变换来重建3D心脏形状，将观察到的平面映射到3D空间。
...
Method: 提出CLIP-Map框架，使用可学习矩阵通过全映射与Kronecker分解来映射和组合预训练权重；引入对角线继承初始化来减少分布偏移问题，提高映射学习效率。

Result: 实验结果表明，CLIP-Map在各种压缩比下都优于基于选择的压缩框架，特别是在高压缩设置下取得了显著性能提升。
...
TL;DR: MGIPT提出了一种多尺度全局-实例提示调优方法，通过自适应尺度实例提示和多尺度全局提示的组合，解决医学图像分割中持续测试时适应的挑战，避免错误累积和灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 医学图像在不同临床中心存在分布偏移问题，阻碍预训练分割模型在多域部署。现有持续测试时适应方法存在错误累积和灾难性遗忘问题，而基于提示调优的方法虽然有所改进，但仍缺乏多尺度提示多样性、实例特定知识整合不足，以及存在隐私泄露风险。
...
Conclusion: MGIPT通过多尺度全局-实例提示调优，有效解决了医学图像分割中持续测试时适应的挑战，在避免错误累积和灾难性遗忘的同时，实现了跨持续变化目标域的鲁棒适应。
...
Method: 提出在流匹配目标下学习条件相关的源分布，更好地利用丰富的条件信号。通过方差正则化和源与目标之间的方向对齐来解决分布坍缩和不稳定性问题，并分析目标表示空间选择对结构化源流匹配的影响。
...
TL;DR: 提出Localized Semantic Alignment (LSA)框架，通过对齐真实视频和生成视频中动态物体周围的语义特征来增强视频生成的时间一致性，无需推理时的外部控制信号。


<details>
  <summary>Details</summary>
Motivation: 现有可控视频生成方法依赖推理时的控制信号来引导生成模型产生时间一致的动态物体，这限制了其作为可扩展和通用数据引擎的实用性。

Method: 提出LSA框架，通过比较真实视频和生成视频片段中动态物体周围的语义特征，引入语义特征一致性损失，结合标准扩散损失对预训练视频生成模型进行微调。

Result: 在nuScenes和KITTI数据集上的实验表明，使用LSA仅微调一个epoch的模型在常见视频生成评估指标上优于基线方法。为测试时间一致性，还采用了mAP和mIoU两个目标检测指标。
...
TL;DR: RISE-Video是一个面向推理的文本图像到视频合成基准测试，专注于评估模型对隐含世界规则的推理能力，而非表面美学质量。


<details>
  <summary>Details</summary>
Motivation: 当前生成视频模型在视觉保真度方面取得了显著进展，但它们在理解和推理隐含世界规则方面的能力仍然是一个关键但未被充分探索的领域。需要建立一个评估框架来测试模型在认知推理方面的表现。

Method: 提出了RISE-Video基准测试，包含467个人工标注的样本，涵盖8个严格类别。引入了四维评估协议：推理对齐、时间一致性、物理合理性和视觉质量。还提出了一个利用大型多模态模型模拟人类中心评估的自动化流水线。

Result: 对11个最先进的TI2V模型进行了广泛实验，揭示了这些模型在隐含约束下模拟复杂场景方面存在普遍缺陷，为未来世界模拟生成模型的进步提供了关键见解。

Conclusion: RISE-Video基准测试将评估重点从表面美学转向深度认知推理，为评估和推进生成视频模型的推理能力提供了一个结构化测试平台，揭示了当前模型在模拟复杂世界规则方面的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型直接从截图生成代码，但没有观察生成代码的视觉结果。人类开发者会迭代渲染实现、与设计比较，并学习视觉差异如何与代码更改相关。受此启发，作者希望让模型也能从视觉差异中学习。
...
Conclusion: 通过让模型学习渲染预测与参考设计之间的视觉差异，VisRefiner框架有效提升了截图到代码生成的质量和保真度，为这一领域提供了新的训练范式。
...
TL;DR: 本文提出GenArena评估框架，采用成对比较范式解决传统绝对评分标准的局限性，显著提升视觉生成模型评估的稳定性和与人类感知的一致性。
...
Method: 提出GenArena统一评估框架，采用成对比较范式而非绝对评分。通过系统分析现有评估方法的局限性，设计基于成对比较的稳定评估协议。

Result: 采用成对比较协议后，现成的开源模型能够超越顶级专有模型。评估准确率提升超过20%，与权威LMArena排行榜的Spearman相关性达到0.86，远超点式方法的0.36。

Conclusion: GenArena为视觉生成模型提供了严谨、自动化的评估标准，解决了传统评估方法的局限性，显著提升了评估的稳定性和与人类感知的一致性。
...
TL;DR: 提出Context Forcing框架解决长视频生成中的学生-教师不匹配问题，通过长上下文教师训练长上下文学生，使用慢-快记忆架构管理计算成本，实现超过20秒的有效上下文长度
...
Result: 该方法实现超过20秒的有效上下文长度，比LongLive和Infinite-RoPE等最先进方法长2-10倍。通过利用扩展的上下文，Context Forcing在长持续时间内保持卓越的一致性，在各种长视频评估指标上超越最先进基线

Conclusion: Context Forcing通过解决学生-教师不匹配问题，成功实现了长视频生成中的长期一致性，为实时长视频生成提供了有效的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉基础模型在2D任务上表现出色，但缺乏3D感知能力。为了将稳健的3D感知注入2D VFMs，需要解决传统方法中存在的特征平均伪影和每场景优化缓慢的问题

Method: 提出Splat and Distill框架：1）将教师模型的2D特征通过前馈方式提升为显式3D高斯表示；2）将这些3D特征"投影"到新视角，生成新的2D特征图；3）用这些特征图监督学生模型，蒸馏几何基础知识。用前馈提升方法替代传统每场景优化

Result: 在单目深度估计、表面法线估计、多视角对应和语义分割等下游任务上进行了全面评估。方法显著优于先前工作，不仅实现了3D感知能力的实质性提升，还增强了2D特征的语义丰富性

Conclusion: Splat and Distill框架成功地将稳健的3D感知注入2D视觉基础模型，通过前馈3D重建管道避免了特征平均伪影，创建了师生模型一致性相互提升的动态学习过程
...
Result: 该方法产生了能够超越训练数据泛化的运动先验，例如可以与未见物体进行交互。进一步展示了其在用户交互控制和真实机器人部署中的有效性。

Conclusion: InterPrior框架通过模仿学习和强化学习的结合，成功创建了可泛化的全身运动控制器，为仿人机器人的复杂交互任务提供了有效的解决方案，并展示了实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs空间推理方法大多采用被动融合策略，将几何信息作为全局流不加区分地融合，导致语义-几何错位和冗余信号。需要从被动融合转向主动感知。

Method: 提出GeoThinker框架，采用空间锚定融合：在选定的VLM层中，语义视觉先验通过帧严格交叉注意力选择性查询和整合任务相关几何信息，并通过重要性门控校准注意力权重。

Result: 在VSI-Bench上达到72.6的峰值分数，创下新SOTA；在具身指代和自动驾驶等复杂下游场景中展现出强大的泛化能力和显著改善的空间感知。

Conclusion: 主动整合空间结构的能力对于下一代空间智能至关重要，GeoThinker通过从被动融合到主动感知的范式转变，显著提升了MLLMs的空间推理性能。
...
TL;DR: SwimBird是一个可切换推理模式的多模态大语言模型，能根据输入动态选择文本推理、视觉推理或交错视觉-文本推理三种模式，解决了现有MLLMs固定推理模式导致的视觉任务表现受限和文本逻辑推理退化问题。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs主要依赖文本思维链进行推理，限制了在视觉密集型任务上的表现。最近的方法虽然通过注入连续隐藏状态作为"视觉思维"来提升视觉性能，但往往以牺牲文本逻辑推理能力为代价。核心问题在于固定的、预定义的推理模式无法根据不同的用户查询自适应选择最合适的思考模态。

Method: 提出SwimBird模型，采用混合自回归公式，统一文本思维的下一个token预测和视觉思维的下一个嵌入预测。设计了系统化的推理模式策划策略，构建了包含所有三种推理模式的多样化监督微调数据集SwimBird-SFT-92K。模型能够根据输入条件动态切换三种推理模式。

Result: 实验表明，SwimBird在保持强大文本逻辑推理能力的同时，显著提升了视觉密集型任务的性能。在涵盖文本推理和挑战性视觉理解的多样化基准测试中，SwimBird取得了最先进的结果，相比先前固定模式的多模态推理方法获得了稳健的增益。

Conclusion: SwimBird通过动态切换推理模式实现了灵活、查询自适应的模态选择，解决了现有MLLMs在视觉和文本推理之间的权衡问题，为多模态推理提供了更有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在多图像空间推理方面仍面临挑战，特别是需要从多视角观察构建连贯的3D理解，并在新的语言指定视角下进行推理。

Method: 提出CAMCUE框架，将每幅图像的相机姿态注入视觉标记中，将自然语言视角描述映射到目标相机姿态，并合成姿态条件化的想象目标视图来支持回答。同时构建了CAMCUE-DATA数据集用于训练和评估。
