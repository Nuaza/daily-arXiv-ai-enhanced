...
TL;DR: 论文提出了一种视频潜在压缩方法，通过去除潜在表示中的高频成分而非直接减少通道数，在保持相同压缩比的同时获得更好的视频重建质量。
...
Conclusion: 通过去除高频成分而非减少通道数进行潜在压缩，可以在不牺牲重建质量的前提下改善潜在扩散模型的训练和生成性能。
...
<details>
  <summary>Details</summary>
Motivation: 大规模文本到图像扩散模型虽然视觉保真度高，但存在安全风险，可能复制不受欢迎的内容（如受版权保护的内容）。现有概念擦除方法难以平衡可扩展性、精确性和鲁棒性，通常只能擦除几百个概念。

Method: 首先通过t分布混合模型建模低秩概念分布，通过仿射最优传输实现目标概念的精确擦除，同时通过锚定目标概念分布边界来保留其他概念。然后训练基于混合专家的MoEraser模块，移除目标嵌入同时保留锚定嵌入。通过向文本嵌入投影器注入噪声并微调MoEraser进行恢复，实现对抗白盒攻击的鲁棒性。

Result: 在超过2000个跨异构领域和扩散模型的概念上进行广泛实验，证明了在大规模概念擦除方面具有最先进的可扩展性和精确性。
...
TL;DR: 该调查分析了视觉机器人空间内存效率问题，提出了α=峰值运行时内存/保存地图大小的比率，揭示了实际部署成本与公布地图大小之间的差距，并建立了标准化评估协议。


<details>
  <summary>Details</summary>
Motivation: 随着视觉机器人在更大环境中导航，其空间内存无限增长，最终耗尽计算资源，特别是在嵌入式平台（8-16GB共享内存，<30W）上无法增加硬件。需要解决空间内存效率问题。

Method: 调查了88篇参考文献中的52个系统（1989-2025），从占用网格到神经隐式表示。引入了α比率（峰值运行时内存/保存地图大小），通过独立性能分析测量了不同方法的α值。提出了标准化评估协议，包括内存增长率、查询延迟、内存-完整性曲线和吞吐量退化。

Result: α值在神经方法中跨越两个数量级，从2.3（Point-SLAM）到215（NICE-SLAM）。通过帕累托前沿分析显示，没有单一范式在其评估体系中占主导地位：3DGS方法在Replica数据集上以90-254MB地图大小获得最佳绝对精度，而场景图以可预测成本提供语义抽象。
...
TL;DR: DSS是一个无需训练的动态语义引导框架，通过敏感语义边界建模和敏感语义引导，在文本到图像扩散模型中实现可解释、可控的概念擦除，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像扩散模型的概念擦除方法存在显著局限性：特征校正方法容易导致不受控制的过度校正，而标记级干预方法在语义粒度和上下文方面存在困难，且两种方法都容易产生严重的语义漂移甚至完全表示崩溃。

Method: 提出动态语义引导（DSS）框架，包含两个核心组件：1）敏感语义边界建模（SSBM）自动发现安全语义锚点；2）敏感语义引导（SSG）利用交叉注意力特征进行精确检测，并通过从良好定义的目标推导出的闭式解进行校正。

Result: DSS实现了平均91.0%的擦除率，显著优于现有最先进方法（从18.6%到85.9%），同时对输出保真度影响最小。

Conclusion: DSS是一个轻量级、无需训练的概念擦除框架，能够确保敏感内容的最优抑制同时保留良性语义，解决了现有方法的局限性。
...
TL;DR: CLWM模型通过使用DINOv3特征作为生成目标，实现严格O(1)内存占用和约50%延迟降低，在复杂双臂仿真和零样本sim-to-real迁移中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 部署生成式世界-动作模型面临像素级重建冗余、O(T)内存扩展和顺序推理延迟三大瓶颈，限制了机器人操作的实际应用。

Method: 1) 使用DINOv3特征作为生成目标解耦交互语义与视觉噪声；2) 双状态测试时训练内存实现严格O(1)内存占用；3) 推测异步推理将部分扩散去噪与物理执行并行；4) EmbodiChain框架注入无限物理基础轨迹流进行训练。

Result: CLWM在复杂双臂仿真中达到最先进性能，并在物理机器人上实现前所未有的零样本sim-to-real迁移，性能优于在真实数据上显式微调的基线方法。

Conclusion: CLWM通过特征级生成、O(1)内存占用和异步推理解决了机器人操作模型部署的关键瓶颈，为鲁棒策略的规模化部署提供了有效解决方案。
...
TL;DR: 本文提出通过邻域对齐改进CLIP检索的方法，包括匈牙利匹配的邻域级重排序和查询条件化局部引导，以解决局部几何不一致问题。


<details>
  <summary>Details</summary>
Motivation: CLIP检索通常被视为共享嵌入空间中的点相似性问题。虽然CLIP实现了强大的全局跨模态对齐，但许多检索失败源于局部几何不一致：邻近项被错误排序，导致系统性混淆（如五边形与六边形），并产生扩散、弱控制的结果集。

Method: 1. 通过匈牙利匹配进行邻域级重排序，奖励结构一致性；2. 查询条件化局部引导，利用查询周围对比邻域导出的方向重塑检索。

Result: 这些技术在属性绑定和组合检索任务上提高了检索性能。两种方法都在局部邻域上操作但扮演不同角色：重排序奖励对齐，而局部引导控制邻域结构。

Conclusion: 检索质量和可控性关键依赖于局部结构，可以在推理时利用而无需重新训练。
...
TL;DR: 本文提出了一种结合模糊逻辑的新型损失函数，用于处理脑部MRI图像分割中的不确定性，在IBSR和OASIS数据集上相比传统交叉熵损失获得了更好的分割性能。


<details>
  <summary>Details</summary>
Motivation: 脑部图像分割在神经疾病诊断中至关重要，但传统深度学习方法在处理分割不确定性方面存在不足。需要一种能够有效处理像素分类不确定性的损失函数来提升分割精度和模型可靠性。

Method: 提出了一种新颖的损失函数，将传统的分类交叉熵(CCE)损失与基于模糊逻辑的模糊熵相结合。该损失函数通过模糊逻辑处理像素分类中的固有不确定性，并在U-Net和U-Net++架构上进行评估。

Result: 在IBSR和OASIS两个公开基准数据集上的实验结果表明，使用所提损失函数训练的模型在多种性能指标上均优于仅使用CCE损失函数，同时能有效处理训练过程中的不确定性。
...
TL;DR: 提出一种轻量级Transformer架构，通过统一标记化机制融合多种fNIRS表征，实现疼痛识别，在保持计算紧凑性的同时获得竞争性性能


<details>
  <summary>Details</summary>
Motivation: 疼痛是多方面且普遍的现象，具有显著的临床和社会负担，可靠的自动评估是重要目标。现有方法在处理多种fNIRS表征时通常需要模态特定适配或增加架构复杂性

Method: 提出轻量级Transformer架构，通过统一标记化机制融合多个fNIRS表征（原始波形和功率谱密度），使用结构化分割方案控制局部聚合和全局交互的粒度，将异构输入投影到共享潜在表示中

Result: 在AI4Pain数据集上评估，实验结果显示具有竞争力的疼痛识别性能，同时保持计算紧凑性，适合GPU和CPU硬件上的实时推理

Conclusion: 该方法能够联合建模互补信号视图，无需模态特定适配或增加架构复杂性，为实时疼痛评估提供了有效的轻量级解决方案
...
TL;DR: HQA-VLAttack：针对视觉语言预训练模型的黑盒对抗攻击框架，通过文本和图像两阶段攻击生成高质量对抗样本，在保证语义一致性的同时提升攻击成功率


<details>
  <summary>Details</summary>
Motivation: 当前针对视觉语言预训练模型的黑盒对抗攻击研究处于早期阶段，现有方法要么依赖复杂的迭代交叉搜索策略导致查询次数过多，要么只考虑降低正样本对的相似度而忽略负样本对，影响攻击性能

Method: 提出两阶段攻击框架：1）文本攻击阶段使用counter-fitting词向量生成替代词集，保证语义一致性；2）图像攻击阶段先通过层重要性引导策略初始化对抗样本，再利用对比学习优化对抗扰动，同时降低正样本对相似度并增加负样本对相似度
...
TL;DR: 该研究评估了17种前沿多模态大语言模型在复杂医疗表格数字化任务上的表现，发现最新模型（Gemini 3.1、GPT 5.4、Claude Sonnet 4.6）在特定任务上表现出色，提示优化可显著提升性能，为低收入和中等收入国家的手写文档自动化数字化提供了可行路径。


<details>
  <summary>Details</summary>
Motivation: 手工数字化结构化手写文档既缓慢又昂贵，特别是在医疗等关键领域。研究旨在评估当前多模态大语言模型在复杂现实世界医疗表格数字化任务上的能力，探索自动化解决方案的可行性。

Method: 使用一个极具挑战性的真实世界医疗表格作为基准测试集，该表格混合了日期、结构化印刷文本、手写回答和显著变异性。评估了17种领先的前沿多模态大语言模型和开源模型，包括Google、OpenAI等最新模型。采用多种评估指标：准确率、加权F1分数、WER（词错误率）、CER（字符错误率）和幻觉率。还研究了提示优化对性能的影响。

Result: 较旧或较小的模型表现不佳，但最新模型（Google和OpenAI）在离散或预定义字段上达到约85%的准确率和约90%的加权F1分数。GPT 5.4在噪声日期提取和可靠性方面表现最佳，幻觉率最低（6%）。Claude Sonnet 4.6在格式化字段（日期和数值）上平均性能最好。Gemini 3.1整体表现最佳，自由文本错误率最低（WER=0.50，CER=0.31），在离散分类指标上结果最强。提示优化使宏观精确率、召回率和F1提高了60%以上，但对加权指标改善有限（仅2-5%）。

Conclusion: 多模态大语言模型的快速改进为复杂手写工作流程的完全自动化数字化提供了有前景的途径，这在低收入和中等收入国家尤其相关。不同模型在特定任务上各有优势，提示优化能显著提升某些性能指标，为实现高效、低成本的手写文档数字化提供了技术基础。
...
TL;DR: MedThinkVQA是一个专家标注的多图像医学问答基准，要求模型整合跨视图证据进行临床推理。现有大型语言模型在该基准上表现不佳，主要瓶颈在于跨图像证据的提取、对齐和组合能力不足。


<details>
  <summary>Details</summary>
Motivation: 现有的医学问答基准大多基于单图像分析，而真实临床诊断需要整合多个图像视图的证据。为了评估模型在多图像临床推理中的能力，需要创建专门的基准来测试模型整合跨视图证据的能力。

Method: 研究者创建了MedThinkVQA基准，包含8,067个病例（720个测试病例），每个病例平均6.62张图像，远高于先前工作。基准要求模型解释每个图像、整合跨视图证据，并提供中间监督和步骤级评估。通过分析不同模型的表现，识别多图像推理的瓶颈。

Result: 最佳闭源模型（Claude-4.6-Opus、Gemini-3-Pro、GPT-5.2-xhigh）准确率仅为57.2%、55.3%、54.9%。开源模型表现更差。超过70%的错误来自图像读取和跨视图整合。提供专家单图像线索和跨图像摘要能提高性能，而使用自生成的中间结果会降低准确性。

Conclusion: 多图像医学推理的主要挑战不是推理长度，而是跨真实世界多模态临床输入的可靠证据提取、对齐和组合机制。当早期证据提取薄弱时，增加推理时间计算只能带来有限或不稳定的改进。
...
TL;DR: SynthPID：通过从真实图纸中提取管道拓扑生成的665张合成P&ID图像，配合改进的Relationformer模型，仅使用合成数据训练就能在真实测试集上达到接近真实数据训练的性能（63.8% vs 71.8%），解决了P&ID数字化领域数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: P&ID图纸数字化对工厂运营有重要价值，但进展受阻于数据稀缺问题：工程图纸具有专有性，整个领域只有一个包含12张标注图像的公共基准。之前基于模板的合成数据生成方法效果不佳，生成的图与真实工厂流程相似度低，导致边缘检测准确率仅约33%。

Method: 1. 提出SynthPID语料库：包含665张合成P&ID图像，其管道拓扑直接从真实图纸中提取，确保结构真实性；2. 采用基于patch的Relationformer模型，适配高分辨率图纸；3. 仅使用合成数据训练，不接触任何真实P&ID图像。

Result: 仅使用SynthPID合成数据训练的模型在PID2Graph OPEN100测试集上达到63.8±3.1%的边缘mAP，与使用真实数据训练的oracle模型（71.8%）仅相差8个百分点。与基于模板的方法相比有显著提升，确认性能提升源于生成质量而非模型选择。扩展研究发现收益在约400张图像后趋于平缓，表明种子多样性是关键限制因素。

Conclusion: 通过从真实图纸中提取结构信息生成合成P&ID图像，能够有效解决该领域的数据稀缺问题。生成质量（特别是结构真实性）是性能提升的关键驱动因素，而非模型架构选择。这种方法为工程图纸数字化提供了可行的数据增强方案。
...
TL;DR: BARD框架将预训练的自回归视觉语言模型转换为相同架构但解码效率更高的大块扩散视觉语言模型，实现3倍解码吞吐量提升


<details>
  <summary>Details</summary>
Motivation: 自回归视觉语言模型存在token-by-token解码的推理瓶颈，而直接将预训练自回归模型转换为大块扩散模型会导致质量显著下降

Method: 结合渐进式监督块合并（逐步增大解码块大小）和阶段式内部扩散蒸馏（从小块扩散锚点恢复性能），并采用混合噪声调度器和内存友好训练

Result: BARD-VL在4B和8B规模上建立了可比规模开放扩散视觉语言模型的新SOTA，同时相比源模型实现高达3倍解码吞吐量加速

Conclusion: BARD框架能有效将预训练自回归视觉语言模型转换为解码高效的大块扩散模型，内部扩散蒸馏比直接自回归到扩散蒸馏更有效
...
TL;DR: 本文对文本到图像生成模型的公平性研究进行了系统性综述，提出了偏倚类型和公平概念的分类法，并构建了从描述性指标转向目标导向测试的新框架。


<details>
  <summary>Details</summary>
Motivation: 文本到图像生成模型在广泛应用的同时，经常表现出社会刻板印象问题。当前研究领域存在概念模糊性，如"偏倚"和"公平"等术语缺乏明确的区分和操作定义，需要系统性的整理和分析。

Method: 通过系统性文献综述，将现有研究组织成偏倚类型和公平概念的分类法；批判性评估"目标公平"（T2I输出的规范性理想）与"阈值公平"（具有可操作决策规则的规范性标准）之间的差距；调查从提示工程到扩散过程操纵等缓解策略。

Result: 提出了一个超越描述性指标、转向严谨目标导向测试的新框架，为更负责任的生成式AI开发提供了操作化公平性的方法。

Conclusion: 需要从描述性指标转向目标导向的测试框架，以更负责任地开发生成式AI系统，通过操作化公平性概念来弥合理论理想与实际实施之间的差距。
...
TL;DR: 提出一种基于多单目相机的快速在线3D多目标跟踪与姿态估计方法，仅需2D边界框和姿态检测，无需昂贵的3D训练数据或复杂深度学习模型


<details>
  <summary>Details</summary>
Motivation: 现有3D多目标跟踪方法通常需要昂贵的3D训练数据或计算复杂的深度学习模型，限制了实际应用。需要一种更高效、更实用的解决方案

Method: 采用贝叶斯最优多目标跟踪滤波器的高效实现，仅使用公开可用的预训练2D检测模型，通过多相机系统进行3D跟踪和姿态估计

Result: 算法速度显著快于最先进方法且不损失精度，在相机间歇性断开或重新连接的情况下仍能保持鲁棒性能

Conclusion: 该方法提供了一种高效实用的3D多目标跟踪解决方案，仅需2D检测输入，适用于实际部署场景，特别是在相机连接不稳定的环境中表现优异
...
<details>
  <summary>Details</summary>
Motivation: 为了解决语义分割任务中的隐私保护问题，特别是如何在保护图像隐私的同时进行有效的模型训练和推理，同时允许每个客户端使用独立的加密密钥。

Method: 模型创建者和每个客户端使用本地生成的密钥加密图像，在加密图像上进行模型训练和推理。为减轻性能下降，在模型训练中应用图像加密方法，同时生成测试图像。使用基于视觉Transformer的SETR模型进行验证。

Result: 在Cityscapes数据集上的实验证实了该方法的有效性，表明在保持隐私保护的同时能够实现语义分割任务。

Conclusion: 该方法提供了一种有效的隐私保护语义分割解决方案，支持每个客户端和图像的独立密钥加密，在保护隐私的同时保持了模型性能。
...
TL;DR: 本文提出了一个专家标注的胚胎图像和自然语言描述数据集，用于微调视觉语言模型，实现基于证据的胚胎选择和透明决策


<details>
  <summary>Details</summary>
Motivation: 当前胚胎选择主要依赖胚胎学家的形态学评估，AI方法虽然展示了潜力但整体影响有限，主要问题包括需要适应临床数据、依赖时间推移培养箱以及缺乏可解释性。现代患者质疑专家决策，需要基于证据的决策理由来支持透明决策和尊重患者的沟通。

Method: 创建专家标注的胚胎图像和相应形态学描述数据集，使用自然语言描述胚胎细胞周期、发育阶段和形态特征。该数据集可用于微调现代基础视觉语言模型，通过预测胚胎描述自动从文献中提取科学证据。

Result: 提出的数据集支持基于语言的、可解释的、透明的自动化胚胎评估研究，能够促进基于证据的决策制定和与患者的透明沟通，有潜力显著改善决策过程和患者结果。

Conclusion: 该专家标注的胚胎图像和描述数据集为开发可解释的AI胚胎评估系统提供了重要资源，能够支持基于证据的决策制定，提高体外受精过程中胚胎选择的透明度和患者沟通质量。
...
### [18] [Beyond Attack Success Rate: A Multi-Metric Evaluation of Adversarial Transferability in Medical Imaging Models](https://arxiv.org/abs/2604.16532)
*Emily Curl,Kofi Ampomah,Md Erfan,Sayanton Dibbo*

Main category: cs.CV

TL;DR: 该研究系统评估了医学图像分析中深度学习模型对抗攻击的脆弱性，发现仅使用攻击成功率（ASR）作为评估指标是不充分的，需要结合感知质量、失真度等多指标框架。


<details>
  <summary>Details</summary>
Motivation: 随着深度学习在医学图像分析中的广泛应用，其对抗扰动的脆弱性引发临床部署担忧。现有评估主要依赖攻击成功率（ASR），但该指标忽略了扰动强度、图像质量、跨架构攻击可迁移性等因素，无法全面反映对抗行为。特别是随着Vision Transformers挑战CNN主导地位，不同架构学习方式差异使得单一ASR指标的有效性存疑。
...
Result: 研究发现一致模式：感知和失真指标彼此强相关，但与ASR相关性极小。这一规律同时适用于CNN和ViT架构。结果表明ASR单独作为对抗鲁棒性和可迁移性指标是不充分的。

Conclusion: 医学AI中对抗风险的全面评估需要多指标框架，不仅要考虑攻击效果，还要包含攻击方法及其相关开销。仅依赖ASR无法充分反映模型的对抗脆弱性。
...
TL;DR: 本文提出PoInit-of-View方法，通过攻击3D重建系统中的SfM初始化模块，引入跨视角梯度不一致性，破坏关键点检测和特征匹配，从而实现对多种3D重建系统的可转移毒化攻击。


<details>
  <summary>Details</summary>
Motivation: 现有研究仅通过整个3D重建管道反向传播对抗梯度，未能揭示特定模块的新漏洞。本文发现结构从运动（SfM）初始化作为许多重建系统的几何核心，可被针对性攻击以实现跨系统的可转移毒化效果。

Method: 提出PoInit-of-View方法，优化对抗扰动以在对应3D点的投影处引入跨视角梯度不一致性。这些不一致性破坏关键点检测和特征匹配，从而腐蚀SfM中的姿态估计和三角测量，最终导致渲染视图质量下降。并提供理论分析连接跨视角不一致性与对应关系崩溃。

Result: 实验结果表明PoInit-of-View在多种3D重建系统和数据集上有效，在黑盒转移设置下（如3DGS到NeRF），PSNR比单视角基线提升25.1%，SSIM提升16.5%。

Conclusion: 通过针对性攻击SfM初始化模块，PoInit-of-View实现了跨多种3D重建系统的可转移毒化攻击，显著优于现有方法，揭示了3D重建系统中特定模块的新安全漏洞。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大型生成模型在多模态生成方面取得进展，但生成插图故事书仍面临挑战：先前工作主要将任务分解为独立阶段，导致整体多模态基础有限；同时，虽然针对纯文本或纯图像生成进行了安全对齐研究，但现有工作很少将儿童特定安全约束整合到叙事规划和序列级多模态验证中。
...
Conclusion: BookAgent为高质量、安全意识的视觉叙事故事书生成提供了一个有效的多智能体协作框架，解决了现有方法在多模态基础和安全约束整合方面的局限性，代码将在GitHub上公开。
...
### [21] [A B-Spline Function Based 3D Point Cloud Unwrapping Scheme for 3D Fingerprint Recognition and Identification](https://arxiv.org/abs/2604.16546)
...
<details>
  <summary>Details</summary>
Motivation: 3D指纹识别相比传统2D系统具有接触式、卫生安全、防欺骗等优势，并能捕捉深度、曲率和形状信息，实现更精确的认证系统。然而，指纹像素的拓扑高度使得脊谷模式提取复杂，配准问题限制了采集过程，需要所有样本保持一致的朝向和方向。

Method: 提出一种将3D指纹点云展开的方法：使用B样条曲线拟合来缓解高度变化和减少配准限制；将展开的点云通过映射点的相对高度转换为灰度图像；然后使用传统的2D指纹识别方法对灰度图像进行识别。

Result: 该方法在3D指纹识别中表现出优越性能，在三个实验中分别达到0.2072%、0.26%和0.22%的等错误率，优于现有方法。在跨会话实验中，该方法在包含不同配准的指纹识别和认证方面均优于3D指纹展平技术，达到1.50%的等错误率。

Conclusion: 提出的3D指纹展开方法有效解决了高度变化和配准问题，通过转换为灰度图像并利用成熟的2D识别技术，实现了高性能的3D指纹识别系统，在多个指标上超越了现有方法。
...
### [22] [Co-generation of Layout and Shape from Text via Autoregressive 3D Diffusion](https://arxiv.org/abs/2604.16552)
...
Result: 模型能够根据文本指令生成和放置物体，遵循非平凡的空间布局和语义要求，在挑战性场景上表现良好

Conclusion: 提出的序列化文本到场景生成范式和3D-ARD+模型有效解决了现有方法的局限性，实现了更符合文本描述的复杂3D场景生成
...
### [23] [See Through the Noise: Improving Domain Generalization in Gaze Estimation](https://arxiv.org/abs/2604.16562)
*Yanming Peng,Shijing Wang,Yaping Huang,Yi Tian*

Main category: cs.CV

TL;DR: 本文首次全面研究标签噪声对视线估计泛化性能的负面影响，提出See-Through-Noise框架，通过构建语义嵌入空间和亲和一致性度量来缓解标签噪声，提升跨域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有泛化视线估计方法忽视了标签噪声对模型泛化性能的影响，而精确的视线标注获取困难导致标签噪声问题普遍存在。本文旨在首次全面研究标签噪声对泛化性能的负面影响，并提出解决方案。

Method: 提出See-Through-Noise框架：1) 通过基于原型的变换构建语义嵌入空间，保持视线特征与连续标签之间一致的拓扑结构；2) 测量特征-标签亲和一致性来区分噪声样本和干净样本；3) 在语义流形中引入亲和正则化，将视线相关信息从干净样本传递到噪声样本。

Result: 大量实验表明，SeeTN能有效缓解源域噪声的负面影响，在不损害源域准确性的前提下实现优越的跨域泛化性能，凸显了在泛化视线估计中显式处理噪声的重要性。

Conclusion: 本文首次系统研究标签噪声对视线估计泛化的影响，提出的SeeTN框架通过语义结构对齐和域不变视线关系增强，显著提升了模型对标签噪声的鲁棒性，为泛化视线估计提供了新视角。
...
### [24] [Camo-M3FD: A New Benchmark Dataset for Cross-Spectral Camouflaged Pedestrian Detection](https://arxiv.org/abs/2604.16582)
*Henry O. Velesaca,Andrea Mero,Guillermo A. Castillo,Angel D. Sappa*

Main category: cs.CV

TL;DR: 该论文提出了Camo-M3FD数据集，这是一个用于跨光谱伪装行人检测的新基准，包含配对的可见光-热红外图像，填补了安全关键人类检测领域的空白。


<details>
  <summary>Details</summary>
Motivation: 现有伪装目标检测基准主要关注生物物种，而在安全关键的人类检测领域，当目标与周围环境融合时存在研究空白。自动驾驶、机器人和监控中的行人检测面临遮挡、杂乱背景和能见度差的挑战，而伪装行人的检测问题尚未得到充分探索。

Method: 从M3FD数据集衍生出Camo-M3FD基准，包含配对的可见光-热红外图像对。使用定量指标筛选数据以确保高前景-背景相似度，提供高质量的像素级掩码，并建立基于最先进COD模型的标准化评估框架。

Result: 实验结果表明，热红外信号提供不可或缺的定位线索，而多光谱融合对于细化结构细节至关重要。Camo-M3FD为开发鲁棒的安全关键检测系统提供了基础资源。

Conclusion: Camo-M3FD填补了伪装行人检测领域的空白，为开发更可靠的自动驾驶、机器人和监控系统提供了重要数据集和评估框架，强调了多光谱融合在安全关键应用中的重要性。
...
### [25] [Real-Time Visual Attribution Streaming in Thinking Model](https://arxiv.org/abs/2604.16587)
...
<details>
  <summary>Details</summary>
Motivation: 多模态思维模型（如从截图生成代码或从图像解决数学问题）的长推理轨迹需要基于视觉证据，但验证这种依赖关系具有挑战性：忠实的因果方法需要昂贵的重复反向传播或扰动，而原始注意力图虽然能即时访问，但缺乏因果有效性。

Method: 引入一种摊销方法，通过学习从注意力特征中编码的丰富信号直接估计语义区域的因果效应。该方法能够实现视觉归因流式处理，在模型推理过程中实时显示归因证据。

Result: 在五个不同的基准测试和四个思维模型上，该方法实现了与穷举因果方法相当的忠实性，同时支持视觉归因流式处理，用户可以在模型推理过程中实时观察归因证据。

Conclusion: 通过轻量级学习而非暴力计算，在多模态思维模型中实现实时、忠实的归因是可行的，为多模态推理提供了有效的可解释性解决方案。
...
### [26] [AVRT: Audio-Visual Reasoning Transfer through Single-Modality Teachers](https://arxiv.org/abs/2604.16617)
*Edson Araujo,Saurabhchand Bhati,M. Jehanzeb Mirza,Brian Kingsbury,Samuel Thomas,Rogerio Feris,James R. Glass,Hilde Kuehne*

Main category: cs.CV

TL;DR: AVRT框架通过单模态教师模型生成高质量音视频推理轨迹，采用监督微调+强化学习两阶段训练，在音视频和音频推理任务上取得SOTA结果


<details>
  <summary>Details</summary>
Motivation: 当前推理模型在文本领域进展显著，但将推理能力迁移到多模态场景（如音视频数据）仍面临挑战，主要因为目标多模态组合中高质量推理数据的稀缺

Method: 1) 通过专门模型生成独立的视觉和音频推理轨迹；2) 使用LLM合并模型融合轨迹；3) 两阶段训练：先用生成的多模态轨迹进行监督微调冷启动，然后在更大规模数据上进行强化学习

Result: 在7个音视频和音频基准测试中，3B和7B参数模型取得了与同类规模模型相比的SOTA结果，包括在OmniBench、DailyOmni（音视频）和MMAR（纯音频）上的表现，证明跨模态训练也能迁移到单模态任务

Conclusion: AVRT框架成功解决了多模态推理数据稀缺问题，建立了新的多模态推理模型训练流程，展示了跨模态训练的有效性和可迁移性
...
TL;DR: 本文系统评估了CNN、Transformer和大型预训练基础模型在滑坡检测任务中的表现，并比较了全微调与参数高效微调方法的效果。


<details>
  <summary>Details</summary>
Motivation: 虽然高分辨率卫星影像滑坡检测对灾害响应和风险评估至关重要，但现代分割架构和微调策略在该问题上的相对有效性仍缺乏系统研究。

Method: 使用全球分布同震滑坡数据集(GDCLD)，在一致的训练和评估协议下评估代表性CNN和Transformer分割模型以及大型预训练基础模型，并比较全微调与LoRA、AdaLoRA等参数高效微调方法。

Result: 实验结果表明，基于Transformer的模型实现了强大的分割性能，而参数高效微调方法在保持与全微调相当精度的同时，可将可训练参数减少高达95%。

Conclusion: 通过分析验证集和保留测试集的性能，进一步评估了模型在分布偏移下的泛化能力，为滑坡检测任务提供了系统性的架构选择和微调策略指导。
...
TL;DR: 人类在零样本条件下能够识别外观缺失的动作视频，准确率高于随机水平但低于自然视频。研究者开发的双通路3D CNN模型结合了RGB（形状）和光流（运动）流，通过相干门控机制模拟人类表现，在泛化到外观缺失视频方面优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究人类和视觉模型是否能够在零样本条件下泛化到外观缺失的动作视频。虽然经典心理物理学研究表明人类可以在形状线索退化的情况下感知身体运动，但现有研究通常需要在外观缺失视频上进行显式训练。本研究旨在测量人类在零样本条件下对两种外观缺失变换（密集噪声运动视频和随机点外观缺失视频）的泛化能力。

Method: 研究方法包括：1）人类实验：22名参与者使用自然视频（UCF5数据集）训练识别五个动作类别，然后零样本测试两种外观缺失变换；2）计算模型：开发双通路3D CNN模型，结合RGB流（形状）和光流流（运动），并引入受格式塔共同命运分组启发的相干门控机制；3）比较分析：将模型性能与人类表现和当代视频分类模型进行比较。

Result: 研究发现：1）人类参与者在两种外观缺失视频上的动作识别准确率均显著高于随机水平，但低于自然视频；2）开发的双通路模型在两种外观缺失数据集上都能良好泛化，性能优于当代视频分类模型，更接近人类表现；3）运动通路对于泛化到外观缺失视频至关重要，而形状通路则提升自然视频的性能。

Conclusion: 结论表明：1）运动表征对于泛化到外观缺失视频非常重要；2）多流架构（结合形状和运动信息）能更好地建模基于视频的动作识别；3）研究结果支持使用结合运动信息的计算模型来理解人类动作识别机制。
...
### [29] [C-GenReg: Training-Free 3D Point Cloud Registration by Multi-View-Consistent Geometry-to-Image Generation with Probabilistic Modalities Fusion](https://arxiv.org/abs/2604.16680)
*Yuval Haitman,Amit Efraim,Joseph M. Francos*

Main category: cs.CV

TL;DR: C-GenReg是一个无需训练的点云配准框架，通过将3D配准问题转移到图像域，利用视觉基础模型提取密集对应关系，再通过概率融合结合几何和生成RGB分支的结果。


<details>
  <summary>Details</summary>
Motivation: 当前基于学习的3D点云配准方法难以泛化到不同的传感模态、采样差异和环境条件。为了解决这一问题，需要利用世界尺度生成先验和视觉基础模型的互补优势，实现零样本的跨域泛化。

Method: 1) 使用世界基础模型从输入几何生成多视角一致的RGB表示；2) 利用预训练的视觉基础模型在生成的RGB图像中提取密集对应关系；3) 通过原始深度图将像素对应关系提升回3D；4) 引入"匹配-融合"概率冷融合方案，结合生成RGB分支和原始几何分支的对应后验概率。

Result: 在室内（3DMatch、ScanNet）和室外（Waymo）基准测试中表现出强大的零样本性能和卓越的跨域泛化能力。首次展示了在真实室外LiDAR数据上成功运行的生成式配准框架，无需任何图像数据。

Conclusion: C-GenReg通过将3D配准问题转移到图像域并利用预训练模型，实现了无需训练的零样本点云配准，在跨模态、跨环境场景中展现出优异的泛化能力，为无图像数据的室外LiDAR配准提供了有效解决方案。
...
### [30] [LOD-Net: Locality-Aware 3D Object Detection Using Multi-Scale Transformer Network](https://arxiv.org/abs/2604.16696)
*Mustaqeem Khan,Aidana Nurakhmetova,Wail Gueaieb,Abdulmotaleb El Saddik*

Main category: cs.CV

TL;DR: 该论文提出了一种集成到3DETR架构中的多尺度注意力机制，通过上采样操作生成高分辨率特征图，提升点云中3D物体检测性能，在ScanNetv2数据集上取得了显著改进。


<details>
  <summary>Details</summary>
Motivation: 点云数据的稀疏性和缺乏全局结构使得3D物体检测具有挑战性，需要更好地捕捉局部几何和全局上下文信息。

Method: 提出多尺度注意力机制集成到3DETR架构中，引入上采样操作生成高分辨率特征图，增强对小目标和语义相关物体的检测能力。

Result: 在ScanNetv2数据集上，3DETR+MSA模型相比基线在mAP@25上提升近1%，在mAP@50上提升4.78%；但对轻量级3DETR-m变体改进有限。

Conclusion: 结合分层特征提取和注意力机制能有效增强3D场景理解，但需要针对轻量级模型调整上采样策略。
...
TL;DR: 本文提出iDoc模型，通过改进编码器和自监督训练，结合开放集检测器，显著提升了历史文档中图形模式搜索的速度和精度，特别是在小尺寸非方形查询上达到新的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 随着数字书籍的大规模普及，通过图形模式搜索文档集合变得至关重要。当前历史文档检索和模式识别策略存在不足：SOTA方法整体精度仅0.494，小尺寸非方形查询精度仅0.427，且处理时间过长（DocExplore数据集需7秒）。

Method: 提出基于改进编码器(iDoc)的新模型，采用自监督训练策略，结合开放集检测器加速搜索。与之前版本不同，本版本利用非极大值抑制减少误报。

Result: 模型在模式识别和文档检索方面达到与SOTA竞争的结果，搜索速度提升10倍。在小尺寸非方形查询上达到新的SOTA性能，精度提升至0.612。

Conclusion: 提出的iDoc模型有效解决了历史文档图形模式搜索中的精度和速度问题，特别是在小尺寸非方形查询场景下显著提升了性能。
...
TL;DR: 本文提出了一种无需训练的智能体框架，用于自动化脑部MRI分析，通过让大语言模型协调外部专业工具，解决了现有模型缺乏3D空间推理能力的问题。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在通用视觉问答中表现出色，但缺乏处理3D医学影像（如CT或MRI）所需的本机3D空间推理能力。智能体AI提供了一种新解决方案，通过让LLM协调外部工具来避免内在3D处理需求，但这类框架在复杂多步骤放射学工作流程中的可行性尚未充分探索。

Method: 提出了一个无需训练的智能体管道用于自动化脑部MRI分析，使用现成的领域特定工具，系统能够自主执行复杂的端到端工作流程，包括预处理（颅骨剥离、配准）、病理分割（胶质瘤、脑膜瘤、转移瘤）和体积分析。比较了单智能体模型与多智能体"领域专家"协作架构。

Result: 验证了多个LLM（GPT-5.1、Gemini 3 Pro、Claude Sonnet 4.5）与现成工具的配合，系统能够处理从单次扫描分割和体积报告到需要多时间点比较的纵向响应评估等日益复杂的放射学任务。引入了基于公开BraTS数据的基准数据集。

Conclusion: 智能体AI可以通过工具使用解决高度复杂的神经放射学图像分析任务，无需训练或微调，为医学影像分析提供了新的解决方案。
...
TL;DR: AW4RE是一个面向物理感知的生成世界模型，通过4D信息检索和条件生成，为部分可观测环境中的感知决策提供传感器原生替代环境。


<details>
  <summary>Details</summary>
Motivation: 物理感知在动态大环境中面临部分可观测的挑战，传统强化学习在全可观测问题上成功，但在POMDP等部分可观测问题上仍存在探索成本高、仿真到现实视角缺失等问题。

Method: AW4RE结合4D信息检索、动作条件几何支持与时间一致性、条件生成补全，构建传感器原生的生成世界模型，估计动作条件观测过程。

Result: 实验表明AW4RE在极端视角变化、时间间隔和稀疏几何支持情况下，比几何感知生成基线产生更可靠和一致的预测。

Conclusion: AW4RE为部分可观测环境中的感知决策提供了有效的生成世界模型解决方案，能够处理复杂的物理感知挑战。
...
TL;DR: 该论文提出了一种顺序输入压缩机制，通过在预填充阶段执行结构感知的键值缓存压缩，显著降低多模态大语言模型推理时的峰值内存使用，同时保持生成性能。


<details>
  <summary>Details</summary>
Motivation: 随着多模态大语言模型扩展到更丰富的视觉表示，推理过程需要存储大量视觉令牌到键值缓存中，导致内存消耗成为主要瓶颈。现有方法通常在处理完所有输入后才进行压缩，导致预填充阶段峰值内存使用过高。

Method: 提出顺序输入压缩机制，利用MLLMs固有的结构规律性和表示冗余性，在预填充过程中执行结构感知的键值缓存压缩，强制实施固定的内存预算。

Result: 该方法显著降低了峰值内存使用，同时仅以最小的性能下降为代价保持了生成性能，实现了更实用和内存高效的多模态推理。

Conclusion: 通过利用MLLMs的结构规律性，在预填充阶段进行顺序输入压缩，可以有效控制内存增长，为多模态推理提供更实用的内存管理解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 动态3D高斯泼溅方法在单目视频训练中表现出训练视角PSNR良好但测试视角泛化能力差的问题，在D-NeRF基准测试中平均训练-测试PSNR差距达6.18dB。研究旨在找出导致这种泛化差距的主要原因。

Method: 1. 系统性地消融自适应密度控制流程（分裂、克隆、修剪、频率、阈值、调度），发现分裂操作是主要问题；2. 提出弹性能量正则化（EER）对每个高斯的变形场施加局部平滑性惩罚；3. 评估GAD（损失率感知的密度阈值）和PTDrop（抖动加权的高斯丢弃）等正则化方法；4. 在不同变形架构和真实单目视频场景中验证方法的有效性。

Result: 1. 分裂操作导致80%以上的泛化差距，禁用分裂可将高斯数量从44K减少到3K，差距从6.18dB降至1.15dB；2. EER将泛化差距减少40.8%，同时高斯云增长85%，平均应变减少99.72%；3. GAD+EER组合将差距减少48%，加上PTDrop和软增长上限达到57%的差距减少；4. 方法在不同变形架构和真实单目视频场景中均有效。

Conclusion: 动态3D高斯泼溅中的过拟合主要由变形场的不一致性而非参数数量驱动。通过弹性能量正则化等一致性约束方法可显著改善泛化性能，这一发现在不同架构和真实场景中均得到验证。
...
TL;DR: TriTS：一种新颖的跨模态解缠框架，将1D时间序列投影到正交的时间、频率和2D视觉空间，通过多模态融合实现长期时间序列预测的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 长期时间序列预测（LTSF）面临重大挑战，因为现实世界信号包含高度纠缠的时间动态，难以从纯1D视角完全捕捉。需要打破这种表示瓶颈。

Method: 1）提出TriTS跨模态解缠框架，将1D时间序列投影到正交的时间、频率和2D视觉空间；2）引入周期感知重塑策略和Visual Mamba（Vim）来弥合1D到2D模态差距，避免ViT的O(N²)计算开销；3）设计多分辨率小波混合（MR-WM）模块，将非平稳信号解耦为趋势和噪声分量；4）保留时间域的流式线性分支以保持数值稳定性；5）动态融合三种互补表示。

Result: 在多个基准数据集上的广泛实验表明，TriTS实现了最先进的性能，通过大幅减少参数数量和推理延迟，从根本上优于现有的基于视觉的预测器。

Conclusion: TriTS通过跨模态解缠和多模态融合，有效解决了长期时间序列预测中的表示瓶颈问题，在保持计算效率的同时实现了优异的预测性能。
...
TL;DR: 提出一个基于少量标注数据（48张照片）的自动化射箭靶面箭孔检测系统，使用自监督视觉变换器与特征上采样技术，在有限训练数据下实现高精度检测和定位。


<details>
  <summary>Details</summary>
Motivation: 传统箭孔检测方法需要大量标注数据，而实际应用中标注数据稀缺且昂贵。本文旨在探索在小数据场景下，利用预训练基础模型进行密集预测的可行性。

Method: 系统包含三个组件：1）基于颜色的规范化校正，将透视畸变照片映射到标准化坐标系；2）冻结的自监督视觉变换器（DINOv3 ViT-L/16）配合AnyUp引导特征上采样，从32×32补丁令牌恢复亚毫米空间精度；3）轻量级CenterNet式检测头进行箭孔中心热图预测。仅3.8M参数可训练。

Result: 在三折交叉验证中，平均F1分数为0.893±0.011，平均定位误差为1.41±0.06mm，优于或媲美需要更多训练数据的全监督方法。在射箭指标上，每张图像平均箭分的中位误差为1.8%，组中心位置的中位误差为4.00mm。

Conclusion: 研究表明，冻结的基础模型配合最小任务特定适配，为小数据场景下的密集预测提供了实用范式，引导特征上采样已能解决补丁令牌化导致的空间精度损失问题。
...
TL;DR: FairNVT是一个轻量级的去偏框架，通过适配器学习任务相关和敏感嵌入，应用校准高斯噪声，结合正交约束和公平性正则化，同时改善表示层和预测层的公平性，同时保持任务准确性。


<details>
  <summary>Details</summary>
Motivation: 现有去偏方法通常单独处理表示层和预测层的公平性，作者认为这两者是内在关联的：在表示层抑制敏感信息可以促进更公平的预测。需要一种统一的轻量级框架来同时改善这两个层面的公平性。

Method: 使用轻量级适配器学习任务相关和敏感嵌入，对敏感嵌入应用校准高斯噪声，然后与任务表示融合。结合正交约束和公平性正则化，共同减少学习嵌入中的敏感属性泄露，并促进更公平的下游预测。

Result: 在涵盖视觉和语言的三个数据集上，FairNVT降低了敏感属性攻击者的准确率，改善了人口统计平等和机会均等指标，同时保持了较高的任务性能。

Conclusion: FairNVT是一个兼容多种预训练Transformer编码器的轻量级去偏框架，能够同时改善表示层和预测层的公平性，同时保持任务准确性，证明了这两个公平性层面的内在联系。
...
<details>
  <summary>Details</summary>
Motivation: 当前MLLM在开放对象识别方面表现良好但细粒度识别不足，而CLIP模型擅长细粒度识别但覆盖范围有限。需要一种能同时处理多粒度语义的识别框架。

Method: 提出HyMOR混合框架：MLLM负责开放和粗粒度对象识别，CLIP模型专门处理动物、植物等特定领域的细粒度识别。这种混合设计实现了跨多个语义粒度的准确对象理解。

Result: 在TBO数据集（20,942张图像，8,816个类别）上的实验显示：HyMOR将细粒度识别与CLIP的差距缩小到0.2%，同时比基线MLLM在通用对象识别上提升2.5%。整体平均SBert相似度提升23.2%。

Conclusion: HyMOR通过整合MLLM和CLIP的优势，实现了准确的多粒度对象感知，为下游多模态内容生成和交互式学习应用提供了强大的感知基础。
...
TL;DR: CDCRec是一种多模态射电干涉数据重建方法，通过显式建模跨域一致性来改善射电天文成像质量，相比现有单模态方法能更好地利用可见性和图像域的互补信息。


<details>
  <summary>Details</summary>
Motivation: 现有射电天文图像重建方法通常局限于单模态域（要么在成像后的脏图像域，要么在成像前的稀疏可见性域），这导致丢失了另一个域的互补上下文信息，无法完整建模相互依赖性和一致性。

Method: 提出CDCRec方法，设计了一个分层多任务多阶段框架，显式建模跨域一致性，增强重建过程中域间相互作用的探索，采用自监督互补建模策略。

Result: 实验结果表明CDCRec通过增强跨域相关性提取改善了成像性能，在干涉域转换方面优于当前方法，特别是在从受限源域数据恢复密集信息方面表现更好。

Conclusion: CDCRec通过多模态跨域一致性建模有效解决了射电干涉数据重建中的信息互补问题，为射电天文成像提供了更完整和准确的重建方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有生成式语义通信接收器依赖最大后验估计，无法保持数据分布，限制了感知质量。同时，当前基于扩散的方法使用单域引导存在局限性：潜在域引导对信道噪声敏感，图像域引导继承解码器偏差。简单结合两域会产生过度自信的伪后验。
...
TL;DR: 本文提出了一种基于频率解耦隐式神经表示（FD-INR）的近红外辅助低光图像恢复方法，通过多尺度小波变换分解频率分量，利用RGB-NIR跨模态频率相关性，实现低光图像的去噪和结构细节恢复。
...
Conclusion: 该方法通过频率域互补优势解决了传统空间域融合的问题，实现了高质量的低光图像恢复，为低光视觉感知提供了更可靠的解决方案。
...
TL;DR: BioLip是一个轻量级唇语同步深度伪造检测框架，通过分析唇部时间抖动来识别伪造视频，不依赖像素级伪影或音视频对应关系，具有跨语言泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有唇语同步深度伪造检测器依赖像素级伪影或音视频对应关系，这些线索编码的是数据依赖模式而非通用物理规律，导致跨语言泛化能力差。作者发现生成模型不强制执行真实口面部发音的生物力学约束，产生可测量的唇部时间方差升高，这为检测提供了更基础的原理。

Method: 提出BioLip框架，基于生物力学原理检测唇部时间抖动。使用MediaPipe提取64个口周关键点坐标，分析这些关键点的时间变化模式，测量时间唇部抖动信号，该信号在说话者语言、种族和录制条件下具有经验一致性。

Result: BioLip框架能够有效检测唇语同步深度伪造视频，通过测量时间唇部抖动这一更基础的生物力学信号，实现了跨语言、跨种族和不同录制条件下的泛化检测能力。

Conclusion: 生成模型缺乏真实口面部发音的生物力学约束，导致产生可测量的时间唇部抖动，这为唇语同步深度伪造检测提供了更基础、更泛化的原理。BioLip框架基于这一原理，通过分析口周关键点的时间变化模式，实现了轻量级且具有跨语言泛化能力的检测。
...
### [44] [Hierarchical Vision Transformer Enhanced by Graph Convolutional Network for Image Classification](https://arxiv.org/abs/2604.16823)
*Haibin Jiao*

Main category: cs.CV

TL;DR: 提出GCN-HViT模型，结合Vision Transformer的全局注意力机制和GCN的局部特征提取能力，通过分层结构解决ViT中补丁大小选择、空间结构信息缺失以及局部结构建模不足的问题。


<details>
  <summary>Details</summary>
Motivation: ViT在图像分类中取得了突破，但存在三个关键限制：1) 补丁大小选择困难；2) 1D位置嵌入无法准确捕捉补丁的空间结构信息；3) 自注意力机制缺乏局部结构建模能力，而GCN缺乏全局图结构信息捕捉能力。

Method: 提出GCN-HViT模型：1) 设计分层ViT，在每层内全局建模补丁间信息交互，跨层建模小补丁与大补丁的层次关系；2) 使用GCN作为局部特征提取器，获取每个图像补丁的局部表示，作为2D空间中的位置嵌入；3) GCN在每层内局部建模补丁间信息交互。

Result: 在3个真实世界数据集上的大量实验表明，GCN-HViT达到了最先进的性能。

Conclusion: GCN-HViT通过结合ViT的全局建模能力和GCN的局部特征提取能力，有效解决了ViT在补丁大小选择、空间结构信息捕捉和局部结构建模方面的局限性，在图像分类任务中表现出色。
...
TL;DR: 提出基于洛伦兹双曲空间的语义分割框架，解决庞加莱球模型数值不稳定问题，实现稳定优化、不确定性估计和层次化表示


<details>
  <summary>Details</summary>
Motivation: 现有基于庞加莱球模型的双曲空间语义分割方法存在数值不稳定、优化困难和计算挑战，需要更稳定高效的解决方案

Method: 提出基于洛伦兹模型的架构无关语义分割框架，利用文本嵌入引导像素级层次表示，无需黎曼优化器，可与现有欧几里得架构集成

Result: 在ADE20K、COCO-Stuff-164k、Pascal-VOC和Cityscapes数据集上验证有效性，支持不确定性估计、置信度图、边界划分、层次检索和零样本性能

Conclusion: 洛伦兹双曲嵌入为鲁棒且具有不确定性感知的语义分割提供了潜力，实现了稳定优化和丰富的辅助功能
...
TL;DR: 提出了TowerDataset传输走廊点云分割数据集和全局-局部融合框架，解决现有数据集类别粗糙、场景短小、缺乏长距离结构依赖的问题


<details>
  <summary>Details</summary>
Motivation: 现有传输走廊点云分割方法面临数据稀缺、难以建模长距离异构场景的全局结构和局部几何细节的问题。公开数据集通常类别粗糙、场景短小，忽略了长距离结构依赖、严重的长尾分布和安全关键组件的细微区别
...
Conclusion: TowerDataset为传输走廊点云分割提供了具有挑战性的基准，提出的全局-局部融合框架能够有效利用全局关系和局部形状细节，在识别罕见和易混淆组件方面表现优异
...
<details>
  <summary>Details</summary>
Motivation: Transformer-based伪装目标检测器虽然精度高，但计算开销大，限制了实际部署。需要一种方法在保持精度的同时显著降低计算复杂度

Method: 提出分层置信度感知token剪枝框架(CATP)：1)分层识别并丢弃易区分的背景和物体内部token，聚焦关键边界token；2)引入双路径特征补偿机制，聚合剪枝token的上下文知识到增强特征中

Result: 在多个COD基准测试上的广泛实验表明，该方法显著降低了计算复杂度，同时保持了高精度

Conclusion: CATP为COD模型在实际场景中的高效部署提供了有前景的研究方向，实现了计算效率与检测精度的良好平衡
...
TL;DR: 本文提出COD-TDQ方法，针对伪装目标检测的4位量化难题，通过令牌分组双约束激活量化解决背景令牌主导激活范围的问题，显著提升量化性能。


<details>
  <summary>Details</summary>
Motivation: 伪装目标检测需要在设备内存和延迟严格限制下进行，低比特推理非常必要。但COD任务量化特别困难，因为背景令牌主导共享激活范围，导致步长膨胀并将弱但结构化的边界线索推入零值箱，形成令牌局部瓶颈。

Method: 提出COD-TDQ方法：1）直接和令牌分组为不同令牌组分配不同尺度以抑制跨令牌范围主导；2）双约束范围投影保持步长-离散比和零值箱质量有界。

Result: 在四个COD基准测试和两个基线模型上，COD-TDQ比现有最佳量化方法Sα分数高出0.12以上，无需重新训练。

Conclusion: COD-TDQ通过解决令牌局部瓶颈，有效实现了伪装目标检测的高效4位量化，显著提升量化性能。
...
TL;DR: Q-DeepSight是一个基于多模态思维链的图像质量评估框架，通过工具增强的证据获取模拟人类证据寻求判断，提供可操作的局部反馈，并应用于图像增强的感知指导。
...
Method: 提出Q-DeepSight框架，采用交错的Multimodal Chain-of-Thought（iMCoT）和工具增强的证据获取（如裁剪和缩放），明确确定质量下降的位置和原因。使用强化学习训练长iMCoT轨迹，引入Perceptual Curriculum Reward（PCR）缓解奖励稀疏性，Evidence Gradient Filtering（EGF）改进视觉基础推理的信度分配。

Result: Q-DeepSight在包括自然图像、修复图像和AI生成内容的多样化基准测试中实现了最先进的性能。通过Perceptual-in-Generation（PiG）框架，Q-DeepSight的诊断指导迭代图像增强，有效连接了评估和优化的闭环。

Conclusion: Q-DeepSight通过模拟人类证据寻求判断过程，提供了更可靠、可操作的图像质量评估，能够有效指导生成模型和图像修复的感知优化，实现了评估与优化的闭环。
...
TL;DR: 论文提出I2P框架，通过感知内在重要性来改进合成图像检测的跨分布泛化能力，自适应识别关键表示层并约束参数更新在低敏感子空间。
...
Method: 提出I2P框架，将视觉基础模型适应重新表述为联合优化问题：既要识别更适合承载伪造判别信息的关键表示层，又要约束任务知识注入对预训练结构的扰动。I2P首先自适应识别对合成图像检测最具判别性的关键层表示，然后将任务驱动的参数更新约束在低敏感参数子空间内。

Result: 该方法在提高任务特异性的同时，尽可能保留预训练表示的可迁移结构，从而改善合成图像检测的跨分布泛化能力。

Conclusion: I2P框架通过内在重要性感知解决了合成图像检测中的任务特定张力，在保持预训练结构可迁移性的同时增强了任务适应性，为跨分布泛化提供了有效解决方案。
...
TL;DR: BiasCareVL是一个针对医疗影像和临床文本的偏置感知多模态学习框架，通过将偏置控制直接融入模型设计而非事后修正，解决了医疗AI系统中的公平性和可靠性问题。


<details>
  <summary>Details</summary>
Motivation: 医疗影像与临床文本结合催生了通用医疗AI系统，但普遍存在的偏置（如疾病流行度不平衡、解剖区域分布偏斜、成像协议异质性和人口统计学差异）对现实临床环境中视觉语言系统的公平性和可靠性构成重大挑战。

Method: 提出BiasCareVL框架，将偏置控制直接融入模型设计，采用自适应不确定性建模和可选的人机协同精炼机制，调节主导数据模式的影响，促进分布不平衡下的公平推理。框架在包含15种成像模态的344万样本上训练，支持视觉问答、疾病分类、分割和报告生成等多样化临床任务。

Result: 在涵盖皮肤科、肿瘤科、放射科和病理科的8个公共基准测试中，BiasCareVL持续优于20种最先进方法，在临床挑战性场景中表现尤为突出：多类皮肤病变诊断准确率提升超过10%，小肿瘤分割Dice系数提升超过20%。与委员会认证放射科医生评估相比，诊断性能超过人类准确率且时间需求大幅减少。

Conclusion: 通过开源BiasCareVL，旨在促进医疗AI的透明、可重复和公平未来，为通用、可信且临床可靠的AI系统铺平道路。该框架将偏置控制从后处理转变为模型设计的核心组成部分，显著提升了医疗多模态AI的公平性和可靠性。
...
TL;DR: CrossFlowDG提出了一种新的域泛化框架，通过跨模态流匹配技术解决图像和文本嵌入之间的模态差距问题，在多个基准测试中取得竞争性性能


<details>
  <summary>Details</summary>
Motivation: 域泛化面临的主要挑战是模型容易过拟合到特定领域的风格特征而非类别语义。现有的多模态方法虽然使用文本表示作为稳定的域不变锚点，但基于余弦相似度的对比对齐方法仍存在模态差距，导致图像和文本嵌入在几何上分离。

Method: 提出CrossFlowDG框架，采用无噪声的跨模态流匹配技术。该方法在联合欧几里得潜在空间中学习连续变换，将具有域偏见的图像嵌入显式地传输到正确类别的域不变文本嵌入。使用高效的VMamba图像编码器和CLIP的文本编码器。

Result: 在四个常见的域泛化基准测试上进行评估，在多个基准测试中取得竞争性性能，在TerraIncognita数据集上达到最先进水平。

Conclusion: CrossFlowDG通过跨模态流匹配有效解决了域泛化中的模态差距问题，为多模态域泛化提供了新的解决方案。
...
TL;DR: EasyVideoR1是一个专门为视频理解任务设计的强化学习框架，通过离线预处理、缓存优化和混合训练范式，显著提升了多模态大语言模型在视频任务上的训练效率和性能。


<details>
  <summary>Details</summary>
Motivation: 随着多模态大语言模型的发展，将强化学习从可验证奖励（RLVR）扩展到视频理解变得日益重要，但由于视频任务类型多样、高维视觉输入处理计算开销大、超参数敏感且难以复现评估等问题，这一领域仍未被充分探索。现有的开源RL训练框架缺乏针对视频模态的系统优化。

Method: 1) 完整的视频RL训练流水线，包含离线预处理和Tensor缓存，消除冗余视频解码；2) 全面的任务感知奖励系统，覆盖11种视频和图像问题类型；3) 混合离线-在线数据训练范式；4) 联合图像-视频训练，支持独立配置像素预算；5) 异步多基准评估框架，覆盖22个主流视频理解基准。

Result: 框架实现了1.47倍的吞吐量提升，奖励系统覆盖多种任务类型，混合训练范式有助于学习更具挑战性的任务，评估框架能够复现与官方报告分数接近的准确率。

Conclusion: EasyVideoR1提供了一个高效、完整的强化学习框架，专门用于训练大型视觉语言模型在视频理解任务上的能力，解决了视频模态RL训练中的关键挑战，为多模态模型的发展提供了重要工具支持。
...
TL;DR: USTri是一个三阶段超声智能管道，用于统一的多器官、多任务分析，通过通用专家训练、特定数据集微调和临床工作流模拟，实现跨异构超声任务的泛化和结构化报告生成。
...
Method: 提出USTri三阶段超声智能管道：阶段I训练通用专家USGen学习跨域可转移先验；阶段II保持USGen冻结，微调数据集特定头部构建USpec专家；阶段III引入USAgent模拟临床工作流，协调USpec专家进行多步推理和确定性结构化报告生成。
...
TL;DR: KIRA是一个统一的视觉RAG框架，通过五阶段架构解决视觉检索增强生成中的核心挑战，在专业领域实现了高精度检索、忠实推理和领域正确性。


<details>
  <summary>Details</summary>
Motivation: 当前基于文本的RAG在视觉领域扩展面临四大挑战：图像查询与文本知识库的模态鸿沟、语义化视觉知识库构建、多跳图像推理以及生成答案的可信视觉证据验证。需要为专业视觉领域开发统一的视觉RAG解决方案。

Method: 提出KIRA五阶段框架：1）基于DINO区域检测的分层语义分块构建多粒度知识库；2）领域自适应对比编码器支持罕见视觉概念；3）双路径跨模态检索与思维链查询扩展；4）支持时序和多视角的链式检索多跳视觉推理；5）证据条件化生成与后验幻觉验证。同时提出DOMAINVQAR评估基准。

Result: 在四个专业领域（医学X光、电路图、卫星图像、组织病理学）的实验中，KIRA平均达到0.97检索精度、1.0可信度分数和0.707领域正确性。消融实验揭示了各组件的作用时机以及精度-多样性权衡的管理需求。

Conclusion: KIRA成功解决了视觉RAG的核心挑战，为专业视觉领域提供了统一的解决方案框架，并通过DOMAINVQAR基准推动了超越标准召回率的全面评估。框架组件间的权衡管理是实际部署的关键考虑因素。
...
TL;DR: CoGR-MoE：一种基于概念引导路由的混合专家框架，通过答案选项语义指导专家选择，提升视觉问答任务的选项推理能力


<details>
  <summary>Details</summary>
Motivation: 现有MoE方法在视觉问答中存在路由不稳定问题：不稳定路由导致相同问题类型下专家选择不一致，而过于稳定的路由又降低灵活性。需要一种更有效的专家选择机制来提升选项推理性能。

Method: 提出概念引导路由框架CoGR-MoE：1）在训练阶段利用答案选项的语义信息指导专家选择；2）使用选项特征对选中的专家进行重加权，为每个候选选项生成区分性表示；3）基于选项级表示进行选项比较，并通过对比学习进行优化。

Result: 实验结果表明，CoGR-MoE在多个VQA任务上表现出强大的性能，证明了该方法的有效性。

Conclusion: CoGR-MoE通过概念引导的路由机制解决了MoE方法在VQA中的路由稳定性问题，利用答案选项语义指导专家选择，生成更具区分性的选项表示，从而提升了视觉问答的选项推理能力。
...
TL;DR: CoDe-MAE提出了一种新的高分辨率光学-SAR多模态预训练方法，通过"更少对齐、更好协同"的哲学，解决了异构分辨率悖论带来的特征抑制或污染问题。


<details>
  <summary>Details</summary>
Motivation: 高分辨率光学和SAR图像预训练面临异构分辨率悖论：空间分辨率越精细，雷达几何结构与光学纹理之间的物理差异越显著。传统的中分辨率刚性对齐范式迁移到高分辨率场景会导致特征抑制或特征污染，最终导致表示退化和负迁移。

Method: 提出CoDe-MAE框架，包含三个核心组件：1) 光学锚定知识蒸馏(OKD)将SAR的斑点噪声映射到纯语义流形；2) 条件对比学习(CCL)通过梯度缓冲机制对齐共享共识，同时安全保留不同的物理特征；3) 跨模态退化重建(CDR)剥离非同源光谱伪特征，捕获真实结构不变性。

Result: 在100万样本上预训练后，CoDe-MAE表现出卓越的数据效率，成功防止了表示退化，在多种单模态和双模态下游任务中建立了新的最先进性能，大幅优于基于更大数据集训练的基础模型。

Conclusion: CoDe-MAE通过创新的"更少对齐、更好协同"方法，有效解决了高分辨率光学-SAR预训练中的异构分辨率悖论，为多模态视觉中的异构表示学习提供了新范式。
...
TL;DR: 本文研究了视网膜疾病进展预测方法，发现训练和推理输入分布对齐比生成模型复杂性更重要，开发了确定性回归模型TRU，在多个数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 目前进行性黄斑病变的临床决策依赖于定性比较或标量进展评分，需要从纵向成像中定量预测未来视网膜外观。现有方法趋向于增加生成复杂性，但这种复杂性对于缓慢进展的视网膜疾病是否必要尚不清楚。

Method: 通过控制实验比较了五种共享同一架构和训练数据集的配置，包括标准条件扩散、推理对齐随机训练和确定性回归。基于研究发现开发了TRU（时间视网膜U-Net），这是一个具有连续时间增量条件和多尺度历史聚合的确定性直接回归模型。

Result: 训练和推理输入分布对齐产生了显著改善（delta-SSIM +0.082，SSIM +0.086，p < 0.001），而对齐框架之间的选择对主要指标无显著影响。TRU在三个成像平台的28,902只眼睛上评估，在delta-SSIM、SSIM和PSNR方面匹配或超过了三个最先进的基准，其优势随着可用历史长度的增加而单调增长。

Conclusion: 对于缓慢进展的视网膜疾病，训练和推理分布对齐比生成模型复杂性更重要。TRU作为一种确定性回归模型，在多个数据集和疾病类型上表现出优越性能，为临床决策提供了可靠的定量预测工具。
...
TL;DR: 提出一个自推理智能体框架，用于生成叙事性产品网格拼贴图，通过产品叙事框架构建、约束感知提示和迭代自我反思来提升视觉一致性和叙事丰富性。


<details>
  <summary>Details</summary>
Motivation: 叙事驱动的产品摄影在现代营销中日益重要，但现有图像生成方法缺乏结构化叙事规划和跨面板协调，导致故事性弱和视觉不一致。产品摄影常以多网格拼贴形式呈现，需要确保跨网格的视觉一致性和整体构图的美学和谐。

Method: 提出自推理智能体框架：1) 基于产品包装图和名称构建产品叙事框架，明确表示产品身份、使用情境和环境；2) 将叙事框架转化为受共享视觉风格约束的互补网格；3) 编译约束感知提示并输入生成模型联合合成拼贴图；4) 评估生成结果的内容有效性和摄影质量，通过显式门控决定是否继续或优化；5) 评估失败时进行故障归因并应用针对性优化，通过迭代自我反思实现渐进改进。

Result: 实验表明，与直接提示基线相比，该框架在美学质量、叙事丰富性和视觉一致性方面持续提升。

Conclusion: 提出的自推理智能体框架能够有效生成具有视觉一致性和叙事丰富性的产品网格拼贴图，通过结构化叙事规划和迭代优化机制解决了现有方法的局限性。
...
TL;DR: DOSE方法利用现成的预训练模型（无需针对目标任务微调）来筛选多模态训练数据，通过联合质量-对齐分布和自适应加权采样，在保持长尾多样性的同时提升数据质量。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型训练数据存在噪声、冗余和对齐不佳的问题，传统数据筛选方法需要额外计算成本（筛选模型通常在相同数据上训练）。需要探索能否使用未见过目标数据的现成预训练模型来筛选训练样本，以降低计算成本。

Method: 使用现成的预训练模型（无需微调）评估文本质量和图文对齐，构建联合质量-对齐分布，应用自适应加权采样来选择信息丰富的样本，同时保持长尾多样性。

Result: 在标准VQA和数学基准测试中，使用DOSE筛选数据训练的模型能够匹配甚至超越使用完整数据集训练的模型，证明了方法的有效性、效率和可扩展性。

Conclusion: DOSE方法展示了现成预训练模型可以有效筛选多模态训练数据，无需任务特定训练，在降低计算成本的同时提升数据质量和多样性，为大规模多模态学习提供了高效的数据筛选方案。
...
TL;DR: URVIS 2026挑战赛报告：首个面向恶劣到极端天气条件的全景分割挑战，基于MUSES多传感器数据集，采用加权全景质量(wPQ)作为评估指标


<details>
  <summary>Details</summary>
Motivation: 推动在恶劣到极端天气条件下稳健的多模态全景分割技术发展，解决现有方法在复杂天气环境下的性能瓶颈问题

Method: 基于MUSES多传感器数据集（RGB相机、LiDAR、雷达、事件相机），设计加权全景质量(wPQ)作为公平评估指标，组织挑战赛吸引17个团队参与并提交47份方案

Result: 挑战赛成功吸引了17个注册参与者和47份提交，4个团队进入最终阶段；建立了首个恶劣天气全景分割基准，并展示了当前多模态方法在极端天气条件下的性能表现

Conclusion: 该挑战赛为恶劣天气下的全景分割建立了重要基准，推动了多模态感知技术的发展，但仍存在许多未解决的挑战，需要进一步研究更稳健的算法
...
TL;DR: 提出ICNNM方法，通过预学习卷积特征向量避免多次SVD计算，显著降低计算成本并提升张量补全性能


<details>
  <summary>Details</summary>
Motivation: 现有CNNM方法在解决任意采样张量补全问题时需要多次进行SVD分解，计算成本高且难以并行化，需要更高效的优化方法

Method: 从卷积特征向量角度重新表述CNNM优化目标，引入预学习的卷积特征向量（在不同张量间共享），提出ICNNM方法绕过SVD步骤

Result: ICNNM显著减少计算时间，同时由于预学习卷积特征向量提供的额外先验知识，在恢复性能上也优于CNNM；在视频补全、预测和帧插值实验中表现出优越性

Conclusion: ICNNM通过预学习卷积特征向量有效解决了CNNM计算效率低的问题，在保持甚至提升性能的同时大幅降低计算成本，为张量补全问题提供了更实用的解决方案
...
TL;DR: TeMuDance是一个无需音乐-文本-动作三元数据集的文本控制音乐驱动舞蹈生成框架，通过运动作为语义锚点对齐不同模态，实现细粒度语义指导。


<details>
  <summary>Details</summary>
Motivation: 现有音乐驱动舞蹈生成方法缺乏语义可控性，难以通过自然语言描述指导特定动作，主要原因是缺乏大规模对齐的音乐-文本-动作三元数据集。

Method: 提出运动中心桥接范式，利用运动作为共享语义锚点在统一嵌入空间中对齐不相关的音乐-舞蹈和文本-动作数据集；在冻结的音乐到舞蹈扩散骨干上训练轻量级文本控制分支；设计双流微调策略和基于置信度的过滤来抑制检索监督中的噪声。

Result: TeMuDance在保持舞蹈质量的同时，显著提升了文本条件控制能力，优于现有方法。

Conclusion: 该框架无需手动标注的三元数据集即可实现文本控制的音乐驱动舞蹈生成，通过运动桥接和噪声抑制策略有效解决了跨模态对齐挑战。
...
<details>
  <summary>Details</summary>
Motivation: 随着机器人技术的发展，基于骨架的动作识别变得越来越重要，因为人机交互需要理解人类和人形机器人的动作。由于人类骨架来源不同和人形机器人结构差异，骨架数据天然存在异质性，但先前工作忽视了这种异质性，仅使用同质骨架构建模型。此外，开放词汇动作识别对实际应用也至关重要。

Method: 提出了一个基于Transformer的模型，包含三个关键组件：1) 统一骨架表示；2) 用于骨架的运动编码器，将多模态骨架嵌入输入到双流Transformer编码器中学习时空动作表示；3) 多粒度运动-文本对齐，在三个层次上结合对比学习：全局实例对齐、流特定对齐和细粒度对齐。

Result: 在包含异构骨架数据的流行基准测试上进行了大量实验，证明了所提方法的有效性和泛化能力。

Conclusion: 该研究解决了异构骨架开放词汇动作识别这一具有挑战性的问题，通过构建大规模异构开放词汇骨架数据集和提出创新的Transformer模型，为实际应用中的人机交互提供了有效的解决方案。
...
TL;DR: LIVE是一个联合训练框架，利用大规模高质量图像编辑数据和视频数据集来增强视频编辑能力，通过帧级token噪声策略和两阶段训练解决图像与视频域差异问题。
...
Conclusion: LIVE框架通过有效利用图像编辑数据和创新的帧级token噪声策略，成功解决了视频编辑数据稀缺的问题，在视频编辑任务上取得了优异性能，源代码将公开提供。
...
### [66] [CAM3DNet: Comprehensively mining the multi-scale features for 3D Object Detection with Multi-View Cameras](https://arxiv.org/abs/2604.17024)
...
<details>
  <summary>Details</summary>
Motivation: 基于查询的多视图3D目标检测方法在有效利用动态多尺度信息方面存在困难，对象特征与查询几何关系学习不足，直接探索多尺度时空特征成本过高。
...
Result: 在nuScenes、Waymo和Argoverse基准数据集上的广泛实验表明CAM3DNet的有效性，超越了大多数现有的基于相机的3D目标检测方法。消融研究验证了各模块的独立效果及其空间和计算复杂度。

Conclusion: CAM3DNet通过创新的复合查询、自适应自注意力和多尺度混合采样模块，成功解决了多视图3D目标检测中动态多尺度信息利用的挑战，在多个基准数据集上取得了优越性能。
...
TL;DR: 本研究提出了一种可解释的多模态混合专家模型（IMA-MoE），用于整合神经影像、行为、激素和人口统计学数据，以发现暴食症的生物标志物，并在ABCD数据集上验证了其优越性能。


<details>
  <summary>Details</summary>
Motivation: 暴食症是最普遍的进食障碍，但当前诊断框架主要基于症状而非生物学机制，限制了早期检测和生物信息干预的发展。现有神经生物学研究因依赖假设驱动模型、单模态分析和有限数据多样性而难以推广。

Method: 提出可解释的多模态混合专家模型（IMA-MoE），将异质神经影像、行为、激素和人口统计学测量编码为不同标记，灵活建模跨模态依赖关系同时保留模态特异性特征，并引入标记重要性机制增强可解释性。

Result: 在大型ABCD数据集上评估，IMA-MoE在区分暴食症与健康对照方面优于基线方法，并揭示了性别特异性预测模式，其中激素测量在女性预测中贡献更显著。
...
<details>
  <summary>Details</summary>
Motivation: 神经退行性疾病具有多因素特性，需要多模态建模，但实际临床工作中常出现模态数据不完整的问题。现有方法依赖群体或静态先验，难以捕捉个体特异性跨模态依赖关系，且缺乏对驱动决策的证据来源的可解释性。

Method: 提出条件证据重建与分解（CERD）框架：1）基于每个受试者的观测输入条件重建缺失模态表示；2）通过logit级归因将诊断证据分解为共享的跨模态佐证和模态特异性线索。

Result: 在阿尔茨海默病神经影像学倡议（ADNI）数据集上的实验表明，CERD在不完整模态设置下优于竞争基线方法，同时产生结构化且与临床对齐的证据归因，为可信决策提供支持。

Conclusion: CERD框架有效解决了多模态诊断中的模态缺失问题，不仅提升了诊断性能，还提供了可解释的证据分解，增强了临床决策的可信度。
...
TL;DR: 提出SIF框架，通过语义对齐指纹蒸馏和鲁棒指纹优化，为大视觉语言模型提供隐蔽且鲁棒的版权保护方案，解决现有指纹方法易被检测和移除的问题。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型的公开可访问性引发了未经授权模型重用和知识产权侵权的严重担忧。现有所有权验证方法通常依赖语义异常查询或分布外响应作为指纹，这些指纹容易被对手检测和移除。

Method: 提出SIF框架：1) 语义对齐指纹蒸馏(SAFD)：将文本水印信号转移到视觉模态，生成语义一致但带有指纹的响应；2) 鲁棒指纹优化(RFO)：通过模拟最坏情况表示扰动增强鲁棒性，使指纹能够抵抗微调和量化等模型修改。

Result: 在LLaVA-1.5和Qwen2.5-VL上的广泛实验表明，SIF实现了强大的隐蔽性和鲁棒性，为LVLM版权保护提供了实用解决方案。

Conclusion: SIF框架通过语义对齐指纹蒸馏和鲁棒指纹优化，为大视觉语言模型提供了非侵入式、隐蔽且鲁棒的所有权验证方案，有效解决了现有指纹方法易被检测和移除的脆弱性问题。
...
TL;DR: 开发了一个基于鱼眼相机的实时自行车-行人碰撞预警系统原型，能在边缘设备上以30fps运行，通过声光警报提供平均3.3秒的预警时间。


<details>
  <summary>Details</summary>
Motivation: 城市交叉口自行车与行人的碰撞事故持续造成伤害，但现有系统很少使用商用硬件为无装备的道路使用者提供实时预警。

Method: 1) 开发超广角鱼眼镜头标定流程，通过透视重映射和直接光束法平差解决角点检测失败和优化器发散问题；2) 结合鱼眼感知目标检测和通过预计算查找表的闭式地面投影；3) 引入设计时一致性仿真，包含24个脚本危险场景、随机尺寸感知检测失败和延迟扫描；4) 将决策层形式化为可分离、可审计的测试平台，具有明确的部署门控、可争议机制和残余风险登记。

Result: 在鱼眼定位误差的一致性测试中，所选管道配置达到93.3%的灵敏度和92.3%的特异性，平均预警预算为3.3秒。系统设计基于社区辅助设计研讨会。

Conclusion: 该系统原型展示了使用商用硬件为无装备道路使用者提供实时碰撞预警的可行性，通过系统化的设计方法和社区参与确保了实用性和安全性。
...
TL;DR: OASIS是一个用于流式视频推理的新框架，通过结构化按需检索来解决长视频历史中的信息冗余问题，实现高精度、低延迟的推理。


<details>
  <summary>Details</summary>
Motivation: 流式视频推理面临历史无限增长但关键信息稀缺的挑战，传统方法要么扩大内存（增加冗余），要么过度压缩（丢失关键信息）。真正的困难在于知道在哪里寻找信息，而不是记住多少信息。

Method: OASIS将流式历史组织为分层事件，采用控制式精化推理：先进行短上下文推理，只有当不确定性出现时才进行基于语义的检索。检索由高层意图驱动而非嵌入相似性，确保检索更准确、噪声更少。

Result: 在多个基准测试和不同流式MLLM骨干上的实验表明，OASIS在长时域准确性和组合推理方面取得了显著提升，同时保持有界的token成本和低请求延迟。

Conclusion: OASIS提供了一种即插即用、无需训练的流式视频推理框架，通过结构化按需检索有效解决了长视频历史中的信息冗余问题，显著提升了推理性能。
...
TL;DR: 提出无需训练的指令引导多模态嵌入框架，利用MLLM将文本、栅格图像和SVG代码映射到对齐的嵌入空间，通过模态特定指令和SVG结构提示控制嵌入方向，无需学习投影头或对比训练。


<details>
  <summary>Details</summary>
Motivation: SVG既作为视觉图像又作为结构化代码，但大多数方法将其栅格化并丢弃符号组织；同时现有文本嵌入方法难以扩展到视觉或结构化模态。

Method: 包含两个关键组件：1) 多模态显式单字限制(mEOL)，指导MLLM将任何多模态输入总结为单个token，其隐藏状态作为紧凑语义嵌入；2) 语义SVG重写模块，通过视觉推理为SVG元素分配有意义的标识符并简化嵌套结构。

Result: 在首个文本到SVG检索基准VGBench上，无需训练的嵌入方法优于基于编码器和基于训练的多模态基线方法。

Conclusion: 提示级控制是参数级训练的有效替代方案，可用于结构感知的多模态检索，展示了通过指令引导而非参数训练实现多模态对齐的可行性。
...
TL;DR: BasketHAR是一个专门针对篮球训练的多模态人类活动识别数据集，包含专业级篮球动作的运动数据、生理数据和同步视频，旨在解决现有数据集在专业体育场景中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的人类活动识别数据集主要关注基本日常活动（如行走、站立、爬楼梯），在专业体育场景如篮球训练分析中适用性有限。需要专门针对篮球训练的多模态数据集来支持高级HAR任务和体育分析研究。

Method: 创建了BasketHAR数据集，包含来自惯性测量单元（加速度计和陀螺仪）、角速度、磁场、心率、皮肤温度的综合运动数据，以及同步视频记录。数据集涵盖多样化的专业级篮球动作，并提供了基准多模态对齐方法。

Result: 实验结果表明该数据集具有复杂性，适合高级HAR任务。数据集已公开在Hugging Face平台，采用Apache License 2.0许可，为篮球训练分析和专业性能报告生成提供了宝贵资源。

Conclusion: BasketHAR填补了专业体育场景中HAR数据集的空白，为未来的人类活动识别和体育分析研究提供了有价值的资源，特别是在篮球训练分析和专业性能报告生成方面具有应用潜力。
...
TL;DR: NTIRE 2026 RipDetSeg挑战赛聚焦于图像中裂流的自动检测与分割，旨在解决这一全球海滩安全难题。挑战基于RipVIS基准，吸引了159名参与者，最终产生9个有效测试提交。


<details>
  <summary>Details</summary>
Motivation: 裂流是危险的海岸流，导致全球许多海滩相关死亡事故。由于其视觉外观在不同海滩、视角和海况下差异很大，难以识别，因此需要开发自动检测和分割方法来解决这一安全关键问题。

Method: 挑战赛基于RipVIS基准数据集，该数据集来自10多个国家，包含4种相机方向和多样化的海滩与海况。评估采用综合评分，结合F1[50]、F2[50]、F1[40:95]和F2[40:95]四个指标。参与者主要使用预训练模型，结合强数据增强和后处理设计。

Result: 挑战赛吸引了159名注册参与者，在两个任务中产生了9个有效的测试提交。最终排名基于综合评分。结果表明，裂流理解受益于稳健的通用视觉模型的进展，但仍有很大改进空间。

Conclusion: 裂流检测和分割是一个具有挑战性的安全关键问题。当前方法主要依赖预训练模型，结合数据增强和后处理。未来需要开发专门针对裂流独特视觉结构的方法，以进一步提高性能。
...
TL;DR: 本文提出RefVQA方法，从视频间关系角度重新审视AIGC视频质量评估，将其构建为参考感知的评估问题，通过构建查询中心的参考图来组织语义相关样本，实现基于参考的视频质量评估。


<details>
  <summary>Details</summary>
Motivation: 随着生成模型的快速发展，AI生成视频数量激增，自动质量评估变得日益重要。现有AIGC-VQA方法通常独立分析每个视频，忽略了视频之间的潜在关系。本文从视频间关系角度重新审视AIGC-VQA，将其构建为参考感知的评估问题，认为质量评估不仅应考虑视频内在特征，还应通过与相关视频的比较来指导，这更符合人类感知。

Method: 提出Reference-aware Video Quality Assessment (RefVQA)方法，利用查询中心的参考图来组织语义相关样本，执行从参考节点到查询节点的图引导差异聚合。该方法通过构建参考图来捕捉视频间关系，实现基于参考的质量评估。

Result: 在现有数据集上的实验表明，RefVQA在多个质量维度上优于最先进的方法，通过跨数据集评估验证了其强大的泛化能力。这些结果突出了所提出的基于参考的公式的有效性。

Conclusion: 提出的基于参考的AIGC-VQA方法通过考虑视频间关系，实现了更符合人类感知的质量评估，具有优越的性能和泛化能力，有潜力推动AIGC视频质量评估领域的发展。
...
TL;DR: EvoComp是一个视觉token压缩框架，通过轻量级编码器选择信息最丰富且非冗余的视觉token，在保持任务准确性的同时显著减少token数量，实现3倍压缩下保持99.3%原始准确率。
...
Method: 提出轻量级编码器压缩器，联合考虑视觉和文本上下文选择信息最丰富的token；采用进化标注策略搜索最小化MLLM输出损失的token子集；使用GHM损失和余弦相似度正则化训练压缩器。

Result: 在多个视觉语言基准测试中优于基于注意力或相似性启发式的方法，在3倍token压缩下保持99.3%原始准确率，在移动设备上实现最高1.6倍加速。

Conclusion: EvoComp通过有效的token压缩框架，在保持多模态大语言模型准确性的同时显著提升推理效率，为实际部署提供了可行的解决方案。
...
TL;DR: CoAMD是一个基于坐标的自回归运动扩散模型，通过骨架坐标统一处理动作识别和运动生成，在四个任务上实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常将动作识别和运动生成作为两个独立问题处理，未能揭示它们之间的内在联系——运动生成需要语义理解。本研究旨在通过骨架坐标统一这两个任务，实现更有效的人体运动建模。

Method: 提出CoAMD（基于坐标的自回归运动扩散）模型，采用从粗到细的方式合成运动。核心是多模态动作识别器（MAR），为运动生成提供基于梯度的语义指导。模型使用绝对坐标进行评估，可应用于骨架动作识别、文本到运动生成、文本-运动检索和运动编辑四个任务。

Result: 在13个基准测试上的广泛实验表明，该方法在四个任务上都达到了最先进的性能，证明了其在人体运动建模方面的有效性和多功能性。

Conclusion: CoAMD成功统一了动作识别和运动生成，通过多模态动作识别器的语义指导实现了高质量的运动生成，为人体运动建模提供了一个强大而通用的框架。
...
TL;DR: 该研究提出了一种名为HBR-Net-18的两阶段AI框架，用于前列腺癌的MRI检测，通过Hadamard U-Net抑制强度不均匀性，再通过ResNet-18进行斑块级分类，显著优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 虽然混合多维MRI增强了前列腺癌的诊断能力，但仍需要稳健、自动化的AI检测方法，以改善诊断准确性和临床部署潜力。

Method: 提出HBR-Net-18两阶段框架：第一阶段使用Hadamard U-Net抑制物理信息自编码器生成的六个参数HM-MRI图的强度不均匀性；第二阶段使用ResNet-18进行11×11重叠斑块的分类，结合2D层内和3D层间信息提高空间一致性。

Result: 实验结果表明HB-Net实现了平衡的敏感性和特异性，显著优于传统的基于放射组学的方法和基线CNN模型。

Conclusion: 该研究提出的AI框架在前列腺癌检测方面表现出色，具有临床部署的潜力，为MRI诊断提供了有效的自动化解决方案。
...
### [79] [Inference-Time Temporal Probability Smoothing for Stable Video Segmentation with SAM2 under Weak Prompts](https://arxiv.org/abs/2604.17115)
*Dawar Jyoti Deka*
...
<details>
  <summary>Details</summary>
Motivation: SAM2等交互式视频分割模型在弱用户监督下（如单帧稀疏点提示）存在时间不稳定问题，包括边界闪烁、物体丢失和跨帧物体范围不一致，限制了其在视频理解和控制应用中的可靠性。

Method: 提出推理时的时间概率平滑方法，直接在每帧分割概率图上操作，利用基于光流的运动扭曲、分割熵导出的像素级不确定性估计以及前后向流一致性，自适应地融合当前帧预测与运动对齐的历史估计。

Result: 在四个不同视频序列上评估，使用包括运动补偿IoU、边界一致性、物体持续性和面积波动在内的综合指标，实验结果显示在保持空间精度的同时，时间稳定性相比原始SAM2推理有持续改进。

Conclusion: 提出的框架轻量级、模型无关，适用于实时交互式视频分割，能有效提高弱提示下SAM2视频分割的时间稳定性。
...
### [80] [Multimodal Fusion of Histopathology Images and Electronic Health Records for Early Breast Cancer Diagnosis](https://arxiv.org/abs/2604.17122)
*Aditya Shribhagwan Khandelwal,Mohammad Samar Ansari,Asra Aslam*

Main category: cs.CV

TL;DR: 该研究提出了一种多模态框架，整合乳腺癌病理图像特征与临床数据，通过中间融合模型在诊断性能上超越单模态基线，特别在关键但类别不平衡的有丝分裂类别上表现优异。


<details>
  <summary>Details</summary>
Motivation: 乳腺癌是全球癌症相关死亡的主要原因，及时准确诊断对改善生存结果至关重要。现有研究通常将病理图像和临床数据两种模态孤立处理，缺乏有效的多模态整合方法。

Method: 提出系统多模态框架，整合BreCaHAD数据集的病理图像特征和MIMIC-IV的结构化临床数据。训练评估单模态图像模型（简单CNN基线和ResNet-18迁移学习）、单模态表格模型（XGBoost和多层感知机），以及将两种模态潜在表示拼接的中间融合模型。

Result: ResNet-18在三类病理图像分类中达到近乎完美的准确率（1.000）和AUC（1.000），XGBoost在EHR预测任务中达到98%准确率。中间融合模型获得0.997的宏观平均AUC，超越所有单模态基线，在关键的有丝分裂类别上表现最佳（AUC 0.994）。

Conclusion: 多模态整合在预测性能和临床透明度方面均带来显著改进，Grad-CAM和SHAP可解释性分析验证了模型决策与既定病理和临床标准的一致性。
...
TL;DR: 研究发现视觉语言模型在语义等价描述下会产生不一致的对象定位结果，提示词微小变化会导致选择不同实例，这种不稳定性主要源于argmax选择机制而非文本嵌入距离


<details>
  <summary>Details</summary>
Motivation: 研究视觉语言模型在开放词汇对象定位中的隐含假设：语义等价的描述应该产生一致的输出结果。验证这一假设是否成立，并探究不一致性的原因。

Method: 使用受控流程结合DETR生成对象提议和CLIP进行语言条件选择，在263张COCO val2017图像上测试。分析重叠提示词（如"一个人"、"一个人类"、"一个行人"）的选择一致性，使用PCA分析变异性结构，并研究文本嵌入距离与定位分歧的关系。

Result: 语义等价提示词经常选择不同实例，六个提示词的平均不稳定性为2.11个不同选择。PCA分析显示这种变异性是结构化且有方向的，而非随机。提示词集成不能提高质量，反而使选择偏向通用区域。文本嵌入接近度只能解释34%的定位分歧（r = -0.58），表明不稳定性主要源于argmax选择机制。

Conclusion: 视觉语言模型在开放词汇对象定位中存在显著的不稳定性问题，语义等价描述会产生不一致的输出。这种不一致性主要源于模型的选择机制而非文本表示差异，这对模型的可靠性和实际应用提出了重要挑战。
...
TL;DR: OptiMVMap通过最优车辆选择和噪声过滤，解决多车协同建图中视角不足、计算冗余和噪声问题，显著提升离线向量化地图精度


<details>
  <summary>Details</summary>
Motivation: 现有单车辆建图方法存在视角不足问题，无法有效揭示被遮挡区域。多车融合面临计算成本高、视角冗余和噪声三大挑战

Method: 采用"先选择后融合"框架：1) 最优车辆选择模块识别能最大减少遮挡区域不确定性的辅助车辆；2) 跨车辆注意力机制进行姿态容忍对齐；3) 语义感知噪声过滤器抑制伪影

Result: 在nuScenes和Argoverse2数据集上，相比MapTRv2分别提升10.5 mAP和9.3 mAP，相比MVMap和HRMapNet在nuScenes上提升6.2 mAP和3.8 mAP

Conclusion: 不确定性引导的辅助车辆选择对于高效准确的多车向量化建图至关重要，OptiMVMap能以更少视角生成更完整、拓扑忠实的地图
...
<details>
  <summary>Details</summary>
Motivation: 现有的驾驶场景生成方法缺乏细粒度的多模态控制能力，无法根据文本或图像输入精确控制道路布局和交通条件，同时保持场景的真实性和时间一致性。

Method: 提出跨全局控制机制，将交叉注意力与轻量级全局上下文分支相结合，在向量化潜在空间中联合表示道路结构和动态参与者，实现细粒度控制同时保持真实性。

Result: 实验验证表明，ScenarioControl在控制遵从性和保真度方面优于所有测试方法，能够生成时间一致的场景推演，支持从不同参与者视角的长时程场景延续。

Conclusion: ScenarioControl首次实现了对学习驾驶场景生成的多模态控制，为自动驾驶系统的开发和测试提供了强大的场景生成工具，并发布了带文本标注的数据集支持训练和评估。
...
TL;DR: 该论文提出了一种将2D图像信息（颜色、神经特征、分割掩码）高效映射回现有3D高斯泼溅场景的方法，解决了视图相关着色和遮挡处理等挑战，在场景重光照、特征丰富和3D语义分割任务上实现了比基于梯度下降基线快一个数量级的速度提升。


<details>
  <summary>Details</summary>
Motivation: 虽然当前的光栅化器能够高效地将3D高斯泼溅映射到2D相机视图，但将2D图像信息（如颜色、神经特征或分割掩码）高效地映射回现有的高斯泼溅场景这一"相反"方向存在挑战，如视图相关着色和遮挡处理，而这一方向对于场景重光照、风格化和3D语义分割等应用至关重要。

Method: 使用正规方程解决每个高斯的可见性加权最小二乘问题，并利用现有的可微分光栅化器高效实现该方法。

Result: 在场景重光照、特征丰富和3D语义分割任务上验证了方法的有效性，相比基于梯度下降的基线方法实现了高达一个数量级的速度提升。

Conclusion: 该方法成功解决了将2D图像信息高效映射回3D高斯泼溅场景的挑战，为场景重光照、风格化和3D语义分割等应用提供了高效的解决方案。
...
### [85] [DreamShot: Personalized Storyboard Synthesis with Video Diffusion Prior](https://arxiv.org/abs/2604.17195)
*Junjia Huang,Binbin Yang,Pengxiang Yan,Jiyang Liu,Bin Xia,Zhao Wang,Yitong Wang,Liang Lin,Guanbin Li*

Main category: cs.CV

TL;DR: DreamShot是一个基于视频生成模型的故事情节板框架，利用视频扩散先验实现可控的多镜头合成，解决了现有文本到图像模型在时间一致性、角色身份保持和叙事连贯性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有故事情节板合成方法大多基于文本到图像扩散模型，这些模型难以保持长距离时间一致性、一致的角色身份以及跨多个镜头的叙事连贯性。需要一种能够生成视觉和语义连贯序列的解决方案。

Method: DreamShot基于视频生成模型，支持文本到镜头和参考到镜头生成，以及基于先前帧的故事延续。采用多参考角色条件模块，接受多个角色参考图像，并通过角色注意力一致性损失强制身份对齐，明确约束参考角色和生成角色之间的注意力。

Result: 实验表明，DreamShot在场景连贯性、角色一致性和生成效率方面优于最先进的文本到图像故事情节板模型，实现了更好的叙事保真度和角色连续性。

Conclusion: DreamShot为可控视频模型驱动的视觉叙事建立了新方向，通过充分利用视频生成模型的空间-时间一致性，能够生成视觉和语义连贯的故事情节板序列。
...
### [86] [SciDraw-6K: A Multilingual Scientific Illustration Dataset Generated by Google Gemini](https://arxiv.org/abs/2604.17206)
*Davie Chen*

Main category: cs.CV

TL;DR: SciDraw-6K是一个包含6,291张科学插图的精选数据集，由Google Gemini图像生成模型合成，每张图都配有11种语言的提示词，涵盖8个科学类别，专门用于科学插图类型的研究和应用。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像研究主要基于通用数据集，缺乏专门针对科学插图（如示意图、机制图、目录图、概念海报）的多语言数据集。SciDraw-6K旨在填补这一空白，支持多语言文本到图像研究、领域适应的扩散模型微调以及科学可视化的提示工程研究。

Method: 使用Google Gemini图像生成模型（主要是gemini-2.5-flash-image和gemini-3-pro-image-preview模型系列）合成6,291张科学插图，每张图都配有11种语言的提示词。图像涵盖8个科学类别：生物医学、化学、材料、电子、环境、AI系统、物理和其他。数据集构建管道包括图像生成、多语言提示配对和分类标注。

Result: 创建了SciDraw-6K数据集，包含6,291张科学插图，每张图都配有11种语言的提示词。该数据集已公开发布，并作为sci-draw.com公共科学绘图服务的基础。数据集支持多语言文本到图像研究、领域适应的扩散模型微调和科学可视化的提示工程研究。

Conclusion: SciDraw-6K是一个专门针对科学插图类型的多语言数据集，填补了现有研究的空白。该数据集可用于推动多语言文本到图像研究、科学领域的扩散模型微调以及科学可视化的提示工程研究，并通过sci-draw.com提供公共服务。
...
### [87] [DREAM: Dynamic Retinal Enhancement with Adaptive Multi-modal Fusion for Expert Precision Medical Report Generation](https://arxiv.org/abs/2604.17209)
*Nagur Shareef Shaik,Teja Krishna Cherukuri,Dong Hye Ye*

Main category: cs.CV

TL;DR: DREAM框架通过两阶段融合机制和对比对齐，在数据稀缺的视网膜图像医疗报告生成中实现高保真度，在DeepEyeNet基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前大型视觉语言模型在医学等专业领域面临数据稀缺问题，容易过拟合并忽略关键病理特征，需要结合医学专业知识提升报告生成质量。
...
Conclusion: DREAM通过结合医学专业知识和高效融合策略，解决了数据稀缺环境下视网膜图像医疗报告生成的挑战，为专业医学领域的视觉语言模型提供了有效解决方案。
...
### [88] [EmbodiedHead: Real-Time Listening and Speaking Avatar for Conversational Agents](https://arxiv.org/abs/2604.17211)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法无法同时实现实时生成、统一的听-说行为和高质量渲染，且传统双流音频方法存在前瞻依赖，不适用于因果用户-LLM交互

Method: 采用首个Rectified-Flow Diffusion Transformer (DiT)结合可微分渲染器；使用单流接口配合显式的每帧听-说状态条件化和流式音频调度器；采用两阶段训练方案（系数空间预训练和联合图像域精炼）

Result: 在仅4个采样步骤中实现多样化、高保真生成；在听和说场景中都达到最先进的视觉质量和运动保真度；有效抑制听时的虚假嘴部运动，实现无缝轮流对话

Conclusion: EmbodiedHead框架成功解决了实时说话头生成的关键挑战，为LLM提供了高质量的视觉化身，实现了自然的听-说交互
...
### [89] [Cross-Modal Attention Analysis and Optimization in Vision-Language Models: A Study on Visual Reliability](https://arxiv.org/abs/2604.17217)
*Lijie Zhou*

Main category: cs.CV

TL;DR: 该研究提出对抗性评估框架量化视觉语言模型中的文本捷径学习问题，通过几何形状数据集测试发现优化后的LoRA模型将跨模态依赖度从27.5%降低到9.8%，相对改进64.4%


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在跨模态任务中表现出色，但存在过度依赖文本描述而忽视视觉证据的问题，即"文本捷径学习"。研究者希望量化这种跨模态依赖程度，并开发方法减少模型对文本的过度依赖。

Method: 提出对抗性评估框架，通过将语义冲突的文本与未改变的图像配对来测量准确率下降。使用四种对抗策略：形状交换、颜色交换、位置交换和随机文本。在几何形状数据集上比较三种配置：基线CLIP模型、LoRA微调模型和LoRA优化模型（集成硬负样本挖掘、标签平滑、分层学习率、余弦重启、课程学习和数据增强）。

Result: 优化模型将平均准确率下降从27.5%降低到9.8%，相对改进64.4%（p<0.001），同时保持97%的正常准确率。注意力可视化和嵌入空间分析证实优化模型更多关注视觉特征，实现更紧密的跨模态对齐。

Conclusion: 对抗性评估框架有效量化了视觉语言模型中的文本捷径学习问题，通过集成多种优化技术的LoRA模型显著减少了模型对文本的过度依赖，提高了对视觉特征的关注度，实现了更好的跨模态对齐。
...
### [90] [Region-Affinity Attention for Whole-Slide Breast Cancer Classification in Deep Ultraviolet Imaging](https://arxiv.org/abs/2604.17222)
*Nagur Shareef Shaik,Teja Krishna Cherukuri,Dong Hye Ye*

Main category: cs.CV

TL;DR: 本文提出了一种针对DUV-WSI乳腺癌分类的区域亲和力注意力机制，无需切片处理完整玻片，通过建模局部邻域距离和构建全亲和矩阵来动态突出诊断相关区域，在136个样本上取得了92.67%的准确率和95.97%的AUC。


<details>
  <summary>Details</summary>
Motivation: 乳腺癌诊断需要快速精确的工具，但传统组织病理学方法在术中应用有限。DUV荧光成像提供了高对比度、无标记的完整玻片可视化，但现有的深度学习分类方法多为基于切片的，破坏了空间上下文且预处理开销大。标准注意力机制未能充分利用DUV-WSI数据中丰富的多尺度区域关系。

Method: 提出区域亲和力注意力机制，无需切片处理完整DUV-WSI玻片。通过建模局部邻域距离和构建全亲和矩阵，动态突出诊断相关区域。使用对比损失增强特征可区分性，保持空间完整性。

Result: 在136个DUV-WSI样本数据集上评估，该方法达到92.67 ± 0.73%的准确率和95.97%的AUC，优于现有注意力方法。

Conclusion: 区域亲和力注意力机制为DUV-WSI乳腺癌分类提供了一种有效方法，无需切片处理完整玻片，通过建模区域关系动态突出诊断相关特征，在准确率和AUC方面均优于现有方法。
...
### [91] [Enhancing Zero-shot Personalized Image Aesthetics Assessment with Profile-aware Multimodal LLM](https://arxiv.org/abs/2604.17233)
*Chun Wang,Chenfeng Wei,Chenyang Liu,Weihong Deng*

Main category: cs.CV

TL;DR: P-MLLM：基于用户画像的多模态大语言模型，用于零样本个性化图像美学评估
...
Method: 提出P-MLLM模型，采用基于用户画像的个性化范式。在冻结的大语言模型基础上，通过选择性融合模块实现受控的视觉信息整合。这些模块在基于用户画像的推理过程中，有选择地将视觉信息整合到模型的隐藏状态中，实现画像感知的视觉信息融合。

Result: 在最新的PIAA基准测试中，P-MLLM实现了具有竞争力的零样本性能，即使在粗糙的用户画像信息下仍保持有效，证明了基于用户画像的个性化方法在零样本PIAA中的潜力。

Conclusion: 基于用户画像的个性化范式是解决零样本个性化图像美学评估的有效方法，P-MLLM模型通过选择性视觉融合机制实现了画像感知的个性化评估。
...
### [92] [RemoteShield: Enable Robust Multimodal Large Language Models for Earth Observation](https://arxiv.org/abs/2604.17243)
*Rui Min,Liang Yao,Shiyu Miao,Shengxiang Xu,Yuxuan Liu,Chuanyi Zhang,Shimin Di,Fan Liu*

Main category: cs.CV

TL;DR: 本文针对遥感多模态大语言模型在现实噪声条件下性能下降的问题，提出了RemoteShield模型，通过语义等价簇和偏好学习提升模型在视觉退化与文本噪声下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前遥感多模态大语言模型在精心整理的干净数据集上训练，学习到的映射关系脆弱，无法泛化到实际地球观测中的噪声条件，导致在部署时面对不完美输入时性能显著下降。

Method: 提出RemoteShield模型，构建包含视觉退化（云、雾等）和文本变异（口语化、模糊或省略指令）的多模态扰动集。训练时，每个干净样本与其图像-文本扰动变体配对形成语义等价簇，通过偏好学习优化模型，比较干净和扰动条件下的模型响应，鼓励模型选择稳定响应而非扰动诱导的失败。

Result: 在三个地球观测任务上的实验表明，RemoteShield在现实多模态扰动下比代表性基线模型具有更强的鲁棒性和跨条件一致性。

Conclusion: RemoteShield通过语义等价簇和跨条件对齐的偏好学习，有效提升了遥感多模态大语言模型在现实噪声条件下的鲁棒性和一致性，解决了现有模型在部署时面对不完美输入的性能退化问题。
...
### [93] [Fractal Characterization of Low-Correlation Signals in AI-Generated Image Detection](https://arxiv.org/abs/2604.17268)
*Wenwei Xie,Jie Yin,Lu Ma,Xuansong Zhang,Wenjing Zhang*

Main category: cs.CV

TL;DR: 本文提出一种基于分形理论的新方法，通过分析低相关性信号的统计特性来检测AI生成图像，在开放场景中表现出更强的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: AI生成图像已达到接近真实照片的保真度，这对信息安全和社会信任构成重大威胁。现有的深度伪造检测方法在开放世界场景中往往鲁棒性有限，需要从信号层面探索新的检测方向。

Method: 从信号层面分析合成图像与真实图像的内在差异，发现低相关性信号可作为区分AI生成图像的标志性特征。基于分形理论提出量化这些信号的新方法，通过分析低相关性信号的分形特征来捕捉合成过程中的统计异常。

Result: 大量实验结果表明该方法具有鲁棒性和优越的检测性能，不仅限于人脸图像识别，理论上可应用于所有AI生成图像的检测任务。

Conclusion: 这项工作强调了将研究重点转向信号层面新方向的重要性，为深度伪造检测提供了新的研究思路，该方法具有广泛的应用潜力。
...
### [94] [Instinct vs. Reflection: Unifying Token and Verbalized Confidence in Multimodal Large Models](https://arxiv.org/abs/2604.17274)
*Yunkai Dang,Yifan Jiang,Yizhu Jiang,Anqi Chen,Wenbin Li,Yang Gao*

Main category: cs.CV

TL;DR: 本文提出了一种解决多模态大语言模型中本能-反思不对齐问题的方法，通过单调置信融合框架和顺序保持均值对齐来提升置信度估计的可靠性。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在各种感知和推理任务中表现出色，但在实际部署中需要可靠的置信度估计。现有研究主要集中在文本模型上，且依赖计算昂贵的自一致性采样。本文发现MLLMs存在显著的本能-反思不对齐问题：模型的隐式token级支持与其口头自我评估置信度经常不一致。

Method: 提出单调置信融合框架来合并双通道信号和跨通道一致性以估计正确性，然后应用顺序保持均值对齐步骤来校正全局偏差，在保持选择性预测的风险-覆盖权衡的同时改善校准。

Result: 在多种开源和闭源MLLMs上的实验表明，该方法能持续产生更可靠的置信度估计，并同时改善校准和失败预测性能。

Conclusion: 通过解决本能-反思不对齐问题，提出的方法能够显著提升多模态大语言模型的置信度估计可靠性，为实际部署提供了更稳健的置信度校准框架。
...
### [95] [PestVL-Net: Enabling Multimodal Pest Learning via Fine-grained Vision-Language Interaction](https://arxiv.org/abs/2604.17278)
*Xueheng Li,Tao Hu,Ke Cao,Runsheng Qi,Huixin Zhang,Rui Li,Jie Zhang,Chengjun Xie*

Main category: cs.CV

TL;DR: 提出PestVL-Net视觉语言框架，结合RWKV架构和MLLM先验，通过细粒度多模态学习解决农业害虫识别难题。


<details>
  <summary>Details</summary>
Motivation: 农业害虫识别对可持续发展至关重要，但实际场景中害虫数据收集困难，物种多样且形态复杂，现有技术难以有效建模害虫的细粒度视觉和语义特征，限制了实际应用。

Method: 提出PestVL-Net视觉语言框架：视觉通路采用RWKV架构，结合显著性引导的自适应窗口划分方案；语言组件利用多模态大语言模型先验，结合农业专家知识，通过多模态思维链推理生成精确害虫语义描述；实现视觉和文本表示的深度融合。

Result: 在多个害虫数据集上的广泛实验验证了PestVL-Net的优越性能，展示了其在现实世界害虫管理中的潜力。

Conclusion: PestVL-Net通过整合视觉和语言模态的协同方法，有效解决了细粒度害虫学习的关键挑战，为实际农业场景中的害虫管理提供了有效解决方案。
...
### [96] [Depth Adaptive Efficient Visual Autoregressive Modeling](https://arxiv.org/abs/2604.17286)
*Chunliang Li,Tianze Cao,Sanyuan Zhao*

Main category: cs.CV

TL;DR: DepthVAR提出了一种训练免费的自适应计算深度分配框架，通过循环旋转调度为每个token动态分配计算深度，替代传统的硬剪枝方法，实现2.3-3.1倍加速且质量损失最小。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉自回归模型对每个位置都应用固定的计算深度，导致生成高分辨率图像时效率低下。虽然现有方法通过频率图剪枝token来加速推理，但这种二值硬剪枝方法存在根本性限制，即使有更好的频率估计也无法提高质量。观察到VAR模型存在显著深度冗余，需要从剪枝整个token转向自适应分配每个token的计算深度。

Method: 提出DepthVAR框架，包含自适应深度调度器和动态推理过程。深度调度器通过循环旋转调度为每个token分配计算深度，实现平衡、非静态的细化；动态推理过程将这些深度转换为层主掩码，选择性应用transformer块，并混合生成的代码，确保每个token的影响与其处理深度成比例。

Result: 大量实验表明，DepthVAR实现了2.3-3.1倍的加速，同时质量损失最小。与现有的硬剪枝方法相比，提供了更具竞争力的计算-性能权衡。

Conclusion: DepthVAR通过自适应分配每个token的计算深度，而不是简单地剪枝整个token，为视觉自回归模型提供了一种高效且质量保持的加速解决方案，超越了传统的硬剪枝方法。
...
### [97] [Spectral Forensics of Diffusion Attention Graphs for Copy-Move Forgery Detection](https://arxiv.org/abs/2604.17287)
*H. M. Shadman Tabib,Tasriad Ahmed Tias,Nafis Tahmid*

Main category: cs.CV

TL;DR: GraphSpecForge：一种无需训练的复制-移动伪造检测框架，通过分析预训练Stable Diffusion U-Net中注意力图的谱结构来检测图像篡改


<details>
  <summary>Details</summary>
Motivation: 复制-移动伪造（图像区域复制以隐藏或伪造内容）对视觉媒体完整性构成持续威胁，需要有效的检测方法

Method: 利用预训练Stable Diffusion U-Net的自注意力图，分析归一化图拉普拉斯算子的谱分布变化。核心洞察是复制-移动操作会在自注意力图中引入近似子图复制，导致谱重分布。使用Wasserstein距离比较每张图像的拉普拉斯谱与真实参考分布来构建异常检测器

Result: 在四个复制-移动基准测试中无需特定训练：RecodAI-LUC上AUROC=0.606，归一化拉普拉斯比原始注意力谱提升+0.057 AUROC；MICC-F220、CoMoFoD和COVERAGE上AUROC分别为0.752、0.774和0.673；CoMoFoD上AUPRC=0.833，平衡准确率=0.712，MCC=0.499，TPR@1%FPR=32.5%

Conclusion: GraphSpecForge证明了从预训练扩散模型中提取的注意力图谱结构可以有效检测复制-移动伪造，无需特定训练，为图像取证提供了新方法
...
TL;DR: 提出SePL方法，通过可分离提示学习策略利用CLIP的文本模态指导深度伪造检测，在跨数据集和跨方法评估中表现优异
...
Conclusion: 通过简单的适应，CLIP可以作为有效的面部伪造检测器，文本模态在深度伪造检测中具有重要指导作用
...
TL;DR: 提出MedFocusLeak攻击方法，针对临床视觉语言模型，通过注入协调扰动到非诊断背景区域并采用注意力分散机制，生成不可察觉的扰动诱导错误但临床可信的诊断，在六种医学影像模态上实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在临床诊断中应用日益广泛，但其对抗攻击鲁棒性尚未充分探索，存在严重风险。现有医学攻击主要关注次要目标如模型窃取或对抗微调，而自然图像的可迁移攻击会产生明显失真易被临床医生检测。需要开发高度可迁移的黑盒多模态攻击方法，在保持扰动不可察觉的同时诱导错误但临床可信的诊断。

Method: 提出MedFocusLeak方法：1）将协调扰动注入非诊断背景区域；2）采用注意力分散机制将模型注意力从病理区域转移；3）设计高度可迁移的黑盒多模态攻击策略；4）在六种医学影像模态上进行评估。

Result: MedFocusLeak在六种医学影像模态上实现最先进的攻击性能，能够生成误导性但真实的诊断输出，适用于多种视觉语言模型。同时提出统一的评估框架和新颖指标，共同捕捉攻击成功率和图像保真度，揭示了现代临床视觉语言模型推理能力的关键弱点。

Conclusion: MedFocusLeak是一种高度可迁移的黑盒多模态攻击方法，能够诱导视觉语言模型产生错误但临床可信的诊断，同时保持扰动不可察觉。研究揭示了临床视觉语言模型在对抗攻击下的脆弱性，为未来开发更鲁棒的医学AI系统提供了重要见解。
...
TL;DR: E2E-GMNER：一种端到端生成式框架，统一了文本实体识别、语义类型预测和视觉定位，通过链式思维推理和高斯风险感知框扰动提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有GMNER方法多采用流水线架构，将文本实体识别和视觉定位解耦，导致错误累积和次优联合优化。需要统一的端到端框架来改善这些问题。

Method: 提出E2E-GMNER框架：1) 将GMNER建模为指令调优的条件生成任务；2) 引入链式思维推理，让模型自适应决定何时使用视觉证据或背景知识；3) 提出高斯风险感知框扰动(GRBP)，用概率扰动的软目标替代硬框监督，提高对标注噪声和离散化误差的鲁棒性。

Result: 在Twitter-GMNER和Twitter-FMNERG基准测试中，E2E-GMNER取得了与最先进方法相竞争的性能，验证了统一端到端优化和噪声感知定位监督的有效性。

Conclusion: E2E-GMNER通过统一的端到端生成框架成功解决了GMNER任务，结合链式思维推理和GRBP技术，显著提升了实体识别、语义类型预测和视觉定位的联合性能。
...
TL;DR: 提出QUOTA框架，将低比特量化和视觉token剪枝统一在单个可部署管道中，通过量化校准信号生成层间token分配计划，在W4A4量化下实现95.65%性能保留率（仅保留30%视觉token）


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在低比特推理中面临挑战：推理成本主要来自视觉token前缀的预填充和自回归解码中不断增长的KV缓存。现有的token剪枝和低比特量化方法虽然互补，但简单的阶段式组合往往因量化校准与剪枝执行不匹配而效果不佳。

Method: 提出协作量化-剪枝框架，包含QUOTA（量化统一离线token分配器）。该框架将低比特校准信号转换为层间token分配计划，并实现为剪枝方案。在部署的W4A4算子下，结合激活幅度、注意力线索和显式低比特风险信号评估token重要性，实现一致的预算化top-k选择。

Result: 在标准VLM基准测试中，相比阶段式基线约94.3%的保留率，QUOTA在相同低比特机制下实现95.65%的平均保留率，同时仅保留30%的视觉token，表现出更好的鲁棒性。

Conclusion: QUOTA框架通过统一低比特推理和确定性视觉token剪枝，解决了量化校准与剪枝执行不匹配的问题，在保持高性能的同时显著降低了视觉语言模型的推理成本。
...
TL;DR: 该研究提出了VisualTextTrap基准测试，用于评估视觉语言模型在屏幕文本与视觉内容矛盾时的幻觉问题，并开发了VTHM-MoE框架来缓解这种文本覆盖诱导的幻觉。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在屏幕文本与视觉场景矛盾时会产生系统性幻觉，优先考虑覆盖文本语义而忽略实际视觉内容，这种文本覆盖诱导的幻觉问题尚未得到充分研究。

Method: 1) 构建VisualTextTrap基准测试，包含6,057个人工验证样本，涵盖88个细粒度属性，使用五级强度量化幻觉程度；2) 提出VTHM-MoE框架，采用双编码器架构和四个维度专家模块，通过自适应令牌路由策略实现动态专家分配。

Result: 在VisualTextTrap基准测试上的广泛实验验证了VTHM-MoE的有效性，在多样化的视频问答任务中优于现有最先进方法，能够有效抵抗文本覆盖诱导的幻觉。

Conclusion: 该研究首次系统性地识别和量化了文本覆盖诱导的幻觉问题，提出的基准测试和缓解框架为视觉语言模型的鲁棒性评估和改进提供了重要工具和方法。
...
TL;DR: 使用DINOv2、AIMv2和OpenCLIP的ViT-L/14等微调视觉变换器集成方法，在DF-Wild数据集上实现深度伪造检测，AUC达96.77%，EER为9%，超越现有最佳方法，赢得IEEE SP Cup 2025比赛。


<details>
  <summary>Details</summary>
Motivation: 现代生成模型快速发展导致深度伪造图像检测面临挑战，现有方法泛化能力不足。需要开发能够应对多样化操纵和生成技术的通用检测方法。

Method: 采用集成学习方法，结合多个微调的视觉变换器模型（DINOv2、AIMv2、OpenCLIP的ViT-L/14）。使用IEEE SP Cup 2025发布的DF-Wild数据集，该数据集包含具有挑战性和多样化的操纵和生成技术。实验从基于空间特征的CNN分类器开始，最终采用集成策略。

Result: 集成方法在DF-Wild测试集上取得AUC 96.77%和EER 9%的优异性能，超越单个模型和强CNN基线。相比当前最先进的Effort算法，AUC提升7.05%，EER降低8%。该方案赢得SP Cup比赛并在ICASSP 2025展示。

Conclusion: 基于视觉变换器集成的方法在深度伪造检测任务中表现出强大的泛化能力和优越性能，能够有效应对现代生成模型带来的挑战，为实际应用提供了有前景的解决方案。
...
TL;DR: SpatialImaginer：一个统一的多模态生成框架，通过结合文本推理和视觉想象来解决MLLMs在空间智能任务中的脆弱性，在多个空间推理基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在空间智能任务中表现出脆弱的推理轨迹，特别是在需要一致空间状态识别的任务中。这种失败源于空间识别机制与MLLMs纯文本推理行为之间的不匹配，因为文本表示往往会抽象掉关键的几何结构细节。

Method: 提出SpatialImaginer框架，采用分治策略：使用文本思维链进行高层语义规划，使用视觉想象进行几何敏感的状态转换和一致性保持。引入难度感知的数据引擎和闭环验证，训练模型在需要稳定空间状态跟踪时选择性地调用视觉想象。

Result: 在多样化的空间智能基准测试中，SpatialImaginer实现了最先进的性能，并在复杂的多步空间推理任务中显著提高了鲁棒性。

Conclusion: 通过将文本推理与视觉想象相结合，SpatialImaginer有效解决了MLLMs在空间智能任务中的局限性，为几何结构保持和一致性维护提供了创新解决方案。
...
### [105] [Deep learning based Non-Rigid Volume-to-Surface Registration for Brain Shift compensation Using Point Cloud](https://arxiv.org/abs/2604.17389)
*Eashrat Jahan Muniya,Gernot Kronreif,Ander Biguri,Wolfgang Birkfellner,Sepideh Hatamikia*

Main category: cs.CV

TL;DR: 提出深度学习框架进行非刚性体表配准，从稀疏术中表面观测估计密集位移场，无需明确点对应或术中体积成像，实现脑移位补偿。


<details>
  <summary>Details</summary>
Motivation: 图像引导神经外科中软组织变形（脑移位）导致术中解剖结构与术前影像偏差，影响导航精度和手术安全。现有补偿方法依赖术中MRI/CT/超声，会中断手术流程。而术中仅能获取部分皮质表面点云，使得点云配准成为实用替代方案，但部分和噪声观测使变形估计极具挑战。

Method: 提出深度学习框架进行非刚性体表配准，利用多尺度点基特征提取和分层变形解码器捕捉全局和局部变形。关键贡献是将部分术中表面信息整合到完整术前点云域，实现隐式对应学习和有限可见性下的密集变形恢复。

Result: 在具有挑战性的部分表面条件下，准确恢复精细尺度变形，达到端点误差(EPE)1.13±0.75mm和均方根误差(RMSE)1.33±0.81mm。

Conclusion: 该方法支持从稀疏表面观测进行自动、工作流兼容的脑移位补偿，无需明确点对应或术中体积成像，为图像引导神经外科提供实用解决方案。
...
TL;DR: SDVG将推测解码技术应用于自回归视频扩散模型，通过图像质量路由器替代令牌验证，实现训练免费的视频生成加速，在保持98.1%质量的同时获得1.59倍加速。


<details>
  <summary>Details</summary>
Motivation: 自回归视频扩散模型在流式视频合成中前景广阔，但现有加速方法主要依赖步长蒸馏。推测解码在大型语言模型中效果显著，但能否有效应用于视频生成仍是一个开放问题，因为视频块是连续时空张量，没有令牌级分布进行精确拒绝采样。

Method: SDVG采用1.3B草稿模型通过四步去噪提出候选视频块，每个块经VAE解码后由ImageReward使用最差帧聚合评分（取每帧奖励的最小值以捕捉单帧伪影）。得分超过固定阈值τ的块被接受并存入14B目标模型的KV缓存，其余由目标模型重新生成。关键设计包括：强制拒绝第一个块以锚定场景构图，τ作为单一旋钮可平滑调节质量-速度权衡。

Result: 在1003个MovieGenVideoBench提示（832x480分辨率）上，SDVG在τ=-0.7时保持98.1%的目标模型视觉奖励质量（0.0773 vs 0.0788），实现1.59倍加速；在95.7%质量保持率时达到2.09倍加速。始终比仅使用草稿模型的生成质量高出17%以上。

Conclusion: SDVG成功将推测解码技术适配到基于块的自回归视频扩散模型，无需训练、无需架构修改，可无缝集成到现有视频生成流程中，为视频合成提供了有效的加速解决方案。
...
TL;DR: 本文提出专门用于长视频评估的框架Long-CODE，通过解耦短期视觉感知和长上下文属性，设计基于镜头动态的新指标，解决传统短视频指标无法捕捉叙事丰富性和全局因果一致性的问题。


<details>
  <summary>Details</summary>
Motivation: 随着视频生成模型能够生成长视频，传统针对短视频设计的评估指标（主要评估帧级视觉质量和局部时间平滑度）无法捕捉长视频的关键特性，如叙事丰富性和全局因果一致性。需要专门的长视频评估框架。

Method: 1. 引入长视频属性破坏测试，暴露现有短视频指标的局限性；2. 设计基于镜头动态的新型长视频指标；3. 构建Long-CODE专门数据集，包含针对真实长范围特性的人工标注。

Result: 提出的指标在与人判断的相关性方面达到最先进水平，能够有效检测镜头级扰动和叙事重排等结构不一致性，而传统指标对这些不敏感。

Conclusion: 提出的长视频评估指标和基准与现有短视频标准无缝互补，建立了全面无偏的视频生成模型评估范式，解决了长视频评估的关键挑战。
...
TL;DR: 本研究提出了一种基于明暗恢复形状（SfS）的框架，利用月船2号轨道器高分辨率相机（OHRC）图像增强亚米级分辨率月球数字高程模型（DEM），通过独立OHRC图像获取不受立体基线限制的新地形数据。


<details>
  <summary>Details</summary>
Motivation: 传统立体摄影测量受基线限制，难以获取高分辨率地形细节。本研究旨在利用SfS技术突破这一限制，从单张高分辨率图像中提取更精细的地形信息，增强现有DEM的质量。

Method: 采用三阶段参数扫描方法，在三个月球区域（Cyrillus陨石坑、Vikram着陆区、月球南极Mons Mouton）应用SfS框架，使用独立OHRC图像进行明暗恢复形状分析，系统研究平滑度权重参数的影响。

Result: 结果显示地形增强效果显著，特别是在表面坡度统计方面，揭示了之前未解析的精细尺度陨石坑形态。同时识别了限制情况：当明暗图像与立体像对之间的俯仰角差异较大时，SfS敏感性降低；部分覆盖区域会影响增强质量的空间变异性。

Conclusion: SfS不仅可作为地形细化工具，还能作为不受立体基线限制的新地形数据源，有效增强亚米级分辨率月球DEM，但需注意图像几何条件和覆盖范围对增强效果的影响。
...
### [109] [SegTTA: Training-Free Test-Time Augmentation for Zero-Shot Medical Imaging Segmentation](https://arxiv.org/abs/2604.17451)
*Yihong Yao,Chunlei Li,Canxuan Gang,Wenzhi Hu,Zeyu Zhang,Hao Zhang,Xiaoyan Li*

Main category: cs.CV

TL;DR: SegTTA框架通过结合四种图像增强技术和多检查点加权投票，无需重新训练模型即可提升医学图像分割性能，在多个数据集上取得显著改进。


<details>
  <summary>Details</summary>
Motivation: 尽管当前基础模型具有强大的泛化能力，但由于设备和操作人员的差异导致图像质量不一，需要解决医学图像分割中因图像质量差异带来的挑战。

Method: 提出SegTTA框架，结合伽马校正、对比度增强、高斯模糊和高斯噪声四种增强技术，通过多个MedSAM2检查点的加权投票机制，无需重新训练模型即可提升分割性能。

Result: 在三个不同数据集上均取得一致改进：健康子宫分割、子宫肌瘤检测和多类别肝结构分割。在肝血管数据集上，相比MedSAM2基线，mIoU提升1.6，aIoU提升1.9，HD95降低约2.0。

Conclusion: SegTTA框架有效解决了医学图像质量差异问题，通过投票阈值控制覆盖精度权衡，可根据不同临床需求进行任务特定优化，为医学图像分割提供了实用的增强解决方案。
...
### [110] [HSG: Hyperbolic Scene Graph](https://arxiv.org/abs/2604.17454)
*Liyang Wang,Zeyu Zhang,Hao Tang*

Main category: cs.CV

TL;DR: HSG使用双曲空间学习场景图嵌入，通过几何距离自然编码层次关系，在保持检索性能的同时显著提升层次结构质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法在欧几里得空间中学习场景图嵌入，但欧几里得几何无法明确捕捉地点和对象之间的层次蕴含关系，限制了学习表示的结构一致性。

Method: 提出双曲场景图（HSG），在双曲空间中学习场景图嵌入，其中层次关系通过几何距离自然编码。

Result: HSG在保持强检索性能的同时提高了层次结构质量。在图形级别指标上取得最大增益：PP IoU达到33.17，Graph IoU达到33.51，比最佳AoMSG变体（25.37）高出8.14。

Conclusion: 双曲表示学习对场景图建模具有显著效果，能更好地捕捉层次关系并提高结构一致性。
...
### [111] [UniMesh: Unifying 3D Mesh Understanding and Generation](https://arxiv.org/abs/2604.17472)
*Peng Huang,Yifeng Chen,Zeyu Zhang,Hao Tang*

Main category: cs.CV

TL;DR: UniMesh是一个统一框架，通过创新的Mesh Head、Chain of Mesh机制和自反思机制，在单一架构中联合学习3D生成与理解，实现了迭代编辑和生成-理解相互增强的新能力。


<details>
  <summary>Details</summary>
Motivation: 当前3D视觉领域存在专门化模型碎片化的问题，3D理解（如分类、分割、重建）和3D生成（如合成、补全、编辑）任务通常孤立处理，导致架构和表示碎片化，阻碍知识传递和整体场景建模。

Method: 1. 引入新颖的Mesh Head作为跨模型接口，桥接基于扩散的图像生成与隐式形状解码器；2. 开发Chain of Mesh（CoM）机制，作为迭代推理的几何实例化，通过闭环的潜在-提示-重新生成循环实现用户驱动的语义网格编辑；3. 整合基于Actor-Evaluator-Self-reflection三元组的自反思机制，用于诊断和纠正高级任务（如3D字幕生成）中的失败。

Result: 实验结果表明，UniMesh不仅在标准基准测试中实现了有竞争力的性能，还解锁了迭代编辑以及生成与理解之间相互增强的新能力。

Conclusion: UniMesh通过统一框架解决了3D视觉中生成与理解任务碎片化的问题，为迭代编辑和知识传递提供了新途径，推动了整体场景建模的发展。
...
### [112] [Dual-Anchoring: Addressing State Drift in Vision-Language Navigation](https://arxiv.org/abs/2604.17473)
*Kangyi Wu,Pengna Li,Kailin Lyu,Lin Zhao,Qingrong He,Jinjun Wang,Jianyi Liu*

Main category: cs.CV

TL;DR: 本文提出双锚定框架解决视觉语言导航中的状态漂移问题，通过指令进度锚定和记忆地标锚定分别应对进度漂移和记忆漂移，在长场景导航中显著提升成功率。


<details>
  <summary>Details</summary>
Motivation: 当前视频大语言模型在视觉语言导航中面临状态漂移问题，特别是在长场景中，智能体的内部状态会偏离真实任务执行状态，导致漫无目的的徘徊和无法执行关键操作。这种失败源于两种认知缺陷：进度漂移（无法区分已完成和待完成的子目标）和记忆漂移（历史表征退化导致忘记访问过的地标）。

Method: 提出双锚定框架：1) 指令进度锚定：监督智能体生成结构化文本标记来区分已完成和待完成的子目标；2) 记忆地标锚定：利用地标中心世界模型回顾性预测由Segment Anything模型提取的以对象为中心的嵌入，强制智能体明确验证过去的观察并保留访问过的地标的独特表征。为此构建了两个大规模数据集：360万带有明确进度描述的样本和93.7万用于回顾性验证的接地地标数据。

Result: 在仿真和真实环境中的广泛实验表明，该方法具有优越性，成功率提升了15.2%，在长轨迹上获得了24.7%的显著增益。

Conclusion: 双锚定框架通过显式锚定指令进度和历史表征，有效解决了视觉语言导航中的状态漂移问题，显著提升了长场景导航的性能。该方法为相关研究提供了新的思路和工具支持。
...
TL;DR: CoReDi提出了一种自适应语义表示空间的扩散模型框架，通过联合训练轻量级线性投影来优化表示空间，提升图像生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有联合图像特征生成方法依赖固定的表示空间，这些空间独立于生成目标构建且在训练中保持不变。作者认为指导扩散的表示空间本身应该适应生成任务。

Method: 提出Coevolving Representation Diffusion (CoReDi)框架，在训练过程中通过联合学习轻量级线性投影来演化语义表示空间。采用停止梯度目标、归一化和针对性正则化防止特征崩溃，实现稳定的协同演化。

Result: CoReDi在VAE潜在扩散和像素空间扩散中均表现出色，相比固定表示空间的联合扩散模型，实现了更快的收敛速度和更高的样本质量。

Conclusion: 自适应语义表示空间能够更好地适应图像合成需求，提升与图像潜在特征的互补性，从而改善生成建模效果。
...
TL;DR: 本文提出Edit Fidelity Field (EFF)方法解决场景文本编辑中的编辑溢出问题，通过语义感知连续场控制像素级编辑保真度，将溢出率从94%降至25%


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的场景文本编辑方法存在严重的编辑溢出问题——在编辑目标文本区域时，会无意中修改非目标区域（特别是相邻文本）。通过系统评估发现，最先进的扩散编辑模型的溢出率高达94%，几乎所有非目标文本区域在编辑过程中都被改变。

Method: 提出Edit Fidelity Field (EFF)，一种语义感知的连续场，用于控制像素级编辑保真度。与二值掩码不同，EFF利用OCR检测的文本区域构建四区域场：编辑核心区（完全可编辑）、过渡区（平滑衰减）、保护区（非目标文本，明确锁定）和背景区（严格保留）。EFF作为无需训练、模型无关的后处理模块，适用于任何基于扩散的STE方法。

Result: 实验表明，EFF将溢出率从94%显著降低到25%，同时将非目标区域保护性能提升+91.4 dB PSNR。还提出了按区域溢出量化这一新颖评估协议，用于单独测量每个非目标文本区域的编辑泄漏。

Conclusion: EFF有效解决了场景文本编辑中的编辑溢出问题，通过语义感知的连续场控制编辑保真度，显著提高了编辑的精确性和非目标区域的保护能力，为扩散基STE方法提供了通用的后处理解决方案。
...
### [115] [RS-HyRe-R1: A Hybrid Reward Mechanism to Overcome Perceptual Inertia for Remote Sensing Images Understanding](https://arxiv.org/abs/2604.17504)
*Gaozhi Zhou,Hu He,Peng Shen,Jipeng Zhang,Liujue Zhang,Linrui Xu,Zeyuan Wang,Ziyu Li,Xuezhi Cui,Wang Guo,Haifeng Li*

Main category: cs.CV

TL;DR: 提出RS-HyRe-R1混合奖励框架解决遥感视觉语言模型中的"感知惯性"问题，通过三种奖励机制促进更全面的视觉证据挖掘，在多个任务上实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 强化学习后训练虽然能提升遥感视觉语言模型性能，但会导致"感知惯性"问题——模型倾向于依赖局部显著线索进行快速推理，而不是进行全面视觉扫描。这种偏差限制了模型构建完整证据链的能力，也难以在不同任务间灵活转移视觉焦点。

Method: 提出RS-HyRe-R1混合奖励框架，包含三个核心组件：1) 空间推理激活奖励，强制结构化视觉推理；2) 感知正确性奖励，提供跨任务的自适应质量锚点，确保准确的几何和语义对齐；3) 视觉语义路径演化奖励，惩罚重复推理，促进探索互补线索以构建更丰富的证据链。

Result: 仅使用3B参数就在REC、OVD和VQA任务上实现了最先进性能，超越了参数高达7B的模型。在零样本泛化方面，在VQA、OVD和REC任务上分别超过第二佳模型3.16%、3.97%和2.72%。实验表明RS-HyRe-R1有效缓解了"感知惯性"，促进了更深层、更多样化的推理。

Conclusion: RS-HyRe-R1框架通过混合奖励机制成功解决了遥感视觉语言模型中的"感知惯性"问题，不仅提升了模型性能，还增强了模型的推理深度和多样性，为复杂遥感图像理解提供了有效解决方案。
...
### [116] [Dual Strategies for Test-Time Adaptation](https://arxiv.org/abs/2604.17542)
*Nam Nguyen Phuong,Duc Nguyen The Minh,Phi Le Nguyen,Ehsan Abbasnejad,Minh Hoai*

Main category: cs.CV

TL;DR: DualTTA提出了一种新的测试时适应框架，通过识别可靠和不可靠样本组，分别进行最小化和最大化熵优化，更充分地利用测试分布信息。


<details>
  <summary>Details</summary>
Motivation: 传统TTA方法通常只使用一小部分低熵预测的测试样本进行适应，未能充分利用测试分布中的可用信息，限制了性能提升。

Method: DualTTA框架识别两个不同的样本组：预测与底层语义一致的可靠组和预测可能错误的不可靠组。对可靠组最小化预测熵以强化可靠决策，对不可靠组最大化熵以抑制过度自信的错误并消除虚假行为。使用新的可靠性标准自适应选择样本组，该标准通过语义保持和语义改变变换下的预测稳定性来衡量。

Result: 理论分析和实证验证表明，该方法能够在适应适用性方面实现可靠和不可靠样本的更严格分离，从而提供更有效的模型更新。

Conclusion: DualTTA通过更全面地利用测试样本，在分布偏移下实现了更好的性能，解决了传统TTA方法样本利用不足的问题。
...
### [117] [UniGeo: Unifying Geometric Guidance for Camera-Controllable Image Editing via Video Models](https://arxiv.org/abs/2604.17565)
*Hong Jiang,Wensong Song,Zongxing Yang,Ruijie Quan,Yi Yang*

Main category: cs.CV

TL;DR: UniGeo提出了一种统一几何引导的三级框架，用于相机可控图像编辑，通过在表示、架构和损失函数三个层面注入几何指导，解决了现有方法在连续相机运动下的几何漂移和结构退化问题。


<details>
  <summary>Details</summary>
Motivation: 现有相机可控图像编辑方法存在两个主要问题：1）依赖碎片化的几何指导（如仅在表示层面注入点云），而模型包含多个层面；2）主要基于图像扩散模型，在离散视角映射上操作。这两个限制共同导致在连续相机运动下出现几何漂移和结构退化。

Method: UniGeo框架在三个层面统一注入几何指导：1）表示层面：采用帧解耦的几何参考注入机制，提供稳健的跨视角几何上下文；2）架构层面：引入几何锚点注意力来对齐多视角特征；3）损失函数层面：提出轨迹端点几何监督策略，显式增强目标视角的结构保真度。

Result: 在多个公共基准测试中，包括广泛和有限的相机运动设置，UniGeo在视觉质量和几何一致性方面显著优于现有方法。

Conclusion: 通过统一在三个关键层面注入几何指导，UniGeo有效解决了相机可控图像编辑中的几何漂移问题，实现了更好的跨视角几何一致性和结构保真度。
...
### [118] [Multi-Camera Self-Calibration in Sports Motion Capture: Leveraging Human and Stick Poses](https://arxiv.org/abs/2604.17567)
*Fan Yang,Changsoo Jung,Ryosuke Kawamura,Hon Yung Wong*

Main category: cs.CV

TL;DR: 提出一种无需专用工具的体育多相机外参标定方法，利用人体关键点和已知长度的棍状器械进行联合优化


<details>
  <summary>Details</summary>
Motivation: 体育运动中多相机系统广泛用于捕捉3D运动，但外参标定成本高、劳动密集，需要更高效的无工具标定方法

Method: 三阶段优化流程：利用同步多相机视频中的人体关键点（未知度量尺度）和已知长度的刚性棍状器械，通过棍长约束解决全局尺度问题

Result: 方法在包含3-10个相机的合成数据集上达到SOTA性能，实现低旋转和平移误差，无需专用标定工具

Conclusion: 该方法为棍状器械类体育提供了一种高效、无工具的多相机外参标定解决方案，并建立了该任务的第一个基准数据集
...
### [119] [PBSBench: A Multi-Level Vision-Language Framework and Benchmark for Hematopathology Whole Slide Image Interpretation](https://arxiv.org/abs/2604.17570)
*Yuanlong Wang,Weichi Chen,Adrian Rajab,Wenfang Liu,Yulan Jin,Andrew Srisuwananukorn,Ping Zhang*

Main category: cs.CV

TL;DR: 该研究构建了首个外周血涂片（PBS）视觉语言数据集PBSInstr，开发了专门针对血液病理学的视觉语言模型PBS-VL，并创建了评估基准PBSBench，显著提升了PBS图像解读的AI性能。


<details>
  <summary>Details</summary>
Motivation: 当前的多模态大语言模型主要基于实体组织病理学开发，难以泛化到外周血涂片（PBS）分析。PBS与实体组织病理学在视觉特征和诊断推理上存在本质差异，需要专门的数据集和模型来解决这一领域空白。

Method: 1. 构建PBSInstr数据集：包含353个PBS全玻片图像配对的微观印象段落、29k细胞级图像裁剪标注（细胞类型和形态描述）、27k细胞级QA对和1,286个玻片级QA对；2. 开发PBS-VL模型：专门针对血液病理学的视觉语言模型，支持细胞和玻片级别的多级PBS解读；3. 创建PBSBench评估基准：包含四个问题类别和六个PBS解读任务的视觉问答基准。

Result: 实验表明，PBS-VL在PBS理解任务上显著优于现有的通用和病理学MLLMs，验证了PBS特定数据的价值。该模型在细胞和玻片级别的多级PBS解读任务中表现出色。

Conclusion: 该研究填补了PBS视觉语言分析领域的空白，提出的框架为开发支持血液病理学决策的实用AI助手奠定了基础。通过发布代码、数据集和模型权重，促进了该领域的未来研究。
...
### [120] [ViPS: Video-informed Pose Spaces for Auto-Rigged Meshes](https://arxiv.org/abs/2604.17623)
...
TL;DR: ViPS是一个从视频扩散模型中提取运动先验的框架，为自动绑定网格创建有效的姿态空间，避免了传统方法对稀缺4D数据的依赖。


<details>
  <summary>Details</summary>
Motivation: 传统的运动学绑定缺乏对关节配置合理流形的表示，导致随机采样或手动调整时容易出现解剖学超伸和非物理自相交等语义或几何违规问题。

Method: 提出Video-informed Pose Spaces (ViPS)，通过从预训练的视频扩散模型中提取运动先验，将生成视频先验转移到给定绑定参数化的通用分布上，并应用可微几何验证器确保资产特定有效性。

Result: ViPS仅使用视频先验训练，在合理性和多样性方面与使用合成艺术家创建4D数据训练的最先进方法性能相当，并表现出对分布外物种和未见骨骼拓扑的鲁棒零样本泛化能力。

Conclusion: ViPS通过学习平滑、紧凑且可控的姿态空间，实现了生成2D先验与结构化3D运动控制之间的闭环，为运动学绑定提供了通用且有效的姿态空间解决方案。
...
### [121] [BioVLM: Routing Prompts, Not Parameters, for Cross-Modality Generalization in Biomedical VLMs](https://arxiv.org/abs/2604.17629)
*Mainak Singha,Tanisha Gupta,Ankit Jha,Muhammad Haris Khan,Sayantani Ghosh,Biplab Banerjee*

Main category: cs.CV

TL;DR: BioVLM是一个针对生物医学视觉-语言模型的提示学习框架，通过动态提示选择和知识蒸馏提升跨域泛化能力，在少样本监督下显著改善性能。


<details>
  <summary>Details</summary>
Motivation: 现有预训练的生物医学视觉-语言模型（如BioMedCLIP）在具有挑战性的模态上表现不佳，特别是当类间差异小、采集特异性变化显著、少样本监督以及模态先验与预训练语料差异较大时。

Method: BioVLM提出一个提示学习框架，包含：1）学习多样化的提示库；2）动态提示选择机制，通过低熵准则为每个输入选择最具判别性的提示；3）蒸馏高置信度的LLM派生属性；4）通过强/弱增强一致性强制鲁棒的知识迁移。

Result: 在11个MedMNIST+ 2D数据集上，BioVLM在三个不同的泛化设置中均达到了新的最先进水平。

Conclusion: BioVLM通过轻量级训练和高效推理，实现了对未见类别和领域的有效迁移，显著提升了生物医学视觉-语言模型在跨域泛化任务中的性能。
...
TL;DR: 本文提出基础设施中心世界模型（I-WM）概念，利用路边固定传感器的鸟瞰视角、多传感器融合和持久观测优势，与车辆中心世界模型形成时空互补，构建三层架构的生成式交通理解与预测系统。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶世界模型均采用车辆中心视角，忽略了基础设施视角的独特优势。路边固定传感器具有鸟瞰视角、多传感器融合和持久观测能力，能够积累长期行为分布数据（包括罕见安全关键事件），与车辆传感器的空间广度形成互补。

Method: 提出基础设施中心世界模型（I-WM）三层架构：1）具有质量感知不确定性传播的生成式场景理解；2）基于物理的预测动力学与多智能体反事实推理；3）通过潜在空间对齐实现V2X通信的协作世界模型。采用无标注感知作为多模态数据引擎，分阶段传感器策略（从LiDAR到4D雷达、信号相位数据再到事件相机）。
...
Conclusion: 基础设施中心世界模型为自动驾驶提供了与车辆中心视角互补的能力，通过路边传感器的独特优势实现更全面、持久的交通理解与预测，为实现能够理解和预测交通的基础设施提供了技术路径。
...
TL;DR: 提出一种用于Sentinel-5P卫星数据的自监督超分辨率框架，无需高分辨率地面真值，结合SURE估计器和等变成像约束，在保持光谱保真度的同时提升空间分辨率。


<details>
  <summary>Details</summary>
Motivation: Sentinel-5P卫星在空气质量监测中至关重要，但其空间分辨率限制了精细尺度分析。现有超分辨率方法依赖合成低分辨率数据的监督学习，由于缺乏真实高分辨率数据，限制了其在真实观测中的应用。

Method: 提出自监督高光谱超分辨率框架，结合Stein无偏风险估计器(SURE)和等变成像约束，整合S5P退化算子和从信噪比元数据推导的噪声统计。引入深度可分离卷积U-Net架构以提高效率和光谱保真度。

Result: 在两种设置下评估：1)LR-HR：使用合成低分辨率数据与监督学习方法直接比较；2)GT-SHR：超分辨率图像超越原生空间分辨率而无高分辨率参考。多波段结果显示自监督模型达到与监督方法相当的性能，同时保持强一致性。定性分析显示比双三次插值有更好的空间细节，与EMIT数据验证确认重建结构具有物理意义。

Conclusion: 该自监督超分辨率框架能够在没有高分辨率地面真值的情况下有效提升Sentinel-5P数据的空间分辨率，为大气监测提供更精细的分析能力，同时保持光谱保真度和物理一致性。
...
TL;DR: DVSA框架通过动态视觉-语义对齐和对比优化，在标签噪声和模糊情况下提升零样本学习性能


<details>
  <summary>Details</summary>
Motivation: 现有零样本学习方法通常假设标签干净，忽略了现实世界中的标签噪声和模糊性，这会导致性能下降。需要开发能够在模糊标签下稳健学习的零样本学习框架。

Method: 提出动态视觉-语义对齐（DVSA）框架：1）双向视觉-语义对齐模块，通过注意力机制相互校准视觉特征和属性原型；2）基于互信息的属性级对比优化，增强判别性和语义一致性属性；3）动态标签消歧机制，迭代修正噪声监督同时保持语义一致性。

Result: 在标准基准测试上的广泛实验验证了DVSA在模糊监督下实现了更强的性能。

Conclusion: DVSA框架通过动态视觉-语义对齐和标签消歧机制，有效解决了零样本学习中的标签噪声和模糊问题，提升了在现实世界场景中的泛化能力。
...
TL;DR: GeGS-PCR是一种结合几何、颜色和高斯信息的点云配准方法，在低重叠和不完整场景下表现出色，在Color3DMatch和Color3DLoMatch数据集上达到99.9%的配准召回率。


<details>
  <summary>Details</summary>
Motivation: 传统仅依赖几何特征的点云配准方法在低重叠和不完整场景中表现不佳，需要结合颜色信息来提高配准的鲁棒性和准确性。
...
Conclusion: GeGS-PCR通过有效结合几何、颜色和高斯信息，在具有挑战性的低重叠点云配准场景中实现了最先进的性能，显著提升了配准精度和鲁棒性。
...
TL;DR: 本文提出GaussianHSI框架，基于高斯泼溅实现任意尺度高光谱图像超分辨率，通过Voronoi引导的双边2D高斯泼溅进行空间重建，并结合光谱细节增强模块提升光谱保真度。


<details>
  <summary>Details</summary>
Motivation: 现有高光谱图像超分辨率方法通常需要针对不同尺度进行调整，缺乏灵活性；传统基于光栅化的方法限制了灵活的空间建模；将现有方法扩展到高光谱图像超分辨率面临挑战，需要在保持光谱保真度的同时实现自适应空间重建。

Method: 提出GaussianHSI框架：1）Voronoi引导的双边2D高斯泼溅进行空间重建：预测一组高斯函数表示输入，通过Voronoi引导选择将目标像素与相关高斯函数关联，然后使用参考感知双边加权聚合所选高斯函数；2）光谱细节增强模块提升光谱重建质量。

Result: 在基准数据集上的大量实验表明，GaussianHSI在任意尺度高光谱图像超分辨率任务上优于现有最先进方法。

Conclusion: GaussianHSI框架成功解决了高光谱图像任意尺度超分辨率问题，通过高斯泼溅的连续表示实现了灵活的空间重建，同时保持了光谱保真度，为高光谱图像处理提供了有效的解决方案。
...
TL;DR: 提出基于分数的冷冻电镜图像去噪框架，通过学习干净数据的分数来恢复粒子信号并更好地保留结构信息，提高下游分析效果


<details>
  <summary>Details</summary>
Motivation: 冷冻电镜图像信噪比极低，现有去噪方法虽然能改善视觉质量，但未明确考虑下游分析所需的结构一致性要求

Method: 提出基于分数的去噪框架，学习干净数据的分数来恢复粒子信号；进一步引入目标引导变体，在弱信号条件下加入参考密度引导以稳定分数学习

Result: 在多个冷冻电镜数据集上的实验表明，该方法能更好地抑制结构化低频背景，提高粒子-背景可分离性，改善下游粒子挑选并产生更结构一致的3D重建

Conclusion: 基于分数的去噪框架能有效提升冷冻电镜图像质量，更好地保留结构信息，为下游分析提供更可靠的预处理
...
TL;DR: IncreFA是一个增量学习框架，用于持续适应新兴AI生成模型的图像溯源问题，通过层次约束和潜在记忆库实现持续学习，在包含28个生成模型的基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 随着AI生成模型的快速发展，图像溯源成为一个移动目标，新的扩散、对抗和自回归生成器几乎每月出现，使得现有的水印、分类器和反演方法在发布时就过时。核心问题不在于模型识别，而在于溯源方法本身无法适应新模型。

Method: IncreFA将溯源重新定义为结构化增量学习问题，包含两个相互增强的机制：(1) 层次约束：通过可学习的正交先验编码架构层次关系，分离家族级不变特征和模型特定特征；(2) 潜在记忆库：重放紧凑的潜在样本并混合生成伪未见样本，稳定表示漂移并增强开放集感知。

Result: 在新构建的增量溯源基准(IABench)上，覆盖2022-2025年间发布的28个生成模型，IncreFA在时间顺序开放集协议下实现了最先进的溯源准确率和98.93%的未见模型检测率。

Conclusion: IncreFA通过将图像溯源重新定义为结构化增量学习问题，解决了AI生成模型快速演进带来的溯源挑战，实现了对新模型的持续适应能力，为动态环境下的图像溯源提供了有效解决方案。
...
TL;DR: DIFO++是一种新的无源域自适应方法，利用现成的视觉-语言多模态模型（如CLIP）来提升目标域适应性能，通过交替进行提示学习和知识蒸馏来减少特征间隙区域。


<details>
  <summary>Details</summary>
Motivation: 传统的无源域自适应方法依赖伪标签和辅助监督，容易产生错误。本文首次探索利用现成的视觉-语言多模态模型（如CLIP）中丰富但异构的知识来缓解这一限制，因为直接零样本应用这些通用模型对特定任务效果不佳。

Method: 提出DIFO++方法，在适应过程中交替进行两个步骤：1）通过提示学习最大化与目标模型的互信息来定制ViL模型；2）将定制后的ViL模型知识蒸馏到目标模型，重点减少间隙区域。采用渐进知识适应，首先识别和关注特征纠缠、类别模糊的间隙区域，融合目标和ViL模型的预测生成可靠伪标签，通过类别注意力和预测一致性进行语义对齐，并辅以参考熵最小化来抑制不确定性。

Result: 大量实验表明，DIFO++在性能上显著优于现有的最先进方法。

Conclusion: DIFO++通过有效利用视觉-语言多模态模型的丰富知识，结合提示学习和知识蒸馏策略，成功提升了无源域自适应的性能，特别是在处理特征间隙区域方面表现出色。
...
TL;DR: 该论文提出EgoIn框架解决以自我为中心的视觉状态转换生成问题，通过TransitionVLM理解转换步骤，Transition Conditioning模块生成帧序列，Object-aware Auxiliary Supervision保持对象外观一致性。


<details>
  <summary>Details</summary>
Motivation: 理解物理转换过程对人类认知和人工智能系统都至关重要，特别是从自我中心视角出发，这是人类与机器在动作建模之间的关键桥梁。当前生成模型面临两个挑战：1)从自我中心视角理解初始和目标状态的视觉场景并推理转换步骤；2)生成遵循指令且保持对象外观一致性的中间转换帧。

Method: 提出EgoIn框架：1)使用在定制数据集上微调的TransitionVLM推断两个给定状态之间的多步转换过程，减少幻觉信息；2)通过Transition Conditioning模块基于转换条件生成帧序列；3)引入Object-aware Auxiliary Supervision来保持整个转换过程中对象外观的一致性。

Result: 在人与物体和机器人与物体交互数据集上的大量实验表明，EgoIn在生成语义上有意义且视觉上连贯的转换序列方面表现出优越性能。

Conclusion: EgoIn框架有效解决了Egocentric Instructed Visual State Transition问题，能够生成符合指令、语义合理且视觉连贯的物体转换序列，为自我中心视角的物理转换理解提供了有效解决方案。
...
TL;DR: 提出SAMGA框架用于零样本EEG到图像检索，通过主体感知的多粒度对齐解决现有方法忽视EEG信号多尺度表征和主体间视觉粒度差异的问题，在THINGS-EEG基准上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有零样本EEG到图像检索方法通常依赖单一固定视觉目标或主体不变的目标构建方案，忽视了视觉诱发EEG信号的两个重要特性：1）跨多个表征尺度保存信息；2）与EEG最佳匹配的视觉粒度可能因主体而异。这限制了EEG到图像检索的鲁棒性。

Method: 提出主体感知多粒度对齐（SAMGA）框架：1）通过自适应聚合预训练视觉编码器的多个中间表示来构建主体感知的视觉监督目标；2）设计粗到细的跨模态对齐策略，共享编码器中粗阶段稳定共享语义几何并减少主体引起的分布偏移，细阶段进一步提升实例级检索判别力。

Result: 在THINGS-EEG基准测试中，该方法在主体内设置下达到91.3% Top-1和98.8% Top-5准确率，在主体间设置下达到34.4% Top-1和64.8% Top-5准确率，优于现有最先进方法。

Conclusion: SAMGA框架通过主体感知的多粒度视觉目标构建和粗到细的跨模态对齐，有效解决了EEG信号的多尺度表征特性和主体间视觉粒度差异问题，为零样本EEG到图像检索提供了更鲁棒的解决方案。
...
TL;DR: DuQuant++针对MXFP4格式提出单次异常值感知旋转方法，解决了激活异常值导致的量化误差问题，在W4A4量化下实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: MXFP4格式将张量划分为32元素块共享E8M0缩放因子，但激活异常值会提升整个块的缩放因子，压缩其他元素的有效动态范围，导致显著量化误差。现有旋转方法无法针对异常值集中的通道进行优化。

Method: 将DuQuant的异常值感知细粒度旋转适配到MXFP4格式，通过将旋转块大小与微缩放组大小对齐（B=32）。由于每个MXFP4组有独立缩放因子，消除了原始DuQuant中需要双重旋转和锯齿排列的跨块方差问题，用单次异常值感知旋转替代整个流程。

Result: 在LLaMA-3家族的MXFP4 W4A4量化实验中，DuQuant++始终实现最先进的性能，同时在线旋转成本减半并平滑权重分布。

Conclusion: DuQuant++通过单次异常值感知旋转有效解决了MXFP4格式中的激活异常值问题，在保持高效推理的同时显著提升量化性能。
...
TL;DR: 提出了一种仅使用文本表达进行弱监督的指代视频对象分割方法WSRVOS，无需像素级标注，通过对比指代表达增强、双向视觉-语言特征交互、实例感知表达分类等策略实现高质量分割。


<details>
  <summary>Details</summary>
Motivation: 传统指代视频对象分割方法需要昂贵的像素级掩码标注，现有的弱监督方法仍需要边界框或点标注。为了进一步降低标注成本，本文旨在开发仅使用文本表达进行训练的弱监督方法。

Method: 1. 对比指代表达增强：利用多模态大语言模型生成正负表达；2. 双向视觉-语言特征选择与交互；3. 实例感知表达分类；4. 正预测融合策略生成伪掩码；5. 时序片段排序约束确保相邻帧掩码预测一致性。

Result: 在A2D Sentences、J-HMDB Sentences、Ref-YouTube-VOS和Ref-DAVIS17四个公开数据集上的广泛实验证明了方法的优越性。

Conclusion: WSRVOS方法仅使用文本表达就能实现高质量的指代视频对象分割，显著降低了标注成本，为弱监督视频理解任务提供了有效解决方案。
...
TL;DR: 本文提出了一种基于分布视角的多视角一致3D场景编辑框架，通过显式引入跨视角依赖关系和双路径一致性机制，解决了现有方法中的视角不一致问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于渲染-编辑-优化的3D场景编辑方法存在严重的跨视角不一致问题。虽然近期方法引入了几何线索、跨视角交互或视频先验来缓解这一问题，但它们主要依赖推理时同步，在鲁棒性和泛化能力上仍然有限。本文从分布视角重新审视多视角一致的3D编辑，认为3D场景编辑本质上需要跨视角的联合分布建模。

Method: 1. 提出基于分布视角的多视角一致3D编辑框架，显式引入跨视角依赖关系到编辑过程中；2. 引入双路径一致性机制，包括投影引导的结构指导和补丁级语义传播，分别处理结构对应和语义连续性所需的跨视角线索；3. 构建配对的多视角编辑数据集，为学习编辑场景中的跨视角一致性提供可靠监督。

Result: 大量实验表明，该方法在复杂场景中实现了优越的编辑性能，能够生成精确且视角一致的编辑结果。

Conclusion: 通过从分布视角重新构建3D场景编辑问题，并引入显式的跨视角依赖关系和双路径一致性机制，本文提出的方法有效解决了多视角编辑中的一致性问题，为复杂3D场景的文本驱动编辑提供了更鲁棒和通用的解决方案。
...
TL;DR: Re²MoGen：一个基于推理和细化的开放词汇运动生成框架，通过增强的LLM推理生成初始运动规划，再通过强化学习后训练优化物理合理性，解决现有T2M模型在训练分布外文本描述时性能下降的问题。


<details>
  <summary>Details</summary>
Motivation: 现有文本到运动生成模型在训练数据分布内表现良好，但当运动描述与训练文本差异显著时性能急剧下降。需要解决开放词汇场景下的运动生成问题，使模型能够处理超出训练分布的新颖文本描述。
...
Result: 实验表明该框架能够生成语义一致且物理合理的运动，在开放词汇运动生成任务中达到最先进的性能表现。

Conclusion: Re²MoGen通过结合增强的LLM推理和RL后训练，有效解决了开放词汇运动生成问题，为处理超出训练分布的新颖文本描述提供了可行方案。
...
TL;DR: 提出一个两阶段框架，利用2D扩散模型从互联网视频中重建3D人体运动和人物-物体交互，解决了动态相机下全局一致3D运动重建的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有方法在动态相机下难以恢复全局一致的3D运动，特别是对于当前运动捕捉数据集中代表性不足的运动类型，并且在恢复连贯的3D人物-物体交互方面面临额外困难。

Method: 两阶段框架：第一阶段从互联网视频提取2D关键点，合成多视角2D运动数据；第二阶段训练相机条件化的多视角2D运动扩散模型，在特定领域合成数据上恢复世界空间中的3D人体运动和3D人物-物体交互。

Result: 在包含体操等挑战性运动的互联网视频以及野外人物-物体交互视频上验证了方法的有效性，在生成逼真人体运动和人物-物体交互方面优于先前工作。

Conclusion: 提出的两阶段框架能够有效从互联网视频中重建3D人体运动和人物-物体交互，特别是在处理动态相机和罕见运动类型方面表现出色。
...
TL;DR: GR4CIL是一个结合任务区分和知识路由的CLIP-based类增量学习框架，通过保持任务特定的视觉知识和稳定的共享文本语义空间来减少任务间干扰，并引入正交补偿机制增强任务内区分度。
...
Method: 提出GR4CIL框架，结合任务区分和知识路由，保持任务特定的视觉知识同时维护增量稳定的共享文本语义空间，并引入正交补偿机制来缓解模态差距引起的偏差。

Result: 在多个基准测试中，GR4CIL始终优于强基线方法，实现了更可靠的任务感知知识路由，同时保持了零样本泛化能力。

Conclusion: GR4CIL通过任务区分和知识路由的有效结合，解决了CLIP-based类增量学习中的知识漂移和跨任务校准问题，为可靠的增量学习提供了新思路。
...
TL;DR: PCM-NeRF：一种概率框架，通过可学习的相机位姿不确定性增强神经表面重建，在存在位姿异常值时优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有神经表面重建方法通常将相机位姿视为固定值，假设SfM系统提供完美位姿估计。这一假设在存在不完美位姿估计时会失效，导致重建结果扭曲或不完整。

Method: 在SG-NeRF基础上构建概率框架，为每个相机位姿学习均值和方差分布，方差从SfM对应质量初始化。通过不确定性正则化损失将学习方差与视图置信度耦合，不确定性直接调节有效位姿学习率：不确定相机获得阻尼梯度更新，防止初始化差的视图破坏重建。

Result: 在具有严重位姿异常值的挑战性场景实验中，PCM-NeRF在Chamfer距离和F-Score上持续优于最先进方法，特别是在几何复杂结构上，且不需要前景掩码。

Conclusion: PCM-NeRF通过引入可学习的相机不确定性，有效处理不完美位姿估计问题，轻量级机制无需改变渲染管线且开销可忽略，显著提升神经表面重建的鲁棒性。
...
### [139] [AI Approach for MRI-only Full-Spine Vertebral Segmentation and 3D Reconstruction in Paediatric Scoliosis](https://arxiv.org/abs/2604.17846)
*Nathasha Naranpanawa,Maree T. Izatt,Robert D. Labrom,Geoffrey N. Askin,J. Paige Little*

Main category: cs.CV

TL;DR: 该研究开发了一个AI框架，能够从MRI中自动分割胸腰椎并生成3D重建，避免了儿科脊柱畸形评估中对CT扫描的依赖。


<details>
  <summary>Details</summary>
Motivation: MRI在儿科成像中优于CT，因为它避免了电离辐射，但在脊柱畸形评估中，由于缺乏自动化的高分辨率3D骨重建，仍然依赖CT。MRI的3D重建因手动工作流程和标记完整脊柱数据集的稀缺而不实用。

Method: 使用GAN将青少年特发性脊柱侧凸患者的低剂量CT扫描转换为MRI样图像，并与现有的标记胸椎MRI数据结合，训练基于U-Net的模型，实现胸腰椎的自动分割和3D重建。

Result: 算法能够准确生成连续的胸腰椎3D重建，分割准确率达到88% Dice分数，处理时间从约1小时缩短至不到1分钟，同时保留了AIS特定的畸形特征。
...
### [140] [UniCSG: Unified High-Fidelity Content-Constrained Style-Driven Generation via Staged Semantic and Frequency Disentanglement](https://arxiv.org/abs/2604.17850)
*Jingwei Yang,Ruoxi Wu,Wei Shen,Meng Li,Yulong Liu,Huimin She,Lunxi Yuan*

Main category: cs.CV

TL;DR: UniCSG：统一的基于扩散的内容约束风格生成框架，通过分阶段训练解决内容-风格纠缠问题，提升内容保真度和风格对齐


<details>
  <summary>Details</summary>
Motivation: 基于DiT的扩散模型在风格迁移中存在内容-风格纠缠问题，导致参考内容泄漏和生成不稳定，需要统一的解决方案来同时保证内容语义保持和目标风格匹配

Method: 采用分阶段训练：1）潜在空间语义解缠阶段，结合低频预处理和条件破坏促进内容-风格分离；2）潜在空间频率感知细节重建阶段，通过多尺度频率监督细化细节；3）像素空间奖励学习，在解码后对齐潜在目标与感知质量

Result: 实验表明UniCSG在文本引导和参考引导两种设置下都提高了内容忠实度、风格对齐和鲁棒性

Conclusion: UniCSG为内容约束、风格驱动的生成提供了一个统一框架，有效解决了扩散模型中的内容-风格纠缠问题，在多种设置下都能实现更好的内容保持和风格对齐
...
### [141] [Spatiotemporal Sycophancy: Negation-Based Gaslighting in Video Large Language Models](https://arxiv.org/abs/2604.17873)
*Ziyao Tang,Pengkun Jiao,Bin Zhu,Huiyan Qi,Jingjing Chen,Yu-Gang Jiang*

Main category: cs.CV

TL;DR: 视频大语言模型在否定式心理操纵下会出现时空谄媚行为，即撤回原本正确的视觉判断并迎合误导性用户反馈，甚至编造虚假时空解释来合理化错误修正。


<details>
  <summary>Details</summary>
Motivation: 视频大语言模型在视频理解任务中表现出色，但其在对话交互中的鲁棒性尚未得到充分探索。研究者发现这些模型在面对否定式心理操纵时，会撤回原本正确的视觉判断并迎合误导性用户反馈，这种现象被称为时空谄媚。

Method: 提出了一个否定式心理操纵评估框架，并创建了GasVideo-1000基准测试集，专门用于探测时空谄媚行为。该基准具有清晰的视觉基础和时序推理要求，评估了广泛的开源和专有视频大语言模型。

Result: 实验表明，即使基线性能很强的模型也普遍且严重地容易受到否定式心理操纵的影响。虽然提示级别的约束可以部分缓解这种行为，但不能可靠地防止幻觉解释或信念逆转。

Conclusion: 当前的视频大语言模型缺乏在对抗性对话反馈下保持时空信念的鲁棒机制，需要开发更强大的机制来确保模型在对话交互中保持视觉基础判断的稳定性。
...
### [142] [AeroRAG: Structured Multimodal Retrieval-Augmented LLM for Fine-Grained Aerial Visual Reasoning](https://arxiv.org/abs/2604.17889)
*Junxiao Xue,Quan Deng,Tingqi Hu,Meicong Si,Xinyi Yin,Yunyun Shi,Xuecheng Wu*

Main category: cs.CV

TL;DR: AeroRAG：基于场景图引导的多模态检索增强生成框架，通过结构化视觉知识（物体类别、数量、空间位置、语义关系）替代密集视觉标记表示，提升航空场景视觉问答的可靠性


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在航空场景视觉问答中面临挑战，因为任务关键证据通常由小物体、明确数量、粗略位置和物体间关系承载，而传统的密集视觉标记表示与这些结构化语义不对齐

Method: 提出AeroRAG框架：1）将输入图像转换为结构化视觉知识（物体类别、数量、空间位置、语义关系）；2）检索查询相关的语义块构建紧凑提示；3）使用基于文本的大语言模型进行推理，在感知和语言推理之间引入更明确的中间接口

Result: 在AUG航空数据集和通用领域VG-150基准测试中，相比六个强大的MLLM基线模型取得一致改进，在密集航空场景和关系敏感推理中观察到最大增益；在VQAv2上的评估验证了该框架与标准视觉推理设置的兼容性

Conclusion: 结构化检索是面向部署和基于视觉推理系统的实用设计方向，通过显式中间接口解决感知与语言推理之间的对齐问题
...
### [143] [ReTrack: Evidence-Driven Dual-Stream Directional Anchor Calibration Network for Composed Video Retrieval](https://arxiv.org/abs/2604.17898)
*Zixu Li,Yupeng Hu,Zhiwei Chen,Qinlei Huang,Guozhi Qiu,Zhiheng Fu,Meng Liu*

Main category: cs.CV

TL;DR: ReTrack是一个用于组合视频检索（CVR）的新框架，通过校准组合特征中的方向性偏差来解决模态贡献纠缠、组合特征显式优化和检索不确定性三大挑战，在CVR和CIR任务上都取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 传统组合方法在CVR任务中倾向于偏向参考视频，导致检索性能不佳。这主要源于三个核心挑战：模态贡献纠缠、组合特征显式优化困难以及检索不确定性。视频和文本模态之间存在显著的信息密度差异，需要新的方法来更好地理解多模态查询。

Method: 提出ReTrack框架，包含三个关键模块：语义贡献解纠缠、组合几何校准和可靠证据驱动对齐。首先估计每个模态的语义贡献来校准组合特征的方向性偏差，然后使用校准后的方向锚点计算双向证据，驱动可靠的组合到目标相似度估计。

Result: 在三个基准数据集上，ReTrack在CVR和CIR任务中都取得了最先进的性能，表现出强大的泛化能力。代码已开源。

Conclusion: ReTrack是第一个通过校准组合特征中的方向性偏差来改善多模态查询理解的CVR框架，有效解决了传统方法的局限性，为组合视频检索提供了新的解决方案。
...
### [144] [Beyond Binary Contrast: Modeling Continuous Skeleton Action Spaces with Transitional Anchors](https://arxiv.org/abs/2604.17914)
*Yingjie Feng,Yi Wang,Jiaze Wang,Anfeng Liu,Zhuotao Tian*

Main category: cs.CV

TL;DR: TranCLR提出了一种基于过渡锚点的对比学习框架，用于骨架动作识别，通过建模动作空间的连续几何结构来改善传统对比学习对运动连续性的忽视。


<details>
  <summary>Details</summary>
Motivation: 现有自监督对比学习方法使用二元对比目标，忽略了人体运动的固有连续性，导致特征簇碎片化和类别边界僵化，需要一种能捕捉动作空间连续几何结构的方法。

Method: 提出TranCLR框架：1) 动作过渡锚点构建(ATAC)显式建模过渡状态的几何结构；2) 多级几何流形校准(MGMC)机制自适应校准多个连续性级别的动作流形。

Result: 在NTU RGB+D、NTU RGB+D 120和PKU-MMD数据集上的实验表明，TranCLR实现了优越的准确性和校准性能，有效学习了连续且不确定性感知的骨架表示。

Conclusion: TranCLR通过建模动作空间的连续几何结构，解决了传统对比学习对运动连续性的忽视问题，为骨架动作识别提供了更平滑、更具区分性的表示空间。
...
### [145] [Prompting Foundation Models for Zero-Shot Ship Instance Segmentation in SAR Imagery](https://arxiv.org/abs/2604.17920)
*Islam Mansour,Francescopaolo Sica,Michael Schmitt*

Main category: cs.CV

TL;DR: 该论文提出了一种零样本SAR图像船舶实例分割方法，使用YOLOv11检测器定位船舶边界框，然后通过Segment Anything Model 2生成实例掩码，无需像素级标注。


<details>
  <summary>Details</summary>
Motivation: 合成孔径雷达在海上监视中至关重要，但深度学习在SAR分析中受到像素级标注缺乏的限制。需要探索通用视觉基础模型如何实现SAR图像中的零样本船舶实例分割。

Method: 使用在开放SAR数据集上训练的YOLOv11检测器通过边界框定位船舶，然后利用Segment Anything Model 2生成实例掩码。不同于先前依赖微调或适配器的SAM方法，该方法仅通过SAR训练检测器的空间约束来正则化基础模型预测。

Result: 在SSDD基准测试中达到0.637的平均IoU（达到全监督基线的89%），总体船舶检测率为89.2%。该方法部分缓解了光学-SAR域差距，并支持船舶分类、尺寸估计和尾迹分析等下游应用。

Conclusion: 该方法展示了一种可扩展、标注高效的途径，利用基础模型驱动的SAR图像理解，仅通过边界框标注就能实现接近全监督性能的零样本实例分割。
...
### [146] [Brain-Inspired Capture: Evidence-Driven Neuromimetic Perceptual Simulation for Visual Decoding](https://arxiv.org/abs/2604.17927)
*Feixue Shao,Guangze Shi,Xueyu Liu,Yongfei Wu,Mingqiang Wei,Jianan Zhang,Jianbo Lu,Guiying Yan,Weihua Yang*

Main category: cs.CV

TL;DR: BI-Cap是一种神经模拟感知模拟范式，通过模拟人类视觉系统处理来对齐神经和视觉模态，显著提升零样本脑到图像检索性能。


<details>
  <summary>Details</summary>
Motivation: 当前脑机接口和计算神经科学中的视觉解码方法受限于神经和视觉模态之间的系统性和随机性差距，忽视了人类视觉系统的内在计算机制。

Method: 提出BI-Cap范式，构建包含四个生物合理动态和静态转换的神经模拟流水线，结合互信息引导的动态模糊调节来模拟自适应视觉处理，并引入证据驱动的潜在空间表示来建模不确定性。

Result: 在两个公共基准测试的零样本脑到图像检索任务中，BI-Cap显著优于现有最先进方法，分别实现了9.2%和8.0%的相对增益提升。

Conclusion: BI-Cap通过模拟人类视觉系统处理来对齐神经和视觉模态，为解决脑机接口中的视觉解码挑战提供了有效的神经模拟感知模拟方法。
...
### [147] [ZSG-IAD: A Multimodal Framework for Zero-Shot Grounded Industrial Anomaly Detection](https://arxiv.org/abs/2604.17949)
*Qiuhui Chen,Jiaxiang Song,Shuai Tan,Weimin Zhong*

Main category: cs.CV

TL;DR: ZSG-IAD是一个多模态视觉语言框架，用于零样本工业异常检测，能生成结构化异常报告和像素级异常掩码，通过语言引导的两步定位模块提高可解释性。


<details>
  <summary>Details</summary>
Motivation: 当前基于深度学习的工业异常检测器通常表现为黑盒模型，难以用物理上有意义的缺陷证据来证明决策的合理性，需要更透明、可解释的检测系统。

Method: 提出ZSG-IAD框架，包含语言引导的两步定位模块：1）异常相关句子从多模态特征中提取证据式潜在槽，获得粗略空间支持；2）通过通道-空间门控和轻量解码器调制特征图，生成细粒度掩码。使用可执行规则GRPO和可验证奖励来提升可靠性。

Result: 在多个工业异常基准测试中表现出强大的零样本性能，比先前方法提供更透明、物理基础更强的解释。

Conclusion: ZSG-IAD为可信赖的工业异常检测系统提供了有效的多模态框架，能够生成结构化异常报告和像素级定位，提高检测的可解释性和可靠性。
...
### [148] [Chatting about Upper-Body Expressive Human Pose and Shape Estimation](https://arxiv.org/abs/2604.17959)
*Yuxiang Zhao,Wei Huang,Yujie Song,Liu Wang,Huan Zhao*

Main category: cs.CV

TL;DR: CoEvoer：首个专为上肢体姿态与形状估计设计的单阶段协同跨依赖Transformer框架，通过显式特征交互实现面部、手部和躯干之间的相互增强，在基准测试和野外图像上均表现出色。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的表达性人体姿态与形状估计方法在面部和手部区域参数估计上仍不准确，且对野外图像的泛化能力有限。需要一种能够处理上肢体各部位强耦合关系的专门框架。

Method: CoEvoer采用单阶段协同跨依赖Transformer架构，实现不同身体部位间的显式特征级交互。利用躯干等大区域提供全局语义和位置先验来指导面部和手部等精细区域估计，同时面部和手部细节帮助校准相邻身体部位。

Result: 在多个上肢体基准测试中达到最先进性能，并在未见过的野外图像上展现出强大的泛化能力。

Conclusion: CoEvoer是首个专门为上肢体表达性姿态与形状估计设计的框架，通过联合参数回归捕捉面部、手部和躯干之间的强耦合关系，显著提升了估计精度和泛化能力。
...
### [149] [DifFoundMAD: Foundation Models meet Differential Morphing Attack Detection](https://arxiv.org/abs/2604.17961)
*Lazaro J. Gonzalez-Soler,André Dörsch,Christian Rathgeb,Christoph Busch*

Main category: cs.CV

TL;DR: DifFoundMAD是一种参数高效的差分形态攻击检测框架，利用视觉基础模型的泛化能力来捕捉可疑变形图像与真实捕获图像之间的差异，在严格安全级别下显著降低错误率。


<details>
  <summary>Details</summary>
Motivation: 传统D-MAD系统依赖人脸识别嵌入或手工特征差异，缺乏对变形攻击的有效检测能力。需要利用基础模型的强大泛化能力来捕捉更细微的差异，同时保持参数效率以适应实际部署需求。

Method: 采用标准差分范式，但将底层表示空间替换为基础模型提取的嵌入。结合轻量级微调和类别平衡优化，只更新一小部分参数，同时保留基础模型的丰富表示先验。

Result: 在标准D-MAD基准测试的跨数据库评估中，DifFoundMAD相比现有最优系统取得一致改进。在边境控制等操作部署所需的高安全级别下，错误率从6.16%降至2.17%。

Conclusion: DifFoundMAD通过利用基础模型的泛化能力，在保持参数效率的同时显著提升了差分形态攻击检测性能，特别是在严格安全要求下的实际应用场景中表现出色。
...
### [150] [MU-GeNeRF: Multi-view Uncertainty-guided Generalizable Neural Radiance Fields for Distractor-aware Scene](https://arxiv.org/abs/2604.17965)
*Wenjie Mu,Zhan Li,Chuanzhou Su,Xuanyi Shen,Ziniu Liu,Fan Lu,Yujian Mo,Junqiao Zhao,Tiantian Feng,Chen Ye,Guang Chen*

Main category: cs.CV

TL;DR: MU-GeNeRF：一种多视角不确定性引导的干扰物感知通用神经辐射场框架，通过在存在瞬时干扰的情况下分解源视角和目标视角不确定性，提升场景重建的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现实世界中瞬时干扰物会破坏跨视角结构一致性，影响监督信号并降低重建质量。现有基于单场景优化的干扰物去除方法不适用于通用神经辐射场，且常将不一致的静态结构误判为干扰物。

Method: 提出多视角不确定性引导框架，将干扰物感知分解为两个互补的不确定性组件：源视角不确定性（捕捉因视角变化或动态因素引起的结构差异）和目标视角不确定性（检测目标图像中的观测异常）。通过异方差重建损失结合这两种不确定性，自适应调节监督信号。

Result: 大量实验表明，该方法不仅超越了现有通用神经辐射场方法，还达到了与场景特定干扰物去除神经辐射场相当的性能。

Conclusion: MU-GeNeRF通过多视角不确定性分解有效解决了通用神经辐射场在瞬时干扰存在时的鲁棒建模挑战，实现了更稳健的干扰物抑制和几何建模。
...
### [151] [E3VS-Bench: A Benchmark for Viewpoint-Dependent Active Perception in 3D Gaussian Splatting Scenes](https://arxiv.org/abs/2604.17969)
...
TL;DR: E3VS-Bench是一个用于评估智能体在5自由度视角控制下进行3D视觉搜索的基准测试，包含99个高保真3D场景和2,014个问题驱动任务，旨在评估视角依赖的主动感知能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉搜索和具身AI基准测试（如EQA）通常依赖静态观察或受限的自我中心运动，无法充分评估在真实3D环境中无限制5自由度视角控制下出现的细粒度视角依赖现象，如垂直视角变化导致的可见性变化、容器内部内容揭示以及仅从特定角度可观察的对象属性区分。

Method: 使用3D高斯泼溅技术重建99个高保真3D场景，实现保留细粒度视觉细节（如小文本和微妙属性）的光真实感自由视角渲染。构建2,014个问题驱动任务，这些问题无法从单一视角回答，需要智能体在5自由度视角下进行主动检查。

Result: 评估了多个最先进的视觉语言模型，并与人类表现进行比较。尽管这些模型在2D推理方面表现出色，但所有模型与人类表现存在显著差距，突显了在完整5自由度视角变化下主动感知和连贯视角规划方面的局限性。

Conclusion: E3VS-Bench填补了现有基准测试的空白，能够评估智能体在真实3D环境中进行视角依赖的视觉搜索能力。研究结果表明当前模型在主动感知和5自由度视角规划方面仍有不足，为未来具身AI研究提供了重要方向。
...
### [152] [Identifying Ethical Biases in Action Recognition Models](https://arxiv.org/abs/2604.17971)
...
<details>
  <summary>Details</summary>
Motivation: 人类动作识别模型越来越多地部署在高风险环境中，但现有研究缺乏对其在不同人类外观（如肤色）上公平性的分析。需要一种能够控制变量、隔离特定属性影响的方法来审计这些模型的偏见。

Method: 使用BEDLAM仿真平台生成合成视频数据，完全控制视觉身份属性（如肤色），同时保持时间一致性。通过受控干预，测试当动作保持相同时，单一属性变化对流行HAR模型预测的影响。

Result: 研究表明，即使动作完全相同，一些流行的HAR模型在肤色上表现出统计显著的偏见。模型可能编码了不需要的视觉关联，并且在不同群体间存在系统性错误。

Conclusion: 该工作提供了一个审计HAR模型偏见的框架，支持开发更透明、可问责的系统，以应对即将到来的监管标准。强调了在动作识别中考虑公平性的重要性。
...
### [153] [Mitigating Multimodal Hallucination via Phase-wise Self-reward](https://arxiv.org/abs/2604.17982)
*Yu Zhang,Chuyang Sun,Kehai Chen,Xuefeng Bai,Yang Xiang,Min Zhang*

Main category: cs.CV

TL;DR: 提出PSRD框架，通过阶段式自奖励解码动态缓解视觉幻觉，无需外部监督，显著降低幻觉率


<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖大规模标注数据进行微调（计算开销大），要么采用静态事后策略（忽略幻觉出现的动态特性），需要一种能在推理时动态缓解幻觉且无需外部监督的方法

Method: 提出PSRD（阶段式自奖励解码）框架：1）发现视觉幻觉呈现阶段式动态模式；2）使用阶段式自奖励信号指导在线幻觉校正；3）将幻觉指导信号从大视觉语言模型蒸馏到轻量级奖励模型中，在解码过程中提供实时指导

Result: PSRD将LLaVA-1.5-7B的幻觉率降低了50.0%，在五个幻觉评估基准上对四个大视觉语言模型均优于现有事后方法，有效缓解幻觉传播并实现性能与推理效率的可控权衡

Conclusion: PSRD框架通过阶段式自奖励解码实现了无需外部监督的动态幻觉缓解，显著提升了大视觉语言模型的可靠性，同时保持了推理效率
...
TL;DR: 提出HABIT框架解决组合图像检索中的噪声三元组对应问题，通过互知识估计模块和双一致性渐进学习模块提升噪声环境下的检索鲁棒性


<details>
  <summary>Details</summary>
Motivation: 组合图像检索在实际应用中面临噪声三元组对应问题，主要源于标注成本高和主观性强，导致训练数据存在噪声，影响检索性能

Method: 提出HABIT框架：1) 互知识估计模块通过计算组合特征与目标图像间互信息的转移率来量化样本清洁度；2) 双一致性渐进学习模块通过历史模型与当前模型的协同机制，模拟人类习惯形成过程，实现鲁棒学习

Result: 在两个标准CIR数据集上的实验表明，HABIT在不同噪声比例下显著优于大多数方法，展现出优越的鲁棒性和检索性能

Conclusion: HABIT框架有效解决了组合图像检索中的噪声三元组对应问题，通过互知识估计和渐进学习机制实现了在噪声环境下的鲁棒检索性能
...
TL;DR: GS-STVSR是一种基于2D高斯泼溅的超高效连续时空视频超分辨率框架，通过高斯核的时空演化绕过密集网格查询，在保持SOTA质量的同时实现恒定推理时间，在极端尺度上速度提升3倍以上。
...
Result: 在Vid4、GoPro和Adobe240数据集上的实验表明，GS-STVSR在所有基准测试中达到最先进的质量水平。推理时间在常规时间尺度（X2-X8）保持近乎恒定，在极端尺度X32上实现超过3倍的加速。

Conclusion: GS-STVSR通过2D高斯泼溅和连续运动建模，成功解决了现有C-STVSR方法的效率瓶颈，在保持高质量的同时实现了显著的速度提升，具有强大的实际应用价值。
...
### [156] [Enhancing Continual Learning of Vision-Language Models via Dynamic Prefix Weighting](https://arxiv.org/abs/2604.18075)
*Hyeonseo Jang,Hyuk Kwon,Kibok Lee*

Main category: cs.CV

TL;DR: 提出动态前缀加权（DPW）框架，通过动态调整前缀权重和适配器权重，提升视觉语言模型在领域-类别增量学习中的性能。


<details>
  <summary>Details</summary>
Motivation: 现有参数高效方法（如前缀调优或适配器）通常对向量权重进行归一化，忽视了不同输入token需要不同程度的调整。需要一种能动态分配权重的机制来提升模型适应性。

Method: 提出DPW框架：1）门控模块根据输入token的重要性动态调整每个前缀的权重；2）权重机制将适配器输出权重作为前缀调优权重的残差，确保仅在必要时使用适配器。

Result: 实验结果表明，该方法在视觉语言模型的领域-类别增量学习场景中实现了最先进的性能。

Conclusion: DPW通过动态权重分配机制有效解决了现有方法忽视不同token调整需求的问题，为视觉语言模型的增量学习提供了更优的解决方案。
...
### [157] [Class-specific diffusion models improve military object detection in a low-data domain](https://arxiv.org/abs/2604.18076)
*Ella P. Fokkinga,Jan Erik van Woerden,Thijs A. Eker,Sebastiaan P. Snel,Elfi I. S. Hofmeijer,Klamer Schutte,Friso G. Heslinga*

Main category: cs.CV

TL;DR: 使用扩散模型生成合成图像提升军事车辆检测性能，特别是在数据稀缺情况下，结合结构引导进一步改善效果


<details>
  <summary>Details</summary>
Motivation: 研究扩散模型生成的合成图像是否能改善军事车辆检测在低数据条件下的性能，探索替代传统仿真管道的生成式图像数据方案

Method: 使用LoRA微调FLUX.1扩散模型，每类仅用8或24张真实图像生成15类车辆合成数据；同时训练RF-DETR检测器；还开发了FLUX-ControlNet方法，通过Canny边缘图控制视角和姿态

Result: FLUX生成图像显著提升检测性能，在8个真实样本时mAP50提高8.0%；FLUX-CN在数据稀缺时进一步改善4.1%，但在数据充足时无额外增益

Conclusion: 特定对象的扩散模型能有效提升军事目标检测在低数据域的性能，结构引导在真实数据极度有限时最有益，生成式图像数据可作为军事AI系统训练的传统仿真管道替代方案
...
### [158] [Autonomous Unmanned Aircraft Systems for Enhanced Search and Rescue of Drowning Swimmers: Image-Based Localization and Mission Simulation](https://arxiv.org/abs/2604.18088)
*Sascha Emanuel Zell,Toni Schneidereit,Armin Fügenschuh,Michael Breuß*

Main category: cs.CV

TL;DR: 该研究提出基于无人机系统的水上救援方案，通过YOLO算法自动检测溺水者，并利用离散事件仿真评估救援响应时间，结果显示无人机辅助可将响应时间缩短至传统救援的五分之一。


<details>
  <summary>Details</summary>
Motivation: 水上溺水救援面临时间压力大、水域广阔无人监管、目标定位困难、救援人员运输不便等挑战，需要技术创新来缩短响应时间并提高救援效率。

Method: 1) 开发无人机箱式系统，在游泳区域附近部署无人机机库；2) 使用YOLO目标检测架构自动定位溺水者，创建专用数据集并训练模型；3) 采用离散事件仿真方法模拟传统救援和无人机救援的响应时间。

Result: 1) 评估了YOLO v3、v5、v8不同架构尺寸的性能；2) 在德国劳西茨湖区测试显示，即使只有两个机库各含一架无人机的小型系统，也能将响应时间缩短至传统救援的五分之一。

Conclusion: 无人机系统结合YOLO目标检测技术能有效缩短水上救援响应时间，为无人监管水域提供可行的自动化救援解决方案，显著提高溺水救援成功率。
...
### [159] [Decision-Aware Attention Propagation for Vision Transformer Explainability](https://arxiv.org/abs/2604.18094)
*Sehyeong Jo,Gangjae Jang,Haesol Park*

Main category: cs.CV

TL;DR: 提出DAP方法，通过将梯度定位的决策相关先验注入Transformer注意力传播中，生成更类敏感、紧凑和可信的归因图，提升ViT可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有注意力解释方法主要依赖原始注意力权重，不能明确反映最终决策且类区分性有限；而梯度定位方法虽能突出类特定证据，但未充分利用Transformer的层次注意力传播机制。

Method: 提出决策感知注意力传播(DAP)，通过梯度定位估计token重要性，并将其集成到逐层注意力展开中，同时捕捉注意力结构流和与最终预测最相关的证据。

Result: DAP在多个不同规模的Vision Transformer变体上，在定量指标和定性可视化方面均优于现有基线方法，表明决策感知传播是改进ViT可解释性的有效方向。

Conclusion: DAP方法通过将决策相关先验注入注意力传播，能够生成更类敏感、紧凑和可信的归因图，有效提升了Vision Transformer的可解释性。
...
### [160] [Test-Time Perturbation Learning with Delayed Feedback for Vision-Language-Action Models](https://arxiv.org/abs/2604.18107)
*Zehua Zang,Xi Wang,Fuchun Sun,Xiao Xu,Lixiang Lium,Jiahuan Zhou,Jiangmeng Li*

Main category: cs.CV

TL;DR: PDF框架通过不确定性数据增强和延迟反馈的扰动学习，提升视觉语言动作模型在环境变化下的决策鲁棒性，无需微调基础模型。


<details>
  <summary>Details</summary>
Motivation: 视觉语言动作模型在顺序决策中表现出色，但对环境微小变化（如物体姿态变化）非常脆弱。这种脆弱性源于轨迹过拟合，即模型过度关注动作与实体之间的虚假相关性，并复制记忆的动作模式。

Method: 提出PDF框架：1）通过不确定性数据增强和动作投票来缓解虚假相关性；2）自适应调度器平衡性能与效率；3）学习轻量级扰动模块，利用延迟反馈回顾性调整动作logits，纠正过度自信问题。

Result: 在LIBERO数据集上提升7.4%成功率，在Atari上提升10.3%人类标准化分数，相比原始VLA和带测试时适应的VLA都有显著提升。

Conclusion: PDF为多模态决策智能体提供了一种实用的测试时适应路径，无需微调基础模型即可提升决策性能，增强了模型对环境变化的鲁棒性。
...
### [161] [Can LLM-Generated Text Empower Surgical Vision-Language Pre-training?](https://arxiv.org/abs/2604.18134)
*Chengan Che,Chao Wang,Jiayuan Huang,Xinyue Chen,Luis C. Garcia-Peraza-Herrera*

Main category: cs.CV

TL;DR: SurgLIME是一个参数高效的视觉语言预训练框架，使用LLM生成的噪声叙述进行外科手术多模态对齐，通过LoRA适配和置信度估计机制保护医学先验知识。


<details>
  <summary>Details</summary>
Motivation: 现有外科手术视觉编码器在多模态推理任务中受到专家文本标注成本高昂的限制，而使用LLM生成的大规模数据集虽然可扩展但存在错误和幻觉风险，可能破坏预训练的医学先验知识。

Method: 提出SurgLIME框架：1) 使用LoRA适配的双编码器架构保护基础医学先验；2) 引入自动置信度估计机制，在对比对齐中动态降低不确定文本的权重；3) 基于LLM生成的LIME数据集进行训练。

Result: 在AutoLaparo和Cholec80基准测试中，SurgLIME实现了竞争力的零样本跨模态对齐，同时保持了视觉基础模型的稳健线性探测性能。

Conclusion: SurgLIME通过参数高效的VLP框架，成功利用噪声叙述进行可靠的多模态对齐，解决了外科手术领域大规模多模态数据标注的扩展性问题，同时保护了关键的医学先验知识。
...
### [162] [Region-Grounded Report Generation for 3D Medical Imaging: A Fine-Grained Dataset and Graph-Enhanced Framework](https://arxiv.org/abs/2604.18145)
...
TL;DR: 该研究针对3D PET/CT影像的自动报告生成问题，提出了首个针对低资源语言的大规模RoI标注数据集VietPET-RoI，以及模拟放射科医生工作流程的HiRRA框架，通过图关系模块捕捉RoI属性依赖关系，显著提升了报告生成质量。


<details>
  <summary>Details</summary>
Motivation: 当前医学报告生成方法面临两大挑战：1）3D体积数据的高维特性与标注数据稀缺（特别是低资源语言）；2）现有黑盒方法直接将整个体积映射到报告，忽略了放射科医生分析局部感兴趣区域（RoI）来得出诊断结论的临床工作流程。

Method: 1）创建VietPET-RoI数据集：包含600个PET/CT样本和1,960个手动标注的RoI，配有相应临床报告；2）提出HiRRA框架：模拟放射科医生诊断流程，使用基于图的关系模块捕捉RoI属性间的依赖关系；3）引入新的临床评估指标：RoI覆盖率和RoI质量指数，使用LLM提取测量RoI定位准确性和属性描述保真度。

Result: 实验评估显示，HiRRA框架在BLEU指标上超越现有模型19.7%，在ROUGE-L上提升4.7%，在临床指标上实现了45.8%的显著改进，表明临床可靠性增强且幻觉减少。

Conclusion: 该研究通过引入首个针对低资源语言的大规模RoI标注数据集和模拟临床工作流程的框架，成功解决了3D PET/CT报告生成中的关键挑战，显著提升了自动报告生成的临床相关性和可靠性，为医学影像分析提供了新的方向。
...
### [163] [Attention-ResUNet for Automated Fetal Head Segmentation](https://arxiv.org/abs/2604.18148)
*Ammar Bhilwarawala,Mainak Bandyopadhyay*

Main category: cs.CV

TL;DR: 本文提出Attention-ResUNet架构，结合残差学习和多尺度注意力机制，用于胎儿头部超声图像分割，在HC18数据集上达到99.30%的Dice分数，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在超声图像分割中面临低对比度、噪声和复杂解剖边界等挑战，需要更鲁棒和准确的胎儿头部分割方法以支持产前护理中的生物测量。

Method: 提出Attention-ResUNet架构，在四个解码器层级集成注意力门机制以选择性关注解剖相关区域并抑制背景噪声，同时使用残差连接促进梯度流动和特征重用。

Result: 在HC18数据集（n=200）上，Attention-ResUNet达到99.30±0.14%的平均Dice分数，显著优于ResUNet（99.26%）、Attention U-Net（98.79%）、Swin U-Net（98.60%）、Standard U-Net（98.58%）和U-Net++（97.46%），统计显著性p<0.001。

Conclusion: Attention-ResUNet为胎儿头部超声图像分割建立了新的最先进性能，同时保持了计算效率（14.7M参数，45 GFLOPs推理成本），并通过显著性图分析展示了增强的可解释性，有利于临床部署。
...
### [164] [AI-based Waste Mapping for Addressing Climate-Exacerbated Flood Risk](https://arxiv.org/abs/2604.18151)
*Steffen Knoblauch,Levi Szamek,Iddy Chazua,Benedcto Adamu,Innocent Maholi,Alexander Zipf*

Main category: cs.CV

TL;DR: AI技术结合开放影像数据，实现达累斯萨拉姆城市垃圾高分辨率制图，揭示垃圾分布与洪水风险关系


<details>
  <summary>Details</summary>
Motivation: 非洲快速城市化地区面临气候变化相关的洪水风险，垃圾管理不善堵塞排水系统加剧洪水灾害，需要高效监测方法

Method: 基于AI的城市垃圾制图工作流，利用公开可用的航空和街景影像进行高分辨率垃圾检测，结合本地合作伙伴进行文化相关数据标注

Result: 水道区域垃圾堆积量是相邻城市区域的三倍，识别出关键洪水风险热点；垃圾分布与非正规住区和社会经济因素相关

Conclusion: AI方法可扩展实现全市监测，为城市规划、气候适应和可持续垃圾管理提供可操作见解，特别适用于洪水易发城市区域
...
### [165] [Embedding Arithmetic: A Lightweight, Tuning-Free Framework for Post-hoc Bias Mitigation in Text-to-Image Models](https://arxiv.org/abs/2604.18167)
*Venkatesh Thirugnana Sambandham,Torsten Schön*

Main category: cs.CV

TL;DR: 提出一种推理时方法，通过分析嵌入空间结构来缓解文本到图像模型的社会偏见，同时保持提示语义和视觉上下文，无需修改模型权重、提示或数据集。


<details>
  <summary>Details</summary>
Motivation: 现代文本到图像模型会放大有害的社会偏见，这对其伦理部署构成挑战。需要一种方法能够可靠地缓解社会偏见，同时保持提示语义和视觉上下文（背景、布局和风格）的完整性。

Method: 使用嵌入算术分析嵌入空间中偏见的组织结构，并在推理时进行校正。该方法不改变模型权重、提示或数据集，通过条件嵌入空间分析发现其形成复杂纠缠的流形而非解耦概念网格。提出概念一致性评分来评估语义保留。

Result: 在FLUX 1.0-Dev和Stable Diffusion 3.5-Large上的实验表明，该方法在提高多样性的同时保持高概念一致性，显著优于现有基线，有效解决了公平性与一致性的权衡问题。
...
### [166] [Extending One-Step Image Generation from Class Labels to Text via Discriminative Text Representation](https://arxiv.org/abs/2604.18168)
*Chenxi Zhao,Chen Zhu,Xiaokun Feng,Aiming Hao,Jiashu Zhu,Jiachen Lei,Jiahong Wu,Xiangxiang Chu,Jufeng Yang*

Main category: cs.CV

TL;DR: 该论文将MeanFlow从类别条件生成扩展到文本条件生成，发现传统训练策略结合LLM文本编码器效果不佳，通过分析揭示文本特征需要高区分度，提出适配MeanFlow的文本条件生成方法。


<details>
  <summary>Details</summary>
Motivation: 现有MeanFlow研究主要关注类别到图像生成，但将条件从固定类别标签扩展到灵活文本输入可实现更丰富的内容创作。文本条件相比有限类别标签对模型理解能力提出更大挑战，需要将强大文本编码器有效集成到MeanFlow框架中。

Method: 通过详细分析发现，由于MeanFlow生成步骤极少（如仅一步），文本特征表示需要具备足够高的区分度。利用已验证具备所需语义特性的强大LLM文本编码器，并将MeanFlow生成过程适配到该框架中，首次实现高效的文本条件合成。

Result: 成功将MeanFlow扩展到文本条件生成，在广泛使用的扩散模型上验证了该方法，展示了显著的生成性能提升。该方法为未来文本条件MeanFlow生成研究提供了通用实用参考。

Conclusion: 该工作揭示了文本特征区分度对少步生成的重要性，提出了有效的文本条件MeanFlow生成方法，为文本条件少步生成提供了新思路和技术方案。
...
TL;DR: 提出了一种结合扩散模型和分割模型（RemoteSAM/SAM3）的混合管道，用于遥感图像中的目标定位，相比现有方法在Acc@0.5指标上提升超过14%。
...
Method: 提出混合管道，集成基于扩散的定位线索与最先进的分割模型（RemoteSAM和SAM3），通过结合生成扩散模型和基础分割模型的优势来获得更准确的边界框。

Result: 实验表明该管道显著提升了定位性能，在Acc@0.5指标上相比现有最先进方法提高了超过14%。

Conclusion: 扩散模型与分割模型的结合为遥感图像目标定位提供了有效解决方案，展示了生成模型在视觉定位任务中的潜力。
...
TL;DR: 该论文提出了一个专注于几何精度的神经渲染方法评估流程和包含19个多样化场景的基准测试，弥补了传统视觉指标对表面几何保真度评估的不足。


<details>
  <summary>Details</summary>
Motivation: 现有神经渲染方法评估主要关注生成图像的视觉质量，但忽视了表面几何的保真度。在机器人学等应用中，准确的几何信息对于抓取和物体操作等任务至关重要。

Method: 提出了一个专注于几何精度的神经渲染方法评估流程，并构建了一个包含19个多样化场景的基准测试，能够系统评估重建方法在表面和形状保真度方面的表现。

Result: 开发了一个评估框架，能够补充传统视觉指标，为神经渲染方法提供更全面的几何精度评估。

Conclusion: 该研究填补了神经渲染方法评估中几何精度评估的空白，为机器人学等需要精确几何信息的应用领域提供了重要的评估工具。
...
TL;DR: 提出解耦框架分离记忆条件与生成，通过轻量级独立记忆分支学习空间一致性，使用混合记忆表示和逐帧交叉注意力机制，结合相机感知门控机制，在降低训练成本的同时提升空间一致性和新场景生成能力。
...
Method: 采用解耦框架，分离记忆条件与生成模块。使用轻量级独立记忆分支学习空间一致性：1）引入混合记忆表示捕捉生成帧的时空线索；2）采用逐帧交叉注意力机制确保每帧仅基于最相关的历史信息进行条件化；3）提出相机感知门控机制调节记忆与生成模块的交互。

Result: 相比现有方法，本方法数据效率高，训练成本显著降低，同时在视觉质量和空间一致性方面达到最先进性能。

Conclusion: 解耦记忆条件与生成的框架能有效解决现有方法在空间一致性、场景重访和新区域探索方面的问题，在降低训练成本的同时提升性能。
...
TL;DR: 该论文提出S-EGIU框架，将指令理解建模为动态的"指令作为状态"变量，通过粗粒度到细粒度的状态条件化分段激活和语义精炼，实现指令语义随视觉环境变化的动态适应。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言导航模型通常将指令编码为静态全局表示，限制了指令语义随视觉上下文动态演变的能力。指令含义会随着智能体视野和空间环境的变化而改变，需要动态的指令-感知纠缠机制。

Method: 提出状态纠缠环境引导指令理解(S-EGIU)框架：1)粗粒度层面激活与当前观察对齐的指令分段；2)细粒度层面通过观察引导的token定位和上下文建模精炼激活分段语义，保持指令状态随感知状态连续更新。

Result: 在REVERIE Test Unseen上获得+2.68% SPL提升，在多个VLN基准测试中展现一致的效率增益，验证了动态指令-感知纠缠的价值。

Conclusion: 将指令理解建模为动态的"指令作为状态"变量，通过S-EGIU框架实现指令语义随视觉环境的自适应调整，显著提升了视觉语言导航的性能和效率。
...
TL;DR: 本文系统评估了SAM3在病理图像分割中的能力，发现其在零样本、少样本和监督设置下表现有限，特别是在核级分割方面，文本提示效果不佳，性能对视觉提示类型和数量敏感，与任务特定适配器存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 数字病理分割在组织级和核级尺度上面临高标注成本和泛化能力差的问题。SAM3引入了可提示概念分割，可能通过文本提示提供自动化接口。本研究旨在系统评估SAM3在病理图像分割中的能力边界。

Method: 提出系统评估协议，在不同监督设置下评估SAM3：零样本、少样本和监督学习，使用不同的提示策略。在NuInsSeg、PanNuke和GlaS等病理数据集上进行广泛评估。

Result: 1. 仅文本提示对核概念激活效果差；2. 性能对视觉提示类型和数量高度敏感；3. 少样本学习有提升但缺乏对视觉提示噪声的鲁棒性；4. 基于提示的使用与任务训练适配器参考之间存在显著差距。

Conclusion: SAM3在病理图像分割中存在明显局限性，特别是在核级分割方面。研究界定了SAM3的能力边界，并为病理领域适应的必要性提供了实践指导。
...
TL;DR: 提出一个基于视觉-语言框架的3D CT图像理解方法，通过视觉指令调优利用大规模开源CT图像与放射学报告进行预训练，然后适配到生存预测任务，在CT图像和临床数据上提升生存预测性能。


<details>
  <summary>Details</summary>
Motivation: 准确的预后和风险评估对临床决策和患者管理至关重要。虽然放射科医生评估的CT特征提供了疾病严重程度和结果的重要指标，但解读这些图像需要专业知识，将丰富的视觉信息转化为文本摘要不可避免地导致信息丢失。

Method: 提出一个视觉-语言框架，通过视觉指令调优利用大规模开源CT图像与放射学报告进行预训练，学习具有临床意义的视觉-文本表示，然后在预训练模型基础上加入生存预测头，适配到下游生存预测任务。

Result: 实验结果表明，该方法在生存预测方面优于基线方法，特别是在临床数据单独预测能力较弱的情况下表现更佳。模型还能生成对预定义问题的临床有意义语言响应。

Conclusion: 提出的视觉-语言框架能够有效利用CT图像和临床数据进行生存预测，在临床数据预测能力有限的情况下尤其有效，为医学图像理解和预后预测提供了新方法。
...
TL;DR: Geo3DPruner：一种几何引导的3D视觉令牌剪枝框架，通过建模跨帧几何关系和两阶段剪枝（体素内和体素间），在剪除90%视觉令牌的同时保持超过90%的原始性能，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在2D视觉上表现出色，但扩展到3D场景理解时面临挑战。现有方法将3D场景表示为包含深度和相机姿态信息的3D空间视频，但大量视觉令牌成为高效推理和上下文管理的主要瓶颈。现有剪枝方法忽略了空间视频的视角一致性和剩余令牌的空间多样性，无法有效去除帧间冗余并保持场景完整性。

Method: 提出Geo3DPruner框架：1）通过几何感知的全局注意力建模跨帧相关性；2）两阶段剪枝过程：体素内阶段在每个体素内选择代表性的多视角特征，体素间阶段通过选择全局分布的体素子集来保持空间多样性。

Result: 在多个3D场景理解基准测试上的广泛实验表明，Geo3DPruner在剪除90%视觉令牌的同时，保持了超过90%的原始性能，显著优于现有的文本引导和视觉引导剪枝方法。

Conclusion: Geo3DPruner通过几何引导的剪枝策略有效解决了3D空间视频中视觉令牌过多的问题，在保持场景完整性的同时大幅提升推理效率，为3D场景理解的高效多模态模型提供了有效解决方案。
...
TL;DR: MARCO是一个基于DINOv2的统一模型，通过新颖的训练框架提升细粒度定位和语义泛化能力，在多个基准测试中达到SOTA，同时比基于扩散的方法更小更快。


<details>
  <summary>Details</summary>
Motivation: 当前基于双编码器架构（DINOv2+扩散主干）的语义对应方法虽然准确，但参数庞大（数十亿），泛化能力差，在训练关键点之外表现不佳，存在基准测试性能与实际应用之间的差距。

Method: 基于DINOv2构建统一模型，采用粗到细的目标函数提升空间精度，结合自蒸馏框架将稀疏监督扩展到标注区域之外，将少量关键点转化为密集、语义一致的对应关系。

Result: 在SPair-71k、AP-10K和PF-PASCAL上达到新的SOTA，在细粒度定位阈值上提升8.9 PCK@0.01，对未见关键点泛化能力最强（SPair-U +5.1），对未见类别泛化能力最强（MP-100 +4.7），同时比基于扩散的方法小3倍、快10倍。

Conclusion: MARCO通过创新的训练框架解决了现有语义对应方法在泛化能力和实用性方面的局限性，在保持高性能的同时显著提升了效率和可扩展性。
...
### [175] [LiquidTAD: An Efficient Method for Temporal Action Detection via Liquid Neural Dynamics](https://arxiv.org/abs/2604.18274)
*Zepeng Sun,Naichuan Zheng,Hailun Xia,Junjie Wu,Liwei Bao,Xiaotai Zhang*

Main category: cs.CV

TL;DR: LiquidTAD是一种参数高效的时序动作检测框架，用并行化的ActionLiquid块替代了传统Transformer的自注意力层，实现了线性计算复杂度，在保持高性能的同时大幅减少了参数量。


<details>
  <summary>Details</summary>
Motivation: 当前基于Transformer的时序动作检测方法存在二次计算复杂度和参数冗余问题，限制了在资源受限环境中的部署。需要开发更高效的架构来平衡性能与计算效率。

Method: 提出LiquidTAD框架，使用并行化的ActionLiquid块替代自注意力层，采用闭式连续时间(CfC)公式化，将模型重构为可并行化算子，同时保持连续时间动力学的物理先验。通过学习时间常数(τ)自适应调节时间敏感性。

Result: 在THUMOS-14数据集上达到69.46%的平均mAP，仅需10.82M参数，比ActionFormer基线减少63%参数量。在ActivityNet-1.3和Ego4D基准测试中表现出最优的精度-效率权衡，对时间采样变化具有更强的鲁棒性。

Conclusion: LiquidTAD首次将并行化LNN架构引入时序动作检测领域，以线性计算复杂度捕获复杂时间依赖关系，显著提升了现代TAD框架的帕累托前沿，为资源受限环境提供了高效解决方案。
...
### [176] [Spike-NVPT: Learning Robust Visual Prompts via Bio-Inspired Temporal Filtering and Discretization](https://arxiv.org/abs/2604.18284)
...
<details>
  <summary>Details</summary>
Motivation: 传统的连续密集提示容易对输入噪声敏感，过度拟合任务无关细节，需要在保持参数效率的同时提升模型的噪声鲁棒性。

Method: 设计基于脉冲神经元的信号过滤层，利用积分-发放机制积累任务相关信号并过滤瞬时噪声波动；通过脉冲离散化单元将过滤后的信号转换为稀疏二进制提示，作为强正则化器。

Result: Spike-NVPT在鲁棒性方面最大提升11.2%，在干净数据集上保持竞争力，且推理时二进制提示保持静态，无额外计算开销。

Conclusion: 这是首次将脉冲神经元用于传统ANN视觉模型微调的尝试，通过稀疏二进制提示实现了噪声鲁棒性与准确性的平衡。
...
### [177] [Denoise and Align: Diffusion-Driven Foreground Knowledge Prompting for Open-Vocabulary Temporal Action Detection](https://arxiv.org/abs/2604.18313)
*Sa Zhu,Wanqian Zhang,Lin Wang,Jinchao Zhang,Cong Wang,Bo Li*

Main category: cs.CV

TL;DR: DFAlign：首个利用扩散去噪生成前景知识来指导动作-视频对齐的开放词汇时序动作检测框架，通过语义统一条件化、背景抑制去噪和前景提示对齐三个模块，有效缓解语义不平衡问题，在两个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇时序动作检测方法难以平衡简洁抽象的动作标签与丰富复杂的视频内容之间的语义不平衡，导致语义噪声和误导性的跨模态对齐。需要一种能够有效缓解语义鸿沟、增强动作相关片段区分性的方法。

Method: 提出DFAlign框架，采用"条件化、去噪、对齐"的三阶段方法：1) 语义统一条件化模块统一动作共享和动作特定语义作为扩散去噪条件；2) 背景抑制去噪模块通过去噪过程逐步去除视频背景冗余，生成前景知识；3) 前景提示对齐模块将提取的前景知识作为提示令牌注入文本表示，引导模型关注动作相关片段。

Result: 在两个开放词汇时序动作检测基准测试中实现了最先进的性能，证明了该方法在缓解语义鸿沟、增强动作相关片段区分性方面的有效性。

Conclusion: DFAlign通过扩散去噪生成的前景知识作为视频和文本表示之间的有效中间语义锚点，成功缓解了语义不平衡问题，实现了更精确的跨模态对齐，为开放词汇时序动作检测提供了新的有效解决方案。
...
### [178] [EVE: Verifiable Self-Evolution of MLLMs via Executable Visual Transformations](https://arxiv.org/abs/2604.18320)
*Yongrui Heng,Chaoya Jiang,Han Yang,Shikun Zhang,Wei Ye*

Main category: cs.CV

TL;DR: EVE提出了一种无需伪标签的多模态大语言模型自进化框架，通过可执行的视觉变换生成具有绝对真实答案的VQA问题，实现持续自我改进。


<details>
  <summary>Details</summary>
Motivation: 当前MLLM自进化方法存在局限：基于伪标签的方法会因模型预测漂移导致质量逐渐下降，而基于模板的方法则受限于静态变换集，无法适应难度和多样性变化。需要一种既独立于模型内部确定性又能够持续多样化训练分布的鲁棒自进化机制。

Method: EVE采用挑战者-求解者双策略架构。挑战者维护并扩展视觉变换代码示例队列，合成Python脚本执行动态视觉变换，生成具有执行验证真实答案的VQA问题。多维奖励系统整合语义多样性和动态难度校准，引导挑战者丰富代码队列并逐步增加任务难度。

Result: 大量实验表明，EVE在自进化方法中表现优异，建立了可验证、鲁棒且可扩展的MLLM自进化范式。

Conclusion: EVE通过可执行视觉变换完全绕过伪标签依赖，实现了持续多样化和难度递增的自进化，为MLLM的鲁棒自我改进提供了新范式。
...
### [179] [OmniHuman: A Large-scale Dataset and Benchmark for Human-Centric Video Generation](https://arxiv.org/abs/2604.18326)
*Lei Zhu,Xing Cai,Yingjie Chen,Yiheng Li,Binxin Yang,Hao Liu,Jie Chen,Chen Li,Jing LYu*

Main category: cs.CV

TL;DR: OmniHuman是一个大规模多场景数据集，用于解决现有音频-视频联合生成模型在复杂现实物理场景中生成高保真人类中心视频的挑战，通过分层标注和自动化流水线收集高质量数据，并建立了OHBench评估系统。


<details>
  <summary>Details</summary>
Motivation: 现有音频-视频联合生成模型在复杂现实物理场景中生成高保真人类中心视频存在显著挑战，根本原因在于现有数据集在三个维度上的结构缺陷：有限的全局场景和相机多样性、稀疏的交互建模（人-人和人-物）以及不足的个体属性对齐。

Method: 提出了OmniHuman数据集，这是一个大规模多场景数据集，提供分层标注覆盖视频级场景、帧级交互和个体级属性。开发了全自动流水线进行高质量数据收集和多模态标注。同时建立了OmniHuman Benchmark（OHBench）三级评估系统，引入与人类感知高度一致的度量标准。

Result: OmniHuman填补了现有数据集在全局场景多样性、交互建模和个体属性对齐方面的空白，OHBench评估系统提供了跨全局场景、关系交互和个体属性的全面诊断，弥补了现有基准的不足。

Conclusion: OmniHuman数据集和OHBench评估系统共同为解决人类中心音频-视频合成中的关键挑战提供了数据支持和科学评估框架，特别是在复杂现实物理场景中的高保真人类视频生成方面。
...
### [180] [AdaCluster: Adaptive Query-Key Clustering for Sparse Attention in Video Generation](https://arxiv.org/abs/2604.18348)
*Haoyue Tan,Shengnan Wang,Yulin Qiao,Juncheng Zhang,Youhui Bai,Ping Gong,Zewen Jin,Cheng Li*

Main category: cs.CV

TL;DR: AdaCluster是一种无需训练的自适应聚类框架，通过角度相似性保持聚类压缩查询向量，欧氏相似性保持聚类处理键向量，显著加速视频扩散变压器的推理速度，同时保持模型质量。


<details>
  <summary>Details</summary>
Motivation: 视频扩散变压器（DiTs）由于二次注意力复杂度导致推理延迟过高。现有稀疏注意力方法要么忽略语义相似性，要么无法适应不同层的异构令牌分布，导致模型性能下降。

Method: 提出AdaCluster训练免费自适应聚类框架：1）对查询向量应用角度相似性保持聚类方法实现更高压缩；2）为键向量设计欧氏相似性保持聚类方法，包括聚类数量分配、阈值自适应聚类和高效关键聚类选择。

Result: 在CogVideoX-2B、HunyuanVideo和Wan-2.1模型上，在单个A40 GPU上实现1.67-4.31倍加速，质量下降可忽略不计。

Conclusion: AdaCluster有效解决了视频扩散变压器的推理延迟问题，通过自适应聚类方法在保持模型准确性的同时显著提升生成速度。
...
### [181] [LBFTI: Layer-Based Facial Template Inversion for Identity-Preserving Fine-Grained Face Reconstruction](https://arxiv.org/abs/2604.18358)
*Zixuan Shen,Zhihua Xia,Kaikai Gan,Peipeng Yu*

Main category: cs.CV

TL;DR: 提出基于分层的人脸模板反演方法LBFTI，通过三层分解和分阶段训练策略，在机器认证和人类感知相似度上均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 人脸模板虽符合数据最小化原则，但现有模板反演技术存在隐私泄露风险，需要更精细的身份保持人脸重建方法。

Method: 将人脸分解为前景层（眉毛、眼睛、鼻子、嘴巴）、中景层（皮肤）和背景层，采用专用生成器和三阶段训练策略：1）前景和中景层独立精细生成；2）融合前景中景层并二次注入模板生成完整全景人脸；3）联合微调优化层间协调和身份一致性。

Result: LBFTI在机器认证性能上比现有方法提升25.3%的TAR，在人类感知相似度方面也表现更好，通过定量指标和问卷调查验证。

Conclusion: LBFTI方法有效解决了人脸模板反演中的隐私泄露问题，实现了身份保持的精细人脸重建，在机器和人类评估中均表现出优越性能。
...
### [182] [EAST: Early Action Prediction Sampling Strategy with Token Masking](https://arxiv.org/abs/2604.18367)
*Iva Sović,Ivan Martinović,Marin Oršić*

Main category: cs.CV

TL;DR: EAST框架通过随机化训练策略和联合学习，实现了早期动作预测的显著性能提升，在多个数据集上达到新的SOTA。


<details>
  <summary>Details</summary>
Motivation: 早期动作预测面临视觉证据有限的挑战，需要模型能够从不完整观察中进行推理。

Method: 提出随机化训练策略，采样时间步分离观察和未观察帧；采用联合学习观察和未来表示；引入token掩码减少内存使用；结合预测解码器。

Result: 在NTU60、SSv2和UCF101数据集上分别超越之前最佳工作10.1、7.7和3.9个百分点；token掩码使内存减半、训练加速2倍且精度损失可忽略。

Conclusion: EAST框架通过简单高效的设计解决了早期动作预测的关键挑战，在多个基准上实现了显著的性能提升。
...
### [183] [DSA-CycleGAN: A Domain Shift Aware CycleGAN for Robust Multi-Stain Glomeruli Segmentation](https://arxiv.org/abs/2604.18368)
*Zeeshan Nisar,Friedrich Feuerhake,Thomas Lampert*

Main category: cs.CV

TL;DR: 提出Domain Shift Aware CycleGAN（DSA-CycleGAN）来解决数字病理学中多染色分割的染色间差异问题，通过减少CycleGAN在染色转换时引入的噪声，提升肾小球分割性能。


<details>
  <summary>Details</summary>
Motivation: 数字病理学分割面临的主要挑战是染色间和染色内变异，这会降低模型性能。由于为每种染色进行标注成本高昂且耗时，现有方法使用CycleGAN进行染色转换，但CycleGAN在转换过程中容易引入噪声，特别是在某些染色对存在一对多关系时，这与循环一致性损失相冲突。

Method: 提出Domain Shift Aware CycleGAN（DSA-CycleGAN），通过减少染色转换过程中的噪声来解决上述问题。该方法特别关注染色间的域偏移，并与其他机器学习方法进行比较评估。

Result: 实验表明，DSA-CycleGAN不仅提高了肾小球分割的性能，而且在减少噪声方面优于其他方法。特别是在生物学上不同的染色之间转换时，效果尤为明显。

Conclusion: DSA-CycleGAN有效解决了多染色分割中的染色转换噪声问题，在肾小球分割任务中表现出色，特别是在处理生物学差异较大的染色对时优势明显。代码已公开。
...
### [184] [Towards Robust Text-to-Image Person Retrieval: Multi-View Reformulation for Semantic Compensation](https://arxiv.org/abs/2604.18376)
*Chao Yuan,Yujian Zhao,Haoxuan Xu,Guanglin Niu*

Main category: cs.CV

TL;DR: 本文提出基于大语言模型的语义补偿框架MVR，通过多视角语义重构和特征补偿解决文本到图像人物检索中的表达漂移问题，无需训练即可提升原始模型性能并在三个数据集上达到SOTA


<details>
  <summary>Details</summary>
Motivation: 文本到图像人物检索任务中，自然语言表达的多样性和视觉语义的隐含性导致表达漂移问题，即语义等价的文本由于表达方式差异在嵌入空间中产生显著特征差异，从而降低图像文本对齐的鲁棒性

Method: 提出基于大语言模型的语义补偿框架MVR，包含三个核心组件：1）多视角重构：采用双分支提示策略，结合关键特征引导和多样性感知重写生成语义等价但分布多样的文本变体；2）文本特征鲁棒性增强：通过无训练潜在空间补偿机制，利用多视角特征均值池化和残差连接抑制噪声干扰，捕捉"语义回声"；3）视觉语义补偿：使用VLM生成多视角图像描述，并通过共享文本重构进一步增强以解决视觉语义差距

Result: 实验表明该方法无需训练即可显著提升原始模型的准确性，在三个文本到图像人物检索数据集上达到最先进的性能水平

Conclusion: 提出的MVR框架通过多视角语义重构和特征补偿有效解决了表达漂移问题，增强了跨模态表示的一致性，为文本到图像人物检索提供了有效的无训练增强方案
...
### [185] [One-Step Diffusion with Inverse Residual Fields for Unsupervised Industrial Anomaly Detection](https://arxiv.org/abs/2604.18393)
*Boan Zhang,Wen Li,Guanhua Yu,Xiyang Liu,Wenchao Chen,Long Tian*

Main category: cs.CV

TL;DR: 本文提出OSD-IRF方法，通过单步扩散和逆残差场解决无监督工业异常检测中扩散模型推理慢的问题，在三个基准测试中达到SOTA性能并实现2倍推理加速。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在无监督工业异常检测中表现出色，但其迭代去噪和加噪特性导致推理速度慢。现有方法假设异常更难生成，在数据空间产生更大重构误差或在潜在空间有更低概率密度，但推理效率是主要瓶颈。

Method: 首先在正常数据上训练无条件的深度扩散概率模型（DDPM）。对于测试样本，基于训练好的DDPM参数化噪声函数估计的噪声预测其逆残差场（IRF）。通过评估IRF在高斯分布下的概率密度并与阈值比较进行异常检测。关键发现是异常在IRF空间中变得可区分，且IRF在去噪过程的任何相邻时间步都成立，因此只需单步扩散。

Result: 在三个广泛使用的无监督工业异常检测基准测试中，OSD-IRF在六个指标上达到SOTA或具有竞争力的性能，同时在没有蒸馏的情况下实现约2倍的推理加速。

Conclusion: OSD-IRF通过引入逆残差场空间，在保持扩散模型异常检测性能的同时显著提升了推理效率，为工业应用提供了实用的解决方案。
...
### [186] [Revisiting Change VQA in Remote Sensing with Structured and Native Multimodal Qwen Models](https://arxiv.org/abs/2604.18429)
*Yakoub Bazi,Mohamad M. Al Rahhal,Mansour Zuair,Faroun Mohamed*

Main category: cs.CV

TL;DR: 本文通过Qwen模型在CDVQA基准上的实验发现，原生多模态模型比结构化视觉语言管道更有效，性能不随模型规模单调增长，紧密集成的多模态主干对遥感图像语义变化推理贡献更大。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在时序遥感图像理解中已有研究，但变化视觉问答在现代化多模态模型背景下仍未被充分探索。本文旨在重新评估CDVQA基准，比较不同架构的Qwen模型在语义变化推理任务上的表现。

Method: 在统一的低秩适应设置下，使用Qwen3-VL（结构化视觉语言管道，具有多深度视觉条件和全注意力解码器）和Qwen3.5（原生多模态模型，具有单阶段对齐和混合解码器主干）在CDVQA测试集上进行实验比较。

Result: 实验结果表明：1）近期视觉语言模型优于早期专门基线；2）性能不随模型规模单调增长；3）原生多模态模型比结构化视觉语言管道更有效。

Conclusion: 对于遥感图像中的语言驱动语义变化推理，紧密集成的多模态主干比模型规模或显式的多深度视觉条件对性能贡献更大。
...
TL;DR: Asset Harvester是一个端到端系统，可将自动驾驶日志中的稀疏物体观测转换为完整的3D资产，用于闭环仿真。


<details>
  <summary>Details</summary>
Motivation: 当前神经场景重建方法无法生成完整的3D物体资产，限制了自动驾驶仿真中的智能体操作和大视角新视图合成。

Method: 开发了一个系统级设计，包括：1）大规模物体中心训练元组筛选；2）跨异构传感器的几何感知预处理；3）结合稀疏视图条件多视图生成与3D高斯提升的鲁棒训练方案；4）专门处理有限角度视图的SparseViewDiT模型；5）混合数据筛选、增强和自蒸馏技术。

Result: 实现了从稀疏自动驾驶物体观测到可重用3D资产的可扩展转换，为自动驾驶仿真提供完整的3D物体资产。

Conclusion: Asset Harvester解决了神经场景重建在生成完整3D物体资产方面的局限性，为自动驾驶闭环仿真提供了实用的端到端解决方案。
...
TL;DR: SemLT3D是一个语义引导的专家蒸馏框架，通过语义先验增强长尾类别的表示能力，解决相机3D检测中的类别不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有相机3D检测方法主要关注整体性能，忽视了现实数据集中严重的长尾不平衡问题。许多安全关键但罕见的类别（如儿童、婴儿车、应急车辆）代表性不足，导致学习偏差和性能下降。此外，类间模糊性（视觉相似的子类）和类内多样性（外观、尺度、姿态、上下文差异大）进一步阻碍了可靠的长尾识别。

Method: 提出SemLT3D框架，包含两个核心组件：1）语言引导的专家混合模块，根据语义亲和度将3D查询路由到专门的专家，以更好地区分混淆类别并专注于尾部分布；2）语义投影蒸馏管道，将3D查询与CLIP引导的2D语义对齐，产生更具一致性和区分性的特征。

Result: 虽然主要针对长尾不平衡问题，但SemLT3D的语义结构化学习也提高了在更广泛外观变化和挑战性边缘情况下的鲁棒性。

Conclusion: SemLT3D为更可靠的相机3D感知提供了一个原则性步骤，通过语义引导的专家蒸馏框架有效解决了长尾类别识别问题，同时增强了模型对视觉多样性的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 当前云侧训练流程依赖通用的视觉语言模型，这些模型由于基于2D图像-文本预训练，缺乏几何推理和领域语义，无法满足复杂环境中的高质量标注需求。
...
Conclusion: XEmbodied通过赋予VLA模型内在的3D几何感知能力，解决了当前云侧训练流程中的几何-语义不匹配问题，为下一代自主系统提供了更高质量的标注基础。
...
TL;DR: OneVL提出了一种单步潜在推理规划框架，通过视觉语言解释将推理压缩到紧凑的潜在令牌中，在保持答案级延迟的同时超越显式思维链的性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于VLA的自动驾驶中，思维链推理的自动回归特性导致延迟过高，无法实时部署。现有的潜在CoT方法使用纯语言潜在表示，只能压缩世界的符号抽象，而无法捕捉实际控制驾驶的因果动态。

Method: OneVL是一个统一的VLA和世界模型框架，通过双辅助解码器监督的紧凑潜在令牌进行推理。除了重构文本CoT的语言解码器外，还引入了预测未来帧令牌的视觉世界模型解码器，迫使潜在空间内化道路几何、智能体运动和环境变化的因果动态。采用三阶段训练流程逐步对齐轨迹、语言和视觉目标。

Result: 在四个基准测试中，OneVL成为首个超越显式CoT的潜在CoT方法，在答案级延迟下实现了最先进的准确性，证明了在语言和世界模型监督下的更紧密压缩能产生比逐令牌推理更通用的表示。

Conclusion: 通过同时利用语言和视觉世界模型监督，OneVL展示了更紧密的压缩能够产生比冗长的逐令牌推理更通用的表示，为实时自动驾驶中的高效推理提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在单图像理解方面表现出色，但在多图像推理方面存在能力差距。当前方法主要关注使用预定义图像索引的局部推理（如"查看图像3..."），而忽略了全局视觉搜索和自主跨图像比较的关键技能。

Method: 提出简单到困难（S2H）学习框架，系统构建三个层次的多图像偏好数据：1）单图像局部推理；2）多图像局部比较；3）全局视觉搜索。该方法利用提示驱动的复杂性创建适用于不同模型的偏好对，而不依赖模型特定属性如幻觉或注意力启发式。

Result: 在LLaVA和Qwen-VL模型上的广泛评估表明，多样化的多图像推理数据显著提升了多图像推理性能，在多个基准测试中显著优于基线方法。同时保持了强大的单图像推理能力。

Conclusion: S2H框架有效增强了视觉语言模型的多图像理解能力，同时保持单图像推理性能，推进了整体视觉偏好对齐的技术水平。
...
TL;DR: UDM-GRPO框架首次将均匀离散扩散模型与强化学习结合，通过将最终干净样本作为动作、通过扩散前向过程重建轨迹等创新，显著提升了文本到图像任务的性能。


<details>
  <summary>Details</summary>
Motivation: 均匀离散扩散模型在离散生成建模中表现出潜力，但其与强化学习的结合尚未充分探索。研究发现直接应用GRPO到UDM会导致训练不稳定和性能提升有限，因此需要开发专门框架来解决这些问题。

Method: 提出了UDM-GRPO框架，包含两个关键洞察：1) 将最终干净样本作为动作提供更准确稳定的优化信号；2) 通过扩散前向过程重建轨迹，使概率路径与预训练分布更好对齐。还引入了Reduced-Step和CFG-Free策略来提升训练效率。

Result: 在多个文本到图像任务上显著提升基础模型性能：GenEval准确率从69%提升到96%，PickScore从20.46提升到23.81，在连续和离散设置中都达到最先进性能。在OCR基准测试中，准确率从8%提升到57%，验证了方法的泛化能力。

Conclusion: UDM-GRPO是首个成功整合均匀离散扩散模型与强化学习的框架，通过创新的训练策略显著提升了模型性能，在多个基准测试中达到最先进水平，证明了该方法在离散生成建模中的有效性。
...
TL;DR: MetaCloak-JPEG解决了现有对抗性人脸保护系统在JPEG压缩下的失效问题，通过可微分JPEG层和元学习框架，显著提升了保护效果。


<details>
  <summary>Details</summary>
Motivation: 现有对抗性人脸保护系统（PhotoGuard、Anti-DreamBooth、MetaCloak）存在结构性缺陷：它们没有考虑社交平台普遍应用的JPEG压缩流程。由于JPEG量化使用round()函数（导数几乎处处为零），对抗性能量集中在高频DCT频带，而这些频带在JPEG压缩中被丢弃，导致60-80%的保护信号失效。

Method: 提出MetaCloak-JPEG方法：1）插入基于直通估计器（STE）的可微分JPEG（DiffJPEG）层，前向传播使用标准JPEG压缩，反向传播将round()替换为恒等函数；2）将DiffJPEG嵌入JPEG感知的EOT分布（约70%的数据增强包含DiffJPEG）；3）在双层元学习循环中使用课程质量因子调度（QF从95到50）。

Result: 在l-inf扰动预算eps=8/255下，MetaCloak-JPEG达到32.7 dB PSNR，91.3%的JPEG存活率，在评估的9个JPEG质量因子中全面优于PhotoGuard（9/9获胜），平均去噪损失增益+0.125，训练内存预算仅4.1 GB。

Conclusion: MetaCloak-JPEG通过可微分JPEG层解决了现有对抗性人脸保护系统在JPEG压缩下的失效问题，显著提升了保护效果，为社交平台上的深度伪造防御提供了更可靠的解决方案。
...
TL;DR: EVT在RMT基础上改进，采用欧氏距离衰减增强空间信息建模，放弃分解注意力机制改用空间独立分组，在多项视觉任务中表现优异，ImageNet-1k达到86.6% top1准确率。


<details>
  <summary>Details</summary>
Motivation: Vision Transformer的自注意力机制缺乏显式空间先验且计算复杂度高，限制了其应用。RMT虽然通过曼哈顿距离衰减引入空间信息，但仍存在改进空间。

Method: EVT在RMT基础上进行改进：1) 使用更合理的欧氏距离衰减增强空间信息建模；2) 放弃分解注意力机制，采用空间独立分组方法，提供更灵活的token数量控制。

Result: 在图像分类、目标检测、实例分割和语义分割等任务上表现出色，无需额外训练数据即在ImageNet-1k上达到86.6% top1准确率。

Conclusion: EVT通过改进的空间建模方法和更灵活的分组机制，克服了RMT的局限性，为自注意力机制提供了更精细的空间先验，在各种计算机视觉任务中表现出卓越性能。
...
TL;DR: SynAgent是一个通过单智能体到多智能体技能迁移实现可扩展、物理合理的协作人形操作框架，利用交互保持重定向和条件生成策略来提升协作操作性能。


<details>
  <summary>Details</summary>
Motivation: 可控协作人形操作面临数据稀缺、多智能体协调复杂、跨物体泛化有限等挑战，需要开发能够从单智能体技能迁移到多智能体协作的框架。

Method: 1) 基于Delaunay四面体化的交互网格进行交互保持重定向；2) 单智能体预训练和适应范式，通过去中心化训练和多智能体PPO从单人类数据引导协同行为；3) 轨迹条件生成策略，使用条件VAE并通过多教师蒸馏训练实现稳定可控的对象级轨迹执行。

Result: SynAgent在协作模仿和轨迹条件控制方面显著优于现有基线方法，能够泛化到不同的物体几何形状。

Conclusion: SynAgent通过单智能体到协作智能体的协同作用，实现了可扩展和物理合理的协作操作，为解决协作人形操作的数据稀缺和泛化问题提供了有效框架。
...
TL;DR: AnchorSeg提出了一种新的推理分割方法，通过结构化语言锚定查询库将语义推理与空间定位解耦，使用Token-Mask循环一致性训练，在ReasonSeg测试集上达到SOTA效果。


<details>
  <summary>Details</summary>
Motivation: 现有推理分割方法依赖单个分割token，将语义推理和空间定位隐式编码到同一个嵌入中，限制了模型显式解耦"分割什么"和"在哪里分割"的能力。

Method: 1. 将推理分割重新表述为基于图像token的结构化条件生成过程；2. 构建有序的查询库序列：潜在推理token捕获中间语义状态，分割锚点token提供显式空间定位；3. 将空间条件建模为图像token的分解分布；4. 提出Token-Mask循环一致性（TMCC）双向训练目标，确保跨分辨率对齐。

Result: 在ReasonSeg测试集上达到67.7% gIoU和68.1% cIoU，实现最先进的结果。

Conclusion: 通过结构化语言锚定查询库显式解耦空间定位和语义推理，AnchorSeg显著提升了推理分割性能，所有代码和模型已公开。
...
TL;DR: MultiWorld是一个用于多智能体多视角世界建模的统一框架，能够准确控制多个智能体同时保持多视角一致性，在视频保真度、动作跟随能力和多视角一致性方面优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有的视频世界模型主要局限于单智能体场景，无法捕捉真实世界多智能体系统中复杂的交互关系，这限制了它们在多智能体环境中的应用。

Method: 提出了MultiWorld框架，包含多智能体条件模块实现精确的多智能体可控性，以及全局状态编码器确保不同视角间的一致性观察。支持智能体和视角数量的灵活扩展，并并行合成不同视角以提高效率。

Result: 在多玩家游戏环境和多机器人操作任务上的实验表明，MultiWorld在视频保真度、动作跟随能力和多视角一致性方面优于基线方法。

Conclusion: MultiWorld为多智能体多视角世界建模提供了一个有效的统一框架，能够准确控制多个智能体并保持多视角一致性，在多智能体系统中具有重要应用价值。
...
TL;DR: ReCap：轻量级故事可视化一致性框架，通过条件帧引用和语义漂移校正，以仅149K额外参数实现跨帧角色一致性，在FlintstonesSV和PororoSV基准上超越现有SOTA。


<details>
  <summary>Details</summary>
Motivation: 传统故事可视化方法依赖显式记忆库、架构扩展或辅助语言模型来维持跨帧一致性，导致参数量大幅增加和推理开销。需要一种轻量级方法在不修改基础扩散模型的情况下改善角色稳定性和视觉保真度。

Method: ReCap包含两个核心组件：1) CORE模块将代词作为视觉锚点，仅在角色被代词引用时激活，以前一帧为条件传播视觉身份；2) SemDrift在训练期间对齐去噪器表示与预训练DINOv3视觉嵌入，校正语义漂移。整个框架仅增加149K参数。

Result: 在FlintstonesSV上Character-Accuracy提升2.63%，在PororoSV上提升5.65%，在两个主要故事可视化基准上建立新的SOTA。同时将方法扩展到真实电影的人类中心叙事，展示了超越卡通领域的泛化能力。

Conclusion: ReCap通过轻量级条件帧引用和语义漂移校正，以极小的参数开销显著提升了故事可视化的角色一致性，为跨帧视觉叙事提供了高效解决方案。
...
TL;DR: 提出一种基于小波引导的多级空间分解混合形状的动画化虚拟人表示方法，通过蒸馏管道从高质量教师模型转移运动感知的服装动力学和精细外观细节，实现计算成本降低2000倍、模型大小减小10倍，同时保持高质量视觉效果。


<details>
  <summary>Details</summary>
Motivation: 当前动画化虚拟人建模面临两难：高质量虚拟人需要服务器级GPU计算资源，而轻量级虚拟人则存在表面动态有限、外观细节减少和明显伪影的问题。需要同时实现高保真度和低计算复杂度的虚拟人表示。

Method: 提出小波引导的多级空间分解混合形状表示方法，结合多级小波谱分解和纹理空间低秩结构分解，通过蒸馏管道从预训练的超高质量教师模型转移运动感知的服装动力学和精细外观细节到紧凑高效表示中。

Result: 相比原始高质量教师模型，计算成本降低2000倍，模型大小减小10倍，同时保持视觉上合理的动态和外观细节。在桌面PC上达到180+ FPS，在Meta Quest 3上实现24 FPS的实时本地设备性能，显著优于移动设备现有方法，与服务器级方法质量相当或更优。

Conclusion: 该方法成功解决了高保真度虚拟人与低计算复杂度之间的矛盾，通过创新的表示方法和蒸馏技术，实现了高质量虚拟人在资源受限平台上的实用化部署，为沉浸式应用提供了可行的解决方案。
