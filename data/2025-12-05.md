...
TL;DR: OnSight Pathology是一个平台无关的计算机视觉软件，通过连续屏幕捕获提供实时AI推理，可在消费级PC上本地运行，无需复杂集成，降低了AI在病理学中的部署门槛。


<details>
  <summary>Details</summary>
Motivation: 传统组织学检查依赖主观解释和专家资源，影响准确性和临床护理；现有数字病理解决方案多为专有系统，存在部署障碍，限制了AI在病理学中的实际应用。

Method: 开发OnSight Pathology平台，作为单一可执行文件，通过连续自定义屏幕捕获技术，在各种数字切片查看器上实时提供AI推理，支持本地消费级PC运行，无需复杂软件集成。

Result: 在2500多张公开全切片图像和临床病例中验证了软件的鲁棒性，成功应用于脑肿瘤分类、有丝分裂检测和免疫组化染色定量等常规病理任务，并展示了与实时显微镜摄像头（包括智能手机）的兼容性。
...
TL;DR: LAPA是一种基于Transformer的端到端多相机点跟踪架构，通过注意力机制联合推理跨视图和时间，将外观匹配与几何约束集成，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统多相机点跟踪流程将检测、关联和跟踪解耦，导致错误传播和时间不一致性，特别是在具有复杂运动和遮挡的挑战性场景中。

Method: 提出LAPA架构：1) 使用跨视图注意力机制结合几何先验建立软对应关系；2) 通过注意力加权聚合构建3D点表示，而非经典三角测量；3) 使用Transformer解码器建模长程依赖，保持时间一致性。

Result: 在TAPVid-3D-MC上达到37.5% APD，在PointOdyssey-MC上达到90.3% APD，显著优于现有方法，特别是在复杂运动和遮挡场景中表现优异。

Conclusion: LAPA通过统一的端到端Transformer架构成功解决了多相机点跟踪中的关键挑战，将外观匹配与几何约束有效集成，为复杂场景下的鲁棒跟踪提供了新方案。
...
<details>
  <summary>Details</summary>
Motivation: 人类自然地将连续经验感知为时间嵌套事件的层次结构，但计算机视觉需要能够预测性和层次性地分割视频的模型。现有方法通常需要监督或仅能进行回顾性分析，缺乏对事件层次结构的无监督学习能力。

Method: PARSE采用统一框架，通过多尺度递归预测器学习事件结构：底层处理短期动态，高层通过基于注意力的反馈整合长期上下文。事件边界通过预测误差的瞬态峰值自然出现，形成时间连贯的嵌套部分关系。
...
TL;DR: MoReGen是一个运动感知、物理基础的文本到视频生成框架，通过集成多智能体LLM、物理模拟器和渲染器，在代码域生成可复现、物理精确的视频，并提出了MoReSet基准和对象轨迹对应性评估指标来量化物理有效性。


<details>
  <summary>Details</summary>
Motivation: 当前文本到视频生成虽然在真实感方面取得了显著进展，但生成符合物理原理、意图对齐的视频仍然是一个核心挑战。现有模型在保持物理有效性方面存在困难，需要系统研究牛顿运动控制的文本到视频生成和评估。

Method: 提出了MoReGen框架，集成了多智能体LLM、物理模拟器和渲染器，在代码域生成可复现、物理精确的视频。同时提出了MoReSet基准，包含1,275个人工标注的视频，涵盖九类牛顿现象，具有场景描述、时空关系和真实轨迹。

Result: 实验结果表明，现有最先进的文本到视频模型在保持物理有效性方面存在困难，而MoReGen为物理一致视频合成提供了一个有原则的方向。通过MoReSet基准和MoRe指标评估，验证了MoReGen在物理精确性方面的优势。

Conclusion: MoReGen框架通过集成LLM、物理模拟器和渲染器，为物理精确的视频生成提供了系统解决方案。提出的对象轨迹对应性评估指标和MoReSet基准为量化物理有效性提供了可靠工具，推动了物理一致视频合成的发展。
...
TL;DR: ReasonX框架利用多模态大语言模型作为感知评判器，通过相对内在比较作为GRPO奖励，在未标记的真实世界图像上微调内在分解模型，显著提升了泛化性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散和transformer的内在图像分解模型虽然受益于合成数据集的配对监督，但在多样化真实场景中的泛化能力仍然有限。需要一种能够利用未标记真实图像进行训练的方法来提升模型的泛化性能。

Method: 提出ReasonX框架：1) 使用多模态大语言模型作为感知评判器，提供内在属性的相对比较；2) 将这些比较作为GRPO奖励；3) 在未标记的真实世界图像上微调内在分解模型；4) 通过奖励模型输出与评判器关系评估之间的一致性来对齐条件内在预测器。

Result: 在多种基础架构和模态上，ReasonX带来了显著改进：在IIW数据集上实现了9-25%的WHDR降低，在ETH3D数据集上深度准确率提升高达46%。

Conclusion: ReasonX展示了MLLM引导的比较监督在连接低层次和高层次视觉推理方面的潜力，为内在图像分解提供了一种模型无关的改进方法，能够有效利用未标记真实图像提升泛化性能。
...
TL;DR: 该研究提出了AdversarialAnatomyBench基准测试，用于评估视觉语言模型在罕见解剖变异上的表现，发现现有模型在罕见解剖变异上的准确率从典型情况的74%骤降至29%，且模型扩展和干预措施无法解决这一问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型基准测试主要评估常见解剖结构的表现，无法捕捉罕见解剖变异带来的挑战。临床实践中存在大量自然发生的罕见解剖变异，这些变异违反了模型学习的"典型"人体解剖先验知识，需要专门的评估基准。

Method: 研究团队构建了AdversarialAnatomyBench基准测试，包含多种成像模态和解剖区域中自然发生的罕见解剖变异。使用该基准测试评估了22个最先进的视觉语言模型，分析其在基础医学感知任务上的表现，并测试了模型扩展、偏误感知提示和测试时推理等干预措施的效果。

Result: 1. 在罕见解剖变异上，模型平均准确率从典型情况的74%降至29%，性能下降达45个百分点。2. 即使是表现最好的GPT-5、Gemini 2.5 Pro和Llama 4 Maverick模型，性能也下降了41-51%。3. 模型错误模式与预期的解剖偏误高度一致。4. 模型扩展和干预措施均无法解决这一问题。

Conclusion: 当前视觉语言模型在罕见解剖变异上的泛化能力存在严重不足，这是先前未被量化的关键局限性。AdversarialAnatomyBench为系统测量和缓解多模态医疗AI系统中的解剖偏误提供了基础，强调了开发能够处理解剖变异多样性的模型的重要性。
...
TL;DR: MVRoom是一个用于3D室内场景的可控新视角合成系统，使用基于粗糙3D布局的多视角扩散模型，通过两阶段设计和布局感知的极线注意力机制实现高质量、一致的多视角生成。


<details>
  <summary>Details</summary>
Motivation: 现有的新视角合成方法在保持多视角一致性和可控性方面存在挑战，特别是在复杂3D室内场景中。需要一种能够利用3D布局信息来增强一致性和可控性的方法。

Method: 采用两阶段设计：第一阶段使用新颖表示来桥接3D布局和图像条件信号；第二阶段进行图像条件多视角生成，引入布局感知的极线注意力机制。还提出迭代框架支持递归多视角生成和文本到场景生成。

Result: 实验结果表明，该方法在新视角合成中实现了高保真度和可控的3D场景生成，在定量和定性评估上都优于现有基准方法。消融研究验证了生成管道中关键组件的有效性。

Conclusion: MVRoom通过创新的两阶段设计和布局感知机制，成功实现了高质量、可控的3D室内场景新视角合成，为文本到场景生成提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 照明对视觉外观有重要影响，但现有照明表示方法（如环境贴图、辐照度、球谐函数、文本）互不兼容，限制了跨模态的照明信息传递。

Method: 提出UniLight联合潜在空间表示，训练模态特定的编码器（文本、图像、辐照度、环境贴图）进行对比学习对齐表示，并加入球谐函数预测任务增强方向性理解。构建多模态数据管道支持大规模训练。

Result: 实验表明UniLight表示能捕捉一致且可迁移的照明特征，在三个任务上表现良好：基于照明的检索、环境贴图生成、以及基于扩散模型的图像合成中的照明控制。

Conclusion: UniLight实现了多种照明表示模态的统一，支持灵活的跨模态照明操作，为照明相关的计算机视觉任务提供了有效的表示框架。
...
Conclusion: 将随机动力学与基于流的序列模型相结合，在生成时间序列预测方面具有潜力，能提升视频运动转移中未来轨迹预测的多样性和真实性。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型日益复杂，输入数据规模增大，尽管专用硬件有进步，但数据加载仍是限制训练和推理速度的主要瓶颈
...
TL;DR: GfM是一种轻量级单目几何估计方法，通过解耦全局和局部结构来解决单相机重建中的投影模糊问题，预测主导道路平面和残差变化，无需完整外部校准即可恢复度量深度。


<details>
  <summary>Details</summary>
Motivation: 传统单目深度估计方法通常会过度平滑道路几何特征（如颠簸、坡度和表面不规则性），丢失对运动规划和稳定性至关重要的信息，需要一种能准确感知车辆3D环境的方法。

Method: 提出Gamma-from-Mono方法，通过预测主导道路表面平面和残差变化（用gamma表示）来解耦全局和局部结构。gamma定义为点相对于平面的垂直偏差与点到相机深度的比值，基于平面视差几何。仅需相机离地高度，即可通过闭式解确定性地恢复度量深度。

Result: 在KITTI和道路表面重建数据集上的评估显示，GfM在近场深度和gamma估计方面达到最先进精度，同时保持竞争力的全局深度性能。轻量级模型（8.88M参数）能适应不同的相机设置，是首个在RSRD上评估的自监督单目方法。

Conclusion: GfM通过物理可解释的表示方法有效解决了单目深度估计中的投影模糊问题，无需大型标注数据集，在近场几何估计方面表现出色，适用于自动驾驶的安全舒适控制。
...
Method: 首先分析文本提示调优方法在联邦学习下的校准表现，评估现有校准技术在不同全局聚合方法中的效果，提出FL²oRA方法——一种基于LoRA的简单方法，自然改善联邦学习中的校准性能

Result: 实验表明文本提示调优在联邦学习下会降低校准指标，现有校准技术改进有限。FL²oRA在多个基准测试中能持续产生良好校准的模型，减少对显式校准过程的需求

Conclusion: 关键挑战不仅在于如何聚合或校准，更在于选择微调哪些组件。FL²oRA方法能有效改善联邦学习环境下CLIP模型的校准性能
...
TL;DR: TOMCap是一种无需对齐图像-文本对的文本训练方法，通过CLIP表示和检索增强来生成图像描述，在无监督图像描述任务中表现优于其他方法。


<details>
  <summary>Details</summary>
Motivation: 减少对人工标注数据的依赖，探索无需对齐图像-文本对的图像描述方法，克服现有无监督方法性能仍低于全监督方法的问题。

Method: 基于预训练语言模型解码器，使用经过模态差距减少处理的CLIP表示作为提示，结合检索到的描述示例和潜在向量表示来引导生成过程。

Result: TOMCap在无训练和纯文本方法中表现最优，通过实验验证了检索增强和模态差距减少组件不同配置的影响。

Conclusion: TOMCap提供了一种有效的无监督图像描述方法，通过CLIP表示和检索增强策略，在无需对齐图像-文本对的情况下实现了有竞争力的性能。
...
TL;DR: 开发了一个基于计算机视觉和物理执行的低成本实时系统，用于自动按性别分选家蟋蟀，在资源受限设备上部署轻量级深度学习模型，实现86.8%的整体分选准确率。


<details>
  <summary>Details</summary>
Motivation: 全球对可持续蛋白质来源的需求推动了对食用昆虫的兴趣，家蟋蟀被认为是工业生产的理想物种。当前养殖实践通常采用混合性别群体饲养，缺乏自动化性别分选，尽管选择性育种、优化繁殖比例和营养差异化等方面存在潜在益处。

Method: 结合计算机视觉和物理执行，开发低成本实时系统。设备集成树莓派5、官方树莓派AI摄像头和自定义YOLOv8 nano目标检测模型，以及伺服驱动的分选臂。

Result: 模型测试中达到0.977的mAP@0.5平均精度，实际蟋蟀群体实验中实现86.8%的整体分选准确率。
...
TL;DR: Mind-to-Face：首个从非侵入式脑电图信号直接解码为高保真面部表情的框架，通过神经信号驱动个性化、情感感知的虚拟化身


<details>
  <summary>Details</summary>
Motivation: 当前的表情化虚拟化身系统严重依赖视觉线索，当面部被遮挡或情感内化时就会失效。需要一种能够直接从神经信号解码面部表情的方法，实现更自然的情感表达

Method: 构建双模态记录装置获取同步的EEG和多视角面部视频数据；使用CNN-Transformer编码器将EEG信号映射到密集的3D位置图（可采样超过65k顶点）；通过改进的3D高斯溅射管道进行渲染，实现照片级真实感和视角一致性

Result: EEG信号能够可靠预测动态、特定于个体的面部表情，包括微妙的情感反应；神经信号包含比先前假设更丰富的情感和几何信息
...
TL;DR: DisentangleFormer提出了一种解耦空间和通道维度的Vision Transformer架构，通过并行处理空间token和通道token流，解决了标准自注意力机制中空间和通道信息纠缠的问题，特别适用于多通道图像分析如高光谱成像。
...
Method: 基于信息论的去相关表示学习原理，提出并行解耦设计：1) 并行解耦：独立处理空间token流和通道token流；2) 压缩token增强器：自适应校准模块动态融合空间和通道流；3) 多尺度FFN：补充全局注意力，捕获多尺度局部上下文。

Result: 在高光谱基准测试中达到最先进性能，在Indian Pine、Pavia University、Houston、大型BigEarthNet遥感数据集以及红外病理数据集上均优于现有模型。在ImageNet上保持竞争性精度的同时，计算成本降低17.8%的FLOPs。

Conclusion: DisentangleFormer通过原则性的空间-通道解耦实现了鲁棒的多通道视觉表示，解决了Vision Transformer中表示纠缠的根本限制，为高光谱成像等应用提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 动态3D场景建模具有挑战性，需要从多视角聚合信息来重建时间演化的3D几何和运动。现有方法通常假设视频已同步或需要预定义场景对象，无法处理现实世界中常见的非同步视频集。

Method: 1) 计算每视频的密集4D特征轨迹和跨视频轨迹对应关系（使用融合Gromov-Wasserstein最优传输方法）；2) 执行全局帧级时间对齐以最大化匹配4D轨迹的重叠运动；3) 基于运动样条支架表示构建多视频4D高斯泼溅实现亚帧同步。

Result: 在Panoptic Studio和SyncNeRF Blender数据集上评估，平均时间误差低于0.26帧（亚帧级同步精度），Panoptic Studio数据集上达到26.3 PSNR的高保真4D重建分数。

Conclusion: SyncTrack4D是首个通用的非同步视频集4D高斯泼溅方法，无需假设预定义场景对象或先验模型，实现了同步的4DGS表示，包含密集显式3D轨迹和各视频时间偏移。
...
TL;DR: 本文提出了一种用于开放集人脸伪造检测（OSFFD）的双层证据检测方法（DLED），通过空间和频率层面的证据融合来估计预测不确定性，能够识别新型伪造类别，在检测新型伪造类别上平均比基线模型提升20%。


<details>
  <summary>Details</summary>
Motivation: 随着人脸伪造生成算法的快速发展，新型伪造类别不断出现，现有的人脸伪造检测方法通常只能进行二元真伪分类或识别已知伪造类别，无法检测新型伪造类型，这在实际应用中存在重大挑战。

Method: 提出双层证据人脸伪造检测方法（DLED），将开放集人脸伪造检测问题重新表述并通过不确定性估计来解决。该方法在空间和频率两个层面收集和融合类别特定的证据，以估计预测的不确定性，从而识别新型伪造类别。

Result: 在多种实验设置下的广泛评估表明，DLED方法在检测新型伪造类别方面取得了最先进的性能，平均比各种基线模型提升20%。同时，在传统的真伪人脸伪造检测任务上，DLED方法也表现出有竞争力的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在生成视频描述时存在严重的事实不准确问题，导致幻觉现象。虽然已有工作尝试缓解静态图像的幻觉问题，但针对动态视频中视觉对象和时间动作的联合幻觉缓解仍是一个未解决的挑战。

Method: 提出自增强对比对齐框架，采用幻觉自增强方案识别MLLM中的潜在幻觉，将原始描述转换为对比负例。同时开发轨迹短语对比对齐，将区域对象和关系引导的动作与其对应的视觉和时间短语进行匹配。
...
TL;DR: FMA-Net++是一个用于视频超分辨率和去模糊的联合框架，专门处理运动与动态变化曝光耦合的复杂退化问题，通过曝光时间感知调制和流引导动态滤波实现高效的并行长程时序建模。
...
Method: 采用序列级架构，基于分层细化与双向传播块实现并行长程时序建模。每个块包含曝光时间感知调制层，根据每帧曝光条件调节特征，驱动曝光感知的流引导动态滤波模块推断运动和曝光感知的退化核。将退化学习与恢复解耦，前者预测曝光和运动感知先验来指导后者。

Result: 在仅使用合成数据训练的情况下，FMA-Net++在新提出的REDS-ME（多曝光）和REDS-RE（随机曝光）基准测试以及GoPro数据集上实现了最先进的准确性和时序一致性，在恢复质量和推理速度方面均优于近期方法，并能很好地泛化到具有挑战性的真实世界视频。

Conclusion: FMA-Net++通过显式建模运动与动态变化曝光的耦合效应，有效解决了现实世界视频恢复中的复杂退化问题，在准确性和效率方面均有显著提升，为视频超分辨率和去模糊任务提供了有效的解决方案。
...
Conclusion: 通过傅里叶分析显式解耦视觉表征，结合双交叉注意力机制和非对称注入策略，能够显著提升视觉语言模型的泛化能力和鲁棒性。
...
TL;DR: 本文对深度学习模型在医学图像分类中的迁移学习技术进行了全面分析，评估了六个预训练模型在胸部X光数据集上的表现，发现InceptionV3在所有指标上表现最佳。


<details>
  <summary>Details</summary>
Motivation: 医学图像分类在疾病识别中至关重要，但从头训练大型深度学习模型通常不可行。迁移学习技术通过重用预训练模型来解决这个问题，但需要系统评估不同模型在医学图像分类中的表现。

Method: 使用六个预训练模型（AlexNet、VGG16、ResNet18、ResNet34、ResNet50和InceptionV3）在自定义胸部X光数据集上进行疾病检测评估。除了标准性能指标外，还进行了不确定性分析和运行时比较。

Result: InceptionV3在所有标准指标上始终优于其他模型。ResNet系列随着深度增加性能逐步提升，而VGG16和AlexNet表现合理但准确率较低。迁移学习在大多数情况下都有益，尤其是在数据有限时。
...
TL;DR: 提出DS2D2方法，通过双流光谱解耦蒸馏解决遥感图像目标检测中的特征混合和细微特征差异问题，在DIOR和DOTA数据集上取得显著效果提升。


<details>
  <summary>Details</summary>
Motivation: 现有蒸馏方法在遥感图像目标检测中存在特征混合问题，且忽视了由细微特征变化引起的差异，导致知识混淆。遥感图像中的密集小目标检测具有挑战性，需要更好的蒸馏方法来提升轻量化检测性能。

Method: 提出DS2D2方法，基于光谱分解集成显式和隐式蒸馏：1）使用一阶小波变换进行光谱分解以保留关键空间特征；2）设计密度无关尺度权重（DISW）处理密集小目标检测；3）通过全频和高频放大器提取隐式知识，将特征差异映射到预测偏差。

Result: 在DIOR数据集上，DS2D2使RetinaNet的AP50提升4.2%，Faster R-CNN的AP50提升3.8%，优于现有蒸馏方法。在DOTA数据集上也验证了有效性。

Conclusion: DS2D2是一种架构无关的蒸馏方法，通过双流光谱解耦有效解决了遥感图像目标检测中的特征混合和细微差异问题，显著提升了轻量化检测模型的性能。
...
TL;DR: 提出了一种可解释的多模态框架，整合RGB和深度数据来识别帕金森病步态模式，使用双YOLOv11编码器提取特征，结合多尺度局部全局提取模块和跨空间融合机制，并利用冻结的大型语言模型生成临床解释。
...
Method: 1) 使用双YOLOv11编码器分别提取RGB和深度模态特征；2) 多尺度局部全局提取模块增强时空表示；3) 跨空间融合机制整合特征；4) 冻结大型语言模型将视觉嵌入和结构化元数据转换为临床文本解释。
...
<details>
  <summary>Details</summary>
Motivation: 现有端到端自动驾驶研究存在两个方向的问题：轨迹生成导向的方法虽然能产生高质量轨迹但决策机制简单；轨迹选择导向的方法能进行多维度评估但缺乏足够的生成能力。需要一种能够整合高质量轨迹生成与全面决策推理的框架。
...
Conclusion: MindDrive为可解释和认知引导的自动驾驶提供了一条有前景的路径，通过结构化推理范式实现了高质量轨迹生成与综合决策推理的有机结合。
...
<details>
  <summary>Details</summary>
Motivation: 随着具身智能向真实世界部署发展，需要能够持续感知和推理流式视觉输入的能力。现有基准无法充分评估模型在具身场景下的流式视频理解能力，因此需要专门针对这一需求的基准。

Method: 构建StreamEQA基准，包含156个独立长视频，通过自动生成和人工精炼的混合流程创建约21K个带时间戳的问答对。基准从两个正交维度评估：具身维度（感知、交互、规划三个层次）和流式维度（后向、实时、前向推理）。

Result: 评估了13个最先进的视频-LLM，发现尽管在传统基准上表现强劲，但这些模型在具身场景下的流式视频理解方面仍然存在困难。

Conclusion: StreamEQA基准将推动具身应用中的流式视频理解研究，填补了现有评估体系的空白，为未来模型发展提供了重要参考。
...
TL;DR: GuidNoise：一种基于单对引导的扩散模型，用于广义噪声合成，仅需单个噪声/干净图像对作为引导，无需相机元数据，能生成高质量合成噪声图像用于数据增强。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型进行真实噪声合成通常需要相机元数据和大量目标特定的噪声-干净图像对，成本高昂且泛化能力有限。需要一种更简单、更通用的噪声合成方法。

Method: 提出GuidNoise方法：1）使用单个噪声/干净图像对作为引导；2）引入引导感知仿射特征修改（GAFM）；3）设计噪声感知细化损失来优化扩散模型的反向过程；4）无需额外元数据，训练和推理都仅需单对图像。

Result: GuidNoise能在多样噪声环境下合成高质量噪声图像，有效生成噪声-干净图像对用于数据增强。在轻量模型和有限训练数据的实际场景中，这种自增强方法显著提升了去噪性能。

Conclusion: GuidNoise通过单对引导的扩散模型实现了广义噪声合成，简化了噪声合成的先决条件，为数据增强提供了高效解决方案，特别适用于资源受限的实际应用场景。
...
TL;DR: dVLM-AD是一个基于扩散的视觉语言模型，用于端到端自动驾驶，通过双向注意力和迭代去噪机制解决了现有自回归模型在高层推理与低层规划之间的一致性和可控性问题。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶领域面临分布外场景的挑战，虽然主流研究通过集成视觉语言模型来提升端到端驾驶系统的泛化能力，但现有基于自回归的VLM/VLA由于因果注意力和顺序标记生成的限制，难以保持高层推理与低层规划之间的一致性和可控性。

Method: 提出dVLM-AD，一个基于离散扩散的视觉语言模型，利用双向注意力和迭代去噪机制，统一了感知、结构化推理和低层规划，为端到端驾驶提供更可控可靠的解决方案。

Result: 在nuScenes和WOD-E2E数据集上评估，dVLM-AD产生了更一致的推理-动作对，规划性能与现有驾驶VLM/VLA系统相当，在长尾WOD-E2E场景中，行为-轨迹一致性提升9%，RFS指标提高6%，优于自回归基线。

Conclusion: 基于扩散的视觉语言模型为可扩展的端到端自动驾驶提供了一条更可控和可靠的途径，解决了自回归模型在推理与规划一致性方面的局限性。
...
Method: 基于流匹配生成范式，构建从噪声到目标的确定性演化路径；采用扩散transformer架构，设计自适应条件注入器和时空感知调制器；创建了两个高质量多模态时间序列数据集。

Result: UniTS在低层和高层时间序列任务中展现出卓越的生成和认知能力，显著优于现有方法，特别是在严重云污染、模态缺失和物候变化预测等挑战性场景中。

Conclusion: UniTS为多种卫星遥感时间序列任务提供了一个统一的生成建模框架，通过创新的架构设计和高质量数据集，实现了跨任务的时空特征统一建模。
...
TL;DR: DeRA是一种新型的1D视频分词器，通过解耦时空表示学习来提高训练效率和性能，在UCF-101上比之前最好的视频分词器LARP提升了25%的rFVD指标。


<details>
  <summary>Details</summary>
Motivation: 传统视频分词器在时空表示学习上存在效率问题，DeRA旨在通过解耦空间和时间表示来提高训练效率和性能，同时解决异质监督带来的梯度冲突问题。

Method: DeRA采用1D潜在空间，将视频编码分解为外观和运动两个流，分别与预训练视觉基础模型对齐以捕获空间语义和时间动态。为解决梯度冲突，提出了对称对齐-冲突投影（SACP）模块，主动重构梯度并抑制冲突方向的分量。

Result: 在UCF-101数据集上，DeRA比之前最先进的视频分词器LARP在rFVD指标上提升了25%。在自回归视频生成任务中，在UCF-101类条件生成和K600帧预测上都达到了新的最先进结果。
...
TL;DR: COMET是一个实时运行的自动回归框架，通过Transformer条件VAE实现精确的关节控制，并引入参考引导反馈机制确保长期稳定性，支持实时风格迁移。
...
Method: 提出COMET框架，采用基于Transformer的条件变分自编码器实现精确交互控制，引入参考引导反馈机制防止误差累积并确保长期稳定性，该机制还可作为即插即用的风格化模块。

Result: COMET能够以实时速度稳健生成高质量运动，在复杂运动控制任务中显著优于现有最先进方法，证明其已准备好用于要求苛刻的交互应用。

Conclusion: COMET框架成功解决了实时角色动画生成中的稳定性和可控性问题，通过创新的反馈机制实现了长期稳定性和实时风格迁移能力，为交互应用提供了实用解决方案。
...
TL;DR: UltraImage是一个解决图像扩散变换器在超出训练分辨率时出现内容重复和质量下降问题的框架，通过频率校正和自适应注意力集中实现高质量的超分辨率图像生成。


<details>
  <summary>Details</summary>
Motivation: 当前图像扩散变换器虽然能生成高保真图像，但在超出训练尺度时会出现内容重复和质量下降的问题，这限制了其在实际应用中的扩展能力。

Method: 1. 通过位置嵌入的频率分析识别出内容重复源于主导频率的周期性；2. 引入递归主导频率校正来约束外推后的频率周期；3. 提出熵引导的自适应注意力集中机制，根据注意力熵分配不同的聚焦因子来平衡局部细节和全局结构。

Result: UltraImage在Qwen-Image和Flux（约4K分辨率）上优于现有方法，显著减少了内容重复并提高了视觉保真度。更重要的是，它能够从1328p的训练分辨率生成高达6K*6K的图像，展示了极强的外推能力。

Conclusion: UltraImage通过解决位置嵌入的频率周期性和注意力稀释问题，成功实现了图像扩散变换器的极端外推，为高质量大尺度图像生成提供了有效的解决方案。
...
TL;DR: 本文提出DuGI-MAE，一种基于MAE的双域引导红外基础模型，通过熵基确定性掩码策略和双域引导模块解决红外图像中的信息丢失、全局关联不足和非均匀噪声问题，在构建的Inf-590K数据集上预训练，在多个下游任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有基于可见光数据训练的MAE等基础模型在红外图像解释任务中表现不佳，而现有的红外基础模型InfMAE仍存在信息token遗漏、全局关联建模不足以及非均匀噪声忽略等局限性。
...
Conclusion: DuGI-MAE通过创新的掩码策略和双域引导模块有效解决了红外基础模型的关键挑战，在构建的大规模红外数据集上预训练后，在多种红外视觉任务中表现出卓越性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有自回归模型在生成长的第一人称视频时存在内容漂移问题，物体身份和场景语义随时间退化，需要解决长期记忆管理挑战。

Method: 结合长期稀疏KV缓存稳定全局上下文，注意力短期记忆通过LoRA进行局部适应，使用内存调节损失确保一致性记忆使用，结构化叙事提示提供显式时间指导。

Result: 在EgoVid-5M基准测试中，EgoLCD在感知质量和时间一致性方面达到最先进性能，有效缓解生成遗忘问题。

Conclusion: EgoLCD通过高效内存管理框架解决了第一人称长视频生成的内容漂移问题，为构建可扩展的具身AI世界模型迈出了重要一步。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分割面临标注数据稀缺和计算成本高的挑战。现有方法仍需下游任务特定训练，而SAM等基础模型在医学领域存在域偏移问题，需要高效的零样本增强方法。

Method: 提出BA-TTA-SAM框架：1）编码器级高斯提示注入，将基于高斯的提示直接嵌入图像编码器，为初始表示学习提供显式指导；2）跨层边界感知注意力对齐，利用ViT主干中的层次特征交互，对齐深层语义响应与浅层边界线索。
...
TL;DR: Auto3R是一个数据驱动的三维扫描与重建自动化系统，通过不确定性量化模型预测最佳扫描视角，无需真实几何信息，在非朗伯体和镜面材质物体上表现优异。
...
Method: 提出Auto3R数据驱动的不确定性量化模型，在迭代的3D重建和扫描过程中，无需真实几何和外观信息，就能高效准确地预测潜在扫描视角的不确定性分布。

Result: 通过大量实验，Auto3R性能大幅超越现有最先进方法。在配备摄像头的机械臂上部署后，能有效数字化真实世界的3D物体，生成即用型、照片级真实的数字资产。

Conclusion: Auto3R实现了3D扫描重建的完全自动化，特别适用于复杂材质物体，为具身系统提供了强大的三维感知能力，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型主要依赖外观匹配，在需要理解物理动力学的场景中表现不佳。物理运动建模面临三个关键挑战：运动信号与外观变化纠缠、需要连续时间运动表示和物理动力学建模、物理属性标注成本高且不切实际。
...
Conclusion: 通过显式将物理运动建模融入视频大语言模型，PhyVLLM能够更好地理解视频中的物理动力学，在无需昂贵标注的情况下提升模型的物理推理能力和通用视频理解性能。
...
TL;DR: Refaçade是一个用于图像和视频中对象重纹理的新方法，能够将参考对象的局部纹理精确转移到目标对象上，解决了现有方法在可控性方面的限制。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像和视频编辑方面取得了显著进展，但对象重纹理任务尚未充分探索。现有方法如基于ControlNet的解决方案存在可控性限制：原始参考图像引入了不需要的结构信息，且无法解耦源对象的视觉纹理和结构信息。

Method: 提出Refaçade方法，包含两个关键设计：1) 使用在成对的纹理/无纹理3D网格渲染上训练的纹理去除器，去除外观信息同时保留源视频的几何和运动；2) 通过拼图排列破坏参考对象的全局布局，使模型专注于局部纹理统计而非对象的全局布局。

Result: 大量实验证明该方法在视觉质量、精确编辑和可控性方面表现优异，在定量和人工评估中都优于强基线方法。

Conclusion: Refaçade方法能够实现图像和视频中精确可控的纹理转移，解决了现有方法在纹理-结构解耦和可控性方面的限制，为对象重纹理任务提供了有效解决方案。
...
TL;DR: X-Humanoid是一个生成式视频编辑方法，将人类视频转换为类人机器人视频，解决了VLA模型和世界模型训练数据稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 具身AI和人形机器人发展受限于大规模多样化训练数据的稀缺。现有方法主要针对第一人称视角的机械臂叠加，无法处理复杂全身运动和第三人称视频中的场景遮挡，不适合将人类视频"机器人化"。

Method: 将Wan 2.2模型适配为视频到视频结构，并针对人类到类人机器人转换任务进行微调。开发了可扩展的数据创建流程，使用Unreal Engine将社区资源转化为17+小时的配对合成视频。

Result: 将训练模型应用于60小时的Ego-Exo4D视频，生成并发布了包含360万+帧"机器人化"类人机器人视频的大规模数据集。定量分析和用户研究显示方法优于现有基线：69%用户认为运动一致性最佳，62.1%用户认为具身正确性最佳。

Conclusion: X-Humanoid成功解决了人类视频到类人机器人视频转换的技术挑战，为具身AI和人形机器人研究提供了高质量的大规模训练数据，显著提升了运动一致性和具身正确性。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在超长视频理解方面存在局限，主要受限于上下文长度不足和长期记忆保留效率低下。虽然已有工作尝试构建外部知识库和检索增强生成系统，但这些方法需要巨大的存储和计算开销。

Method: 提出VideoMem框架，将长视频理解建模为序列生成任务，通过动态更新全局内存缓冲区自适应保留关键信息并丢弃冗余内容。采用渐进分组相对策略优化算法，包含渐进状态传播模块（自适应保留有效当前状态并传播到下一步）和时间级联奖励模块（缓解奖励稀疏性）。

Result: 在多个超长视频理解基准测试中，VideoMem显著优于现有的开源模型，表现出卓越的性能。

Conclusion: VideoMem通过自适应内存管理和创新的训练算法，有效解决了超长视频理解的挑战，为长视频理解任务提供了高效且可扩展的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅（3DGS）在新视角合成中表现出色，但其表面重建质量有待提升。作者观察到良好重建的表面具有低构型熵的特征，即主导基元清晰定义表面几何，而冗余成分被抑制。

Method: 提出三个互补技术贡献：1）通过熵最小化实现熵驱动的表面建模；2）使用表面邻域冗余指数（SNRI）和图像熵引导加权的自适应空间正则化；3）通过竞争性跨尺度熵对齐实现多尺度几何保持。

Result: 在DTU和T&T基准测试中达到竞争性几何精度，在Mip-NeRF 360上获得优于现有方法的渲染质量。具体指标：DTU上Chamfer Distance为0.64，T&T上F1分数为0.44，Mip-NeRF 360上SSIM为0.855（最佳），LPIPS为0.136（最佳）。
...
TL;DR: 本文提出针对文档视觉问答（DocVQA）系统的新型对抗攻击方法，通过视觉不可察觉但语义有针对性的文档内容伪造，诱导模型产生特定或普遍错误答案。


<details>
  <summary>Details</summary>
Motivation: 尽管现有DocVQA模型表现出色，但它们仍易受对抗攻击。本文旨在揭示DocVQA系统在面对视觉不可察觉的文档内容伪造时的脆弱性，这些伪造旨在诱导特定或普遍的错误答案。
...
Result: 攻击方法对两种最先进的DocVQA模型（Pix2Struct和Donut）都表现出有效性，成功诱导了特定或普遍的错误答案，证明了当前DocVQA系统存在严重的安全漏洞。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在3D感知推理方面仍有困难，现有方法通常孤立地增强感知（通过深度、分割等辅助模态）或推理（通过空间VQA数据集训练和强化学习），缺乏统一的方法来同时提升空间感知和推理能力

Method: 提出COOPER统一MLLM，利用深度和分割作为辅助模态，采用两阶段训练：第一阶段学习辅助模态生成能力，第二阶段学习自适应交错推理能力，使模型能够内在增强空间感知并通过交错推理实现更强的空间智能

Result: COOPER在空间推理上平均提升6.91%，同时保持通用性能；仅训练辅助模态生成的变体在距离和大小估计上获得7.92%的提升，表明学习生成辅助模态有助于内化空间知识并增强空间理解

Conclusion: 统一的MLLM可以通过学习生成辅助模态和自适应交错推理来发展内在的空间感知增强能力，实现更强的空间智能，这为提升多模态模型的空间推理能力提供了有效途径
...
TL;DR: 本文综述了病理学深度学习模型开发中高质量大规模数据集创建的关键步骤与挑战，包括图像采集、标注软件选择和标注创建，并提供了标准操作流程以支持数据集开发。


<details>
  <summary>Details</summary>
Motivation: 深度学习在病理图像分析中潜力巨大，但其开发和验证严重依赖高质量大规模数据集。然而，创建这样的数据集面临时间限制、领域变异性和标注偏见等挑战，需要系统指导来推动高质量数据集的创建。

Method: 本文提供了数据集创建的综合指南，涵盖三个关键步骤：1) 图像采集，2) 标注软件选择，3) 标注创建。特别强调处理图像变异源（领域偏移），并提出了标注质量的三个"C"标准：正确性、完整性和一致性。通过使用先进技术减轻单个标注者的局限性来提升标注质量。

Result: 文章提供了标准操作流程作为补充材料，概述了数据集开发的最佳实践。强调了开放数据集在推动创新和增强深度学习研究可重复性方面的重要性。
...
TL;DR: TARDis：一种物理感知框架，将缺失的CT相位重新定义为连续时间衰减曲线上的缺失采样点，通过解耦静态解剖和动态灌注特征来解决缺失模态问题


<details>
  <summary>Details</summary>
Motivation: 临床CT扫描中由于辐射担忧或扫描限制，常常无法获得完整的多相位序列，导致"缺失模态"问题。现有深度学习方法通常将缺失相位视为独立的缺失通道，忽略了血流动力学的内在时间连续性。
...
Conclusion: TARDis通过将缺失模态重新定义为连续时间衰减曲线上的缺失采样点，并解耦静态和动态特征，为解决CT扫描中的缺失模态问题提供了有效方案，具有在减少辐射暴露的同时保持诊断精度的潜力。
...
TL;DR: 该研究提出了AbstainEQA数据集，专注于具身问答中的"弃权"能力——让智能体知道何时因信息不足而拒绝回答问题，这是现有EQA基准忽略的重要能力。
...
Method: 1. 通过500个人类查询的初步研究，识别出需要弃权的五种代表性类别；2. 基于认知理论的人类沟通错误分类；3. 在OpenEQA基础上，让标注者将明确问题转化为模糊变体；4. 构建包含1,636个弃权案例和对应原始实例的AbstainEQA数据集。

Result: 1. 最佳前沿模型仅达到42.79%的弃权召回率，而人类达到91.17%；2. 模型扩展、提示工程和推理仅带来边际改进；3. 微调模型容易过拟合到文本线索；4. 弃权能力是具身环境中可靠交互的基本前提。

Conclusion: 弃权能力是具身问答中可靠交互的基本前提，也是有效澄清的必要基础。当前模型在这方面表现不佳，需要新的方法来解决这一核心挑战。
...
TL;DR: 提出一种零样本检测管道，能同时检测有害图像内容、识别关键元素并进行像素级定位，通过融合基础分割模型和视觉语言模型实现高精度恶意内容检测。


<details>
  <summary>Details</summary>
Motivation: 当前图像内容审核通常只提供图像级别的NSFW标志，但实际审核需要知道哪些具体对象使图像非法以及这些对象的位置，需要更细粒度的检测方法。

Method: 使用基础分割模型(SAM)生成候选对象掩码并细化为独立区域，通过视觉语言模型使用开放词汇提示对每个区域进行恶意相关性评分，加权融合生成统一的恶意对象图，采用多分割器集成增强对抗攻击鲁棒性。

Result: 在790张包含毒品、性、暴力和极端主义内容的数据集上，方法达到85.8%元素级召回率、78.1%精确率和92.1%分割成功率，比直接零样本VLM定位提高27.4%召回率；对抗PGD攻击时精度和召回率下降不超过10%，处理每张图像仅需数秒。

Conclusion: 该方法首次实现了实用化的细粒度、可解释恶意图像审核工具，能无缝集成到现有VLM工作流中，为内容审核提供更精确的对象级检测能力。
...
TL;DR: HeFT是一个零样本点跟踪框架，利用预训练视频扩散模型的视觉先验，通过分析VDiT内部表示发现注意力头具有不同功能，提出头-频率感知特征选择策略，在TAP-Vid基准上达到最先进的零样本跟踪性能。
...
Method: 分析VDiT内部表示，发现注意力头作为最小功能单元具有匹配、语义理解和位置编码等专门化功能；提出头-频率感知特征选择策略，联合选择信息量最大的注意力头和低频分量；通过单步去噪提取判别特征，应用特征选择，采用软argmax定位和前后一致性检查进行对应估计。

Result: 在TAP-Vid基准测试中，HeFT实现了最先进的零样本跟踪性能，接近监督方法的准确性，同时消除了对标注训练数据的需求。

Conclusion: 视频扩散模型可以作为强大的基础模型用于广泛的下游任务，HeFT展示了利用这些模型视觉先验进行零样本点跟踪的潜力，为统一的视觉基础模型铺平了道路。
...
TL;DR: SEASON是一种无需训练的自诊断对比解码方法，通过动态诊断每个输出token的幻觉倾向，并对其对应的时空负样本进行自适应对比解码，从而提升视频大语言模型的时空忠实度。


<details>
  <summary>Details</summary>
Motivation: 当前视频大语言模型在视频理解中难以有效感知和利用丰富的时序信息，导致生成的事件描述存在时序不一致或因果不合理的问题，产生严重的幻觉问题。现有研究主要关注空间幻觉（如物体不匹配），而时序推理在视频理解中相对较少探索。

Method: 提出自诊断对比解码（SEASON），这是一种无需训练的方法，通过动态诊断每个输出token的幻觉倾向，并对其对应的时空负样本进行自适应对比解码，从而自适应地增强每个输出token的时空忠实度。

Result: 在三个幻觉检测基准测试中，SEASON优于所有现有的无需训练幻觉缓解方法。同时，在四个通用视频理解基准测试中进一步提升了视频大语言模型的性能。

Conclusion: SEASON通过自诊断对比解码有效解决了视频大语言模型中的时空幻觉问题，无需额外训练即可显著提升模型的时空忠实度和视频理解能力。
...
Method: 提出I2I-Bench基准测试，具有三个核心特点：(1) 多样化任务：涵盖10个任务类别，包括单图像和多图像编辑任务；(2) 全面评估维度：包含30个解耦的细粒度评估维度，采用自动化混合评估方法，结合专业工具和大规模多模态模型；(3) 严格对齐验证：确保基准评估与人类偏好的一致性。

Result: 使用I2I-Bench对众多主流图像编辑模型进行了基准测试，揭示了不同模型在各种维度上的差距和权衡。所有组件将开源以促进未来研究。

Conclusion: I2I-Bench是一个全面、可扩展的图像编辑基准测试框架，通过自动化混合评估方法解决了现有基准的局限性，为图像编辑模型的评估和发展提供了重要工具。
...
TL;DR: Live Avatar：首个实现实时高保真虚拟人生成的算法-系统协同设计框架，通过时间步强制流水线并行打破自回归瓶颈，实现20FPS端到端生成


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的视频生成方法存在顺序计算和长序列不一致性问题，限制了在实时流式音频驱动虚拟人合成中的实际应用

Method: 1. 时间步强制流水线并行（TPP）：将去噪步骤流水线化到多个GPU，打破自回归瓶颈；2. 滚动汇帧机制（RSFM）：通过缓存参考图像动态重新校准外观，增强时间一致性；3. 自强制分布匹配蒸馏：实现大规模模型的可流式因果适配

Result: 在5个H800 GPU上实现20FPS端到端生成，首次实现实用级实时高保真虚拟人生成，达到最先进性能
...
TL;DR: 本文提出Reward Forcing框架，通过EMA-Sink和Re-DMD两项关键技术解决视频生成中初始帧复制和运动动态不足的问题，实现高效流式视频生成。
...
Method: 提出Reward Forcing框架：1) EMA-Sink：维护固定大小的token，从初始帧初始化并通过指数移动平均融合被移出滑动窗口的token，无需额外计算成本；2) Re-DMD：基于奖励的分布匹配蒸馏，通过视觉语言模型评估动态性，优先处理高动态样本，引导模型输出分布偏向高奖励区域。

Result: Reward Forcing在标准基准测试中达到最先进性能，在单个H100 GPU上实现23.1 FPS的高质量流式视频生成，显著提升运动质量同时保持数据保真度。
...
TL;DR: 该论文提出了跨视角点对应任务(CVPC)和CrossPoint-Bench基准测试，构建了CrossPoint-378K数据集，并开发了CroPond模型，在跨视角点对应任务上显著超越了现有最先进模型。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在跨视角点对应能力上存在严重不足，特别是精确的点级对应能力，这对于精确的affordance交互至关重要。现有模型与人类表现存在巨大差距，需要专门的任务、基准和数据集来推动这一能力的发展。

Method: 1) 提出跨视角点对应(CVPC)任务；2) 构建CrossPoint-Bench基准测试，采用"感知-推理-对应"的三层认知过程设计；3) 创建CrossPoint-378K数据集，包含378K问答对，覆盖900个场景，专注于可操作的affordance区域；4) 开发CroPond模型，在CrossPoint-378K数据集上进行训练。

Result: 评估显示最先进模型(如Gemini-2.5-Pro)与人类表现存在超过54.65%的准确率差距。CroPond模型在CrossPoint-Bench上取得了最先进性能，比Gemini-2.5-Pro提升了39.7%的准确率。

Conclusion: 该研究揭示了当前VLMs在跨视角点对应能力上的严重不足，提出的任务、基准、数据集和模型为推进这一领域的发展奠定了基础，有助于实现更精确的具身AI交互能力。
...
Method: 提出OmniScaleSR框架，引入显式的扩散原生尺度控制机制，与隐式尺度适应协同工作，实现尺度感知和内容感知的扩散过程调制。同时结合多领域保真度增强设计，提高重建精度。

Result: 在双三次降质基准测试和真实世界数据集上的实验表明，OmniScaleSR在保真度和感知真实感方面均超越了现有最先进方法，特别是在大放大倍数下表现尤为出色。

Conclusion: OmniScaleSR通过显式尺度控制与隐式尺度适应的协同作用，成功解决了扩散模型在任意尺度超分辨率中的尺度控制问题，实现了高保真度和高真实感的任意尺度图像超分辨率。
...
TL;DR: E3AD是一个情感感知的端到端自动驾驶框架，通过整合连续情感模型和双路径空间推理，提升视觉定位和路径规划性能，实现更人性化的自动驾驶体验。


<details>
  <summary>Details</summary>
Motivation: 当前端到端自动驾驶系统主要采用视觉-语言-动作模型，但普遍忽略了乘客的情感状态，而情感状态对驾驶舒适度和自动驾驶接受度至关重要。需要开发能够理解自然语言指令、推断情感并规划可行轨迹的自动驾驶系统。
...
Result: 在真实世界数据集上，E3AD提升了视觉定位和路径规划性能，在情感估计方面达到了最先进的VAD相关性。结果表明，将情感注入VLA风格的驾驶能够产生更人性化的定位、规划和反馈。

Conclusion: 将情感感知整合到端到端自动驾驶框架中，能够实现更人性化的视觉定位、路径规划和反馈，提升自动驾驶系统与人类需求的契合度。
...
<details>
  <summary>Details</summary>
Motivation: 现有草图到形状模型忽略笔划的时间顺序，丢弃了关于结构和设计意图的关键线索。VR草图提供了比传统CAD工具更快、更直观的3D设计方式，但需要更好地捕捉设计过程信息。

Method: 1) 从任意形状生成顺序VR草图的自动化流水线；2) 包含4个类别超过20k合成和900手绘草图-形状对的数据集；3) 顺序感知草图编码器与基于扩散的3D生成器相结合

Result: 相比先前工作获得更高的几何保真度，从合成草图到真实草图的泛化效果好且只需最小监督，即使在部分草图上也能表现良好

Conclusion: VRSketch2Shape是首个从顺序VR草图生成3D形状的框架，通过利用笔划顺序信息显著提升了生成质量，为VR草图设计提供了更有效的工具
...
TL;DR: PaCo-RL是一个用于一致图像生成的强化学习框架，包含PaCo-Reward一致性评估器和PaCo-GRPO高效RL算法，在数据稀缺情况下学习复杂视觉一致性标准
...
Method: 提出PaCo-RL框架：1) PaCo-Reward：通过自动子图配对构建的大规模数据集训练的成对一致性评估器，采用生成式自回归评分机制，结合任务感知指令和思维链推理；2) PaCo-GRPO：采用分辨率解耦优化策略大幅降低RL成本，结合对数驯服多奖励聚合机制确保平衡稳定的奖励优化

Result: PaCo-Reward显著提高了与人类视觉一致性感知的对齐度，PaCo-GRPO在一致性性能上达到最先进水平，同时提高了训练效率和稳定性

Conclusion: PaCo-RL作为一个实用且可扩展的解决方案，展示了强化学习在一致图像生成任务中的潜力，能够以数据自由的方式学习复杂且主观的视觉标准
...
TL;DR: LaFiTe是一个3D原生纹理生成框架，通过生成式稀疏潜在颜色场实现高保真、无缝的3D表面纹理，在重建质量上超过现有方法10dB以上。


<details>
  <summary>Details</summary>
Motivation: 现有3D原生纹理生成方法缺乏强大且通用的潜在表示，这严重限制了生成纹理的保真度和通用性，成为进一步发展的主要障碍。

Method: 使用变分自编码器将复杂表面外观编码为稀疏结构化潜在空间，然后解码为连续颜色场；基于此表示，采用条件修正流模型合成高质量、连贯的纹理。

Result: 在重建质量上超过现有方法10dB PSNR以上，有效解耦了纹理外观与网格拓扑和UV参数化，为3D原生纹理生成设立了新基准。
...
TL;DR: RobustSplat++通过延迟高斯增长策略、尺度级联掩码引导和外观建模，解决了3D高斯溅射在野外场景中受瞬态物体和光照变化影响的问题，显著提升了渲染质量。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯溅射方法在处理包含瞬态物体和光照变化的野外场景时，由于高斯密度化过程会无意中建模这些干扰因素，导致渲染图像出现伪影。

Method: 1. 延迟高斯增长策略：优先优化静态场景结构，再允许高斯分裂/克隆，避免早期优化过拟合瞬态物体
2. 尺度级联掩码引导：先利用低分辨率特征相似性监督获得可靠的初始瞬态掩码，再逐步过渡到高分辨率监督实现更精确的掩码预测
3. 结合延迟增长策略和掩码引导进行外观建模，处理包含瞬态和光照变化的野外场景

Result: 在多个具有挑战性的数据集上的广泛实验表明，该方法优于现有方法，清晰证明了其鲁棒性和有效性。

Conclusion: RobustSplat++通过创新的延迟高斯增长策略和尺度级联掩码引导方法，有效解决了3D高斯溅射在野外场景中的鲁棒性问题，显著提升了渲染质量。
...
TL;DR: LatentFM：一种在潜在空间中运行的基于流的医学图像分割模型，通过变分自编码器编码图像和掩码，学习条件速度场生成多样分割结果，并提供不确定性估计。


<details>
  <summary>Details</summary>
Motivation: 受流匹配（FM）在生成模型中取得的显著进展启发，作者希望将这种能够学习精确数据密度的无模拟流框架应用于医学图像分割，实现高精度且具有不确定性感知的预测。
...
Conclusion: LatentFM成功地将流匹配框架应用于医学图像分割，实现了高精度、不确定性感知的预测，为临床医生提供了更丰富的信息进行深入分析，展示了在潜在空间中运行流模型的优势。
...
TL;DR: FreeGen：用于自动驾驶自由视点场景合成的重建-生成协同训练框架，通过重建模型保证插值一致性，生成模型提升未见视点真实感
...
Method: 提出重建-生成协同训练框架，重建模型提供稳定几何表示确保插值一致性，生成模型进行几何感知增强提升未见视点真实感

Result: 实验证明FreeGen在自由视点驾驶场景合成方面达到了最先进的性能

Conclusion: 通过重建与生成的协同训练，实现了插值一致性和外推真实性的平衡，为自动驾驶仿真和预训练提供了有效解决方案
...
TL;DR: 本文提出MID-FFD新范式解决多域深度伪造检测中域差异主导特征空间的问题，通过DevDet框架放大真假差异，在保持泛化能力的同时提升单图像判断准确率。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测方法追求通用性，但在训练伪造样本和域有限的情况下，期望完全覆盖未见变体过于理想化。虽然引入大规模多域数据训练可行且重要，但在多域场景中，域间差异而非细微的真假差异主导特征空间，导致检测器在域未指定条件下单图像判断准确率低。

Method: 提出MID-FFD研究范式，包含足够多的真假域训练数据；提出模型无关框架DevDet，包括Face Forgery Developer（FFDev）放大真假差异，以及Dose-Adaptive detector Fine-Tuning策略（DAFT）。

Result: 实验证明该方法在MID-FFD场景下预测真假的优越性，同时保持对未见数据的原始泛化能力。

Conclusion: MID-FFD范式为现实世界深度伪造检测提供了更实用的框架，DevDet通过放大真假差异解决了多域场景中的域主导问题，在保持泛化能力的同时显著提升了单图像判断准确率。
...
TL;DR: LineAR是一种无需训练、基于渐进式KV缓存压缩的自回归图像生成方法，通过利用视觉注意力的内在特性，以行级管理缓存，在保持生成质量的同时显著减少内存占用并提高吞吐量。
...
Method: 提出LineAR方法，这是一种无需训练的渐进式KV缓存压缩流水线。通过利用视觉注意力的内在特性，以2D视图在行级管理缓存，保留视觉依赖区域，同时逐步淘汰对后续行生成无害的信息较少标记，由行间注意力指导。

Result: 在六个自回归图像生成模型上的实验验证了其有效性：在LlamaGen-XL上将ImageNet FID从2.77提升到2.68，在Janus-Pro-1B上将COCO FID从23.85提升到22.86，仅保留1/6 KV缓存；在Lumina-mGPT-768上仅使用1/8缓存就提升了DPG；实现了显著的内存和吞吐量提升，包括在LlamaGen-XL上减少67.61%内存和7.57倍加速，在Janus-Pro-7B上减少39.66%内存和5.62倍加速。

Conclusion: LineAR通过高效的KV缓存压缩，在保持甚至提升生成质量的同时，显著减少了自回归图像生成的内存占用并提高了吞吐量，具有广泛适用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频姿态估计方法在自接触场景（如手触脸）中经常失败，而可穿戴生物阻抗传感能廉价、无干扰地测量真实皮肤接触，因此需要结合两者优势来提升野外3D人体姿态捕捉精度

Method: BioTUCH框架：1）使用现成姿态估计器初始化姿态；2）在测量到自接触时进行接触感知的姿态优化，最小化重投影误差和输入估计偏差，同时强制执行顶点接近约束

Result: 使用同步RGB视频、生物阻抗测量和3D动作捕捉的新数据集验证，测试三种输入姿态估计器，平均重建精度提升11.7%；还开发了微型可穿戴生物阻抗传感器用于大规模接触感知训练数据收集

Conclusion: BioTUCH通过结合视觉姿态估计与生物阻抗传感，显著提升了自接触场景下的3D人体姿态重建精度，为姿态估计和运动生成方法提供了更好的训练数据
...
TL;DR: SP-Det是一种自提示检测框架，通过自动生成文本提示来指导胸部X光多病灶检测，无需专家标注


<details>
  <summary>Details</summary>
Motivation: 现有可提示检测框架依赖人工标注作为提示，这在临床应用中劳动密集且不切实际，需要自动生成提示的方法

Method: 提出SP-Det框架，包含专家无关的双文本提示生成器（DTPG）和双向特征增强器（BFE），DTPG生成语义上下文提示和疾病信标提示，BFE集成诊断上下文与疾病特定嵌入

Result: 在两个胸部X光数据集上的实验表明，SP-Det优于最先进的检测方法，同时完全消除了对专家标注提示的依赖

Conclusion: SP-Det通过自动生成丰富的文本上下文，实现了无需专家标注的高效多病灶检测，为临床应用提供了实用解决方案
...
Result: 在空地跟踪站上的实验表明，SDG-Track实现了35.1 FPS的系统吞吐量，同时保持了97.2%的逐帧检测精度。系统在NVIDIA Jetson Orin Nano上成功跟踪了现实操作条件下的敏捷FPV无人机。

Conclusion: SDG-Track通过观察者-跟随者架构和双空间恢复机制，有效解决了边缘设备上小型无人机跟踪的分辨率-速度冲突，实现了高精度和高帧率的实时跟踪性能。
...
<details>
  <summary>Details</summary>
Motivation: 该研究的动机源于诊断性MRI扫描中胎儿头部运动跟踪的挑战性问题。目标是实现6自由度头部姿态估计，支持在每次2D切片采集前快速获取3D MRI体积，从而实现2D诊断MRI切片的自动自适应处方。

Method: E(3)-Pose方法通过显式建模旋转等变性和物体对称性来联合处理姿态估计问题。该方法通过构造捕获解剖对称性和刚性姿态等变性，从而产生稳健的胎儿头部姿态估计。

Result: 在公开可用的代表性临床胎儿MRI数据集上的实验表明，该方法在跨域情况下具有优越的稳健性和泛化能力。最重要的是，E(3)-Pose在临床MRI体积上实现了最先进的精度，为临床转化铺平了道路。

Conclusion: E(3)-Pose通过显式建模旋转等变性和物体对称性，成功解决了现有方法因解剖对称性、低分辨率、噪声和伪影导致的姿态模糊问题，在临床MRI数据上实现了卓越的性能，具有重要的临床应用前景。
...
<details>
  <summary>Details</summary>
Motivation: 尽管Flow Matching方法取得了巨大进展，但仍然存在训练和推理不一致导致的曝光偏差问题。本文研究了曝光偏差的根本原因：1）模型对训练中的偏差输入缺乏泛化能力；2）早期去噪过程中低频内容捕捉不足，导致偏差累积。

Method: 提出ReflexFlow方法，包含两个组件：1）反漂移校正（ADR）：在训练时计划采样下，通过重新设计的损失函数反射性地调整偏差输入的预测目标；2）频率补偿（FC）：通过反映缺失的低频分量，并使用曝光偏差重新加权损失来补偿这些分量。

Result: 在CIFAR-10、CelebA-64和ImageNet-256上的实验表明，ReflexFlow在减轻曝光偏差方面优于先前方法，在CelebA-64上实现了35.65%的FID降低。

Conclusion: ReflexFlow是一种模型无关、兼容所有Flow Matching框架的简单有效方法，能够动态纠正曝光偏差，提高生成质量。
...
TL;DR: SFD提出了一种语义优先的潜在扩散范式，通过异步去噪语义和纹理潜在表示，让语义形成先于纹理生成，实现更自然的从粗到细的图像生成。


<details>
  <summary>Details</summary>
Motivation: 现有潜在扩散模型虽然遵循从粗到细的生成过程，但语义和纹理是同步去噪的，忽略了语义形成先于纹理生成的顺序特性。这限制了语义先验对纹理生成的指导作用。

Method: SFD首先构建复合潜在表示，包含从预训练视觉编码器提取的紧凑语义潜在和纹理潜在。核心创新是使用不同的噪声调度异步去噪语义和纹理：语义去噪先于纹理一个时间偏移，为纹理细化提供更清晰的高层语义指导。

Result: 在ImageNet 256×256数据集上，SFD在引导生成下达到FID 1.06（LightningDiT-XL）和FID 1.04（1.0B LightningDiT-XXL），比原始DiT收敛速度快达100倍。SFD还能改进现有方法如ReDi和VA-VAE。

Conclusion: SFD通过显式优先处理语义形成，实现了更自然的从粗到细生成过程，证明了异步、语义主导建模的有效性，在图像质量和收敛速度方面都取得了显著提升。
...
TL;DR: LiteVGGT是一个高效的3D视觉基础模型，通过几何感知的缓存token合并策略，在保持VGGT核心性能的同时实现10倍加速和内存大幅减少，能够处理1000张图像的大规模场景。


<details>
  <summary>Details</summary>
Motivation: 现有3D视觉基础模型如VGGT在处理长序列时存在耗时和内存消耗大的问题，限制了其在大规模场景（超过数百张图像）中的应用。

Method: 提出几何感知的缓存token合并策略：1) 分析token的几何重要性，优化锚点token选择以更好地保留重建关键信息；2) 跨层缓存和重用合并索引，显著减少延迟；3) 支持高效微调和FP8量化。

Result: LiteVGGT实现了高达10倍的加速和显著的内存减少，能够高效处理1000张图像的大规模场景，同时保持VGGT的核心性能。

Conclusion: LiteVGGT通过创新的token合并策略解决了3D视觉基础模型在大规模场景中的效率瓶颈，为大规模3D重建应用提供了高效解决方案。
...
Method: 提出了FASTer框架：FASTerVQ将动作块编码为单通道图像以捕获全局时空依赖并保持高压缩比；FASTerVLA在此基础上采用分块自回归解码和轻量级动作专家。

Result: 实验表明FASTerVQ具有优越的重建质量、高令牌利用率和强大的跨任务、跨具身泛化能力；FASTerVLA在推理速度和任务性能上均超越先前最先进的VLA模型。

Conclusion: FASTer框架通过创新的动作分词器和自回归策略设计，在机器人操作任务中实现了更高的效率和性能，为通用机器人学习提供了有效解决方案。
...
TL;DR: GeoPE是一种新的位置编码方法，通过四元数和李代数几何平均，在3D欧几里得空间中构建旋转操作，有效分离空间维度，恢复2D图像的自然空间拓扑结构。
...
Method: 提出几何位置嵌入(GeoPE)框架，使用四元数将旋转扩展到3D欧几里得空间。通过计算李代数中的几何平均来构建统一的旋转操作，克服非交换性并确保对称性，创建几何耦合的编码。

Result: 在图像分类、目标检测和3D语义分割任务上的大量实验表明，GeoPE始终优于现有的2D RoPE变体，并显著增强了形状偏置，证实了其捕捉真实几何结构的能力。

Conclusion: GeoPE通过恢复2D空间流形，有效分离空间维度，为视觉Transformer提供了更好的几何结构表示能力，在多个视觉任务上取得了显著改进。
...
<details>
  <summary>Details</summary>
Motivation: 视网膜疾病自动诊断很重要，但传统深度学习方法需要大量标注数据，且数据通常存在类别不平衡问题，限制了实际应用的可靠性。少样本学习可以解决数据稀缺问题，但需要处理类别不平衡带来的偏差。

Method: 提出平衡少样本情景学习框架，包含三个关键组件：(1) 平衡情景采样，确保每个5-way 5-shot情景中所有类别平等参与；(2) 针对性增强，包括CLAHE和颜色/几何变换，提高少数类别的多样性；(3) 使用在ImageNet上预训练的ResNet-50编码器，计算嵌入空间中的原型，使用余弦相似度进行分类。

Result: 在RFMiD数据集上训练100个情景并在1000个测试情景上评估，框架实现了显著的准确率提升，减少了对多数类别的偏向，对代表性不足的疾病有显著改进。

Conclusion: 研究表明，结合平衡采样和CLAHE增强预处理的少样本学习管道，可以在数据受限条件下提供更稳健和临床公平的视网膜疾病诊断。
...
TL;DR: 该论文提出了一种名为MoLD的自适应方法，通过门控机制动态整合CLIP-ViT多层特征来改进AI生成图像检测，相比仅使用最后一层特征的方法有显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有AI生成图像检测方法主要利用CLIP-ViT的最后一层特征，但作者通过系统分析发现早期层提供更局部化和泛化性更强的特征，且不同层捕捉数据的不同方面，对检测任务各有独特贡献。

Method: 提出MoLD（多层特征动态整合）方法，使用门控机制自适应地整合来自多个ViT层的特征，能够根据输入图像动态调整各层特征的权重。

Result: 在GAN和扩散模型生成的图像上进行广泛实验，MoLD显著提高了检测性能，增强了跨不同生成模型的泛化能力，并在真实场景中表现出鲁棒性。方法还可成功应用于其他预训练ViT（如DINOv2）。

Conclusion: 通过系统分析ViT层特征对AI生成图像检测的贡献，提出的MoLD方法通过动态整合多层特征，在检测性能、泛化能力和鲁棒性方面均有显著提升，展现了方法的可扩展性和通用性。
...
<details>
  <summary>Details</summary>
Motivation: 需要学习同时包含语义和几何信息的像素级表示，以实现跨图像的精确点对应，同时避免传统动量师生训练的复杂性。

Method: 使用稳定的对比损失函数家族，将图像中的每个像素映射到过完备描述符，这些描述符既具有视图不变性又具有语义意义。

Result: 在合成2D和3D环境中的实验验证了损失函数的特性以及生成的过完备表示的有效性。

Conclusion: 提出的稳定对比损失能够学习到同时捕获语义和几何信息的像素级表示，无需动量师生训练即可实现精确的点对应。
...
TL;DR: LVLM-based文本到图像系统比非LVLM模型产生更多社会偏见，系统提示是主要驱动因素。FairPro框架通过元提示让LVLM自我审计，构建公平感知的系统提示，显著减少偏见同时保持文本-图像对齐。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型(LVLM)已成为图像生成的主流范式，但其是否放大社会偏见尚未充分研究。作者发现LVLM模型比非LVLM模型产生更明显的社会偏见图像，需要深入理解偏见传播机制并开发实用解决方案。

Method: 1) 创建包含1024个提示的基准，涵盖四个语言复杂度级别；2) 通过解码中间表示、令牌概率诊断和嵌入关联分析揭示系统提示如何编码人口统计先验；3) 提出FairPro训练免费元提示框架，让LVLM在测试时自我审计并构建公平感知的系统提示。

Result: 实验显示LVLM模型(SANA和Qwen-Image)产生显著的社会偏见图像。FairPro框架能大幅减少人口统计偏见，同时保持文本-图像对齐质量。系统提示被确定为偏见传播的主要驱动因素。

Conclusion: 系统提示在LVLM偏见传播中起核心作用。FairPro提供了一种实用、可部署的方法来构建更负社会责任的文本到图像系统，为理解偏见机制和开发公平解决方案提供了重要见解。
...
Result: 优化后的框架在保持原有性能的同时，内存消耗减少了97%以上，使其能够在嵌入式GPU上高效运行。

Conclusion: 通过内存优化策略，成功解决了VANICP在资源受限环境中的部署限制，为点云配准算法在嵌入式系统中的应用提供了可行方案。
...
TL;DR: 提出基于扩散Transformer的单图像反射去除框架，利用预训练基础模型，通过物理渲染合成数据和LoRA高效适配，在反射去除任务上取得SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有反射去除方法通常依赖特定任务架构，缺乏泛化能力；同时缺乏多样化、可扩展且逼真的训练数据。本文旨在利用基础扩散模型的泛化优势，结合物理合成数据，提供可扩展的高质量反射去除方案

Method: 1. 采用预训练的扩散Transformer基础模型，通过条件输入反射污染图像，引导生成干净透射层；2. 在Blender中构建基于物理的渲染管线，使用Principled BSDF合成逼真的玻璃材质和反射效果；3. 采用LoRA进行高效模型适配；4. 系统分析现有反射去除数据源的多样性、可扩展性和真实感

Result: 在领域内和零样本基准测试中均达到最先进性能，证明了预训练扩散Transformer结合物理基础数据合成和高效适配的有效性

Conclusion: 预训练扩散Transformer配合物理基础数据合成和高效适配，为反射去除提供了可扩展、高保真的解决方案，展示了基础模型在图像修复任务中的强大泛化能力
...
TL;DR: GNVC-VD是首个基于DiT的生成式神经视频压缩框架，通过视频扩散变换器统一时空潜在压缩和序列级生成细化，解决了传统感知编解码器的时间建模不足和闪烁问题。
...
Method: 1. 引入统一的流匹配潜在细化模块，利用视频扩散变换器通过序列级去噪联合增强帧内和帧间潜在表示；2. 从解码的时空潜在表示初始化细化，而非从纯高斯噪声开始，学习适应压缩引起退化的校正项；3. 使用条件适配器将压缩感知线索注入中间DiT层，在极低比特率下有效去除伪影并保持时间一致性。

Result: 大量实验表明，GNVC-VD在感知质量上超越了传统和学习的编解码器，显著减少了先前生成方法中持续的闪烁伪影，即使在低于0.01 bpp的极低比特率下也能保持良好性能。

Conclusion: GNVC-VD展示了将视频原生生成先验整合到神经编解码器中的前景，为下一代感知视频压缩提供了有希望的解决方案，通过序列级生成细化实现了时空一致的高质量视频重建。
...
### [80] [HTR-ConvText: Leveraging Convolution and Textual Information for Handwritten Text Recognition](https://arxiv.org/abs/2512.05021)
...
Method: 提出HTR-ConvText模型：1）特征提取阶段集成残差CNN主干和带位置编码的MobileViT块；2）引入ConvText编码器，结合全局上下文和局部特征的混合架构；3）辅助模块注入文本上下文以缓解CTC的弱点

Result: 在IAM、READ2016、LAM和HANDS-VNOnDB数据集上评估，相比现有方法获得了更好的性能和泛化能力，特别是在训练样本有限和手写多样性高的场景下

Conclusion: HTR-ConvText通过同时捕捉细粒度笔画级局部特征和全局上下文依赖，有效解决了手写文本识别的关键挑战，在资源受限场景下表现出色
...
<details>
  <summary>Details</summary>
Motivation: 当前地球观测数据涵盖广泛的空间、光谱和时间分辨率，但现有基础模型通常需要固定输入分辨率或基于特定传感器编码器，限制了跨异构EO模态的泛化能力。需要一种能够统一处理不同传感器和分辨率数据的解决方案。

Method: RAMEN将模态、空间和时间分辨率作为关键输入特征，通过单一统一的Transformer编码器重建来自多样来源的掩码多模态EO数据，将空间分辨率定义为可控输出参数，允许用户在推理时直接控制所需细节水平。

Result: RAMEN在社区标准PANGAEA基准测试中表现出色，能够有效迁移到已知和未见过的传感器配置，在包含各种多传感器和多分辨率的下游任务中优于更大的最先进模型。

Conclusion: RAMEN提供了一种传感器无关的解决方案，能够学习跨地球观测数据的共享视觉表示，通过将空间分辨率作为可控参数，实现了空间精度和计算成本之间的显式权衡，为异构EO数据分析提供了统一框架。
...
TL;DR: 提出了一种语义引导的分层合成架构，用于面部图像修复，通过结合CNN和Vision Transformer生成清晰语义布局，再使用多模态纹理生成器进行纹理细化，有效处理大面积不规则掩码。
...
Method: 提出语义引导的分层合成架构：第一阶段结合CNN和Vision Transformer生成清晰语义布局；第二阶段使用多模态纹理生成器进行多尺度信息融合和纹理细化；通过动态注意力机制处理任意掩码配置。

Result: 在CelebA-HQ和FFHQ数据集上实验表明，模型在LPIPS、PSNR和SSIM等指标上优于现有方法，特别是在大面积修复场景中能产生语义保持更好的视觉上令人印象深刻的结果。

Conclusion: 提出的语义引导分层合成架构有效解决了面部图像修复中的关键挑战，通过语义布局生成和纹理细化两阶段处理，实现了更好的语义一致性和视觉质量，且能处理任意掩码配置。
...
<details>
  <summary>Details</summary>
Motivation: 现有4D语义场构建方法主要依赖逐场景优化的高斯泼溅技术，泛化能力有限且难以扩展到实际应用。需要一种能够联合训练、直接推理的统一框架来提升4D场景理解的实用性。

Method: 提出4DLangVGGT框架，包含两个关键组件：4D视觉几何Transformer（StreamVGGT）捕捉动态场景的时空几何表示；语义桥接解码器（SBD）将几何感知特征投影到语言对齐的语义空间，在保持结构保真度的同时增强语义可解释性。

Result: 在HyperNeRF和Neu3D数据集上的实验表明，该方法不仅有效泛化，而且达到最先进性能：逐场景训练下提升2%，多场景训练下提升1%。

Conclusion: 4DLangVGGT通过统一的Transformer架构实现了4D语言接地，无需逐场景优化，显著提升了大规模部署的实用性，为开放词汇4D场景理解建立了新范式。
...
TL;DR: 该论文提出了一种结合生成模型先验和接触信息来改善机器人操作中物体几何重建的方法


<details>
  <summary>Details</summary>
Motivation: 机器人操作需要准确的物体几何信息，但相机只能捕捉物体的部分观测，尤其是在遮挡情况下。需要利用额外信息来减少视觉信号的模糊性。

Method: 结合两种信息源：1) 生成模型学习常见物体形状的先验，可以对未见几何部分进行合理猜测；2) 从视频和物理交互中获取的接触信息提供几何边界的稀疏约束。通过接触引导的3D生成将两者结合，灵感来源于生成模型中的拖拽式编辑。

Result: 在合成数据和真实世界数据上的实验表明，该方法相比纯3D生成和基于接触的优化方法，能够改善重建效果。

Conclusion: 结合生成模型先验和接触信息能够有效减少视觉重建的模糊性，为机器人操作提供更准确的物体几何信息。
...
TL;DR: Deep Forcing是一种无需训练的方法，通过深度下沉和参与式压缩两种机制，解决自回归视频扩散中的时间重复、漂移和运动减速问题，实现12倍以上的外推生成能力。


<details>
  <summary>Details</summary>
Motivation: 现有的自回归视频扩散方法虽然实现了实时帧流，但仍存在时间重复、漂移和运动减速等问题。直接将StreamingLLM式的注意力下沉应用于视频扩散会导致保真度下降和运动停滞。

Method: 提出Deep Forcing方法，包含两种无需训练的机制：1) Deep Sink：将滑动窗口的一半分配给持久下沉token，并重新对齐其时间RoPE相位到当前时间线，稳定长序列生成中的全局上下文；2) Participative Compression：执行重要性感知的KV缓存剪枝，只保留最近注意力中活跃参与的token，安全丢弃冗余和退化的历史信息，最小化分布外长度生成中的错误累积。

Result: 该方法实现了超过12倍的外推能力（如从5秒训练扩展到60秒以上生成），在图像质量上优于LongLive，在美学质量上优于RollingForcing，几乎保持了整体一致性，在动态程度上获得显著提升，同时保持实时生成。
...
TL;DR: 本文提出视觉推理追踪器（VRT）任务，要求模型不仅定位目标对象，还要预测形成推理路径的中间对象，并创建了VRT-Bench基准、新评估指标和VRT-80k数据集。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视觉定位和视觉问答任务上表现良好，但推理过程不透明，只输出最终预测而不展示中间步骤或细粒度证据，这与人类通过视觉推理链进行思考的方式形成对比。

Method: 提出VRT任务，要求模型定位目标对象并预测推理路径中的中间对象；创建VRT-Bench基准用于评估视觉推理；开发新的推理轨迹质量评估指标；构建VRT-80k大规模数据集用于训练推理模型。

Result: 实验发现现有模型虽然能产生正确最终输出，但在中间推理的定位上存在困难；而使用VRT-80k数据集训练的模型在追踪推理路径方面取得了显著改进。

Conclusion: 通过引入VRT任务和相关资源，能够提高多模态模型的推理透明度，使其更接近人类视觉推理过程，为可解释AI提供了重要进展。
...
TL;DR: 该论文提出了空间美学评估范式SA-IQA，针对AI生成室内图像的质量评估，构建了首个包含18,000张图像和50,000个标注的SA-BENCH基准，并通过MLLM微调和多维度融合方法开发了综合奖励框架，应用于AIGC生成优化和高质量图像筛选。


<details>
  <summary>Details</summary>
Motivation: 当前图像质量评估方法主要针对人像和艺术图像，缺乏对室内场景的系统性评估。为了解决AI生成室内图像的美学质量评估问题，需要建立专门的空间美学评估范式。

Method: 1. 提出空间美学评估范式，从布局、和谐、照明、失真四个维度评估室内图像美学质量；2. 构建SA-BENCH基准数据集；3. 通过MLLM微调和多维度融合方法开发SA-IQA评估框架；4. 应用于两个下游任务：GRPO强化学习优化AIGC生成流程和Best-of-N高质量图像筛选。

Result: SA-IQA在SA-BENCH基准上显著优于现有方法，为空间美学评估设立了新标准。该方法能有效优化AIGC生成质量并筛选高质量图像。

Conclusion: 该研究填补了AI生成室内图像质量评估的空白，提出的空间美学评估范式和SA-IQA框架为相关研究和应用提供了重要工具，代码和数据集将开源以推动该领域发展。
...
Method: 提出EvoIR框架，包含频率调制模块（FMM）显式分解高低频特征并自适应调制，以及进化优化策略（EOS）通过种群进化迭代调整频率感知目标，动态平衡结构精度和感知保真度。

Result: 在多个基准测试上的广泛实验表明，EvoIR优于现有的最先进全合一图像修复方法。

Conclusion: EvoIR通过协同频率调制模块和进化优化策略，实现了更好的图像修复效果，证明了这两个组件的互补作用。
...
<details>
  <summary>Details</summary>
Motivation: 标准扩散使用高斯噪声同时破坏幅度和相位，相位信息的丢失会破坏空间结构，不适用于需要几何一致性的任务（如重渲染、仿真增强、图像到图像转换）。

Method: 提出相位保持扩散（φ-PD），模型无关的扩散过程重构，保留输入相位同时随机化幅度；进一步提出频率选择性结构（FSS）噪声，通过单一频率截止参数控制结构刚性。

Result: φ-PD在照片级真实感和风格化重渲染、驾驶规划器的仿真到真实增强等任务中产生可控的空间对齐结果；应用于CARLA仿真器时，将CARLA到Waymo规划器性能提升50%。

Conclusion: φ-PD无需推理时间成本，与任何图像或视频扩散模型兼容，是现有条件方法的补充，广泛应用于图像到图像和视频到视频生成任务。
...
TL;DR: ShadowDraw框架将3D对象转化为阴影绘画艺术，通过优化场景参数使投射阴影与线条绘画结合，形成可识别的图像。


<details>
  <summary>Details</summary>
Motivation: 创建一种将3D对象转化为阴影绘画艺术的计算方法，通过算法设计连接艺术叙事，扩展计算视觉艺术的设计空间。

Method: 系统预测场景参数（物体姿态和光照）和部分线条绘画，优化场景配置以产生有意义的阴影，使用阴影笔触指导线条绘画生成，并采用自动评估确保阴影绘画的一致性和视觉质量。
...
TL;DR: DraCo是一种新颖的交错推理范式，通过生成低分辨率草稿图像作为预览，提供更具体的视觉规划和指导，然后验证语义对齐并进行选择性修正，解决了文本规划的粗粒度问题和罕见属性组合生成的难题。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型方法存在局限性：要么仅将模型视为独立生成器，要么依赖抽象的文本规划。文本规划过于粗粒度，难以生成罕见属性组合，需要更具体的视觉规划和验证机制。

Method: 提出Draft-as-CoT（DraCo）交错推理范式：1）首先生成低分辨率草稿图像作为预览，提供具体结构化视觉规划；2）利用模型固有理解能力验证草稿与输入提示的语义对齐；3）通过选择性修正和超分辨率进行细化。还开发了DraCo-240K训练数据集和DraCo-CFG专用分类器自由引导策略。

Result: 在GenEval上提升8%，Imagine-Bench提升0.91分，GenEval++提升3%，显著优于直接生成和其他基于CoT的生成方法。

Conclusion: DraCo通过将视觉草稿纳入思维链推理，实现了更有效的规划和验证，解决了多模态生成中的关键挑战，在多个基准测试中取得了显著性能提升。
...
Method: 引入动态高斯泼溅的新应用：场景被动态建模以保留附近时间变化，通过固定模型时间参数渲染静态场景。提出Splannequin正则化方法，检测高斯基元的隐藏和缺陷状态，并应用时间锚定。在前向相机运动下，隐藏状态锚定到最近良好观察的过去状态，缺陷状态锚定到具有更强监督的未来状态。

Result: 该方法显著提高了视觉质量，实现了高保真、用户可选择的冻结时间渲染，获得了96%的用户偏好。无需架构更改，零推理开销，可轻松集成到现有动态高斯管道中。

Conclusion: Splannequin方法有效解决了单目捕获中稀疏时间监督导致的伪影问题，为从Mannequin-Challenge视频合成高质量冻结3D场景提供了实用解决方案。
...
TL;DR: Light-X是一个视频生成框架，能够从单目视频中实现可控渲染，同时支持视角和光照控制，通过解耦几何和光照信号实现高质量的联合控制。


<details>
  <summary>Details</summary>
Motivation: 现有光照控制方法在视频应用中面临光照保真度和时间一致性之间的权衡。超越重光照，实现对真实世界场景的生成建模需要同时控制相机轨迹和光照，因为视觉动态本质上由几何和光照共同塑造。

Method: 1) 提出解耦设计：通过动态点云沿用户定义相机轨迹投影捕捉几何和运动，通过重光照帧一致投影到相同几何提供光照线索；2) 引入Light-Syn：基于退化的合成流程，通过逆映射从野外单目视频合成训练对，覆盖静态、动态和AI生成场景。

Result: Light-X在联合相机-光照控制方面优于基线方法，在文本和背景条件设置下超越了先前的视频重光照方法。

Conclusion: Light-X通过解耦几何和光照信号的设计，结合合成训练数据策略，实现了从单目视频中进行高质量视角和光照联合控制的可控渲染。
