...
TL;DR: HIVE提出了一种新颖的分层预训练框架，通过引入视觉编码器与LLM之间的分层交叉注意力，改善了视觉语言对齐，在多个视觉语言任务上超越了传统方法。
...
Result: HIVE在图像分类和多个视觉语言任务（MME、GQA、OK-VQA、ScienceQA）上表现优异，超越了基于自注意力的方法。

Conclusion: 分层特征整合能显著提升视觉语言模型的性能和表达能力，为更高效、更具表达力的视觉语言模型开辟了新途径。
...
TL;DR: RawGen是首个基于扩散模型的文本到原始图像生成框架，能够为任意目标相机生成物理一致的线性原始图像，同时支持sRGB到原始图像的转换，解决了原始数据稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 原始线性raw数据对低级视觉任务更忠实，但大规模原始数据收集困难，现有数据集有限且与特定相机硬件绑定。现有扩散模型只能生成经过ISP处理的sRGB图像，无法生成物理一致的线性表示。

Method: 利用大规模sRGB扩散模型的生成先验，通过潜在空间和像素空间的专门处理合成物理有意义的线性输出。构建多对一逆ISP数据集，将使用不同ISP参数生成的多个sRGB渲染版本锚定到共同的场景参考目标。微调条件去噪器和专门解码器。

Result: RawGen在传统逆ISP方法（假设固定ISP）上表现出优越性能。通过RawGen可扩展的文本驱动合成数据增强训练流程，能够有益于下游低级视觉任务。

Conclusion: RawGen是首个能够为任意目标相机生成物理一致线性原始表示的扩散框架，解决了原始数据稀缺问题，并为低级视觉任务提供了可扩展的合成数据生成方案。
...
TL;DR: 提出Q-Mask框架，通过因果查询驱动的掩码解码器改进OCR中的文本区域定位能力，解决现有视觉语言模型在文本锚定方面的不足。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在OCR任务中虽然能识别文本，但在将查询文本准确对应到图像空间区域（文本锚定）方面表现不佳，这限制了实际应用中的可靠推理能力。

Method: 提出Q-Mask框架，基于因果查询驱动的掩码解码器，采用视觉链式思维推理范式，先通过查询条件生成视觉掩码确定文本位置，再进行识别，实现文本锚定与识别的解耦。构建TextAnchor-26M大规模数据集进行训练。

Result: Q-Mask显著提升了文本锚定和理解能力，在TextAnchor-Bench基准测试中表现出色，能够处理多样化的视觉场景。

Conclusion: 通过因果视觉解码和视觉链式思维范式，Q-Mask有效解决了文本区域定位问题，为视觉语言模型提供了更可靠的文本锚定能力，支持更精确的下游推理任务。
...
<details>
  <summary>Details</summary>
Motivation: 研究发现MIM（掩码图像建模）目标会导致学习到的表示保留非语义信息，这最终会损害推理性能。需要一种方法来抑制这些非语义信息，提升模型性能。

Method: 提出SOAP（语义正交伪影投影）方法：1）使用PCA分析真实和合成的非语义图像，建立模型无关的语义不变性评分；2）基于该评分，在补丁表示中直接抑制非语义信息；3）作为后处理抑制方法，无需训练，可作为单个线性头附加到任何模型。

Result: SOAP方法在各种基于MIM的模型中带来了一致的零样本性能改进，证明了其有效性。

Conclusion: SOAP是一种简单有效的模型无关方法，能够直接抑制MIM表示中的非语义信息，提升模型性能，且具有零训练成本和易于部署的优点。
...
TL;DR: UCell是一个用于单细胞分割的轻量级模型（10-30M参数），通过递归结构设计实现了参数效率，性能可与大10-20倍的模型相媲美，且无需大规模预训练。


<details>
  <summary>Details</summary>
Motivation: 在生物医学研究中，模型扩展常受限于训练数据不足和高昂的数据生成成本。当前研究多集中于构建更大的基础模型，而提升小模型能力的替代方案却被忽视。本文旨在探索在单细胞分割任务中构建参数高效的小型模型。

Method: 设计了UCell模型，核心创新是在前向计算图中引入递归结构，从而构建参数效率更高的架构。模型仅需10-30M参数，可直接在显微成像数据上从头训练，无需依赖自然图像的大规模预训练。

Result: 在多个基准测试中，UCell的性能与比其大10-20倍的模型相当，且对未见过的域外数据具有相似的泛化能力。通过一系列单样本和少样本微调实验，验证了模型在多样化小数据集上的适应能力。

Conclusion: UCell展示了在生物医学领域构建参数高效小型模型的可行性，能够在不依赖大规模预训练的情况下实现与大模型相当的性能，从而将模型构建与外部商业利益解耦，为数据受限场景提供了实用解决方案。
...
TL;DR: QAsk-Nav是首个可复现的协作实例对象导航基准，将导航与协作提问能力分开评估，解决了现有基准只关注导航成功率的问题。


<details>
  <summary>Details</summary>
Motivation: 现有协作实例对象导航基准主要关注导航成功率，缺乏对协作交互能力的一致评估方法，无法明确评估智能体与人类通过自然语言对话解决视觉模糊性的能力。

Method: 提出QAsk-Nav基准，包含：1）轻量级提问协议（独立于导航评分）；2）增强的导航协议（具有真实、多样、高质量的目标描述）；3）开源数据集（包含28,000条质量检查的推理和提问轨迹）。

Result: 基于QAsk-Nav基准开发了Light-CoNav模型，比现有模块化方法小3倍、快70倍，在未见过的物体和环境上超越了最先进的协作导航方法。

Conclusion: QAsk-Nav为协作实例对象导航提供了首个可复现的基准，能够独立评估导航和协作提问能力，推动了该领域的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究通常基于预处理好的社交线索进行操作，而现实场景中AI助手需要从原始数据中感知和推理。现有方法和多模态大语言模型在Omni-MMSI任务上表现不佳，主要因为缺乏可靠的身份归因能力，导致社交互动理解不准确。

Method: 提出Omni-MMSI-R，一个参考引导的流程，使用工具生成身份归因的社交线索，并进行链式思维社交推理。为此构建了参与者级别的参考对，并在现有数据集基础上整理了推理标注。

Result: 实验表明，Omni-MMSI-R在Omni-MMSI任务上优于先进的大语言模型和其他对比方法。

Conclusion: Omni-MMSI任务对于开发能够感知和响应人类互动的AI助手至关重要，而提出的Omni-MMSI-R流程通过解决身份归因问题，显著提升了社交互动理解能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有无监督分割方法在复杂多组件形态场景中表现不佳，因为它们依赖粗粒度补丁级表示，抑制了细粒度结构细节。

Method: 提出EASe框架，包含SAUCE（语义感知上采样与通道激励）用于选择性校准低分辨率特征，以及CAFE（线索注意力特征聚合器）利用注意力分数作为语义分组信号进行训练免费的多粒度掩码分割。

Result: EASe在主要标准基准测试和具有复杂形态的多样化数据集上表现出优于先前最先进方法的性能。

Conclusion: EASe通过直接在像素级特征表示上操作，实现了准确的细粒度密集语义掩码发现，解决了复杂形态场景中的分割挑战。
...
TL;DR: 本文提出TPC-CMA方法，通过三阶段课程学习减少视觉语言模型中的模态间隙，将模态间隙分解为质心间隙和分布间隙，并证明分布间隙才是跨模态任务性能的真正预测指标。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（如CLIP）虽然学习了图像和文本的共享嵌入空间，但它们的表示在几何上仍然分离，存在模态间隙。这种间隙限制了需要跨模态互换性的任务，如字幕生成和联合聚类。现有的后处理方法只能部分改善跨模态兼容性，主要减少全局质心偏移而未能解决底层分布不匹配问题。

Method: 提出TPC-CMA（三阶段跨模态对齐课程学习）微调框架：1）将模态间隙分解为质心间隙和分布间隙；2）通过联合缓解质心偏移和重塑分布结构来减少两个组件；3）采用三阶段课程学习，通过梯度感知调度逐步引入对齐，实现稳定优化。

Result: 实验表明该方法显著改善跨模态对齐：当α_target=0.05时，模态间隙减少66.6%，仅导致4.84%的准确率下降；在更强对齐下（α_target=0.5），间隙减少82.3%，聚类ARI从0.318提升到0.516，字幕生成CIDEr分数提高57.1%。

Conclusion: 本文提出的TPC-CMA框架有效解决了视觉语言模型中的模态间隙问题，通过明确减少质心间隙和分布间隙，显著提升了跨模态任务的性能，为视觉语言模型的跨模态对齐提供了新的解决方案。
...
TL;DR: SANA-I2I是一个无文本的高分辨率图像到图像生成框架，通过移除文本条件，仅使用配对源-目标图像在潜在空间学习条件流匹配模型，专门应用于胎儿MRI运动伪影减少任务。


<details>
  <summary>Details</summary>
Motivation: 现有方法如SanaControlNet结合文本和图像控制，但在医学成像等特定领域，文本描述可能不必要或难以获取。作者希望开发一个完全依赖图像对的无文本生成框架，专门针对医学图像处理任务。

Method: 1. 完全移除文本条件，仅使用配对源-目标图像训练
2. 在潜在空间学习条件流匹配模型，学习将目标图像分布映射到另一个分布的条件速度场
3. 针对胎儿MRI运动伪影减少任务，采用合成数据生成策略（基于Duffy等人的方法）模拟真实运动伪影，以解决真实配对数据难以获取的问题

Result: 实验结果表明，SANA-I2I能有效抑制运动伪影同时保持解剖结构，在少量推理步骤下达到竞争性性能。这证明了基于流的无文本生成模型在医学成像监督图像到图像任务中的效率和适用性。

Conclusion: SANA-I2I展示了无文本条件流匹配模型在医学图像处理任务中的有效性，特别是在配对数据稀缺的情况下，通过合成数据策略能够实现高质量的图像转换，为医学成像应用提供了高效实用的解决方案。
...
TL;DR: 使用冻结的DINOv3 ViT-B嵌入，通过基于最近邻的自训练方法，仅需少量标注数据就能在海洋物种识别任务上接近全监督模型的性能。


<details>
  <summary>Details</summary>
Motivation: 水下图像物种分类受限于专家标注成本高，且监督模型在不同数据集间迁移性差。研究旨在探索半监督方法能否在最小标注成本下解决这一问题。

Method: 采用冻结的DINOv3 ViT-B嵌入（无需微调），通过基于最近邻的自训练方法，将少量标注种子传播到未标注数据中。在AQUA20基准（20种海洋物种）上评估，仅使用不到5%的训练标签。

Result: 使用少量标签时，该方法能显著缩小与全监督ConvNeXt基线的差距；在全监督条件下，差距缩小到几个百分点，多个物种甚至超过监督基线。嵌入空间的类别可分性（ROC-AUC）即使在极低标签条件下也很高。

Conclusion: 该方法无需训练、无需领域特定数据工程、无需水下适应模型，为标签高效的海洋物种识别建立了一个实用且可立即部署的基线。
...
<details>
  <summary>Details</summary>
Motivation: 激光雷达对自动驾驶感知至关重要，但易受干扰攻击影响，导致点云数据随机化。研究发现虽然点云被破坏，但底层全波形数据仍保留攻击信号与真实信号的可区分特征。

Method: 提出PULSAR-Net，采用带有轴向空间注意力机制的新型U-Net架构，专门设计用于在全波形表示中识别攻击信号与真实物体回波。为解决现有数据集中缺乏干扰攻击下的全波形数据问题，开发了物理感知的数据集生成流程来合成逼真的干扰攻击全波形表示。

Result: 尽管完全在合成数据上训练，PULSAR-Net在真实世界静态和驾驶场景中，对受干扰攻击遮挡的车辆分别达到92%和73%的重建率。

Conclusion: 通过利用激光雷达系统中先前未充分利用的中间全波形表示和同步激光传感，PULSAR-Net能够有效对抗干扰攻击，重建真实点云，为自动驾驶感知系统提供更强的安全性。
...
TL;DR: 提出动态图模型，通过自适应节点选择机制从RGB-D数据中提取关键局部特征，用于室内场景识别，在公开数据集上优于现有方法。
...
Method: 提出动态图模型，包含自适应节点选择机制，从RGB和深度两种模态中提取关键局部特征进行图建模。节点按三个不同层次分组，表示物体间的远近关系。图模型根据注意力权重动态更新，最终融合RGB和深度模态的优化特征进行场景识别。

Result: 在SUN RGB-D和NYU Depth v2等公开数据集上进行实验，结果显示该方法相比现有最先进方法具有优越性能，能够有效利用RGB和深度两种模态的关键局部特征。

Conclusion: 提出的动态图模型通过自适应节点选择机制，能够有效利用RGB-D数据中的关键局部特征，在室内场景识别任务中取得了优于现有方法的性能。
...
TL;DR: mmAnomaly：结合毫米波雷达与RGBD视觉的多模态异常检测框架，通过条件潜在扩散模型生成预期毫米波频谱，实现遮挡场景下的异常检测与定位
...
Conclusion: mmAnomaly建立了毫米波感知中上下文感知异常检测的准确且可解释框架，通过多模态融合有效解决了毫米波信号复杂失真问题
...
TL;DR: TEP方法通过跟踪增强提示改进SAM3在复杂视频目标分割中对微小和语义主导目标的性能，在PVUW 2026挑战赛测试集上获得第一名（56.91%）


<details>
  <summary>Details</summary>
Motivation: 当前最先进的SAM3在复杂视频目标分割任务中对传统目标表现出色，但在微小目标和语义主导目标上表现不佳，主要原因是SAM3对这些特定目标类型的理解不足

Method: 提出TEP：通过跟踪增强提示推进复杂视频目标分割。这是一种无需训练的方法，利用外部跟踪模型和多模态大语言模型引入跟踪增强提示，帮助SAM3更好地理解具有挑战性的目标
...
<details>
  <summary>Details</summary>
Motivation: 心电图数字化可以解锁数十亿存档临床记录，但现有方法在真实世界图像上表现不佳，尽管基准测试数据看起来不错。需要一种无需修改底层数字化器的质量保证方案。

Method: VLM-in-the-Loop插件模块，通过标准化接口包装任何数字化后端，采用工具锚定机制：将VLM评估锚定在特定领域信号分析工具的定量证据上，形成闭环反馈。

Result: 工具锚定将判决一致性从71%提升到89%，保真度分离度翻倍（ΔPCC 0.03→0.08）。在四个不同后端上均改善性能：29.4%边界导联改善，41.2%失败肢体导联恢复，有效导联数翻倍（2.5→5.8）。在428个真实临床HCM图像上达到98.0%优秀质量。

Conclusion: VLM-in-the-Loop通过工具锚定机制显著提升心电图数字化质量，其插件架构和工具锚定机制具有领域参数化特性，可推广到其他质量标准可客观测量的领域。
...
TL;DR: 研究探索了轨迹预测模型QCNet从美国数据迁移到韩国道路环境的适应性，比较了四种训练策略，发现选择性微调解码器同时冻结编码器在准确性和训练效率方面表现最佳。
...
Method: 使用韩国自动驾驶数据集，比较了四种训练策略：零样本迁移、从头训练、完全微调和编码器冻结。重点研究了Query-Centric Trajectory Prediction (QCNet)模型从美国数据到韩国道路环境的迁移能力。

Result: 实验结果表明，利用预训练知识显著提高了预测性能。具体而言，选择性微调解码器同时冻结编码器在准确性和训练效率之间取得了最佳平衡，与从头训练相比，预测误差减少了66%以上。
...
TL;DR: 本文提出了一个无需训练的三阶段管道，结合多模态大语言模型和SAM3，在PVUW 2026 MeViS-Text挑战赛中取得第一名成绩。


<details>
  <summary>Details</summary>
Motivation: 解决基于运动中心语言表达的视频对象分割问题，需要模型同时理解外观、时间行为和对象交互，这是一个具有挑战性的多模态任务。

Method: 采用完全无需训练的三阶段方法：1) Gemini-3.1 Pro分解目标事件并生成判别性描述；2) SAM3-agent生成种子掩码并由SAM3跟踪器传播；3) Qwen3.5-Plus和行为级验证进行细化修正。

Result: 在PVUW 2026 MeViS-Text测试集上排名第一，Final分数达到0.909064，J&F分数为0.7897，无需任务特定微调。

Conclusion: 通过结合强大的多模态大语言模型和SAM3，构建了一个完全无需训练的管道，在运动中心语言表达的视频对象分割任务中取得了优异表现。
...
TL;DR: FADE是一种针对基于查询传播的多目标跟踪方法的对抗攻击框架，通过时间查询泛滥和时间记忆破坏两种策略，利用TBP方法的架构漏洞，显著降低跟踪性能。


<details>
  <summary>Details</summary>
Motivation: 基于查询传播的跟踪方法虽然实现了端到端的长时序建模，但其依赖查询传播的架构存在未探索的对抗攻击漏洞。研究者旨在揭示和利用这些特定漏洞，开发针对性的攻击框架。

Method: FADE采用两种攻击策略：1) 时间查询泛滥：生成时间一致的虚假跟踪查询，耗尽跟踪器的有限查询预算，迫使终止有效轨迹；2) 时间记忆破坏：直接攻击查询更新器的记忆，通过状态去相关切断时间链接，并擦除匹配轨迹的学习特征身份。同时开发了可微分管道，通过模拟先进感知传感器欺骗来优化攻击的物理世界可实现性。

Result: 在MOT17和MOT20基准测试上的实验表明，FADE对最先进的TBP跟踪器非常有效，导致显著的身份切换和轨迹终止。

Conclusion: FADE成功揭示了基于查询传播的跟踪方法在对抗攻击下的架构脆弱性，为未来开发更鲁棒的跟踪系统提供了重要见解，并强调了在物理世界中实现此类攻击的可能性。
...
TL;DR: 本文提出FLB方法缓解大视觉语言模型中的物体幻觉问题，通过存储并重用首个生成token的logit来维持视觉信息，避免长期衰减


<details>
  <summary>Details</summary>
Motivation: 大视觉语言模型在多模态任务中表现出色，但物体幻觉问题依然存在。现有方法如重训练和外部接地方法成本高或结构复杂，而无需训练的方法如对比解码存在长期衰减问题，即随着生成过程进行，视觉接地减弱而语言先验主导。

Method: 提出First Logit Boosting (FLB)方法，这是一种简单有效的无需训练技术。FLB存储第一个生成token的logit，并将其添加到后续token的预测中，从而有效缓解视觉信息的长期衰减。

Result: 实验结果表明，FLB在各种任务、基准测试和骨干模型中显著减少了物体幻觉。该方法能(1)在整个生成过程中维持嵌入在第一个token中的视觉信息，(2)通过"The" token的稳定效应抑制幻觉词。推理开销可忽略不计，适用于实时多模态系统。

Conclusion: FLB是一种简单高效的无需训练方法，能有效缓解大视觉语言模型中的物体幻觉问题，通过维持首个token的视觉信息来对抗长期衰减，具有实际应用价值。
...
TL;DR: 7T MRI能更好显示多发性硬化白质病变，但传统自动分割工具在7T数据上效果不佳。研究训练了基于transformer的模型在7T数据上，相比传统方法显著提升了分割性能，特别是对小病变的检测。


<details>
  <summary>Details</summary>
Motivation: 7T MRI能提供更好的多发性硬化白质病变可视化，但由于对比度和伪影与1.5-3T MRI不同，现有的自动分割工具可能无法直接应用于7T数据，需要专门针对7T MRI开发分割方法。

Method: 使用7T FLAIR扫描数据，基于LST工具输出并经过专家手动修订生成参考病变掩码。训练了3D UNETR和SegFormer两种transformer模型，在多个分辨率(0.5x0.5x0.5³, 1.0x1.0x1.0³, 1.5x1.5x2.0³)上进行训练。使用BraTS 2023框架的体素级和病变级指标进行评估，并与传统方法LST-LPA和LST-AI进行比较。

Result: 在原生0.5x0.5x0.5³分辨率的测试集上，7T训练的transformer模型与LST-AI性能相当，同时能检测到传统方法遗漏的小病变。最佳模型(SegFormer)在体素级Dice系数达到0.61，病变级Dice系数0.20，显著优于传统LST-LPA工具(Dice 0.39，病变级Dice 0.02)。在降采样图像上训练的模型性能下降，说明原生7T分辨率对小病变检测很重要。

Conclusion: 针对7T MRI专门训练的transformer模型能显著提升多发性硬化白质病变的自动分割性能，特别是对小病变的检测。研究提供了开源模型，为超高场强MS研究的自动化病变量化提供了可重复、即用型资源。
...
TL;DR: 本文提出MUPO方法解决GRPO强化学习中存在的多样性崩溃问题，通过激励多组策略优化来促进发散性思维，提升视觉语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 尽管强化学习（特别是GRPO）能够提升视觉语言模型的推理能力，但其有效性的底层机制和局限性尚未充分探索。研究发现RL模型倾向于深度但狭窄的推理，而基础模型虽然推理不够精细，但展现出更广泛和多样化的思维模式。GRPO容易导致多样性崩溃，使模型过早收敛到有限的推理策略子集，丢弃大多数潜在替代方案，导致局部最优和可扩展性差。

Method: 提出Multi-Group Policy Optimization (MUPO)，一种简单而有效的方法，旨在激励跨多个解决方案的发散性思维。该方法通过多组策略优化来保持推理策略的多样性，避免过早收敛到局部最优。

Result: 在已建立的基准测试中证明了MUPO方法的有效性，表明该方法能够解决GRPO的多样性崩溃问题，提升模型的推理能力和可扩展性。

Conclusion: MUPO通过激励发散性思维和多组策略优化，有效解决了强化学习在视觉语言模型推理训练中存在的多样性崩溃问题，为提升模型推理能力提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 胸部X光检查量增加导致放射科医生工作量增加和诊断错误风险上升，现有AI系统大多只生成最终预测，缺乏将视觉证据转化为放射学发现和诊断预测的明确推理过程。

Method: 使用从30个公共数据集的36个CXR解读任务中整理的1470万条指令和推理样本进行训练，采用两阶段框架结合指令微调和强化学习来提升推理质量。

Result: 在17个评估设置中，CheXOne在零样本设置下优于现有医学和通用领域基础模型，在独立公共基准测试中表现强劲。临床阅读研究显示，CheXOne生成的报告在55%的情况下与住院医师报告相当或更好。

Conclusion: 明确的推理可以提高AI辅助CXR解读中的模型性能、可解释性和临床实用性，生成的推理轨迹具有高临床事实性并为最终预测提供因果支持。
...
TL;DR: ARGS框架通过自回归方式生成多尺度3D高斯表示，使用高斯简化策略和树状结构实现可控细节层次，仅需O(log n)步骤


<details>
  <summary>Details</summary>
Motivation: 自回归框架在2D图像生成中表现出色，但在3D对象生成领域尚未充分探索。现有方法在生成多尺度3D表示方面存在挑战，需要一种能够控制细节层次、保持视觉保真度且时间可控的生成方法。

Method: 提出自回归高斯泼溅（ARGS）框架，采用高斯简化策略并反转简化过程来指导下一尺度生成。利用层次树结构，生成过程仅需O(log n)步骤。提出基于树的transformer自回归预测树结构，使叶节点能够关注其内部祖先以增强结构一致性。

Result: 实验表明，ARGS能够有效生成具有可控细节层次的多尺度高斯表示，在视觉保真度和时间消耗预算方面表现良好，实现了高效的3D对象生成。

Conclusion: ARGS框架成功将自回归范式扩展到3D对象生成，通过树状结构和简化策略实现了高效、可控的多尺度3D表示生成，为3D内容创作提供了新方法。
...
TL;DR: PC-SAM是一个统一框架，结合了全自动道路分割和交互式分割，通过精心设计的微调策略，使点提示的影响仅限于对应补丁，解决了原始SAM在遥感道路分割中无法进行精细局部修正的问题。


<details>
  <summary>Details</summary>
Motivation: 当前全自动道路分割方法在识别具有挑战性的道路段时仍存在不足，经常产生假阳性和假阴性区域，且不支持感兴趣区域的局部分割或现有掩码的细化。虽然SAM模型在自然图像的交互式分割中表现良好，但在遥感道路分割中性能较差，无法支持细粒度的局部细化。

Method: 提出PC-SAM框架，将全自动道路分割和交互式分割集成到统一框架中。通过精心设计微调策略，约束点提示对其对应补丁的影响，克服原始SAM无法进行精细局部修正的局限性，实现细粒度的交互式掩码细化。

Result: 在多个代表性遥感道路分割数据集上的大量实验表明，结合点提示时，PC-SAM在道路掩码分割方面显著优于最先进的全自动模型，同时提供灵活的局部掩码细化和局部道路分割。

Conclusion: PC-SAM通过统一全自动和交互式分割框架，解决了遥感道路分割中的现有局限性，实现了更好的分割性能和灵活的局部细化能力，为下游任务提供了更有效的支持。
...
<details>
  <summary>Details</summary>
Motivation: 开放集目标检测（OSOD）面临两个主要挑战：1）文本表示与复杂视觉概念对齐困难；2）稀有类别的图像-文本对稀缺。这导致在专业领域或复杂对象上性能不佳。现有的视觉提示方法虽然部分解决了这些问题，但通常涉及复杂的多模态设计和多阶段优化，延长了开发周期。

Method: 1. 提出PET-DINO通用检测器，支持文本和视觉提示；2. 设计对齐友好的视觉提示生成（AFVPG）模块，基于先进的文本提示检测器构建；3. 引入两种提示增强训练策略：迭代级的批内并行提示（IBP）和整体训练级的动态记忆驱动提示（DMD），实现多提示路径的并行建模。

Result: 综合实验表明，PET-DINO在各种基于提示的检测协议中展现出竞争力的零样本目标检测能力。其优势归因于继承式哲学和提示增强训练策略，这些策略在构建有效的通用目标检测器中发挥了关键作用。

Conclusion: PET-DINO通过创新的视觉提示生成模块和训练策略，有效解决了开放集目标检测中的对齐问题和训练效率问题，为构建通用目标检测器提供了有效方案。
...
Method: 提出Relational Grounding Transformer (RegFormer)，利用空间定位信号作为推理过程的引导，促进局部感知的交互学习。通过学习局部化交互线索，模块能够区分人类、物体及其交互，实现从图像级交互推理到精确高效的实例级推理的直接迁移。

Result: 实验表明RegFormer能有效学习实例级交互推理的空间线索，运行效率高，甚至达到与全监督模型相当的性能。

Conclusion: RegFormer通过空间定位引导的Transformer架构，解决了弱监督HOI检测中的计算效率和假阳性问题，实现了从图像级到实例级推理的有效迁移。
...
TL;DR: MAESIL是一种用于3D医学图像的自监督学习框架，通过"超级块"和双掩码策略有效捕获3D结构信息，在CT数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 3D医学影像（如CT）的深度学习训练面临标注数据稀缺的挑战。现有方法通常将3D扫描作为独立2D切片处理，丢弃了关键的轴向一致性和3D结构上下文信息。

Method: 提出MAESIL框架，核心创新是"超级块"作为3D块状输入单元，平衡3D上下文保留与计算效率。采用3D掩码自编码器策略和双掩码策略学习全面的空间表示。

Result: 在三个不同的大规模公共CT数据集上验证，MAESIL在PSNR和SSIM等关键重建指标上显著优于AE、VAE和VQ-VAE等现有方法。

Conclusion: MAESIL为3D医学成像任务提供了一个强大实用的预训练解决方案，能够有效捕获3D结构信息。
...
TL;DR: 提出基于可学习性指导的数据集蒸馏方法，通过分阶段构建合成数据集，减少样本冗余，在多个数据集上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的数据集蒸馏方法存在样本冗余问题，不同子集包含80-90%重叠的训练信号。这是因为现有方法只关注视觉多样性或平均训练动态，而没有考虑样本间的相似性，导致多个样本共享相似信息而非互补知识。

Method: 提出可学习性驱动的数据集蒸馏方法，通过连续阶段增量构建合成数据集。从一个小集合开始，训练模型并根据可学习性分数生成新样本，这些分数识别当前模型能从什么中学习，创建自适应课程。引入可学习性指导扩散（LGD），平衡当前模型的训练效用和参考模型下的有效性，生成与课程对齐的样本。

Result: 方法将冗余减少39.1%，促进训练阶段的专业化，在ImageNet-1K（60.1%）、ImageNette（87.2%）和ImageWoof（72.9%）上达到最先进的结果。

Conclusion: 可学习性指导的数据集蒸馏通过减少冗余和创建自适应课程，显著提升了数据集蒸馏的效果，在多个基准测试中达到SOTA性能。
...
TL;DR: AceTone是首个支持多模态条件色彩分级的统一框架，通过生成式色彩变换直接生成3D-LUT，在文本引导和参考图像引导的色彩分级任务上达到SOTA性能。
...
Method: 将色彩分级建模为生成式色彩变换任务，使用VQ-VAE将3D-LUT压缩为离散token，构建大规模数据集AceTone-800K，训练视觉语言模型预测LUT token，并通过强化学习对齐感知保真度和美学。

Result: AceTone在文本引导和参考图像引导的色彩分级任务上达到最先进性能，LPIPS指标提升高达50%，人类评估确认其结果视觉愉悦且风格一致。

Conclusion: AceTone展示了语言驱动、美学对齐的色彩分级新途径，为多模态条件色彩分级提供了统一框架。
...
TL;DR: FreqPhys是一个基于频率引导的远程光电容积描记(rPPG)框架，通过显式利用生理频率先验来增强信号恢复的鲁棒性，特别在运动伪影和光照波动等挑战性条件下表现优异。
...
Method: FreqPhys包含四个核心模块：1)生理带通滤波模块抑制带外干扰；2)生理频谱调制和自适应频谱选择强调脉搏相关频率成分；3)跨域表示学习模块融合频谱先验和深度时域特征；4)频率感知条件扩散过程逐步重建高质量rPPG信号。

Result: 在六个基准测试上的广泛实验表明，FreqPhys相比最先进方法有显著改进，特别是在具有挑战性的运动条件下表现突出。

Conclusion: 该研究强调了显式建模生理频率先验的重要性，提出的频率引导框架能有效提升rPPG信号恢复的鲁棒性和准确性。
...
Method: TRiGS采用统一的连续几何变换，整合了SE(3)变换、分层贝塞尔残差和可学习的局部锚点，为单个基元建模几何一致的刚性运动。这种连续表示保留了时间一致性，有效缓解了无界内存增长。

Result: 在标准基准测试中实现了高保真渲染，同时能够扩展到长视频序列（如600到1200帧）而没有严重的内存瓶颈，在时间稳定性方面显著优于先前工作。
...
TL;DR: Reliev3R提出了一种弱监督训练范式，无需昂贵的多视角几何标注，仅使用单目相对深度和图像稀疏对应关系来训练前馈重建模型。


<details>
  <summary>Details</summary>
Motivation: 前馈重建模型在重建质量和多任务适应性方面表现出色，但过度依赖多视角几何标注（如3D点云和相机位姿），使得全监督训练方案难以扩展。需要一种低成本的训练方法。

Method: 设计了一种模糊感知的相对深度损失和基于三角测量的重投影损失，利用预训练模型的零样本预测提供的单目相对深度和图像稀疏对应关系，为多视角几何一致性提供监督。

Result: 使用更少数据从头训练，Reliev3R能够达到与全监督兄弟模型相当的性能，实现了低成本3D重建监督和可扩展的前馈重建模型。

Conclusion: Reliev3R通过弱监督范式减少了对几何传感器数据和计算密集型结构从运动预处理的依赖，为低成本3D重建监督和可扩展的前馈重建模型迈出了重要一步。
...
TL;DR: TF-SSD是一种基于视觉基础模型的训练无关协同显著目标检测方法，通过结合SAM和DINO实现跨图像的显著对象分割，无需训练即可获得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有协同显著目标检测方法主要基于训练方法，受限于封闭数据集且泛化能力有限。本文探索利用具有强大泛化能力和显著理解能力的视觉基础模型来解决这一问题。

Method: 提出训练无关方法TF-SSD：1）使用SAM生成原始候选掩码池；2）基于SAM的质量掩码生成器过滤冗余掩码；3）利用DINO注意力图的图像内显著过滤器识别显著掩码；4）提出跨图像原型选择器，通过计算原型相似度选择最终预测掩码。

Result: 实验表明TF-SSD优于现有方法，比最近的训练无关方法提升了13.7%的性能，代码已开源。

Conclusion: TF-SSD成功展示了视觉基础模型在协同显著目标检测中的潜力，提供了一种无需训练的高性能解决方案，具有良好的泛化能力。
...
TL;DR: STAR是一个基于拓扑锚点的两阶段空间导航框架，通过监督微调和空间感知分段直接偏好优化，在复杂拓扑结构中实现更准确的空间推理，其32B版本在RedMaze-23K数据集上超越了DeepSeek-V3并达到GPT-4性能的82.4%。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型空间导航方法（如VoT）在复杂拓扑结构中容易产生级联错误，需要一种更可靠的方法来提升结构化空间导航的性能。

Method: 提出STAR两阶段框架：第一阶段使用监督微调帮助模型内化空间语义并剪枝冗余路径；第二阶段采用空间感知分段直接偏好优化（SDPO）来精炼长视野导航中的自我校正能力。

Result: STAR在开源模型中达到最先进性能：其32B变体在RedMaze-23K数据集上超越DeepSeek-V3（29.27% vs. 25.00%），并达到GPT-4性能的82.4%。

Conclusion: STAR框架通过拓扑锚点和两阶段优化策略，有效解决了复杂拓扑结构中的空间导航问题，为LLMs的空间推理能力提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 高致病性禽流感和地方性家禽疾病的早期检测对全球粮食安全至关重要。计算机视觉模型虽然擅长从粪便图像分类疾病，但规模化部署受到农场数据隐私问题和机构数据孤岛的阻碍。此外，现有开源农业数据集存在严重的、未记录的数据污染问题。

Method: 1. 创建并发布poultry-fecal-fl数据集，包含8,770张独特图像，涵盖四种疾病类别，消除了公共仓库中46.89%的重复数据；2. 开发FecalFed隐私保护联邦学习框架；3. 在高度异构、非独立同分布条件下评估（Dirichlet α=0.5）；4. 使用服务器端自适应优化（FedAdam）配合Swin-Small架构；5. 同时评估边缘优化的Swin-Tiny模型。

Result: 1. 单一农场孤立训练在数据异构下崩溃，仅获得64.86%准确率；2. 联邦学习方法在不集中敏感数据的情况下恢复性能：Swin-Small架构达到90.31%准确率，接近集中式上限95.10%；3. 边缘优化的Swin-Tiny模型保持89.74%的竞争性性能。
...
TL;DR: HarassGuard是一个基于视觉语言模型的系统，仅使用视觉输入检测社交VR中的物理骚扰，在保护隐私的同时达到与现有方法相当的检测性能。


<details>
  <summary>Details</summary>
Motivation: 社交VR平台虽然提供沉浸式社交体验，但也让用户面临严重的在线骚扰风险。现有的安全措施大多是反应性的，而主动检测骚扰的解决方案通常依赖敏感的生理数据，引发隐私担忧。

Method: 构建IRB批准的骚扰视觉数据集，应用提示工程，并微调视觉语言模型（VLM）来检测社交VR中的骚扰行为，同时考虑社交VR中的上下文信息。

Result: HarassGuard在二分类任务中达到88.09%的准确率，在多分类任务中达到68.85%的准确率，与最先进的基线方法（LSTM/CNN、Transformer）性能相当，但使用的微调样本显著减少（200 vs. 1,115）。

Conclusion: HarassGuard在上下文推理和隐私保护检测方面具有独特优势，为社交VR中的骚扰检测提供了一种有效的隐私保护解决方案。
...
TL;DR: 提出一种无需数据增强的方法，利用3D基础模型的几何先验来提高自动驾驶轨迹规划在相机视角变化下的鲁棒性


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶轨迹规划模型严重依赖训练时见过的相机视角，在视角变化时性能下降明显，需要提高对相机视角变化的鲁棒性

Method: 使用3D基础模型获取深度估计，将像素级3D位置作为位置嵌入注入模型，并通过交叉注意力融合中间几何特征

Result: 在VR-Drive相机视角扰动基准测试中，大多数扰动条件下性能下降减少，在俯仰角和高度扰动下改进明显，但在纵向平移扰动下增益较小

Conclusion: 该方法能提高轨迹规划对相机视角变化的鲁棒性，但对纵向平移扰动的改进有限，需要更视角无关的集成方法来实现全面鲁棒性
...
Result: KG-CMI在三个Med-VQA数据集（VQA-RAD、SLAKE和OVQA）上均优于现有最先进方法。通过可解释性实验进一步验证了框架的有效性。

Conclusion: KG-CMI框架通过有效整合医学知识和支持自由形式答案，显著提升了医学视觉问答的性能，为解决Med-VQA中的知识利用不足和答案多样性问题提供了有效方案。
...
TL;DR: 研究发现对抗攻击下物体检测器的防御工具存在假设缺陷：检测数量与精度同步下降的假设不成立。在EMS-YOLO模型上观察到"质量腐蚀"现象——检测数量保持70%以上而mAP从0.528暴跌至0.042。


<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击监控和防御工具都假设检测精度下降时检测数量会同步下降，但这种耦合关系从未被实际测量验证。研究者希望检验这一基础假设的正确性。

Method: 在四个SNN架构和两种威胁模型（l-∞和l-2）下进行对抗攻击测试，特别关注EMS-YOLO模型在标准PGD攻击下的表现。评估了五种标准防御组件对质量腐蚀现象的检测和缓解能力。

Result: 在EMS-YOLO模型上发现了"质量腐蚀"现象：检测数量保持70%以上时，mAP从0.528暴跌至0.042。这种现象仅在四个检测器中的一个出现，且所有五种标准防御组件都无法检测或缓解该问题。

Conclusion: 对抗攻击的失效模式具有底物依赖性，现有防御生态系统可能基于单一底物校准的共享假设。这首次证明了对抗攻击失效模式的底物依赖性，对物体检测器的防御设计提出了重要挑战。
...
TL;DR: TALENT框架通过目标感知高效调优解决基于参数高效调优的参考图像分割中的非目标激活问题，使用校正成本聚合器和目标感知学习机制提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于参数高效调优的参考图像分割方法存在非目标激活问题，即视觉特征无法强调文本所指的目标实例，反而激活了同类别但不相关的对象。

Method: 提出TALENT框架：1) 校正成本聚合器高效聚合文本所指特征；2) 目标感知学习机制包括上下文成对一致性学习和目标中心对比学习，前者使用句子级文本特征构建文本所指亲和图，后者增强目标定位同时抑制无关关联。

Result: 在多个指标上优于现有方法，如在G-Ref验证集上获得2.5%的mIoU提升。

Conclusion: TALENT框架通过目标感知高效调优有效解决了非目标激活问题，显著提升了参考图像分割的性能。
...
TL;DR: 本文提出DirectFisheye-GS方法，将鱼眼相机模型集成到3DGS框架中，无需预处理即可直接使用鱼眼图像训练，并通过特征重叠驱动的跨视图联合优化策略解决边缘浮游伪影问题。


<details>
  <summary>Details</summary>
Motivation: 鱼眼相机具有更宽的视场，可以用更少的输入实现高质量重建，但现有3DGS方法需要先对鱼眼图像进行去畸变预处理，这会带来两个问题：1) 图像边缘的黑边导致信息损失，丧失了鱼眼大视场的优势；2) 去畸变的拉伸和插值重采样会稀释细节密度，导致3DGS过拟合低频区域，产生模糊和浮游伪影。

Method: 1) 将鱼眼相机模型集成到原始3DGS框架中，支持原生鱼眼图像输入训练；2) 提出特征重叠驱动的跨视图联合优化策略，建立跨视图一致的几何和光度约束，解决因畸变增加和随机选择视图优化导致的极端形状问题。

Result: DirectFisheye-GS在公共数据集上达到或超越了最先进方法的性能，有效解决了鱼眼图像边缘的浮游伪影问题。

Conclusion: 该方法实现了无需预处理的鱼眼图像直接训练，提出的跨视图联合优化策略不仅适用于鱼眼相机，同样可应用于现有的针孔相机管道，显著提升了3DGS在鱼眼相机场景下的重建质量。
...
TL;DR: 研究发现AI在皮肤镜图像诊断中的系统性误分类并非算法偏差，而是源于图像本身的内在复杂性。专家在AI误分类图像上的诊断表现也显著下降，表明这些图像具有固有的视觉模糊性。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注AI算法与人类专家的性能对比，但本研究从新视角出发，探究皮肤镜图像的内在复杂性，特别是AI系统性误分类图像是否反映了算法偏差还是图像本身的视觉模糊性。

Method: 使用多种CNN架构进行实验，识别出在所有模型中都系统性误分类的图像子集。然后让专家皮肤科医生独立评估这些困难案例和对照组图像，通过Cohen's kappa和Fleiss kappa等指标量化诊断表现和专家共识。

Result: 1. 人类专家在AI误分类图像上的诊断表现显著下降：与真实标签的一致性从对照组的kappa=0.61降至困难图像的kappa=0.08；2. 专家共识严重恶化：组间一致性从对照组的Fleiss kappa=0.456降至困难图像的0.275；3. 图像质量是导致这些系统性失败的主要因素。

Conclusion: AI在皮肤镜诊断中的系统性误分类反映了图像本身的内在复杂性而非算法偏差，这些困难图像对人类专家同样具有挑战性。研究强调了图像质量在皮肤镜诊断中的重要性，并公开了所有数据、代码和模型以促进透明度和可重复性。
...
TL;DR: CL-VISTA是一个专为视频大语言模型设计的持续学习基准，包含8个多样化任务，通过6种评估协议在3个维度上系统评估持续学习方法，揭示了性能、计算效率和内存占用之间的基本权衡。


<details>
  <summary>Details</summary>
Motivation: 现有持续学习基准存在两个主要问题：1）许多仍依赖未经大规模预训练的模型；2）通常将单个数据集划分为子任务，导致任务冗余度高，对预训练视频大语言模型的遗忘效应不明显。需要专门针对视频大语言模型的持续学习基准来有效评估其适应非平稳现实世界数据的能力。

Method: 提出了CL-VISTA基准，通过精心策划8个涵盖感知、理解和推理的多样化任务，产生显著的分布偏移以有效暴露灾难性遗忘。建立了包含6种不同协议的综合评估框架，覆盖3个关键维度：性能、计算效率和内存占用。其中性能维度包含通用视频理解评估，以检验持续学习方法是真正增强基础智能还是仅导致任务特定过拟合。

Result: 对10种主流持续学习方法进行广泛基准测试，揭示了一个基本权衡：没有任何单一方法能在所有维度上实现普遍优越性。成功缓解灾难性遗忘的方法往往以牺牲泛化能力或产生过高计算和内存开销为代价。

Conclusion: CL-VISTA为推进多模态基础模型的持续学习提供了关键见解，揭示了当前持续学习方法在性能、效率和内存占用之间的固有权衡，为未来研究指明了方向。
...
TL;DR: MoonAnything是一个基于真实月球地形的统一基准数据集，提供几何和光度监督，包含LunarGeo（立体图像和深度图）和LunarPhoto（光度真实图像）两个子数据集，共13万+样本，支持3D重建、姿态估计、反射率估计等任务。


<details>
  <summary>Details</summary>
Motivation: 月球探测任务需要准确的感知系统，但现有数据集缺乏几何和光度监督的完整性，通常缺少几何真值、光度真实性、光照多样性或大规模覆盖。
...
Conclusion: MoonAnything为月球感知任务提供了首个全面的几何和光度监督基准，适用于低纹理、高对比度条件下的算法测试，并可推广到其他无大气天体。数据集和工具已开源供社区使用。
...
<details>
  <summary>Details</summary>
Motivation: 现有统一分割方法通常依赖异构任务和模态的共享编码器，导致特征纠缠、梯度干扰和次优的病灶区分。需要一种更好的统一模型来处理多样化的医学病灶分割任务。
...
Conclusion: TP-Seg通过任务条件适配和原型引导的解码机制，有效解决了统一医学病灶分割中的特征纠缠问题，为AI辅助诊断提供了更高效、通用的解决方案。
...
TL;DR: TTA-Vid：一种基于测试时强化学习的视频推理方法，无需大规模监督数据和显式标签，通过测试时自适应和自适应帧选择实现跨数据集泛化。


<details>
  <summary>Details</summary>
Motivation: 现有视频推理模型依赖大规模监督数据和多阶段训练流程，成本高昂且难以适应新领域。需要一种无需显式标签、能在测试时自适应的方法来降低训练成本并提高适应性。

Method: 提出TTA-Vid方法，包含两个同时工作的组件：1）测试时自适应，在推理时对多个帧子集进行逐步推理，使用跨不同帧子集计算的批量感知频率奖励作为伪真值更新模型；2）多臂老虎机策略进行自适应帧选择，学习优先选择信息丰富的帧。
...
TL;DR: 提出基于注意力机制对偶形式的无训练token剪枝框架，通过选择最优的rank-1更新子集来近似原始权重矩阵，在保持性能的同时显著降低计算成本。
...
Method: 从注意力机制的对偶形式视角出发，将注意力重新表述为隐式线性层，其权重矩阵是rank-1外积的和。提出新的度量标准量化token的信息量和信息重复度，并引入渐进分块最大边际相关性算法高效选择最优子集。

Result: 大量实验表明，该方法在性能和效率之间实现了更好的权衡，同时为现有剪枝方法提供了新的理论视角。

Conclusion: 基于注意力对偶形式的无训练token剪枝框架有效解决了LVLMs计算成本问题，为token剪枝提供了理论基础，并在实际应用中取得了良好效果。
...
TL;DR: PrivHAR-Bench是一个多层级基准数据集，用于标准化视频动作识别中的隐私-效用权衡评估，包含9个不同隐私强度的变换层级和背景移除变体。


<details>
  <summary>Details</summary>
Motivation: 现有隐私保护人体活动识别研究通常采用二元评估范式（清晰视频vs单一隐私变换），限制了跨方法可比性，且无法揭示隐私强度与识别效用之间的细微关系。

Method: 创建PrivHAR-Bench数据集，包含15个活动类别、1,932个源视频，应用从轻量级空间模糊到加密块置换的渐进式视觉隐私变换，分为9个并行层级，并提供背景移除变体以分离人体运动特征与场景背景影响。

Result: 使用R3D-18模型验证显示，识别准确率随隐私强度增加呈可测量的下降曲线：层级内准确率从88.8%（清晰）降至53.5%（加密+背景移除），跨域准确率降至4.8%，证实了基准的有效性。

Conclusion: PrivHAR-Bench为隐私保护人体活动识别方法提供了标准化评估基准，能够量化隐私-效用权衡关系，数据集、生成管道和评估代码已公开可用。
...
TL;DR: 提出SurgSTU-Pipeline生成管道，创建包含7515个手术视频片段和15万个细粒度时空问答对的SurgSTU数据集，用于提升视觉语言模型在手术视频中的时空理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有手术视觉语言数据集缺乏对复杂交错时空动态的捕捉和评估能力，而大规模数据集创建面临手动标注成本高或大语言模型生成易出错的问题。

Method: 开发SurgSTU-Pipeline确定性生成管道，包含时空连续性过滤机制，能够可靠地创建用于细粒度时空多模态理解的手术数据集。将该管道应用于公开手术数据集，生成SurgSTU数据集。

Result: 创建了包含7515个视频片段和15万个细粒度时空问答样本的SurgSTU数据集。评估显示，最先进的通用视觉语言模型在零样本设置下表现不佳，但通过上下文学习可以改善其时空能力。在SurgSTU训练数据集上微调的视觉语言模型在所有时空任务中取得最高性能。

Conclusion: SurgSTU数据集有效提升了视觉语言模型在手术视频中的时空理解能力，验证了生成管道的有效性。代码将公开提供。
...
TL;DR: 该论文引入了一个新任务：给定同一场景的两个视图，识别违反3D运动一致性的物体，并提出了生成空间不一致图像对的方法来评估多模态大语言模型的空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 空间一致性是视觉世界的基本属性，也是理解物理现实模型的关键要求。尽管多模态大语言模型（MLLMs）取得了进展，但它们经常难以跨多个视图进行3D几何推理。研究者希望评估模型对物理世界的深层理解能力。

Method: 提出了一个简单且可扩展的方法，从多视图场景中生成真实的空间不一致图像对，用于系统评估模型识别3D运动一致性违反的能力。

Result: 结果显示，最先进的多模态大语言模型在识别空间不一致性方面显著低于人类观察者，且在不同场景属性上表现出很大变异性，表明它们对3D结构的理解是脆弱和不完整的。

Conclusion: 该研究强调了需要开发对物理世界有更深入基础理解的方法，多模态大语言模型在空间一致性推理方面仍有很大改进空间。
...
TL;DR: 本文重新审视了人机交互物体检索任务，通过主动学习循环和预训练ViT表征，在多物体复杂场景中实现基于用户反馈的物体类别检索。


<details>
  <summary>Details</summary>
Motivation: 在多物体数据集中，目标物体可能只占据图像的小区域且场景复杂，全局描述符往往不足。需要更适应的局部描述符来有效检索目标物体类别。

Method: 将检索任务构建为二分类问题，通过主动学习循环迭代优化：系统选择信息丰富的样本供用户标注，利用预训练ViT表征，探索不同的表征策略（全局上下文与局部细节的平衡）。

Result: 比较了多种表征策略在多物体数据集上的表现，揭示了全局上下文与细粒度局部物体细节之间的权衡关系。

Conclusion: 为基于主动学习的交互式物体类别检索管道设计提供了实用见解，展示了预训练ViT表征在人机交互物体检索任务中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 当前端到端自动驾驶从基于稀疏感知的传统范式发展为视觉-语言-动作(VLA)模型，但作者认为车辆在3D世界中运行，密集3D几何信息应作为决策的关键线索。然而现有几何重建方法依赖多帧输入的批量处理，无法应用于在线规划。

Method: 提出流式驾驶视觉几何变换器(DVGT-2)，采用在线处理方式，使用时间因果注意力和历史特征缓存支持实时推理。进一步提出滑动窗口流式策略，在特定间隔内使用历史缓存避免重复计算，能够同时输出密集几何重建和当前帧的轨迹规划。

Result: DVGT-2在多个数据集上实现了更优的几何重建性能，且速度更快。同一训练好的DVGT-2模型无需微调即可直接应用于不同相机配置的规划任务，包括闭环NAVSIM和开环nuScenes基准测试。

Conclusion: Vision-Geometry-Action (VGA)范式通过密集3D几何信息为自动驾驶提供了更全面的决策依据，DVGT-2模型实现了高效的在线几何重建和规划，展示了3D几何信息在自动驾驶中的核心价值。
...
Method: 构建CLeaRS基准测试，包含10个精选子集、超过20.7万张图像-文本对，涵盖多种解释任务、传感模态和应用场景，并定义三种评估协议。

Result: 广泛基准测试显示所有设置下都存在灾难性遗忘，现有持续学习方法在适应遥感视觉语言模型时效果有限，难以处理任务、指令和模态转换。

Conclusion: 需要为遥感视觉语言模型开发专门的持续学习方法，CLeaRS基准为这一研究方向提供了重要基础。
...
<details>
  <summary>Details</summary>
Motivation: 预训练语言模型适应为视觉语言模型时，由于表示偏移和跨模态干扰，会降低其原有的语言能力。这种损失难以恢复，现有方法通常需要引入额外的对齐层模块，增加了架构复杂性和推理参数。

Method: 提出LinguDistill方法：1）使用原始冻结语言模型作为教师；2）通过层级的KV缓存共享，让教师能够接触到学生的多模态表示；3）在语言密集型数据上选择性地蒸馏教师的强语言信号，恢复语言能力，同时保持学生在多模态任务上的视觉基础。

Result: 在语言和知识基准测试上恢复了约10%的性能损失，同时在视觉密集型任务上保持了可比性能。无需额外模块，为多模态模型中的模态特定退化问题提供了高效实用的解决方案。

Conclusion: 语言能力可以在不增加额外模块的情况下恢复，通过使用原始语言模型作为教师进行蒸馏，既能恢复语言能力，又能保持多模态性能，为多模态适应中的能力退化问题提供了简单有效的解决方案。
...
Method: 提出DisCo框架：1) 文本-视觉解耦模块：从参考图像和实体词提取主题身份，文本提示仅包含修改指令并使用通用代词；2) 设计专用奖励信号，通过强化学习无缝再耦合视觉定义的主题和文本生成的上下文。

Result: 实验表明该方法实现了最先进的性能，能够同时实现高保真主题保持和精确文本控制，生成高度逼真和连贯的图像。

Conclusion: DisCo框架通过解耦和再耦合视觉与文本信息，有效解决了相似性-可控性悖论，为同时保持主题身份和精确文本控制提供了有效解决方案。
...
Result: 在定量、定性和人工评估中一致优于现有基线方法，并提出了新的对象接地分数（OGS）联合评估空间对齐和语义一致性
...
TL;DR: SparkleMotion提出了一种结合骨骼关节和表面锚点的结构化表示方法，通过显式运动学-几何分解来平衡表达能力和鲁棒性，在噪声、遮挡和领域偏移下表现优异。


<details>
  <summary>Details</summary>
Motivation: 点云运动捕捉虽然具有丰富的空间几何信息和隐私保护优势，但从噪声、非结构化的点云中学习鲁棒表示仍然具有挑战性。现有方法在点基方法（几何细节丰富但噪声敏感）和骨架基方法（鲁棒但过于简化）之间存在权衡困境。

Method: 提出Sparkle表示方法，统一骨骼关节和表面锚点，通过显式运动学-几何分解构建结构化表示。SparkleMotion框架通过分层模块嵌入几何连续性和运动学约束来学习这种表示，显式解耦内部运动学结构和外部表面几何。

Result: SparkleMotion不仅在准确性上达到最先进水平，更重要的是在严重领域偏移、噪声和遮挡下的鲁棒性和泛化能力方面表现优异。在多种传感器类型和挑战性真实场景中的广泛实验证明了其优越性。

Conclusion: 通过显式解耦运动学结构和表面几何，Sparkle表示方法能够有效平衡表达能力和鲁棒性，为点云运动捕捉提供了一种有效的结构化表示方案，在复杂现实条件下具有出色的性能。
...
### [59] [Adversarial Attenuation Patch Attack for SAR Object Detection](https://arxiv.org/abs/2604.00887)
*Yiming Zhang,Weibo Qin,Feng Wang*

Main category: cs.CV

TL;DR: 提出了一种对抗性衰减补丁(AAP)方法，通过能量约束优化和衰减部署框架，在SAR目标检测系统中实现攻击效果与隐蔽性的平衡，并具有物理实现的潜力。


<details>
  <summary>Details</summary>
Motivation: 现有SAR对抗攻击方法虽然能有效欺骗检测器，但通常引入明显扰动且主要局限于数字域，忽略了攻击SAR系统的物理实现约束。

Method: 提出对抗性衰减补丁(AAP)方法，采用能量约束优化策略结合基于衰减的部署框架，实现攻击效果与隐蔽性的平衡，并与信号级电子干扰机制对齐以实现物理实现。
...
### [60] [IDDM: Identity-Decoupled Personalized Diffusion Models with a Tunable Privacy-Utility Trade-off](https://arxiv.org/abs/2604.00903)
*Linyan Dai,Xinwei Zhang,Haoyang Li,Qingqing Ye,Haibo Hu*

Main category: cs.CV

TL;DR: IDDM是一种针对个性化文本到图像扩散模型的防御方法，旨在减少生成图像的身份可链接性，同时保持高质量的个性化生成效果


<details>
  <summary>Details</summary>
Motivation: 现有防御方法主要针对未经授权的个性化模型训练，但无法解决授权个性化后生成的公开图像仍然泄露身份信息的问题。当用户在社交媒体分享个性化生成的图像时，这些图像仍可通过人脸识别系统链接到真实用户身份，导致身份追踪和画像分析

Method: 提出IDDM（Identity-Decoupled personalized Diffusion Models）模型端防御方法，将身份解耦集成到个性化流程中。采用交替优化策略：在短期个性化更新之间穿插身份解耦数据优化，使用两阶段调度来平衡身份可链接性抑制和生成效用

Result: 在多个数据集、多样化提示词和最先进的人脸识别系统上的广泛实验表明，IDDM能持续降低身份可链接性，同时保持高质量的个性化生成效果

Conclusion: IDDM为个性化扩散模型提供了一种有效的模型端输出免疫防御，支持授权个性化同时减少公开生成图像的身份泄露风险，通过可调控制实现隐私-效用的平衡，适应不同的隐私需求
...
### [61] [JAMMEval: A Refined Collection of Japanese Benchmarks for Reliable VLM Evaluation](https://arxiv.org/abs/2604.00909)
*Issa Sugiura,Koki Maeda,Shuhei Kurita,Yusuke Oda,Daisuke Kawahara,Naoaki Okazaki*

Main category: cs.CV

TL;DR: JAMMEval是一个经过系统精炼的日语视觉问答基准数据集，通过两轮人工标注改进了七个现有日语基准数据集的质量，为日语VLM评估提供更可靠的基准。


<details>
  <summary>Details</summary>
Motivation: 日语VQA基准相比英语基准缺乏迭代优化，存在模糊问题、错误答案和无需视觉基础即可解决的实例等问题，影响了评估的可靠性，导致模型比较得出误导性结论。

Method: 通过两轮人工标注系统性地精炼七个现有日语基准数据集，构建JAMMEval数据集，改进数据质量和评估可靠性。

Result: JAMMEval能够更好地反映模型能力、降低运行间方差、提高区分不同能力水平模型的能力，为日语VLM评估提供更可靠的基准。

Conclusion: JAMMEval解决了现有日语VQA基准的问题，提供了更可靠的评估工具，有助于推动日语视觉语言模型的发展。
...
### [62] [ProCap: Projection-Aware Captioning for Spatial Augmented Reality](https://arxiv.org/abs/2604.00912)
*Zimo Cao,Yuchen Deng,Haibin Ling,Bingyao Huang*

Main category: cs.CV

TL;DR: ProCap框架解决了空间增强现实中虚拟投影内容与物理场景的语义混淆问题，通过两阶段流程分离虚拟和物理层，并创建了首个大规模SAR语义基准数据集RGBP。


<details>
  <summary>Details</summary>
Motivation: 空间增强现实（SAR）使用投影仪将数字内容直接投射到物理场景上，但标准视觉语言模型难以区分物理场景和投影内容，导致虚拟-物理语义混淆，限制了SAR支持智能交互的能力。

Method: ProCap采用两阶段流程：1）通过自动分割在视觉上隔离虚拟和物理层；2）使用区域感知检索避免因投影变形导致的语义歧义。同时创建了RGBP数据集，包含65个不同物理场景和超过18万个投影的密集解耦标注。

Result: 建立了双描述评估协议，使用任务特定标记独立评估物理场景和投影描述。实验表明ProCap为未来SAR研究提供了稳健的语义基础。

Conclusion: ProCap框架成功解决了SAR中虚拟投影内容与物理场景的语义混淆问题，通过明确的解耦方法和大规模数据集，为空间增强现实的智能交互能力奠定了基础。
...
### [63] [Benchmarking and Mechanistic Analysis of Vision-Language Models for Cross-Depiction Assembly Instruction Alignment](https://arxiv.org/abs/2604.00913)
*Zhuchenyang Liu,Yao Zhang,Yu Xiao*

Main category: cs.CV

TL;DR: IKEA-Bench是一个评估视觉语言模型在组装任务中跨描绘对齐能力的基准，发现视觉编码是提升跨描绘鲁棒性的关键瓶颈


<details>
  <summary>Details</summary>
Motivation: 2D组装图通常抽象且难以理解，需要智能助手来监控进度、检测错误并提供逐步指导。在混合现实环境中，系统必须从相机画面识别已完成和进行中的步骤，并与图解说明对齐。视觉语言模型在此任务中显示出潜力，但面临描绘差距问题，因为组装图和视频帧共享的视觉特征很少。

Method: 构建IKEA-Bench基准，包含29个IKEA家具产品的1,623个问题，涵盖6种任务类型。评估19个视觉语言模型（2B-38B参数）在三种对齐策略下的表现。进行三级机制分析，包括图解和视频在ViT子空间中的分布分析。

Result: 关键发现：(1) 组装指令理解可通过文本恢复，但文本同时会降低图解到视频的对齐；(2) 架构家族比参数数量更能预测对齐准确性；(3) 视频理解仍然是硬瓶颈，不受策略影响。机制分析显示图解和视频占据不相交的ViT子空间，添加文本会使模型从视觉驱动转向文本驱动推理。
...
TL;DR: 提出一种无需训练的CCA后处理方法，通过两个预训练图像编码器的共享结构进行表示选择和降维，在保持语义内容的同时丢弃冗余维度。
...
Conclusion: 提出的CCA后处理方法能有效提高图像表示效率，通过跨模型一致性指导表示蒸馏和精炼，实现更好的表示压缩和性能提升。
...
### [65] [YieldSAT: A Multimodal Benchmark Dataset for High-Resolution Crop Yield Prediction](https://arxiv.org/abs/2604.00940)
...
<details>
  <summary>Details</summary>
Motivation: 现有作物产量预测数据集存在稀缺、质量低、局限于区域或单一作物类型等问题，这阻碍了可扩展数据驱动解决方案的发展。数据获取成本高、数据质量异质性和数据隐私法规进一步限制了数据集的创建。

Method: 发布YieldSAT数据集，涵盖阿根廷、巴西、乌拉圭和德国等多个气候区的2173个专家管理的农田，包括玉米、油菜籽、大豆和小麦等主要作物类型。数据集包含多光谱卫星图像和辅助环境数据，将产量预测作为像素回归任务，并探索了数据融合架构和领域感知的深度集成方法。

Result: 创建了一个包含超过1220万个10米分辨率产量样本和11.3万张标记卫星图像的大规模数据集。通过比较各种深度学习模型和数据融合架构，展示了大规模高分辨率作物产量预测的潜力。领域感知的深度集成方法在处理真实世界条件下的严重分布偏移时表现出显著的性能提升。

Conclusion: YieldSAT为解决作物产量预测中的数据稀缺和质量问题提供了重要资源，推动了可扩展数据驱动解决方案的发展。数据集公开可用，有助于进一步研究和解决真实世界条件下的分布偏移挑战。
...
### [66] [Enhancing Gradient Inversion Attacks in Federated Learning via Hierarchical Feature Optimization](https://arxiv.org/abs/2604.00955)
*Hao Fang,Wenbo Yu,Bin Chen,Xuan Wang,Shu-Tao Xia,Qing Liao,Ke Xu*

Main category: cs.CV

TL;DR: 该论文提出了一种名为GIFD的新方法，通过搜索GAN中间层的分层特征来改进联邦学习中的梯度反演攻击，相比现有方法具有更强的表达能力和泛化性。


<details>
  <summary>Details</summary>
Motivation: 联邦学习虽然通过传输梯度而非原始数据来保护隐私，但研究发现梯度本身也存在隐私泄露风险。现有攻击方法仅在GAN的潜在空间进行梯度反演，限制了表达能力和泛化性。

Method: 提出GIFD方法：1）解构GAN模型，搜索中间层的分层特征；2）从初始潜在空间逐步优化到更接近输出图像的中间层；3）设计正则化器，通过添加小l1球约束避免生成不真实图像；4）扩展到分布外设置，处理标签不一致的挑战场景。

Result: 大量实验表明，该方法能够实现像素级重建，在各种联邦学习场景中优于现有基线方法。

Conclusion: GIFD方法通过搜索GAN中间层的分层特征，显著提升了梯度反演攻击的表达能力和泛化性，即使在分布外和标签不一致的挑战性场景下也能有效工作。
...
### [67] [DLWM: Dual Latent World Models enable Holistic Gaussian-centric Pre-training in Autonomous Driving](https://arxiv.org/abs/2604.00969)
...
TL;DR: DLWM提出了一种用于自动驾驶的双潜在世界模型范式，通过两阶段训练实现基于3D高斯表示的全面预训练，显著提升了3D占用感知、4D占用预测和运动规划任务的性能。


<details>
  <summary>Details</summary>
Motivation: 基于视觉的自动驾驶因其低成本和高性能而备受关注。与密集BEV或稀疏查询模型相比，高斯中心方法通过3D语义高斯描述场景，是一种全面且稀疏的表示。需要一种专门为高斯中心预训练设计的范式来提升自动驾驶系统的整体性能。
...
Conclusion: DLWM为自动驾驶提供了一种有效的双潜在世界模型范式，通过两阶段高斯中心预训练实现了对3D场景的全面稀疏表示，显著提升了多种下游任务的性能。
...
### [68] [ACT Now: Preempting LVLM Hallucinations via Adaptive Context Integration](https://arxiv.org/abs/2604.00983)
*Bei Yan,Yuecong Min,Jie Zhang,Shiguang Shan,Xilin Chen*

Main category: cs.CV

TL;DR: ACT是一种无需训练的动态推理干预方法，通过自适应整合上下文信息来缓解大视觉语言模型的幻觉问题，在保持生成能力的同时显著减少幻觉。


<details>
  <summary>Details</summary>
Motivation: 现有缓解大视觉语言模型幻觉的方法主要依赖静态的单步状态增强视觉关注或抑制语言先验，但忽视了生成过程中的动态上下文变化，且难以纠正信息损失问题。

Method: 提出自适应上下文整合（ACT）方法，包含两个核心组件：1）视觉上下文探索——通过时空分析自适应放大负责视觉探索的注意力头；2）语义上下文聚合——边缘化潜在语义查询以有效聚合视觉证据，解决token预测离散性造成的信息损失。

Result: 在多种大视觉语言模型上的广泛实验表明，ACT显著减少了幻觉，在判别性和生成性基准测试中均取得有竞争力的结果，且不损害基本生成能力。

Conclusion: ACT作为一种无需训练、高度自适应的推理干预方法，通过动态整合上下文信息有效缓解了大视觉语言模型的幻觉问题，提供了稳健的解决方案。
...
### [69] [Maximizing T2-Only Prostate Cancer Localization from Expected Diffusion Weighted Imaging](https://arxiv.org/abs/2604.00985)
...
TL;DR: 该研究提出了一种仅使用T2加权MRI图像进行前列腺癌定位的方法，在训练时利用DWI图像作为潜在模态，通过期望最大化算法和流匹配生成模型，实现了仅用T2图像就能达到甚至超越多序列方法的性能。


<details>
  <summary>Details</summary>
Motivation: 虽然多参数MRI（包括DWI和T2w序列）被推荐用于前列腺癌检测，但仅使用T2w图像具有临床优势：降低成本、减少对其他序列采集的专业知识要求。本研究探索更具挑战性的应用：仅用T2w图像基于独立组织病理学标签定位个体癌症。

Method: 提出期望最大化算法框架：1）E步：使用基于流匹配的生成模型（潜在模态生成器）近似潜在DWI图像的后验分布；2）M步：同时优化癌症定位器和生成模型，最大化癌症存在的期望似然。将DWI图像作为训练时可用的潜在特权模态。

Result: 该方法在4,133名前列腺癌患者的内外部数据集上评估，仅用T2图像的方法性能优于缺乏训练DWI图像的方法，也优于现有的特权学习和不完整模态框架。在患者层面F1分数比T2w+DWI基线提高14.4%，区域层面QWK提高5.3%，与使用多序列输入的基线方法相比具有竞争力或更好。

Conclusion: 该研究提供了一个新颖的理论框架，用于从特权DWI模态中学习，实现了仅使用T2w图像就能获得优异的前列腺癌定位性能，为临床实践中减少序列需求同时保持诊断准确性提供了可行方案。
...
### [70] [Customizing Large Vision Model-Guided Low-Rank Approximation for Ground-Roll Denoise](https://arxiv.org/abs/2604.00998)
*Jiacheng Liao,Feng Qian,Ziyin Fan,Yongjian Guo*

Main category: cs.CV

TL;DR: 提出一种无需训练的语义引导框架，通过大型视觉模型提取地震数据中的地滚波语义先验，将其转化为软掩码嵌入低秩逆问题，实现自适应地滚波衰减并保持反射信号完整性。


<details>
  <summary>Details</summary>
Motivation: 地滚波是陆地和垂直地震剖面数据中的主要相干噪声，严重掩盖反射事件并影响后续成像和解释。传统方法存在适应性有限、信号泄漏或依赖标注训练数据等问题，特别是在强信号噪声重叠情况下表现不佳。

Method: 将地滚波衰减重新定义为语义引导的信号分离问题。使用可提示的大型视觉模型将地震道集转换为视觉表示，通过文本或图像提示定位地滚波主导区域，提取高级语义先验。将语义响应转化为连续软掩码，嵌入掩码条件化的低秩逆公式中，实现空间自适应抑制和反射保持重建。开发基于交替方向乘子法的高效求解器。

Result: 在合成和实际VSP数据集上的广泛实验表明，该方法在保持反射连续性和波形保真度的同时，实现了优异的地滚波衰减效果，一致优于代表性的变换域滤波和隐式神经表示方法。

Conclusion: 提出的训练免费框架通过语义引导的信号分离方法有效解决了地滚波衰减问题，无需任务特定训练或手动标注，实现了物理一致且稳定的信号恢复，为地震数据处理提供了新的自适应解决方案。
...
### [71] [EgoSim: Egocentric World Simulator for Embodied Interaction Generation](https://arxiv.org/abs/2604.01001)
...
Method: 1. 将3D场景建模为可更新的世界状态；2. 通过几何动作感知的观察模拟模型生成具身交互；3. 使用交互感知的状态更新模块确保空间一致性；4. 设计可扩展的数据采集管道，从大规模单目自我中心视频中提取静态点云、相机轨迹和具身动作；5. 引入EgoCap低成本数据采集系统，使用未校准的智能手机。

Result: EgoSim在视觉质量、空间一致性以及对复杂场景和野外灵巧交互的泛化能力方面显著优于现有方法，同时支持跨具身转移到机器人操作。

Conclusion: EgoSim通过建模可更新的3D场景状态，解决了现有自我中心模拟器的局限性，实现了空间一致的交互视频生成和持续的场景状态更新，为具身交互研究提供了有效的模拟平台。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于对抗训练的方法计算成本高且难以泛化到未见攻击类型，需要一种通用、强大且实用的防御框架来增强视觉语言模型对抗对抗性图像扰动的鲁棒性。

Method: 提出Paraphrase-Decomposition-Aggregation (PDA)框架，在测试时进行：1）提示改写（paraphrasing），2）问题分解（question decomposition），3）一致性聚合（consistency aggregation）。通过实例化为不变式来平衡鲁棒性和效率。

Result: 在多个视觉语言模型架构和基准测试（视觉问答、分类、字幕生成）上，PDA对各种对抗性扰动实现了一致的鲁棒性提升，同时保持了有竞争力的干净准确率。

Conclusion: PDA建立了一个通用、强大且实用的视觉语言模型推理时防御框架，无需修改底层模型，通过文本增强有效提升对抗攻击的鲁棒性。
...
### [73] [Forecasting Motion in the Wild](https://arxiv.org/abs/2604.01015)
...
<details>
  <summary>Details</summary>
Motivation: 视觉智能需要预测智能体的未来行为，但现有视觉系统缺乏通用的运动和行为的表示方法。特别是在处理野外动物等非刚性智能体时，需要一种能够解耦运动和外观、并能泛化到不同类别的表示方法。

Method: 1. 提出密集点轨迹作为行为视觉标记，这是一种结构化中层表示，能够解耦运动和外观，泛化到各种非刚性智能体；2. 设计扩散变换器模型，建模无序轨迹集并显式处理遮挡问题；3. 构建大规模评估数据集：300小时无约束动物视频，包含鲁棒的镜头检测和相机运动补偿

Result: 轨迹标记预测实现了类别无关、数据高效的预测，优于现有最先进基线方法，能够泛化到稀有物种和形态，为野外预测性视觉智能提供了基础

Conclusion: 密集点轨迹作为行为视觉标记是一种有效的表示方法，结合扩散变换器模型能够实现复杂运动模式的连贯预测，为预测性视觉智能在野外场景的应用奠定了基础
...
### [74] [Diff3R: Feed-forward 3D Gaussian Splatting with Uncertainty-aware Differentiable Optimization](https://arxiv.org/abs/2604.01030)
*Yueh-Cheng Liu,Jozef Hladký,Matthias Nießner,Angela Dai*

Main category: cs.CV

TL;DR: Diff3R是一个新颖的3D高斯泼溅框架，通过将可微分的3DGS优化层集成到训练循环中，将前馈预测与测试时优化相结合，学习预测优化的初始化参数而非传统零次结果。


<details>
  <summary>Details</summary>
Motivation: 当前3D高斯泼溅技术存在两种主要方向：前馈模型在稀疏视图设置下推理速度快，但质量有限；逐场景优化能产生高质量渲染但计算成本高。需要结合两者的优势。

Method: 1) 在训练循环中集成可微分的3DGS优化层，学习预测测试时优化的最优初始化；2) 使用隐函数定理和专门为3DGS优化的可扩展矩阵自由PCG求解器计算梯度；3) 在优化过程中加入数据驱动的不确定性模型，自适应控制参数变化。

Result: 该方法能有效缓解欠约束区域的过拟合问题，提高对输入异常值的鲁棒性。由于优化层是模型无关的，可以无缝集成到现有的前馈3DGS架构中，为测试时优化提供改进。

Conclusion: Diff3R成功地将前馈预测与测试时优化相结合，通过可微优化层学习最优初始化，结合隐函数定理和不确定性模型，在保持计算效率的同时提高了渲染质量。
...
<details>
  <summary>Details</summary>
Motivation: 高分辨率月球数字高程模型对于表面移动规划、着陆点特征描述和行星科学研究至关重要。Chandrayaan-2上的OHRC相机具有目前月球轨道成像中最佳的地面采样能力（20-30厘米/像素），但需要开发有效的DEM生成方法。

Method: 1. 通过几何分析图像元数据（基线高度比计算和收敛角估计）从非配对OHRC档案中识别候选立体像对；2. 应用密集立体匹配和光线三角测量生成点云；3. 将点云网格化为DEM（空间分辨率24-54厘米）；4. 通过ICP配准和恒定偏差校正建立绝对高程一致性；5. 使用LRO NAC数字地形模型进行验证。

Result: 在五个地理分布的月球站点生成了有效空间分辨率24-54厘米的DEM。验证显示：垂直RMSE为5.85米（原生OHRC分辨率），通过平面特征匹配评估的水平精度小于30厘米。

Conclusion: 成功开发了首个完全开源的OHRC亚米级DEM生成流程，证明了从非配对OHRC档案生成高质量月球地形数据的技术可行性，为月球探测和科学研究提供了重要工具。
...
TL;DR: ONE-SHOT是一个参数高效的人与环境组合视频生成框架，通过解耦人类动态和环境线索，无需3D对齐即可实现精细控制


<details>
  <summary>Details</summary>
Motivation: 当前视频基础模型在人类中心视频合成方面取得进展，但精细独立编辑主体和场景仍然困难。现有方法通过刚性3D几何组合实现环境控制时，面临精确控制与生成灵活性之间的权衡，且3D预处理限制了实际可扩展性

Method: 提出参数高效的组合式人-环境视频生成框架：1）规范空间注入机制，通过交叉注意力解耦人类动态和环境线索；2）Dynamic-Grounded-RoPE位置嵌入策略，在不同空间域间建立对应关系而无需启发式3D对齐；3）混合上下文集成机制，支持分钟级长序列生成时保持主体和场景一致性

Result: 实验表明该方法显著优于最先进方法，在视频合成中提供更优的结构控制和创意多样性

Conclusion: ONE-SHOT框架通过解耦生成过程，实现了精细的人与环境组合视频生成，在控制精度和生成灵活性之间取得了良好平衡，且避免了繁重的3D预处理
...
TL;DR: CROWD数据集是一个从YouTube公开视频中筛选和分割的手动整理城市行车记录仪数据集，包含51,753个连续片段，覆盖全球238个国家和地区，专注于日常驾驶场景，排除事故和编辑内容，提供时间、车辆类型标签及机器生成的物体检测数据。


<details>
  <summary>Details</summary>
Motivation: 现有数据集往往关注事故或特殊事件，缺乏反映日常驾驶场景的数据。为了支持跨域鲁棒性和交互分析研究，需要构建一个包含普通、连续、未编辑的城市驾驶场景的大规模多样化数据集。

Method: 从YouTube公开视频中手动筛选和分割行车记录仪片段，排除事故、事故后果和编辑内容，专注于日常驾驶场景。数据集包含51,753个片段，覆盖全球238个国家和地区，提供手动标注的时间（白天/夜晚）和车辆类型标签，并使用YOLOv11x和BoT-SORT生成机器检测和跟踪数据。

Result: 构建了包含51,753个片段、20,275.56小时视频的CROWD数据集，覆盖全球7,103个命名居住地，包含手动标注的时间、车辆类型信息，并提供所有80个MS-COCO类别的机器生成检测和跟踪数据，以CSV文件形式分发。

Conclusion: CROWD数据集为计算机视觉和自动驾驶研究提供了一个大规模、多样化、专注于日常驾驶场景的基准数据集，通过提供视频标识符和片段边界而非原始视频，既支持可重复研究又避免了版权问题。
...
TL;DR: PHASOR是一种基于体积扩散框架的虚拟CT对比增强方法，通过视频扩散模型和两个互补模块解决现有方法在解剖异质性和空间错位方面的问题。


<details>
  <summary>Details</summary>
Motivation: 对比增强CT（CECT）在临床中很重要，但对比剂的有创性和辐射风险限制了其广泛应用。现有虚拟对比增强（VCE）方法在处理解剖异质性和空间错位方面存在困难，导致增强模式不一致和细节错误。

Method: PHASOR采用体积扩散框架，将CT体积视为连贯序列，利用视频扩散模型提升结构一致性和体积精度。包含两个核心模块：1）解剖路由专家混合（AR-MoE）将不同增强模式锚定到解剖语义，使用器官特定记忆捕获显著细节；2）强度相位感知表示对齐（IP-REPA）突出复杂对比信号，同时减轻不完美空间对齐的影响。

Result: 在三个数据集上的广泛实验表明，PHASOR在合成质量和增强准确性方面显著优于最先进的方法。

Conclusion: PHASOR通过创新的体积扩散框架和互补模块，成功解决了CT虚拟对比增强中的解剖异质性和空间对齐问题，为无创CT对比增强提供了高质量解决方案。
...
TL;DR: ProOOD是一个轻量级即插即用方法，通过原型引导的语义补全和尾部挖掘增强3D语义占据预测，结合EchoOOD进行训练无关的OOD检测，显著提升长尾类别性能和异常检测能力。


<details>
  <summary>Details</summary>
Motivation: 当前3D语义占据预测方法存在两个主要问题：1）对长尾类别偏差敏感，容易将异常样本过度自信地分配给稀有类别；2）对分布外输入脆弱，需要更可靠的异常检测能力以确保自动驾驶安全。

Method: ProOOD包含三个核心组件：1）原型引导的语义补全，用类别一致的特征填充遮挡区域；2）原型引导的尾部挖掘，增强稀有类别表示以减少OOD吸收；3）EchoOOD，融合局部logit一致性与局部全局原型匹配，生成体素级OOD分数。

Result: 在五个数据集上的实验表明，ProOOD在3D占据预测和OOD检测方面均达到最先进性能：在SemanticKITTI上，整体mIoU提升+3.57%，尾部类别mIoU提升+24.80%；在VAA-KITTI上，AuPRCr提升+19.34分，在所有基准测试中均表现一致提升。

Conclusion: ProOOD通过原型引导的语义增强和训练无关的OOD评分，显著提升了3D语义占据预测的校准性和OOD检测的可靠性，为安全关键的城市驾驶提供了更稳健的解决方案。
...
TL;DR: 提出ProTPS方法，通过原型引导的文本提示选择来解决持续学习中的灾难性遗忘问题，并在三个不同设置中验证其有效性，同时发布了真实世界的Marine112数据集。
...
Method: 提出Prototype-guided Text Prompt Selection (ProTPS)方法，通过学习类别特定的视觉原型和文本提示，利用视觉原型指导每个类别的文本提示选择和学习，增加训练灵活性以鼓励学习独特的文本提示。

Result: 在类别增量(CI)和跨数据集持续(CDC)学习设置中，ProTPS性能接近理论上限；在真实世界Marine112数据集（112个海洋物种，自然长尾分布）的类别和域增量(CDI)学习设置中，ProTPS优于现有最先进方法。

Conclusion: ProTPS通过原型引导的文本提示选择有效解决了持续学习中的灾难性遗忘问题，在多个学习设置中表现优异，同时发布的Marine112数据集为社区带来了新的挑战。
...
TL;DR: ReinDriveGen是一个可控的动态驾驶场景生成框架，支持自由编辑演员轨迹来模拟安全关键场景，并通过RL后训练策略提升分布外场景的生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有的驾驶场景生成方法缺乏对动态场景的完全可控性，难以模拟安全关键场景（如碰撞、失控车辆、行人乱穿马路等），这些场景通常超出训练数据分布，需要专门的生成和优化方法。
...
Result: 实验表明，ReinDriveGen在编辑驾驶场景上优于现有方法，并在新颖的自我视角合成任务上取得了最先进的结果。

Conclusion: ReinDriveGen实现了对动态驾驶场景的完全可控编辑，能够生成高质量的安全关键场景视频，其RL后训练策略有效解决了分布外场景的生成质量问题，为自动驾驶系统的测试和验证提供了有力工具。
...
TL;DR: 该论文研究开放集监督3D异常检测，提出Open3D-AD方法，利用正常样本、模拟异常和部分真实异常数据建模概率密度分布，并在新工业数据集Open-Industry上建立基准。


<details>
  <summary>Details</summary>
Motivation: 在实际制造场景中，虽然获取高精度点云成本高，但收集少量异常样本是可行的。因此需要研究开放集监督3D异常检测，仅用正常样本和少量已知异常样本训练模型，以在测试时识别未知异常。

Method: 提出Open3D-AD方法：1) 将通用开放集异常检测方法适配到3D点云输入；2) 利用正常样本、模拟异常和部分真实异常建模正常和异常数据的概率密度分布；3) 引入对应分布子采样减少正常与非正常分布重叠，实现更强的双分布建模。

Result: 在Open-Industry数据集（15个类别，每个类别5种真实异常类型）以及Real3D-AD和Anomaly-ShapeNet等现有数据集上建立全面基准。基准结果和消融研究证明了Open3D-AD的有效性，并揭示了开放集监督3D异常检测的潜力。

Conclusion: 该研究提出了针对3D点云的开放集异常检测方法Open3D-AD，通过建模双分布和减少分布重叠，在工业数据集上表现出色，为实际制造场景中的异常检测提供了有效解决方案。
...
TL;DR: TRACE是一个基于3D高斯泼溅的网格引导编辑框架，通过3D几何锚定实现自动化的高保真场景变换，支持局部姿态调整和组件替换等细粒度操作。


<details>
  <summary>Details</summary>
Motivation: 现有编辑方法缺乏对中心主体结构完整性的保护，无法实现细粒度的部件级操作。TRACE旨在通过显式3D几何锚定视频扩散，实现局部姿态调整和组件替换等精细编辑，同时保持主体结构完整性。

Method: TRACE采用三阶段方法：1) 多视角3D锚合成，利用在MV-TRACE数据集上训练的稀疏视角编辑器生成空间一致的3D锚；2) 有形几何锚定，通过两阶段注册确保插入网格与3DGS场景的精确空间同步；3) 上下文视频掩码，将3D投影集成到自回归视频管道中实现时间稳定的物理基础渲染。

Result: 大量实验表明，TRACE在编辑多功能性和结构完整性方面持续优于现有方法，特别是在局部姿态调整和组件替换等细粒度操作上表现突出。

Conclusion: TRACE通过显式3D几何锚定实现了网格引导的3DGS编辑，为细粒度场景变换提供了自动化、高保真的解决方案，在保持主体结构完整性的同时实现了局部部件级操作。
