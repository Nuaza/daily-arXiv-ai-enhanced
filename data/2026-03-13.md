...
TL;DR: 提出RC-NF模型用于机器人异常检测，通过解耦任务感知的机器人和物体状态处理，仅需正样本进行无监督训练，在仿真和真实世界实验中均表现出色，显著提升VLA机器人系统的鲁棒性。
...
Result: 在LIBERO-Anomaly-10基准测试中，RC-NF在所有异常类型上均达到最先进性能；真实世界实验中作为即插即用模块，提供实时OOD信号，响应延迟低于100毫秒，支持状态级回滚或任务级重规划。
...
TL;DR: Senna-2提出了一种增强视觉语言模型与端到端驾驶策略双系统一致性的方法，通过三阶段训练范式实现决策与规划的对齐，显著提升驾驶安全性和一致性。
...
Method: 采用一致性导向的三阶段训练范式：1）驾驶预训练实现初步决策与规划，通过决策适配器将VLM决策以隐式嵌入形式传递给E2E策略；2）在开环设置中对齐VLM和E2E策略；3）在3DGS环境中通过自底向上的分层强化学习进行闭环对齐，增强安全性和效率。

Result: 实验表明Senna-2实现了优越的双系统一致性（F1分数提升19.3%），在开环设置中显著降低最终位移误差（FDE减少5.7%），在闭环设置中大幅减少事故率（AF-CR减少30.6%）。

Conclusion: Senna-2通过明确对齐视觉语言模型与端到端驾驶策略的双系统，实现了更一致的决策制定和规划，显著提升了自动驾驶系统的安全性和决策跟随能力。
...
TL;DR: FMVR是一种简单即插即用的频率调制视觉恢复策略，通过将少量视觉token分解为高低频分量并调制，在减少视觉token的同时保持视觉语义完整性，显著降低计算量。


<details>
  <summary>Details</summary>
Motivation: 大型多模态模型在处理大量视觉token时计算成本高，现有方法在减少视觉token时会丢失视觉语义信息，需要一种能在减少计算量的同时保持视觉语义完整性的方法。

Method: FMVR通过AvgPool和MaxPool将少量视觉token的表示分解为低频和高频分量，使用轻量级可学习参数进行调制。高频分量作为显著性滤波器增强重要语义，低频分量作为反显著性滤波器增强弱语义。同时结合Matryoshka表示学习实现弹性调整视觉token数量。

Result: 在10个图像基准和4个视频基准上的实验表明，FMVR-LLaVA将LLaVA-1.5-7B的FLOPs降低了89%，同时保持了接近100%的原始准确率。

Conclusion: FMVR是一种有效的频率调制视觉恢复策略，能够在显著减少计算成本的同时保持多模态模型的推理能力，为大型多模态模型在不同计算预算下的部署提供了实用解决方案。
...
TL;DR: 提出slot合并方法，解决固定slot数量导致的多个slot竞争重叠实体区域的问题，通过Soft-IoU评估重叠度并合并，无需额外可学习模块，提升对象分解和掩码质量。


<details>
  <summary>Details</summary>
Motivation: 传统的基于slot的对象中心学习方法中，slot数量固定且只能被细化，这导致多个slot可能竞争重叠的同一实体区域，而不是专注于不同区域，影响了对象分解的效果。

Method: 引入slot合并操作：1) 使用Soft-IoU评分量化slot注意力图之间的重叠度；2) 通过重心更新合并选定的slot对，保持梯度流；3) 采用固定策略，从重叠统计中推断决策阈值，无需额外可学习模块。

Result: 该方法集成到DINOSAUR的特征重建流程中，改善了对象分解和掩码质量，在对象发现和分割基准测试中超越了其他自适应方法。

Conclusion: slot合并是一种轻量级、即插即用的操作，能够有效解决固定slot数量导致的竞争问题，提升对象中心学习性能，无需增加模型复杂度。
...
<details>
  <summary>Details</summary>
Motivation: 虽然语义3D城市模型日益详细，但材料信息整合仍未被充分利用。材料及其物理属性的结构化表示可显著扩展城市数字孪生的应用范围和分析能力。同时，城市移动激光扫描数据包含大量受材料特性影响的观测信息。

Method: 提出辐射指纹方法：将来自同一语义对象在不同距离、入射角、环境条件、传感器和扫描活动下的LiDAR观测分组。使用A2D2数据集中的312.4百万条激光束，自动关联6368个语义3D城市模型对象。基于CityGML 3.0标准构建厘米级精度的LOD3模型。

Result: 成功提取物体表面的辐射指纹，揭示出类内重复模式，这些模式指示了类别主导材料。开发了3DSensorDB地理数据库解决方案，并开源了语义模型和方法实现。

Conclusion: 该方法能够有效利用移动激光扫描数据增强语义3D城市模型的材料信息，为城市数字孪生提供更丰富的分析能力。开源工具促进了该领域的研究和应用发展。
...
<details>
  <summary>Details</summary>
Motivation: 心脏和肺部超声检查技术要求高，操作者需要找到患者特定的肋间声窗并在不同成像平面间导航。在远程超声中，新手或机器人难以在没有专家现场协助的情况下正确放置探头

Method: 使用混合现实头显的RGB相机采集患者图像，边缘服务器推断患者特定的体表和骨骼模型，通过骨骼标志点估计肋间区域，将引导信息投影到重建的体表模型上

Result: 初步实验表明，该探头放置预测和混合现实引导方法能够实现一致的初始放置，误差在可接受的解剖变异范围内，适用于远程超声设置

Conclusion: 提出的框架能够自动化患者注册和初始探头放置引导，仅使用RGB图像即可为远程超声提供有效的解剖引导，降低操作难度
...
TL;DR: 本文提出了一种基于层次粒度对齐和状态空间模型的多模态框架，用于解决野外环境下面部动作单元检测的挑战，在Aff-Wild2数据集上取得了最先进的性能。
...
Method: 1) 使用DINOv2和WavLM基础模型提取鲁棒的高保真视觉和音频表征；2) 层次粒度对齐模块动态对齐全局面部语义与细粒度局部活跃区域；3) Vision-Mamba架构实现O(N)线性复杂度的时序建模，捕捉超长程动态；4) 非对称交叉注意力机制深度同步副语言音频线索与细微视觉运动。

Result: 在具有挑战性的Aff-Wild2数据集上的广泛实验表明，该方法显著优于现有基线，实现了最先进的性能。该框架在第10届野外情感行为分析竞赛的面部动作单元检测赛道中获得了最高排名。

Conclusion: 提出的基于层次粒度对齐和状态空间模型的多模态框架有效解决了野外面部动作单元检测的关键挑战，通过强大的基础模型、动态粒度对齐、高效时序建模和深度跨模态同步，实现了卓越的性能。
...
TL;DR: 提出UniCompress算法，通过可学习的全局元令牌指导的压缩-解压缩机制，将视觉令牌数量减少4倍，显著降低推理延迟和训练成本，同时保持图像理解和生成任务的性能。
...
Conclusion: UniCompress通过有效的令牌压缩机制解决了统一模型中视觉令牌过多导致的效率问题，为资源受限场景下的多模态应用提供了可行的解决方案。
...
Conclusion: ReDiff框架通过可靠性感知机制有效解决了MRI合成中细节恢复与结构保真度的平衡问题，提高了合成图像的解剖一致性和临床可靠性
...
<details>
  <summary>Details</summary>
Motivation: 当前人形机器人的运动跟踪主要限于无接触社交互动或孤立运动，而辅助场景需要持续感知人类伙伴并快速适应其动态姿势和力学交互

Method: 将紧密交互、力交换的人类动作序列模仿建模为多智能体强化学习问题，联合训练支持者和接收者的伙伴感知策略，引入伙伴策略初始化方案、动态参考重定向和接触促进奖励

Result: AssistMimic是首个在现有基准上成功追踪辅助交互动作的方法，展示了多智能体RL在物理基础和社交感知人形控制中的优势

Conclusion: 多智能体强化学习框架能够有效解决人形机器人在物理接触辅助交互中的适应性问题，为服务机器人应用提供了新的技术路径
...
Method: 提出了DriveXQA数据集和MVX-LLM架构。数据集包含四种视觉模态、五种传感器故障、五种天气条件，QA对分为三个层次：全局场景层、异中心层和自我车辆中心层。MVX-LLM采用双交叉注意力投影器来融合多模态信息，减少信息冗余。

Result: 实验表明，双交叉注意力投影器在雾天等挑战性条件下表现优异（GPTScore：53.5 vs. 基线的25.1）。

Conclusion: 该研究填补了多模态大语言模型在自动驾驶异常场景理解方面的空白，提出的数据集和架构为多传感器融合的自动驾驶视觉问答提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 数字条纹投影（DFP）可实现微米级3D重建，但扩展到大规模测绘仍具挑战性，因为六自由度位姿估计精度往往无法匹配重建精度。传统ICP配准在数百万点云上效率低下，通常依赖下采样或特征选择，这会减少局部细节并降低位姿精度。漂移校正方法改善了长期一致性，但未解决密集DFP点云中的采样敏感性。

Method: 提出高精度位姿估计方法，在移动DFP系统中增加一个固定的、经过内参标定的全局投影仪。利用全局投影仪的相位衍生像素约束和PnP式重投影目标，在固定参考系中估计DFP系统位姿，无需依赖确定性特征提取。实验证明该方法在坐标保持下采样下具有采样不变性。

Result: 实验证明：1）相对于参考系统实现亚毫米级位姿精度，具有量化不确定性边界；2）在激进下采样下具有高重复性；3）在均匀表面和低重叠视图中鲁棒运行；4）用于校正基于ICP的轨迹时减少误差累积。

Conclusion: 该方法将DFP扩展到准静态场景（如检测和计量）中的精确3D测绘，代价是为额外投影仪测量进行时间复用采集。解决了DFP大规模测绘中的位姿精度和采样敏感性问题。
...
TL;DR: DeepHistoViT是一种基于Transformer的框架，用于组织病理图像的自动分类，在多个癌症数据集上实现了接近完美的分类性能。


<details>
  <summary>Details</summary>
Motivation: 组织病理学是癌症诊断的金标准，但人工检查耗时耗力且存在观察者间差异。需要可靠的计算机辅助诊断工具来提高效率和一致性。

Method: 提出DeepHistoViT框架，采用定制的Vision Transformer架构，集成注意力机制来捕捉细粒度细胞结构，并通过基于注意力的定位提高可解释性。

Result: 在三个公开的组织病理数据集（肺癌、结肠癌、急性淋巴细胞白血病）上评估，在肺癌和结肠癌数据集上达到100%的准确率、精确率、召回率、F1分数和ROC-AUC；在白血病数据集上分别达到99.85%、99.84%、99.86%、99.85%和99.99%。

Conclusion: Transformer架构在组织病理图像分析中非常有效，DeepHistoViT有潜力作为可解释的计算机辅助诊断工具，支持病理学家的临床决策。
...
TL;DR: DORI是一个专门评估视觉语言模型物体方向推理能力的基准测试，通过四个维度、两个粒度层次来系统评估模型的方向认知能力，发现当前SOTA模型在物体方向理解上表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言基准测试大多将方向与位置和场景理解混为一谈，无法专门评估物体方向推理能力。人类学习物体方向是渐进式的，从识别物体朝向到心理旋转再到物体间方向推理，需要专门的基准来评估AI系统的方向认知能力。

Method: 提出DORI基准，基于人类方向认知阶段将方向分解为四个维度，每个维度在粗粒度（分类）和细粒度（度量）两个层次评估。使用13,652张图像、33,656个多选题，涵盖67个物体类别，通过边界框隔离、标准化空间参考系和结构化提示来隔离方向与其他混淆因素。

Result: 评估24个SOTA视觉语言模型显示：在通用空间基准上表现良好的模型在物体中心方向任务上接近随机水平。最佳模型在粗粒度判断上仅达到54.2%，细粒度判断仅45.0%，最大失败出现在复合旋转和物体间参考系变化上。粗粒度到细粒度的大差距表明模型依赖分类启发式而非几何推理。

Conclusion: 方向理解是多模态系统尚未解决的挑战，现有基准隐藏了这一局限性。DORI揭示了模型在方向推理上的根本缺陷，对机器人操作、3D场景重建和人机交互有重要影响。
...
TL;DR: R-MSD是一种可靠的多样本蒸馏框架，通过建模教师采样方差来增强蒸馏稳定性，在视频理解任务中显著优于传统单样本蒸馏方法。
...
TL;DR: 本文提出了一种基于角色特定查询的密集视频描述方法，通过分离定位和描述任务、对比对齐、抑制机制和概念增强模块来解决多任务干扰和时间冗余问题。


<details>
  <summary>Details</summary>
Motivation: 密集视频描述任务中，现有的基于查询的框架虽然能同时处理定位和描述，但共享查询会导致多任务干扰和时间冗余问题，影响定位精度和描述质量。

Method: 1. 使用角色特定查询分离定位和描述任务；2. 通过对比对齐确保分离查询间的语义一致性；3. 设计抑制机制惩罚查询间的时间重叠；4. 引入轻量级模块捕捉核心事件概念以增强描述语义丰富度。

Result: 在YouCook2和ActivityNet Captions等主要DVC基准测试上进行了广泛实验，证明了方法的有效性。

Conclusion: 通过角色特定查询分离任务、对比对齐、抑制机制和概念增强，有效解决了密集视频描述中的多任务干扰和时间冗余问题，提高了定位精度和描述质量。
...
TL;DR: DART是一个无需训练、基于SAM3的实时多类别检测框架，通过共享视觉主干计算实现O(1)复杂度，在80个类别上实现25倍加速，在COCO数据集上达到55.8 AP和15.8 FPS。


<details>
  <summary>Details</summary>
Motivation: SAM3虽然实现了最先进的提示式检测和分割，但每次前向传播只能处理单个文本提示，检测N个类别需要N次独立执行，其中439M参数的视觉主干计算占主导，导致效率低下。

Method: 利用视觉主干是类别无关的结构不变性，共享主干计算将复杂度从O(N)降至O(1)；结合批量多类别解码、仅检测推理和TensorRT FP16部署；对于极端延迟需求，使用冻结编码器-解码器的适配器蒸馏。

Result: 在COCO val2017上，DART达到55.8 AP和15.8 FPS（4类别，1008x1008分辨率），在单个RTX 4080上超越专门训练的开集检测器；3个类别时实现5.6倍加速，80个类别时扩展到25倍加速；适配器蒸馏版本在13.9ms主干下达到38.7 AP。

Conclusion: DART通过训练免费的优化将SAM3转化为实时多类别检测器，显著提升效率而不修改模型权重，为提示式视觉模型的实际部署提供了高效解决方案。
...
TL;DR: STaRC是一个用于密集视频描述的新框架，通过亮点检测模块监督帧级显著性，利用显著性分数作为统一的时间信号驱动检索和指导字幕生成，在YouCook2和ViTT基准测试中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的检索增强密集视频描述方法通常无法实现与真实事件边界对齐的准确时间分割，因为它们依赖启发式策略而忽略了地面真实事件边界。

Method: 提出STaRC框架：1) 通过亮点检测模块监督帧级显著性，该模块使用DVC地面真实注释派生的二元标签训练；2) 将显著性分数作为统一的时间信号，通过显著性引导的分割驱动检索；3) 通过注入解码器的显式显著性提示来指导字幕生成。

Result: 在YouCook2和ViTT基准测试中，STaRC在大多数指标上实现了最先进的性能，通过显著性约束的分割产生了与真实事件转换紧密对齐的时间连贯片段。

Conclusion: STaRC通过监督帧级显著性并利用显著性作为统一的时间信号，克服了现有检索增强方法在时间分割上的局限性，实现了更准确的检索和上下文基础的字幕生成。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频大语言模型存在幻觉问题，输出可能违反视频证据（忠实性）或可验证的世界知识（事实性）。现有基准对事实性幻觉覆盖有限，且主要在干净环境下评估模型。

Method: 提出INFACT基准，包含9800个QA实例，具有细粒度的忠实性和事实性分类体系。在四种模式下评估模型：基础（干净）、视觉退化、证据损坏和时间干预。使用抵抗率和时间敏感性分数量化可靠性。

Result: 对14个代表性视频大语言模型的实验显示，基础模式下的高准确率不能可靠转化为诱导模式下的高可靠性。证据损坏降低稳定性，时间干预导致最大性能下降。许多开源基线在事实性问题上时间敏感性分数接近零，表明对顺序敏感问题存在明显的时间惯性。

Conclusion: INFACT基准揭示了视频大语言模型在非理想条件下的可靠性问题，强调了需要超越干净环境评估模型性能，并关注模型对时间顺序的敏感性。
...
TL;DR: SPEGC提出了一种基于语义提示增强图聚类的持续测试时适应方法，用于解决医学图像分割中的域偏移问题，通过增强特征表示和优化聚类结构来提升模型在动态变化域中的适应能力。
...
Method: 1. 设计语义提示特征增强机制，利用解耦的共性和异质性提示池将全局上下文信息注入局部特征，减轻域偏移下的噪声干扰；2. 基于增强特征设计可微分图聚类求解器，将全局边稀疏化重构为最优传输问题，以端到端方式从原始相似度矩阵中提取精炼的高阶结构表示；3. 使用这种鲁棒的结构表示指导模型适应，确保预测在聚类级别上一致并动态调整决策边界。

Result: 在两个医学图像分割基准测试上的大量实验表明，SPEGC优于其他最先进的持续测试时适应方法。

Conclusion: SPEGC通过语义提示增强和可微分图聚类有效解决了医学图像分割中的持续测试时适应问题，显著提升了模型在动态变化域中的性能表现。
...
TL;DR: OrthoEraser：一种基于稀疏自编码器和解析正交化投影的文本到图像模型安全概念擦除方法，能在消除有害内容的同时保护良性语义的完整性。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型面临对抗诱导的安全风险，现有的概念擦除方法在抑制选定神经元时往往会对良性属性造成附带损害，因为敏感语义和良性语义在激活子空间中存在非正交叠加和纠缠。

Method: OrthoEraser使用稀疏自编码器实现高分辨率特征解耦，将擦除重新定义为解析正交化投影以保持良性流形不变性。首先用SAE分解密集激活并分离敏感神经元，然后通过耦合神经元检测识别易受干预的非敏感特征，最后采用解析梯度正交化策略将擦除向量投影到耦合神经元的零空间。

Result: 实验结果表明，OrthoEraser在安全擦除方面实现了高精度，有效移除有害内容的同时保持了生成流形的完整性，显著优于现有最先进基线方法。

Conclusion: 通过正交化解耦敏感概念与关键良性子空间，OrthoEraser能够在消除有害内容的同时保护非敏感语义，为文本到图像模型的安全概念擦除提供了有效解决方案。
...
TL;DR: 提出Manifold-Optimal Guidance (MOG)框架，解决传统Classifier-Free Guidance在高引导尺度下导致的过饱和、纹理伪影和结构崩溃问题，通过几何感知的黎曼更新纠正离流形漂移。


<details>
  <summary>Details</summary>
Motivation: 传统Classifier-Free Guidance (CFG)在高引导尺度下会导致过饱和、纹理伪影和结构崩溃，这是因为标准CFG在环境空间中进行欧几里得外推，无意中将采样轨迹推离高密度数据流形。

Method: 提出Manifold-Optimal Guidance (MOG)框架，将引导重新表述为局部最优控制问题，产生封闭形式的几何感知黎曼更新，无需重新训练即可纠正离流形漂移。进一步引入Auto-MOG，这是一种动态能量平衡调度，自适应校准引导强度。

Result: MOG在保真度和对齐方面优于基线方法，几乎不增加计算开销。Auto-MOG有效消除了手动超参数调优的需求。

Conclusion: MOG通过几何感知的流形最优引导解决了传统CFG的局限性，提供了一种高效、无需重新训练的解决方案，显著提升了条件扩散模型的生成质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有组合图像检索模型在标准基准测试中表现良好，但在语义对齐的困难负样本场景下性能显著下降。作者认为这种性能下降源于模型的焦点失衡问题——模型过度关注某一模态而忽视另一模态。

Method: 1. 提出FBCIR多模态焦点解释方法，识别模型检索决策中最关键的视觉和文本输入成分；2. 基于分析结果，设计CIR数据增强流程，为现有数据集添加精心设计的困难负样本，促进平衡的跨模态推理。

Result: FBCIR分析显示焦点失衡在现有CIR模型中普遍存在，尤其在困难负样本设置下更明显。提出的数据增强流程在多个CIR模型上均能一致提升困难场景下的性能，同时保持标准基准测试的能力。

Conclusion: FBCIR解释方法和数据增强流程为CIR模型的诊断和鲁棒性改进提供了新视角，通过识别和解决焦点失衡问题，有效提升了模型在挑战性场景下的表现。
...
TL;DR: 提出一个统一的UCOD框架，通过多线索原生感知模块和伪标签进化融合机制，在无监督伪装目标检测中同时提升伪标签可靠性和特征保真度。


<details>
  <summary>Details</summary>
Motivation: 现有无监督伪装目标检测方法面临两个主要问题：1) 伪标签噪声干扰细粒度纹理学习；2) 现有细化策略忽视内在感知线索，导致边界溢出和结构模糊。而不使用伪标签指导则会产生粗糙特征和细节丢失。

Method: 提出统一UCOD框架，包含三个核心模块：1) 多线索原生感知模块，整合低层纹理线索与中层语义，实现掩码与原生目标信息的精确对齐；2) 伪标签进化融合，通过师生交互智能细化标签，使用深度可分离卷积进行语义去噪，并引入谱张量注意力融合平衡语义和结构信息；3) 局部伪标签细化，利用注意力多样性恢复精细纹理和增强边界保真度。

Result: 在多个UCOD数据集上的大量实验表明，该方法实现了最先进的性能，具有卓越的细节感知能力、鲁棒的边界对齐以及在复杂伪装场景下的强泛化能力。

Conclusion: 提出的统一框架有效解决了UCOD中伪标签噪声和特征保真度问题，通过整合多线索感知和智能标签进化机制，显著提升了无监督伪装目标检测的性能。
...
TL;DR: MDS-VQA提出了一种基于模型信息的数据选择机制，通过识别对基础VQA模型既困难又多样的未标记视频，在有限标注预算下提升视频质量评估模型的性能。
...
Method: MDS-VQA是一种模型信息的数据选择机制，包含两个核心：1）通过基于排序目标训练的失败预测器估计视频对基础VQA模型的难度；2）使用深度语义视频特征测量内容多样性，并通过贪心算法在有限标注预算下平衡难度和多样性。

Result: 实验表明，MDS-VQA能够识别多样且具有挑战性的样本，对主动微调特别有效。仅使用目标领域5%的选定子集进行微调，模型平均SRCC从0.651提升到0.722，并获得最高的gMAD排名，显示出强大的适应和泛化能力。

Conclusion: MDS-VQA通过系统性地选择对现有模型既困难又多样的样本，有效解决了VQA领域模型设计与数据集构建的脱节问题，在有限标注预算下显著提升了模型的性能和泛化能力。
...
TL;DR: Mobile-GS：针对移动设备优化的实时3D高斯溅射方法，通过消除深度排序、引入神经视角相关增强和压缩技术，在保持高质量渲染的同时实现实时性能和紧凑模型尺寸。


<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射（3DGS）虽然能实现高质量渲染，但其高计算需求和存储成本对移动设备部署构成重大挑战。需要开发适合移动设备的实时高斯溅射方法。

Method: 1. 提出深度感知的顺序无关渲染方案，消除高斯深度排序需求；2. 引入神经视角相关增强策略，通过视角方向、3D高斯几何和外观属性建模视角相关效果；3. 采用一阶球谐蒸馏、神经向量量化和基于贡献的剪枝策略压缩3D高斯表示。

Result: Mobile-GS在保持高视觉质量的同时实现了实时渲染和紧凑模型尺寸，适合移动应用。实验证明该方法在移动设备上表现优异。

Conclusion: Mobile-GS通过创新的渲染方案、神经增强和压缩技术，成功解决了3DGS在移动设备上的部署难题，为移动端高质量实时渲染提供了有效解决方案。
...
TL;DR: RiskMV-DPO：一种用于自动驾驶场景生成的物理感知、风险可控多视图生成框架，通过整合目标风险等级和物理基础风险建模，合成高风险动态轨迹作为扩散视频生成的几何锚点。


<details>
  <summary>Details</summary>
Motivation: 生成安全关键的驾驶场景对于评估和改进自动驾驶系统至关重要，但长尾风险情况在真实世界数据中很少见，且难以通过手动场景设计来指定。现有生成方法通常将风险作为事后标签处理，并且在多视图驾驶场景中难以保持几何一致性。

Method: 提出了RiskMV-DPO框架：1）整合目标风险等级与物理基础风险建模，自主合成多样化的高风险动态轨迹作为几何锚点；2）引入几何-外观对齐模块确保时空一致性；3）采用区域感知直接偏好优化（RA-DPO）策略，通过运动感知掩码聚焦于局部动态区域的学习。
...
TL;DR: ReHARK提出了一种无需训练的少样本适应框架，通过全局近端正则化和多阶段精炼流程，解决了CLIP等视觉语言模型在单样本适应中的稳定性-可塑性困境，在11个基准测试中取得了新的最优性能。
...
Method: ReHARK是一个协同的免训练框架，通过再生核希尔伯特空间（RKHS）中的全局近端正则化重新解释少样本适应。采用多阶段精炼流程：1）混合先验构建：融合CLIP和GPT-3的零样本文本知识与视觉类原型；2）支持集增强：生成中间样本平滑视觉和文本模态之间的过渡；3）自适应分布校正：对齐测试特征统计量与增强支持集以缓解域偏移；4）多尺度RBF核：使用核集合捕获不同尺度的复杂特征几何。

Result: 在11个多样化基准测试中展示了优越的稳定性和准确性。ReHARK建立了单样本适应的新最优性能，平均准确率达到65.83%，显著优于现有基线方法。
...
Method: 提出多帧节点引导框架Mango-GS，使用时间Transformer在短帧窗口内建模运动依赖，通过稀疏控制节点（包含解耦的规范位置和潜在编码）实现高效运动传播，采用输入掩码策略和两种多帧损失进行端到端训练

Result: Mango-GS实现了最先进的重建质量和实时渲染速度，能够高保真重建动态场景并支持交互式渲染

Conclusion: Mango-GS通过多帧节点引导方法有效解决了动态3D场景重建中的时间一致性问题，在保持高质量重建的同时实现了实时渲染性能
...
TL;DR: 本文提出DIAE模型，通过多模态美学感知和双分支弱监督框架解决图像美学增强问题，在缺乏完美配对数据的情况下实现更好的美学编辑效果。
...
Method: 提出DIAE模型：1）引入多模态美学感知(MAP)，将模糊的美学指令转化为明确指导，通过详细标准化的美学属性和文本-图像对的多模态控制信号；2）收集"不完美配对"数据集IIAEData；3）采用双分支监督框架进行弱监督训练。

Result: 实验结果表明，DIAE在图像美学评分和图像内容一致性评分方面均优于基线方法，取得了更优的性能。

Conclusion: DIAE通过多模态美学感知和双分支弱监督框架，有效解决了图像美学增强中的关键挑战，在缺乏完美配对数据的情况下实现了更好的美学编辑效果。
...
TL;DR: TornadoNet是一个用于街景建筑损伤评估的基准测试，比较了CNN和Transformer架构在灾害后多级损伤检测中的表现，并研究了有序分类监督策略的效果。
...
Method: 使用2021年中西部龙卷风爆发的3,333张高分辨率地理标记图像和8,890个标注建筑实例，基于IN-CORE损伤状态的五级分类框架，系统比较YOLO系列CNN检测器和RT-DETR Transformer模型。引入软有序分类目标和显式有序距离惩罚来对齐监督与损伤严重程度的有序特性。

Result: CNN-based YOLO模型在检测精度和吞吐量上表现最佳（46.05% mAP@0.5，66-276 FPS），而Transformer-based RT-DETR模型在有序一致性上更强（88.13%有序Top-1准确率）。使用校准有序监督的RT-DETR实现了44.70% mAP@0.5，提升了4.8个百分点，有序指标也得到改善。

Conclusion: 有序感知监督与检测器架构对齐时能改善损伤严重程度估计，CNN和Transformer架构在损伤检测中各有优势，为灾害响应提供了方法论见解和可部署工具。
...
TL;DR: SVLL是一个三阶段视觉语言学习框架，通过解耦空间基础与时间推理，结合新颖的Bias-DPO对齐目标，提升具身任务规划的视觉基础和因果连贯性，在AI2-THOR基准测试中超越现有模型。


<details>
  <summary>Details</summary>
Motivation: 现有具身任务规划方法面临关键权衡：端到端训练导致过早的时间绑定，而标准强化学习方法存在优化不稳定问题。需要一种既能建立稳健视觉基础又能保证因果连贯性的统一框架。
...
Conclusion: SVLL通过分阶段学习和Bias-DPO对齐，有效解决了具身规划中的视觉基础与因果连贯性平衡问题，确保严格遵循环境可供性并抑制物理上不可能的捷径，为稳健的具身任务规划提供了有效解决方案。
...
TL;DR: R4Det：一种改进的4D雷达-相机融合3D目标检测方法，通过全景深度融合、可变形门控时序融合和实例引导动态优化模块解决现有方法的深度估计不准确、时序融合依赖车辆姿态以及雷达点云稀疏等问题。


<details>
  <summary>Details</summary>
Motivation: 现有4D雷达与相机融合的3D目标检测方法存在三个主要问题：1）绝对深度估计模块不够鲁棒和准确，导致3D定位不准确；2）时序融合模块在车辆姿态缺失或不准确时性能急剧下降甚至失效；3）对于小物体，稀疏的雷达点云可能完全无法从其表面反射，此时检测只能依赖视觉单模态先验。

Method: 提出R4Det框架，包含三个核心模块：1）全景深度融合模块，通过绝对深度和相对深度相互增强来提高深度估计质量；2）可变形门控时序融合模块，不依赖车辆姿态信息；3）实例引导动态优化模块，从2D实例引导中提取语义原型。

Result: 在TJ4DRadSet和VoD数据集上实现了最先进的3D目标检测结果。

Conclusion: R4Det通过改进深度估计、设计不依赖车辆姿态的时序融合方法以及引入实例引导优化，有效解决了现有4D雷达-相机融合3D检测方法的局限性，在多个数据集上取得了优越性能。
...
Method: 提出系统性解决方案：1) 基于HTML的自动编辑管道生成330K训练对；2) 双语和多语言基准测试；3) 两阶段训练策略：字形引导的监督微调注入空间和内容先验，多目标强化学习对齐生成质量。
...
TL;DR: 该论文提出LabanLite运动表示法和LaMoGen框架，通过符号推理实现语言驱动的运动合成，解决了现有方法在时间精度、细节和可解释性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有基于文本-运动嵌入的方法在合成时间精确、详细的运动方面存在困难，且缺乏可解释性。人类运动具有高度表现力并与语言自然对齐，但现有方法无法充分利用这种对齐关系。

Method: 1. 开发LabanLite运动表示法，基于Labanotation系统扩展，将原子身体部位动作编码为离散符号与文本模板对；2. 提出LaMoGen框架，让大语言模型通过符号推理组合运动序列；3. 建立基于Labanotation的基准测试，包含结构化描述-运动对和三个评估指标。

Result: 实验表明LaMoGen在可解释性和可控性方面建立了新的基准，在作者提出的基准测试和两个公共数据集上优于先前方法。结果突出了符号推理和基于智能体设计在语言驱动运动合成中的优势。

Conclusion: LabanLite和LaMoGen框架通过符号推理实现了语言驱动的运动合成，提供了更好的可解释性、时间精度和细节控制，为运动生成领域带来了新的可能性。
...
Method: 提出两阶段框架：1) 运动先验驱动初始化，利用3D点轨迹和低维运动结构进行软分解；2) 几何与运动约束细化，通过可学习运动基元（关节轴、枢轴点、每帧运动标量）强制物理合理的关节运动。

Result: 在合成基准测试和真实世界随意拍摄的单目视频上达到最先进性能，显著提升了非受控真实条件下数字孪生创建的可行性。

Conclusion: Articulat3D通过联合几何和运动约束，成功从单目视频构建高质量关节物体数字孪生，解决了现有方法的可扩展性限制。
...
TL;DR: DyWeight是一种轻量级、基于学习的多步求解器，通过动态梯度加权和隐式耦合范式，显著提升扩散模型采样效率，减少函数评估次数，同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型的采样过程需要数百次函数评估，速度缓慢。现有的多步ODE求解器虽然通过重用历史梯度提高了效率，但依赖于手工设计的系数，无法适应扩散采样过程中的非平稳动态变化。

Method: 提出动态梯度加权(DyWeight)，采用简化的隐式耦合范式，放松经典数值约束，学习无约束的时变参数来自适应聚合历史梯度，同时内在缩放有效步长。这种隐式时间校准确保在大积分步长下，求解器的数值轨迹与模型内部去噪动态准确对齐。

Result: 在CIFAR-10、FFHQ、AFHQv2、ImageNet64、LSUN-Bedroom、Stable Diffusion和FLUX.1-dev等多个数据集和模型上的实验表明，DyWeight以显著更少的函数评估次数实现了优越的视觉保真度和稳定性，在高效扩散求解器中建立了新的最先进水平。

Conclusion: DyWeight通过动态梯度加权和隐式时间校准，有效解决了现有多步求解器无法适应扩散采样非平稳动态的问题，在保持生成质量的同时大幅提升了采样效率。
...
Method: 采用三个关键技术：1) 多视图提示结合不平衡最优传输实现细粒度补丁到提示对应，同时抑制不可靠区域；2) 双向提示设计捕获互补的干净导向和噪声感知线索；3) 对齐引导的选择性精炼策略，仅修正错误标记样本。
...
TL;DR: MedPruner：一种无需训练、模型无关的分层token剪枝框架，专门为高效3D医学图像理解设计，可减少95%以上视觉token同时保持或提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前医学视觉语言模型在处理3D容积数据时存在显著计算效率问题，主要由于直接拼接连续2D切片导致大量解剖冗余，且缺乏灵活性来处理不同切片间异质信息密度。

Method: 提出MedPruner框架，包含两阶段机制：1）切片间锚点过滤模块消除切片级时间冗余；2）动态信息核心选择策略通过量化累积注意力权重实现自适应token级压缩。

Result: 在三个3D医学基准测试和三种不同医学VLM上的实验显示，现有架构存在大量token冗余。MedPruner使模型（如MedGemma）在保留少于5%视觉token的情况下保持或超越原始性能，大幅降低计算开销。

Conclusion: MedPruner验证了动态token选择对于实际临床部署的必要性，为高效3D医学图像理解提供了有效的解决方案，显著减少了计算负担同时保持模型性能。
...
TL;DR: SegAnyPET是一个用于3D全身PET图像通用分割的基础模型，通过大规模数据集和提示工程策略实现器官和病灶的零样本分割，支持临床人机协作工作流。


<details>
  <summary>Details</summary>
Motivation: PET成像在疾病管理中具有不可替代的作用，但由于PET图像解剖对比度低、数据获取和标注成本高，深度学习模型在定量PET图像分析方面发展严重受限。

Method: 构建了迄今为止最大最全面的PET数据集（11041个3D全身PET扫描，59831个分割掩码），开发了SegAnyPET基础模型，采用3D架构和提示工程策略进行掩码生成。

Result: 在多中心、多示踪剂、多疾病数据集上的广泛评估表明，SegAnyPET在各种分割任务中实现了强大的零样本性能，支持高效的人工校正和临床人机协作工作流。

Conclusion: SegAnyPET作为通用分割基础模型，具有广泛的临床应用潜力，能够推动分子成像的临床转化和应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有统一3D生成模型虽然能从单张图像生成高质量3D资产，但存在两个主要限制：1）仅限于单视角输入，无法利用多视角互补信息；2）独立估计的对象姿态常导致物理上不合理的布局（如穿透和漂浮现象）。

Method: 提出MV-SAM3D框架：1）将多视角融合建模为3D潜在空间中的多扩散过程；2）提出两种自适应加权策略（注意力熵加权和可见性加权），实现置信度感知融合；3）引入物理感知优化，在生成过程中和生成后注入碰撞和接触约束。

Result: 在标准基准测试和真实世界多对象场景上的实验表明，该方法在重建保真度和布局合理性方面均有显著提升，且无需额外训练。

Conclusion: MV-SAM3D成功扩展了布局感知3D生成，通过多视角一致性和物理合理性增强了场景级3D生成能力，为实用场景级3D生成开辟了新途径。
...
TL;DR: HouseMind是一个多模态大语言模型，统一了建筑平面图的理解、生成和编辑功能，通过离散房间实例标记实现几何、语义和空间层次推理。
...
Conclusion: HouseMind成功解决了建筑平面图设计中的多模态推理挑战，实现了理解、生成和编辑的统一框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态抑郁症检测方法面临两个主要问题：1) 模态间不一致性和抑郁症无关干扰，不同模态的抑郁相关线索可能冲突，同时大量无关内容掩盖关键抑郁信号；2) 个体抑郁表现差异大，导致模态和线索重要性因人而异，阻碍可靠融合。
...
Conclusion: IDRL框架通过解耦表示和个体感知融合，有效解决了多模态抑郁症检测中的模态不一致性和个体差异问题，为稳健的抑郁症诊断提供了有效解决方案。
...
TL;DR: FL-MedSegBench是首个用于医学图像分割的联邦学习综合基准，涵盖9个分割任务和10种成像模态，系统评估了8种通用FL方法和5种个性化FL方法在多个维度上的表现。
...
Method: 创建FL-MedSegBench基准，包含9个分割任务和10种成像模态，涵盖2D和3D格式及真实的临床异质性。系统评估了8种通用FL方法和5种个性化FL方法在分割精度、公平性、通信效率、收敛行为和未见域泛化能力等多个维度。

Result: 实验发现：(1)个性化FL方法（特别是基于客户端特定批量归一化的方法如FedBN）持续优于通用方法；(2)没有单一方法在所有数据集上表现最佳，性能具有数据集依赖性；(3)基于归一化的个性化方法对降低通信频率具有显著鲁棒性；(4)Ditto和FedRDN等方法能保护性能较差的客户端；(5)方法对未见域的泛化能力与其在参与客户端上的表现密切相关。

Conclusion: FL-MedSegBench为医学图像分割的联邦学习研究提供了首个综合基准，揭示了不同方法的优缺点，并提供了基于实证的临床部署指南。将发布开源工具包以促进可重复研究和加速临床适用的FL解决方案。
...
Method: 基于注意力劫持和恢复现象：对输入图像进行渐进掩码，当掩码比例超过触发器鲁棒性阈值时，注意力从恶意触发器特征快速转向良性内容，导致嵌入向量突变。通过提取掩码轨迹上的嵌入序列，应用基于密度的聚类（如DBSCAN）检测突变

Result: 在多种攻击类型、数据集和模型家族上一致优于现有防御方法，支持CNN、ViT、CLIP、LLaVA-1.5等多种架构

Conclusion: BackdoorIDS是一种即插即用、无需重新训练、完全零样本推理时的后门检测方法，能有效保护视觉编码器免受后门攻击
...
TL;DR: 该论文提出了OSCBench基准测试，专门评估文本到视频生成模型在对象状态变化（OSC）方面的性能，发现现有模型在这方面存在明显不足。


<details>
  <summary>Details</summary>
Motivation: 现有文本到视频生成基准主要关注感知质量、文本-视频对齐或物理合理性，但忽略了动作理解的关键方面——文本提示中明确指定的对象状态变化（OSC）。OSC指由动作引起的对象状态转变，如削土豆皮或切柠檬。

Method: 从教学烹饪数据构建OSCBench基准，将动作-对象交互系统组织为常规、新颖和组合场景，以探测分布内性能和泛化能力。使用人类用户研究和基于多模态大语言模型的自动评估方法，评估六个代表性的开源和专有T2V模型。

Result: 尽管在语义和场景对齐方面表现良好，但当前T2V模型在准确和时序一致的对象状态变化方面持续存在困难，特别是在新颖和组合设置中。模型在OSC方面的表现明显落后于其他评估维度。

Conclusion: OSC是文本到视频生成的关键瓶颈，OSCBench可作为诊断基准，推动状态感知视频生成模型的发展。
...
TL;DR: 论文提出了VTEdit-Bench基准测试，用于系统评估通用多参考图像编辑模型在虚拟试穿任务中的表现，发现顶级通用编辑器在传统任务上具有竞争力且泛化更稳定，但在复杂参考配置（特别是多服装条件）方面仍有挑战。


<details>
  <summary>Details</summary>
Motivation: 随着虚拟试穿技术的发展，现实场景需求已超出专用VTON模型的能力范围。通用多参考图像编辑模型在视觉编辑方面表现出强大泛化能力，但缺乏系统评估基准来探索其在VTON任务中的优势和局限性。

Method: 提出VTEdit-Bench基准，包含24,220个测试图像对，涵盖5个代表性VTON任务，复杂度逐步增加。同时提出VTEdit-QA评估器，基于参考感知的视觉语言模型从模型一致性、服装一致性和整体图像质量三个方面评估VTON性能。

Result: 系统评估了8个通用编辑模型和7个专用VTON模型。结果显示，顶级通用编辑器在传统任务上具有竞争力，在更复杂场景中泛化更稳定，但在复杂参考配置（特别是多服装条件）方面仍面临挑战。

Conclusion: VTEdit-Bench为评估通用编辑模型在VTON任务中的表现提供了系统基准，揭示了通用编辑器在虚拟试穿领域的潜力和局限性，为未来更灵活的VTON系统发展指明了方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有自回归扩散模型虽然支持流式生成，但在扩展到小时级实时人体动画时面临两大挑战：1）大多数强制策略传播样本级表示时存在扩散状态不匹配，导致学习信号不一致和收敛不稳定；2）历史表示无限制增长且缺乏结构，无法有效重用缓存状态，严重限制推理效率。

Method: 提出Neighbor Forcing方法，这是一种扩散步骤一致的自回归公式，在相同噪声条件下传播时间相邻帧作为潜在邻居，提供分布对齐的稳定学习信号。在此基础上引入结构化ConvKV记忆机制，将因果注意力中的键和值压缩为固定长度表示，实现恒定内存推理和真正无限视频生成。
...
Conclusion: Neighbor Forcing和ConvKV记忆机制有效解决了自回归扩散模型在长序列生成中的关键瓶颈，实现了高效、高质量的小时级实时人体动画生成，为流式视频合成提供了实用解决方案。
...
TL;DR: 提出一种基于稀疏3D手部关节的运动可控视频生成框架，解决现有方法在自我中心视角下3D一致性差、遮挡处理不佳、跨具身泛化能力弱的问题


<details>
  <summary>Details</summary>
Motivation: 现有运动可控视频生成方法在自我中心应用中存在3D一致性差的问题，它们依赖2D轨迹或隐式姿态，将3D几何信息压缩为空间模糊信号或过度依赖人体先验知识。在严重的自我中心遮挡情况下，这会导致运动不一致、产生幻觉伪影，且无法泛化到机器人手部

Method: 提出一个从单参考帧生成自我中心视频的框架，使用稀疏3D手部关节作为具身无关的控制信号。引入高效的控制模块，通过惩罚隐藏关节的不可靠视觉信号提取遮挡感知特征，采用3D加权机制处理动态遮挡的目标关节，并直接将3D几何嵌入注入潜在空间以强制结构一致性

Result: 开发了自动化标注流程，获得超过100万个高质量自我中心视频片段及精确手部轨迹，构建了跨具身基准。实验表明该方法显著优于现有基线，能生成高保真自我中心视频，并展现出对机器人手的卓越跨具身泛化能力

Conclusion: 提出的基于稀疏3D手部关节的框架有效解决了自我中心视频生成中的3D一致性、遮挡处理和跨具身泛化问题，为虚拟现实和具身AI应用提供了更可靠的解决方案
...
TL;DR: 本文提出了一种新的任务CI-ICE（组合可解释内在概念提取），旨在从单张图像中提取可组合的对象级和属性级概念，并通过HyperExpress方法在双曲空间中实现准确的概念解耦和组合优化。


<details>
  <summary>Details</summary>
Motivation: 现有无监督概念提取方法无法提取可组合的内在概念，这限制了从单张图像中提取结构化、可解释概念的能力。为了解决这个问题，作者提出了CI-ICE任务，旨在利用基于扩散的文本到图像模型从单张图像中提取可组合的对象级和属性级概念。
...
Result: 该方法在从单张图像中提取组合可解释内在概念方面表现出色，能够准确解耦概念并保持其组合特性。

Conclusion: 通过引入CI-ICE任务和HyperExpress方法，成功解决了从单张图像中提取可组合内在概念的问题，为无监督概念提取提供了新的解决方案。
...
TL;DR: RICE-NET是一个多模态3D深度学习模型，整合纵向MRI数据和放疗剂量分布，用于区分胶质母细胞瘤治疗后患者的肿瘤复发与放射性对比增强，在独立测试集上F1分数达0.92。


<details>
  <summary>Details</summary>
Motivation: 区分胶质母细胞瘤治疗后患者的肿瘤复发与放射性对比增强是临床主要挑战。现有方法依赖临床稀疏可用的扩散MRI或未考虑放疗剂量图，而放疗剂量图在肿瘤委员会中对此区分越来越受关注。

Method: 提出RICE-NET多模态3D深度学习模型，整合纵向MRI数据与放疗剂量分布，使用常规T1加权MRI数据进行自动病变分类。通过92名患者队列进行训练和验证。

Result: 模型在独立测试集上达到F1分数0.92。消融实验量化了每个时间点和模态的贡献，显示可靠分类很大程度上依赖于放疗剂量图。遮挡式可解释性分析进一步证实模型关注临床相关区域。
...
Method: 提出FutureCAD框架，结合大语言模型(LLM)和B-Rep接地变换器(BRepGround)。LLM生成可执行的CadQuery脚本，并通过文本查询机制用自然语言指定几何选择，BRepGround将这些查询接地到目标基元。采用监督微调建立基本CAD生成能力，然后通过强化学习提高泛化能力。

Result: 实验表明，FutureCAD在CAD生成性能上达到了最先进水平。构建了包含真实世界CAD模型的新数据集用于训练框架。

Conclusion: FutureCAD通过弥合参数化建模与B-Rep合成之间的范式差距，为复杂工业产品设计提供了高质量的AI驱动CAD生成解决方案，实现了文本到CAD的高保真生成。
...
<details>
  <summary>Details</summary>
Motivation: 孔隙材料的数字重建在多个领域至关重要，传统方法如微CT和统计重建已有基础，但深度学习特别是GAN的出现为孔隙介质重建带来了革命性变革，需要系统分析其发展历程和应用现状。
...
Result: 孔隙度精度达到原始样本的1%以内，渗透率预测的平均相对误差减少高达79%，重建体积从最初的64³提升到当前的2,200³体素，为特定应用需求选择合适的GAN架构提供了全面框架。

Conclusion: GAN在孔隙材料重建方面取得了实质性进展，但仍面临计算效率、大规模重建的内存限制以及2D到3D转换中结构连续性等挑战，需要进一步研究解决。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉文本压缩方法（如DeepSeek-OCR）虽然报告了高压缩比，但其评估协议严重依赖下游任务性能。由于多模态大语言模型具有强大的语言先验，这种评估方法无法准确衡量文本保留质量。

Method: 提出了一个新的评估框架，解耦多模态大语言模型的能力以准确评估视觉文本压缩质量。在该框架下，进一步引入了ZeroSense基准，确保测试样本的低语义相关性，消除上下文依赖。

Result: 在多个数据集上的广泛实验表明，视觉文本压缩质量与下游任务准确性存在显著差异，突显了解耦评估框架的必要性。
...
<details>
  <summary>Details</summary>
Motivation: 滚动快门相机普遍存在，但其在运动恢复结构（SfM）中的应用尚未完全解决，需要开发新的几何建模方法

Method: 表征滚动快门单视图几何，分析可恢复的运动和场景参数，系统推导最小重建问题，开发概念验证求解器

Result: 评估多个代表性案例，展示了方法的可行性，同时也指出了实际应用中的局限性

Conclusion: 该方法为滚动快门SfM提供了理论基础和实际解决方案，但实际应用中仍存在限制需要进一步研究
...
TL;DR: 提出Think While Watching框架，解决多模态大语言模型在连续视频流多轮交互中的在线推理问题，通过记忆锚定和并行处理提升性能


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs主要针对离线视频理解，在线推理能力弱，难以处理连续到达的视频流多轮交互。现有流式方法采用交替感知-生成范式，无法并发处理，且随着流增长导致早期记忆衰减，损害长距离依赖建模

Method: 提出Think While Watching框架：1）构建三阶段多轮思维链数据集，采用阶段匹配训练策略；2）通过段级流式因果掩码和流式位置编码确保严格因果性；3）推理时引入高效流水线，重叠观看和思考过程，自适应选择最佳注意力后端

Result: 在单轮和多轮流式输入协议下均取得显著效果：基于Qwen3-VL，在StreamingBench上单轮准确率提升2.6%，在OVO-Bench上提升3.79%；多轮设置下保持性能同时减少56%输出token

Conclusion: Think While Watching框架通过记忆锚定和并行处理机制，有效解决了MLLMs在连续视频流多轮交互中的在线推理问题，在保持性能的同时显著提升了效率
...
<details>
  <summary>Details</summary>
Motivation: 传统基于视频的世界模型依赖序列帧生成和窗口级处理，导致显著延迟。需要一种能够实现低延迟实时空间推理的替代方案，支持交互式世界模拟。
...
Conclusion: 该模型通过帧基架构和空间一致性机制，成功实现了低延迟实时空间推理，为交互式世界模拟提供了实用解决方案。
...
TL;DR: PicoSAM3是一个轻量级可提示视觉分割模型，专为边缘设备和传感器执行优化，在Sony IMX500视觉传感器上实现实时推理，参数量仅130万。


<details>
  <summary>Details</summary>
Motivation: 针对智能眼镜和物联网设备等对延迟敏感和隐私保护要求高的应用，需要能够在设备端实时执行的分割模型，特别是在传感器级别直接处理。

Method: 结合密集CNN架构与感兴趣区域提示编码、高效通道注意力机制，并从SAM2和SAM3进行知识蒸馏，模型参数量仅130万，支持INT8量化。
...
<details>
  <summary>Details</summary>
Motivation: RGB-NIR图像配准在传感器融合、图像增强和越野自主系统中很重要，需要评估现有技术在林业应用中的适用性

Method: 评估了经典和深度学习图像配准技术，包括NeMAR（6种配置训练）和MURF方法，在林业越野数据上进行测试

Result: NeMAR部分成功但GAN损失不稳定影响几何一致性；MURF在大尺度特征对齐方面表现良好但在密集植被细节处理上存在困难

Conclusion: 初步评估显示现有方法需要进一步改进以实现鲁棒的多尺度配准，特别是针对林业越野应用
...
TL;DR: AstroSplat：基于物理的Gaussian splatting框架，用于小行星表面重建和光度特性表征，相比传统球谐参数化方法在渲染和重建精度上有显著提升


<details>
  <summary>Details</summary>
Motivation: 基于图像的表面重建和表征对小行星探测任务至关重要，但现有的Gaussian splatting方法依赖球谐强度参数化，这种纯外观建模方法无法显式建模材料属性或光-表面相互作用

Method: 提出AstroSplat框架，将行星反射模型集成到Gaussian splatting中，实现基于物理的表面重建和光度特性表征

Result: 在NASA黎明号任务拍摄的真实图像上验证，相比传统球谐参数化方法，在渲染性能和表面重建精度方面表现更优
...
<details>
  <summary>Details</summary>
Motivation: 家庭机器人部署面临不可预测的安全风险，现有安全评估方法局限于静态图像、文本或一般危险场景，无法充分评估动态不安全动作检测。需要专门针对家庭场景的安全基准测试。
...
Result: HD-Guard在延迟和性能之间实现了优越的平衡。分析揭示了当前基于VLM的安全检测中的关键瓶颈。HomeSafe-Bench为家庭场景不安全动作检测提供了具有挑战性的评估基准。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型能捕捉多模态动作分布但推理延迟高，流匹配和一致性方法虽能单步生成却会丢失多模态特性，导致动作平均化甚至物理不可行。机器人领域存在离线训练和实时推理的计算预算不对称性，这自然促使我们将迭代细化从推理时转移到训练时。

Method: Ada3Drift学习训练时的漂移场，将预测动作吸引向专家演示模式，同时排斥其他生成样本，实现从3D点云观察的单步高保真生成。针对少样本机器人场景，引入sigmoid调度的损失函数从粗分布学习到模式锐化细化的过渡，以及多尺度场聚合捕捉不同空间粒度的动作模式。

Result: 在三个仿真基准（Adroit、Meta-World和RoboTwin）和真实世界机器人操作任务上的实验表明，Ada3Drift达到最先进的性能，同时比基于扩散的替代方案减少10倍函数评估次数。

Conclusion: 通过将迭代细化从推理时转移到训练时，Ada3Drift成功解决了机器人视觉运动策略中多模态保真度与实时推理之间的权衡问题，实现了高效的单步生成同时保持动作分布的多模态特性。
...
<details>
  <summary>Details</summary>
Motivation: SAR能够实现全球全天候地球观测，但由于不同传感器和区域的成像机制差异导致的域偏移严重阻碍了语义泛化能力，需要解决跨域语义分割的挑战。

Method: 提出CrossEarth-SAR模型，基于新颖的物理引导稀疏专家混合架构，融合物理描述符；构建CrossEarth-SAR-200K大规模数据集用于预训练；建立包含22个子基准的测试套件，覆盖8种不同域差距。

Result: 在20个基准测试中达到最先进水平，在多域迁移任务中某些基准上比先前方法提升超过10% mIoU。

Conclusion: CrossEarth-SAR是首个十亿级SAR视觉基础模型，通过物理引导的稀疏专家混合架构有效解决了跨域语义分割问题，为SAR图像域泛化语义分割建立了统一标准。
...
TL;DR: 提出了一种基于3D空间的全景图拼接方法，通过多视图几何对应和Transformer架构解决传统方法在弱纹理、大视差和重复图案场景下的失真和对齐问题。
...
Method: 将2D对齐任务扩展到3D摄影测量空间，采用基于Transformer的架构实现3D感知和全局信息聚合；直接利用相机姿态指导3D空间的图像扭曲进行全局对齐；采用多特征联合优化策略计算接缝；构建了大规模真实场景数据集用于训练和评估。

Result: 大量实验表明，该方法在对齐精度和感知质量方面显著优于现有替代方案。

Conclusion: 通过将全景拼接扩展到3D空间并利用多视图几何一致性，能够有效解决传统方法在挑战性场景中的失真和对齐问题，显著提升了拼接质量。
...
TL;DR: Nyxus是一个专为大规模2D/3D图像数据设计的可扩展特征提取库，支持CPU/GPU并行计算，提供Python包、命令行工具、Napari插件和容器化部署，适用于生物医学领域。


<details>
  <summary>Details</summary>
Motivation: 现代成像仪器产生TB到PB级数据，传统图像分析算法效率不足，深度学习改进了区域分割但特征提取库分散难以比较性能。需要高效、可扩展、标准化的特征提取工具。

Method: 开发Nyxus库，采用可扩展的核外计算架构，支持2D/3D图像数据，提供全面的生物医学特征集（包括放射组学和细胞分析），支持CPU/GPU并行计算，提供多种访问方式。

Result: Nyxus实现了高效的大规模图像特征提取，经过严格标准测试，提供多领域特征覆盖，支持程序化调优特征集以优化计算效率或覆盖范围，适用于机器学习应用。

Conclusion: Nyxus解决了大规模图像数据处理的计算瓶颈，提供了标准化、可扩展的特征提取解决方案，支持多种用户需求和工作流程，为科学计算和机器学习应用提供了新方法。
...
TL;DR: SeGP-CL提出了一种持续学习视觉语言模型的方法，通过语义几何保持来缓解灾难性遗忘，特别关注新旧语义接口处的脆弱邻域，使用对抗锚点和跨模态几何蒸馏来保持预训练获得的语义结构。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型持续学习方法在适应新任务时，没有明确保持从预训练和先前阶段继承的跨模态语义几何结构，导致新任务监督引起几何失真，特别是在新旧语义接口处的脆弱邻域。

Method: 1. 使用双目标投影梯度下降（DPGD）构建紧凑的对抗锚点集，探测易漂移区域；2. 通过锚点引导的跨模态几何蒸馏（ACGD）保持跨模态结构；3. 通过轻量级文本语义几何正则化（TSGR）稳定跨任务的文本参考框架；4. 训练后估计锚点诱导的原始空间漂移，转移旧视觉原型，并通过融合跨模态和视觉线索进行双路径推理。

Result: 在五个持续学习基准测试上的广泛实验表明，SeGP-CL一致地提高了稳定性和前向迁移能力，实现了最先进的性能，同时更好地保持了视觉语言模型的语义几何结构。

Conclusion: SeGP-CL通过明确保持跨模态语义几何结构，有效缓解了视觉语言模型持续学习中的灾难性遗忘问题，特别是在无示例约束下，通过对抗锚点和几何蒸馏机制保护了新旧语义接口处的脆弱邻域。
...
TL;DR: 提出了一种基于h变换的训练自由引导视觉生成方法，通过修改采样过程中的转移概率，在无需知道前向变换算子或配对数据的情况下实现高质量生成


<details>
  <summary>Details</summary>
Motivation: 现有训练自由方法要么需要知道前向变换算子（如双三次下采样），要么难以在引导和合成质量之间取得平衡，限制了实际应用

Method: 使用h变换约束随机过程，通过向原始微分方程添加漂移函数来修改每个采样时间步的转移概率，并引入噪声感知调度来减少近似误差的影响

Result: 在多种图像和视频生成任务上的实验表明，该方法在保持引导一致性的同时实现了高质量的合成效果，具有良好的泛化能力

Conclusion: 提出的基于h变换的训练自由引导方法有效解决了现有方法的局限性，无需配对数据或前向变换知识，在引导精度和生成质量之间取得了良好平衡
...
Method: 结合显式表示（定向平面基元）和隐式表示（神经渲染）的混合方法，定向平面基元处理几何变形，神经渲染提供精细外观细节。

Result: NBAvatar在LPIPS指标上比基于高斯的方法降低30%（高分辨率兆像素渲染），PSNR和SSIM也有提升，在结构相似性上优于InteractAvatar方法。

Conclusion: NBAvatar通过结合显式和隐式表示，能够有效处理手-脸交互引起的非刚性变形，在渲染质量和几何一致性方面优于现有方法。
...
TL;DR: 提出一个两阶段优化框架，用于多自由移动相机的密集动态场景重建和相机姿态估计，通过时空连接图和宽基线初始化实现鲁棒跟踪，在合成和真实数据集上优于现有方法。
...
Method: 两阶段优化框架：第一阶段通过构建时空连接图扩展单相机视觉SLAM到多相机设置，利用相机内时间连续性和相机间空间重叠，引入宽基线初始化策略；第二阶段通过宽基线光流优化密集相机间和相机内一致性来细化深度和相机姿态。

Result: 在合成和真实世界基准测试中显著优于最先进的前馈模型，同时需要更少的内存。还创建了MultiCamRobolab真实世界数据集，包含运动捕捉系统提供的地面真实姿态。

Conclusion: 提出的两阶段优化框架成功解决了多自由移动相机的密集动态场景重建和相机姿态估计问题，通过解耦跟踪和细化任务，实现了鲁棒性和精度提升。
...
### [70] [Beyond Convolution: A Taxonomy of Structured Operators for Learning-Based Image Processing](https://arxiv.org/abs/2603.12067)
*Simone Cammarasana*

Main category: cs.CV

TL;DR: 本文系统性地对扩展或替代标准卷积的算子进行了分类，提出了五类算子家族，并分析了它们在图像处理任务中的适用性。
...
Method: 提出系统性的算子分类法，将替代标准卷积的算子分为五大家族：(1)基于分解的算子，(2)自适应加权算子，(3)基自适应算子，(4)积分和核算子，(5)基于注意力的算子。对每类算子提供形式化定义、结构特性分析及适用任务讨论。

Result: 建立了完整的算子分类框架，分析了各类算子在线性性、局部性、等变性、计算成本和任务适用性等维度的比较，为不同图像处理任务选择合适的算子提供了指导。

Conclusion: 该分类法为理解和选择替代卷积算子提供了系统框架，指出了该研究领域的开放挑战和未来方向，有助于推动更有效的图像处理架构设计。
...
TL;DR: LoV3D是一个用于训练3D视觉语言模型的管道，能够读取纵向T1加权脑部MRI，生成区域级解剖评估，进行纵向比较，并输出三类诊断（认知正常、轻度认知障碍或痴呆）及诊断摘要。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习工具在脑部MRI分析中存在碎片化问题：分类器将扫描简化为标签，体积测量管道产生无法解释的测量结果，而视觉语言模型可能产生流畅但可能幻觉的结论。需要一种能够提供全面、可解释且减少幻觉风险的纵向脑部MRI分析工具。

Method: LoV3D采用分步管道训练3D视觉语言模型，通过强制标签一致性、纵向连贯性和生物学合理性来减少幻觉风险。训练过程中引入临床加权验证器，基于标准化体积指标的规范参考自动评分候选输出，驱动直接偏好优化而无需人工标注。
...
Conclusion: LoV3D通过分步管道和自动验证机制，显著提高了纵向脑部MRI分析的诊断准确性和可解释性，同时减少了幻觉风险，并在跨站点、扫描仪和人群的测试中表现出高泛化能力。
...
TL;DR: Node-RF结合神经ODE和动态NeRF，实现连续时空表示，能在恒定内存成本下超越观测轨迹进行长期外推预测
...
Conclusion: Node-RF通过连续时空表示克服了现有方法的局限性，能够在恒定内存成本下实现超越观测轨迹的长期外推预测
...
TL;DR: EvoTok是一个统一的多模态图像分词器，通过残差演化过程在共享潜在空间中解决视觉理解与生成之间的粒度差异问题


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型面临视觉理解与生成之间的粒度差异挑战：理解需要高级语义抽象，而图像生成需要细粒度像素级表示。现有方法要么在同一表示上施加两种监督导致干扰，要么在分离特征空间上解耦监督导致不一致。

Method: 提出EvoTok统一图像分词器，通过残差向量量化将图像编码为级联的残差标记序列，形成演化轨迹：早期阶段捕获低级细节，深层阶段逐步过渡到高级语义表示。该方法在共享潜在空间中实现残差演化过程。

Result: 在仅使用1300万图像的小规模数据集训练下，EvoTok在ImageNet-1K 256×256分辨率上达到0.43 rFID的重建质量。与大型语言模型集成后，在9个视觉理解基准中的7个表现良好，在GenEval和GenAI-Bench等图像生成基准上取得显著结果。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于文本到图像模型分数蒸馏的方法存在Janus问题（多面问题）和文本提示不忠实的问题，主要原因是高质量交互数据的稀缺。这限制了3D人-物交互生成在AR、XR和游戏等应用中的效果。
...
Conclusion: Hoi3DGen框架能够精确遵循输入交互描述生成高质量纹理网格，显著提升了3D人-物交互生成的质量和文本一致性，为AR、XR和游戏应用提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视频推理方面表现出色，但在自我中心视频中对人类意图的细粒度理解能力尚未充分探索。现有基准测试主要关注情节级意图推理，忽略了步骤级意图理解，而智能助手、机器人模仿学习和增强现实指导等应用需要理解每个步骤中人物在做什么、为什么做以及下一步计划。

Method: 作者提出了EgoIntent基准测试，包含3,014个步骤，涵盖15个不同的室内外日常生活场景。该基准测试评估模型的三个互补维度：局部意图（What）、全局意图（Why）和下一步计划（Next）。关键设计是每个视频片段在查询步骤的关键结果（如接触或抓取）发生前被截断，不包含后续步骤的帧，避免了未来帧泄漏，实现了对预期步骤理解和下一步计划的干净评估。

Result: 评估了15个多模态大语言模型，包括最先进的闭源和开源模型。即使表现最好的模型在三个意图维度上的平均得分也只有33.31，表明自我中心视频中的步骤级意图理解仍然是一个极具挑战性的问题。

Conclusion: 自我中心视频中的步骤级意图理解是一个尚未解决的重要问题，EgoIntent基准测试为评估和推进该领域研究提供了重要工具，现有模型在该任务上表现不佳，需要进一步研究。
...
TL;DR: 该论文提出CDRL和CA-TTS框架，解决MLLMs中的置信度校准问题，通过置信度驱动的方法提升模型感知敏感性和测试时扩展效果，在四个基准测试上获得8.8%的稳定提升。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型主要关注提升视觉感知准确性，但忽略了"模型是否知道自己不知道"这一关键问题。通过探测实验发现MLLMs存在严重的置信度校准问题，需要解决模型过度自信的问题。

Method: 提出置信度驱动强化学习(CDRL)，使用原始-噪声图像对和基于置信度的奖励来增强感知敏感性并校准模型置信度。进一步提出置信度感知测试时扩展(CA-TTS)，通过置信度信号动态协调自一致性、自反思和视觉自检模块，专家模型扮演多种角色来调度这些模块并提供外部验证。

Result: 提出的集成框架在四个基准测试上建立了新的最先进结果，获得一致的8.8%性能提升。更多的消融研究证明了每个模块的有效性和扩展优势。

Conclusion: 该研究不仅解决了MLLMs中的置信度校准问题，还展示了校准后的置信度能够实现更有效的测试时扩展，为多模态大语言模型的可靠性和性能提升提供了新的解决方案。
...
### [78] [GlyphBanana: Advancing Precise Text Rendering Through Agentic Workflows](https://arxiv.org/abs/2603.12155)
*Zexuan Yan,Jiarui Jin,Yue Ma,Shijian Wang,Jiahui Hu,Wenxiang Jiao,Yuan Lu,Linfeng Zhang*

Main category: cs.CV

TL;DR: GlyphBanana是一个无需训练的文本渲染方法，通过智能工作流和辅助工具将字形模板注入潜在空间和注意力图，显著提升复杂字符和数学公式的生成精度。


<details>
  <summary>Details</summary>
Motivation: 当前生成模型在渲染复杂文本和数学公式时面临挑战，主要原因是遇到分布外提示时模型的指令跟随能力有限。

Method: 采用智能工作流，集成辅助工具将字形模板注入潜在空间和注意力图，通过迭代细化生成图像，无需训练即可应用于多种T2I模型。

Result: 在复杂字符和公式渲染方面优于现有基线，实验证明该方法有效且可泛化到不同T2I模型。

Conclusion: GlyphBanana提供了一种无需训练的高精度文本渲染解决方案，通过智能工作流和工具集成有效解决了复杂文本生成的挑战。
...
Method: 提出LatentGeo框架，学习连续潜在视觉表示来内化辅助几何构造，设计三阶段课程通过辅助视觉监督逐步对齐和内化这些潜在表示，然后使用LaGDPO（潜在感知强化学习）程序在策略优化期间稳定潜在表示并提高最终任务正确性。

Result: 在GeoAux（新提出的针对视觉依赖几何问题的基准）和MathVerse上进行实验，结果显示LatentGeo在几何推理任务上取得实质性增益，特别是在需要辅助构造的任务上。广泛的消融研究验证了框架中每个组件的有效性。

Conclusion: LatentGeo通过连续潜在视觉表示成功解决了多模态大语言模型在辅助几何构造表示方面的挑战，避免了像素级渲染或外部执行器的依赖，实现了端到端优化，显著提升了几何推理能力。
...
TL;DR: BehaviorVLM：一个无需任务特定微调的统一视觉语言框架，通过多阶段推理实现姿态估计和行为理解，大幅减少人工标注需求


<details>
  <summary>Details</summary>
Motivation: 当前动物行为分析严重依赖人工标注或不稳定的无监督流程，限制了可扩展性和可重复性，需要一种轻标注、可扩展的解决方案

Method: 1) 姿态估计：利用量子点标注数据，通过时间、空间和跨视角推理的多阶段管道；2) 行为理解：结合深度嵌入聚类、基于VLM的视频描述和基于LLM的推理来合并和标记行为片段

Result: 框架显著减少人工标注工作量，通过几何检查暴露低置信度标签，可直接从视觉信息进行行为分割而无需关键点，实现可扩展、可解释的轻标注多动物行为分析

Conclusion: BehaviorVLM为神经科学中的动物行为分析提供了一个统一、可扩展的解决方案，通过详细推理步骤指导预训练视觉语言模型，减少对人工标注的依赖，提高可重复性
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在多媒体取证中能够生成可解释的伪造检测理由，但处理高分辨率图像和视频时计算成本高昂。现有的视觉token剪枝方法主要基于语义驱动，会保留显著物体而丢弃背景区域，而伪造痕迹（如高频异常和时间抖动）往往存在于背景区域。

Method: 提出ForensicZip框架，将时间token演化建模为带有松弛虚拟节点的生灭最优传输问题，量化表示瞬时生成伪影的物理不连续性。取证评分进一步整合基于传输的新颖性和高频先验，在大比例压缩下分离取证证据和语义内容。

Result: 在深度伪造和AIGC基准测试中，在仅保留10%token的情况下，ForensicZip实现了2.97倍加速和超过90%的FLOPs减少，同时保持了最先进的检测性能。

Conclusion: ForensicZip通过伪造驱动的token压缩方法，有效解决了现有语义驱动剪枝方法丢弃重要伪造证据的问题，在保持检测性能的同时显著提升了多模态大语言模型在多媒体取证中的计算效率。
...
TL;DR: 提出Verifier元模型，通过评估多个预训练跟踪器的预测可靠性来选择高质量伪标签，用于在无标注视频上进行高效的自训练微调


<details>
  <summary>Details</summary>
Motivation: 长期点跟踪模型通常在合成数据集上训练，但在真实世界视频中性能下降，因为特征差异和缺乏密集标注。自训练是实用解决方案，但伪标签质量依赖于教师模型的可靠性，而教师模型在不同帧和场景中表现不一

Method: 引入Verifier元模型，从多个预训练跟踪器获取候选轨迹，逐帧评估其可靠性，选择最可信的预测生成高质量伪标签轨迹，用于微调

Result: 在四个真实世界基准测试中取得最先进结果，同时比先前自训练方法需要更少数据

Conclusion: Verifier引导的伪标签生成显著提高了监督质量，实现了对无标注视频的数据高效适应，解决了真实世界微调问题
...
<details>
  <summary>Details</summary>
Motivation: 解决无约束视频中表情识别的挑战，包括面部定位不准确、姿态变化大、运动模糊、时间不稳定等因素

Method: 两阶段双模态方法：第一阶段使用DINOv2 ViT-L/14提取视觉特征，配合PadAug数据增强和MoE训练头；第二阶段多尺度重裁剪面部，融合Wav2Vec 2.0音频特征，通过门控融合模块和时序平滑

Result: 在ABAW数据集上，验证集Macro-F1达到0.5368，5折交叉验证为0.5122±0.0277，优于官方基线

Conclusion: 提出的两阶段双模态方法有效提升了无约束视频中表情识别的鲁棒性和准确性
...
TL;DR: SceneAssistant：基于视觉反馈驱动的智能体，用于开放词汇的3D场景生成，通过视觉语言模型进行空间推理和规划，迭代优化场景布局
...
Method: 结合现代3D对象生成模型与视觉语言模型的空间推理和规划能力，为VLM提供一组原子操作（缩放、旋转、聚焦等），通过视觉反馈迭代优化场景布局

Result: 能够生成多样化、开放词汇、高质量的3D场景，定性和定量评估均优于现有方法，同时支持基于自然语言指令编辑现有场景

Conclusion: SceneAssistant通过视觉反馈驱动的智能体方法，成功实现了开放词汇的3D场景生成和编辑，为数字内容创作提供了更灵活的工具
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散模型加速方法（如token合并或下采样）通常只关注在减少计算量下的合成质量优化，而忽视了模型的判别能力。本文旨在同时优化生成质量和分类性能。

Method: 提出BiGain框架，基于频率分离原理，包含两个频率感知算子：1）拉普拉斯门控token合并，鼓励频谱平滑token合并，抑制高对比度token合并以保留边缘和纹理；2）插值-外推KV下采样，通过可控的最近邻和平均池化之间的插值外推来下采样键/值，同时保持查询完整以保持注意力精度。

Result: 在DiT和U-Net架构以及多个数据集（ImageNet-1K、ImageNet-100、Oxford-IIIT Pets、COCO-2017）上，BiGain持续改善了扩散模型分类的速度-精度权衡，同时在可比加速下保持或提升了生成质量。例如，在ImageNet-1K上，对Stable Diffusion 2.0进行70% token合并时，BiGain将分类准确率提高了7.15%，同时将FID改善了0.34（1.85%）。

Conclusion: 平衡的频谱保留（保留高频细节和低频/中频语义）是扩散模型中token压缩的可靠设计原则。BiGain是首个在加速扩散模型下同时研究和推进生成与分类性能的框架，支持更低成本的部署。
...
<details>
  <summary>Details</summary>
Motivation: 扩散变换器（DiTs）存在两个主要问题：1）计算量（FLOPs）与图像分辨率锁定，限制了延迟-质量权衡的灵活性；2）计算均匀分配到所有空间标记，浪费资源在不重要区域。

Method: 引入弹性潜在接口变换器（ELIT），在标准变换器块前插入潜在接口——可学习的变长标记序列。通过轻量级的读写交叉注意力层在空间标记和潜在标记间移动信息，并优先处理重要区域。训练时随机丢弃尾部潜在标记，使模型学习生成重要性排序的表示。
...
TL;DR: FIRM框架通过构建高质量评分数据集和专门奖励模型，解决现有RL图像编辑和生成中奖励模型幻觉和噪声问题，提升生成保真度和指令遵循能力。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习中的奖励模型存在幻觉问题和噪声评分，误导优化过程，需要开发更准确可靠的奖励模型来指导忠实的图像生成和编辑。

Method: 1. 设计定制化数据收集流程构建高质量评分数据集（FIRM-Edit-370K和FIRM-Gen-293K）；2. 训练专门奖励模型（FIRM-Edit-8B和FIRM-Gen-8B）；3. 提出FIRM-Bench基准测试；4. 设计"Base-and-Bonus"奖励策略（CME用于编辑，QMA用于生成）。

Result: FIRM模型在人类判断对齐方面优于现有指标，FIRM-Qwen-Edit和FIRM-SD3.5模型实现显著性能突破，有效缓解幻觉问题，在保真度和指令遵循方面建立新标准。

Conclusion: FIRM框架通过构建高质量数据集、专门奖励模型和创新的奖励策略，为图像编辑和生成提供了准确可靠的RL指导，解决了现有奖励模型的幻觉问题，提升了生成质量。
...
Method: DVD框架通过三个核心设计：1) 将扩散时间步重新用作结构锚点以平衡全局稳定性和高频细节；2) 潜在流形校正(LMR)缓解回归引起的过度平滑，通过微分约束恢复锐利边界和连贯运动；3) 全局仿射相干性作为固有属性限制窗口间发散，实现无需复杂时间对齐的长视频推理。

Result: DVD在多个基准测试中实现了最先进的零样本性能，仅使用领先基线1/163的任务特定数据就成功解锁了视频基础模型中隐含的深度几何先验。

Conclusion: DVD是首个将预训练视频扩散模型确定性地转化为单次深度回归器的框架，解决了视频深度估计的核心挑战，并开源了整个训练套件以促进开源社区发展。
...
### [89] [EndoCoT: Scaling Endogenous Chain-of-Thought Reasoning in Diffusion Models](https://arxiv.org/abs/2603.12252)
*Xuanlang Dai,Yujie Zhou,Long Xing,Jiazi Bu,Xilin Wei,Yuhong Liu,Beichen Zhang,Kai Chen,Yuhang Zang*

Main category: cs.CV

TL;DR: EndoCoT框架通过迭代思维引导和终端思维接地模块，激活MLLMs的推理潜力，为DiT提供渐进式指导，显著提升复杂任务的解决能力


<details>
  <summary>Details</summary>
Motivation: 当前MLLMs作为文本编码器集成到扩散框架中存在两个关键限制：1) MLLMs文本编码器推理深度不足，单步编码无法激活思维链过程；2) 解码过程中指导保持不变，阻碍了DiT将复杂指令逐步分解为可执行的去噪步骤

Method: 提出Endogenous Chain-of-Thought (EndoCoT)框架：1) 迭代思维引导模块通过迭代细化潜在思维状态激活MLLMs的推理潜力；2) 终端思维接地模块通过将最终状态与真实答案对齐，确保推理轨迹基于文本监督；3) 将这些状态桥接到DiT的去噪过程中

Result: 在多个基准测试（迷宫、TSP、VSP、数独）上平均准确率达到92.1%，比最强基线高出8.3个百分点

Conclusion: EndoCoT框架通过激活MLLMs的推理潜力并提供渐进式指导，使DiT能够以逐步方式解决复杂任务，显著提升了多模态大语言模型在扩散框架中的表现
...
TL;DR: AutoGaze是一个轻量级模块，通过自动选择多尺度视频补丁来消除时空冗余，显著减少视觉token数量并加速视频理解模型处理长高分辨率视频的能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在处理长高分辨率视频时面临挑战，因为它们对每个像素都平等处理，忽略了视频中存在的显著时空冗余，导致计算效率低下。

Method: AutoGaze通过自回归方式选择最小化的多尺度补丁集，在用户指定的误差阈值内重建视频，使用下一个token预测和强化学习进行训练，在ViT或MLLM处理前移除冗余补丁。

Result: AutoGaze将视觉token减少4-100倍，加速ViT和MLLM高达19倍，使MLLM能够扩展到1000帧4K分辨率视频，在VideoMME基准上达到67.0%的优异性能。在HLVid基准上比基线提升10.1%，比之前最佳MLLM提升4.5%。
...
TL;DR: Spatial-TTT：基于测试时训练的流式视觉空间智能方法，通过动态调整参数来从长时视频流中捕获和组织空间证据，实现高效的空间理解。


<details>
  <summary>Details</summary>
Motivation: 人类通过连续的视觉观察来感知和理解现实空间，因此需要能够从潜在无限长的视频流中持续维护和更新空间证据的能力。核心挑战不仅在于处理更长的上下文窗口，更在于如何选择、组织和长期保留空间信息。

Method: 提出Spatial-TTT方法，采用测试时训练机制，动态调整部分参数来捕获和组织长时场景视频中的空间证据。设计了混合架构，采用大块更新与滑动窗口注意力并行的高效空间视频处理。引入基于3D时空卷积的空间预测机制，增强模型对跨帧几何对应性和时间连续性的捕捉。构建了带有密集3D空间描述的数据集，指导模型以结构化方式更新参数以记忆和组织全局3D空间信号。

Result: 大量实验表明，Spatial-TTT显著提升了长时空间理解能力，在视频空间基准测试中达到了最先进的性能水平。

Conclusion: Spatial-TTT通过测试时训练机制有效解决了流式视觉空间智能中的信息组织和保留问题，为从长时视频流中理解和维护空间结构提供了创新解决方案。
...
Method: 采用渐进式两阶段训练范式：第一阶段整合主体外观、全局运动、局部动态和相机运动等综合控制信号，引入条件感知3D旋转位置嵌入协调异构输入，采用分层运动注入策略增强全局运动引导，并使用组和角色嵌入解决多主体模糊性；第二阶段设计潜在身份奖励反馈学习范式，训练潜在身份奖励模型，在潜在空间提供运动感知的身份奖励。

Result: 在自建的大规模数据集和综合的DreamOmni基准测试中，DreamVideo-Omni在生成高质量视频和精确可控性方面表现出优越性能。

Conclusion: DreamVideo-Omni通过统一框架实现了和谐的多主体定制和全方位运动控制，有效解决了现有方法的局限性，在视频合成领域取得了显著进展。
...
Method: 提出VST范式，支持"边看边思考"机制，在视频流播放过程中激活推理。开发了完整的后训练流程：VST-SFT用于结构适配离线VideoLLM到因果流式推理，VST-RL通过多轮视频交互环境中的自我探索进行端到端改进。还设计了自动训练数据合成管道，利用视频知识图谱生成高质量的流式问答对。

Result: VST-7B在在线基准测试中表现强劲：StreamingBench上达到79.5%，OVO-Bench上达到59.3%。在离线长格式或推理基准测试中保持竞争力。相比Video-R1，VST响应速度快15.7倍，在VideoHolmes上提升+5.4%，展示了更高的效率和跨多样化视频理解任务的强泛化能力。

Conclusion: VST范式通过"边看边思考"机制有效解决了在线视频理解中实时响应性与深度推理之间的权衡问题，在保持实时响应的同时提高了理解质量和认知连贯性，为在线视频大语言模型提供了新的解决方案。
...
Method: 构建了包含10个学术领域520个样本的GRADE基准，提出了联合评估学科推理、视觉一致性和逻辑可读性的多维度评估协议，对20个最先进的开源和闭源模型进行了广泛实验。

Result: 实验显示当前模型在隐式、知识密集型编辑设置下存在显著局限性，性能差距较大。通过严格分析和消融实验揭示了模型缺陷和学科编辑中的约束。
...
<details>
  <summary>Details</summary>
Motivation: 传统视频分词器在不同视频的时间块上采用均匀token分配，导致在简单、静态或重复片段上浪费token，而在动态或复杂片段上分配不足。这种低效分配需要改进。
...
Conclusion: EVATok通过自适应token分配解决了传统视频分词器的效率问题，在保持高质量重建的同时显著降低了计算成本，为视频生成任务提供了更高效的解决方案。
